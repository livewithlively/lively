...
//  v6 knowledge 는 category(repo-free)로만 연결돼 repo 직접참조 없음). 따라서 repo 쓰기는 단일
...
// repo 이름변경 — repo 는 도메인맵 자기완결이라 물리 이름변경 안전(soft-alias 불요 — v6 knowledge 는
//  category(repo-free)로만 연결돼 repo 행 PK 참조가 없다). 단 보호 리포는
...
//  cross-DB cascade 없음(v6 knowledge 는 category(repo-free)로만 연결돼 repo PK 참조 없음). 따라서
//  repo cascade 는 단일 DB(domainmap) 안에서 안전. 기본 가드:
...
export async function hardDeleteRepo(name: string, force: boolean, actor: Actor): Promise<RepoDeleteResult> {
  ...
  const rid = ex.id;
  // v6: category 는 repo-free(레포 삭제와 무관·생존). 레포 삭제는 코드측(code_unit/data_entity/mapping/debt)만 정리.
  ...
  if (!force && (code_units > 0 || data_entities > 0)) {
    return { blocked: true, id: rid, name: nm, refs: { code_units, data_entities } };
  }
  return withTx(async (client) => {
    const del = async (sql: string): Promise<number> => (await client.query(sql, [rid])).rowCount ?? 0;
    // 자식 먼저(참조 순서) — domainmap 엔 FK 가 없어 순서 무관하나 의미상 leaf→root. category 는 repo-free라 미삭제.
    ...
    await logChange(client, {
      ...
      note: `hard-delete repo '${nm}' (cascade code=${code}, entities=${entities}, mappings=${mappings})`,
    });
    return {
      deleted: true, id: rid, name: nm,
      removed: { code_units: code, data_entities: entities, mappings, debts },
    };
  });
}
