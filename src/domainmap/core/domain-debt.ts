...
//
// 비파괴·멱등: upsertDebtRow(title 키 upsert; 사람이 닫은 resolved/dismissed/ack 은 재개 안 함).
...
// 도메인별 active/removed 매핑 코드 카운트 읽기(repo 스코프). rejected 매핑 제외(질의 술어는 queries.ts
...
export async function evaluateDomainStructureDebt(
  ...
  const counts = await readDomainStructureCounts(pool, r.id);
  const findings = classifyDomainDebt(counts);
  const tally: Record<string, number> = {};
  const bump = (k: string) => { tally[k] = (tally[k] ?? 0) + 1; };
  for (const f of findings) {
    const action = await upsertDebtRow(pool, r.id, null, actor, {
      kind: "domain_debt", title: f.title, detail: f.detail, cited_refs: [],
    });
