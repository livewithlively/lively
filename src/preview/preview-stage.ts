...
export interface StageResult { worktree_path: string; merge_status: Record<string, string>; conflicts: string[]; failures: Record<string, string>; head: string | null; inputs: StageInputs; }
...
export async function ensureStageWorktree(id: string, repo: string, baseRef: string, branches: string[]): Promise<StageResult> {
  ...
  return { worktree_path: wt, merge_status, conflicts, failures, head, inputs };
}
