...
import { absoluteUrl } from "../gateway-url.js";
...
// 정규 주소(#1169) — 모드에 무관하게 `/preview/<id>/ui/` 다(preview/routes 가 루트 진입을 여기로 302 한다).
//  **절대 URL** 로 돌려준다: 에이전트가 이 값을 사람에게 그대로 전달하면 바로 클릭된다. 예전엔 상대경로만 줘서
//  에이전트가 게이트웨이 주소를 따로 캐거나(org_overview 덤프) 못 누르는 링크를 보냈다.
//  게이트웨이 주소 미설정이면 상대경로로 폴백한다(링크가 없어지는 것보다 낫다 — gateway-url.ts).
const previewPath = (id: string): string => "/preview/" + id + "/ui/";
