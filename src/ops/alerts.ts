...
//  ③ **로그·감사에 URL 을 넣지 않는다.** `redactDeep`(src/org/ingest/redact.ts)은 URL 패턴을 안 잡는다 → 스냅샷에 넣으면 그대로 샌다.
...
//  부정한다 — src/net/mcp-ssrf-fetch.ts 머리주석: "org_mcp_server.url 은 **관리자 입력(길이검증만)** → 사설/메타데이터
...
import { getMemberSecret, setMemberSecret, deleteMemberSecret, GATEWAY_OWNER } from "../org/credentials/member-secret-store.js";
