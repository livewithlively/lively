...
export async function pushOutbox(opts?: { limit?: number }): Promise<{ pushed: number; deferred: number; failed: number; deleted: number; scanned: number }> {
  ...
  const rows: Array<{ id: number; entity_id: number; op: string; ext_id_snapshot: string | null; close_note: CloseNote | null }> = await q(itemsPool,
    `SELECT id, entity_id, op, ext_id_snapshot, close_note FROM external_outbox
  ...
  for (const ob of rows) {
    try {
      ...
      if (!p.external_id && isRestrictedForPublish(p.id)) {
        await markDone(ob.id); deferred++; continue;
      }
