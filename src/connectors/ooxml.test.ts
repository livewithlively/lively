...
t("docx: 한글+영문 본문 추출·문단 개행·엔티티 디코드", () => {
  const doc = `<?xml version="1.0"?><w:document><w:body>`
    ...
    + `<w:p><w:r><w:t xml:space="preserve">어니스트AI &amp; 투자자</w:t></w:r></w:p>`
    + `</w:body></w:document>`;
...
t("pptx: 슬라이드 텍스트 순서 추출(수치 정렬)", () => {
  ...
  const s2 = `<p:sld><a:t>어니스트 파일럿</a:t></p:sld>`;
...
t("xlsx: sharedStrings 인덱스 해소·숫자 리터럴·행/셀 구분", () => {
  const sst = `<sst><si><t>항목</t></si><si><t>투자금액</t></si><si><t>어니스트AI</t></si></sst>`;
