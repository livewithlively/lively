...
import { orgTimezone } from "../org/timezone.js";
...
// 통합 DB 전역 풀 — 생성부는 db/client.ts 로 이동(#1313 R10, pg 외 무의존 leaf). 여기 재수출은
// 구 소비자·dist 참조 스크립트 호환 shim — 신규 코드는 db/client.js 를 직결한다.
...
export async function uiStats(): Promise<UiStats> {
  ...
  //  #1313 R10: 풀 생성부가 db/client.ts(leaf)로 내려가 org/timezone→items/store 정적 의존이 사라짐 → 지연 import 를 정적으로 환원.
