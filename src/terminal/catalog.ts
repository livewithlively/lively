...
export interface SessionInfo {
  ...
  managed?: string;
  // #2544 — false = 이 행은 **관측이 아니라 기억**이다: 매니지드 중계가 tmux 를 «못 본» 틱에 DB desired 행으로 채운 것.
  //  attached·agentState 는 기본값(오프라인)이고 restorable 도 아니다(«모른다» 를 «죽었다» 로 그리면 열기=복원이 잘못 발동한다).
  //  관측된 행(tmux 에서 읽은 것)은 이 필드가 없다.
  observed?: boolean;
