 ...
 *   codex 는 커서가 `›` 이고 꼬리가 "Press enter to confirm or esc to go back" 이라 대화상자 **셋 중 둘**(훅 검토·
 *   업데이트 알림)이 통째로 안 잡혔다 — 답을 기다리는 세션이 목록에서 «대기중» 으로 서고, 「지금 볼 것」 에도
 *   안 올랐다(실측 2026-09-24, #4135). 하네스가 답할 수 있으면 그 답을 쓴다.
