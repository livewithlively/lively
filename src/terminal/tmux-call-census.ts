...
//
// ── 왜 필요한가 ───────────────────────────────────────────────────────────────
// 이 프로젝트의 완료 조건 하나가 **«게이트웨이가 그 테넌트에 tmux 를 부른 횟수 0»** 인데, 이 계기 전엔 그걸 **잴 수단이
//  없었다.** 당시의 그림자 대조(d3 — #2600 T3-a 로 걷었다)는 **불일치와 첫 건에만 동사를 싣고** 일치한 호출은 개별 로그가
//  없었다 — 그래서 그 로그로 «무슨 동사가 남았나» 를 세면 **불일치만 세게 된다**(2026-09-08 실측: 그렇게 세고 «list-sessions
//  0건» 이라 결론했는데, 같은 창의 요약은 compared 100 이었다. 31% 줄었을 뿐인데 «닫혔다» 고 읽었다).
//  게다가 그림자는 표본(25%)·동시상한에 묶여 **전량이 아니었다**. 완료 조건을 재려면 표본이 아니라 **전수**가,
//  그리고 route 모드(off/on)와 **무관하게** 도는 계수가 필요하다.
//
// ── 무엇을 세나 ───────────────────────────────────────────────────────────────
 ...
 *
 *  ── 규율 ──
 *  · **seam 자신의 배관은 건너뛴다** — 이름이 `tmux` 로 시작하는 모듈(`tmux-exec`·`tmux-route`·
