...
//   사람이 `lively init` 으로 명시한 바인딩(abs_path)이 있으면 그것이 먼저다.
...
/** 이 노드에서의 프로젝트 폴더 절대경로 — 명시 바인딩 > «세션이 그 프로젝트 폴더에서 돈다» 인 행의 dir > null(슬롯 조립은 훅 몫). */
export function folderAbsFromRow(explicit: string | null | undefined, folder: string, row: SessionRowLike | null | undefined): string | null {
