...
//  (상태 push 마다) 실패 로그를 찍어 노드 로그를 채웠다(하루 9천 줄, 실 오류를 가림).
//  #2599 T2 — 판별자는 실행 토폴로지 하나다(종전엔 6곳이 각자 env 를 읽었고, 그중 넷은 모듈 로드에 얼고 둘은
//  호출 시점에 읽어 **같은 질문에 두 답이 나오는 창**이 있었다).
...
export async function loadDesiredOne(id: string): Promise<SessionState | undefined> {
  if (onNode()) return undefined;   // 노드: tmux 가 유일한 진실(아래 resolveSessionDir 주석)
  try {
...
export async function loadDesiredMap(ids: string[]): Promise<Map<string, SessionState>> {
  ...
  if (!ids.length || onNode()) return out;   // 노드: 전부 tmux 폴백(조용히)
  try {
