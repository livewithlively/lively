...
// ── 매치 불가 표현식(2월 30일) → null. 366일 스캔 상한이 무한루프를 막는다는 보증. ──
{
  ...
  // 이 상한은 **성능 회귀 탐지선이 아니라 무한루프 방지선**이다 — 상한 없는 스캔은 10초가 아니라
  //  영원히 안 끝나므로, 넉넉히 잡아도 보증(366일에서 반드시 멈춘다)은 그대로다. 반대로 값을 조이면
  //  머신 성능에 매인 단언이 되어 느린 CPU 에서 거짓 실패한다: 실측 2026-09-08, CPU 제한 컨테이너
  //  (CI 러너, node:22 arm64)에서 이 파일 전체가 11.3 초 걸려 종전 10초 단언이 깨졌다.
  //  전체 실행시간 상한은 러너의 파일당 예산(run-tests --budget)이 더 넓게 지킨다.
  assert.ok(Date.now() - t0 < 60_000, "366일 스캔 상한이 있어 유한 시간에 끝난다(무한루프 아님)");
}
