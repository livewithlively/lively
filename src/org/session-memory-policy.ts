...
//
// 왜 필요(#1059): 어니스트 박스가 claude 세션 누적 baseline(~8GB) + Ollama 임베딩 3.3GB 스파이크로 스왑0 물리
...
//  직행한다(terminal-isolation.wrapAsMember 의 cap-gated 분기). 운영자가 캡을 걸어야(예: 어니스트 16GB → high 3072
...
//  src/org/storage-policy.ts(같은 seam 의 원형).
...
function envSeed(): SessionMemoryPolicyPatch {
  const p: SessionMemoryPolicyPatch = {};
  if (process.env.LIVELY_SESSION_MEM_HIGH_MB) p.per_session_high_mb = clampInt(process.env.LIVELY_SESSION_MEM_HIGH_MB, SESSION_MEM_MB_MIN, SESSION_MEM_MB_MAX, DEFAULT_SESSION_MEMORY_POLICY.per_session_high_mb);
  if (process.env.LIVELY_SESSION_MEM_MAX_MB) p.per_session_max_mb = clampInt(process.env.LIVELY_SESSION_MEM_MAX_MB, SESSION_MEM_MB_MIN, SESSION_MEM_MB_MAX, DEFAULT_SESSION_MEMORY_POLICY.per_session_max_mb);
  return p;
}
...
export function normalizeSessionMemoryPolicy(raw: unknown): SessionMemoryPolicy {
  ...
  const base = { ...DEFAULT_SESSION_MEMORY_POLICY, ...envSeed() }; // DB 값이 없는 항목은 env 시드 → 기본값 순
  const out: SessionMemoryPolicy = {
    per_session_high_mb: r.per_session_high_mb !== undefined ? clampInt(r.per_session_high_mb, SESSION_MEM_MB_MIN, SESSION_MEM_MB_MAX, base.per_session_high_mb) : base.per_session_high_mb,
    per_session_max_mb: r.per_session_max_mb !== undefined ? clampInt(r.per_session_max_mb, SESSION_MEM_MB_MIN, SESSION_MEM_MB_MAX, base.per_session_max_mb) : base.per_session_max_mb,
  };
...
export function resolveSessionMemoryPolicy(dbRaw: unknown): SessionMemoryPolicy {
  return normalizeSessionMemoryPolicy(dbRaw);
}
