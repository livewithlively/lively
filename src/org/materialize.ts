...
export const DEFAULT_CONTEXT_ONTOLOGY_GUIDE = [
  ...
  "- **⚠ 사람의 초기 맥락은 라이블리 밖에 있을 수 있다** — 노션·클릭업·슬랙·메일에 먼저 쓰고 여기로 흘러온다. 그러니 \"라이블리에 없다 = 조직에 없다\"가 아니다. 커넥터가 덮는 범위는 위 경로로 이미 들어와 있으니 **먼저 SoT를 찾고**, 거기 없을 때만 외부를 직접 본다(`ext__*` 프록시 툴 — 개인·미공유 페이지, 커넥터 없는 툴, 방금 쓴 것). **끌어온 건 반드시 남긴다 → 아래 기록.**",
  "",
  ...
  "- **외부에서 끌어온 건 남긴다** — `ext__*` 프록시나 외부 MCP 로 읽은 내용은 **세션이 끝나면 증발한다**(게이트웨이는 무엇을 읽었는지 안 남긴다 — 호출 로그에 응답이 없다). 그러니 조직에 남을 값이면: 미러가 이미 갖고 있으면(먼저 검색해 확인) **중복 저장 말고**, SoT에 없으면 `source_save`로 **원문**을 남긴다(외부 좌표 `external_*` 포함 — 나중에 미러와 이어진다). 파생 인사이트는 그와 별개로 `knowledge_save`.",
  "- **외부에 쓸 때도 SoT가 먼저다** — 과업은 `ext__clickup__*`이 아니라 `project_create_v6`로 만든다(라이블리에 남고, 아웃박스가 클릭업에 껍데기를 만들어 링크백한다). 외부에 직접 쓰면 그 맥락은 SoT에 안 남는다. 투영 경로가 없는 곳(노션 등)에서만 직접 쓰고, 그 땐 위 규칙대로 남긴다.",
  "",
  "**원칙**",
