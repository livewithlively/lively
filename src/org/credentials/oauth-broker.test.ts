...
// 실행: npm run build && node dist/org/credentials/oauth-broker.test.js
...
t("verifyState: 페이로드 위변조 → 서명 불일치 throw", () => {
  ...
  // mac 변조는 '반드시 달라지는' 방식으로 — 종전의 `slice(0,-2)+"AA"` 는 mac 이 우연히 "AA" 로 끝나면(HMAC 이
  //  매 실행 e(만료초)에 따라 달라지고, canonical base64url 43자의 끝 2자가 "AA" 일 확률 ≈ 1/1024) 변조가
  //  무변조가 되어 throw 가 안 일어났다(전체 체인에서 간헐 red — 2026-07-31 실측·결정적 재현). 첫 글자 플립은
  //  디코드 시 바이트 0 을 항상 바꾸므로 어떤 mac 에서도 확정적으로 불일치한다.
  const tamperedMac = (mac[0] === "A" ? "B" : "A") + mac.slice(1);
  throws(() => verifyState(`${body}.${tamperedMac}`), /서명 불일치/); // mac 변조(결정적)
});
