...
// 실행: npm run build && node dist/org/credentials/member-secret-store.test.js
...
t("normalizeScopeKey: 빈값(단일) 허용, host/account 형태 통과", () => {
  ...
  assert.equal(normalizeScopeKey("git.example.com"), "git.example.com");
  assert.equal(normalizeScopeKey("123456789012:ap-northeast-2"), "123456789012:ap-northeast-2");
});
