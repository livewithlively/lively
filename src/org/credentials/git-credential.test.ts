...
//  DB 없이 도는 순수 로직만(CRUD 는 박스 DB 통합검증). 실행: npm run build && node dist/org/credentials/git-credential.test.js
...
const { hostOf, prepareGitAuth, isAuthError } = await import("../../project/project-provision.js");
const { safeHost, buildSshConfigBlock, buildGitCredLines } = await import("./git-credential-materialize.js");
const { sanitizeCloneUrl } = await import("../../domainmap/core/url-util.js");
...
// ── sanitizeCloneUrl: http(s) 자격 제거하되 ssh:// 의 로그인 유저(git@)는 보존(#522 버그수정) ──
{
  ...
  assert.equal(sanitizeCloneUrl("https://user:pass@git.example.com/g/r.git"), "https://git.example.com/g/r.git");
  // scp-식 ssh: 조기통과(임베드 시크릿 없음) — git@ 보존
  assert.equal(sanitizeCloneUrl("git@git.example.com:acme-dev/backend/example-one.git"), "git@git.example.com:acme-dev/backend/example-one.git");
  // ssh:// : 로그인 유저(git)를 벗기면 로컬유저로 접속해 실패 → 반드시 보존(회귀 방지 핵심)
  assert.equal(sanitizeCloneUrl("ssh://git@git.example.com/acme-dev/backend/example-one.git"), "ssh://git@git.example.com/acme-dev/backend/example-one.git");
...
{
  assert.equal(isAuthError("fatal: could not read Username for 'https://git.example.com': terminal prompts disabled"), true);
  assert.equal(isAuthError("remote: HTTP Basic: Access denied\nfatal: Authentication failed for 'https://git.example.com/g/r.git/'"), true);
  assert.equal(isAuthError("git@git.example.com: Permission denied (publickey).\nfatal: Could not read from remote repository."), true);
...
// ── describeGitError: 관측성 안전요약 — 원격 URL·토큰 절대 비노출, fs 코드·timeout 만 남긴다(#606) ──
{
  ...
  const secretUrl = "https://x-access-token:SECRET123@git.example.com/g/r.git";
  ...
  assert.equal(d1.includes("git.example.com"), false, "원격 URL 이 새면 안 됨");
  assert.ok(d1.includes("<repo>") || d1.includes("<url>"));
  // scp 형도 스크럽
  assert.equal(describeGitError("git@git.example.com:g/r.git: Permission denied").includes("example"), false);
...
//   worker 구분이 우회되지 않는다. (누가 스케줄러에서 GATEWAY_OWNER 를 직접 집으면 그 구분이 새므로 걸린다.)
{
  ...
  const gate = stripComments(readFileSync(new URL("../../project/project-provision.js", import.meta.url), "utf8"));
