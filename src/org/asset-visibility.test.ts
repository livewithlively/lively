...
// ── targetsMember (관리자 정책 기본값) ──
{
  ...
  assert.equal(targetsMember(["alice"], "yoon"), false); // 지정 + 미포함
...
// ── effectiveVisible: 개인 오버라이드 우선 ──
{
  ...
  assert.equal(ev(true, ["alice"], "yoon", true), true);
...
// ── effectiveVisible: 오버라이드 없으면 정책 기본값 ──
{
  ...
  assert.equal(ev(true, ["alice"], "yoon", null), false); // 카나리 밖
...
// ── 카나리 롤아웃 시나리오(문서화) — #654 훅 롤아웃이 실제로 이러길 원함 ──
{
  ...
  assert.equal(ev(true, target, "alice", null), false); // 나머지는 안 받고
  // 카나리 검증 후 전원 확대(target=null)
  assert.equal(ev(true, null, "alice", null), true);
  // 확대 후 한 명이 본인만 opt-out
  assert.equal(ev(true, null, "alice", false), false);
