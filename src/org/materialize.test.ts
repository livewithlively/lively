...
import { buildKnowledgeIndex, WRITE_GUIDE_BLOCK, type AreaMapEntry } from "./materialize.js";
...
const area = (over: Partial<AreaMapEntry>): AreaMapEntry =>
...
t("v4-a: R 규칙은 전문(body_md)이 인덱스에 주입된다(enforced)", () => {
  const idx = buildKnowledgeIndex([
    unit({ name: "rule-1", kind: "R", title: "푸시 전 빌드·린트", body_md: "push 전 반드시 빌드 + 린트 확인. 깨지면 push 금지." }),
  ]);
...
t("v4-b: K/H/W 는 정적 주입 안 됨(제목리스트 폐기 — name 도 미노출)", () => {
  const idx = buildKnowledgeIndex([
    unit({ name: "k1", kind: "K", title: "어떤 지식", body_md: "지식 본문" }),
    unit({ name: "h1", kind: "H", title: "어떤 절차", body_md: "절차 본문" }),
    unit({ name: "w1", kind: "W", title: "어떤 작업", body_md: "작업 본문" }),
  ]);
...
t("v4-c: observed 제외 필터 폐기 — 주입은 kind 가 결정(observed R 주입·observed K 미주입은 K 라서)", () => {
  const idx = buildKnowledgeIndex([
    unit({ name: "obs-rule", kind: "R", title: "수집된 규칙", body_md: "observed 규칙 전문", confidence: "observed" }),
    unit({ name: "obs-k", kind: "K", title: "수집된 지식", body_md: "observed 지식 본문", confidence: "observed" }),
  ]);
...
t("v4-d: area 지도가 space 별로 key — name (N) 렌더(product→business 순)", () => {
  const idx = buildKnowledgeIndex([], [
    area({ space: "product", key: "agent-gateway", name: "에이전트 게이트웨이", active_units: 8 }),
    area({ space: "business", key: "gtm", name: "GTM", active_units: 16 }),
    area({ space: "product", key: "audit-history", name: "감사 이력", active_units: 0 }),
  ]);
  assert.ok(idx.includes("## area 지도 (주제 — 검색으로 소환)"), "area 지도 헤더");
...
t("v4-f: 중요도 캡 폐기 — R 50개 초과여도 전부 주입(잘림·'외 N건' 없음)", () => {
  const many = Array.from({ length: 60 }, (_, i) =>
    unit({ name: `rule-${i}`, kind: "R", title: `규칙${i}`, body_md: `규칙${i} 전문` }));
...
t("v4-g: R 전문의 OpenAI 키가 [REDACTED] 로 마스킹", () => {
  const idx = buildKnowledgeIndex([
    unit({ name: "leaky", kind: "R", title: "정상 제목", body_md: "키 sk-ABCDEFGHIJKLMNOP1234 가 본문에 노출" }),
  ]);
...
t("v4-g: R 제목·area 표시명의 라이블리 토큰이 마스킹", () => {
  const idx = buildKnowledgeIndex(
    [unit({ name: "r2", kind: "R", title: "제목에 lvk_abcdefghijklmnopqrstuvwxyz 섞임", body_md: "본문" })],
    [area({ space: "product", key: "k", name: "area 이름 lvk_zyxwvutsrqponmlkjihgfedcba" })],
  );
...
t("v4-g: 시크릿 없는 정상 입력은 마스킹 없이 원문 유지", () => {
  const idx = buildKnowledgeIndex(
    [unit({ name: "clean", kind: "R", title: "깨끗한 규칙", body_md: "평범한 규칙 전문" })],
    [area({ space: "product", key: "billing", name: "결제" })],
  );
  assert.ok(!idx.includes("[REDACTED]"));
...
t("v4-h: lifecycle!=active 인 R 은 전문 주입에서 제외", () => {
  const idx = buildKnowledgeIndex([
    unit({ name: "live", kind: "R", title: "활성 규칙", body_md: "활성 규칙 전문", lifecycle: "active" }),
    unit({ name: "dead", kind: "R", title: "폐기 규칙", body_md: "폐기 규칙 전문", lifecycle: "superseded" }),
  ]);
...
t("v4-i: 멱등 — 같은 입력 2회 호출 byte-identical(중복 누적 0)", () => {
  const units = [
    unit({ name: "r1", kind: "R", title: "A", body_md: "A 전문", sort: 0 }),
    unit({ name: "r2", kind: "R", title: "B", body_md: "B 전문", sort: 1 }),
  ];
  const amap = [area({ space: "product", key: "k1", name: "n1", active_units: 3 })];
  ...
  assert.equal((a.match(/## area 지도/g) ?? []).length, 1, "area 지도 헤더 1개");
...
t("v4-j: R 은 sort→name 안정 순서로 전문 주입", () => {
  const idx = buildKnowledgeIndex([
    unit({ name: "z", kind: "R", title: "Z규칙", body_md: "Z", sort: 0 }),
    unit({ name: "a", kind: "R", title: "A규칙", body_md: "A", sort: 0 }),
    unit({ name: "m", kind: "R", title: "M규칙", body_md: "M", sort: -1 }),
  ]);
