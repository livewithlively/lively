...
t("H1-b: 요약(첫 본문줄)의 라이블리 토큰이 마스킹", () => {
  const idx = buildKnowledgeIndex([
    unit({ name: "doc", kind: "K", title: "정상 제목", body_md: "lvk_abcdefghijklmnopqrstuvwxyz 가 본문에" }), // V4-P2a: F→K
  ]);
...
// ── recalled kind 필터(V4-P2a 4종 R/K/H/W): recalled=K/H 만 섹션화, W(query)·R(enforced)은 인덱스 제외 ──
t("recalled kind 필터: W(작업)·R 은 인덱스에서 제외, K/H 만 포함", () => {
  const idx = buildKnowledgeIndex([
  ...
  assert.ok(idx.includes("ctx_cat name=k1"), "K 는 포함(recalled)");
  assert.ok(idx.includes("ctx_cat name=h1"), "H 는 포함(recalled)");
...
// ── H2(P-V3-3a): confidence='observed'(커넥터 수집물) 는 recalled kind(K/H) 여도 인덱스에서 영구 제외 ──
//  V4-P2a: recalled = K/H(4종 narrow). observed 가 recalled kind 라도 제외됨을 K·H 둘로 증명.
t("H2: observed 는 recalled kind(K/H) 여도 주입 인덱스에서 영구 제외", () => {
  const idx = buildKnowledgeIndex([
    unit({ name: "curated", kind: "K", title: "큐레이션 지식", confidence: "human" }),
    unit({ name: "obsK", kind: "K", title: "수집된 K", confidence: "observed" }),
    unit({ name: "obsH", kind: "H", title: "수집된 H", confidence: "observed", domain_key: "billing" }),
  ]);
  assert.ok(idx.includes("ctx_cat name=curated"), "큐레이션 K 는 포함");
  assert.ok(!idx.includes("ctx_cat name=obsK"), "observed K 는 인덱스 제외");
  assert.ok(!idx.includes("ctx_cat name=obsH"), "observed H(recalled)도 인덱스 제외");
});
...
t("멱등: 같은 입력 2회 호출 → 동일 출력(중복 누적 0)", () => {
  const units = [
    ...
    unit({ name: "h1", kind: "H", title: "B", updated_at: new Date("2026-06-11").toISOString() }), // V4-P2a: D→H
  ];
...
t("recalledKinds 인자가 섹션/순서/라벨을 결정한다(레지스트리 주입)", () => {
  const units = [
    unit({ name: "k1", kind: "K", title: "지식" }),
    unit({ name: "w1", kind: "W", title: "작업" }),
    unit({ name: "h1", kind: "H", title: "절차" }),
  ];
  // 레지스트리가 W 만 recalled 라고 말하면(가상 정책) W 섹션만 나오고 K/H 는 제외돼야 한다(상수 하드코딩 아님 증명).
  //  V4-P2a: 인자 주입 메커니즘 검증이라 임의 kind 로 정책을 흉내 — W 를 가상 recalled 로 줘 인자가 권위임을 증명.
  const idx = buildKnowledgeIndex(units, [{ kind: "W", label: "작업섹션" }]);
  assert.ok(idx.includes("## 작업섹션 (W)"), "인자로 준 라벨/순서로 섹션 출력");
  assert.ok(idx.includes("ctx_cat name=w1"), "W 는 포함");
