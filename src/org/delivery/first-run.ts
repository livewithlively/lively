...
//   "시도때도없이 떠서 돌아버리겠어"). 보여준 사실 자체를 서버에 남겨 그 고리를 끊는다. 못 끝낸 사람은
//   버리지 않고 홈의 «처음 설정 이어서 하기»(welcomePending)로 안내한다 — 자동으로 끌고 가지만 않을 뿐이다.
//
...
/** 사실 → **홈에 «이어서 하기» 를 띄울까**. 보여는 줬는데 아직 안 끝낸 사람만(#2171). */
export function isWelcomePending(f: FirstRunFacts): boolean {
  return !f.onboardedAt && !!f.shownAt;
}
...
export async function memberWelcomeState(memberId: string): Promise<{ firstRun: boolean; pending: boolean }> {
  ...
  const [profile, everCalledMcp, everHadSession] = await Promise.all([
    getLivProfile(memberId).catch(() => ({} as { onboarded_at?: string | null; welcome_shown_at?: string | null })),
