...
/**
 * 리브의 제안 카드(«지금 n가지가 걸립니다» · [제가 해 드릴게요] · [직접 볼게요] · [나중에])를 사람에게 내보내나.
 *
 * ⚠ **꺼 둔다**(원준 2026-09-14: «이렇게 제안하는거 일단 꺼줘. 사용자한테 제안하지 않게.»).
 *  판정(livFindings)은 그대로 두고 **내보내는 자리(me_liv_home 의 findings·total)만** 막는다 — 다시 켤 때는
 *  이 한 줄을 true 로 바꾸고 liv-proposals-off 시험을 함께 고친다.
 */
export const LIV_PROPOSALS_ON = false;
