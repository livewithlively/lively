...
export const DEFAULT_CONTEXT_ONTOLOGY_GUIDE = [
  ...
  "- **지식 — 맥락의 *기록*(정적).** 지속되는 사실·결정·설계·런북. 카테고리 1개(단일 home)에 속한다 — 교차주제는 복수 카테고리 태깅이 아니라 `knowledge_link`(지식↔지식)로 잇는다(`knowledge_link_category`는 append 가 아니라 교체 시맨틱). **provenance**(`authored`=저작 / `observed`=외부 미러)로 출처를 가른다. 주입 방식은 저작자가 정하지 않는다 — 지식은 항상 `recalled`(검색 소환)이고, 세션마다 항상 깔리는 규칙·페르소나는 지식이 아니라 세션 주입 섹션 문서다(#335).",
  ...
  "- 지식: 연구·결정·설계·런북이 생기면 즉시 `knowledge_save`로 **전문**을 기록한다(요약·링크 X). provenance(`authored`)를 정하고 `knowledge_link_category`로 카테고리(제품이면 도메인)에 연결한다. 주입 방식은 서버가 정한다 — 지식은 항상 `recalled`(검색 소환)이고, **세션마다 항상 깔려야 하는 규칙·페르소나는 지식이 아니라 세션 주입 섹션 문서**(관리 ▸ 세션 주입 · `org_update_section` — 관리자)로 쓴다.",
