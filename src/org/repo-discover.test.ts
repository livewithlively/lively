...
await ta("listGitlab: 서브그룹 경로는 full_path 로, 이름은 리프만(레포명은 경로 컴포넌트라 슬래시 불가)", async () => {
  const s = stubFetch([[
    { path: "core-api", path_with_namespace: "hf-dev/platform/backend/core-api", http_url_to_repo: "https://git.honestfund.kr/hf-dev/platform/backend/core-api.git", ssh_url_to_repo: "git@git.honestfund.kr:hf-dev/platform/backend/core-api.git", default_branch: "develop", visibility: "private" },
  ]]);
  try {
    const r = await listGitlab("git.honestfund.kr", "T");
    const o = r.options[0];
    assert.equal(o.name, "core-api");                                        // 리프만 — repo_create 에 넣을 값
    assert.equal(o.full_path, "hf-dev/platform/backend/core-api");           // 사람이 구분하는 라벨
...
await ta("listGitlab: min_access_level=20(Reporter) 로 조회 — 그룹 상속 접근을 잡고, 목록=클론가능 을 보장", async () => {
  const s = stubFetch([[]]);
  try {
    await listGitlab("git.honestfund.kr", "T");
