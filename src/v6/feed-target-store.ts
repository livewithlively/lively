...
export async function listPublishableForFeedTarget(feedTargetId: number, limit = 500): Promise<string[]> {
  ...
  if (!ft) return [];
  // 공개범위(#1291) — **발행은 조직 밖으로 내보내는 일이다.** 인입(커넥터)은 데이터 생산이라 특권으로 두지만,
  //  반출은 읽기와 같은 무게로 봐야 한다: 잠긴 지식이 노션 카드로 나가면 라이블리에서 아무리 막아도 소용이 없다.
  //  그래서 발행 대상은 'open' 인 것만. (대상이 지정된 지식을 특정 외부 대상에만 보내는 건 별개 문제라 v2 범위 밖.)
  const rows = ft.all_categories
    ? await q(itemsPool,
        `SELECT k.name FROM knowledge k
         WHERE k.provenance='authored' AND k.lifecycle='active' AND k.visibility IS DISTINCT FROM 'members'
    ...
    : await q(itemsPool,
        `SELECT DISTINCT k.name
         ...
         JOIN knowledge k ON k.name=kc.name AND k.lifecycle='active' AND k.provenance='authored'
                          AND k.visibility IS DISTINCT FROM 'members'
