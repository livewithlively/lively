...
//    인접해 시맨틱 질량게이팅은 bleed 위험 → 리터럴 1차(고정밀·결정론·무latency). (찰스 wiki-router 등가.)
//  - (b) = 라이블리 code_unit↔category 매핑 위 MODULE 최장-조상-prefix 매칭. 어니스트 code_unit 이
...
import { q, itemsPool } from "../domainmap/db.js";
import { findRecommendedKnowledge } from "./knowledge-store.js";

// ─────────────────────────── 순수 헬퍼 (DB 불요 — recall-router.test.ts 가 잠금) ───────────────────────────

// term-index·경로세그먼트에서 버리는 제네릭 토큰. 모듈 스캐폴딩/언어/빌드 단어는 도메인 변별력이 없다.
//  (어니스트 실측: aml-app·credit-facade·bond-domain 처럼 basename 앞토큰이 변별자, 뒤는 대개 제네릭.)
...
//  전부 소문자. 수동 키워드 큐레이션 없이 도메인맵에서 파생(찰스 kw/ac 의 라이블리판).
...
//  찰스: kw=부분문자열 includes, ac=단어경계 정규식 — 그 구분의 자동판정판.
...
//  member 머신마다 체크아웃 위치가 달라 repo.root_path 는 못 믿는다 → 세그먼트 매칭(경로-무관, 찰스식).
...
// 매칭 후보 경로 — box 마다 code_unit.path 컨벤션이 다르다(어니스트=repo-relative `domain/bond-domain`,
...
async function hubIndex(categoryId: number, exclude: string[], limit: number) {
  const rows = await q(itemsPool, `
    SELECT k.name, k.title, k.is_wiki
    FROM knowledge k
    JOIN knowledge_category kc ON kc.name=k.name AND kc.state<>'rejected'
    WHERE kc.category_id=$1 AND k.lifecycle='active' AND NOT (k.name = ANY($2::text[]))
    ORDER BY k.is_wiki DESC, k.updated_at DESC
    LIMIT $3`, [categoryId, exclude, limit]);
  return rows.map((r) => ({ name: r.name, title: r.title, is_wiki: !!r.is_wiki }));
}

export async function routeRecall(input: RecallInput): Promise<RecallResult> {
  ...
  if (text.trim() && hubCap > 0) {
    ...
    for (const h of hits.slice(0, hubCap)) {
      ...
      const index = await hubIndex(d.id, [...seen], 5).catch(() => []);
  ...
  if (paths.length && leafCap > 0) {
    ...
    for (const catId of orderedCats.slice(0, leafCap)) {
      ...
      let recs: any[] = [];
      try {
        recs = await findRecommendedKnowledge({ text: text || undefined, categoryIds: [catId], exclude: [...seen], limit: 3 });
      } catch { recs = []; }
  ...
  if (input.semantic && text.trim()) {
    try {
      const recs = await findRecommendedKnowledge({ text, exclude: [...seen], limit: 4, minScore: 0.6 });
