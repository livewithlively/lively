...
const REPOS = ["honest-one", "gitlab-code-harness", "domain-wiki"];
t("pickRepoRelpath: 체크아웃 루트 무관하게 repo 세그먼트로 분리", () => {
  assert.deepEqual(
    pickRepoRelpath("/srv/lively/shared/repos/honest-one/domain/bond-domain/src/main/kotlin/kr/honest/Bond.kt", REPOS),
    { repo: "honest-one", relpath: "domain/bond-domain/src/main/kotlin/kr/honest/Bond.kt" },
  );
  assert.deepEqual(
...
t("pickRepoRelpath: repo 세그먼트 없음 → null", () => {
  ...
  assert.equal(pickRepoRelpath("/tmp/honest-one", REPOS), null, "repo 가 마지막 세그먼트면 relpath 없음 → null");
});
...
t("longestPrefixPath: 리프파일 → 최장 모듈 매칭", () => {
  const rel = "domain/bond-domain/src/main/kotlin/kr/honest/Bond.kt";
...
t("unitMatchCandidates: repo-relative·repo-prefixed 양 컨벤션 후보(box 별 상이 대비)", () => {
  const c = unitMatchCandidates("honest-one", "domain/bond-domain/src/X.kt");
  assert.ok(c.includes("domain/bond-domain"), "어니스트식(repo-relative) 조상");
  assert.ok(c.includes("honest-one/domain/bond-domain"), "라이블리식(repo-prefixed) 조상");
  assert.ok(c.includes("honest-one/domain/bond-domain/src/X.kt"));
});
