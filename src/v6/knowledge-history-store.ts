...
export const META_OPS = [
  ...
  "move", "set_props_ui", "link_knowledge", "unlink_knowledge", "link_source", "unlink_source",
] as const;
...
export async function listKnowledgeHistory(
  ...
  const where = `WHERE a.entity='knowledge' AND a.entity_key=$1 AND ${opFilter}`;
  const params: unknown[] = [name, ops];

  const [rowsRes, countRes] = await Promise.all([
    itemsPool.query(
      `SELECT a.id, a.at, a.op, a.actor, a.actor_kind, a.channel,
...
export interface KnowledgeHistoryEntry {
  audit_id: number; at: string; op: string;
  actor: string | null; actor_display: string | null; actor_kind: string | null; channel: string | null;
  before: { title: string | null; body_md: string } | null;
  after: { title: string | null; body_md: string } | null;
}
export async function getKnowledgeHistoryEntry(name: string, auditId: number): Promise<KnowledgeHistoryEntry | undefined> {
  const r = await one(itemsPool,
    `SELECT a.id, a.at, a.op, a.actor, a.actor_kind, a.channel,
       ...
       FROM org_content_audit a
       LEFT JOIN org_member m ON m.id = a.actor
      WHERE a.entity='knowledge' AND a.entity_key=$1 AND a.id=$2`, [name, auditId]) as Record<string, unknown> | undefined;
  ...
  return {
    ...
    before: r.has_before ? { title: (r.title_before as string | null) ?? null, body_md: (r.body_before as string | null) ?? "" } : null,
    after: r.has_after ? { title: (r.title_after as string | null) ?? null, body_md: (r.body_after as string | null) ?? "" } : null,
  };
}
...
): Promise<{ op: string; snapshot: Record<string, unknown> | null }> {
  const r = await one(itemsPool,
    `SELECT op, before, after FROM org_content_audit
      WHERE entity='knowledge' AND entity_key=$1 AND id=$2`, [name, auditId]) as Record<string, unknown> | undefined;
  if (!r) throw new Error("이력 항목을 찾지 못했습니다");
  const snap = (direction === "before" ? r.before : r.after) as Record<string, unknown> | null;
  return { op: String(r.op), snapshot: snap ?? null };
}
