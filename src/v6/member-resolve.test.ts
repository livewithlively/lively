...
// 어니스트 실데이터 형태의 매핑 소스.
const identities = [
  { external_id: "5776468", email: "namjo.yoon@honestfund.kr", member_id: "namjo-yoon" }, // via=identity(관리탭 수동 매핑)
  { external_id: "216133775", email: "lively@lvly.io", member_id: "daon" },
];
const memberEmails = [
  { email: "lively1@honestai.tech", id: "lively1" }, // via=email(org_member.email 자동매칭)
  { email: "daon@lvly.io", id: "daon" },
];
...
t("buildMemberResolver: person_identity.email 역매칭(#697 via=identity 핵심)", () => {
  ...
  assert.equal(r("namjo.yoon@honestfund.kr"), "namjo-yoon");
  assert.equal(r("NAMJO.YOON@honestfund.kr"), "namjo-yoon"); // 대소문자 무시
  assert.equal(r("5776468"), "namjo-yoon");                  // ClickUp 숫자 id(external_id)
});

t("buildMemberResolver: org_member.email 직접 매칭(②)", () => {
  const r = buildMemberResolver(identities, memberEmails);
  assert.equal(r("lively1@honestai.tech"), "lively1");
});
...
t("reresolveMemberList: 치환 발생 → 새 배열(순서 보존)", () => {
  const r = buildMemberResolver(identities, memberEmails);
  assert.deepEqual(reresolveMemberList(["namjo.yoon@honestfund.kr", "lively1@honestai.tech"], r),
    ["namjo-yoon", "lively1"]);
  assert.deepEqual(reresolveMemberList(["lively1@honestai.tech", "5776468"], r),
...
t("reresolveMemberList: 서로 다른 raw 가 같은 member 로 → dedup(축약, non-null)", () => {
  ...
  assert.deepEqual(reresolveMemberList(["namjo.yoon@honestfund.kr", "5776468"], r), ["namjo-yoon"]);
});
