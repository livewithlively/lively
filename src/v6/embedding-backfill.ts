...
import { memAvailableMb } from "../ops/host-mem.js"; // #1059 백필 pre-flight 메모리 게이트
...
// 카테고리(#1153): 분류의 **정의**를 임베딩한다 — 이름 + 한 줄 설명 + should(정의·범위·규칙).
//  이 벡터가 '정의-내용 불일치'를 재는 고정점이다. should 가 비면 정의가 없는 것이므로 대상에서 뺀다
//  (이름만 임베딩하면 빈약한 벡터로 엉뚱한 거리가 나와, 정의가 없다는 사실이 '조금 어긋남'으로 흐려진다 —
//   정의 없음은 별도 상태로 드러내야 할 더 강한 신호다).
export const CATEGORY_TARGET: EmbeddingTarget = {
  name: "category",
  table: "category",
  idCol: "id",
  activeFilter: "state <> 'merged' AND should IS NOT NULL AND btrim(should) <> ''",
  selectCols: "id, name, description, should",
  buildText: (r) => embeddingInputText({
    title: (r as { name?: string | null }).name,
    summary: (r as { description?: string | null }).description,
    body_md: (r as { should?: string | null }).should,
  }),
};
...
//  호출: 부팅 후·10분 주기(index.ts) + connector_sync 완료 후(scheduler/index.ts·delivery.ts).
...
export async function runAutoBackfillSweep(): Promise<void> {
  ...
  try {
    ...
    //  겹쳐 물리 초과→OOM 스래싱 되던 것(고객사 A 2026-07-21)을 사전 회피. 가용 메모리가 임계 미만이면 이번 스윕을
