...
// 계약(에러형·레코드형)은 코어 소유 — EE 유무에 따라 identity 가 갈리면 tools/db.ts 의 instanceof 분기가 깨진다.
import { AuditWriteError, type DbAccessRecord, type SubjectKeyCapture } from "../../db/access-log.js";
...
// ── 기록 — 레코드형(DbAccessRecord)은 코어 계약(db/access-log.ts). ──
