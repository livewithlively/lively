...
//  일반 실패와 다르게 다뤄야 한다: 후속 접수 차단 · 즉시 알림 · 그 크론 정지.
//
// ⚠ '재시도 금지'는 이 모듈이 아니라 **스케줄러 구조가 이미 보장한다** — 확인해 두지 않으면 없는 방어를 있다고
//  믿게 된다(#1675 리뷰에서 이 착각이 잡혔다). `exit≠0` 으로 끝난 위탁은 `finish()` 로 곧장 종결되고,
//  재큐(`requeue`)는 **노드 유실 분기에만** 있다. 즉 자격 실패가 재시도 경로에 도달하는 길 자체가 없다.
//  그래서 ②의 실질은 '재시도 억제'가 아니라 **큐 유입 차단**이고, 그 수단이 ③의 크론 자동 정지다.
//
