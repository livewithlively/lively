...
//  이게 없으면 미지원 노드에 op 를 던지고 `unknown op: x` 라는 **문자열**을 받는다 — "미지원"과 "실패"가 구별 불가.
{
  ...
  assert.ok((NODE_OPS as readonly string[]).includes("chatTranscript"), "원격 대화 UI가 Codex 응답 파일을 읽는 op가 필요하다");
  assert.ok(!(NODE_BASELINE_OPS as readonly string[]).includes("chatTranscript"), "구 노드가 못 하는 chatTranscript 를 기준선에 넣었다");
