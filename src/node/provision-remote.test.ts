...
// ── 1. 완료 → result 배열을 그대로 반환. ──
{
  ...
  assert.equal(out.length, 2, "done.result 의 개수를 그대로 돌려줘야 한다");
  assert.deepEqual(out, repos, "done.result 배열을 그대로 돌려줘야 한다");
...
// ── 2. running 지속 → 결국 완료(running 여러 번 뒤 done, 결과 1번). ──
{
  ...
  assert.deepEqual(out, repos, "running 을 거쳐 done 의 결과를 돌려줘야 한다");
...
// ── 4. known:false → reprovision 후 이어감. 재지시는 known:false 를 만난 횟수만큼만. ──
{
  ...
  assert.deepEqual(out, repos, "재지시 후 폴링을 이어가 done 을 정상 완료해야 한다");
...
// ── 7. 일시 폴 실패(온라인) → 몇 번 봐주고 회복해 완주. ──
{
  ...
  assert.deepEqual(out, repos, "일시 실패를 삼키고 회복되면 정상 완료해야 한다");
