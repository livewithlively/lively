...
//  근거는 우리 코드에 있다 — 재연결 백오프 상한은 30초다(agent.ts BACKOFF_MAX_MS). 프로세스가 살아서
//  깨어 있으면 **30초 안에** 다시 붙는다. 그러므로 수 분 이상의 공백은 "그 머신이 네트워크에서 사라진 상태"다.
...
/** '긴 공백' 임계 — 이를 **초과**하면 길다. 재연결 백오프 상한 30초의 10배 — 프로세스가 깨어 있으면 나올 수 없다. */
