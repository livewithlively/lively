...
// 상류 접속 — SSRF-안전 fetch + StreamableHTTP + (선택) 인증 헤더. 호출자가 close 책임.
...
async function connectUpstream(url: string, headers: Record<string, string>): Promise<Client> {
  ...
  const transport = new StreamableHTTPClientTransport(new URL(url), { requestInit: { headers }, fetch: ssrfFetch });
