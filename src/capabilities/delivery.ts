...
// 워크스페이스 폴더를 **id 로** 푼다 — DB 프로젝트가 없어도(고아) 폴더는 남아 있으므로 폴더가 진실이다.
//  project/<id> 를 먼저 보고 없으면 legacy-project/<id>(완료 보관 — done 처리는 폴더를 '이동'할 뿐 지우지 않는다).
//  projectAbsPath 를 거치므로 `..` 탈출·범위 이탈은 거기서 차단된다.
async function workspaceDirOf(id: number): Promise<string | null> {
  for (const rel of [`${PROJECT_SUBDIR}/${id}`, `${LEGACY_SUBDIR}/${id}`]) {
    const abs = projectAbsPath(rel);
    if (await fsp.stat(abs).then((s) => s.isDirectory()).catch(() => false)) return abs;
  }
  return null;
}

// 회수 대상 id 목록을 받아 계획(또는 적용)한다. analyze/reclaim 이 같은 코드를 쓰므로 **본 것과 지우는 것이 갈리지 않는다.**
async function planOrApply(
  ids: number[], user: LivelyUser, opts: { apply: boolean; removeWorktree: boolean },
): Promise<{ projects: unknown[]; total_reclaimable_bytes: number; total_freed_bytes: number }> {
  if (!Array.isArray(ids) || !ids.length) throw new HttpError(400, "project_ids 가 필요합니다");
  if (ids.length > 40) throw new HttpError(400, "한 번에 최대 40개까지 — 나눠서 호출하세요(오래 걸리면 프록시가 끊습니다)");
  for (const id of ids) if (!Number.isInteger(id) || id <= 0) throw new HttpError(400, `잘못된 project_id: ${id}`);
  ...
  let totalFreed = 0;
  for (const id of ids) {
    const dir = await workspaceDirOf(id);
    if (!dir) { projects.push({ project_id: id, skipped: "워크스페이스 폴더가 없습니다" }); continue; }
    const p = await getV6Project(id).catch(() => null);
    const repos = await reposIn(dir);
    if (!repos.length) { projects.push({ project_id: id, name: p?.name ?? String(id), dir, repos: 0, reclaimable_bytes: 0, freed_bytes: 0, results: [] }); continue; }
    ...
    totalFreed += freed;
    projects.push({
      project_id: id, name: p?.name ?? String(id), orphan: !p, dir, repos: repos.length,
      reclaimable_bytes: reclaimable, freed_bytes: freed, results,
    });
  }
...
const workspaceBatchCapabilities = (): Capability[] => [
  restOnly("org_workspace_analyze", "워크스페이스 일괄 분석(dry-run)",
    "주어진 프로젝트들의 회수 가능량을 한 번에 계산한다 — **아무것도 지우지 않는다.** " +
    "DB 에서 삭제된 고아 폴더도 다룬다(프로젝트별 REST 는 404 로 죽어 회수 불가). " +
    ...
    async (input: { project_ids: number[] }, user: LivelyUser) =>
      ({ dry_run: true, ...await planOrApply(input.project_ids, user, { apply: false, removeWorktree: false }) }),
    { project_ids: z.array(z.number().int().positive()).min(1).max(40).describe("분석할 프로젝트 id (최대 40)") }),

  restOnly("org_workspace_reclaim", "워크스페이스 일괄 회수",
    "주어진 프로젝트들의 **재생성 가능한 파생물만** 실제로 지운다(node_modules·build·target·.gradle…). " +
    "소스·커밋·.env·data/ 는 절대 지우지 않고, 활성 세션이 붙어 있으면 그 프로젝트는 건너뛴다. " +
    "remove_worktree=true 여도 **푸시 완료 + 더티 없음**인 워크트리만 제거한다(아니면 이유를 남기고 유지). " +
    "고아 폴더도 회수한다. ⚠ 한 번에 최대 40개. admin 전용.",
    [{ method: "POST", paths: ["/api/ui/org/workspace/reclaim"], parse: (req) => ({ ...(req.body ?? {}) }) }],
    async (input: { project_ids: number[]; remove_worktree?: boolean }, user: LivelyUser) =>
      ({ dry_run: false, ...await planOrApply(input.project_ids, user, { apply: true, removeWorktree: !!input.remove_worktree }) }),
    {
      project_ids: z.array(z.number().int().positive()).min(1).max(40).describe("회수할 프로젝트 id (최대 40)"),
    ...
    async (_input: unknown, user: LivelyUser) => {
      ...
      for (const f of found) {
        ...
        items.push({
          ...
          //  실측(dev 박스): 307개 중 184개가 레포 없는 12KB 껍데기, 57개가 고아. 목록의 78% 가 '눌러봐야 에러'였다.
          repos: (await reposIn(f.dir)).length, // 0 = 회수할 파생물이 없다(정상 — provision 안 한 프로젝트)
          orphan: !project, // DB 에 프로젝트가 없다(삭제됨) — 폴더만 남았다. 프로젝트별 REST 는 404 라 여기서만 회수 가능.
        });
      }
