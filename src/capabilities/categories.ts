...
const categoryEdgeList: Capability = {
  ...
  scope: "context",
  input: { axis: z.enum(["should", "is"]).optional() },
  expose: {
    mcp: true,
    rest: [{ method: "GET", paths: ["/api/ui/category-edges"],
      parse: (req) => ({ axis: req.query?.axis ? String(req.query.axis) : undefined }) }],
  },
  handler: async (input: any) => ({ edges: await listCategoryEdges({ axis: input.axis }) }),
};
...
const categoryEdgeRemove: Capability = {
  ...
  handler: async (input: any) => removeCategoryEdge(input.id),
};
