...
const recallRoute: Capability = {
  ...
  // 공개범위(#1291) — 훅이 멤버 토큰으로 부르므로 게이트웨이가 신원을 안다. 그 신원으로 소환 결과를 거른다
  //  (여기가 안 막히면 잠긴 지식의 소환키가 남의 세션 첫머리에 그대로 깔린다).
  handler: async (input, _user, ctx) => routeRecall(input, ctx?.viewer ?? null),
};
