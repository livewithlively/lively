...
// id 파싱은 http/rest-util 의 parsePosInt 로 수렴(#1313 R46) — 이 파일의 문구 계열('… 는 양의 정수여야 합니다')은
//  capabilities/*.ts 다수의 'id 가 올바르지 않습니다'(parseId)와 **다르다**. 두 계열을 각각 이름 있는 함수로 둔다.
...
const dmDomainDetail: Capability = {
  ...
  expose: {
    ...
    rest: [{
      ...
      parse: (req) => ({ repo: parseRepo(req.params.repo), id: parsePosInt(req.params.id, "domain id") }),
    }],
  },
...
const dmMappingConfirm: Capability = {
  ...
  expose: {
    ...
    rest: [{
      ...
      parse: (req) => ({ id: parsePosInt(req.params.id, "mapping id") }),
    }],
  },
...
const dmMappingMove: Capability = {
  ...
  expose: {
    ...
    rest: [{
      ...
      parse: (req) => ({
        id: parsePosInt(req.params.id, "mapping id"),
        domainId: parsePosInt(((req.body ?? {}) as Record<string, unknown>).domainId, "domainId"),
      }),
    }],
  },
...
const dmDebtStatus: Capability = {
  ...
  expose: {
    ...
    rest: [{
      ...
      parse: (req) => {
        const id = parsePosInt(req.params.id, "debt id");
...
const dmRestore: Capability = {
  ...
  expose: {
    ...
    rest: [{
      ...
      parse: (req) => ({ changeId: parsePosInt(req.params.changeId, "change id") }),
    }],
  },
...
const dmLayoutSave: Capability = {
  ...
  scope: "context",
  // mcp:false 여도 parse 산출과 같은 필드를 선언한다(#1403 — types.ts 의 input 규약). saveDomainLayout 이 요구하는 형태.
  input: { positions: z.array(z.object({ id: z.number().int(), x: z.number(), y: z.number() })) },
  expose: {
    mcp: false,
    rest: [{
      method: "POST",
