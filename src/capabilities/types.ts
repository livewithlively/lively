...
export interface CapabilityCtx {
  ...
  ip?: string;              // 요청 IP
  readOnly?: boolean;       // 읽기전용 세션(#1007+) — 이 요청이 read-only 모드인가(x-lively-mode=readonly 파생). 강제는 어댑터가 하고, 여기선 관측용(me 가 반환).
  incognito?: boolean;      // 인코그니토 세션(#1007+) — 이 요청이 incognito 모드인가(x-lively-mode=incognito 파생). incognito = lively 툴 0개+전체 차단(읽기·쓰기 모두). 관측용.
  // 맥락 가시성(#1291) — 이 요청의 **열람 신원**. 문자열=그 멤버로 필터, null=특권(admin·내부 경로, 필터 없음).
  //  actor 와 값은 같지만 의미가 다르다: actor 는 '누가 썼나'(감사), viewer 는 '누가 보나'(접근통제).
  //  admin 은 어댑터에서 null 로 정규화된다 — 그래서 핸들러는 admin 분기를 따로 쓰지 않는다.
  //  두 어댑터(capabilities/index.ts, web.ts)가 이 한 필드를 채우므로 MCP·REST 가 자동으로 같은 규칙을 탄다.
  viewer?: string | null;
}
...
export interface Capability {
  ...
  //  ⚠ 규약(#1403): **expose.mcp 와 무관하게 mount.parse 가 싣는 필드를 그대로 선언한다.** input 은 MCP 표면이자
  //   핸들러 입력의 계약 선언이고(R53 이후 `z.infer<z.ZodObject<typeof xInput>>` 로 타입을 파생하는 게 표준),
  //   mcp:false 라고 비워 두면 나중에 mcp:true 로 여는 순간 전 필드가 조용히 strip 된다. 정말로 **입력을 안 받는**
  //   op 만 `{}` 가 정답이다. mcp-input-schema.test 의 R4 가 이 정합을 mcp 여부와 무관하게 검사한다.
  //  ⚠ 평평한 raw shape 만 쓸 수 있다 — z.discriminatedUnion 등 non-object 스키마를 넘기면 SDK(1.29)의
  //   normalizeObjectSchema 가 정규화하지 못해 **tools/list 에 빈 스키마**가 실리고 호출 때만 union 으로 검증한다
  //   (= "광고는 되는데 안 되는 툴" 재생산). action 별 조건부 필수 같은 건 필드 `.describe()` 로 표현하라.
  input: ZodRawShape;
  expose: {
    mcp: boolean;
  ...
  //  ⚠ 그 '1곳'은 **이 선언뿐**이다(#1313 R53). 구현부는 any 를 쓰지 말고 좁혀 받는다 —
  //   input zod shape 을 명명 상수로 빼고 `type XInput = z.infer<z.ZodObject<typeof xInput>>` 로 파생,
  //   user 는 LivelyUser, ctx 는 `ctx?: CapabilityCtx`. (shape 추출은 등가 변환이라 표면 input 해시가 안 바뀐다.)
