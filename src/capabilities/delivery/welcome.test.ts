...
t("W6 없는 답을 지어내지 않는다 — 이어 여는 사람에게 남의 이름을 부르지 않는다", () => {
  //  ⚠ 2026-08-31 — 종전엔 «demoJump 일 때만 지어낸다» 를 글자로 못 박았다. 그 코드는 replayChatTo 안에
  //   있었는데, 리브 챗 문답을 통째로 걷어내면서(#1631, 원준님) 그 함수가 사라졌다.
  //   지켜야 할 것은 «지어내는 자리가 demoJump 뒤에 있다» 가 **아니라** «지어내지 않는다» 다.
  //   지금은 지어내는 코드 자체가 없으므로, 그것이 **다시 생기지 않는지**를 본다.
  assert.doesNotMatch(OB, /S\.name = '원준'/, "화면이 이름을 지어낸다 — 그 사람의 답이 아니다");
  assert.doesNotMatch(OB, /S\.read\.total = demoJump \? 41/, "연출 숫자 41 이 실제 화면에 샌다");
  //  중립 폴백(`||`)은 허용된다 — 없는 것을 주장하지 않고 화면만 안 깨지게 하는 값이라서다.
  //  그러나 **대입**으로 답을 만드는 것은 안 된다(#1813 «900ms 뒤 무조건 연결됐어요» 와 같은 모양).
  assert.doesNotMatch(OB, /S\.aiConnected = (?!true)/, "연결 여부를 화면이 채워 넣는다 — 그건 서버가 답할 것이다");
});
