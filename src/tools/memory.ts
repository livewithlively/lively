...
//  도메인 귀속은 domainmap 약결합(V5 탈-repo: key 만으로 귀속, repo-비의존). (member/internal 분리는 2026-06-17 폐기.)
...
async function classifyDomain(text: string): Promise<{ key: string; score: number } | null> {
  let domains: Awaited<ReturnType<typeof listAllDomains>>;
  try { domains = await listAllDomains(); } catch { return null; }
...
export function registerMemoryTools(server: McpServer): void {
  server.registerTool(
    ...
    async ({ note, title, domain, name }, extra) => {
      ...
      // 도메인: 지정되면 존재 검증(fail-open), 생략되면 자동분류. V5 탈-repo — 전 repo·전 space 통합 통제어휘.
      let domainKey: string | null = null;
      let domainNote = "";
      if (domain) {
        // domainmap 가용 시에만 존재 검증(다운=fail-open, 약결합 슬러그라 저장은 진행). 결과로 분기.
        let domains: Awaited<ReturnType<typeof listAllDomains>> | null = null;
        try { domains = await listAllDomains(); } catch { domains = null; }
        if (domains && domains.length) {
          const matches = domains.filter((d) => d.key === domain);
          if (!matches.length) throw new Error(`해당 도메인을 찾을 수 없습니다. domain_list 로 확인하세요.`);
          if (matches.length > 1) {
            throw new Error(`도메인 '${domain}' 가 여러 space(${matches.map((m) => m.space).join("/")})에 있어 모호합니다 — 고유 key 로 지정하세요.`);
          }
        }
        domainKey = domain;
      } else {
        const c = await classifyDomain(`${title ?? ""} ${note}`);
      ...
      const saved = await upsertMemory({
        ...
        domain_repo: null, // V5 탈-repo: 도메인 귀속은 key 만(repo-비의존).
      }, user.userId, "mcp");
