...
async function fillProjects(zone, onCount, projectsP, listsP) {
  ...
  const draw = () => {
    ...
    if (selectedListId === undefined || !byList.has(selectedListId)) selectedListId = currentOrder[0];
    const grid = el('div', { class: 'pjv-ov-grid dash-ov-grid' });
    for (const listId of currentOrder) grid.append(projBlock(listId, listById.get(listId), byList.get(listId)));
...
function dashModal(title, content) {
  ...
  const box = el('div', { class: 'dash-modal', role: 'dialog', 'aria-modal': 'true', 'aria-label': title },
...
async function fillSessions(zone, onCount, projectsP?) {
  ...
  const draw = () => {
    ...
    for (const s of shown) {
      ...
      if (pid) badges.append(el('span', { class: 'dash-badge dash-badge-proj', title: '프로젝트: ' + (projName.get(pid) || pid), text: projName.get(pid) || ('프로젝트 #' + pid) }));
      if (s.owned) { if ((s.invites || []).length) badges.append(el('span', { class: 'dash-badge', text: '초대 ' + s.invites.length })); }
      else badges.append(el('span', { class: 'dash-badge', title: '소유: ' + memberName(s.owner), text: memberName(s.owner) + ' · 초대받음' }));
      const info = el('div', { class: 'dash-sess-info' },
        el('div', { class: 'dash-sess-title' },
          el('span', { class: 'dash-sess-name', title: s.label, text: s.label || '(이름 없음)' }), badges),
...
async function fillFolders(zone) {
  ...
  if (goEl) {
    ...
    btn.onclick = () => dashModal('팀 공유 폴더',
      dirs.length ? el('div', { class: 'proj-file-grid dash-foldpop-grid' }, ...dirs.map(dashFolderCard)) : dashEmpty('공유 워크스페이스에 폴더가 없어요.'));
  ...
  for (const name of dirs) grid.append(dashFolderCard(name));
