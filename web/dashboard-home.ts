...
async function fillProjects(zone, onCount, projectsP, listsP) {
  ...
  const projBlock = (listId, l, arr) => {
    ...
    const card = el('div', { class: 'pjv-ov-card', role: 'button', tabindex: '0', title: name },
    ...
    card.addEventListener('keydown', (e: any) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); go(); } });
    return card;
  };
...
async function fillFolders(zone) {
  ...
  for (const name of dirs) grid.append(el('div', { class: 'proj-file-card', title: name },
