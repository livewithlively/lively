...
async function fillSessions(zone, onCount, projectsP?) {
  ...
  const draw = () => {
    ...
    const chips: any[] = [['all', '전체'], ['mine', '내 것'], ['invited', '초대받음']];
    ...
    for (const s of shown) {
      ...
      if (pid) { const mine = projName.has(pid); badges.append(el('span', { class: 'dash-badge dash-badge-proj' + (mine ? ' dash-badge-proj-mine' : ''), title: (mine ? '내 프로젝트: ' : '프로젝트: ') + (projName.get(pid) || pid), text: projName.get(pid) || ('프로젝트 #' + pid) })); }
      ...
      const info = el('div', { class: 'dash-sess-info' },
        ...
        el('div', { class: 'dash-sess-sub', title: subTitle, text: sub }));
      ...
      if (s.owned) {
        ...
        moreBtn.onclick = (e: any) => { e.stopPropagation(); openSessMenu(moreBtn, s, reloadSessions); };
        acts.append(moreBtn);
      }
      const box = el('div', { class: 'dash-sess-box' + (s.attached ? ' live' : ''), role: 'button', tabindex: '0',
        title: '클릭하면 새 창에서 열립니다' }, info, acts);
      box.addEventListener('click', open);
      box.addEventListener('keydown', (e: any) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); open(); } });
      list.append(box);
    }
