...
export function applyToOpenTabs(): boolean {
  try { return localStorage.getItem(OPEN_TABS_KEY) === '1'; } catch { return false; }
}
export function setApplyToOpenTabs(on: boolean): void {
  try { if (on) localStorage.setItem(OPEN_TABS_KEY, '1'); else localStorage.removeItem(OPEN_TABS_KEY); }
  catch { /* 스토리지 차단 — 이 탭에선 꺼진 것으로 동작한다 */ }
}

/** 열린 세션 탭들의 하네스 테마를 지금 바꾼다. 호출부(테마 전환 직후)가 결과를 사람에게 알린다. */
export async function pushThemeToOpenTabs(ids: string[]): Promise<{ applied: number; notes: string[] }> {
  if (!ids.length) return { applied: 0, notes: [] };
  const theme = isDark() ? 'dark' : 'light';
  const { api } = await import('./lib/net.js');
  const out: any = await api('/api/ui/terminal/sessions/theme', {
    method: 'POST', body: JSON.stringify({ theme, ids }),
  });
  const rows: any[] = Array.isArray(out?.results) ? out.results : [];
