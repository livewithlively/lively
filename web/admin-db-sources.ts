...
function dbSourceEditor(detail, data) {
  ...
  for (const s of sources) {
    listCol.append(el('div', { class: 'mini-row' + (s.name === sel ? ' sel' : ''),
      onclick: () => { state.admin.dbSrcSel = s.name; rerenderPanel(detail, 'db-sources', data); } },
...
function dbSourceForm(root, s, data, detail, isNew) {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      await loadAdmin(true); state.admin.dbSrcSel = payload.name; toast('저장됨 — 즉시 조회 가능'); rerenderPanel(detail, 'db-sources', state.admin.data);
  ...
  if (!isNew) actions.append(el('button', { class: 'btn-text', text: '제거', onclick: async () => {
    ...
    try { await api('/api/ui/org/db-source/remove', { method: 'POST', body: JSON.stringify({ name: s.name }) }); await loadAdmin(true); state.admin.dbSrcSel = null; toast('제거됨'); rerenderPanel(detail, 'db-sources', state.admin.data); }
