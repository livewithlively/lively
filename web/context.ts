...
function buildHeader(selStage: CtxStage, selItem: CtxItem): HTMLElement {
  const crumbBar = el('div', { class: 'pjv-crumbbar' },
    el('nav', { class: 'pjv-crumbs', 'aria-label': '현재 위치' },
      ...
      el('span', { class: 'ctx-crumb-sub', text: '자료가 지식이 되어 AI 에 닿는 길 — 수집 › 증류 › 분류 › 관리 › 전달' })));
  ...
  for (const s of STAGES) {
    ...
    if (s.step && s.step > 1) tabs.append(el('span', { class: 'ctx-vtab-flow', 'aria-hidden': 'true', text: '›' }));
    const tab = el('a', {
      ...
      s.step ? el('span', { class: 'ctx-vtab-step', 'aria-hidden': 'true', text: String(s.step) }) : ctxFlowIcon(),
      el('span', { class: 'ctx-vtab-label', text: s.label }),
      s.step && s.step <= 4 ? el('span', { class: 'ctx-vtab-dot', 'aria-hidden': 'true' }) : null,
      s.adminEdit ? el('span', { class: 'ctx-vtab-lock', 'aria-hidden': 'true', title: '관리자만 고칠 수 있습니다' }, ctxLockIcon()) : null);
    tabs.append(tab);
    if (s.solo) tabs.append(el('span', { class: 'pjv-vtab-sep', 'aria-hidden': 'true' }));   // 개요 | ①›②›③›④›⑤ — 조망과 흐름을 한 칸 띄운다
  }
...
async function paintStageHealth(view: HTMLElement): Promise<void> {
  ...
  const map: Record<string, string> = { collect: lv.collect, distill: lv.distill, classify: lv.classify, manage: lv.manage };
  for (const [key, level] of Object.entries(map)) {
    const dotEl = view.querySelector('.ctx-vtab[data-stage="' + key + '"] .ctx-vtab-dot') as HTMLElement | null;
...
function ctxFlowIcon(): SVGElement {
