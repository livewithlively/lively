...
import type { BlockEditorOpts, EditorCtx, EditorState } from './editor/context.js';
...
// 에디터 조립
// ════════════════════════════════════════════
export function createBlockEditor(opts: BlockEditorOpts = {}) {
  ...
  const st: EditorState = { dirty: false, composing: false, histLock: false, slash: null, mention: null };
  const ctx = { opts, root, st } as EditorCtx;
  ctx.markDirty = () => { st.dirty = true; if (!st.histLock) ctx.snapSoon(); if (opts.onChange) opts.onChange(); };       // 구조 변경 — 즉시 스냅샷(프레임 단위 코얼레싱)
  ctx.markDirtyType = () => { st.dirty = true; if (!st.histLock) ctx.snapLater(); if (opts.onChange) opts.onChange(); };  // 타이핑 — 유휴 600ms 버스트 단위
