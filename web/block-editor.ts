...
function inlineDomToMd(node: any): string {
  ...
  for (const c of node.childNodes) {
    ...
    if (tag === 'CODE') { out += '`' + c.textContent.replace(/`/g, '').replace(/\u200B/g, '') + '`'; continue; }
  ...
  while (i < blocks.length) {
    ...
    switch (b.type) {
      ...
      case 'columns': {
        ...
        if (cols.length === 1) { chunks.push(blocksToMd(cols[0])); break; }
        chunks.push(':::columns\n' + cols.map((c) => ':::column\n' + blocksToMd(c) + '\n:::').join('\n') + '\n:::');
        break;
      }
      case 'pagecard':
      ...
      case 'collection': {
        ...
        if (a.view === 'cards') line += ' view=cards';
        if (a.sort === 'title') line += ' sort=title';
        chunks.push(line + '\n:::');
        break;
      }
      case 'divider':
...
export function createBlockEditor(opts: {
  ...
  function blockData(block: HTMLElement): any {
    ...
    switch (type) {
      ...
      case 'toggle': {
        ...
        return { type, summary: sum ? inlineDomToMd(sum as HTMLElement) : '', children: kids ? childBlocksOf(kids as HTMLElement) : [] };
      }
      case 'columns': {
        const cols = Array.from(block.querySelectorAll(':scope > .be-main > .be-cols > .be-col'));
        return { type, cols: cols.map((c) => childBlocksOf(c as HTMLElement)) };
      }
      default: return { type: 'p', text: t ? inlineDomToMd(t) : '' };
    }
  }
  // 컨테이너(togglekids/col)의 직계 블록 데이터 — toggle/columns 재귀 직렬화용.
  function childBlocksOf(container: HTMLElement): any[] {
    return Array.from(container.children)
      .filter((n: any) => n.classList && n.classList.contains('be-block'))
  ...
  function normalizeStructure() {
    root.querySelectorAll('.be-block[data-type="columns"]').forEach((cb: any) => {
      const colsWrap = cb.querySelector(':scope > .be-main > .be-cols');
      if (!colsWrap) return;
      const cols = Array.from(colsWrap.children).filter((c: any) => c.classList.contains('be-col'));
      const hasBlock = (c: any) => Array.from(c.children).some((n: any) => n.classList && n.classList.contains('be-block'));
      const nonEmpty = cols.filter(hasBlock);
      if (nonEmpty.length <= 1) {
        // 해체 — 남은 블록을 컬럼 자리에 펼치고 컨테이너 제거(노션 동일).
        let anchor: any = cb;
        nonEmpty.forEach((c: any) => Array.from(c.children).forEach((n: any) => {
          if (n.classList && n.classList.contains('be-block')) { anchor.after(n); anchor = n; }
        }));
        cb.remove();
      } else {
        cols.forEach((c: any) => { if (!hasBlock(c)) c.remove(); });
      }
    });
    root.querySelectorAll('.be-togglekids').forEach((k: any) => {
      if (!Array.from(k.children).some((n: any) => n.classList && n.classList.contains('be-block'))) {
  ...
  function duplicateBlock(block: HTMLElement) {
  ...
  function moveBlockDir(block: HTMLElement, dir: number) {
    const sib = dir < 0 ? block.previousElementSibling : block.nextElementSibling;
    if (!sib || !sib.classList.contains('be-block')) return;
  ...
  function ensureOne() {
    ...
    if (bs.length === 1 && bs[0].dataset.type === 'p' && opts.placeholder) {
      ...
      if (t) t.dataset.ph = opts.placeholder;
    }
  }
  function isEmptyNow() {
    const bs = blockEls();
  ...
  function focusBlock(block: HTMLElement, atStart = true) {
    ...
    const focusable = block.querySelector('.be-div, .be-raw, .be-pagecard, .be-collection') as HTMLElement;
  ...
  function convertBlock(block: HTMLElement, to: any) {
    ...
    if (cur.type === 'toggle' && to.type !== 'toggle' && Array.isArray(cur.children) && cur.children.length) {
      let anchor: HTMLElement = nb;
      for (const c of cur.children) { const cb = makeBlock(c); anchor.after(cb); anchor = cb; }
    }
    ensureOne();
    renumber();
    markDirty();
  ...
  const SLASH_ITEMS: any[] = [
    ...
    { k: 'collection', ic: '▤', label: '컬렉션 (라이브 목록)', hint: '조건에 맞는 문서 자동 목록', kw: 'collection database view live 컬렉션 목록 라이브 데이터베이스 자동 최신', apply: (b) => {
      ...
      if (empty) { nb = makeBlock(data); b.replaceWith(nb); ensureOne(); renumber(); markDirty(); }
      else nb = insertBlockAfter(b, data);
      const gear = nb.querySelector('.be-coll-gear') as HTMLElement;
      if (gear) setTimeout(() => gear.click(), 0);   // 삽입 즉시 조건 설정 열기
    } },
  ...
  let mention: any = null;   // { block, node, from(''[[' 시작 오프셋), query, menu, items, sel, timer }
  ...
  function paintMention() {
    ...
    mention.menu.replaceChildren(
      ...
      ...(!items.length ? [el('div', { class: 'be-slash-empty', text: mention.query ? '결과 없음' : '검색어를 입력하세요' })]
        : items.map((it: any, idx: number) => {
          const row = el('button', { class: 'be-slash-item' + (idx === mention.sel ? ' on' : ''), type: 'button' },
    ...
    const r = caretRange();
    let rect: DOMRect = r ? r.getBoundingClientRect() : mention.block.getBoundingClientRect();
    if (!rect.width && !rect.height && !rect.top) rect = mention.block.getBoundingClientRect();
  ...
  function scheduleMentionSearch() {
    ...
    mention.timer = setTimeout(async () => {
      ...
      try {
        ...
        const r = await api(url);
        if (!mention) return;
        mention.items = ((r && r.entries) || []).filter((e: any) => !String(e.name || '').startsWith('category-home-'));
        mention.sel = 0;
        paintMention();
  ...
  function syncMention(target: HTMLElement) {
    ...
    if (mention.query !== m[1] || !mention.items.length) { mention.query = m[1]; scheduleMentionSearch(); }
    paintMention();
  }
  ...
  function openLinkPop() {
    ...
    if (old) { old.remove(); return; }
    let n: any = r0.commonAncestorContainer;
    if (n.nodeType === 3) n = n.parentElement;
    ...
    const input = el('input', { class: 'be-linkpop-in', type: 'text', spellcheck: 'false',
      ...
      value: existing ? (existing.getAttribute('href') || '') : '' }) as HTMLInputElement;
    const close = () => { pop.remove(); document.removeEventListener('mousedown', onDoc, true); };
    const restoreSel = () => { const s = window.getSelection()!; s.removeAllRanges(); s.addRange(saved); };
    const apply = () => {
      const url = input.value.trim();
      if (!url) { restoreSel(); document.execCommand('unlink'); close(); markDirty(); return; }
      const safe = safeHref(url);
      if (!safe) { toast('허용되지 않는 URL 입니다', true); input.focus(); return; }
      restoreSel();
  ...
  const turnBtn = el('button', { class: 'be-tool be-tool-turn', type: 'button', title: '블록 유형 전환' },
    turnLabel, el('span', { class: 'be-turn-caret', 'aria-hidden': 'true', text: '⌄' }));
  ...
  turnBtn.onclick = () => {
    ...
    for (const it of SLASH_ITEMS) {
      if (!['text', 'h1', 'h2', 'h3', 'bullet', 'numbered', 'todo', 'toggle', 'quote', 'callout', 'code'].includes(it.k)) continue;
      const cur = curTypeKey(block) === (it.k === 'text' ? 'p' : it.k);
      const b = el('button', { class: 'be-turnpop-item' + (cur ? ' on' : ''), type: 'button' },
        el('span', { class: 'be-slash-ic', text: it.ic }), el('span', { text: it.label }),
        cur ? el('span', { class: 'be-turnpop-check', text: '✓' }) : null);
      b.addEventListener('mousedown', (e) => e.preventDefault());
    ...
    pop.style.top = (r.bottom + 6) + 'px';
    const onDoc = (ev: any) => { if (!pop.contains(ev.target) && ev.target !== turnBtn) close(); };
    setTimeout(() => document.addEventListener('mousedown', onDoc, true), 0);
  };
  const markBtns: Record<string, HTMLElement> = {};
  const addTool = (key: string, label: string, title: string, fn: () => void, cls?: string) => {
  ...
  tools.append(
    turnBtn,
    el('span', { class: 'be-tools-sep', 'aria-hidden': 'true' }),
    addTool('b', 'B', '굵게 — ⌘B', () => document.execCommand('bold'), 'be-tool-b'),
    addTool('i', 'i', '기울임 — ⌘I', () => document.execCommand('italic'), 'be-tool-i'),
    addTool('u', 'U', '밑줄 — ⌘U', () => document.execCommand('underline'), 'be-tool-u'),
    addTool('s', 'S', '취소선', () => document.execCommand('strikeThrough'), 'be-tool-s'),
    addTool('code', '</>', '인라인 코드 — ⌘E', () => toggleWrap('code'), 'be-tool-code'),
    addTool('mark', '형광', '하이라이트 — ⌘⇧H', () => toggleWrap('mark'), 'be-tool-mark'),
    el('span', { class: 'be-tools-sep', 'aria-hidden': 'true' }),
    addTool('a', '링크', '링크 걸기', () => openLinkPop(), 'be-tool-link'),
  );
  function positionTools() {
    ...
    if (toolsBlock) turnLabel.textContent = TURN_LABEL[curTypeKey(toolsBlock)] || '텍스트';
    const on = (sel: string) => !!(ancEl && ancEl.closest && ancEl.closest(sel));
  ...
  const onSelChange = () => {
    if (!root.isConnected) { document.removeEventListener('selectionchange', onSelChange); tools.remove(); closeSlashMenu(); closeMention(); return; }
  ...
  root.addEventListener('keydown', (e: any) => {
    ...
    if (mention) {
      if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        ...
        if (n) { mention.sel = (mention.sel + (e.key === 'ArrowDown' ? 1 : n - 1)) % n; paintMention(); }
        return;
      }
