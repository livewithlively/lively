...
function layout(domains: any[], edges: any[]) {
  ...
  const st = new Map<number, number>(); // 0 방문전 · 1 스택 · 2 완료
  const back = new Set<string>();
  const visit = (root: number) => { // 반복 DFS(재귀 깊이 회피)
    const path: Array<{ n: number; i: number }> = [{ n: root, i: 0 }];
    st.set(root, 1);
    while (path.length) {
      const top = path[path.length - 1];
      const kids = adj.get(top.n)!;
      if (top.i < kids.length) {
        const v = kids[top.i++]; const s = st.get(v) || 0;
        if (s === 1) back.add(top.n + '>' + v);         // 스택 안 → back-edge(사이클 폐합)
        else if (s === 0) { st.set(v, 1); path.push({ n: v, i: 0 }); }
  ...
  let maxL = 0;
  for (const d of core) {
    const L = layer.get(d.id) || 0; maxL = Math.max(maxL, L);
  ...
  const pos = new Map<number, { x: number; y: number; cross: boolean }>();
...
function clipEnds(a: any, b: any) {
  ...
  const cl = (c: any, sx: number, sy: number) => {
...
async function renderDomainmap(view: any, params: any) {
  ...
  try {
    ...
    view.replaceChildren(pageHead('도메인 맵', '도메인 간 의존과 should·is·debt.', [], '맵'),
      errorNote(e, '도메인 맵을 불러오지 못했습니다'));
    return;
  }
  ...
  const sE: any[] = (data.edges && data.edges.should) || [];
  const iE: any[] = (data.edges && data.edges.is) || [];
  ...
  const head = pageHead('도메인 맵',
    '도메인이 어떤 순서로 의존을 흘려보내는지, 설계 의도(should)와 실제 코드(is)가 어디서 어긋나는지를 한 화면에서 봅니다.',
    [], '맵');
  ...
  function sevClass(e: any) {
    if (e.sev === 'ok') return 'ok';
    if (e.sev === 'is_no_should') return 'viol';
  ...
  function buildControls() {
    ...
    if (repos.length > 1) {
      const s = el('select', { class: 'dmx-repo' }) as HTMLSelectElement;
      for (const r of repos) s.append(el('option', { value: r, text: r }));
      s.value = repo; s.addEventListener('change', () => { state.dmRepo = s.value; location.hash = '#/domainmap?repo=' + encodeURIComponent(s.value); });
      bar.append(el('div', { class: 'dmx-repo-wrap' }, el('span', { class: 'dmx-repo-lbl', text: '레포' }), s));
    }
    return bar;
  }
  function lg(cls: string, txt: string) {
  ...
  function buildGraph() {
    ...
    const hd = el('div', { class: 'dmx-canvas-head' },
    ...
    if (!hasIs && layer !== 'should') {
      wrap.append(el('div', { class: 'dmx-note' },
        el('strong', { text: '실제(is) 의존은 아직 측정 전입니다.' }),
        document.createTextNode(' 코드 스캔(refresh)이 import 를 수집하면 실선과 괴리가 채워집니다. 지금은 의도(should)만 신뢰할 수 있습니다.')));
    }
    ...
    for (const [id, col] of [['ok', 'var(--dmx-ok)'], ['warn', 'var(--dmx-warn)'], ['viol', 'var(--dmx-viol)'], ['pending', 'var(--dmx-pending)']] as any) {
      defs.appendChild(sv('marker', { id: 'dmx-ah-' + id, markerWidth: 8, markerHeight: 8, refX: 6.5, refY: 3, orient: 'auto', markerUnits: 'userSpaceOnUse' },
        sv('path', { d: 'M0,0 L7,3 L0,6 Z', fill: col })));
    }
  ...
  function buildPanel() {
    ...
    const tags = el('div', { class: 'dmx-ptags' },
      d.cross_cutting ? el('span', { class: 'dmx-tag cross', text: '횡단' }) : el('span', { class: 'dmx-tag sp', text: '제품 도메인' }),
      (gaps.length && hasIs) ? el('span', { class: 'dmx-tag debt', text: '괴리 ' + gaps.length }) : null);
    panel.append(el('div', { class: 'dmx-ph' },
      el('div', { class: 'dmx-pk', text: d.key }),
      el('div', { class: 'dmx-pn', text: d.name || d.key }), tags));
    ...
    if (mode === 'dev') {
      if (!det) is.append(el('div', { class: 'dmx-loading', text: '코드 매핑 불러오는 중…' }));
        ...
        for (const u of units.slice(0, 14)) ul.append(el('li', { title: u.path, text: u.path || u.label }));
        is.append(el('div', { class: 'dmx-metric' },
          m(fmtNum(units.length), '코드유닛'), m(fmtNum(d.repos || 0), '레포'), m(fmtNum(outs.filter((e) => e.is).length), 'is 의존')));
        if (units.length) is.append(ul);
        if (units.length > 14) is.append(el('div', { class: 'dmx-more', text: '… 외 ' + fmtNum(units.length - 14) + '개' }));
      }
    } else {
      is.append(el('p', { class: 'dmx-muted' },
        el('strong', { text: fmtNum(d.units || 0) }), document.createTextNode(' 코드유닛'),
    ...
    else for (const e of gaps) {
      const s = e.sev;
      debt.append(el('div', { class: 'dmx-gapline' },
        el('span', { class: 'dmx-gd dmx-g-' + (s === 'is_no_should' ? 'viol' : 'warn') }),
        el('span', {}, el('b', { text: (byId.get(e.from)?.name || e.from) + ' → ' + (byId.get(e.to)?.name || e.to) }),
          document.createTextNode(s === 'should_no_is' ? ' — 선언됐으나 코드 미확인' : ' — 코드에 있으나 선언 없음'))));
    }
    panel.append(debt);
    return panel;
  }
  function m(v: string, label: string) { return el('div', {}, el('b', { text: v }), el('span', { text: label })); }
  ...
  function buildLower() {
    ...
    const h = el('div', { class: 'dmx-lower-head' },
      el('h3', { text: 'should ↔ is 의존 괴리' }),
      el('span', { class: 'dmx-c', text: hasIs ? String(showGap.length) + '건' : 'is 측정 전' }));
    wrap.append(h);
    if (!hasIs) {
      wrap.append(el('p', { class: 'dmx-hint', text: '실제(is) 의존이 수집되면 선언(should)과의 차집합으로 괴리가 여기 쌓입니다. 지금은 의도 뷰만 신뢰하세요.' }));
      return wrap;
    }
    if (!showGap.length) { wrap.append(el('p', { class: 'dmx-hint', text: '선언과 코드가 일치합니다 — 표면화된 괴리 없음.' })); return wrap; }
    const grid = el('div', { class: 'dmx-dgrid' });
    for (const e of showGap) {
      const viol = e.sev === 'is_no_should';
      grid.append(el('div', { class: 'dmx-dcard ' + (viol ? 'viol' : 'warn') },
        el('div', { class: 'dmx-dt' }, el('span', { class: 'dmx-sev', text: e.sev }),
          el('span', { text: (byId.get(e.from)?.name || e.from) + ' → ' + (byId.get(e.to)?.name || e.to) })),
        el('div', { class: 'dmx-why', text: viol ? '코드엔 이 의존이 있는데 경계 선언이 없음 — 문서 안 된 결합(경계 침범 후보).' : '경계엔 선언됐는데 코드 import 가 확인되지 않음 — 의도된 결합이 누락됐거나 다른 도메인을 경유.' })));
    }
    wrap.append(grid);
    return wrap;
  }
