...
function layout(domains: any[], edges: any[]) {
  ...
  const cols = new Map<number, any[]>(); let maxL = 0;
  for (const d of core) { const L = layer.get(d.id) || 0; maxL = Math.max(maxL, L); if (!cols.has(L)) cols.set(L, []); cols.get(L)!.push(d); }
  let maxRows = 0; cols.forEach((arr) => { maxRows = Math.max(maxRows, arr.length); });
  const pos = new Map<number, { x: number; y: number }>();
  const coreH = Math.max(1, maxRows) * ROW_GAP;
  cols.forEach((arr, L) => {
    const colH = arr.length * ROW_GAP; const y0 = PAD + (coreH - colH) / 2;
...
async function renderDomainmap(view: any, params: any) {
  ...
  let data: any;
  try { data = await api('/api/ui/domainmap/map?' + new URLSearchParams({ repo: repo || 'product', limit: '200' })); }
  ...
  const relOf = new Map<string, string>(); const weightOf = new Map<string, number>();
  ...
  function fitAll() {
    ...
    vw.k = k; vw.tx = W / 2 - (LO.width / 2) * k; vw.ty = H / 2 - (LO.height / 2) * k; applyTransform();
  }
  ...
  function buildCanvas() {
    ...
    if (!hasIs && layer !== 'should') canvas.append(el('div', { class: 'dmx-note' },
      el('strong', { text: '실제(is) 의존은 아직 측정 전입니다.' }),
      document.createTextNode(' 코드 스캔(refresh)이 import 를 수집하면 실선과 괴리가 채워집니다.')));
    ...
    svg.appendChild(gViewport); stageInner.append(svg);

    const zc = el('div', { class: 'dmx-zoom-ctrl' });
    zc.append(
      zbtn('+', '확대', () => zoomAt(stageInner.clientWidth / 2, stageInner.clientHeight / 2, 1.3)),
  ...
  function wire(stg: any) {
    ...
    let m: '' | 'pan' | 'node' = '', sx = 0, sy = 0, moved = 0, ptx = 0, pty = 0, downId: number | null = null;
    ...
    stg.addEventListener('pointerdown', (e: PointerEvent) => {
      ...
      if ((e.target as any).closest('.dmx-zoom-ctrl') || (e.target as any).closest('.dmx-panel')) return; // 컨트롤/드로어는 팬 아님
      e.preventDefault();
      const r = stg.getBoundingClientRect(); sx = e.clientX - r.left; sy = e.clientY - r.top; moved = 0;
    ...
    const end = (e: PointerEvent) => {
      if (m === 'node' && downId != null && moved <= 4) select(downId);
      m = ''; downId = null; stg.classList.remove('dragging');
  ...
  function buildPanel() {
    ...
    panel.append(close, el('div', { class: 'dmx-ph' },
      ...
      el('div', { class: 'dmx-pn', text: d.name || d.key }),
      el('div', { class: 'dmx-ptags' },
    ...
    if (!hasIs) debt.append(el('div', { class: 'dmx-muted', text: '실제(is) 측정 전이라 의존 괴리는 아직 판정할 수 없습니다.' }));
      ...
      el('span', {}, el('b', { text: (byId.get(e.from)?.name || e.from) + ' → ' + (byId.get(e.to)?.name || e.to) }),
        document.createTextNode(e.sev === 'should_no_is' ? ' — 선언됐으나 코드 미확인' : ' — 코드에 있으나 선언 없음'))));
    panel.append(debt);
    return panel;
  }
