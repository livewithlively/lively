...
function openGitCredentialManager(scope: 'me' | 'gateway') {
  ...
  const render = (data: any) => {
    ...
    if (!data.encryption_ready) rows.push(el('p', { class: 'gate-error', style: 'margin:0 0 10px', text: '⚠ 서버에 ENCRYPTION_KEY 가 설정되지 않아 자격을 저장할 수 없습니다 — 관리자에게 게이트웨이 env(ENCRYPTION_KEY) 설정을 요청하세요.' }));
    ...
    submit.addEventListener('click', async () => {
      if (!data.encryption_ready) { toast('ENCRYPTION_KEY 미설정 — 저장할 수 없습니다', true); return; }
