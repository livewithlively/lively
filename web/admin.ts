...
async function myLoginsSection(detail) {
  ...
  const head = el('div', { class: 'card' },
    sectionTitle('내 서비스 로그인', 'AI가 **나로서** 아래 서비스들을 쓰게 로그인해 둬요 — 암호화되고 다른 사람에겐 안 보여요. 우리 게이트웨이에 접속하는 [접속 열쇠]와는 다른 거예요.'));
  const svcCard = el('div', { class: 'card' });   // 서비스별 탭이 여기 그려짐
  const gitCard = el('div', { class: 'card' },
    el('h3', { class: 'admin-subhead', text: '레포 접근 (개발자용)' }),
    el('p', { class: 'admin-hint', style: 'margin:0 0 10px', text: '코드 저장소(GitHub·GitLab)를 클론·푸시할 때 쓰는 SSH 키/토큰이에요. 코드를 안 다루면 신경 안 써도 돼요.' }),
