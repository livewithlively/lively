...
function blockMore() {
  const link = (href: string, title: string, desc: string) => el('a', { class: 'docs-linkcard', href },
    el('span', { class: 'docs-linkcard-t' }, el('span', { text: title }),
      el('span', { class: 'docs-linkcard-arrow', 'aria-hidden': 'true', text: '→' })),
    el('span', { class: 'docs-linkcard-d', text: desc }));
  return el('div', { class: 'card docs-card ob-block' },
    el('div', { class: 'card-head ob-block-head' }, el('h2', { text: '③ 더 해보기' })),
    el('p', { class: 'guide-lead', text: 'AI 세션을 열었다면, 이제 이런 것들을 해볼 수 있어요.' }),
    el('div', { class: 'docs-linkgrid' },
      link('#/start/examples', '이런 걸 시켜보세요', 'AI 세션에 무엇을 시킬지 예시 모음'),
      link('#/start/project', '프로젝트 체험', '프로젝트 화면을 예시 프로젝트로 한 바퀴 둘러보기'),
      link('#/start/tour', 'Lively 둘러보기', '실제 화면 위에서 눌러보며 익히는 투어')));
}
