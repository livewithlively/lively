...
const KIND_LABEL: Record<string, string> = { subagent: '서브에이전트', command: '커맨드', hook: '훅', skill: '스킬' };
function kindLabel(k: string) { return KIND_LABEL[k] || '스킬'; }

// 한 줄 = 이름 + 종류 + 상태뱃지 + (선택)토글 버튼.
function harnessRow(name: string, kind: string, tag: { text: string; cls: string } | null, toggle: any, sub?: string) {
  return el('div', { class: 'ob-done-row' },
    ...
    el('span', { class: 'ob-done-tag', text: kindLabel(kind) }),
    tag ? el('span', { class: 'ob-step-badge ' + tag.cls, text: tag.text }) : null,
    toggle || null);
}

// 라이블리 자산 끄기/켜기 — 멤버 단위(me_asset_pref). 모든 머신에 반영.
async function toggleLively(targetKind: string, id: string, turnOn: boolean, view: any) {
...
async function toggleLocal(machineId: string, kind: string, id: string, disable: boolean, view: any) {
...
function toggleBtn(text: string, onclick: () => any) {
...
export async function renderStartHarness(view: any) {
  ...
  const livelyRow = (x: any, targetKind: string, sub?: string) => harnessRow(
    x.label || x.id, x.kind || 'hook',
    x.effective ? null : { text: '꺼짐', cls: 'off' },
    toggleBtn(x.effective ? '끄기' : '켜기', () => toggleLively(targetKind, x.id, !x.effective, view)),
    sub);
  const livelyRows = [
    ...d.lively.skills.map((a: any) => livelyRow(a, 'harness_asset', a.description ? String(a.description).slice(0, 56) : '')),
    ...d.lively.hooks.map((h: any) => livelyRow(h, 'org_hook')),
  ];
  const livelyCard = el('div', { class: 'ob-sec' },
    ...
    el('p', { class: 'ob-step-desc', text: '조직이 관리하는 스킬·훅입니다. 여기서 끄면 내 모든 컴퓨터에 반영됩니다(다음 세션부터).' }),
  ...
  let shadowTotal = 0;
  const machineCards = machines.map((m: any) => {
    const rows = (m.assets || []).map((a: any) => {
    ...
    return el('div', { class: 'ob-sec' },
      el('div', { class: 'ob-sec-head' },
        el('span', { class: 'ob-sec-title', text: title }),
        el('span', { class: 'ob-sec-count', text: String((m.assets || []).length) })),
      fresh ? el('p', { class: 'admin-hint', text: fresh }) : null,
      el('div', { class: 'ob-done-list' }, ...(rows.length ? rows : [el('p', { class: 'admin-hint', text: '내가 직접 만든 로컬 자산은 없습니다(라이블리가 준 것만).' })])));
  });
  const localWrap = machines.length
    ? machineCards
    : [el('div', { class: 'ob-sec' },
      el('div', { class: 'ob-sec-head' }, el('span', { class: 'ob-sec-title', text: '내 컴퓨터에 있는 것' })),
      el('p', { class: 'admin-hint', text: '아직 내 컴퓨터의 하네스를 못 봤습니다. 내 컴퓨터에서 claude(또는 codex)를 한 번 켜면, 다음 세션에 자동으로 여기 나타납니다. 컴퓨터가 여러 대면 각각 따로 보입니다. (웹 [터미널] 세션은 회사 서버라 로컬이 안 보입니다.)' }))];

  // shadow(가림)가 있으면 라이블리 채택 권고 배너.
  const shadowBanner = shadowTotal
    ? el('div', { class: 'ob-banner amber' },
      el('div', { class: 'ob-banner-title', text: `내 로컬 자산 ${shadowTotal}개가 라이블리 것과 이름이 같아 라이블리 배포를 가리고 있어요.` }),
      el('p', { class: 'ob-banner-sub', text: '같은 이름이면 조직이 함께 관리·개선하는 라이블리 쪽을 쓰시길 권합니다(위에서 로컬 것을 끄면 라이블리 것이 살아납니다). 내 것이 더 낫다면 AI 에게 "이 스킬 라이블리에 올리게 정리해줘" 라고 해서 관리자에게 전달하세요.' }))
    : null;

  slot.replaceChildren(...[shadowBanner, livelyCard, ...localWrap].filter(Boolean) as any[]);
}
