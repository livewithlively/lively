...
function toolLabel(name: string): { label: string } {
  if (TOOL_LABELS[name]) return { label: TOOL_LABELS[name] };
  if (name.startsWith('mcp__lively__')) return { label: '라이블리 · ' + name.slice('mcp__lively__'.length) };
...
export interface ProjectChatOpts {
  projectId: number;
  /** 빈 화면(첫 대화 전)에 그릴 것 — 프로젝트 화면이 상태에 맞는 제안 칩을 만든다. */
  opening?: HTMLElement | null;
  /** 입력칸 아래 얇은 바(desktop 변형)에 앉힐 것 */
  bar?: { left?: HTMLElement | null; right?: HTMLElement | null };
  /** 한 턴이 끝났다(성공·실패 무관) — 리브가 프로젝트를 바꿨을 수 있으니 화면이 개요를 다시 읽는 신호. */
  onTurnDone?: () => void;
  /** 대화가 있는지 없는지가 바뀌었다(되그리기 결과·첫 턴·새 대화) — 화면이 빈 상태 안내를 조정하는 데 쓴다. */
  onHasTurns?: (has: boolean) => void;
}
export interface ProjectChatHandle {
  /** 글을 넣고 바로 보낸다(제안 칩·[본문 정리] 버튼이 부른다). 도는 중이면 입력칸에만 남긴다. */
  say(text: string): void;
  /** 이어가던 대화를 놓고 새로 시작한다(다음 턴부터 restart). */
  restart(): void;
  focus(): void;
  destroy(): void;
}

export function mountProjectChat(host: HTMLElement, opts: ProjectChatOpts): ProjectChatHandle {
  const base = `/api/ui/v6/projects/${encodeURIComponent(String(opts.projectId))}/chat`;
  let view: ChatView;
  let running: string | null = null;    // 지금 도는 턴 id — 멈추려면 무엇을 멈출지 알아야 한다
  let stopping = false;
  let restartNext = false;              // 다음 턴을 새 대화로
  let dead = false;                     // destroy 뒤엔 폴링·콜백을 멈춘다(화면을 떠났다)
  let hasTurns = false;

  const setHas = (h: boolean): void => { if (hasTurns !== h) { hasTurns = h; opts.onHasTurns?.(h); } };
  ...
  async function stopTurn(): Promise<void> {
    ...
    if (!r?.stopped) { view.setNote(r?.reason || '멈추지 못했습니다 — 리브가 계속 일하고 있습니다.'); stopping = false; }
  }
  ...
  async function drain(turnId: string, t: ChatTurn, from0 = 0): Promise<void> {
    ...
    for (;;) {
      ...
      let tail: TailResult;
      try {
        tail = await api(`${base}/turn/${encodeURIComponent(turnId)}?from=${from}`) as TailResult;
        fails = 0;
      } catch (e) {
        fails++;
  ...
  async function replayHistory(): Promise<void> {
    ...
    let chat: Chat = null;
    try { chat = ((await api(base)) as { chat: Chat }).chat; } catch { return; }
    if (dead) return;
    const turns = chat?.turns ?? [];
    if (!turns.length) { setHas(false); return; }
    view.removeOpening();
    setHas(true);
    for (const tt of turns) {
      ...
      try { tail = await api(`${base}/turn/${encodeURIComponent(tt.id)}?from=0`) as TailResult; }
  ...
  async function sendTurn(text: string): Promise<void> {
    ...
    try {
      const r = await api(`${base}/turn`, { method: 'POST', body: JSON.stringify({ text, restart: restartNext }) }) as TurnStart;
      restartNext = false;
      ...
      if (!dead) { view.input.focus(); opts.onTurnDone?.(); }
    }
  }
  ...
  return {
    say(text: string): void {
      ...
      if (view.input.disabled) { view.input.focus(); return; }   // 도는 중 — 글만 남기고 사람이 때를 고른다
    ...
    focus(): void { view.input.focus(); },
    destroy(): void { dead = true; view.destroy(); },
  };
}
