...
export interface AnalyzedProject {
  ...
  results?: { worktree_removable?: boolean }[];
}

export interface ReclaimBatches {
  /** 파생물을 지울 폴더(회수할 파생물이 실제로 있는 것만). */
  selFolders: string[];
  /** 워크트리까지 지울 폴더(지금도 '제거 가능' 판정인 것만). */
  wtFolders: string[];
  /** 이번 실행이 건드릴 폴더 전체 = selFolders ∪ wtFolders. */
  actFolders: string[];
  /** remove_worktree=false 로 보낼 묶음. */
  derivedOnly: string[];
  /** 파생물 대상의 회수 가능 바이트 합. */
  selBytes: number;
}
 ...
 */
export function wtRemovable(pr: AnalyzedProject | undefined | null): boolean {
 ...
 */
export function planReclaimBatches(
  selected: Iterable<string>,
  wtSelected: Iterable<string>,
  analyzed: Map<string, AnalyzedProject>,
): ReclaimBatches {
