...
import { DASH_LAYOUT_KEY, DASH_LAYOUT_VER, type DashLayout, dashResetLayout, dashSaveLayout } from './prefs.js';
...
const DASH_WIDGETS = [
  ...
  //   라이블리 로그(#1570 시안 D → #1715 기본 승격): 기본 배치 3열 상단 — 사용자 결정(2026-08-15).
  //    같은 기록(mcp_call_log)을 두 위젯이 다르게 읽는다 — 무엇을 언제 했는지 **시간순 그대로**가 기본이고,
  //    기간 브리핑(lvlog)은 원하는 사람만 꺼낸다(2026-08-07 결정의 반대 방향 — 그때는 브리핑이 기본이었다).
  ...
  //   내 라이블리 사용 내역(#1570 시안 A): 위 라이블리 로그와 같은 기록의 기간 브리핑 — 원하는 사람만 꺼낸다.
...
//  #1715(2026-08-15 사용자 결정): 3열 위를 '라이블리 로그'로 교체 — 브리핑 대신 시간순 기록이 기본.
...
function dashDefaultLayout(): DashLayout {
...
function dashLayout(): DashLayout {
  let raw: any = null;
  try { raw = JSON.parse(localStorage.getItem(DASH_LAYOUT_KEY) || 'null'); } catch { /* 파손된 값은 기본으로 */ }
  if (!raw || !Array.isArray(raw.cols)) return dashDefaultLayout();
  const seen = new Set<string>();
  const take = (arr): string[] => {
    const out: string[] = [];
    for (const k of Array.isArray(arr) ? arr : []) { if (typeof k !== 'string' || !DASH_W[k] || seen.has(k)) continue; seen.add(k); out.push(k); }
    return out;
  };
  const cols: string[][] = [0, 1, 2].map((i) => take(raw.cols[i]));
  ...
  // 구버전 저장값 마이그레이션 — 기본값이 바뀐 판(버전)마다 **그 한 번**만 저장값을 손본다.
  //  demote = 그 판에서 '기본 숨김'이 된 위젯(열에 남아 있으면 숨김으로) · promote = 기본으로 올라온 위젯
  //  (숨김에 남아 있으면 한 번 되꺼낸다 — seen 에서도 빼서 아래 unseen 삽입이 **기본 자리**로 되살리게).
  //  ⚠ 판 번호 이후에 저장한 사람은 건너뛴다(sv >= ver) — 그 사람은 그 판을 이미 겪었고, 지금 열/숨김에 있는
  //   자리는 사람이 직접 정한 것이다. 예전엔 '버전이 다르기만 하면 off 위젯을 전부 숨김으로' 되돌렸는데,
  //   그러면 v4 에서 '검토 대기'를 일부러 꺼내 둔 사람이 판이 오를 때마다 그걸 다시 잃는다(#1715 에서 수정).
  //  되돌림을 겪은 사람이 다시 손대면 그때 최신 판으로 저장되므로, 어느 쪽이든 두 번 일어나지 않는다.
  const sv = Number(raw.v) || 0; // v 없는 초창기 저장값 = 0(모든 이관 대상)
  if (sv < DASH_LAYOUT_VER) {
    const DEMOTED: [number, string][] = [[2, 'task'], [3, 'review'], [5, 'lvlog']];
    // v4(#1570) notif: 알림 개편(#1571)의 과도기 상태(최신 알림→통합 인박스 대체, 곧 원복)를 연 브라우저에
    //  notif 가 hidden 으로 저장돼 "숨긴 적 없는데 사라졌다"가 됐다 — off 였던 적은 없지만 되꺼내기는 같다.
    // v5(#1715) lvlogd: '라이블리 로그'가 3열 상단 기본이 됐다 — 그전엔 기본 숨김이라 hidden 에 박혀 있다.
    const PROMOTED: [number, string][] = [[4, 'notif'], [5, 'lvlogd']];
    for (const [ver, k] of DEMOTED) {
      if (sv >= ver) continue;
      for (let i = 0; i < cols.length; i++) cols[i] = cols[i].filter((x) => x !== k);
      if (seen.has(k) && !hidden.includes(k)) hidden.push(k);
    }
    for (const [ver, k] of PROMOTED) {
      if (sv >= ver) continue;
      const at = hidden.indexOf(k);
      if (at >= 0) { hidden.splice(at, 1); seen.delete(k); } // 열에 있으면(사람이 둔 자리) 그대로 둔다
    }
  }
  for (const w of DASH_WIDGETS) {
    if (seen.has(w.key)) continue;
    if (w.off) { hidden.push(w.key); continue; }
    ...
    if (at < 0) target.push(w.key); else target.splice(at, 0, w.key);
  }
  return { cols, hidden };
}
...
async function renderMyDashboard(view) {
  ...
  for (const keys of layout.cols) for (const k of keys) zones[k] = dashZone(k, DASH_W[k].title);
  ...
  const strip = el('div', { class: 'dash-strip' },
    ...
    el('div', { class: 'dash-acts' },
      ...
      el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '+ 새 프로젝트', onclick: () => openProjectV2Form(() => renderMyDashboard(view), { stay: true }) }),
  ...
  if (colEls.length) {
    ...
    zonesEl.append(el('div', { class: 'dash-allhidden' },
      el('div', { class: 'dash-allhidden-t', text: '표시할 위젯이 없어요.' }),
      el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '대시보드 편집 열기', onclick: () => openDashLayoutModal(() => renderMyDashboard(view)) })));
  }
  ...
  if (!(document.activeElement as any)?.closest?.('.dash-modal-ov')) document.getElementById('view')!.focus?.();
  ...
  if (zones.notif) fillNotifications(zones.notif, projectsP);
  if (zones.proj) fillProjects(zones.proj, (n) => { counts.projects = n; drawSummary(); }, projectsP, listsP);
  if (zones.task) fillMyTasks(zones.task);
  if (zones.sess) fillSessions(zones.sess, (n) => { counts.sessions = n; drawSummary(); }, projectsP);
  ...
  if (zones.lvlogd) fillLivelyLogTimeline(zones.lvlogd);
  if (zones.prev) fillPreviews(zones.prev);
...
async function fillOnboarding(slot) {
  try {
    ...
    if (me && me.status && !me.status.complete) {
      const s = me.status;
      slot.replaceChildren(el('a', { class: 'dash-ob', href: '#/start', title: '시작하기 — 준비 상황 보기' },
        el('span', { text: `시작하기 ${s.done}/${s.total}` }),
...
function dashInitColResize(zonesEl, colKeys: string[][]) {
  ...
  const captureFr = () => {
    const w = kids.map((k) => Math.max(1, (k as any).offsetWidth || 100));
  ...
  const mkHandle = (idx) => {
    ...
    const onUp = () => { if (!dragging) return; dragging = false; document.removeEventListener('pointermove', onMove); document.removeEventListener('pointerup', onUp); document.body.classList.remove('dash-col-resizing'); dashSaveCols(sig, cols); };
    h.addEventListener('pointerdown', (e: any) => { e.preventDefault(); if (!frMode) captureFr(); dragging = true; startX = e.clientX; w0 = cols[idx]; w1 = cols[idx + 1]; document.body.classList.add('dash-col-resizing'); document.addEventListener('pointermove', onMove); document.addEventListener('pointerup', onUp); apply(); });
    // 기본(wide 열 = 3열-최소 px)으로 복귀. v2 도 함께 지운다 — 안 그러면 기본 배치에선 예전 fr 이 되살아난다.
    h.addEventListener('dblclick', () => {
      try { localStorage.removeItem(DASH_COLS_KEY); localStorage.removeItem(DASH_COLS_KEY_V2); } catch { /* */ }
      frMode = false;
    ...
    h.addEventListener('keydown', (e: any) => {
      ...
      if (a >= MIN_FR && b >= MIN_FR) { cols[idx] = a; cols[idx + 1] = b; apply(); dashSaveCols(sig, cols); }
    });
    return h;
  };
  for (let i = 0; i < n - 1; i++) zonesEl.insertBefore(mkHandle(i), kids[i + 1]); // col0 | H0 | col1 | H1 | …
...
function openDashLayoutModal(onApply) {
  ...
  let timer: any = null;
  const scheduleApply = () => { if (timer) clearTimeout(timer); timer = setTimeout(() => { timer = null; onApply(); }, 400); };
  ...
  const card = (k, ci, i) => {
    ...
    c.append(el('div', { class: 'dash-lay-top' }, dashGripIcon(),
      el('span', { class: 'dash-lay-txt' },
        el('span', { class: 'dash-lay-name', text: w.title }),
        el('span', { class: 'dash-lay-desc', text: w.desc }))));
    ...
    if (ci < 0) {
      ...
      const mv = (label, aria, disabled, run) => {
        ...
        if (disabled) b.setAttribute('disabled', 'disabled'); else b.onclick = run;
        return b;
      };
      ctl.append(
        mv('←', '왼쪽 열로', ci === 0, () => { place(k, ci - 1, lay.cols[ci - 1].length); commit(k); }),
        mv('↑', '위로', i === 0, () => { place(k, ci, i - 1); commit(k); }),
        mv('↓', '아래로', i === keys.length - 1, () => { place(k, ci, i + 1); commit(k); }),
        mv('→', '오른쪽 열로', ci === lay.cols.length - 1, () => { place(k, ci + 1, lay.cols[ci + 1].length); commit(k); }));
    ...
    c.addEventListener('keydown', (e: any) => {
      if (ci < 0) { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); show(k); commit(k); } return; }
      const keys = lay.cols[ci];
      if (e.key === 'ArrowLeft' && ci > 0) { e.preventDefault(); place(k, ci - 1, lay.cols[ci - 1].length); commit(k); }
      else if (e.key === 'ArrowRight' && ci < lay.cols.length - 1) { e.preventDefault(); place(k, ci + 1, lay.cols[ci + 1].length); commit(k); }
      else if (e.key === 'ArrowUp' && i > 0) { e.preventDefault(); place(k, ci, i - 1); commit(k); }
      else if (e.key === 'ArrowDown' && i < keys.length - 1) { e.preventDefault(); place(k, ci, i + 1); commit(k); }
      else if (e.key === 'Delete' || e.key === 'Backspace') { e.preventDefault(); hide(k); commit(k); }
    });
    // 드래그 — 카드 위/아래 절반으로 '앞/뒤' 삽입(요약 카드 재정렬 #671 과 같은 결). 열 컨테이너로는 전파하지 않는다.
    c.addEventListener('dragstart', (e: any) => { dragKey = k; try { e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', k); } catch { /* */ } c.classList.add('drag-src'); });
    ...
    c.addEventListener('dragover', (e: any) => {
      ...
      (c.parentElement as any)?.classList.remove('drop-in'); // 카드 위에선 '이 카드 앞/뒤'가 답 — 열 전체 강조는 끈다
    ...
    c.addEventListener('drop', (e: any) => {
      ...
      const src = dragKey; dragKey = null;
      if (!src || src === k) return;
      if (ci < 0) { hide(src); commit(src); return; }
      // 같은 열 안에서 위로 끌어올릴 땐 자기 자신을 뺀 뒤의 인덱스로 보정해야 한 칸씩 밀리지 않는다.
      const from = lay.cols[ci].indexOf(src);
      let at = i + (after ? 1 : 0);
      if (from >= 0 && from < at) at--;
      place(src, ci, at); commit(src);
    });
    return c;
  };
  // 드롭 영역(열·트레이) 공통 — 카드가 아닌 빈 곳에 떨어뜨리면 맨 끝으로.
  const dropZone = (box, onDrop) => {
    box.addEventListener('dragover', (e: any) => { if (!dragKey) return; e.preventDefault(); box.classList.add('drop-in'); });
    box.addEventListener('dragleave', (e: any) => { if (!box.contains(e.relatedTarget)) box.classList.remove('drop-in'); });
    box.addEventListener('drop', (e: any) => { e.preventDefault(); box.classList.remove('drop-in'); const src = dragKey; dragKey = null; if (src) onDrop(src); });
    return box;
  };
  const draw = (focusKey?) => {
    ...
    lay.cols.forEach((keys, ci) => {
      ...
      if (!keys.length) box.append(el('div', { class: 'dash-lay-drop', text: '비어 있어요' }));
    ...
    if (!lay.hidden.length) tray.append(el('div', { class: 'dash-lay-drop', text: '카드를 여기로 끌면 화면에서 숨겨져요' }));
    ...
    body.replaceChildren(
      el('p', { class: 'dash-lay-hint', text: '화면에 보일 위젯과 자리를 정합니다. 카드를 끌어 옮기거나, 카드를 고른 뒤 버튼·방향키(←→ 열 이동 · ↑↓ 순서)를 쓰세요. 열 폭과 높이는 대시보드에서 위젯 사이 경계를 끌어 조절합니다.' }),
      cols,
      el('div', { class: 'dash-lay-th', text: '숨긴 위젯' }),
      tray,
      el('div', { class: 'dash-lay-foot' }, el('span', { class: 'dash-lay-note', text: '바뀐 배치는 이 기기에 저장돼요.' }), reset));
    if (focusKey) (body.querySelector('[data-wkey="' + focusKey + '"]') as any)?.focus?.(); // 옮긴 카드를 계속 조작할 수 있게 포커스 유지
  };
  draw();
...
async function startDashboardSessionTour(returnTo?: string) {
  ...
  const opts = returnTo ? {
    onEnd: async (reason: string) => {
      if (reason === 'complete') {
        try { await api('/api/ui/me/onboarding', { method: 'POST', body: JSON.stringify({ step: 'connect', state: 'done', by: 'self' }) }); }
