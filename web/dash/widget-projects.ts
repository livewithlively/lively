...
// 위젯 한 벌의 지역 상태 — 종전 fillProjects 클로저의 let/const 를 그대로 옮겨 담은 것(이름·의미 무변경).
type ProjCtx = {
  zone: any;
  onCount: (n: number | null) => void;
  projects: any[];              // mine=1 결과(인라인 생성 시 push, 새로고침 시 교체)
  lists: any[];
  listById: Map<number, any>;   // ⚠ 참조 유지 계약(파일 헤더) — 교체 금지, projRefillListById 로 갱신
  favLists: Set<number>;        // ⚙ 팝오버 상단으로 끌어올릴 즐겨찾기 리스트(백그라운드 로드)
  folders: any[];               // ⚙ 트리용 스페이스›폴더(백그라운드 로드)
  foldersLoaded: boolean;
  foldersP: Promise<any[]>;
  ovOpen: Set<number>;          // ⚙ 트리에서 펼쳐 둔 폴더
  ovSeeded: boolean;            // 최초 1회 '켜진 리스트의 조상 펼치기'를 했는가
  allProjects: any[] | null;    // pinned 리스트를 채우기 위한 조직 전체 목록(지연·캐시)
  allLoading: boolean;
  dragListId: any;              // 개요 카드 드래그 재정렬 중인 리스트
  mode: string;                 // 진행 중 | 전체
  selectedListId: any;
  currentOrder: any[];
  autoIds: Set<number>;
  mineIds: Set<number>;
  justCreated: Set<number>;
  draw: () => void;             // ⚠ 안정된 참조(파일 헤더)
  reloadAll: () => Promise<void>;
  setListShown: (id: any, on: boolean) => void;
};
...
function projRefillListById(ctx: ProjCtx) {
  ctx.listById.clear();
  for (const l of ctx.lists) ctx.listById.set(l.id, l);
}
...
function projEnsureAllProjects(ctx: ProjCtx) {
  if (ctx.allProjects || ctx.allLoading || !dashOvPinned().size) return;
  ctx.allLoading = true;
...
function projSetListShown(ctx: ProjCtx, id, on) {
  const h = dashOvHidden(); const p = dashOvPinned(); const n = Number(id);
  if (on) { h.delete(n); if (!ctx.autoIds.has(n)) p.add(n); }
  else { p.delete(n); if (ctx.autoIds.has(n)) h.add(n); }
...
function projRow(ctx: ProjCtx, p) {
  const cell = el('div', { class: 'pjv-trow-title-cell' },
    ...
    dashProjStatusControl(p, ctx.listById, () => ctx.draw()),
    el('span', { class: 'pjv-trow-title clickable' + (projIsDone(p) ? ' done' : ''), title: p.name, text: p.name }));
  ...
  if (chip) {
    ...
    const openT = (e: any) => { e.preventDefault(); e.stopPropagation(); openProjTasksPopover(tb, p, ctx.reloadAll); };
  ...
  sess.addEventListener('click', (e: any) => { e.preventDefault(); e.stopPropagation(); openProjectSessionForm(p.id, ctx.reloadAll, '/api/ui/v6/projects/', p.name); });
  ...
  more.addEventListener('click', (e: any) => { e.preventDefault(); e.stopPropagation(); openProjRowMenu(more, p, ctx.reloadAll); });
...
function sessCleanupNote(r: any): string {
  const s = r && r.sessions; if (!s) return '';
...
function projInlineAdd(ctx: ProjCtx, listId, countEl?) {
  ...
  const commit = (keepOpen, forcedListId?: number) => {
    ...
    const pending = el('div', { class: 'dash-projrow2 dash-projrow-pending' },
      el('div', { class: 'pjv-trow-title-cell' },
        el('span', { class: 'dash-projstatus-btn dash-projstatus-static', 'aria-hidden': 'true' }, dashStatusIconSvg('todo', '#94a3b8', 0)),
        el('span', { class: 'pjv-trow-title', text: name })));
    ...
    (async () => {
      try {
        ...
        ctx.projects.push(p);
        if (pending.isConnected) pending.replaceWith(projRow(ctx, p)); else row.parentNode?.insertBefore(projRow(ctx, p), row);
  ...
  input.addEventListener('keydown', (e: any) => { if (e.key === 'Enter' && !e.isComposing && (e as any).keyCode !== 229) { e.preventDefault(); commit(true); } else if (e.key === 'Escape') { e.preventDefault(); input.value = ''; collapse(); } });
...
function projGroup(ctx: ProjCtx, listId, l, rawAll) {
  const isUn = !listId;
  ...
  fBtn.onclick = (e: any) => { e.stopPropagation(); projOpenListFilter(ctx, fBtn, listId, l, rawAll, paint); };
  paint();
...
function projBlock(ctx: ProjCtx, listId, l, arr) {
  const b = projBrk(arr);
  ...
  const pick = () => { if (ctx.selectedListId !== listId) { ctx.selectedListId = listId; ctx.draw(); } };
  ...
  card.addEventListener('dragstart', (e: any) => { ctx.dragListId = listId; try { e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', String(listId)); } catch { /* */ } card.classList.add('drag-src'); });
  ...
  card.addEventListener('dragover', (e: any) => {
    if (ctx.dragListId == null || ctx.dragListId === listId) return;
    e.preventDefault();
  ...
  card.addEventListener('drop', (e: any) => {
    ...
    clearDrop();
    if (ctx.dragListId == null || ctx.dragListId === listId) return;
    dashReorderList(ctx.currentOrder, ctx.dragListId, listId, after); ctx.draw();
  });
  ...
  hideBtn.addEventListener('click', (e: any) => { e.stopPropagation(); e.preventDefault(); ctx.setListShown(Number(listId), false); ctx.draw(); });
  card.append(hideBtn);
  return card;
}

function projDraw(ctx: ProjCtx) {
  ...
  if (!shown.length && !pinned.size) { ctx.zone.body.replaceChildren(dashEmpty(ctx.mode === 'active' ? '진행 중인 내 프로젝트가 없어요.' : '내가 참여한 프로젝트가 없어요.')); return; }
  ...
  for (const p of ctx.projects) { const k = p.list_id || 0; if (!byListAll.has(k)) byListAll.set(k, []); byListAll.get(k).push(p); }
  ...
  if (ctx.allProjects) {
    for (const id of pinned) { if (Number(id) > 0) { byList.set(Number(id), []); byListAll.set(Number(id), []); } } // 0(미분류)은 pinned 대상이 아니다 — 내 미분류 묶음을 덮지 않게
    for (const p of byMode(ctx.allProjects)) { const k = Number(p.list_id || 0); if (k > 0 && pinned.has(k)) byList.get(k)!.push(p); }
    for (const p of ctx.allProjects) { const k = Number(p.list_id || 0); if (k > 0 && pinned.has(k)) byListAll.get(k)!.push(p); } // 필터용 원본도 동형(모드만 미적용)
  }
  ...
  if (visibleOrder.length) {
    ...
    for (const listId of visibleOrder) gridEl.append(projBlock(ctx, listId, ctx.listById.get(listId), byList.get(listId) || []));
    ...
    gridEl = el('div', { class: 'dash-ov-allhidden' },
      ...
      el('button', { class: 'dash-ov-restore', type: 'button', text: '다시 표시', onclick: () => { dashSaveOvHidden(new Set()); ctx.draw(); } }));
  }
...
async function projReloadAll(ctx: ProjCtx) {
  try {
...
async function fillProjects(zone, onCount, projectsP, listsP) {
  ...
  const ctx = {
    ...
    favLists: new Set<number>(),
    folders: [] as any[],
    foldersLoaded: false,
    ovOpen: new Set<number>(), // ⚙ 트리에서 펼쳐 둔 폴더(팝오버를 닫았다 열어도 유지). 최초 1회 '켜진 리스트의 조상'을 펼친다.
    ovSeeded: false,
    allProjects: null,
    allLoading: false,
    dragListId: null,
    ...
    currentOrder: [] as any[],     // 현재 표시 중인 리스트 순서(드래그 재정렬 기준).
    autoIds: new Set<number>(),    // 내 프로젝트가 있어 '자동으로' 뜨는 리스트(체크 해제 시에만 hidden 에 기록).
    mineIds: new Set<number>(),    // 내가 멤버인 프로젝트 id — 리스트별 필터의 '내 프로젝트만' 판정용.
    justCreated: new Set<number>(), // 이 화면에서 방금 만든 리스트 — 비어도 개요에 뜨게(#762 '만들었는데 안 보임' 방지).
  } as unknown as ProjCtx;
  ...
  api('/api/ui/v6/favorites').then((d) => { ctx.favLists = new Set(((d && d.project_lists) || []).map((x) => Number(x))); }).catch(() => { /* 무시 */ });
  ...
  dashPrefsSync().then((changed) => { if (changed && zone.body.isConnected) ctx.draw(); }); // #1129 서버(계정) 정본 반영 → 필요 시 재렌더
}
...
function openProjRowMenu(anchor, p, onChanged) {
  ...
  del.onclick = async () => {
    ...
    if (!confirm('프로젝트 ‘' + (p.name || '') + '’을(를) 삭제할까요?\n하위 태스크가 함께 사라지고, 이 프로젝트의 AI 세션도 종료됩니다(대화록은 📜 세션 기록에 남아요).')) return;
    try {
      const r: any = await api('/api/ui/v6/projects/' + p.id + '/delete', { method: 'POST', body: '{}' });
