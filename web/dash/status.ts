...
function dashSessState(s): { key: string; label: string; cls: string } {
...
function dashSessRank(s): number { return sessRankShared(s); }
// 'AI 가 더 안 도는' 세션인가 — 예전 key==='offline' 판정을 대체한다(exited·offline·restorable 모두 죽은 것 — #1059 E 포함).
function dashSessDead(s): boolean { return sessIsDead(s); }
...
export {
  ...
  DASH_TASK_STATUS,
  dashTaskStatusIcon,
  dashProjStatusControl,
  dashTaskStatusControl,
};
