...
function dashListFilter(listId): { who: string; q: string; st: string[]; pri: string[]; tags: string[]; due: string } {
...
function dashSaveListFilter(listId, v) {
  ...
  if (!n || !dashListFilterOn(n)) delete all[k]; // 기본값이면 항목째 지워 저장소를 깨끗이
  else all[k] = n;
...
function dashProjFilterStatusKey(p, l) {
  ...
  if (s && s.statusMode === 'custom' && Array.isArray(s.statuses) && s.statuses.length) {
    const d: any = dashResolveStatus(p, dashListStatusDefs(l));
    if (d) return String(d.key);
  }
  if (p.status === 'done' || p.status_category === 'done' || p.status_category === 'closed') return 'done';
...
function dashLocalDay(offsetDays?) {
  const t = new Date(); t.setDate(t.getDate() + (offsetDays || 0));
...
function dashListFilterPreds(lf, l, mineIds): any {
...
function dashSessFilter2(): { src: string[]; state: string[] } {
  try {
    ...
    const src = DASH_SESS_SRC_KEYS.includes(old as string) ? [old as string] : [];
    const state: string[] = [];
    if (src.length) dashSaveSessFilter2({ src, state });
    localStorage.removeItem(DASH_SESS_FILTER_KEY);
    return { src, state };
  } catch { return { src: [], state: [] }; }
}
function dashSaveSessFilter2(f: { src: string[]; state: string[] }) {
  try {
    if (!f.src.length && !f.state.length) localStorage.removeItem(DASH_SESS_FILTER2_KEY);
    else localStorage.setItem(DASH_SESS_FILTER2_KEY, JSON.stringify({ src: f.src, state: f.state }));
  } catch { /* 무시 */ }
}
...
function dashSessSort() { try { const v = localStorage.getItem(DASH_SESS_SORT_KEY); return ['active', 'priority', 'recent', 'name'].includes(v as string) ? v : 'active'; } catch { return 'active'; } }
...
export {
  ...
  dashListFilterOn,
  dashSaveListFilter,
  dashProjFilterStatusKey,
  dashLocalDay,
  dashListFilterPreds,
  DASH_TASKCOUNT_KEY,
  DASH_PROJFILTER_KEY,
  dashTaskCountMode,
  dashSaveTaskCountMode,
  dashProjFilterDefault,
  dashSaveProjFilterDefault,
  dashTaskChip,
  DASH_FOLD_VIEW_KEY,
  DASH_SESS_FILTER_KEY,
  DASH_SESS_FILTER2_KEY,
  DASH_SESS_SHOWCLOSED_KEY,
  DASH_SESS_ONLINEONLY_KEY,
  dashSessOnlineOnly,
  dashSaveSessOnlineOnly,
  DASH_SESS_SORT_KEY,
  DASH_SESS_DENSITY_KEY,
  DASH_LOG_TYPE_KEY,
  dashFoldView,
  dashSaveFoldView,
  DASH_SESS_SRC_KEYS,
  DASH_SESS_STATE_KEYS,
  dashSessFilter2,
  dashSaveSessFilter2,
  dashSessShowClosed,
  dashSaveSessShowClosed,
  dashSessSort,
  dashSaveSessSort,
  dashSessDensity,
  dashSaveSessDensity,
