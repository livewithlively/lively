...
function dashDueDays(due): number | null {
...
function dashDueLabel(n: number | null) {
  if (n == null) return '';
...
async function fillMyTasks(zone) {
  let tasks: any[];
  try { tasks = await api('/api/ui/v6/my-tasks').then((d) => (d && d.tasks) || []); }
  catch (e) { zone.body.replaceChildren(errorNote(e, '내 할 일을 불러오지 못했습니다')); return; }
  let mode = dashTaskFilterDefault();
  // 상태를 바꾸면(완료 처리 등) 목록에서 빠져야 하므로 서버에서 다시 받는다. 실패 시 기존 목록 유지.
  const reload = async () => {
    try { tasks = await api('/api/ui/v6/my-tasks').then((d) => (d && d.tasks) || []); } catch { /* 기존 유지 */ }
    draw();
  };
  const taskRow = (t) => {
    const n = dashDueDays(t.due_date);
    const cell = el('div', { class: 'pjv-trow-title-cell' },
      dashTaskStatusControl(t, reload),
      el('span', { class: 'pjv-trow-title clickable', title: t.name || '', text: t.name || '(제목 없음)' }));
    if (t.due_date) cell.append(el('span', { class: 'dash-task-due' + (n != null && n < 0 ? ' over' : n === 0 ? ' now' : ''), title: '마감 ' + t.due_date, text: dashDueLabel(n) }));
    ...
    if (t.project_name && t.project_id) {
      ...
      const openP = (e: any) => { e.preventDefault(); e.stopPropagation(); pjvOpenProjectModal(Number(t.project_id), reload); };
      pb.addEventListener('click', openP);
      pb.addEventListener('keydown', (e: any) => { if (e.key === 'Enter' || e.key === ' ') openP(e); });
      cell.append(pb);
    }
  ...
  const draw = () => {
    ...
    if (!shown.length) { zone.body.replaceChildren(dashEmpty(mode === 'soon' ? '이번 주 안에 마감인 일이 없어요.' : '나에게 할당된 일이 없어요.')); return; }
    ...
    for (const b of DASH_TASK_BUCKETS) {
      ...
      box.append(el('div', { class: 'dash-task-gh' + (b.key === 'over' ? ' over' : '') },
        el('span', { text: b.label }), el('span', { class: 'dash-task-gn', text: String(arr.length) })));
...
async function fillReviewQueue(zone) {
  ...
  const draw = () => {
    ...
    zone.body.replaceChildren(...shown.map((it) => el('a', { class: 'dash-row dash-rvw', href: '#/knowledge/review', title: it.title },
      ...
      el('span', { class: 'dash-rvw-nm', text: it.title }),
      it.catName ? el('span', { class: 'dash-rvw-cat', text: it.catName }) : null,
