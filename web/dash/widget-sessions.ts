...
import { confirmForceRestore, confirmSessionEnd, notTrashed, retireSessions, retiredToastText } from '../session-actions.js';
...
type SessCtx = {
  ...
  sessions: any[];
  cfg: any;                       // /terminal/config — 라벨·멤버명 보강용(실패 시 null)
  projects: any[];                // mine=1 프로젝트(칩·'내 프로젝트' 판정)
  myProjIds: Set<any>;
  projName: Map<any, string>;
  projListId: Map<any, any>;
  listName: Map<any, string>;
  closedPids: Set<number>;        // 완료·보관 프로젝트 id
  projNamesFilled: boolean;
  fSrc: Set<string>;              // 출처 축 다중선택(빈 집합 = 전체)
  fState: Set<string>;            // 상태 축 다중선택(빈 집합 = 전체)
  showClosed: boolean;
  onlineOnly: boolean;
  sortMode: string;
  density: string;
  fdd: any;                       // ⚠ 계약① — 헤더 필터 칩. **재생성 금지**
  fddLbl: any;                    // ⚠ 계약① — 그 칩 안의 라벨 노드. 여기 textContent 만 갈아낀다
  selected: Set<string>;
  shownNow: any[];                // #1150 지금 필터에 걸려 화면에 있는 세션
  bar: any;                       // 하단 일괄 바(선택이 있을 때만 보임)
  lastSig: string;                // 폴링 재렌더 판정용 상태 서명
  polling: boolean;
  draw: () => void;               // 안정된 참조 — 팝오버·타이머가 콜백으로 들고 있다
  reloadSessions: () => Promise<void>;
};

const sessMemberName = (ctx: SessCtx, id) => {
  const m = ((ctx.cfg && ctx.cfg.members) || []).find((x) => x.id === id);
  return (m && m.name) || id || '';
};
// 프로젝트 → 소속 리스트(영역) 이름(#853) — 카드에 '프로젝트:' 와 나란히 '리스트:' 뱃지로.
const sessProjList = (ctx: SessCtx, pid) => ctx.listName.get(ctx.projListId.get(pid)) || '';
...
function dashSessWhen(s): number { return Number(s && (s.lastActive || s.created)) || 0; }
...
async function sessFillProjNames(ctx: SessCtx) {
  if (ctx.projNamesFilled) return false;
  if (!ctx.sessions.some((s) => Number(s.projectId) > 0 && !ctx.projName.has(Number(s.projectId)))) return false;
  ctx.projNamesFilled = true;
  const all = await api('/api/ui/v6/projects').then((d) => (d && d.projects) || []).catch(() => []);
  if (!all.length) { ctx.projNamesFilled = false; return false; }
  for (const p of all) {
    if (!ctx.projName.has(p.id)) { ctx.projName.set(p.id, p.name); ctx.projListId.set(p.id, p.list_id); }
    if (sessIsProjClosed(p)) ctx.closedPids.add(p.id);
  }
  return true;
}
...
function sessPaintFilterChip(ctx: SessCtx) {
...
function sessOpenSelectedGrid(ctx: SessCtx) {
...
function sessRenderBar(ctx: SessCtx) {
  const bar = ctx.bar;
  // #853 하단 일괄 바 = 선택이 하나라도 있거나(호버 체크로 바로 진입) ⚙ 선택모드일 때 표시. 다 풀면 자동으로 숨는다.
  const n = ctx.selected.size;
  bar.hidden = n === 0; if (bar.hidden) return;   // 선택이 있으면 뜨고 다 풀면 사라진다(별도 '선택 모드' 없음)
  ...
  const restoreBtn: any = el('button', { class: 'dash-bulkbar-btn', type: 'button',
  ...
  const pickBtn: any = el('button', { class: 'dash-bulkbar-btn', type: 'button', 'aria-haspopup': 'true',
  ...
  bar.replaceChildren(
    ...
    restoreBtn,
    delBtn,
    el('button', { class: 'dash-bulkbar-btn', type: 'button', title: '선택 해제', text: '완료', onclick: () => { ctx.selected.clear(); ctx.draw(); } }));
}

function sessDraw(ctx: SessCtx) {
  const zone = ctx.zone;
  ...
  for (const id of [...ctx.selected]) if (!ownedShown.has(id)) ctx.selected.delete(id);
  ...
  if (!shown.length) {
    zone.body.replaceChildren(
      (ctx.fSrc.size || ctx.fState.size) ? dashEmpty('고른 조건에 맞는 세션이 없어요 — 필터에서 ‘초기화’를 눌러 보세요.')
  ...
  for (const s of shown) {
    ...
    const statusEl = stt.label ? el('span', { class: 'dash-scard-status is-' + stt.cls },
    ...
    if (pid) { // #853 팝업이 아니라 프로젝트 페이지로 아예 이동.
      ...
      if (lname && ctx.density !== 'compact') chips.append(el('span', { class: 'dash-scard-list', title: '리스트(영역): ' + lname, text: '리스트: ' + lname }));
    }
    if (!s.owned) chips.append(el('span', { class: 'dash-scard-tag', title: '소유: ' + sessMemberName(ctx, s.owner), text: sessMemberName(ctx, s.owner) + ' · 초대받음' }));
    ...
    const when = el('div', { class: 'dash-scard-when' + (worked ? '' : ' none'), title: worked ? '마지막 작업 — 클로드가 마지막으로 작업한 시각' : '아직 작업 기록이 없어요 (만든 시각)' },
      ctx.density === 'compact' ? null : el('span', { class: 'dash-scard-when-lbl', text: worked ? '마지막 작업' : '만든 시각' }),
    ...
    if (s.owned) { // 소유 세션만 일괄 선택 가능(비소유는 서버가 종료 403).
      ...
      const cb: any = el('input', { type: 'checkbox', class: 'dash-sess-check', 'aria-label': (s.label || '세션') + ' 선택' }); cb.checked = ctx.selected.has(s.id);
      cb.onchange = () => {
        if (cb.checked) ctx.selected.add(s.id); else ctx.selected.delete(s.id);
    ...
    if (ctx.density === 'compact') box.append(main, acts);
  ...
  list.append(el('div', { class: 'dash-sess-addrow' },
    el('button', { class: 'dash-sess-addbtn', type: 'button', 'data-tour': 'new-session', title: '새 세션 만들기',
      onclick: () => { if (ctx.cfg) openTermCreateForm(ctx.cfg, null, () => ctx.reloadSessions()); else location.hash = '#/terminal'; } },
...
async function sessRefetch(ctx: SessCtx) {
  ...
  if (withSess.length) {
    ...
    for (const arr of arrs) for (const x of arr) if (x.owned && !seen.has(x.id)) { seen.add(x.id); ctx.sessions.push(x); }
  }
...
async function sessPollTick(ctx: SessCtx) {
  ...
  try {
    ...
    if (sig !== ctx.lastSig) { ctx.lastSig = sig; const sc = ctx.zone.body.scrollTop; ctx.draw(); ctx.zone.body.scrollTop = sc; }
  } catch { /* 무시 */ } finally { ctx.polling = false; }
}
...
async function fillSessions(zone, onCount, projectsP?) {
  ...
  const fdd: any = el('button', { class: 'dash-chip dash-chip-dd', type: 'button', 'aria-haspopup': 'true', title: '세션 보기 필터' },
    fddLbl, el('span', { class: 'dash-chip-caret', 'aria-hidden': 'true', text: '▾' }));
  const ctx = {
    ...
    myProjIds: new Set<any>((projects || []).map((p) => p.id)),
    projName: new Map<any, string>((projects || []).map((p) => [p.id, p.name])),
    projListId: new Map<any, any>((projects || []).map((p) => [p.id, p.list_id])),
    listName: new Map<any, string>((lists || []).map((l) => [l.id, l.name])),
    closedPids: new Set<number>((projects || []).filter(sessIsProjClosed).map((p) => p.id)),
    projNamesFilled: false,
    fSrc: new Set<string>(saved.src),
    fState: new Set<string>(saved.state),
    ...
    lastSig: '',
    polling: false,
  } as unknown as SessCtx;
  ctx.draw = () => sessDraw(ctx);
  ctx.reloadSessions = async () => { try { await sessRefetch(ctx); await sessFillProjNames(ctx); } catch { /* 유지 */ } ctx.draw(); }; // 세션 변경(이름 수정·삭제·종료) 후 새로고침.
  ...
  sessFillProjNames(ctx).then((changed) => { if (changed) ctx.draw(); }).catch(() => { /* 이름 폴백(#id) 유지 */ });
}
...
function dashOpenSessionTab(id: string, label?: string, nodeId?: string) {
...
export type { SessCtx };
export {
  fillSessions, sessPaintFilterChip, dashSessWhen, dashOpenSessionTab,
  dashTourStep1, dashSessionEmpty,
};
