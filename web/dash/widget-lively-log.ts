...
function lvlHead(label: string, note?: string) {
  return el('div', { class: 'dash-task-gh' }, el('span', { text: label }),
    note ? el('span', { class: 'dash-task-gn', text: note }) : null);
}

// 한 사건 = 한 줄. 왼쪽에 사실(제목·주체), 오른쪽에 횟수/시각.
function lvlRow(main: HTMLElement, right?: string, title?: string) {
  return el('div', { class: 'dash-row dash-lvl-row', title: title || '' }, main,
    right ? el('span', { class: 'dash-lvl-when', text: right }) : null);
}
...
async function fillLivelyLog(zone) {
  ...
  const draw = async () => {
    let d: any;
    try { d = await api('/api/ui/me/lively-log?window=' + encodeURIComponent(win)); }
    ...
    const dbQueries = dbRows.reduce((a: number, r: any) => a + Number(r.queries || 0), 0);
    ...
    if (!lead.length && s.calls) lead.push(`라이블리 도구를 ${n(s.calls)}번 썼어요`);
    ...
    if (reads.length) {
      ...
      for (const r of reads) {
        ...
        const main = el('span', { class: 'dash-lvl-main' },
          el('span', { class: 'dash-lvl-title', text: r.title || r.name }),
          other ? el('span', { class: 'dash-lvl-by', text: other + ' 저작' }) : null);
    ...
    if (used.length) {
      ...
      for (const u of used) {
        const main = el('span', { class: 'dash-lvl-main' },
          el('span', { class: 'dash-lvl-title', text: u.title || u.name }),
          el('span', { class: 'dash-lvl-by', text: `${u.readers}명이` }));
    ...
    if (dbRows.length) {
      ...
      for (const r of dbRows) {
        const main = el('span', { class: 'dash-lvl-main' },
          el('span', { class: 'dash-lvl-title', text: r.source }),
          el('span', { class: 'dash-lvl-by', text: `${n(r.tables)}개 테이블 · ${n(r.rows_read)}행` }));
    ...
    if (harnesses.length > 1) {
      ...
      const main = el('span', { class: 'dash-lvl-main' },
        el('span', { class: 'dash-lvl-title', text: harnesses.map((h: any) => h.harness).join(' · ') }));
