...
//  (lvlog-designs.html) 중 **A 브리핑**과 **D 정돈된 로그**를 둘 다 위젯으로 만들었다.
//   · lvlogd(로그) — KPI 스트립 + 시간순 감사 로그(사람 말 번역·연속 중복 접기). **기본 배치 3열 상단**(#1715).
//   · lvlog(브리핑) — 문장 리드 + 스탯 타일 3 + 활동 구성 스택바 + 많이 기댄 지식. 기본 숨김(편집에서 꺼냄).
//   기본이 A→D 로 바뀐 이유(2026-08-15 사용자 결정): 같은 기록이라면 "무엇을 언제 했는지"가 먼저다 —
//   요약은 근거를 한 건씩 따라갈 수 없지만, 시간순 로그는 훑기도 되고 따라가기도 된다.
//
//  ── 지키는 것 ──
        ...
        (d.pendingReview || []).length
          ? (() => {
              ...
              b.onclick = () => {
                ...
                panel.append(el('div', { class: 'dash-pop-head' }, el('strong', { text: '검토가 필요한 지식' }),
                  el('span', { class: 'dash-pop-sub', text: '승인해야 검색·세션주입에 반영돼요' })));
                for (const it of pend) {
                  panel.append(el('a', { class: 'dash-pop-opt dash-lvl-pop-row', href: '#/k/' + encodeURIComponent(it.name), title: it.name },
                    el('span', { class: 'dash-pop-name', text: it.title || it.name })));
                }
