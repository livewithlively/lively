...
function svg(...kids: any[]) {
  const n = sv('svg', { viewBox: '0 0 24 24', fill: 'none', 'aria-hidden': 'true' });
  n.append(...kids);
  return n;
}
// 선 글리프 공통 — 획 굵기·끝맺음을 한 곳에서 정해 서비스끼리 굵기가 튀지 않게 한다.
const stroked = (d: string, w = 1.9) => sv('path', { d, stroke: 'currentColor', 'stroke-width': w, 'stroke-linecap': 'round', 'stroke-linejoin': 'round' });
const filled = (d: string) => sv('path', { d, fill: 'currentColor' });

// ── Notion — 페이지 테두리 + N. 원 마크와 같은 구성(획 굵은 N 한 글자). ──
const icNotion = () => svg(
  sv('rect', { x: 3.2, y: 2.6, width: 17.6, height: 18.8, rx: 2.6, stroke: 'currentColor', 'stroke-width': 1.9 }),
  stroked('M8.6 17V7.6L15.4 16.4V7', 2.1));

// ── Linear — 라운드 스퀘어 + 좌하단 모서리로 모이는 대각선 세 줄(원 마크의 구성).
//  세 줄을 대각선 **한쪽에만** 둔다 — 양쪽에 두면 빗금 패턴으로 읽혀 Linear 로 안 보인다(실측). ──
const icLinear = () => svg(
  sv('rect', { x: 2.6, y: 2.6, width: 18.8, height: 18.8, rx: 5, stroke: 'currentColor', 'stroke-width': 1.8 }),
  stroked('M3.0 6.2 17.8 21.0', 1.8),
  stroked('M3.0 11.4 12.6 21.0', 1.8),
  stroked('M3.0 16.6 7.4 21.0', 1.8));

// ── Slack — 캡슐 4 + 정사각 4 로 도는 바람개비. 원 마크의 격자(24 기준) 그대로. ──
function icSlack() {
  const n = svg();
  const cap = (x: number, y: number, w: number, h: number) => sv('rect', { x, y, width: w, height: h, rx: 2.52, fill: 'currentColor' });
  n.append(
    cap(0, 6.31, 11.36, 5.04), cap(12.65, 0, 5.04, 11.36),      // 좌상 가로 · 우상 세로
...
// ── Gmail — 봉투 + M 자 접힘. ──
const icGmail = () => svg(
  sv('rect', { x: 2.2, y: 4.9, width: 19.6, height: 14.2, rx: 2.4, stroke: 'currentColor', 'stroke-width': 1.9 }),
  stroked('M2.9 6.4 12 13.2 21.1 6.4'),
  stroked('M2.5 18.6V7.2M21.5 18.6V7.2', 1.9));

// ── Google Drive — 삼각형 + 중심에서 갈라지는 Y(원 마크의 3분할). ──
const icDrive = () => svg(
  stroked('M12 3.4 22 20.6H2Z', 1.9),
  stroked('M12 3.6V14.2', 1.7), stroked('M12 14.2 3.2 20.4', 1.7), stroked('M12 14.2 20.8 20.4', 1.7));

// ── Google 캘린더 — 달력 시트 + 31. 숫자가 이 브랜드의 식별자다. ──
function icCalendar() {
  const n = svg(sv('rect', { x: 3.2, y: 4.4, width: 17.6, height: 16.4, rx: 2.6, stroke: 'currentColor', 'stroke-width': 1.9 }),
    stroked('M8 2.9v3.4M16 2.9v3.4', 1.9), stroked('M3.4 9.4h17.2', 1.6));
...
// ── GitHub — 옥토캣 실루엣(원 마크). ──
const icGithub = () => svg(filled(
  'M12 .8a11.2 11.2 0 0 0-3.54 21.83c.56.1.77-.24.77-.54l-.01-1.9c-3.12.68-3.78-1.5-3.78-1.5-.51-1.3-1.25-1.65-1.25-1.65-1.02-.7.08-.68.08-.68 1.13.08 1.72 1.16 1.72 1.16 1 1.72 2.63 1.22 3.27.93.1-.72.39-1.22.71-1.5-2.49-.29-5.11-1.25-5.11-5.55 0-1.23.44-2.23 1.16-3.02-.12-.28-.5-1.42.11-2.97 0 0 .94-.3 3.09 1.15a10.7 10.7 0 0 1 5.62 0c2.14-1.45 3.08-1.15 3.08-1.15.61 1.55.23 2.69.11 2.97.72.79 1.15 1.79 1.15 3.02 0 4.31-2.62 5.26-5.12 5.54.4.35.76 1.03.76 2.08l-.01 3.08c0 .3.2.65.77.54A11.2 11.2 0 0 0 12 .8z'));
...
const icClickup = () => svg(
  filled('M2.8 18.4 12 10.6l9.2 7.8-2.9 3.4L12 15.9l-6.3 5.9Z'),
  filled('M12 2.2 17.4 8.6H6.6Z'));
...
function icFigma() {
  const n = svg();
  const g = 0.5;                                  // 조각 사이 틈
  const left = (y: number) => filled(`M12 ${y + g}H9.25a2.75 2.75 0 0 0 0 5.5H12Z`);
  n.append(left(3), left(9), left(15),
    filled(`M12 ${3 + g}h2.75a2.75 2.75 0 0 1 0 5.5H12Z`),
    sv('circle', { cx: 15, cy: 12, r: 2.75, fill: 'currentColor' }));
  return n;
}

// ── Prometheus — 횃불(불꽃 + 손잡이 띠). ──
const icPrometheus = () => svg(
  filled('M12 2.2c3.5 3.3 5.3 5.8 5.3 8.8a5.3 5.3 0 0 1-10.6 0c0-1.8.7-3.3 2-4.8.2 1.5.9 2.3 1.7 2.5-.2-2.4.3-4.4 1.6-6.5z'),
  sv('rect', { x: 7.4, y: 17.9, width: 9.2, height: 3.6, rx: 1.3, fill: 'currentColor' }));
...
const SVC_MARK: Record<string, () => any> = {
