...
function audPeriodQs(st: any, key: string, relDays: Record<string, number>): { since: string | null; until: string | null } {
  if (st[key] === AUD_CUSTOM) return { since: audDayStart(st.from), until: audDayEnd(st.to) };
  return { since: audSinceDays(relDays[st[key]] || 0), until: null };
}

// 필터바용 기간 컨트롤. opts=[[value,label],…](상대 기간). 상태는 st[key] + st.from/st.to 에 적는다.
function audPeriodField(opts: Array<[string, string]>, st: any, key: string, onchange: () => void) {
  const sel = audSelect([...opts, [AUD_CUSTOM, '직접 지정'] as [string, string]], st[key] || opts[0][0], (v) => {
    st[key] = v;
    // 「직접 지정」을 처음 고르면 오늘 하루로 채워 둔다 — 빈 칸 두 개보다 바로 쓸 수 있다.
    if (v === AUD_CUSTOM && !st.from && !st.to) { const t = audToday(); st.from = t; st.to = t; }
    onchange();
  });
  if (st[key] !== AUD_CUSTOM) return audField('기간', sel);
  ...
  fromIn.onchange = () => { if (!toIn.value) toIn.value = fromIn.value; apply(); };
  toIn.onchange = apply;
  return el('div', { class: 'aud-f' },
    el('label', { text: '기간' }),
    el('div', { class: 'aud-daterange' }, sel, fromIn, el('span', { class: 'aud-dash', text: '~' }), toIn));
}
...
async function audExportCsv(kind: string, filters: URLSearchParams) {
  const q = new URLSearchParams(filters); q.set('kind', kind);
  toast('CSV 준비 중…');
  let plan: any;
  try { plan = await api('/api/ui/audit-export/plan?' + q.toString()); }
  catch (e: any) { toast('CSV 준비 실패 — ' + e.message, true); return; }
  const total = plan.total || 0;
  if (!total) { toast('이 조건으로 내려받을 기록이 없습니다'); return; }
  ...
  if (parts > 1 && !confirm(total.toLocaleString() + '행입니다 — 엑셀 한 시트에 담기지 않아 파일 ' + parts + '개로 나눠 받습니다.\n'
      + '브라우저가 「여러 파일 다운로드」를 물으면 허용해 주세요. 계속할까요?')) return;
  if (plan.snapshot_id) q.set('snapshot_id', String(plan.snapshot_id));
  for (let p = 1; p <= parts; p++) {
    const q2 = new URLSearchParams(q); q2.set('part', String(p));
    await audDownload('/api/ui/audit-export.csv?' + q2.toString(), base + (parts > 1 ? '-part' + p + 'of' + parts : '') + '.csv');
    if (p < parts) await new Promise((r) => setTimeout(r, 800)); // 연속 트리거를 브라우저가 묶어 취소하지 않게 간격
  }
...
async function audDownload(url: string, filename: string) {
  ...
  let res: any;
  try { res = await fetch(appUrl(url), { headers: { Authorization: 'Bearer ' + token } }); }
  catch (e: any) { toast('다운로드 실패 — ' + e.message, true); return; }
...
function audExportBtn(onclick: () => void) {
