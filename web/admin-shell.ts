...
const ADMIN_SECTIONS = [
  ...
  // AI 개인화 편집도 좌하단 내 프로필 창 [AI 개인화] 탭으로 옮겨 갔다(#1843·#1898) — 여기는 계정(로그인)만 남는다.
...
const PERSONAL_HIDDEN = [
  // [조직 정보](#1898) — 매니지드에서는 세 칸이 전부 **CP 소유이거나 갈라진 입구**다:
  //  · 게이트웨이 주소 — CP 가 프로비저닝에 심고 wake·5분 틱마다 재단언한다(lvly-cloud assertTenantGatewayUrl).
  //    여기서 고쳐도 5분 안에 되돌아오고, 그 사이 CLI 승인 링크·OAuth 콜백이 어긋난다(#1771 이 그 고장이었다).
  //  · 조직 표시명 — 매니지드의 조직 이름 정본은 CP 의 workspaces.name 이고 그게 org_profile.name 으로 동기된다.
  //    이 칸이 쓰는 display_name 은 CP 가 한 번도 안 건드리는데 표시 우선순위는 display_name ?? name 이라,
  //    여기서 한 번 쓰면 CP 쪽 이름 변경이 영영 화면에 안 뜬다(입구가 둘로 갈린다).
  //  · 조직 시간대 — CP 가 Asia/Seoul 을 심고 코어 폴백도 Asia/Seoul 이라 국내 조직은 만질 일이 없다.
  //    해외 조직이 생기면 그 테넌트만 ui_profile=full 로 열거나 CP 가 값을 넣는다.
  'profile',           // 조직 정보(표시명 · 게이트웨이 주소 · 시간대)
  // [화면](#1898) — 매니지드는 CP 가 모든 테넌트에 ui_mode=v2 를 민다(재프로비저닝마다 다시 민다). 그래서
  //  이 화면의 '조직 기본'을 classic 으로 내려도 다음 재프로비저닝에 조용히 되돌아간다 — 지켜지지 않는
  //  선택지를 보여주는 자리다(#2200 이 "정책을 코드가 조용히 되돌리는 것"을 금한 그 형태). '이 브라우저에서만'은
  //  새 셸의 [나] 창에 이미 있고, 클래식에서 돌아오는 버튼은 [내 정보] 창에 있다(me-profile shellBackRow).
  'ui',                // 화면(셸 선택)
...
function sectionHidden(key, data) {
  // [내 AI 계정] — 새 셸에서는 **내 프로필 창 [AI 계정 연결] 탭**이 그 자리다(#1898, 원준 2026-08-25).
  //  개인 설정을 보러 그 창에 들어왔는데 계정 로그인만 설정 앱으로 내보내면, 고치러 갔다가 보던 화면을 잃는다.
  //  ⚠ 지우지 않는 이유는 아래 [외부 서비스 관리]와 같다 — 클래식 화면에는 그 창이 없다(유일한 자리).
  if (key === 'me-ai' && uiMode() === 'v2') return true;
  // [외부 서비스 관리] — 새 셸에서는 **사이드바 [외부 앱 연결]** 이 그 자리다(#1719, 원준 2026-08-21).
  //  (#1898 이 내 프로필 창에도 같은 것을 폈었으나 2026-09-03 에 뺐다 — 입구는 [외부 앱 연결] 하나다.)
