...
function keepNote(fate: LogFate, policy: SessionLogPolicy | null): string {
  ...
  if (fate === 'some') return `${keep} 대화 기록은 수집이 켜진 AI(하네스) 세션만 📜 「세션 기록」에 남아 💬 「이어 질문하기」로 이어받을 수 있어요 — 나머지는 이 박스 안에만 남습니다.${days}`;
...
export async function confirmProjectTrash(opts: { name: string; sessN: number; liveN: number; othersLive: number }): Promise<boolean> {
  const bits = [`프로젝트와 그 안의 내 세션 ${opts.sessN}개가 함께 휴지통으로 갑니다.`];
  if (opts.liveN > 0) bits.push(`그중 돌고 있는 ${opts.liveN}개는 그 자리에서 멈춥니다.`);
  return confirmDialog({
