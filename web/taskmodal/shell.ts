...
let _pjvTmEntryY: number | null = null;
...
function pjvOpenTaskModal(taskId, pageReload) {
  ...
  if (!_pjvTmUrl) _pjvTmEntryY = pjvScrollTopNow();   // 첫 진입에서만 — 드릴인은 이 값을 이어받는다(#1233-1)
  if (_pjvTmUrl) {
  ...
  function closeModal(mode: PjvTmClose = 'user') {
    ...
    if (mode !== 'swap') _pjvTmEntryY = null;   // 스택을 완전히 빠져나갈 때만 버린다(드릴인은 유지)
    // 주소 되돌리기(#810) — 우리가 넣은 항목을 pop 하면 뒤에 있던 주소로 돌아간다(프로젝트 모달 · 프로젝트 페이지 · 보드).
    let popped = false;
    let retHash: string | null = null;   // 돌아갈 주소 — 아래에서 '그 페이지는 이미 그려져 있다'고 라우터에 알릴 때 쓴다
    if (mode === 'user') {
      if (_pjvTmUrl) { retHash = _pjvTmUrl.ret; _pjvTmUrl = null; try { history.back(); popped = true; } catch (_) { /* noop */ } }
    } else if (mode !== 'swap') {
    ...
    if (routerWillRedraw) {
      ...
      if (dirty && pageReload) pjvReloadKeepScroll(pageReload);
    ...
    if (mode !== 'swap') pjvRestoreScroll(backY);
  }
