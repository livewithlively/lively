...
import { api, busy, cardHead, el, keepSideScroll, relTime, replaceKids, toast } from './core.js';
...
function editorPage(d, isNew: boolean): HTMLElement {
  ...
  async function save() {
    const body = collect();
    if (!body.key) { showStep('basic'); toast('식별자(key)가 필요합니다', true); (keyIn as HTMLInputElement).focus(); return; }
