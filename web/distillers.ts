...
function editorCard(d, rerender) {
  ...
  const group = (title, ...kids) => el('div', { style: 'margin:20px 0 0;padding-top:14px;border-top:1px solid var(--line,#e5e7eb)' },
    el('h3', { class: 'ps-block-title', style: 'margin:0 0 12px', text: title }), ...kids);
  ...
  previewBtn.addEventListener('click', async () => {
    if (isNew) { toast('먼저 저장한 뒤 확인할 수 있어요', true); return; }
    ...
    try {
      ...
      for (const s of (r.sample || [])) {
        rows.append(el('div', { class: 'mini-meta' },
          el('span', { class: 'pill', text: String(s.channel || s.kind) }),
          el('span', { text: ' ' + String(s.title || '(제목 없음)').slice(0, 80) })));
      }
      previewBox.replaceChildren(
        el('div', { class: 'mini-meta' },
          el('span', { class: 'pill', text: '지금 맡은 자료 ' + Number(r.backlog).toLocaleString() + '건' }),
          r.reviewed ? el('span', { class: 'pill', text: '이미 판정 ' + Number(r.reviewed).toLocaleString() }) : null),
  ...
  card.append(
    ...
    F('우선순위', '높을수록 자료를 먼저 가져갑니다. 겹치는 자료는 우선순위가 높은 증류기 하나만 처리합니다. 낮은 값 + 넓은 범위 = 나머지를 받는 기본 라인.', prioIn),
    ...
    group('① 무엇을 집을까 — 범위',
      ...
      F('제외 채널', '알림봇·모니터링 채널처럼 지식화할 게 없는 곳을 빼세요.', excCh),
    ...
    group('② 얼마나 걸러낼까 — 사전 필터',
      el('p', { class: 'admin-hint', style: 'margin:0 0 10px' },
        el('span', { text: 'AI에게 보내기 ' }), el('b', { text: '전에' }),
        el('span', { text: ' 서버가 스레드를 걸러냅니다. 비워두면 필터가 꺼집니다(전부 보냄). ' }),
        el('b', { text: '⚠ 세게 걸수록 지식을 놓칩니다' }),
        el('span', { text: ' — 실측에서 4축을 모두 요구했더니(AND) 이미 지식이 된 스레드의 21%가 탈락했습니다. 결합은 OR 을 권합니다.' })),
      el('p', { class: 'admin-hint', style: 'margin:0 0 12px;padding:8px 10px;border-radius:6px;background:var(--bg-soft,#f6f7f9)' },
        ...
        el('span', { text: 'AI에게 "이 증류기 사전 필터를 튜닝해줘"라고 하면 실측으로 정합니다 — 이미 지식이 된 스레드가 몇 %나 걸러지는지(유실률), 이 채널에서 판별력 높은 단어가 무엇인지, 조합별 절감 대비 유실이 얼마인지를 계산해 최적값을 넣어줍니다.' })),
      ...
      fold('참여자·메시지 수 조건 (신중히)',
        ...
        F('최소 참여자 수', '', rAut),
        F('최소 메시지 수', '', rMsg)),
      F('조건 결합', 'OR 권장. AND 는 모든 축을 만족해야 해서 값진 스레드를 많이 버립니다.', rMatch)),

    group('③ 무엇을 지식으로 만들까 — 기준과 형식',
    ...
    group('④ 어떻게 돌릴까 — 실행',
      ...
      F('한 배치 메시지 상한', '스레드를 최근순으로 담다가 이 수를 넘으면 멈춥니다. 토큰이 메시지 수의 제곱으로 늘어나니 작을수록 쌉니다. ⚠ 첫 스레드는 예외 — 상한보다 커도 통째로 담습니다(대화를 자르지 않으려고). 20~40 권장.', batchMsgIn),
