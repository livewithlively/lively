...
export function renderSessionListInto(container: any, sessions: SessRow[], emptyMsg: string): void {
  if (!sessions.length) { container.replaceChildren(el('p', { class: 'admin-hint', text: emptyMsg })); return; }
  container.replaceChildren(...sessions.map(sessionRowEl));
}

function sessionRowEl(s: SessRow): any {
  ...
  const list = el('div', { style: 'display:none;margin-top:6px;padding-left:4px;border-left:2px solid rgba(127,127,127,.25)' },
    ...qs.map((q) => {
      const a = el('a', { href: 'javascript:void 0', class: 'admin-hint',
        ...
        text: `${q.i + 1}. ${q.text}` });
      a.addEventListener('click', (e: any) => { e.preventDefault(); document.getElementById('turn-' + q.i)?.scrollIntoView({ behavior: 'smooth', block: 'start' }); });
      return a;
    }));
...
function userBubble(text: string): any {
  return el('div', { style: 'background:rgba(37,99,235,.10);border-left:3px solid #2563eb;border-radius:6px;padding:8px 12px;margin:8px 0' },
    el('div', { style: 'font-weight:700;font-size:11px;color:#2563eb;margin-bottom:2px', text: '나' }),
    el('div', { style: 'white-space:pre-wrap;font-size:14px', text: text }));
}
