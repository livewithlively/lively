...
function el(tag: string, attrs?: any, ...kids: any[]): any {
  ...
  if ((tag === 'input' || tag === 'textarea') && !n.hasAttribute('autocomplete')) {
    ...
    n.setAttribute('data-1p-ignore', ''); n.setAttribute('data-lpignore', 'true');
    n.setAttribute('data-bwignore', 'true'); n.setAttribute('data-protonpass-ignore', 'true');
  }
...
let alpha = 0, running = false, dragNode: any = null;
...
function loop() { force(); updateGeometry(); paint(); if (alpha > ALPHA_MIN) requestAnimationFrame(loop); else running = false; }
...
function openDetail(name: string) {
  ...
  try { url = new URL('.', location.href).href + url; } catch (_) { /* 상대경로 폴백 */ }
