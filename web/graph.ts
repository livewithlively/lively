...
export {}; // 이 파일을 ES 모듈로(전역 스코프 오염·식별자 충돌 방지). graph.html 이 type="module" 로 로드.

const TOKEN_KEY = 'lively_ui_token';
const SVG_NS = 'http://www.w3.org/2000/svg';

// ── 미니 DOM 헬퍼(core.js 와 동형, 자족) ──
function el(tag: string, attrs?: any, ...kids: any[]): any {
  const n: any = document.createElement(tag);
  if (attrs) for (const [k, v] of Object.entries<any>(attrs)) {
    if (v == null) continue;
...
async function api(path: string): Promise<any> {
  ...
  if (res.status === 401) { const e: any = new Error('인증이 필요합니다'); e.status = 401; throw e; }
  let data: any = null; try { data = await res.json(); } catch (_) { /* noop */ }
  if (!res.ok) { const e: any = new Error((data && data.error) || ('요청 실패 (' + res.status + ')')); e.status = res.status; throw e; }
  return data;
}
...
function hashColor(s: string): string { // 미래 신규 카테고리용 결정적 색
  let h = 0; for (let i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) >>> 0;
  return `hsl(${h % 360} 62% 60%)`;
}
const catColor = (key: string) => key && CAT_COLOR[key] || (key ? hashColor(key) : '#94A3B8');
...
const rad = (count: number) => 24 + Math.sqrt(count) * 15;          // 영역 반경(노드 수)
const nodeR = (deg: number) => 4.5 + Math.sqrt(deg) * 2.6;          // 노드 반경(연결차수)
const regionFont = (count: number) => clamp(13 + Math.sqrt(count) * 2.4, 14, 30);
...
async function main() {
  let data: any;
  try { data = await api('/api/ui/knowledge-graph?limit=1000'); }
  catch (e) { renderState(e && e.status === 401 ? 'auth' : 'error', e); return; }
  const nodes = (data && data.nodes) || [], edges = (data && data.edges) || [];
  if (!nodes.length) { renderState('empty'); return; }
  build(nodes, edges);
}
...
function build(nodes: any[], edges: any[]) {
  ...
  for (const e of edges) {
    ...
    a.deg++; b.deg++;
    if (!adj.has(a.name)) adj.set(a.name, new Set());
    if (!adj.has(b.name)) adj.set(b.name, new Set());
    adj.get(a.name)!.add(b.name); adj.get(b.name)!.add(a.name);
  }
  ...
  if (reducedMotion()) { for (let i = 0; i < 140; i++) force(); updateGeometry(); fitInitial(); paint(); }
...
function layoutAnchors() {
  ...
  for (let i = 0; i < n; i++) { const s = rad(CATS[i].nodes.length) + rad(CATS[(i + 1) % n].nodes.length) + 90; segs.push(s); total += s; }
  const R = Math.max(340, total / (Math.PI * 2));
  let acc = 0;
  for (let i = 0; i < n; i++) {
    const ang = (acc / total) * Math.PI * 2 - Math.PI / 2;
    CATS[i].ax = Math.cos(ang) * R; CATS[i].ay = Math.sin(ang) * R;
    acc += segs[i];
  }
}
...
function initPositions() {
  for (const c of CATS) {
    ...
    arr.forEach((nd: any, i: number) => {
      const a = i * 2.399963229728653;            // 황금각
      const rr = base * Math.sqrt((i + 0.5) / arr.length);
      nd.ax = c.ax; nd.ay = c.ay;                  // 앵커(인력 목표)
      nd.x = c.ax + Math.cos(a) * rr; nd.y = c.ay + Math.sin(a) * rr;
      nd.vx = 0; nd.vy = 0;
    });
  }
}
...
function force() {
  const a = alpha;
  // 앵커 인력(영역 중력)
  for (const n of NODES) { n.vx += (n.ax - n.x) * GRAV * a; n.vy += (n.ay - n.y) * GRAV * a; }
  ...
  for (let i = 0; i < NODES.length; i++) {
    ...
    for (let j = i + 1; j < NODES.length; j++) {
      ...
      if (d2 > REP_CUT2 || d2 === 0) { if (d2 === 0) { dx = (i - j) * 0.01 + 0.1; dy = 0.1; d2 = dx * dx + dy * dy; } else continue; }
      const d = Math.sqrt(d2), m = REP * a / d2, ux = dx / d, uy = dy / d;
      ni.vx -= ux * m; ni.vy -= uy * m; nj.vx += ux * m; nj.vy += uy * m;
    }
  }
  // 링크 스프링
  ...
  // 적분 + 마찰(드래그 중 노드는 고정)
  for (const n of NODES) {
    if (n === dragNode) { n.x = n.fx; n.y = n.fy; n.vx = 0; n.vy = 0; continue; }
  ...
  for (let it = 0; it < COLL_ITERS; it++) {
    for (let i = 0; i < NODES.length; i++) {
      ...
      for (let j = i + 1; j < NODES.length; j++) {
        ...
        if (ni !== dragNode) { ni.x -= ux * push; ni.y -= uy * push; }
        if (nj !== dragNode) { nj.x += ux * push; nj.y += uy * push; }
      }
    }
  }
...
function updateGeometry() {
  for (const n of NODES) { n.elDot.setAttribute('cx', n.x.toFixed(1)); n.elDot.setAttribute('cy', n.y.toFixed(1)); }
  for (const l of LINKS) { l.elLine.setAttribute('x1', l.a.x.toFixed(1)); l.elLine.setAttribute('y1', l.a.y.toFixed(1)); l.elLine.setAttribute('x2', l.b.x.toFixed(1)); l.elLine.setAttribute('y2', l.b.y.toFixed(1)); }
  for (const c of CATS) {
    let cx = 0, cy = 0; for (const n of c.nodes) { cx += n.x; cy += n.y; } cx /= c.nodes.length; cy /= c.nodes.length;
    c.cx = cx; c.cy = cy;
    let d: string | null = null, topY = cy;
    if (c.nodes.length >= 3) {
      let h = convexHull(c.nodes);
      if (h.length >= 3) {
        h = h.map((v: any) => { const dx = v.x - cx, dy = v.y - cy, L = Math.hypot(dx, dy) || 1; return { x: v.x + dx / L * TERR_PAD, y: v.y + dy / L * TERR_PAD }; });
        d = smoothClosed(h); topY = Math.min(...h.map((v: any) => v.y));
      }
    }
    if (!d) { let r = TERR_PAD + 8; for (const n of c.nodes) r = Math.max(r, Math.hypot(n.x - cx, n.y - cy) + TERR_PAD); d = circlePath(cx, cy, r); topY = cy - r; }
    c.topY = topY;
    c.elHalo.setAttribute('d', d); c.elFill.setAttribute('d', d);
  }
}
...
function paint() {
  ...
  const placed: number[][] = [];
  const overlaps = (b: number[]) => placed.some((p) => b[0] < p[2] && b[2] > p[0] && b[1] < p[3] && b[3] > p[1]);
  const labelBox = (n: any, fontPx: number) => {
    const [sx, sy] = toScreen(n.x, n.y); const off = n.r * view.k + 5;
    const w = Math.min(n.title.length, 22) * fontPx * 0.62 + 4;
    return [sx + off, sy - fontPx * 0.7, sx + off + w, sy + fontPx * 0.7];
  };
  const placeLabel = (n: any, strong: boolean) => {
    const fontPx = strong ? 13 : 12;
    const b = labelBox(n, fontPx);
    const lab = n.elLab;
    if (b[2] > 0 && b[0] < W && b[3] > 0 && b[1] < H && !overlaps(b)) {
      const [sx, sy] = toScreen(n.x, n.y); const off = n.r * view.k + 5;
      lab.setAttribute('x', (sx + off).toFixed(1)); lab.setAttribute('y', (sy + fontPx * 0.36).toFixed(1));
      lab.setAttribute('font-size', String(fontPx));
      lab.classList.add('show'); lab.classList.toggle('strong', strong); lab.classList.remove('dim');
      placed.push(b); return true;
    }
    lab.classList.remove('show', 'strong'); return false;
  };
  for (const n of NODES) n.elLab.classList.remove('show', 'strong', 'dim');

  // 1) 강제표시(호버 집합) — 항상, 우선 배치.
  if (hoverSet) for (const n of NODES) if (hoverSet.has(n.name)) placeLabel(n, n.name === hoverName);
  // 2) 검색 매치.
  if (query) { for (const n of NODES) if (n._match && !(hoverSet && hoverSet.has(n.name))) placeLabel(n, true); }
  // 3) 일반 — 줌 충분 시. 우선순위: solo 대상 > 차수 > 영역 큰 순.
    ...
    let shown = 0;
    for (const n of cand) { if (shown >= cap) break; if (placeLabel(n, false)) shown++; }
  }
  // dim 상태 노드의 라벨은 가리기.
  for (const n of NODES) if (n.elNd.classList.contains('dim')) n.elLab.classList.remove('show', 'strong');

  const pct = document.getElementById('atlas-pct'); if (pct) pct.textContent = Math.round(view.k * 100) + '%';
}

// ── 렌더 루프 ──
function ensureRunning() { if (!running) { running = true; raf = requestAnimationFrame(loop); } }
...
function buildScene() {
  ...
  for (const n of NODES) {
    ...
    n.elNd = sv('g', { class: 'nd' }); n.elNd.append(n.elDot); gNodes.append(n.elNd);
    n.elLab = sv('text', { class: 'nlab', text: n.title.length > 24 ? n.title.slice(0, 23) + '…' : n.title }); gNlab.append(n.elLab);
    n.elNd.addEventListener('mouseenter', () => { if (!dragNode) { hoverName = n.name; paint(); } });
    n.elNd.addEventListener('mouseleave', () => { if (hoverName === n.name && !dragNode) { hoverName = null; paint(); } });
  }
  ...
  for (const sp of SPACE_ORDER) {
    ...
    for (const c of list) {
      const rowBtn = el('button', { class: 'leg-row', type: 'button', 'data-cat': c.key },
        ...
        el('span', { class: 'leg-count', text: String(c.nodes.length) }));
      rowBtn.addEventListener('click', () => {
        soloCat = soloCat === c.key ? null : c.key;
        for (const b of box.querySelectorAll('.leg-row')) {
          (b as any).classList.toggle('solo', (b as any).dataset.cat === soloCat);
          (b as any).classList.toggle('off', !!soloCat && (b as any).dataset.cat !== soloCat);
        }
        if (soloCat) fitToBox(c.ax - rad(c.nodes.length) * 1.6, c.ay - rad(c.nodes.length) * 1.6, c.ax + rad(c.nodes.length) * 1.6, c.ay + rad(c.nodes.length) * 1.6, true);
        paint();
      });
      grp.append(rowBtn);
    }
...
function zoomBox() {
  ...
  const cluster = el('div', { class: 'zoom-cluster' },
    el('button', { type: 'button', 'aria-label': '확대', title: '확대', text: '+', onclick: () => zoomBy(1.25) }),
    el('button', { type: 'button', 'aria-label': '축소', title: '축소', text: '−', onclick: () => zoomBy(1 / 1.25) }),
    el('button', { class: 'zoom-fit', type: 'button', title: '전체 맞춤', text: '맞춤', onclick: () => fit(true) }));
...
function zoomBy(factor: number) { zoomAt(root.clientWidth / 2, root.clientHeight / 2, factor); }
function centerOn(n: any, k?: number) {
  if (k) view.k = clamp(k, K_MIN, K_MAX);
  view.tx = root.clientWidth / 2 - n.x * view.k; view.ty = root.clientHeight / 2 - n.y * view.k;
}
function bbox(pts: { x: number; y: number; pad?: number }[]) {
  let x0 = Infinity, y0 = Infinity, x1 = -Infinity, y1 = -Infinity;
  for (const p of pts) { const pad = p.pad || 0; x0 = Math.min(x0, p.x - pad); y0 = Math.min(y0, p.y - pad); x1 = Math.max(x1, p.x + pad); y1 = Math.max(y1, p.y + pad); }
  return { x0, y0, x1, y1 };
}
function fitToBox(x0: number, y0: number, x1: number, y1: number, animate?: boolean) {
  ...
  if (animate && !reducedMotion()) animateView(target); else { view.k = target.k; view.tx = target.tx; view.ty = target.ty; paint(); }
}
function fit(animate?: boolean) { const b = bbox(NODES.map((n) => ({ x: n.x, y: n.y, pad: n.r + 16 }))); fitToBox(b.x0, b.y0, b.x1, b.y1, animate); }
function fitInitial() { const b = bbox(CATS.map((c) => ({ x: c.ax, y: c.ay, pad: rad(c.nodes.length) + 56 }))); fitToBox(b.x0, b.y0, b.x1, b.y1, false); }
function animateView(target: any) {
  const s0 = { ...view }, t0 = performance.now(), dur = 420;
  const ease = (t: number) => 1 - Math.pow(1 - t, 3);
  const step = (now: number) => {
    const p = clamp((now - t0) / dur, 0, 1), e = ease(p);
    view.k = s0.k + (target.k - s0.k) * e; view.tx = s0.tx + (target.tx - s0.tx) * e; view.ty = s0.ty + (target.ty - s0.ty) * e;
...
function wire() {
  ...
  let mode: '' | 'pan' | 'node' = '', sx = 0, sy = 0, moved = 0, panTx = 0, panTy = 0, downNode: any = null;
  const findNode = (t: any) => { const g = t.closest && t.closest('.nd'); if (!g) return null; return NODES.find((n) => n.elNd === g) || null; };

  stage.addEventListener('pointerdown', (e: PointerEvent) => {
    if (e.button !== 0) return;
    const r = stage.getBoundingClientRect(); sx = e.clientX - r.left; sy = e.clientY - r.top; moved = 0;
  ...
  stage.addEventListener('pointermove', (e: PointerEvent) => {
    ...
    if (mode === 'pan') { view.tx = panTx + (cx - sx); view.ty = panTy + (cy - sy); paint(); }
    else if (mode === 'node') {
      if (!dragNode && moved > 4) { dragNode = downNode; hoverName = downNode.name; }
      if (dragNode) { dragNode.fx = (cx - view.tx) / view.k; dragNode.fy = (cy - view.ty) / view.k; reheat(0.4); }
    }
  });
  const end = (e: PointerEvent) => {
    if (mode === 'node' && downNode && moved <= 4) openDetail(downNode.name);   // 클릭(거의 안 움직임)
    if (dragNode) { dragNode.fx = undefined; dragNode.fy = undefined; }
    mode = ''; dragNode = null; downNode = null; stage.classList.remove('dragging');
    try { stage.releasePointerCapture(e.pointerId); } catch (_) { /* noop */ }
  };
  stage.addEventListener('pointerup', end); stage.addEventListener('pointercancel', end);

  window.addEventListener('keydown', (e: KeyboardEvent) => {
    const inSearch = document.activeElement === (window as any).__atlasSearch;
    if (e.key === '/' && !inSearch) { e.preventDefault(); (window as any).__atlasSearch.focus(); }
    else if (e.key === 'Escape') { if (inSearch) { (window as any).__atlasSearch.value = ''; (window as any).__atlasSearch.dispatchEvent(new Event('input')); (window as any).__atlasSearch.blur(); } }
    else if (!inSearch && (e.key === '+' || e.key === '=')) { e.preventDefault(); zoomBy(1.25); }
    else if (!inSearch && (e.key === '-' || e.key === '_')) { e.preventDefault(); zoomBy(1 / 1.25); }
    else if (!inSearch && e.key === '0') { e.preventDefault(); fit(true); }
  });
  let rT = 0; window.addEventListener('resize', () => { clearTimeout(rT); rT = window.setTimeout(paint, 80) as any; });
}

function openDetail(name: string) {
  const hash = '#/k/' + encodeURIComponent(name);
  try { if (window.opener && !window.opener.closed) { window.opener.location.hash = hash; window.opener.focus(); return; } } catch (_) { /* cross-origin/closed */ }
  try { const dir = new URL('.', location.href).href; window.open(dir + hash, '_blank'); } catch (_) { /* noop */ }
}

// ── 기하 유틸 ──
function convexHull(nodes: any[]) {
...
function smoothClosed(pts: any[]): string | null {
...
// ── 상태 화면(인증·빈·오류) ──
function renderState(kind: 'auth' | 'empty' | 'error', e?: any) {
  const card = el('div', { class: 'atlas-state-card' });
  if (kind === 'auth') {
    card.append(el('div', { class: 'ic', text: '🔒' }), el('h2', { text: '로그인이 필요합니다' }),
      el('p', { text: '원래 창(WIKI)에서 로그인한 뒤 다시 「지식 그래프」를 열어 주세요.' }),
      el('div', { class: 'row' }, el('button', { class: 'atlas-btn', type: 'button', text: '다시 시도', onclick: () => location.reload() }),
        el('button', { class: 'atlas-btn', type: 'button', text: '닫기', onclick: () => window.close() })));
  } else if (kind === 'empty') {
    card.append(el('div', { class: 'ic', text: '🗺️' }), el('h2', { text: '아직 그릴 지식이 없습니다' }),
      el('p', { text: 'WIKI에서 지식을 추가하면 도메인 영토로 이 지도에 나타납니다.' }),
      el('div', { class: 'row' }, el('button', { class: 'atlas-btn', type: 'button', text: '닫기', onclick: () => window.close() })));
  } else {
    card.append(el('div', { class: 'ic', text: '⚠️' }), el('h2', { text: '지도를 불러오지 못했습니다' }),
      el('p', { text: (e && e.message) || '알 수 없는 오류' }),
      el('div', { class: 'row' }, el('button', { class: 'atlas-btn', type: 'button', text: '다시 시도', onclick: () => location.reload() })));
  }
