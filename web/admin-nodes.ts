...
function nodeGroup(title, hint, nodes, rowFn, emptyText) {
  ...
  if (!nodes.length) rows.append(el('div', { class: 'wikicat-empty', text: emptyText }));
...
export async function nodesPanel(detail, data) {
  ...
  const rotate = async (n) => {
    ...
    if (!ok) return;
    try { await api('/api/ui/nodes/' + encodeURIComponent(n.id) + '/rotate', { method: 'POST', body: '{}' });
      toast('접속 열쇠를 다시 발급했습니다 — 그 컴퓨터에서 lively node --daemon 을 다시 실행하세요'); reload();
  ...
  const manageActs = (n) => el('div', { class: 'wikicat-row-acts' },
    admin ? el('button', { class: 'btn btn-ghost btn-sm', text: n.shared ? '공유 해제' : '공유로 지정',
      title: n.shared ? '다시 연결한 사람만 쓰도록 되돌립니다.' : '조직 구성원 전체가 이 컴퓨터를 쓸 수 있게 합니다.',
      onclick: () => setShared(n, !n.shared) }) : null,
    el('button', { class: 'btn btn-ghost btn-sm', text: n.enabled === false ? '연결 허용' : '연결 차단',
      ...
      onclick: () => setEnabled(n, n.enabled === false) }),
    el('button', { class: 'btn btn-ghost btn-sm', text: '접속 열쇠 재발급', onclick: () => rotate(n) }),
    el('button', { class: 'btn btn-ghost btn-sm', text: '지우기', onclick: () => remove(n) }));
  ...
  const connect = el('div', { class: 'card' },
    psBlock('내 컴퓨터 연결하기',
      '연결할 컴퓨터(macOS·Linux)에서 아래 세 줄을 차례로 실행하세요. 설치가 끝나면 이 목록에 나타납니다.',
      el('div', {},
        ...
        el('div', { class: 'admin-actions' },
          ...
          el('span', { class: 'admin-hint', style: 'margin:0 0 0 10px' },
            'tmux 가 없으면 자동으로 설치합니다. 연결은 그 컴퓨터에서 라이블리 쪽으로만 나가므로 공유기·방화벽 설정은 필요하지 않습니다.')))));

  // ── 무엇인지 설명 (처음 쓰는 사람용) ──
  const explain = el('div', { class: 'card' },
    psBlock('노드가 무엇인가요?',
      '라이블리에 연결한 컴퓨터를 노드라고 부릅니다. 내 노트북일 수도 있고, 팀이 함께 쓰는 서버일 수도 있습니다.', null),
    psBlock('연결하면 무엇을 할 수 있나요?',
      '첫째, 웹 브라우저에서 그 컴퓨터의 AI 세션을 열 수 있습니다 — 사무실 컴퓨터를 집에서 이어 쓰는 식입니다. '
      + '둘째, 오래 걸리는 작업(전체 빌드·대량 테스트 같은 것)을 그 컴퓨터에 맡길 수 있습니다. 맡긴 동안 내 컴퓨터는 느려지지 않습니다.', null),
    psBlock('누가 쓸 수 있나요?',
      '기본은 연결한 사람만 씁니다 — 남이 내 노트북에서 무언가를 실행할 수 없습니다. '
      + '여럿이 함께 쓰는 서버라면 관리자가 그 컴퓨터를 공유로 지정하고, 그때부터 조직 전체가 함께 씁니다.', null));

  detail.replaceChildren(...[
    sectionHead('컴퓨터(노드) · ' + nodes.length + '대',
      '라이블리에 연결한 컴퓨터를 보고 관리합니다. 연결하면 그 컴퓨터에서 AI 세션을 열고, 오래 걸리는 작업을 맡길 수 있습니다.'),
    el('div', { class: 'admin-stack' }, ...[
      explain,
      connect,
      el('div', { class: 'card' },
        el('div', { class: 'wikicat' }, ...[
          nodeGroup('내 컴퓨터', '내가 연결한 컴퓨터입니다. 기본적으로 나만 쓸 수 있습니다.',
            mine, (n) => nodeRow(n, null, manageActs(n)),
            '아직 연결한 컴퓨터가 없습니다 — 위 세 줄을 그 컴퓨터에서 실행하면 여기에 나타납니다.'),
          nodeGroup('조직 공유 컴퓨터', '관리자가 공유로 지정해 구성원 누구나 쓸 수 있는 컴퓨터입니다.',
            sharedOthers, (n) => nodeRow(n, memberName(n.owner_member), sharedActs(n)),
            '아직 공유로 지정된 컴퓨터가 없습니다.'),
          admin ? nodeGroup('다른 구성원의 컴퓨터',
            '구성원이 각자 연결한 개인 컴퓨터입니다. 공유로 지정하기 전에는 연결한 본인만 씁니다.',
            privateOthers, (n) => nodeRow(n, memberName(n.owner_member), manageActs(n)),
            '다른 구성원이 연결한 컴퓨터가 없습니다.') : null,
        ].filter(Boolean))),
      admin ? null : el('p', { class: 'admin-hint' },
        '공유 컴퓨터로 지정하는 것은 관리자만 할 수 있습니다 — 조직 전체가 쓸 컴퓨터가 필요하면 관리자에게 요청하세요.'),
    ].filter(Boolean)),
  ].filter(Boolean));
}
