...
function gateCard(preset: any, obs: any, reload: () => void) {
  ...
  return el('div', { class: 'rq-gate' },
    ...
    el('div', { class: 'rq-gate-opts' },
      el('label', {}, el('span', { class: 'rq-opt-label', text: '새 지식을 쓸 때' }), createSel),
      el('label', {}, el('span', { class: 'rq-opt-label', text: '기존 지식을 고칠 때' }), updSel)),
    hintBits.length
...
export async function reviewQueuePanel(detail, data): Promise<void> {
  ...
  for (const [k, n] of cats) catSel.append(el('option', { value: k, text: n }));
  ...
  const stats = obs ? el('div', { class: 'rq-stats' },
    ...
    stat(obs.approved + obs.rev_approved, '최근 승인', obs.days + '일 내 승인'),
    stat(obs.rejected + obs.rev_rejected, '최근 반려', obs.days + '일 내 반려·되돌리기'),
