...
export function createKeys(ctx: EditorCtx) {
  ...
  root.addEventListener('input', (e: any) => {
    ...
    if (type === 'p') {
      ...
      if ((m = /^(\d+)[.)]\s$/.exec(txt))) { convertBlock(block, { type: 'numbered', indent: 0, start: Number(m[1]), text: '' }); return; }
      ...
      let lm;
      if (/^\[( |x)?\]\s$/.test(txt) && type !== 'todo') { convertBlock(block, { type: 'todo', indent, checked: /x/.test(txt), text: '' }); return; }
      if (/^([-*+])\s$/.test(txt) && type !== 'bullet') { convertBlock(block, { type: 'bullet', indent, text: '' }); return; }
      if ((lm = /^(\d+)[.)]\s$/.exec(txt))) {
        if (type !== 'numbered') { convertBlock(block, { type: 'numbered', indent, start: Number(lm[1]), text: '' }); return; }
        ...
        const n = Number(lm[1]);
        target.textContent = '';
        if (n > 1) block.dataset.start = String(n); else delete block.dataset.start;
        renumber();
        markDirty();
