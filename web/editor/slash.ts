...
export function createSlash(ctx: EditorCtx) {
  ...
  const openRawEditor = (block: HTMLElement, wrap: HTMLElement) => ctx.openRawEditor(block, wrap);
  const promptPageCard = (block: HTMLElement) => ctx.promptPageCard(block);
  const promptImage = (block: HTMLElement) => ctx.promptImage(block);
  const pickAndUploadFile = (block: HTMLElement, asImage: boolean) => ctx.pickAndUploadFile(block, asImage);
  const insertLinkAtBlock = (block: HTMLElement, bookmark: boolean) => ctx.insertLinkAtBlock(block, bookmark);
  const wrapOrInsertInline = (tag: string) => ctx.wrapOrInsertInline(tag);
  ...
  function paintSlash() {
    ...
    if (st.slash.sel >= items.length) st.slash.sel = Math.max(0, items.length - 1);
    if (!items.length) { st.slash.menu.replaceChildren(el('div', { class: 'be-slash-empty', text: '결과 없음' })); return; }
    const kids: any[] = [];
    let lastCat = '';
    items.forEach((it, idx) => {
      ...
      const row = el('button', { class: 'be-slash-item' + (idx === st.slash.sel ? ' on' : ''), type: 'button', role: 'menuitem' },
    ...
    const onEl = st.slash.menu.querySelector('.be-slash-item.on') as HTMLElement;
  ...
  function syncSlashQuery() {
    ...
    if (/\s/.test(st.slash.query)) { closeSlashMenu(); return; }   // 공백 = 메뉴 포기(노션 동일)
    paintSlash();
