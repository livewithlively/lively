...
import type { TableAlign, TableModel } from '../lib/table-md.js';
import { inlineDomToMd } from './serialize.js';

const ALIGN_CSS: Record<string, string> = { l: 'left', c: 'center', r: 'right' };
...
export function buildTableEditor(md: string, hooks: { onEdit: (md: string) => void; onStructure: (md: string) => void }): HTMLElement | null {
  const parsed = parseTableMd(md);
  if (!parsed) return null;
  const model: TableModel = parsed;

  const wrap = el('div', { class: 'be-tablewrap' });
  const emit = (structural: boolean) => (structural ? hooks.onStructure : hooks.onEdit)(tableToMd(model));

  // 셀 좌표: r = -1 이 헤더 행. 포커스 셀을 기억해 '행/열 삭제'가 어디를 지울지 정한다.
  let focus = { r: 0, c: 0 };

  function cellMd(r: number, c: number): string { return r < 0 ? (model.head[c] || '') : ((model.rows[r] || [])[c] || ''); }
  function setCellMd(r: number, c: number, v: string) { if (r < 0) model.head[c] = v; else model.rows[r][c] = v; }

  function makeCell(r: number, c: number): HTMLElement {
    const tag = r < 0 ? 'th' : 'td';
    const cell = el(tag, { class: 'be-tcell', contenteditable: 'true', spellcheck: 'false' }) as HTMLElement;
    cell.dataset.r = String(r);
    cell.dataset.c = String(c);
    if (ALIGN_CSS[model.align[c]]) cell.style.textAlign = ALIGN_CSS[model.align[c]];
    for (const n of renderInline(cellMd(r, c))) cell.append(n);
    cell.addEventListener('focus', () => { focus = { r, c }; wrap.dataset.col = String(c); });
    cell.addEventListener('input', (e: any) => {
    ...
    cell.addEventListener('paste', (e: any) => {
    ...
    cell.addEventListener('keydown', (e: any) => {
      ...
      if (!((e.metaKey || e.ctrlKey) && (k === 's' || k === 'z' || k === 'y'))) e.stopPropagation();
      if (e.key === 'Tab') { e.preventDefault(); step(e.shiftKey ? -1 : 1); return; }
      if (e.key === 'Enter') {
        e.preventDefault();
        if (r >= 0 && r === model.rows.length - 1) { addRow(); return; }   // 마지막 행에서 Enter = 행 추가
        moveTo(r + 1, c);
        return;
      }
  ...
  function paint() {
    ...
    const htr = el('tr');
    for (let c = 0; c < model.align.length; c++) htr.append(makeCell(-1, c));
    thead.append(htr);
    table.append(thead);
    if (headerEmpty) table.classList.add('be-table-nohead');
    const tbody = el('tbody');
    for (let r = 0; r < model.rows.length; r++) {
      const tr = el('tr');
      for (let c = 0; c < model.align.length; c++) tr.append(makeCell(r, c));
      tbody.append(tr);
    }
    table.append(tbody);
  ...
  function step(dir: number) {
    ...
    if (c < 0) { c = cols - 1; r--; }
    if (r < -1) return;
    if (r >= model.rows.length) { addRow(); return; }
    moveTo(r, c);
  }
  ...
  function addCol() {
    model.head.push('');
    model.align.push('');
    for (const r of model.rows) r.push('');
    emit(true);
    paint();
    moveTo(focus.r, model.align.length - 1);
  }
  function delRow() {
    if (focus.r < 0 || model.rows.length <= 1) return;   // 헤더와 마지막 한 행은 남긴다
  ...
  function delCol() {
    if (model.align.length <= 1) return;
    const c = focus.c;
    model.head.splice(c, 1);
    model.align.splice(c, 1);
    for (const r of model.rows) r.splice(c, 1);
    emit(true);
    paint();
    moveTo(focus.r, Math.min(c, model.align.length - 1));
  }
  function cycleAlign() {
    const order: TableAlign[] = ['', 'c', 'r', 'l'];
    const c = focus.c;
  ...
  function controls() {
    ...
    const btn = (label: string, title: string, fn: () => void) => {
      ...
      b.addEventListener('mousedown', (e: any) => e.preventDefault());
      b.addEventListener('click', (e: any) => { e.preventDefault(); e.stopPropagation(); fn(); });
      return b;
    };
    bar.append(
      btn('＋행', '아래에 행 추가 (표 끝에서 Tab·Enter 도 같은 동작)', addRow),
      btn('＋열', '오른쪽에 열 추가', addCol),
      btn('행 삭제', '커서가 있는 행 삭제', delRow),
      btn('열 삭제', '커서가 있는 열 삭제', delCol),
      btn('정렬', '커서가 있는 열의 정렬 바꾸기 (기본 → 가운데 → 오른쪽 → 왼쪽)', cycleAlign),
    );
    return bar;
  }
