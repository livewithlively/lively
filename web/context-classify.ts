...
export async function renderClassifiers(host: HTMLElement): Promise<void> {
  ...
  if (!list.length && !creating) {
    ...
    body.append(el('div', { class: 'card ctx-empty' },
      el('p', { class: 'ctx-empty-t', text: '분류기 없음 — 전역 기본 분류로 돌고 있습니다' }),
      el('p', { class: 'admin-hint', text: '전 미분류 지식을 한 기준으로 봅니다(설정할 것이 없는 종전 동작). 팀·도메인마다 기준을 나누고 대상·모델·주기를 따로 주려면 분류기를 만드세요.' })));
  }
...
async function classifyJobCard(rerender: () => void): Promise<HTMLElement> {
  ...
  if (!job.enabled) {
    ...
    on.addEventListener('click', async () => {
      ...
      try { await api('/api/ui/cron', { method: 'POST', body: JSON.stringify({ id: job.id, enabled: true }) }); toast('분류 잡을 켰습니다'); rerender(); }
