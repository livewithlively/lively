...
function wkOriginTitle(e: any): string {
  ...
  if (e.updated_at) bits.push('갱신 ' + absTime(e.updated_at));
  if (e.created_at && String(e.created_at).slice(0, 16) !== String(e.updated_at).slice(0, 16)) bits.push('만듦 ' + absTime(e.created_at));
  return bits.join(' · ');
}
...
function wkDeck(md: string, cap = 150): string {
  ...
  for (const raw of lines) {
    ...
    if (/^#{1,6}\s/.test(line2) || /^[-*+]\s|^\d+[.)]\s|^\||^!\[|^---+$|^\$\$/.test(line2)) { if (buf.length) break; continue; }
    buf.push(line2);
