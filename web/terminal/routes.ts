...
import {
  ...
  buildSessAxisFilter, saveTsessCollapsed, saveTsessFilter, sessDead, sessState,
  tsessCollapsed, tsessConfirmEnd, tsessFilter, tsessPeriodMatch, tsessScopeOf, ymdShort,
...
async function renderTerminal(view) {
  ...
  const fScope = new Set<string>(filt.scope);
  const fPeriod: TsessPeriod = { ...filt.period };
  ...
  function repaintBulk() {
    ...
    bulkBar.replaceChildren(
      ...
      ...pickBtns, openBtn, restoreBtn, endBtn,
  ...
  function draw(reopen: '' | 'state' | 'scope' | 'period' | 'proj' = '') {
    let projDDRef: any = null;
    ...
    const stateDD = buildSessAxisFilter({
      ...
      onChange: (next: Set<string>) => { fState.clear(); for (const k of next) fState.add(k); persist(); draw('state'); },
    });
    ...
    const scopeDD = buildSessAxisFilter({
      ...
      onChange: (next: Set<string>) => { fScope.clear(); for (const k of next) fScope.add(k); persist(); draw('scope'); },
    });
    ...
    const buildPeriodFooter = () => {
      ...
      const mk = (which: 'from' | 'to') => {
        const i: any = el('input', { type: 'date', class: 'pjv-date-input tsess-period-date', value: fPeriod[which] || '', title: which === 'from' ? '시작 날짜' : '끝 날짜' });
        i.onchange = () => {
          fPeriod[which] = i.value || '';
          fPeriod.preset = (fPeriod.from || fPeriod.to) ? 'range' : 'all';
          persist(); draw('period');
        };
        i.onmousedown = (e: any) => e.stopPropagation();   // 달력 클릭이 바깥클릭으로 안 잡히게
        return i;
      };
      ...
      if (fPeriod.from || fPeriod.to) {
        ...
        clr.onmousedown = (e: any) => { e.preventDefault(); e.stopPropagation(); fPeriod.from = ''; fPeriod.to = ''; fPeriod.preset = 'all'; persist(); draw('period'); };
        box.append(clr);
      }
      return box;
    };
    ...
    const buildPeriodBasis = () => {
      ...
      for (const [k, label, hint] of [['last', '마지막 작업', '클로드가 마지막으로 작업한 시각(없으면 만든 시각)'], ['created', '만든 날', '세션을 만든 시각']] as [TsessBasis, string, string][]) {
        const b = el('button', { class: 'tsess-period-basisbtn' + (fPeriod.basis === k ? ' on' : ''), type: 'button', text: label, title: hint });
        b.onmousedown = (e: any) => { e.preventDefault(); e.stopPropagation(); if (fPeriod.basis === k) return; fPeriod.basis = k; persist(); draw('period'); };
        seg.append(b);
      }
      return seg;
    };
    ...
    for (const id of [...projF]) if (!projIds.includes(id)) projF.delete(id);   // 세션이 사라진 프로젝트는 선택 해제
    if (projIds.length) {
      const projDD = buildSessProjFilter({
        projects: projects || [], projIds, projName, lists: projLists || [], folders: projFolders || [], selected: projF,
        onChange: (next: Set<number>) => { projF.clear(); for (const v of next) projF.add(v); draw('proj'); },
      });
    ...
    if (reopen === 'state') stateDD.open();
    else if (reopen === 'scope') scopeDD.open();
    else if (reopen === 'period') periodDD.open();
    else if (reopen === 'proj' && projDDRef) projDDRef.open();
    ...
    if (sel.ids.size) { const vis = new Set(rendered.map((s) => s.id)); for (const id of [...sel.ids]) if (!vis.has(id)) sel.ids.delete(id); }
  ...
  async function bulkRestore(btn) {
    const items = pickedRestorable(); if (!items.length) return;
