...
function openTermCreateForm(cfg, view, onCreated?, opts?: { project?: { id: any; name?: string; base?: string } }) {
  ...
  const modeField = el('div', { 'data-tour': 'mode', style: 'flex:1 1 150px;min-width:0' }, fieldInfo('라이블리 모드',
    '이 세션이 라이블리(회사 맥락)를 얼마나 쓸지 정합니다.\n\n· **일반** — 읽고 씁니다.\n· **읽기전용** — 읽기만 하고 쓰지 않습니다. 기밀 작업에 씁니다.\n· **인코그니토** — 라이블리를 아예 쓰지 않습니다. 클린룸이 필요할 때 씁니다.',
    modeSel));
  ...
  const writeVisSel = el('select', { class: 'term-input' },
  ...
  const writeVisField = el('div', { style: 'flex:1 1 150px;min-width:0' }, fieldInfo('기록 범위',
  ...
  const advBody = el('div', { class: 'term-adv-body', hidden: '' },
    el('div', { class: 'term-adv-row' },
      el('div', { 'data-tour': 'node', style: 'flex:1 1 150px;min-width:0' }, fieldInfo('실행 위치',
