...
function tsessConfirmEnd(title, extraLines: string[] = [], sessions?: Array<{ harness?: string }>) {
...
const TSESS_SCOPE_OPTS: Array<{ key: string; label: string; hint: string; match: (s) => boolean }> = [
...
function tsessCollapsed(): Set<string> {
  try {
    const raw = JSON.parse(localStorage.getItem(TSESS_SECT_KEY) || '[]');
    return new Set<string>((Array.isArray(raw) ? raw : []).filter((k) => TSESS_SECTIONS.some((s) => s.key === k)));
  } catch { return new Set<string>(); }
}
function saveTsessCollapsed(c: Set<string>) {
  try { if (c.size) localStorage.setItem(TSESS_SECT_KEY, JSON.stringify([...c])); else localStorage.removeItem(TSESS_SECT_KEY); } catch { /* noop */ }
}
// 기간 — 프리셋 + **하루 단위 직접 지정**(달력). 기준(basis)은 사용자가 고른다:
//  last=마지막 작업(기본, 없으면 만든 시각으로 폴백) · created=만든 날. 둘은 실제로 다르다 —
//  7/20 에 만들어 7/25 까지 쓴 세션은 '만든 날'로는 7/20, '마지막 작업'으로는 7/25 에 잡힌다.
//  ⚠ 기준은 프리셋에도 함께 걸린다(기준을 '만든 날'로 두면 '오늘' = 오늘 만든 세션).
type TsessBasis = 'last' | 'created';
function sessBasisTime(s, basis: TsessBasis): number {
  return Number(basis === 'created' ? s.created : (s.lastActive || s.created)) || 0;
}
const TSESS_PERIOD_OPTS: Array<{ key: string; label: string; hint: string; match: (s, t: number) => boolean }> = [
...
function startOfDayMs(d?: Date): number { const x = d ? new Date(d) : new Date(); x.setHours(0, 0, 0, 0); return x.getTime(); }
// 'YYYY-MM-DD'(date input 값) → 그 날 00:00 / 다음날 00:00 의 epoch초. 형식이 아니면 0.
function ymdStartSec(v: string): number { const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(v || ''); if (!m) return 0; return new Date(+m[1], +m[2] - 1, +m[3], 0, 0, 0, 0).getTime() / 1000; }
function ymdEndSec(v: string): number { const st = ymdStartSec(v); return st ? st + DAY : 0; }
function ymdShort(v: string): string { const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(v || ''); return m ? Number(m[2]) + '/' + Number(m[3]) : ''; }
// 기간 선택 = 프리셋 키(또는 'range') + 직접 지정 범위 + 기준.
type TsessPeriod = { preset: string; from: string; to: string; basis: TsessBasis };
const TSESS_PERIOD_ALL: TsessPeriod = { preset: 'all', from: '', to: '', basis: 'last' };
function tsessPeriodIsAll(p: TsessPeriod) { return p.preset === 'all' && p.basis === 'last'; }
// 구 저장값(period 가 문자열)도 그대로 읽어 객체로 올린다.
function tsessPeriodOf(v: any): TsessPeriod {
  if (typeof v === 'string') return { ...TSESS_PERIOD_ALL, preset: TSESS_PERIOD_OPTS.some((o) => o.key === v) ? v : 'all' };
  const o = v || {};
...
function tsessPeriodMatch(p: TsessPeriod, s): boolean {
...
type TsessFilter = { state: string[]; scope: string[]; period: TsessPeriod };
function tsessFilter(): TsessFilter {
  const states = (v: any) => (Array.isArray(v) ? v : []).filter((k) => TSESS_STATE_OPTS.some((o) => o.key === k));
  const scopes = (v: any) => (Array.isArray(v) ? v : []).filter((k) => TSESS_SCOPE_OPTS.some((o) => o.key === k));
  const period = (v: any) => tsessPeriodOf(v);
  try {
    ...
    const f: TsessFilter = { state: [], scope: [], period: { ...TSESS_PERIOD_ALL } };
    ...
    if (v4raw) {
      ...
      const v4: any = JSON.parse(v4raw) || {};
      f.state = states(v4.state); f.period = period(v4.period);
      f.scope = scopes([v4.scope]);
    } else if (v3raw) {
      // v3 → v5. 출처 4버킷 중 **소속 성분만** 살린다(소유 성분은 필터가 아니라 섹션이 됐다 — #1229).
      const v3: any = JSON.parse(v3raw) || {};
      f.state = states(v3.state); f.period = period(v3.period);
      const src: string[] = Array.isArray(v3.src) ? v3.src : [];
      ...
      if (nProj && !nNon) f.scope = ['proj'];
      else if (nNon && !nProj) f.scope = ['nonproj'];
    } else {
      // v2 → v5. '내 것만'(TSESS_MINE_KEY)은 옮길 곳이 없다 — 소유는 섹션이 됐고, 섹션은 거르지 않는다(키만 지운다).
      const view = localStorage.getItem(TSESS_VIEW_KEY) || 'all';
      if (view === 'waiting' || view === 'busy') f.state = [view];
      else if (view === 'ended') f.state = ['exited', 'restorable'];     // 구 '끝남' = sessDead
      else if (view === 'today' || view === 'week' || view === 'stale') f.period = { ...TSESS_PERIOD_ALL, preset: view };
    }
    localStorage.removeItem(TSESS_FILTER_V4_KEY); localStorage.removeItem(TSESS_FILTER_V3_KEY);
    localStorage.removeItem(TSESS_VIEW_KEY); localStorage.removeItem(TSESS_MINE_KEY);
    if (f.state.length || f.scope.length || !tsessPeriodIsAll(f.period)) saveTsessFilter(f);
    return f;
  } catch { return { state: [], scope: [], period: { ...TSESS_PERIOD_ALL } }; }
}
function saveTsessFilter(f: TsessFilter) {
  try {
    if (!f.state.length && !f.scope.length && tsessPeriodIsAll(f.period)) localStorage.removeItem(TSESS_FILTER_KEY);
    else localStorage.setItem(TSESS_FILTER_KEY, JSON.stringify(f));
  } catch { /* noop */ }
}
...
function buildSessAxisFilter(opts: {
  title: string; items: Array<{ key: string; label: string; hint?: string; count: number }>;
  multi: boolean; sel: Set<string> | string; allLabel?: string; allCount: number; onChange: (next: any) => void;
  header?: any;   // 목록 **위** 영역(기간의 '기준'처럼 목록 전체에 걸리는 설정). 여기 클릭은 드롭다운을 닫지 않는다.
  footer?: any;   // 목록 아래 영역(기간의 '직접 고르기'). 여기 클릭은 드롭다운을 닫지 않는다.
  btnLabel?: string;  // 버튼에 띄울 문구를 호출부가 지정(직접 지정 범위처럼 항목으로 표현 못 하는 선택).
}) {
  const isMulti = opts.multi;
  const selSet: Set<string> = isMulti ? (opts.sel as Set<string>) : new Set([opts.sel as string]);
  const active = !!opts.btnLabel || (isMulti ? selSet.size > 0 : (opts.sel as string) !== 'all');
  ...
  const btn = el('button', { type: 'button', class: 'tsess-projfilter-btn' + (active ? ' active' : ''), title: opts.title + '(으)로 필터' },
    el('span', { text: btnText }), el('span', { class: 'tsess-projfilter-chev', text: '▾' }));
  ...
  const renderList = () => {
    const row = (key: string, label: string, count: number, on: boolean, hint?: string) =>
      el('button', {
        ...
        onmousedown: (e: any) => {
          e.preventDefault();
          if (!isMulti) { close(); opts.onChange(key); return; }
          if (selSet.has(key)) selSet.delete(key); else selSet.add(key);
          opts.onChange(new Set(selSet));           // 목록·카운트는 호출부가 다시 그린다(닫지 않는다)
        },
      }, ...(isMulti ? [el('span', { class: 'dash-pop-box', text: on ? '✓' : '' })] : []),
         el('span', { class: 'tsess-opt-name', text: label }), el('span', { class: 'tsess-opt-n', text: String(count) }));
    ...
    const head = isMulti && selSet.size
      ? [el('div', { class: 'tsess-projfilter-head' },
          el('span', { class: 'tsess-projfilter-headn', text: '전체 ' + total + '개 중 ' + selSet.size + '칸 선택' }),
          el('button', {
            class: 'dash-pop-clear', type: 'button', title: opts.title + ' 조건을 지우고 전체 보기',
            onmousedown: (e: any) => { e.preventDefault(); selSet.clear(); opts.onChange(new Set()); },
          }, '초기화'))]
      : [];
    listBox.replaceChildren(
      ...head,
      // 단일 축만 '전체' 행을 둔다. 다중 축은 아무것도 안 고른 상태가 곧 전체다(#1238).
      ...(isMulti ? [] : [row('all', opts.allLabel || '전체', total, (opts.sel as string) === 'all')]),
      ...opts.items.map((i) => row(i.key, i.label, i.count, selSet.has(i.key), i.hint)));
  };
  ...
  const placeIntoView = () => {
    ...
    if (!dd.isConnected) { requestAnimationFrame(() => { if (dd.isConnected && !dd.hidden) placeIntoView(); }); return; }
    dd.style.left = ''; dd.style.right = '';       // CSS 기본(right:0)으로 되돌린 뒤 측정
    let r = dd.getBoundingClientRect();
    if (r.left < M) {                              // 왼쪽으로 넘침 → 앵커 왼쪽 정렬로 뒤집기
      dd.style.right = 'auto'; dd.style.left = '0px';
    ...
    if (over > 0) {                                // 그래도 오른쪽으로 넘침 → 그만큼 왼쪽으로 당긴다
      ...
      dd.style.right = 'auto'; dd.style.left = `${cur - over}px`;
      if (dd.getBoundingClientRect().left < M) dd.style.left = `${cur - over + (M - dd.getBoundingClientRect().left)}px`;
    }
  };
  const open = () => {
    dd.hidden = false; renderList(); placeIntoView();
    ...
    setTimeout(() => { if (!dd.hidden && docHandler) document.addEventListener('mousedown', docHandler); }, 0);
  };
  btn.addEventListener('click', () => (dd.hidden ? open() : close()));
  if (opts.header) dd.append(opts.header);
  dd.append(listBox);
  if (opts.footer) dd.append(opts.footer);
  wrap.append(btn, dd);
...
export {
  TSESS_STATUS, tsessConfirmEnd, sessDead, sessState, relAgo, shortDir, sessAgeDays, sessSinceMidnight,
  TSESS_STATE_OPTS, TSESS_SCOPE_OPTS, tsessScopeOf, TSESS_SECTIONS, tsessCollapsed, saveTsessCollapsed,
  TSESS_PERIOD_OPTS, tsessPeriodMatch, ymdShort, tsessFilter, saveTsessFilter, buildSessAxisFilter,
};
export type { TsessBasis, TsessPeriod };
