...
function sessPickMode(): string { try { return localStorage.getItem(SESSPICK_MODE_KEY) === 'add' ? 'add' : 'replace'; } catch { return 'replace'; } }
function saveSessPickMode(v: string) { try { localStorage.setItem(SESSPICK_MODE_KEY, v); } catch { /* noop */ } }
...
export function openSessionSelectPicker(anchor, opts) {
  const cands: any[] = opts.candidates || [];
  ...
  const kinds = (ks: string[]) => new Set(cands.map(st).filter((k) => ks.includes(k))).size;
  const groups: Array<{ title: string | null; items: Array<{ label: string; hint?: string; match: (s) => boolean }> }> = [
  ...
  let close = () => { /* anchoredPopover 반환값으로 대체 */ };
  const render = () => {
    const rows: any[] = [
    ...
    for (const [k, label, hint] of [['replace', '바꾸기', '고른 조건으로 선택을 교체합니다'], ['add', '추가', '지금 선택에 더합니다']] as [string, string, string][]) {
    ...
    for (const g of groups) {
      ...
      if (g.title) rows.push(el('div', { class: 'dash-pop-gh', text: g.title }));
      for (const { it, ids } of items) {
        const all = ids.every((id) => opts.selected.has(id));   // 이 묶음이 이미 전부 선택돼 있나 → ✓ + 다시 누르면 해제
        const b = el('button', { class: 'dash-pop-opt' + (all ? ' sel' : ''), type: 'button', title: it.hint || it.label },
          el('span', { class: 'dash-pop-name', text: it.label }),
          el('span', { class: 'selpick-n' }, all ? el('span', { class: 'dash-pop-check', text: '✓' }) : null, el('span', { text: String(ids.length) })));
        b.onclick = () => {
          const next = new Set<string>(all || mode === 'add' ? opts.selected : []);
...
export function openGridPicker(items) {
  ...
  const openTabs = () => {
    let blocked = 0;
    for (const it of items) { if (!window.open(termUrl(it.id, it.label, it.node), '_blank')) blocked++; }
    back.remove();
    if (blocked) toast(blocked + '개 탭이 브라우저 팝업 차단에 막혔어요 — 주소창의 팝업 차단을 이 사이트에 허용한 뒤 다시 시도하세요', true);
    else toast(n + '개 탭으로 열었어요');
  };
  // 탭 도식 — 위 탭 스트립 + 아래 본문(브라우저 창 하나에 탭 여러 개). 그리드 미리보기와 같은 크기라 나란히 읽힌다.
  const tabsPrev = el('div', { class: 'tgrid-prev tgrid-prev--tabs' },
    el('span', { class: 'tgrid-tabbar' }, ...Array.from({ length: Math.min(n, 4) }, (_, i) => el('span', { class: 'tgrid-tab' + (i === 0 ? ' on' : '') }))),
    el('span', { class: 'tgrid-tabbody' }));
  container.append(
    el('div', { class: 'tgrid-gh', text: '따로 — 세션마다 탭 하나' }),
    el('div', { class: 'tgrid-opts' },
      el('button', { class: 'tgrid-opt', type: 'button', title: '고른 세션을 각각 새 탭으로 엽니다 (창 사이를 오가며 볼 때)', onclick: openTabs },
        tabsPrev, el('div', { class: 'tgrid-opt-lbl', text: '각각 새 탭' }), el('div', { class: 'tgrid-opt-dim', text: n + '개 탭' }))),
    el('div', { class: 'tgrid-gh', text: '함께 — 한 탭에 나란히' }));
  ...
  for (const L of gridLayouts(n)) {
    ...
    grids.append(el('button', { class: 'tgrid-opt', type: 'button', title: '한 탭 안에 ' + L.cols + '×' + L.rows + ' 로 나란히 (한눈에 볼 때)', onclick: () => openGrid(L.cols, L.rows) },
