...
async function renderConnectorMemberPanel(panel, system) {
  ...
  const bulkBar = suggested.length ? (() => {
    ...
    btn.addEventListener('click', async () => {
      if (!confirm(`이메일이 일치하는 ${suggested.length}명을 구성원과 연결할까요?\n\n연결된 사람은 그대로 두고 새로 연결할 것만 추가합니다.`)) return;
      (btn as any).disabled = true;
      let ok = 0; const failed: string[] = [];
      for (const r of suggested) {
        try { await postIdentities(r.suggested_member_id, r.user || {}, true); ok++; }
        catch (e: any) { failed.push(`${(r.user || {}).name || (r.user || {}).id}: ${e.message}`); }
      }
    ...
    return el('div', { class: 'cu-map-bulk' }, btn,
      el('span', { class: 'admin-hint', text: ' 이메일이 정확히 같은 사람만 대상이에요.' }));
  })() : null;
