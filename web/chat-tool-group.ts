...
/** 접힌 줄이 요약할 카드 한 장 — chat-view.ts 가 DOM(dataset·클래스)에서 읽어 만든다. */
export interface ToolGroupItem {
  label: string;      // 사람 말 이름(예: '라이블리' · '명령')
  detail?: string;    // 한 줄 대상(예: 'project_create_v6' · 'npm test')
  running?: boolean;  // 아직 도는 중
  err?: boolean;      // 실패로 끝남
}
...
const named = (it: ToolGroupItem): string => (it.label || '도구') + (it.detail ? ' ' + it.detail : '');
 ...
 */
export function toolGroupSummary(items: ToolGroupItem[]): string {
  ...
  if (n === 1) return `${named(items[0])} 사용함${tail}`;
