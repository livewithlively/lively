...
const upFailMsg = (status, bodyErr): string => bodyErr || ('업로드 실패 (' + status + ')'
...
async function authUpload(url, file, signal?) {
  ...
  if (!res.ok) { let m = ''; try { m = (await res.json()).error; } catch (_) { /* */ } throw new Error(upFailMsg(res.status, m)); }
}
...
function authUploadProgress(url, file, onProgress, signal?): Promise<any> {
  return new Promise<any>((resolve, reject) => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) { let j = null; try { j = JSON.parse(xhr.responseText); } catch (_) { /* */ } resolve(j); }
      else { let m = ''; try { m = JSON.parse(xhr.responseText).error; } catch (_) { /* */ } reject(new Error(upFailMsg(xhr.status, m))); }
    };
...
//   전송 중이던 파일은 서버가 임시파일에만 쓰고 있었으므로 목적지엔 흔적이 없다(부분 파일 X, 덮어쓰던 원본도 무사 — src/terminal/upload-file.ts).
