...
const pjvTlState = { scale: 'week', collapsed: new Set<string>(), showActual: false, depth: 'subtask', reschedule: true, linking: null as any };
...
const pjvTlTasks = { byProject: new Map<number, any[]>(), pending: new Set<number>(), failed: false, edges: [] as any[] };
...
function pjvTlFetchTasks(ids, done) {
  ...
  if (!need.length) return pjvTlTasks.pending.size > 0;
  for (const id of need) pjvTlTasks.pending.add(id);
  // 서버 상한(500)에 맞춰 나눠 던진다 — 완료까지 켠 전체 스코프면 프로젝트가 수백 개다.
  const chunks: number[][] = [];
  for (let i = 0; i < need.length; i += 500) chunks.push(need.slice(i, i + 500));
  Promise.all(chunks.map((c) => api('/api/ui/v6/project-tasks?ids=' + c.join(','))
    ...
    .then((all) => {
      ...
      for (const id of need) { pjvTlTasks.byProject.set(id, []); pjvTlTasks.pending.delete(id); }
      ...
      for (const e of all.flatMap((r) => r.edges)) {
        ...
        seen.add(k); pjvTlTasks.edges.push(e);
      }
      done();
    });
  return true;
...
function pjvTimelineView(projects, ctx) {
  ...
  const spans: any[] = [];
  const flatNodes: any[] = [];   // 의존 피커가 고를 수 있는 전체 노드(#1308)
  const collectSpans = (n) => { if (n.own) spans.push(n.own); flatNodes.push(n); for (const c of n.children) collectSpans(c); };
  for (const n of nodes) collectSpans(n);
  ...
  for (const d of PJV_TL_DEPTHS) {
    ...
    b.onclick = (e) => { e.stopPropagation(); if (pjvTlState.depth === d.key) return; pjvTlState.depth = d.key; rerender(); };
    depthWrap.append(b);
  }
  // 하위 작업이 몇 개 그려졌는지 — '보이는 것'을 숫자로도 확인할 수 있게(로딩 중이면 그 사실을 말한다).
  let taskRowCount = 0;
  const countRows = (n) => { for (const c of n.children) { taskRowCount++; countRows(c); } };
  for (const n of nodes) countRows(n);
  const taskLegend = el('span', { class: 'pjv-gt-legend' },
    el('span', { class: 'pjv-gt-key task' }),
    el('span', { text: loadingTasks ? '하위 작업 불러오는 중…' : (pjvTlTasks.failed ? '하위 작업을 못 불러왔어요' : '하위 작업 ' + taskRowCount) }));
  wrap.append(el('div', { class: 'pjv-gt-toolbar' }, scaleWrap, depthWrap, actualBtn, reschedBtn,
  ...
  const rowSub = el('div', { class: 'pjv-gt-subs' });
  {
    ...
    for (let i = 0; i < days; i += step) {
      ...
      if (sc.key !== 'month') cell.append(el('span', { text: sc.key === 'day' ? String(d.getDate()) : pjvTlFmt(ms) }));
      rowSub.append(cell);
    }
  }
  axis.append(rowMon, rowSub);
  inner.append(el('div', { class: 'pjv-gt-headrow' },
    el('div', { class: 'pjv-gt-sheethead', style: 'width:' + PJV_GT_SHEET_W + 'px' },
  ...
  const savePatch = (n, patch) => {
    ...
    if (n.kind === 'project') {
      ...
      api('/api/ui/v6/projects/' + n.row.id, { method: 'POST', body: JSON.stringify(body) })
        .then((d) => { if (d && d.rescheduled && d.rescheduled.length) pjvTlClearTasks(); pjvReloadKeepScroll(reload); })
  ...
  const attachDrag = (grip, bar, n, span, mode, rangeEl?) => {
    grip.addEventListener('pointerdown', (ev: any) => {
      ...
      const paint = () => {
        ...
        if (rangeEl) rangeEl.textContent = range;   // 좌측 시트의 기간도 같이 따라간다
      };
      (bar.parentElement || bar).append(dragTip);
      paint();
      const onMove = (mv: any) => {
        const dd = Math.round((mv.clientX - startX) / dayPx);
        if (mode === 'move') { dFrom = span.from + dd * PJV_TL_DAY; dTo = span.to + dd * PJV_TL_DAY; }
        else if (mode === 'l') dFrom = Math.min(span.from + dd * PJV_TL_DAY, span.to);
        else dTo = Math.max(span.to + dd * PJV_TL_DAY, span.from);
        paint();
      };
      const onUp = () => {
        ...
        dragTip.remove();
        if (dFrom !== span.from || dTo !== span.to) saveSpan(n, dFrom, dTo);
        else if (rangeEl) rangeEl.textContent = pjvTlFmt(span.from) + ' – ' + pjvTlFmt(span.to);
      };
  ...
  //  DOM 순서가 곧 진실이다).
  const nodeX = new Map<number, { x: number; w: number }>();
  // 엣지 방향 규약(서버와 동일): from = 후행(blocked) · to = 선행(blocking).
  const linkEdge = (fromId, toId, unlink?) => {
    if (fromId === toId) { toast('자기 자신에는 의존을 걸 수 없습니다', true); return; }
  ...
  const attachLinkNode = (nd, n, side) => {
    nd.addEventListener('pointerdown', (ev: any) => {
      if (ev.button !== 0) return;
      ev.preventDefault(); ev.stopPropagation();
      ...
      const onMove = (mv: any) => paint(mv);
      const onUp = (up: any) => {
        nd.removeEventListener('pointermove', onMove);
        nd.removeEventListener('pointerup', onUp);
        ghost.remove();
        const hit = document.elementFromPoint(up.clientX, up.clientY) as any;
        ...
        if (!otherId || otherId === Number(n.row.id)) return;
        // 오른쪽 노드에서 끌었다 = 이 노드가 선행(끝나야 상대가 시작) / 왼쪽 = 이 노드가 후행.
        if (side === 'r') linkEdge(otherId, n.row.id);
        else linkEdge(n.row.id, otherId);
      };
  ...
  const renderNode = (n) => {
    ...
    const openDetail = () => {
      if (n.kind === 'project') { location.hash = '#/projects2/p/' + row.id; return; }
    ...
    const caret: any = n.children.length
    ...
    r.append(el('div', { class: 'pjv-gt-sheet', style: 'width:' + PJV_GT_SHEET_W + 'px;padding-left:' + (10 + n.depth * 15) + 'px' },
      caret, pjvStatusIconStd(row.status), nameEl,
      n.children.length ? el('span', { class: 'pjv-gt-count', text: n.done + '/' + n.children.length }) : null,
      outOfRange ? el('span', { class: 'pjv-gt-warn', text: '⚠',
        title: '하위 일정이 이 항목의 기간(' + pjvTlFmt(n.own.from) + ' – ' + pjvTlFmt(n.own.to) + ')을 벗어납니다 — 하위 전체는 '
          + pjvTlFmt(n.span.from) + ' – ' + pjvTlFmt(n.span.to) }) : null,
      rangeEl));
    ...
    if (n.own) {
      ...
      if (asg.length) {
        ...
        for (const id of asg) faces.append(personFace(id, 'pjv-ava', id));
        track.append(faces);
      }
      track.append(bar);
    } else if (n.span) {
    ...
    r.append(track);
    bodyEl.append(r);
    if (!collapsed) for (const c of n.children) renderNode(c);
  };
  ...
  for (const g of groups) {
    ...
    caret.onclick = (e) => {
      e.stopPropagation();
      if (collapsed) pjvTlState.collapsed.delete(g.key); else pjvTlState.collapsed.add(g.key);
      rerender();
    };
    gr.append(el('div', { class: 'pjv-gt-sheet', style: 'width:' + PJV_GT_SHEET_W + 'px' },
      caret,
      el('span', { class: 'pjv-gt-dot', style: g.color ? ('background:' + g.color) : '' }),
      el('span', { class: 'pjv-gt-name group', text: g.name }),
    ...
    for (const n of g.rows) renderNode(n);
  }
  ...
  if (unscheduled.length) {
    ...
    if (!collapsed) {
      ...
      for (const n of unscheduled.slice(0, 200)) renderNode(n);
  ...
  {
    const rowEls = Array.from(bodyEl.children) as any[];
    const yOf = new Map<number, number>();
    rowEls.forEach((elm, i) => {
      const nid = elm.dataset && elm.dataset.nid;
      if (nid) yOf.set(Number(nid), i * PJV_GT_ROW_H + PJV_GT_ROW_H / 2);
    });
    ...
    if (links.length) {
      ...
      for (const e of links) {
        const a = nodeX.get(e.to_id) as any, b = nodeX.get(e.from_id) as any;   // to=선행, from=후행
        const ay = yOf.get(e.to_id) as number, by = yOf.get(e.from_id) as number;
        ...
        hit.addEventListener('click', (ev: any) => { ev.stopPropagation(); pjvGtLinkMenu(ev, e, linkEdge); });
        svg.append(hit);
  ...
  if (dep) {
    ...
    if (mine.length) {
      menu.append(mk('걸린 의존 ' + mine.length + '건 모두 해제', () => {
        for (const e of mine) dep.linkEdge(e.from_id, e.to_id, true);
      }, true));
    }
  }
...
function pjvGtDepPicker(anchor, n, dep, asPredecessor) {
  ...
  const pop = el('div', { class: 'pjv-menu pjv-gt-deppick' },
    el('div', { class: 'pjv-subtask-pop-head', text: asPredecessor ? '먼저 끝나야 할 작업 고르기' : '이 작업 뒤에 올 작업 고르기' }),
    search, listBox);
  ...
  const rebuild = () => {
    ...
    for (const x of items) {
      const b = el('button', { class: 'pjv-menu-item', type: 'button' },
        el('span', { class: 'pjv-gt-depname', text: x.row.name, title: x.row.name }));
      b.onclick = () => {
        close();
        if (asPredecessor) dep.linkEdge(n.row.id, x.row.id);   // from=이 작업(후행), to=고른 것(선행)
        else dep.linkEdge(x.row.id, n.row.id);
      };
      listBox.append(b);
    }
    if (!items.length) listBox.append(el('div', { class: 'pjv-menu-empty', text: '해당하는 작업이 없습니다' }));
  };
