...
import { UP_CONFIRM, UpItem, upControl, upDropZone, upPrecheckOverwrite, upProgress, upSend, upToast } from './files-upload.js';
...
function pjvTeamPicker(preselected, onChange?) {
  ...
  const fire = () => { try { onChange && onChange(); } catch (_) { /* */ } };
  let all: any[] = [];
  ...
  const paint = () => {
    listBox.replaceChildren(...(all.length ? all.map((t) => {
      ...
      const row = el('div', { class: 'proj-mp-item' + (on ? ' sel' : ''), role: 'button', tabindex: '0' },
        el('span', { class: 'proj-mp-check', text: on ? '✓' : '' }),
        el('span', { class: 'proj-mp-name', text: t.name || t.key || String(t.id) }));
      const toggle = () => { on ? selected.delete(Number(t.id)) : selected.add(Number(t.id)); paint(); fire(); };
      row.onclick = toggle;
      row.addEventListener('keydown', (e: any) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggle(); } });
      return row;
    }) : [el('div', { class: 'proj-mp-empty', text: '팀이 없어요 — 관리 ▸ 팀에서 먼저 만들어 주세요' })]));
  };
  paint();
  api('/api/ui/teams').then((d) => { all = (d && d.teams) || []; paint(); }).catch(() => { /* 조회 실패해도 폼은 살아있다 */ });
  return {
    box,
...
export type { UpItem } from './files-upload.js';
