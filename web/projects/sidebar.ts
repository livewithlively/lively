...
function pjvListSettingsMenu(menu, close, list, reload, favCtx?: { isFav: boolean, onToggle: (next: boolean) => void }) {
  ...
  menu.append(mk(isFav ? '즐겨찾기 해제' : '즐겨찾기에 추가', () => {
    if (favCtx) { favCtx.onToggle(!isFav); return; }
    pjvSetFavorite('project_list', Number(list.id), !isFav)
      .then(() => { if (isFav) pjvFavListCache.delete(Number(list.id)); else pjvFavListCache.add(Number(list.id)); if (reload) reload(); })
      .catch((e: any) => toast('즐겨찾기 저장 실패 — ' + (e && e.message || e), true));
  }));
...
function pjvVisLock(row: any) {
  ...
  if (!visAxisOn('project')) return null;
...
let pjvFavListCache = new Set<number>();
...
function pjvFavSecOpen() {
  try { return localStorage.getItem('pjv:favSecOpen') !== '0'; } catch (_) { return true; }
}
function pjvSetFavSecOpen(open: boolean) {
  try { localStorage.setItem('pjv:favSecOpen', open ? '1' : '0'); } catch (_) { /* noop */ }
}
