...
function openLocalWorkModal(id, p, opts?) {
  ...
  const fillHarness = (list) => {
    ...
    if (list.some((h) => h.key === want)) harnessSel.value = want;   // '클로드로 실행' 기본값 선주입 + 사용자가 고른 값 보존
  };
  ...
  const autoBlock = el('section', { class: 'ps-block' },
    ...
    el('label', { style: 'display:flex;gap:8px;align-items:center;cursor:pointer' }, autoChk, autoText));
  ...
  (async () => {
    try {
      ...
      if (hs.length) fillHarness(hs.map((h) => ({ key: h.key, label: h.label || h.key })));
