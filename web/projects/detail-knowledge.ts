...
function projectKnowledgeSection(id, p, reload) {
  ...
  const knAddBtn = el('button', { class: 'btn btn-ghost btn-sm pjk-add-btn', type: 'button', text: '＋ 지식 연결', 'data-tour': 'pd-link-kn',   // #853 '프로젝트 체험' 투어 앵커
    ...
    onclick: () => openKnowledgePicker(id, 'required',
      { required: cur.required.map(knName), produced: cur.produced.map(knName) }, refresh) });  // 관계별 연결 상태(#1133)
  ...
  async function fetchRecs() {
    ...
    try { recsCache = await api('/api/ui/v6/projects/' + id + '/recommend-knowledge?limit=2').then((d) => (d && d.entries) || []); }
    catch (_) { recsCache = []; }
    return recsCache;
  }
  function recRow(m) {
  ...
  const reqCol = el('div', { class: 'pjk-col pjk-col-req' },
    el('div', { class: 'pjk-col-head' }, el('span', { class: 'pjk-col-title', text: '필요 지식' }), reqCount, knAddBtn),  // 버튼 = 칼럼 머리 우측(#1133)
    reqList);
