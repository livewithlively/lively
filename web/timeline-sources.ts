...
export async function loadProjectTimeline(id: number, detail?: any): Promise<Item[]> {
  ...
  for (const f of feedRes as any[]) {
    ...
    if (f.field === 'created') out.push({ id: 'ev:' + f.id, kind: 'project', verb: '시작', label: '이 프로젝트를 시작했어요', key: 'ev|created', ts: s(f.ts), actor });
    else if (f.field === 'status') out.push({ id: 'ev:' + f.id, kind: 'project', verb: '정함', label: statusWord(f.to) + '(으)로 옮겼어요', key: 'ev|status|' + s(f.to), ts: s(f.ts), actor });
  }
  ...
  const nearActivity = (iso: string): boolean => {
    const n = Date.parse(iso); if (!Number.isFinite(n)) return false;
  ...
  for (const t of tasks) {
    if (!t.completed_at) continue;                 // 끝낸 일만 — 만든 일은 계획이지 결과가 아니다
    if (nearActivity(s(t.completed_at))) continue;
