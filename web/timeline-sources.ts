...
function fromActivity(a: any, opts?: { showProject?: boolean }): Item {
  ...
  if (a.commit_sha) kids.push({ verb: '커밋', label: [s(a.repo), s(a.commit_sha).slice(0, 8)].filter(Boolean).join(' ') + (a.touchCount ? ' · 코드 ' + a.touchCount + '곳' : '') });
...
export async function loadProjectTimeline(id: number, detail?: any): Promise<Item[]> {
  ...
  for (const t of tasks) {
    ...
    if (t.completed_at) out.push({ id: 'tk-done:' + t.id, kind: 'task', verb: '끝냄', label: humanSummary(t.name, 40), key: 'tk|done|' + t.id, ts: s(t.completed_at), actor: who, href: '#/projects2/t/' + t.id });
    if (t.created_at) out.push({ id: 'tk-new:' + t.id, kind: 'task', verb: '만듦', label: humanSummary(t.name, 40), key: 'tk|new|' + t.id, ts: s(t.created_at), actor: who, href: '#/projects2/t/' + t.id, tier: 2 });
  }
  return out;
}
