...
function pjvOpenTaskModal(taskId, pageReload) {
  ...
  const onKey = (e) => {
    ...
    if (document.querySelector('.be-slash, .be-turnpop, .be-linkpop, .be-blockmenu, .be-mentionmenu, .ov-back')) return;
  ...
  const onPaste = (e) => {
    ...
    if (tgt && tgt.closest && tgt.closest('.be')) return;
  ...
  function pjvtmMain(d, t, members, refresh, closeModal, pageReload) {
    ...
    main.append(el('div', { class: 'pjv-tm-fields' },
      pjvtmFieldRow('◎', '상태', pjvtmStatusField(t, refresh, d.list)),
  ...
  function pjvtmStatusField(t, refresh, listStatus?) {
    ...
    if (!isCustom && raw && raw.toLowerCase() !== meta.label.toLowerCase() && raw.toLowerCase() !== String(t.status || '').toLowerCase()) {
      return el('span', { class: 'pjv-tm-statuswrap' }, ctrl,
