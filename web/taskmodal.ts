...
function pjvtmFieldControlFn() { return (PJ as any).pjvFieldControl || null; }
...
function pjvtmSideResizer(box) {
  ...
  r.addEventListener('mousedown', (e: any) => {
    e.preventDefault();
    const side = box.querySelector('.pjv-tm-side') as HTMLElement | null;
    if (!side) return;
    ...
    const onUp = () => {
      ...
      if (cur) { try { localStorage.setItem('pjv:tmSideW', cur.trim()); } catch (_) { /* noop */ } }
    };
    document.addEventListener('mousemove', onMove);
    document.addEventListener('mouseup', onUp);
  });
  r.addEventListener('dblclick', (e: any) => {
    e.preventDefault();
    box.style.removeProperty('--pjv-side-w');
    try { localStorage.removeItem('pjv:tmSideW'); } catch (_) { /* noop */ }
  });
  return r;
}
...
function pjvOpenTaskModal(taskId, pageReload) {
  ...
  let descCommit: ((save: boolean) => void) | null = null;
  const back = el('div', { class: 'pjv-tm-back' });
  const box = el('div', { class: 'pjv-tm' });
  // Activity 패널 폭 복원(#496) — 저장돼 있으면 기본값 대신 그 폭으로. box 는 재렌더에도 유지.
  try { const _sw = localStorage.getItem('pjv:tmSideW'); if (_sw) box.style.setProperty('--pjv-side-w', _sw.indexOf('px') >= 0 ? _sw : (_sw + 'px')); } catch (_) { /* noop */ }
  back.append(box);
  // 바깥(배경) 클릭 — 본문 편집 중이면 먼저 저장하고 모달은 유지(한 번 더 눌러야 닫힘). 아니면 닫기.
  back.addEventListener('mousedown', (e) => {
    if (e.target !== back) return;
    if (descEditing && descCommit) { descCommit(true); return; }
    closeModal();
  });
  const onKey = (e) => {
    if (e.key !== 'Escape') return;
    const pb: any = document.querySelector('.pjv-tm-paste-back');
    if (pb) { pb._cancel && pb._cancel(); return; } // 붙여넣기 다이얼로그가 떠 있으면 그것부터 닫기
    if (descEditing) return; // 본문 편집 중이면 에디터 자체의 Esc(취소)가 처리 — 모달은 닫지 않음
    if (!document.querySelector('.pjv-pop')) closeModal();
  };
  // 클립보드 이미지 붙여넣기(Ctrl/⌘+V) — 이 태스크 첨부폴더로 업로드(터미널 붙여넣기와 같은 결).
  //  이미지가 있을 때만 가로채고, 텍스트 등 비-이미지 붙여넣기는 기본 동작 그대로 통과시킨다.
  const onPaste = (e) => {
    if (!pasteCtx || pasteCtx.pid == null) return;
    if (document.querySelector('.pjv-tm-paste-back')) return; // 이미 다이얼로그 열림
    const dt = e.clipboardData; if (!dt) return;
    let blob: any = null;
    for (const it of (dt.items || [])) { if (it.kind === 'file' && /^image\//.test(it.type || '')) { blob = it.getAsFile(); break; } }
    if (!blob) for (const f of (dt.files || [])) { if (/^image\//.test(f.type || '')) { blob = f; break; } }
    if (!blob) return; // 이미지 없음 → 기본 붙여넣기 유지
    e.preventDefault();
  ...
  function closeModal() {
    ...
    if (descEditing && descCommit) { try { descCommit(true); } catch (_) { /* noop */ } }
  ...
  function render(d) {
    ...
    box.replaceChildren(
      ...
      pjvtmSideResizer(box), // 본문↔Activity 경계 드래그 핸들(#496)
  ...
  function pjvtmMain(d, t, members, refresh, closeModal, pageReload) {
    ...
    main.append(el('div', { class: 'pjv-tm-typebar' },
      ...
      extUrl ? el('a', { class: 'pjv-tm-extlink', href: extUrl, target: '_blank', rel: 'noopener',
    ...
    const saveTitle = () => {
      ...
      if (v && v !== lastSavedName) { lastSavedName = v; pjvPatchTask(t.id, { name: v }, refresh); }
    };
    ...
    titleIn.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && !e.isComposing && e.keyCode !== 229) { e.preventDefault(); titleIn.blur(); }
    });
    main.append(titleIn);
  ...
  function pjvtmStatusField(t, refresh) {
    ...
    if (raw && raw.toLowerCase() !== meta.label.toLowerCase() && raw.toLowerCase() !== String(t.status || '').toLowerCase()) {
      return el('span', { class: 'pjv-tm-statuswrap' }, btn,
        el('span', { class: 'pjv-tm-statusraw', title: '원본 상태(읽기전용): ' + raw, text: raw }));
    }
    return btn;
  }
  ...
  function pjvtmCustomFields(d, t, refresh) {
    ...
    if (!t.field_values) t.field_values = d.field_values || {}; // pjvFieldControl 이 t.field_values 를 읽고/갱신
    const fieldControl = pjvtmFieldControlFn();
    const grid = el('div', { class: 'pjv-tm-fields pjv-tm-cfields' });
    for (const f of fields) {
      grid.append(pjvtmFieldRow('▤', f.name,
        fieldControl ? fieldControl(t, f, refresh) : pjvtmFieldReadonly(f, (t.field_values || {})[f.id])));
    }
    const sec = el('div', { class: 'pjv-tm-block pjv-tm-cfblock' });
    sec.append(el('div', { class: 'pjv-tm-block-head' },
      el('span', { class: 'pjv-tm-block-title', text: '커스텀 필드' })));
    sec.append(grid);
    return sec;
  }
  ...
  function pjvtmFieldReadonly(f, v) {
    ...
    if (has) {
      ...
      if (f.field_type === 'dropdown') txt = optName(v);
      else if (f.field_type === 'labels' && Array.isArray(v)) txt = v.map(optName).join(', ');
      else if (Array.isArray(v)) txt = v.map((x) => (x && typeof x === 'object' ? (x.name || x.id || '') : String(x))).join(', ');
  ...
  function pjvtmDescription(t, refresh) {
    ...
    const editBody = () => {
      ...
      ce.focus();
      try { const r = document.createRange(); r.selectNodeContents(ce); r.collapse(false); const s = window.getSelection(); if (s) { s.removeAllRanges(); s.addRange(r); } } catch (_) { /* noop */ }
      const toolbar = buildWysiwygToolbar(ce);
      let fin = false;
      const done = (save) => {
        if (fin) return; fin = true;
        descEditing = false; descCommit = null;
        toolbar.destroy();
        const nv = save ? mdFromDom(ce) : (t.description || '');
        if (save && nv !== (t.description || '')) {
          t.description = nv; dirty = true;
      ...
      descEditing = true; descCommit = (save) => done(save);
  ...
  function pjvtmSubtasks(d, t, members, refresh, pageReload) {
    ...
    const head = el('div', { class: 'pjv-tgroup-head pjv-tgroup-head-cols' },
      el('div', { class: 'pjv-trow-title-cell' },
        el('span', { class: 'pjv-row-check-spacer', 'aria-hidden': 'true' }),
        el('span', { class: 'pjv-colhead', text: '이름' })),
    ...
    for (const s of subs) list.append(pjvTaskRow(d.project.id, s, members, refresh, 0, []));
  ...
  function pjvtmAttachments(d, t, refresh) {
    ...
    if ((d.attachments || []).length) sec.append(pjvtmDbAttachments(d.attachments));
  ...
  function pjvtmDbAttachments(atts) {
    ...
    for (const a of atts) {
      ...
      if (isImg(a)) {
        ...
        rows.append(el('div', { class: 'pjv-tm-att' },
          fileIconSvg(name, false),
          url ? el('a', { class: 'pjv-tm-att-name pjv-tm-att-link', href: url, target: '_blank', rel: 'noopener', title: '원본 열기(새 탭)', text: name })
              : el('span', { class: 'pjv-tm-att-name', text: name }),
          a.size != null && Number(a.size) > 0 ? el('span', { class: 'pjv-tm-att-size', text: fmtSize(Number(a.size)) }) : null));
      }
    }
  ...
  function pjvtmPasteImage(blob, ctx, mount) {
    ...
    const back2: any = el('div', { class: 'pjv-tm-paste-back' },
      el('div', { class: 'pjv-tm-paste-card' },
        el('div', { class: 'pjv-tm-paste-title', text: '이미지 붙여넣기' }),
        el('img', { class: 'pjv-tm-paste-preview', src: url, alt: '붙여넣은 이미지 미리보기' }),
        el('label', { class: 'pjv-tm-paste-label' }, '이름 · 주석', nameIn),
        err,
        el('div', { class: 'pjv-tm-paste-actions' }, cancelBtn, okBtn)));
    const cleanup = () => { URL.revokeObjectURL(url); back2.remove(); };
    back2._cancel = cleanup;
    cancelBtn.onclick = cleanup;
    back2.addEventListener('mousedown', (e) => { if (e.target === back2) cleanup(); });
    ...
    nameIn.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); doUpload(); } });
  ...
  function pjvtmSide(d, t, refresh) {
    ...
    for (const f of feed) {
      ...
      if (f.kind === 'event' && prev && prev.kind === 'event' && prev.field === f.field && NOISY_FIELDS.has(f.field)) feedShown[feedShown.length - 1] = f;
      else feedShown.push(f);
    }
    for (const f of feedShown) feedBox.append(f.kind === 'comment' ? pjvtmComment(f, cctx) : pjvtmEvent(f));
    side.append(feedBox);
