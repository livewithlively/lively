...
async function toolUsagePanel(detail) {
  ...
  const filterQs = (extra?) => {
    ...
    if (TOOL_USAGE_STATE.window === AUD_CUSTOM) {
      const s = audDayStart(TOOL_USAGE_STATE.from); if (s) q.set('since', s);
      const u = audDayEnd(TOOL_USAGE_STATE.to); if (u) q.set('until', u);
      if (!s && !u) q.set('window', 'all'); // 날짜를 아직 안 골랐다 → 전체 기간
    } else q.set('window', TOOL_USAGE_STATE.window);
