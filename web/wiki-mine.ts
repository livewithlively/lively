...
async function renderMineSurface(box: HTMLElement, ctx: any) {
  ...
  const docsOf = new Map<string, any[]>();          // catId → 최신순 문서
  const iconOf = new Map<string, string>();         // catId → 대문 아이콘
  const freshOf = new Map<string, number>();        // catId → 이번 주 새 문서 수
  const failed = new Set<string>();                 // 못 불러온 카테고리
  const catNameOf = new Map<string, string>();      // 문서 name → 소유 카테고리 이름('전체 최신' 행의 출처 표기)
  let globalDocs: any[] | null = null;              // '전체 최신' — 위키 전체 최신 변경(경량)
  ...
  const searchBtn = el('button', { class: 'wk-mc-search', type: 'button', title: '전체 지식 의미검색 (⌘K)' },
    el('span', { 'aria-hidden': 'true', text: '🔍' }),
    el('span', { text: '검색' }),
    el('span', { class: 'wk-hero-kbd', text: '⌘K' }));
  searchBtn.onclick = () => openWikiSearch();
  mine.append(el('div', { class: 'wk-mc-head' },
    el('h1', { class: 'wk-mc-title', text: '★ 내 소유 카테고리' }),
    el('span', { class: 'wk-mc-stats' },
      el('span', { class: 'wk-row-m', text: '카테고리 ' + allOwned.length }),
      el('span', { class: 'wk-row-m', text: '지식 ' + totalDocs }),
      freshStat),
    el('span', { class: 'wk-mc-sp' }),
    searchBtn,
    hasMemoryScope() ? el('a', { class: 'btn btn-primary wk-mc-new', href: '#/knowledge/new',
  ...
  let selId = '';
  try { selId = localStorage.getItem(WK_MINE_SEL) || ''; } catch (_) { /* noop */ }
  if (selId !== ALL_ID && !catById.has(selId)) selId = ALL_ID;   // 첫 방문 기본 = 전체 최신

  function select(id: string) {
    if (id !== ALL_ID && !catById.has(id)) return;
    selId = id;
    try { localStorage.setItem(WK_MINE_SEL, id); } catch (_) { /* noop */ }
    paintMaster();
    paintDetail();
  }
  ...
  function paintMaster() {
    ...
    const allRow = el('button', { class: 'wk-mc-mrow wk-mc-mall', type: 'button', role: 'option',
      ...
      title: '위키 전체의 최근 변경을 오른쪽에 보여줍니다' },
    el('span', { class: 'wk-mc-mic', 'aria-hidden': 'true', text: '∗' }),
    el('span', { class: 'wk-mc-mname', text: '전체 최신' }),
    el('span', { class: 'wk-mc-mmeta' },
      freshTotal ? el('span', { class: 'wk-row-m wk-m-new', text: '+' + freshTotal }) : null));
    allRow.addEventListener('click', () => select(ALL_ID));
    allRow.addEventListener('keydown', (ev: any) => {
      if (ev.key !== 'ArrowDown') return;
      ev.preventDefault();
      const first = orderedCats()[0];
      if (!first) return;
      select(String(first.id));
      const n = mlist.querySelector('[data-cat-id="' + String(first.id) + '"]') as HTMLElement;
      if (n) n.focus();
    });
    const rows = cats.map((c) => {
      ...
      const row = el('button', { class: 'wk-mc-mrow', type: 'button', role: 'option',
        ...
        title: (c.name || c.key) + ' — 최근 문서를 오른쪽에 보여줍니다. 끌면 순서가 바뀝니다.' },
      icon ? el('span', { class: 'wk-mc-mic', 'aria-hidden': 'true', text: icon })
        : el('span', { class: 'wk-mc-mic letter', 'aria-hidden': 'true', text: letterOf(c) }),
      el('span', { class: 'wk-mc-mname', text: c.name || c.key }),
      el('span', { class: 'wk-mc-mmeta' },
        freshN ? el('span', { class: 'wk-row-m wk-m-new', text: '+' + freshN }) : null,
        Number.isFinite(docsN) ? el('span', { class: 'wk-row-m', text: String(docsN) }) : null));
      row.addEventListener('click', () => select(id));
      row.addEventListener('keydown', (ev: any) => {
        if (ev.key !== 'ArrowDown' && ev.key !== 'ArrowUp') return;
        ev.preventDefault();
        const ids = orderedCats().map((x) => String(x.id));
        const ni = ids.indexOf(id) + (ev.key === 'ArrowDown' ? 1 : -1);
        if (ni < 0) { select(ALL_ID); const a = mlist.querySelector('.wk-mc-mall') as HTMLElement; if (a) a.focus(); return; }
        const next = ids[ni];
        if (!next) return;
        select(next);
        const n = mlist.querySelector('[data-cat-id="' + next + '"]') as HTMLElement;
        if (n) n.focus();
      });
      // 드래그 정렬(세로) — 홈 카드와 같은 저장·같은 표식(삽입선).
      row.addEventListener('dragstart', (ev: any) => {
        dragId = id;
        row.classList.add('drag-src');
        try { ev.dataTransfer.effectAllowed = 'move'; ev.dataTransfer.setData('text/plain', id); } catch (_) { /* noop */ }
      });
      row.addEventListener('dragend', () => { dragId = null; mlist.querySelectorAll('.drop-before, .drop-after').forEach((n: any) => n.classList.remove('drop-before', 'drop-after')); });
      row.addEventListener('dragover', (ev: any) => {
      ...
      row.addEventListener('drop', (ev: any) => {
        if (!dragId || dragId === id) return;
        ev.preventDefault();
        const before = row.classList.contains('drop-before');
        if (knApplyCatReorder(allOwned, dragId, id, before)) {
          paintMaster();
          if (ctx.onCatChanged) ctx.onCatChanged();   // 사이드바 ★구역 순서도 즉시 맞춘다
        }
        dragId = null;
      });
      return row;
    ...
    mfoot.replaceChildren(knCatOrderSaved().length
      ? el('button', { class: 'wk-sec-act', type: 'button', title: '끌어서 바꾼 카테고리 순서를 기본으로 되돌립니다', text: '↺ 순서 초기화',
        onclick: () => { knCatOrderClear(); paintMaster(); if (ctx.onCatChanged) ctx.onCatChanged(); } })
      : el('span', { class: 'wk-sec-hint', text: '드래그해서 순서 변경' }));
  }

  // ── 디테일 — 선택 카테고리의 헤더 밴드(오로라 = 이 화면의 유일한 장식) + 최근 문서. ──
  function paintDetail() {
    if (selId === ALL_ID) { paintAllDetail(); return; }
    ...
    dhead.replaceChildren(cover,
      el('div', { class: 'wk-mc-dbar' },
        el('span', { class: 'wk-mc-dic' + (icon ? '' : ' letter'), 'aria-hidden': 'true', text: icon || letterOf(c) }),
        el('span', { class: 'wk-mc-dmain' },
          name,
          el('span', { class: 'wk-mc-dmeta' },
            docs ? el('span', { class: 'wk-row-m', text: '지식 ' + docs.length }) : null,
            latest ? el('span', { class: 'wk-row-m', text: '최근 ' + latest }) : null,
            (freshOf.get(id) || 0) ? el('span', { class: 'wk-row-m wk-m-new', text: '이번 주 +' + freshOf.get(id) }) : null)),
        el('span', { class: 'wk-mc-dacts' },
          el('button', { class: 'wk-sec-act', type: 'button', text: '전체 보기 →', onclick: go }),
          hasMemoryScope() && c.key ? el('a', { class: 'wk-sec-act', href: '#/knowledge/new?category=' + encodeURIComponent(c.key),
    ...
    if (!docs.length) {
      dbody.replaceChildren(wkEmpty('아직 지식이 없습니다.',
    ...
    docs.length > DETAIL_DOCS ? el('div', { class: 'wk-mc-more' },
      el('button', { class: 'wk-sec-act', type: 'button', text: '… ' + (docs.length - DETAIL_DOCS) + '개 더 — 전체 보기 →', onclick: go })) : null);
  }

  // '전체 최신' 뷰 — 위키 전체의 최근 변경(내 소유 포함). 소유 문서엔 출처 카테고리 이름을 메타로 단다.
  function paintAllDetail() {
    dhead.replaceChildren(el('div', { class: 'wk-mc-dbar plain' },
      el('span', { class: 'wk-mc-dic letter', 'aria-hidden': 'true', text: '∗' }),
      el('span', { class: 'wk-mc-dmain' },
        el('span', { class: 'wk-mc-dname', text: '전체 최신' }),
        el('span', { class: 'wk-mc-dmeta' },
          el('span', { class: 'wk-row-m', text: '위키 전체의 최근 변경입니다.' }),
          freshTotal ? el('span', { class: 'wk-row-m wk-m-new', text: '이번 주 +' + freshTotal }) : null)),
      el('span', { class: 'wk-mc-dacts' },
    ...
    if (!globalDocs.length) {
      dbody.replaceChildren(wkEmpty('아직 지식이 없어요. 첫 페이지로 시작해 보세요.',
    ...
    globalDocs.length > ALL_DOCS ? el('div', { class: 'wk-mc-more' },
      el('a', { class: 'wk-sec-act', href: '#/knowledge?all=1', text: '… 더 보기 — 전체 지식 →' })) : null);
  }

  // ── 선적재 — 카테고리별 경량 인덱스(세션 캐시). 도착하는 대로 마스터 배지·현재 디테일을 갱신. ──
  for (const c of allOwned) knFetchCategoryIndex(c.id).then((rows) => {
    const id = String(c.id);
    const docs = laneDocsOf(rows);
    docsOf.set(id, docs);
    for (const r of docs) catNameOf.set(r.name, c.name || c.key);
    const homeDoc = rows.find((r) => isCategoryHomeDoc(r.name));
    if (homeDoc && homeDoc.icon) iconOf.set(id, homeDoc.icon);
    const freshN = docs.filter(wkIsFresh).length;
    if (freshN) { freshOf.set(id, freshN); freshTotal += freshN; freshStat.textContent = '이번 주 새 글 ' + freshTotal; }
    if (!mine.isConnected) return;
    paintMaster();
    if (id === selId || selId === ALL_ID) paintDetail();   // 전체 최신 뷰의 출처 라벨·배지도 도착분을 반영
  }).catch(() => {
    failed.add(String(c.id));
    if (mine.isConnected && String(c.id) === selId) paintDetail();
  });
  ...
  api('/api/ui/knowledge?' + new URLSearchParams({
    ...
    .then((r) => {
      globalDocs = (((r && r.entries) || []) as any[]).filter((e) => !e.is_folder && !isCategoryHomeDoc(e.name));
      if (mine.isConnected && selId === ALL_ID) paintDetail();
    })
    .catch(() => { globalFail = true; if (mine.isConnected && selId === ALL_ID) paintDetail(); });

  // ── 화면 높이에 맞추기 — 분할 패널이 뷰포트를 채우고 각자 스크롤한다(모바일은 자연 흐름). ──
  function fit() {
    if (!split.isConnected) return;
    if (matchMedia('(max-width: 900px)').matches) { split.style.height = ''; return; }
    const top = split.getBoundingClientRect().top;
    split.style.height = Math.max(420, window.innerHeight - top - 26) + 'px';
  }
  const onResize = () => { if (!split.isConnected) { window.removeEventListener('resize', onResize); return; } fit(); };
