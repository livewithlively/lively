...
function tokenize(title: string): Set<string> {
  ...
  for (const m of (t.match(TOKEN_RE) || [])) {
    if (KO_RE.test(m)) {
      ...
      if (w.length >= 2 && !STOP_KO.has(w)) out.add(w);
    } else {
      const w = m.toLowerCase().replace(/^[.\-_+]+|[.\-_+]+$/g, '');
      if (w.length >= 2 && !STOP_EN.has(w) && !/^[\d.]+$/.test(w)) out.add(w);
    }
  }
  return out;
}
...
function mineThreads(docs: any[]): any[] {
  const buckets = new Map<string, any[]>();
  for (const d of docs) {
    const nums = String(d.title || '').match(ISSUE_RE) || [];
    for (const n of new Set(nums)) {
      if (!buckets.has(n)) buckets.set(n, []);
      (buckets.get(n) as any[]).push(d);
    }
  }
  const threads: any[] = [];
  for (const [id, members] of buckets) {
    if (members.length < 2) continue;   // 단건 번호는 스레드가 아니라 그냥 문서
    // 최신 판정 = 제목 파싱 날짜 우선(시간 규율), 없으면 updated_at 날짜부. sorted[0] = 진짜 현재 상태.
    const keyOf = (d: any) => parseTitleDate(d.title, d.updated_at) || String(d.updated_at || '').slice(0, 10);
  ...
  for (const t of threads) for (const d of t.docs) threaded.add(d.name);
  (threads as any).threadedNames = threaded;
  return threads;
}

// ── ② 주제 그룹 — 검증된 빈도 방식(V2). n<minDocs 면 빈 배열(소형 카테고리는 지도 무의미). ──
function mineThemes(docs: any[], catName: string, opts: any = {}): any[] {
  const n = docs.length;
  if (n < (opts.minDocs || 24)) return [];
  ...
  docToks.forEach((toks, i) => {
    for (const w of toks) {
      if (catToks.has(w)) continue;
      if (!df.has(w)) df.set(w, new Set());
      (df.get(w) as Set<number>).add(i);
    }
  });
  ...
  for (const s of seeds) {
    let absorbed = false;
    for (const k of kept) {
      if (k.w !== s.w && (s.w.includes(k.w) || (s.w.startsWith(k.w) && /^[a-z0-9_.+-]+$/.test(s.w)))) { absorbed = true; break; }
    }
    if (!absorbed) kept.push(s);
  }
  seeds = kept;
  ...
  for (const g of groups) for (const d of g.docs) covered.add(d.name);
  (groups as any).coveredNames = covered;
  return groups;
}
...
function parseTitleDate(title: string, ref?: string): string {
  const t = String(title || '');
  const refD = ref ? new Date(ref) : new Date();
  const refY = isNaN(refD.getTime()) ? new Date().getFullYear() : refD.getFullYear();
  let y = 0, mo = 0, d = 0;
  for (const m of t.matchAll(/\b(20\d{2})[-.](\d{1,2})[-.](\d{1,2})\b/g)) { y = +m[1]; mo = +m[2]; d = +m[3]; }
  if (!y) for (const m of t.matchAll(/(\d{1,2})월\s*(\d{1,2})일/g)) { mo = +m[1]; d = +m[2]; }
  if (!mo) {
    const tail = t.match(/\(([^)]*)\)\s*$/);   // 짧은 M.D 는 꼬리 괄호 메타 안에서만 신뢰
    //  뒤에 숫자·점·%·글자·한글이 붙으면 날짜가 아니라 단위(1.5배·3.2GB) — 오독 금지(파싱 실패가 안전).
    if (tail) for (const m of tail[1].matchAll(/(?:^|[\s·,])(\d{1,2})[./](\d{1,2})(?![\d.%가-힣A-Za-z])/g)) { mo = +m[1]; d = +m[2]; }
  }
...
function topicStem(title: string): string {
  return String(title || '')
...
function threadParts(thread: any): { label: string, kids: { doc: any, delta: string, date: string }[] } {
  const docs = thread.docs as any[];
  const titles = docs.map((x) => String(x.title || ''));
  const cleanTail = (s: string) => s.replace(/\([^)]*\)\s*$/, '')   // 닫힌 꼬리 괄호
  ...
  if (cutAt > 0 && prefix.length < Math.min(...titles.map((t) => t.length))) prefix = prefix.slice(0, cutAt + 1);
...
function mineEvents(docs: any[]): any[] {
  const now = Date.now();
  const isLive = (iso: string) => !!iso && (now - new Date(iso + 'T00:00:00').getTime()) <= 14 * 86400e3
    && (now - new Date(iso + 'T00:00:00').getTime()) >= -2 * 86400e3;
  const dateOf = (x: any) => parseTitleDate(x.title, x.updated_at);
  const threads = mineThreads(docs);
  const threaded: Set<string> = (threads as any).threadedNames || new Set();
  const events: any[] = [];
  ...
  for (const x of rest) {
    ...
    if (s.length < 6) continue;   // 짧은 줄기는 우연 일치 위험 — 접지 않는다
    if (!stems.has(s)) stems.set(s, []);
    (stems.get(s) as any[]).push(x);
  }
  ...
  for (const [stem, members] of stems) {
    ...
    for (const m of sorted) canonNames.add(m.name);
  ...
  for (const x of rest) {
    if (canonNames.has(x.name)) continue;
    const dm = dateOf(x);
