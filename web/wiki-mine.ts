...
function mnMast(actions: any[] = []) {
  return el('div', { class: 'wk-mn-titlerow' },
    el('h1', { class: 'wk-mn-title' }, el('span', { text: '내 소유 카테고리' }), el('span', { class: 'wk-mn-pulse', 'aria-hidden': 'true' })),
...
async function renderMineSurface(box: HTMLElement, ctx: any) {
  ...
  mine.append(
    ...
    el('div', { class: 'wk-mn-dateline' },
      el('span', { class: 'wk-row-m wk-mn-date', text: (now.getMonth() + 1) + '월 ' + now.getDate() + '일 (' + WK_DOW[now.getDay()] + ')' }),
  ...
  function fillLane(c: any, body: HTMLElement, bits: any, rows: any[]) {
    ...
    if (homeDoc && homeDoc.icon) { iconOf.set(String(c.id), homeDoc.icon); bits.ic.textContent = homeDoc.icon; bits.ic.hidden = false; paintStrip(); }
  ...
  function lane(c: any) {
    ...
    const foot = el('div', { class: 'wk-l2-foot' },
  ...
  function paintStrip() {
    ...
    const slots = cats.map((c) => {
      ...
      icon ? el('span', { class: 'wk-mn-chip-ic', 'aria-hidden': 'true', text: icon })
        : el('span', { class: 'wk-mn-chip-ic letter', 'aria-hidden': 'true', text: letter }),
  ...
  function applyOrder() {
    paintStrip();
    grid.append(...orderedCats().map((c) => laneEls.get(String(c.id))).filter(Boolean) as HTMLElement[]);
