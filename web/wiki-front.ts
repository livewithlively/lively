...
const PERIODS: Array<[string, string]> = [['', '전체 기간'], ['7d', '이번 주'], ['30d', '최근 30일'], ['today', '오늘']];
const AUTHORS: Array<[string, string]> = [['', '작성 전체'], ['human', '사람'], ['ai', 'AI'], ['observed', '외부 미러']];
function dropPill(label: string, opts: Array<[string, string]>, cur: string, onPick: (v: string) => void) {
...
export async function renderRecentSurface(box: HTMLElement, ctx: any) {
  ...
  if (!shown.length) {
    ...
    for (const e of shown) { const day = wkDayLabel(e.updated_at); const last = byDay[byDay.length - 1]; if (last && last[0] === day) last[1].push(e); else byDay.push([day, [e]]); }
...
async function paintReviewLane(host: HTMLElement, bySpace: any, ctx: any) {
  ...
  for (const it of items) {
    const decide = async (ok: boolean) => {
      ...
      try {
        if (it.kind === 'new') {
          if (ok) await api('/api/ui/knowledge/' + encodeURIComponent(it.e.name) + '/lifecycle', { method: 'POST', body: JSON.stringify({ lifecycle: 'active' }) });
          else await api('/api/ui/knowledge/' + encodeURIComponent(it.e.name) + '/delete', { method: 'POST' });
        } else {
        ...
        ctx.repaint();
      } catch (e) { toast('처리하지 못했습니다 — ' + ((e as Error).message || '')); }
    };
    const acts = el('span', { class: 'wk-rv-acts' },
      el('button', { class: 'wk-rv-btn ok', type: 'button', text: it.kind === 'new' ? '승인' : '반영', onclick: (ev: any) => { ev.stopPropagation(); void decide(true); } }),
      el('button', { class: 'wk-rv-btn no', type: 'button', text: '반려', onclick: (ev: any) => { ev.stopPropagation(); void decide(false); } }));
