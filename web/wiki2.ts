...
// ── 큐 아이템(#783 큐 API 2본 병합 — review.ts 와 동일 규칙) ──
interface W2Item {
...
function splitNote(note: string): { sum: string; cause: string | null } {
  const m = /(?:^|[\s.·])계기\s*[—\-–:]\s*/.exec(note);
  if (!m) return { sum: note, cause: null };
  return {
...
// ── 변경 블록 파생 — lineDiff 헝크를 헤딩 컨텍스트로 묶는다(클라 파생). 어휘 3개: 바뀜/추가/삭제. ──
...
function wordSpans(a: string, b: string): { prev: any; next: any } | null {
  ...
  if (ea - s > a.length * 0.9 && eb - s > b.length * 0.9) return null;
  return {
...
// ── 판정(엔드포인트 #783 그대로) ──
async function w2Approve(it: W2Item): Promise<boolean> {
  try {
    if (it.kind === 'new') {
    ...
    return true;
  } catch (e: any) { toast('실패 — ' + e.message, true); return false; }
}
async function w2Reject(it: W2Item): Promise<boolean> {
...
function feedLine(r: any): { sum: string; who: string } {
  if (r.provenance === 'observed') return { sum: '노션 원본 갱신 반영', who: r.external_system || 'notion' };
  if (r.change_note) {
...
export async function renderWiki2(view: any, sub: string, params: URLSearchParams): Promise<void> {
  ...
  const mode: 'board' | 'flow' | 'history' = sub === 'history' ? 'history' : (sub === 'review' ? 'flow' : 'board');
  if (params && params.has('cat')) f.cat = params.get('cat') || '';
  else if (mode === 'board' && (!params || Array.from(params.keys()).length === 0)) { f.cat = ''; f.by = ''; }
  ...
  function hashFor(m: 'board' | 'flow' | 'history', extra?: Record<string, string>): string {
  ...
  const syncHash = (m: 'board' | 'flow' | 'history') => history.replaceState(null, '', hashFor(m, scopeOut ? { scope: 'out' } : undefined));

  const main = el('section', { class: 'wk2-main' });
  const sideCtl = createWikiSide({
    selected: () => f.cat,
    onSelect: (v: string) => { f.cat = v || ''; location.hash = hashFor(mode === 'flow' ? 'board' : mode); },
    tools: false,
  });
  ...
  const allCats = (): any[] => {
    ...
    return ([] as any[]).concat(bs.business || [], bs.product || [], bs.system || []);
  };
  const catByKey = (key: string) => allCats().find((c) => String(c.key) === String(key)) || null;
  const gone = () => document.body.dataset.route !== 'wiki2';

  // ══════ 보드 ══════
  async function paintBoard(box: HTMLElement): Promise<void> {
    ...
    try {
      ...
      items = qs;
      feed = (fd && fd.entries) || [];
      if (pol) gateOn = ((pol.policies || []) as any[]).some((p) => p.enabled);
    } catch (e) {
      box.replaceChildren(errorNote(e, '검토 현황을 불러오지 못했습니다 — 검토 권한(memory)이 필요합니다'));
      return;
    }
    const selCat = f.cat ? sideCtl.findCat(f.cat) : null;
    const owned = (selCat ? [selCat] : mineKeys.map(catByKey).filter(Boolean)) as any[];
    const ownedKeys = new Set(owned.map((c) => String(c.key)));
    // 담당 카테고리별 정본 수 — limit=1 total 만(가볍게, 병렬).
    const canonN = new Map<string, number>();
    await Promise.all(owned.map(async (c) => {
      try { const r = await api('/api/ui/knowledge?limit=1&category=' + encodeURIComponent(String(c.id))); canonN.set(String(c.key), Number(r.total ?? 0)); }
      catch (_) { /* 생략 — 카드 메타만 빠짐 */ }
    }));
    if (gone()) return;

    const waitingOf = (key: string) => items.filter((i) => i.cat === key && !(i.kind === 'edit' && i.mode === 'applied'))
      .sort((a, b) => Number(!!b.conflict) - Number(!!a.conflict) || String(a.at).localeCompare(String(b.at)));
    const appliedOf = (key: string) => items.filter((i) => i.cat === key && i.kind === 'edit' && i.mode === 'applied');
    ...
    const kids: any[] = [];
    kids.push(el('div', { class: 'wk2-hero' },
      el('h2', { text: selCat ? (selCat.name || selCat.key) : '검토 현황' }),
      el('span', { class: 'sum' },
        el('span', { text: '검토 대기 ' }), el('b', { text: String(nWait) }),
        el('span', { text: ' · 사후확인 ' }), el('b', { text: String(nAck) }),
        el('span', { text: '  ｜  이번 주 ' + (selCat ? '변화 ' : '담당 카테고리 변화 ') }), el('b', { text: String(selCat ? feed.filter((r) => String(r.category_key || '') === String(selCat.key)).length : weekOwned) })),
      el('span', { class: 'sp' }),
      (() => {
        const n = selCat ? (waitingOf(String(selCat.key)).length + appliedOf(String(selCat.key)).length) : items.length;
        const b = el('button', { class: 'btn btn-primary', text: '검토 시작' + (n ? ` (${n})` : '') }) as HTMLButtonElement;
        b.disabled = !n;
        b.onclick = () => { location.hash = hashFor('flow'); };
        return b;
      })()));
    if (queueTotal === 0 && gateOn === false) {
      kids.push(el('div', { class: 'wk2-gateoff' },
        el('span', { text: '지금은 모든 AI 변경이 검증 없이 즉시 반영되고 있습니다 — ' }),
        el('a', { href: '#/system/ingest-policy', text: '게이트 정책 설정 →' })));
    }

    const zoneCard = (c: any, opts?: { extended?: boolean }) => {
      ...
      const head = el('div', { class: 'wk-ccard-body wk2-zhead' },
        el('span', { class: 'wk-ccard-ic letter', 'aria-hidden': 'true', text: initial }),
        el('div', { class: 'wk2-zh-row' },
          el('span', { class: 'nm', text: c.name || c.key }),
          mineKeys.includes(key) ? el('span', { class: 'own', text: '담당' }) : null,
          el('span', { class: 'sp' }),
          el('span', { class: 'm', text: (cn != null ? `정본 ${cn} · ` : '') + `이번 주 ${feed.filter((r) => String(r.category_key || '') === key).length}` })));
      let todo: any;
      if (first) {
        const isAck = first.kind === 'edit' && first.mode === 'applied';
        const goBtn = el('button', { class: 'btn btn-primary btn-sm', text: '검토 →' }) as HTMLButtonElement;
        goBtn.onclick = () => { f.cat = String(c.id); location.hash = hashFor('flow'); };
        todo = el('div', { class: 'wk2-todo' },
          el('div', { class: 'lb', text: `검토 대기 · ${waiting.length + applied.length}` }),
          el('div', { class: 'q', text: first.note ? splitNote(first.note).sum : (first.kind === 'new' ? '신규 문서 제안' : '수정 제안') }),
          el('div', { class: 'qm' },
            el('span', { text: first.title + (isAck ? ' — 이미 반영됨, 사후확인 · ' : ' · ') }),
            el('span', { class: 'm', text: `AI(${first.agent || first.who}) · ${relTime(first.at)}` })),
          el('div', { class: 'act' }, goBtn,
            waiting.length && applied.length ? el('span', { class: 'more', text: `사후확인 ${applied.length}건 포함` }) : null));
      } else {
        todo = el('div', { class: 'wk2-todo ok' },
          el('div', { class: 'q' }, el('span', { class: 'wk2-okdot' }), el('span', { text: '검토 대기 없음' })));
      }
    ...
    for (const c of owned) grid.append(zoneCard(c, { extended: !!selCat }));
    ...
    if (!selCat && outItems.length) {
      ...
      const isAck = first.kind === 'edit' && first.mode === 'applied';
      const goBtn = el('button', { class: 'btn btn-ghost btn-sm', text: '검토 →' }) as HTMLButtonElement;
      ...
      histA.onclick = (e: any) => { e.preventDefault(); f.cat = ''; location.hash = hashFor('history'); };
      grid.append(el('div', { class: 'wk2-zone' },
        wkAurora('wiki2-out', 'system', { cls: 'wk-ccard-cover', watermark: '외' }),
        el('div', { class: 'wk-ccard-body wk2-zhead' },
          el('span', { class: 'wk-ccard-ic letter', 'aria-hidden': 'true', text: '외' }),
          el('div', { class: 'wk2-zh-row' }, el('span', { class: 'nm', text: '담당 외 대기' }), el('span', { class: 'sp' }), el('span', { class: 'm', text: outItems.length + '건' }))),
        el('div', { class: 'wk2-todo' },
          el('div', { class: 'lb', text: `다른 카테고리의 검토 대기 · ${outItems.length}` }),
          el('div', { class: 'q', text: first.note ? splitNote(first.note).sum : (first.kind === 'new' ? '신규 문서 제안' : '수정 제안') }),
          el('div', { class: 'qm' }, el('span', { text: `${first.title}${isAck ? ' — 이미 반영됨, 사후확인' : ''} · ${first.catName} · ` }), el('span', { class: 'm', text: `AI(${first.agent || first.who}) · ${relTime(first.at)}` })),
          el('div', { class: 'act' }, goBtn)),
        el('div', { class: 'wk2-fu' },
          el('div', { class: 'lb', text: '조직 전체' }),
          el('div', { class: 'wk2-fur' }, el('span', { class: 'tx', text: `오늘 변화 ${today.length}건 · 미러 갱신 ${today.filter((r) => r.provenance === 'observed').length}건` }))),
        el('div', { class: 'wk2-zf' }, el('span', { class: 'sp' }), histA)));
    }
    kids.push(grid);
    ...
    if (!selCat) {
      ...
      if (rest.length) {
        ...
        for (const c of rest) {
          const chipEl = el('span', { class: 'wk2-oc', role: 'button', tabindex: '0' },
            el('span', { text: c.name || c.key }),
            Number.isFinite(Number(c.knowledge_count)) ? el('span', { class: 'm', text: String(c.knowledge_count) }) : null,
            byCatMap.get(String(c.key)) ? el('span', { class: 'n', text: String(byCatMap.get(String(c.key))) }) : null);
    ...
    if (!owned.length && !outItems.length) {
      kids.push(el('div', { class: 'wk2-empty' },
        el('div', { class: 'big', text: '담당 카테고리가 없습니다' }),
        el('div', { text: '팀의 오너 카테고리가 설정되면 여기에 현황 카드가 표시됩니다. 사이드바에서 카테고리를 직접 선택할 수도 있습니다.' })));
    }
    box.replaceChildren(...kids);
  }

  // ══════ 검토 플로우 — 한 건씩. 카드 5구역: 위상 → 제안 → 계기·출처 → 변경 → 결과. ══════
  async function paintFlow(box: HTMLElement): Promise<void> {
    box.replaceChildren(skeleton('검토 항목을 불러오는 중'));
    let items: W2Item[] = [];
    try { items = await fetchQueue(); }
    catch (e) { box.replaceChildren(errorNote(e, '검토 항목을 불러오지 못했습니다 — 검토 권한(memory)이 필요합니다')); return; }
    const selCat = f.cat ? sideCtl.findCat(f.cat) : null;
    const ownedSet = new Set(mineKeys.map(String));
    let scoped = items;
    if (selCat) scoped = items.filter((i) => i.cat === String(selCat.key));
    else if (scopeOut) scoped = items.filter((i) => !ownedSet.has(i.cat));
    // 순서: (전체일 때) 담당 먼저 → 그 외. 그 안에서 대기(경합 → 오래된 순) → 사후확인.
    const rank = (i: W2Item) => (i.kind === 'edit' && i.mode === 'applied' ? 1 : 0);
    scoped = scoped.slice().sort((a, b) =>
      Number(ownedSet.has(b.cat)) - Number(ownedSet.has(a.cat))
      || rank(a) - rank(b)
    ...
    box.replaceChildren(el('div', { class: 'wk2-flowbar' },
      el('span', { class: 'ttl', text: '지식 검토' + (selCat ? ' — ' + (selCat.name || selCat.key) : scopeOut ? ' — 담당 외' : '') }),
      prog, bar, quitBtn), holder, kbd);

    const setProg = () => {
      prog.textContent = `${Math.min(idx + 1, scoped.length)} / ${scoped.length}`;
      (bar.firstChild as HTMLElement).style.width = scoped.length ? `${Math.round((idx / scoped.length) * 100)}%` : '0%';
    };

    const showDone = () => {
      keys?.abort(); keys = null;
      ...
      const parts: string[] = [];
      if (stats.approved) parts.push(`승인 ${stats.approved}`);
      if (stats.acked) parts.push(`확인 ${stats.acked}`);
      if (stats.rejected) parts.push(`반려 ${stats.rejected}`);
      if (stats.held) parts.push(`보류 ${stats.held}`);
      const histA = el('a', { href: '#', text: '기록에서 보기' });
      histA.onclick = (e: any) => { e.preventDefault(); location.hash = hashFor('history'); };
      const boardA = el('a', { href: '#', text: '보드로 →' });
      boardA.onclick = (e: any) => { e.preventDefault(); location.hash = hashFor('board'); };
      holder.replaceChildren(el('div', { class: 'wk2-done' },
        el('div', { class: 'big' }, el('span', { class: 'wk2-okdot' }),
          el('span', { text: processed ? `검토 완료 — ${processed}건 처리 (${parts.join(' · ')})` : (scoped.length ? `검토 종료 — ${parts.join(' · ') || '처리 없음'}` : '검토할 항목이 없습니다') })),
        el('div', { class: 'dim' }, el('span', { text: processed ? '확정된 내용은 각 지식의 정본에 반영되었습니다 · ' : '' }), histA, el('span', { text: '  ·  ' }), boardA)));
      void loadSummary();
    };

    const next = () => { idx++; if (idx >= scoped.length) showDone(); else void showCard(); };
    ...
    async function showCard(): Promise<void> {
      ...
      let detail: any = null, kd: any = null;
      try {
      ...
      } catch (e) { holder.replaceChildren(errorNote(e, '내용을 불러오지 못했습니다')); return; }
      if (gone()) return;
      ...
      // ① 위상 — 이 지식이 지금 무엇인가.
      const stBits: any[] = [];
      if (it.kind === 'new') stBits.push(el('span', { class: 'ok', text: '신규 제안 — 아직 정본 아님' }));
      else if (k.provenance === 'observed') stBits.push(el('span', { text: '노션 미러 — 원본이 진실' }));
      else stBits.push(el('span', { class: 'ok', text: '정본 v' + curVer }));
      const catType = [it.catName || k.category_name, TYPE_LABEL[String(k.type)] || k.type].filter(Boolean).join(' · ');
      if (catType) { stBits.push(el('span', { class: 'dot', text: '·' }), el('span', { text: catType })); }
      if (it.kind !== 'new' && k.updated_by) {
        stBits.push(el('span', { class: 'dot', text: '·' }),
          el('span', {}, el('span', { text: `마지막 수정 ${k.updated_by}${k.confidence === 'human' ? '' : '(AI)'} ` }), el('span', { class: 'm', text: relTime(String(k.updated_at)) })));
      }
      const links = (k.links || {}) as any;
      const backs: any[] = Array.isArray(links.incoming) ? links.incoming : [];
      stBits.push(el('span', { class: 'dot', text: '·' }), el('span', { text: `참조 문서 ${backs.length}` }));
      const standing = el('div', { class: 'wk2-standing' },
        el('div', { class: 't', text: it.title }),
        el('div', { class: 'st' }, ...stBits));

      // ② 제안.
      const chipTxt = it.kind === 'new' ? 'AI 신규 제안' : (isAck ? 'AI 수정 — 반영됨 · 사후확인' : 'AI 수정 제안');
      const prop = el('div', { class: 'wk2-prop' },
        el('span', { class: 'wk2-chip pr', text: chipTxt }),
        el('span', { class: 'm', text: `${it.agent || it.who} · ${relTime(it.at)}${(it.edits || 1) > 1 ? ` · ${it.edits}회 누적` : ''}` }));
      ...
      if (it.kind === 'edit' && detail) {
        ...
        if (!cur) { body.append(el('p', { class: 'wk2-warn', text: '대상 지식이 삭제되었습니다 — 반려로 정리하세요.' })); }
        else {
          ...
          const whyBits: any[] = [];
          if (noteSplit && noteSplit.cause) whyBits.push(el('span', { class: 'lbl', text: '계기' }), el('span', { text: noteSplit.cause + '  ' }));
          else if (!noteSplit) whyBits.push(el('span', { class: 'lbl', text: '요약' }), el('span', { text: '제안자가 변경 요약을 제출하지 않아 구조 요약으로 표시합니다  ' }));
          const srcs: any[] = Array.isArray(k.sources) ? k.sources : [];
          if (srcs.length) {
            whyBits.push(el('span', { class: 'lbl', text: '출처' }));
            srcs.slice(0, 2).forEach((s: any, i: number) => {
              whyBits.push(el('span', { text: (i ? ' · ' : '') + (s.title || s.name || s.kind || '자료') + ' ' }),
                el('span', { class: 'm', text: s.occurred_at ? relTime(s.occurred_at) : '' }));
            });
          }
          if (k.external_url) whyBits.push(el('span', { text: '  ' }), el('a', { href: String(k.external_url), target: '_blank', rel: 'noopener', text: '원본 열기 ↗' }));
          if (whyBits.length) body.append(el('div', { class: 'wk2-fwhy' }, ...whyBits));
          if (it.conflict) body.append(el('p', { class: 'wk2-warn', text: '⚠ 제안 이후 정본이 또 수정되었습니다 — 승인하면 그 수정을 이 제안본으로 덮어씁니다. 원문 비교로 확인하세요.' }));
          const renderBlock = (b: W2Block) => {
            ...
            if (b.op === '바뀜') {
              kids2.push(el('div', { class: 'wk2-ba' },
                el('span', { class: 'lab', text: '이전' }), el('span', { class: 'prev' }, ws ? ws.prev : mdClip(b.before.join('\n'))),
                el('span', { class: 'lab', text: '이후' }), el('span', { class: 'next' }, ws ? ws.next : mdClip(b.after.join('\n')))));
            } else if (b.op === '추가') kids2.push(mdClip(b.after.join('\n')));
            else kids2.push(el('div', { class: 'wk2-ba' }, el('span', { class: 'lab', text: '삭제' }), el('span', { class: 'prev' }, mdClip(b.before.join('\n')))));
        ...
        if (noteSplit && noteSplit.cause) body.append(el('div', { class: 'wk2-fwhy' }, el('span', { class: 'lbl', text: '계기' }), el('span', { text: noteSplit.cause })));
        try {
        ...
        body.append(el('div', { class: 'wk2-chg' },
      ...
      // ⑤ 결과 — 판정의 의미.
      const topRefs = backs.slice(0, 3).map((b: any) => b.title || b.name).filter(Boolean);
      const refTxt = backs.length
        ? `참조 문서 ${backs.length}건(${topRefs.map((t: string) => t).join(' · ')}${backs.length > 3 ? ` 외 ${backs.length - 3}` : ''})이 확정된 내용을 근거로 참조합니다.`
        : '이 지식을 참조하는 문서는 아직 없습니다.';
      let okK = '승인하면', okV: any[] = [], noK = '반려하면', noV = '';
      if (it.kind === 'new') {
      ...
      const outcome = el('div', { class: 'wk2-outcome' },
        el('div', { class: 'row' }, el('span', { class: 'k', text: okK }), el('span', { class: 'v' }, ...okV)),
        el('div', { class: 'row' }, el('span', { class: 'k no', text: noK }), el('span', { class: 'v', text: noV })));

      // 판정.
      const okBtn = el('button', { class: 'btn btn-primary', text: it.kind === 'new' ? '승인' : (isAck ? '확인' : '승인') }) as HTMLButtonElement;
      const noBtn = el('button', { class: 'skip', text: isAck ? '되돌리기' : '반려' }) as HTMLButtonElement;
      const holdBtn = el('button', { class: 'skip', text: '보류' }) as HTMLButtonElement;
      const lock = (b: boolean) => { okBtn.disabled = b; noBtn.disabled = b; holdBtn.disabled = b; };
      okBtn.onclick = async () => { lock(true); const ok = await w2Approve(it); if (!ok) { lock(false); return; } if (isAck) stats.acked++; else stats.approved++; next(); };
      noBtn.onclick = async () => { lock(true); const ok = await w2Reject(it); if (!ok) { lock(false); return; } stats.rejected++; next(); };
      ...
      const inEditable = (t: any): boolean => {
        const e2 = (t && t.nodeType === 1 ? t : document.activeElement) as HTMLElement | null;
        return !!(e2 && e2.closest && e2.closest('input, textarea, select, [contenteditable="true"], [contenteditable=""]'));
      };
      document.addEventListener('keydown', (e: any) => {
        if (!document.body.contains(holder)) { keys?.abort(); keys = null; return; }
        if (e.metaKey || e.ctrlKey || e.altKey) return;
        if (inEditable(e.target)) return;
        if (e.key === 'a') { e.preventDefault(); okBtn.click(); }
        else if (e.key === 'r') { e.preventDefault(); noBtn.click(); }
        else if (e.key === 's') { e.preventDefault(); holdBtn.click(); }
        else if (e.key === 'Escape') { e.preventDefault(); quit(); }
      }, { signal: keys.signal });
    }

    if (!scoped.length) showDone();
    else void showCard();
  }
  ...
  async function paintHistory(box: HTMLElement): Promise<void> {
    ...
    interface Ev { at: string; kind: 'upd' | 'mirror' | 'archived' | 'pending'; name: string; title: string;
      catName: string; sum: string; sub: string; human: boolean; mirror: boolean }
    const evs: Ev[] = [
      ...feed.map((r: any): Ev => {
        const L = feedLine(r);
        let sub = '';
        if (r.provenance === 'observed') sub = '노션 미러 — 원본이 진실';
        else if (r.change_note && r.change_reviewed_by) sub = `AI 제안 → ${r.change_reviewed_by} 승인 · 정본 v${r.version} 확정`;
        else if (r.change_note) sub = `AI 수정 · 정본 v${r.version}`;
        else if (r.confidence === 'human') sub = `${r.updated_by || '사람'} 직접 수정 · 정본 v${r.version}`;
        else sub = `AI 수정 · v${r.version}`;
        if (r.lifecycle === 'archived') sub = '보관됨 · ' + sub;
        return {
    ...
    const boardBtn = el('button', { class: 'btn btn-ghost btn-sm', text: '보드 →' }) as HTMLButtonElement;
    boardBtn.onclick = () => { location.hash = hashFor('board'); };

    const kids: any[] = [el('div', { class: 'wk2-hero' },
      el('h2', { text: (selCat ? (selCat.name || selCat.key) + ' — ' : '') + '기록' }),
      el('span', { class: 'sum' }, el('span', { text: '최근 14일 ' }), el('b', { text: String(evs.length) }), el('span', { text: '건' })),
      el('span', { class: 'sp' }), bySel, boardBtn)];

    const days = new Map<string, Ev[]>();
    for (const e2 of evs) {
      const kkey = dayKey(e2.at);
      if (!days.has(kkey)) days.set(kkey, []);
    ...
    const trow = (e2: Ev, showTm = true) => {
      const goFlow = e2.kind === 'pending' ? (() => { const a = el('a', { href: '#', text: '검토 →', style: 'font-size:12px;white-space:nowrap' }); a.onclick = (ev: any) => { ev.preventDefault(); location.hash = hashFor('flow'); }; return a; })() : null;
      return el('div', { class: 'wk2-trow' },
        showTm ? el('span', { class: 'tm', text: tm(e2.at) }) : null,
        wkTick({ confidence: e2.human ? 'human' : null, provenance: e2.mirror ? 'observed' : null }),
        el('div', { class: 'body' },
          el('div', { class: 'l1' },
            el('a', { class: 't', href: '#/k/' + encodeURIComponent(e2.name), target: '_blank', rel: 'noopener', text: e2.title }),
            el('span', { class: 'k', text: e2.sum }),
            e2.kind === 'pending' ? el('span', { class: 'wk2-pill', text: '검토 대기' }) : null,
            el('span', { class: 'sp' }), goFlow),
    ...
    for (const [dk, list] of days) {
      ...
      const day = el('div', {},
        el('div', { class: 'wk2-dayh' }, el('span', { text: dk }),
          el('span', { class: 'm', text: `변화 ${rest.length}${mirrors.length ? ' · 미러 갱신 ' + mirrors.length : ''}` })));
      const cats = new Map<string, Ev[]>();
      for (const e2 of rest) {
        if (!cats.has(e2.catName)) cats.set(e2.catName, []);
        cats.get(e2.catName)!.push(e2);
      }
      for (const [cn, ces] of cats) {
        if (!selKey && cats.size > 1) day.append(el('div', { class: 'wk2-dom', text: cn }));
        for (const e2 of ces) day.append(trow(e2));
      }
      if (mirrors.length) {
        const det = el('details', { class: 'wk2-mirror' },
          el('summary', {}, el('span', { text: `○ 노션 미러 갱신 ${mirrors.length}건 — 미분류` }), el('span', { class: 'm', text: '펼치기 ▸' })));
        for (const e2 of mirrors) det.append(trow(e2));
        day.append(det);
      }
      kids.push(day);
    }
  ...
  async function repaint(): Promise<void> {
    ...
    else if (mode === 'flow') await paintFlow(box);
    else await paintBoard(box);
  }
