...
const STATE = (cat: string | null | undefined): { word: string; cls: string } =>
  cat === 'done' ? { word: '끝남', cls: 'done' } : cat === 'unstarted' ? { word: '시작 전', cls: 'todo' } : { word: '진행 중', cls: 'run' };
...
export function mountProjectView(host: HTMLElement, opts: ProjectViewOpts): ProjectViewHandle {
  ...
  let dead = false;
  let fileTotal: number | null = null;   // 공유 폴더 재귀 파일 수(자료 n) — 마운트·업로드 때만 잰다
  ...
  function paintDoor(): void {
    ...
    const vital = (label: string, val: string, href?: string, extra?: HTMLElement | null): HTMLElement => {
    ...
    const facesWrap = members.length
      ? el('span', { class: 'pv-faces', title: members.map((m) => String(m.member_id ?? m)).join(' · ') },
          ...members.slice(0, 5).map((m) => personFace(String(m.member_id ?? m), 'pv-face-s')),
          members.length > 5 ? el('span', { class: 'pv-face-more', text: '+' + (members.length - 5) }) : null)
      : null;

    door.replaceChildren(
      el('div', { class: 'pv-eyebrow' },
        el('span', { class: 'pv-idchip mono', text: '#' + p.id }),
        p.list && p.list.name ? el('span', { class: 'pv-listname', text: p.list.name }) : null,
        el('span', { class: 'pv-eyebrow-sp' }),
        p.updated_at ? el('span', { class: 'pv-when', text: '수정 ' + relTime(p.updated_at) }) : null),
      el('div', { class: 'pv-title-line' },
        el('h1', { class: 'pv-title', text: p.name }),
        el('span', { class: 'pv-stpill ' + st.cls }, el('span', { class: 'd', 'aria-hidden': 'true' }), el('span', { text: st.word }))),
      el('div', { class: 'pv-vitals' },
        busy ? el('span', { class: 'pv-live' }, el('span', { class: 'v2-dot busy', 'aria-hidden': 'true' }), el('span', { text: `지금 ${busy}개 돌아가는 중` })) : null,
        vital('세션', String(sess.length)),
        vital('할 일', tasks.length ? `${tdone}/${tasks.length}` : '0', '#/app/projects2/p/' + id,
          tasks.length ? el('span', { class: 'pv-minibar', 'aria-hidden': 'true' }, el('span', { class: 'f', style: 'width:' + Math.round((tdone / tasks.length) * 100) + '%' })) : null),
        vital('자료', fileTotal == null ? '—' : String(fileTotal)),
        vital('지식', String(knN)),
        facesWrap));
  }
  ...
  function paintComments(list: FeedComment[]): void {
    ...
    if (!list.length) {
      cz.body.replaceChildren(el('div', { class: 'pv-blank' },
        el('p', { class: 'h', text: '아직 코멘트가 없어요.' }),
        el('p', { class: 'sub', text: '결정·부탁·메모를 남기면 이 방의 모두가 봅니다.' })));
      return;
    }
    cz.body.replaceChildren(...list.map((c) => el('div', { class: 'pv-cmt' },
      personFace(String(c.actor || ''), 'pv-cface', c.display_name),
      el('div', { class: 'pv-cmt-main' },
        el('div', { class: 'pv-cmt-h' },
          el('b', { text: c.display_name || c.actor || '?' }),
          el('span', { class: 'ts', text: relTime(c.ts) })),
  ...
  async function loadComments(): Promise<void> {
    ...
    } catch { if (!dead) cz.body.replaceChildren(el('div', { class: 'pv-blank' }, el('p', { class: 'h', text: '코멘트를 불러오지 못했어요.' }))); }
  }
  ...
  function paintSessions(): void {
    ...
    if (!sess.length) {
      sz.body.replaceChildren(el('div', { class: 'pv-blank' },
        el('p', { class: 'h', text: '아직 이 방에서 돈 세션이 없어요.' }),
    ...
    sz.body.replaceChildren(...sess.slice(0, 60).map((s) => {
      ...
      return el('a', { class: 'pv-row' + (s.live && s.alive ? '' : ' dim') + acc, href: '#/s/' + encodeURIComponent(s.id), title: s.label },
  ...
  function paintFolder(items: any[]): void {
    ...
    if (!folderItems.length) {
      shelfBody.replaceChildren(el('div', { class: 'pv-shelf-blank' },
        el('span', { text: fst.path ? '빈 폴더예요 — ' : '아직 파일이 없어요 — ' }),
        el('span', { text: '파일이나 폴더를 이 선반에 끌어다 놓으면 올라갑니다.' })));
      return;
    }
    ...
    shelfBody.replaceChildren(...sorted.slice(0, 120).map((it) => {
      ...
      return el('button', { class: 'pv-file' + (isDir ? ' dir' : ''), type: 'button', title: it.name + (it.mtime ? '\n' + fmtFileDateFull(it.mtime) : ''),
        ...
        el('span', { class: 'ic', 'aria-hidden': 'true' }, fileIconSvg(it.name, isDir)),
        el('span', { class: 'nm', text: it.name }),
        el('span', { class: 'mt', text: isDir ? '폴더' : fmtSize(it.size) }));
    }));
  }
  ...
  async function loadFileTotal(): Promise<void> {
    ...
    try { const d: any = await api(F + id + '/shared/manifest'); if (!dead && d && typeof d.count === 'number') { fileTotal = d.count; paintDoor(); } }
    catch { /* 문패만 '—' 로 남는다 */ }
  }
  ...
  async function reload(): Promise<void> {
    try {
      ...
      if (dead) return;
      if (d && d.project) { detail = d; paintDoor(); if (!editing) paintBody(); paintSessions(); }
    } catch { /* 다음 턴에 다시 */ }
  }
  ...
  const tick = window.setInterval(() => { if (!document.hidden) { paintSessions(); paintDoor(); } }, 25000);
