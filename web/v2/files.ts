...
interface LsItem { name: string; type: 'dir' | 'file'; size: number; mtime?: number; link?: boolean; linkTarget?: string }
...
const authFetch = (path: string, init?: RequestInit): Promise<Response> => {
  ...
  return fetch(apiUrl(path), Object.assign({}, init, { headers, credentials: 'same-origin' as RequestCredentials }));
};

const fileIcon = (name: string, type: string): string =>
  type === 'dir' ? '📁' : /\.(png|jpe?g|gif|svg|webp|avif|bmp)$/i.test(name) ? '🖼' : /\.(md|markdown)$/i.test(name) ? '📝' : '📄';

/** 세션 파일 탐색기를 host 에 그린다(우패널 한 칸을 통째로 쓴다). */
export function createSessionFiles(host: HTMLElement, opts: FilesOpts): FilesHandle {
  const sid = opts.sessionId;
  const nodeQ = (join: string): string => (opts.node ? join + 'node=' + encodeURIComponent(String(opts.node)) : '');
  const sUrl = (suffix: string): string =>
    '/api/ui/terminal/sessions/' + encodeURIComponent(sid) + suffix + nodeQ(suffix.indexOf('?') >= 0 ? '&' : '?');

  let curDir = '';
  let shareRoot: string | null = null;   // 'shared' | 'personal' | null(좌표 없음 = 원격 노드 등 → 링크 버튼 안 그림)
  let sharePath = '';
  // 지금 보는 폴더가 어떤 심링크를 타고 들어온 것인지 — {세션 기준 경로: 링크 목적지}. 경로줄 아래 한 줄로 말해 준다.
  let linkFrom: { path: string; target: string } | null = null;
  let destroyed = false;
  ...
  const upInput = el('input', { type: 'file', multiple: '', hidden: '' }) as HTMLInputElement;

  const iconBtn = (label: string, title: string, onclick: () => void): HTMLElement =>
    el('button', { class: 'fx-hbtn', type: 'button', title, 'aria-label': title, text: label, onclick });

  const head = el('div', { class: 'v2-aside-h fx-head' },
    el('b', { text: '파일' }),
    el('span', { class: 'fx-scope', text: '세션 작업 폴더' }),
    el('span', { class: 'fx-hacts' },
      iconBtn('＋', '새 폴더', () => { void newFolder(); }),
      iconBtn('⬆', '업로드', () => { upInput.value = ''; upInput.click(); }),
      iconBtn('⟳', '새로고침', () => { void loadDir(curDir); }),
      opts.onClose ? iconBtn('×', '타임라인으로 돌아가기', () => opts.onClose!()) : null));
  ...
  function paintPath(): void {
    ...
    const kids: HTMLElement[] = [el('button', { class: 'fx-crumb', type: 'button', text: '/', title: '세션 작업 폴더', onclick: () => { void loadDir(''); } })];
    let acc = '';
    ...
    linkEl.hidden = !on;
    if (on && linkFrom) linkEl.replaceChildren(el('span', { class: 'fx-link-ic', text: '↪', 'aria-hidden': 'true' }), el('span', { class: 'mono', text: linkFrom.target }));
  }
  ...
  async function loadDir(p: string): Promise<void> {
    ...
    const mine = ++seq;
    curDir = p || '';
    if (linkFrom && !(curDir === linkFrom.path || curDir.startsWith(linkFrom.path + '/'))) linkFrom = null;
    paintPath();
    ...
    catch (e: any) { if (mine === seq) listEl.replaceChildren(el('p', { class: 'v2-empty', text: '폴더를 읽지 못했어요 — ' + (e?.message || e) })); return; }
    if (destroyed || mine !== seq) return;
    shareRoot = data.shareRoot || null;
    sharePath = data.sharePath || '';
    const items: LsItem[] = Array.isArray(data.items) ? data.items : [];
    const rows: HTMLElement[] = [];
    if (data.parent !== null && curDir) rows.push(el('div', { class: 'fx-row fx-up' },
      el('button', { class: 'fx-open', type: 'button', title: '상위 폴더', onclick: () => { void loadDir(String(data.parent || '')); } },
        el('span', { class: 'fx-ic', text: '↩', 'aria-hidden': 'true' }), el('span', { class: 'fx-nm', text: '..' }))));
    for (const it of items) {
      ...
      rows.push(el('div', { class: 'fx-row' + (it.link ? ' is-link' : '') },
        el('button', { class: 'fx-open', type: 'button', title: tip,
          ...
          it.link ? el('span', { class: 'fx-link', text: '↪', 'aria-hidden': 'true' }) : null,
          el('span', { class: 'fx-sz', text: isDir ? '' : fmtSize(it.size) })),
        el('span', { class: 'fx-acts' },
          shareRel ? act(shareLinkIcon(), '링크 복사', () => { void copyFileLink(shareRoot as string, shareRel, isDir ? 'dir' : 'file'); }) : null,
          !isDir ? act('⬇', '다운로드', () => { void download(childPath, it.name); }) : null)));
    }
    listEl.replaceChildren(...(rows.length ? rows : [el('p', { class: 'v2-empty', text: '이 폴더는 비어 있어요. 파일을 끌어다 놓거나 [⬆] 로 올릴 수 있어요.' })]));
  }
  function act(label: string | Node, title: string, onClick: () => void): HTMLElement {
    return el('button', { class: 'fx-act', type: 'button', title, 'aria-label': title, onclick: onClick }, label as any);
  }

  // ── 미리보기 — 목록 자리에 제자리 전환. 판정·렌더·툴바는 공용 렌더러가 소유한다(#1436). ──
  async function showPreview(rel: string, name: string, size: number, shareRel: string | null): Promise<void> {
    const mine = ++seq;
    const viewUrl = sUrl('/file?path=' + encodeURIComponent(rel));
    const dlUrl = sUrl('/file?path=' + encodeURIComponent(rel) + '&download=1');
    const mkBtn = (label: string, onClick: () => void): HTMLElement => el('button', { class: 'fx-hbtn', type: 'button', text: label, onclick: onClick });
    const bar = el('div', { class: 'fx-pbar' },
      el('button', { class: 'fx-hbtn', type: 'button', text: '← 뒤로', onclick: () => { void loadDir(curDir); } }),
      el('span', { class: 'fx-pname', title: name, text: name }));
    ...
    try {
      out = await buildFilePreview({
        ...
        onError: (m: string) => toast(m, true),
      });
    } catch (e: any) {
      if (mine === seq) stage.replaceChildren(el('p', { class: 'v2-empty', text: '미리보기 실패 — ' + (e?.message || e) }));
      return;
    }
    if (destroyed || mine !== seq) return;
    bar.append(...out.tools,
      shareRel ? el('a', { class: 'fx-hbtn', href: fileLinkUrl(shareRoot as string, shareRel), target: '_blank', rel: 'noopener', title: '전체화면(새 탭)으로 열기', text: '⛶' }) : null,
      el('button', { class: 'fx-hbtn', type: 'button', title: '다운로드', text: '⬇', onclick: () => { void download(rel, name); } }));
  ...
  async function uploadMany(files: File[]): Promise<void> {
    for (const f of files) {
      ...
      try {
        ...
        if (!res.ok) { const d = await res.json().catch(() => null); throw new Error((d && d.error) || String(res.status)); }
        toast('업로드: ' + f.name);
      } catch (e: any) { toast('업로드 실패: ' + f.name + ' — ' + (e?.message || e), true); }
    }
    void loadDir(curDir);
  }
  upInput.addEventListener('change', () => { const fs = [...(upInput.files || [])]; if (fs.length) void uploadMany(fs); });
  ...
  root.addEventListener('drop', (e: DragEvent) => {
    ...
    e.preventDefault(); root.classList.remove('fx-drag');
  ...
  async function newFolder(): Promise<void> {
    ...
    try { await api(sUrl('/mkdir?path=' + encodeURIComponent(rel)), { method: 'POST', body: '{}' }); toast('폴더를 만들었어요: ' + name.trim()); void loadDir(curDir); }
    catch (e: any) { toast('폴더를 만들지 못했어요 — ' + (e?.message || e), true); }
  }
