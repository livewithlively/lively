...
export type ShellSurface =
  /** 앱이다 — AppPackage 로 뜬다(매니페스트·AppInstance·권한·창 규칙을 전부 받는다). */
  | { kind: "app"; appId: string; note?: string }
  /** OS 표면이다 — 앱이 **아니어야** 하는 이유가 있다. 이유 없이 여기 넣지 마라. */
  | { kind: "os"; why: string }
  /** 아직 앱이 아니다 — 앱화 대상. plan 은 "무엇으로 만들 것인가". */
  | { kind: "todo"; plan: string };
 ...
 */
export const SHELL_SURFACES: Record<string, ShellSurface> = {
  ...
  inbox: { kind: "todo", plan: "「확인할 것」 — 내용을 그리는 화면이므로 앱이다. global builtin(project=global, single-instance) 후보." },
...
export function activeNavKey(page: string, id?: string): string {
  if (page === "" || page === "dashboard") return "home";
  if (page === "p" || page === "s") return page + ":" + (id ?? "");
