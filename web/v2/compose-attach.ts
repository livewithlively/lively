...
export interface ComposerAttach {
  ...
  fileIn: HTMLElement;
  /** 입력칸 붙여넣기를 건다 — **입력칸당 한 번만**(같은 ta 에 두 번 걸면 첨부가 두 벌씩 올라간다). */
  wirePaste(ta: HTMLTextAreaElement): void;
  /** 드롭존(zone 에 드롭, hi 에 .drop-active)을 건다 — 판을 새로 그릴 때마다 그 판에. */
  wireDrop(zone: HTMLElement, hi?: HTMLElement): void;
  /** 아직 올리는 중인 파일이 있다 — 전송을 막고 사람에게 말할 것. */
  busy(): boolean;
  /** 다 올라간 첨부의 프롬프트 꼬리('' 가능) — 지시 본문 뒤에 그대로 붙인다. */
  tail(): string;
  /** 전송 후 초기화(올리는 중이던 것은 취소). */
  clear(): void;
}

/** projectId() > 0 이면 그 프로젝트 공유 폴더로, 아니면 내 개인 폴더로 올린다. onChanged 는 성공 뒤(자료 칸 갱신용). */
export function composerAttach(opts: { projectId: () => number; onChanged?: () => void; dead?: () => boolean }): ComposerAttach {
  const items: Att[] = [];
  const chips = el('div', { class: 'pn-att', hidden: true });

  function paint(): void {
    chips.hidden = !items.length;
    chips.replaceChildren(...items.map((a) => el('span', { class: 'pn-att-c', title: a.abs || a.name },
      pnIcon('doc', 'pn-i sm'),
      el('span', { class: 'n', text: a.pct == null ? a.name : `${a.name} · ${Math.floor(a.pct)}%` }),
      el('button', { class: 'pn-att-x', type: 'button', title: a.pct == null ? '첨부 빼기' : '올리기 취소', 'aria-label': a.pct == null ? '첨부 빼기' : '올리기 취소', text: '✕',
        onclick: () => { a.ctl?.abort(); const i = items.indexOf(a); if (i >= 0) items.splice(i, 1); paint(); } }))));
  }

  const urlFor = (nm: string): string => {
  ...
  async function attachFiles(files: File[]): Promise<void> {
    ...
    for (const f of files) {
      ...
      try {
        ...
        const i = items.indexOf(a); if (i >= 0) { items.splice(i, 1); paint(); }
  ...
  const btn = el('button', { class: 'v2-launch-att', type: 'button', title: '파일 첨부', 'aria-label': '파일 첨부', onclick: () => fileIn.click() },
  ...
  return {
    ...
    wireDrop(zone, hi) {
      ...
      upDropZone(zone, hi || zone, (list) => void attachFiles(list.map((u) => u.file as File)));
    },
