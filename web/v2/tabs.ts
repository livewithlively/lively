...
//  ⚠ 서버로 올리지 않는다(#2460) — 열린 창은 **그 기기의 사실**이다. 노트북에서 연 탭이 사무실
//   데스크톱에서 되살아나면 그게 더 이상하다. 사람이 바뀌면 지워진다(lib/local-owner.ts).
const STORE_KEY = deviceStore('lively_v2_tabs');   // #1875 — 열린 창은 **그 워크스페이스의 것**이다(net.ts wsKey 머리말)
...
export function routeKey(route: string): string {
  ...
  //  자료(#2423)는 **한 창 안에서 돌아다니는 앱**이다 — 출처를 고르면 쿼리가, 자료를 열면 뒤 세그먼트가 붙지만
...
export function createTabs(centerHost: HTMLElement, asideHost: HTMLElement, hooks: TabsHooks): TabsApi {
  ...
  function beginDrag(t: ShellTab, node: HTMLElement, e: PointerEvent): void {
    ...
    const els = tabs.map((x) => (scroll as HTMLElement).querySelector('[data-tab="' + x.id + '"]') as HTMLElement);
  ...
  function paint(): void {
    ...
    const on = scroll.querySelector('.v2-tab.on') as HTMLElement | null;
    if (on && scroll.scrollWidth > scroll.clientWidth) on.scrollIntoView({ inline: 'nearest', block: 'nearest' });
  }
