...
let inited = false;
let last: { host: HTMLElement; data: V2Data; activeKey: () => string; openSessions: () => Array<{ id: string; active: boolean }> } | null = null;
...
const isMachineLabel = (s: string) => /^이어보기\s*[·:]/.test(s) || /^위탁\s*#/.test(s);
...
function sessText(s: Sess, projName: string): { main: string; sub: string } {
...
// ── 그리기 ──
export function drawSide(host: HTMLElement, data: V2Data, activeKey: () => string, openSessions?: () => Array<{ id: string; active: boolean }>): void {
  init();
...
function render(): void {
  ...
  host.replaceChildren(
    ...
    el('div', { class: 'v2-side-sec' }, countEl,
      ...
      el('a', { class: 'v2-add', href: '#/projects2', title: '새 프로젝트 — 프로젝트 앱(보드)에서 만듭니다', 'aria-label': '새 프로젝트' },
...
function pinnedBlock(): HTMLElement[] {
  if (!last) return [];
