...
import { THEME_ORDER, applyToOpenTabs, harnessThemeSync, pushThemeToOpenTabs, setApplyToOpenTabs, setHarnessThemeSync, setThemePref, themePref, type ThemePref } from '../theme.js'; // #1683 다크모드 — 사이드바 3단 토글
...
let renaming = false;
function beginRename(row: HTMLElement, nameEl: HTMLElement, s: Sess): void {
  if (renaming) return;
  renaming = true;
  const shown = nameEl.textContent || '';
  const input = el('input', { class: 'v2-ss-edit', type: 'text', value: String(s.label || shown), 'aria-label': '세션 이름' }) as HTMLInputElement;
  ...
  const finish = async (save: boolean): Promise<void> => {
    ...
    if (!save || !next || next === s.label) { nameEl.replaceChildren(document.createTextNode(shown)); return; }
    nameEl.replaceChildren(document.createTextNode(next));
    try { await hooks.onRenameSession?.(s.id, next); }
...
async function pushThemeIfOn(): Promise<void> {
...
function themeSeg(): HTMLElement {
  ...
  return el('div', { class: 'v2-theme-wrap' },
    ...
    el('label', { class: 'v2-theme-ai',
      title: '켜면 테마를 바꿀 때 **지금 열려 있는 세션 탭**의 하네스까지 그 자리에서 바꿉니다.\n세션 입력창에 하네스의 테마 명령을 넣는 방식이라, 쓰던 글 뒤에 한 줄이 붙을 수 있어요.\n하네스마다 지원 여부가 달라 바꾼 개수와 못 바꾼 이유를 알려드립니다.' },
      el('input', { type: 'checkbox', class: 'v2-theme-ai-cb', ...(applyToOpenTabs() ? { checked: '' } : {}),
        onchange: (e: any) => { setApplyToOpenTabs(!!e.target.checked); redraw(); } }),
      el('span', { text: '현재 열린 탭 모두 적용' })),
    el('label', { class: 'v2-theme-ai',
