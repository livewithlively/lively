...
function render(): void {
  ...
  host.replaceChildren(
    ...
    ...stateChips(liveAll),
    el('div', { class: 'v2-side-flt', role: 'group', 'aria-label': '필터' },
      toggle('완료 숨김', !showDone, doneCount ? String(doneCount) : '', '완료된 프로젝트를 목록에서 뺍니다(살아 있는 세션이 있으면 그래도 보여요). 누르면 완료도 보입니다.',
        () => { showDone = !showDone; saveFlag(DONE_KEY, showDone); redraw(); }),
      toggle('내 프로젝트만', mineOnly, '', '내가 만들었거나 팀원인 프로젝트만 봅니다.', () => { mineOnly = !mineOnly; saveFlag(MINE_KEY, mineOnly); redraw(); })),
    treeEl!,
...
function stateChips(liveAll: Sess[]): HTMLElement[] {
  const counts = new Map<string, number>();
  for (const s of liveAll) counts.set(s.stateKey, (counts.get(s.stateKey) || 0) + 1);
  if (stateFilter && !counts.has(stateFilter)) counts.set(stateFilter, 0);   // 켜 둔 필터의 상태가 0이 돼도 칩은 남겨 끌 수 있게
  const keys = [...counts.keys()].sort((a, b) => rankOf(a) - rankOf(b));
  if (!keys.length) return [];
  return [el('div', { class: 'v2-side-sum', role: 'group', 'aria-label': '세션 상태' },
    ...keys.map((k) => { const st = SESS_STATES[k]; const on = stateFilter === k; return el('button', {
...
function toggle(label: string, on: boolean, cnt: string, tip: string, onclick: () => void): HTMLElement {
  return el('button', { class: 'v2-tg' + (on ? ' on' : ''), type: 'button', 'aria-pressed': String(on), title: tip, onclick },
    el('span', { class: 'v2-tg-mark', 'aria-hidden': 'true' }), el('span', { text: label }), cnt ? el('span', { class: 'v2-cnt', text: cnt }) : null);
}
...
function projRow(r: Row, sess: Sess[], activeKey: string): HTMLElement {
  ...
  const row = el('a', { class: 'v2-pj-row' + (isOn ? ' on' : '') + (r.done ? ' is-done' : '') + (sess.length ? '' : ' quiet'), href, 'data-nav': p ? pk : 'app:terminal', title: (p ? p.name + '\n' : '') + tipBits.filter(Boolean).join(' · ') },
    caret, el('span', { class: 'n', text: p ? p.name : '프로젝트 없는 세션' }),
...
function sumEl(sess: Sess[]): HTMLElement | null {
  ...
  for (const s of sess) { if (s.stateKey === 'waiting') c.wait++; else if (s.stateKey === 'done') c.done++; else if (s.stateKey === 'busy') c.busy++; else c.rest++; }
...
function sessRow(s: Sess, activeKey: string): HTMLElement {
  ...
  return el('a', { class: 'v2-ss-row' + (activeKey === 's:' + s.id ? ' on' : '') + (s.owned ? '' : ' other'), href: '#/s/' + encodeURIComponent(s.id), 'data-nav': 's:' + s.id, title: tip, role: 'treeitem' },
    ...
    el('span', { class: 'w ' + cls, text: st ? st.label : s.stateLabel }));
}
