...
function glyph(kind: 'folder' | 'folder-open' | 'chat' | 'home', cls: string): SVGElement {
  ...
  host.replaceChildren(
    ...
    el('nav', { class: 'v2-fixed', 'aria-label': '바로 가기' },
      el('a', { class: 'v2-nav' + (last.activeKey() === 'home' ? ' on' : ''), href: '#/', 'data-nav': 'home',
        title: '홈 — 무엇이든 시켜서 세션을 여는 자리' }, glyph('home', 'v2-nav-ic'), el('span', { class: 'n', text: '홈' })),
...
function sessRow(s: Sess, activeKey: string, text: { main: string; sub: string }): HTMLElement {
  ...
  return el('a', { class: 'v2-ss-row' + (activeKey === 's:' + s.id ? ' on' : '') + (s.owned ? '' : ' other'), href: '#/s/' + encodeURIComponent(s.id), 'data-nav': 's:' + s.id, title: tip + '\n세션 대화를 엽니다', role: 'treeitem' },
    ...
    el('span', { class: 'v2-ss-main' }, el('span', { class: 't', text: main })),
