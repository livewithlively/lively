 ...
 */
export function startLiveSync(onChange: () => void): void {
  if (started) return;
  started = true;
  ...
  if (!document.hidden) void connect(onChange);
}

/** 지금 스트림이 붙어 있나. 진단용(화면이 이 값으로 무엇을 바꾸지는 않는다). */
export function liveSyncAlive(): boolean { return !!ctl; }

function stopStream(): void {
  if (timer) { clearTimeout(timer); timer = 0; }
  const c = ctl;
  ctl = null;
  if (c) { try { c.abort(); } catch { /* 이미 끝난 연결 */ } }
}

function scheduleRetry(onChange: () => void): void {
  if (dead || timer || document.hidden) return;   // 숨은 동안은 안 붙는다 — 돌아올 때 visibilitychange 가 붙인다
  timer = window.setTimeout(() => { timer = 0; void connect(onChange); }, retryDelay(tries++));
}

async function connect(onChange: () => void): Promise<void> {
  if (dead || ctl) return;
  const token = (() => { try { return localStorage.getItem(TOKEN_KEY) || ''; } catch { return ''; } })();
  if (!token) { scheduleRetry(onChange); return; }   // 아직 로그인 전 — 곧 다시 본다
  ...
  try {
    ...
    if (res.status === 401) { dead = true; return; }
    ...
    if (ctl === mine) {
      ...
      if (openedAt && stableConnection(Date.now() - openedAt)) tries = 0;
