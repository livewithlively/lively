 ...
 *
 * 전환은 `location.reload()` 를 거치므로 **부팅 판정을 처음부터 다시 탄다.** 그런데 옮겨 간
 *  워크스페이스에는 그 사람의 흔적이 0이라(거기서 MCP 를 부른 적도, 세션을 연 적도 없다) 코어의
 *  «흔적 없으면 처음 오는 사람» 판정이 참이 되어 **전환할 때마다 처음 설정이 떴다**
