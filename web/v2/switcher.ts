...
async function openMenu(anchor: HTMLElement): Promise<void> {
  ...
  panel.append(
    email ? el('div', { class: 'v2-ws-acct', text: email }) : null,
    sectionLabel(w.kind === 'personal' ? '개인 워크스페이스' : '팀 워크스페이스'),
    el('div', { class: 'v2-ws-cur' },
      el('span', { class: 'v2-ws-badge ' + w.kind, text: w.kind === 'personal' ? '개인' : '팀' }),
      el('span', { class: 'v2-ws-name', text: w.name }),
      el('span', { class: 'v2-ws-check', 'aria-hidden': 'true', text: '✓' })));
...
async function refreshMine(wrap: HTMLElement): Promise<void> {
  try {
    ...
    wrap.replaceChildren(sectionLabel('내 워크스페이스'),
...
function mineRow(w: any, active: boolean, wrap: HTMLElement): HTMLElement {
  const open = el('button', { class: 'v2-ws-team-open', type: 'button', title: active ? '지금 이 워크스페이스예요' : '이 워크스페이스로 전환',
    ...
    active ? el('span', { class: 'v2-ws-check', 'aria-hidden': 'true', text: '✓' }) : null) as HTMLButtonElement;
...
async function refreshTeam(wrap: HTMLElement): Promise<void> {
  try {
    const d: any = await api('/api/ui/me/linked-workspaces');
    const links: any[] = (d && d.links) || [];
    wrap.replaceChildren(sectionLabel('연결한 팀 워크스페이스'),
      ...(links.length ? links.map((l) => teamRow(l, wrap)) : [el('p', { class: 'v2-ws-empty', text: '아직 연결한 팀 워크스페이스가 없어요. 아래에서 연결하면 지식·프로젝트를 그 팀으로 올릴 수 있어요.' })]));
  } catch (e: any) {
