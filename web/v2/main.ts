...
function drawSide(): void {
  ...
  const panel = el('div', { class: 'stu-panel' },
    el('div', { class: 'stu-panel-h' }, el('b', { text: '프로젝트' }),
      el('button', { class: 'btn-text', type: 'button', text: railPanelPin ? '핀 해제' : '핀 고정', title: '고정하면 새로고침해도 펼쳐져 있어요',
...
type FabPos = { rx: number; ry: number };
function loadFabPos(): FabPos | null {
  try { const v = JSON.parse(localStorage.getItem(FAB_POS_KEY) || 'null'); return v && typeof v.rx === 'number' && typeof v.ry === 'number' ? v : null; } catch (_) { return null; }
}
function placeFab(fab: HTMLElement): void {
...
function makeFabDraggable(fab: HTMLElement): void {
  fab.addEventListener('pointerdown', (e: PointerEvent) => {
    if (e.button !== 0) return;
    const tgt = e.target as HTMLElement;
    const onGrip = !!tgt.closest('.stu-fab-grip');
    if (!onGrip && tgt.closest('a, button')) return;   // 손잡이가 아니면 링크·버튼은 제 일을 한다
    ...
    const move = (ev: PointerEvent): void => {
      ...
      fab.style.left = Math.round(x) + 'px'; fab.style.top = Math.round(y) + 'px';
    };
    const up = (ev: PointerEvent): void => {
      ...
      try { localStorage.setItem(FAB_POS_KEY, JSON.stringify({ rx: (r.left - rr.left) / rr.width, ry: (r.top - rr.top) / rr.height })); } catch (_) { /* noop */ }
      const swallow = (ev2: Event): void => { ev2.stopPropagation(); ev2.preventDefault(); };
      fab.addEventListener('click', swallow, { capture: true, once: true });   // 끌고 놓은 손짓이 '열기'로 새지 않게
    };
    window.addEventListener('pointermove', move); window.addEventListener('pointerup', up);
  });
}
window.addEventListener('resize', () => { if (panelFab) placeFab(panelFab); });
