...
export async function bootV2(): Promise<void> {
  ...
  setInterval(() => { if (document.hidden) return; void loadData().then(() => { drawSide(); const c = parse(); if (c.segs[0] === 's' && c.segs[1]) { const sid = decodeURIComponent(c.segs[1]); refreshSession(data, sid);
      ...
      if (!(document.activeElement && document.activeElement.classList.contains('v2-pj-sel'))) drawAsideSession(data.sessions.find((x) => x.id === sid || x.logId === sid) || null); } }); }, 20000);
}
...
function projectPicker(s: Sess): HTMLElement {
  if (!s.owned || !s.live) return el('div', {});
  const sel = el('select', { class: 'v2-pj-sel', 'aria-label': '이 세션의 프로젝트' }) as HTMLSelectElement;
  ...
  for (const p of mine) sel.append(el('option', { value: String(p.id), text: p.name }));
  if (s.projectId && !mine.some((p) => Number(p.id) === Number(s.projectId))) sel.append(el('option', { value: String(s.projectId), text: projName(data, s.projectId) }));
  sel.value = s.projectId ? String(s.projectId) : '';
  const note = el('span', { class: 'v2-fine v2-pj-note' });
  sel.onchange = async () => {
    const pid = Number(sel.value) || 0;
    sel.disabled = true; note.textContent = '바꾸는 중…';
    try {
      const r: any = await api('/api/ui/terminal/sessions/' + encodeURIComponent(s.id) + '/project', { method: 'POST', body: JSON.stringify({ projectId: pid || null }) });
      ...
      note.textContent = ''; sel.disabled = false; sel.value = s.projectId ? String(s.projectId) : '';
