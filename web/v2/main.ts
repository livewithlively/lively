...
// 팝아웃 창(#1744) — `?solo=1` 로 여는 같은 앱(좌측·탭 줄만 없다). 실험장으로 갈아타도 이 창은 그대로 선다.
...
async function renderRoute(tab: ShellTab): Promise<void> {
  ...
  try {
    if (page === '' || page === 'dashboard') {
      ...
      if (!s) { const seeded = takeCreated(id); if (seeded) { data.sessions = mergeSessions([seeded as any], []).concat(data.sessions); s = findSess(id); } }
      ...
      if (!s) {
        ...
        for (let i = 0; i < 4 && !s; i++) { await new Promise((r) => setTimeout(r, 800)); if (seq !== tab.seq) return; await loadData(); s = findSess(id); }
        drawSide();
      }
      if (seq !== tab.seq) return;
