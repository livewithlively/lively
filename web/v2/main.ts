...
import { dotCls, mergeSessions, projName, refreshSession, renderHome, renderSession, unmountSession, type Sess, type V2Data } from './views.js';
...
let routeSeq = 0;
let projView: ProjectViewHandle | null = null;   // 프로젝트 화면(#1757) — 떠날 때 대화 폴링을 끈다
...
async function route(): Promise<void> {
  ...
  if (projView) { projView.destroy(); projView = null; }   // 프로젝트 화면도 같은 이유(리브 턴 폴링) — 같은 프로젝트로 다시 와도 새로 그린다(대화는 서버에서 되그린다)
