...
let cached: RunHarness[] | null = null;
let inflight: Promise<RunHarness[]> | null = null;
export function runCatalog(): Promise<RunHarness[]> {
  if (cached) return Promise.resolve(cached);
  if (!inflight) {
    inflight = api('/api/ui/terminal/config')
      .then((cfg: any) => {
        const hs: RunHarness[] = Array.isArray(cfg && cfg.harnesses) ? cfg.harnesses : [];
        cached = hs;
        return hs;
      })
      .catch(() => [] as RunHarness[])   // 못 받으면 빈 목록 — 부르는 쪽이 칸을 안 그리고 지난번 설정 그대로 연다
...
export interface RunPrefs { harness?: string; flags?: Record<string, string>; autoApprove?: boolean; [k: string]: unknown }
...
export interface RunPick { harness: string; flags: Record<string, string> }
export interface RunPicker {
  /** 세 칸을 담은 요소 — 부르는 쪽이 원하는 자리에 붙인다. */
  el: HTMLElement;
  /** 지금 고른 값. flags 는 **빈 값을 뺀다**(안 넘기는 게 '지난번 그대로'라서). */
  value(): RunPick;
  /** 입력 잠금(보내는 중). */
...
export function createRunPicker(opts?: { onChange?: (p: RunPick) => void; remember?: boolean }): RunPicker {
  ...
  function paint(): void {
    ...
    if (!list.some((h) => h.key === harnessKey)) harnessKey = list[0].key;
  ...
  return {
    el: root,
    value: pick,
    disable(on: boolean) { provSel.disabled = on || !harnesses.length; modelSel.disabled = on; effortSel.disabled = on; },
  };
}
