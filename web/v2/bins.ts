...
export function renderTrash(host: HTMLElement, data: V2Data, hooks: BinHooks = {}): void {
  ...
  const purge = (s: Sess): Promise<void> => guard(async () => {
    ...
    if (!await confirmSessionPurge({ title: `「${name}」을(를) 완전히 지울까요?`, sessions: [{ harness: String((s.raw && s.raw.harness) || '') }] })) return;
    try {
  ...
  const empty = (): Promise<void> => guard(async () => {
    if (!ss.length) return;
    if (!await confirmSessionPurge({ title: '휴지통을 비울까요?', n: ss.length, sessions: ss.map((s) => ({ harness: String((s.raw && s.raw.harness) || '') })) })) return;
    try {
