...
const SANDBOX_CSP =
  "<meta http-equiv=\"Content-Security-Policy\" content=\"default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:; font-src data:; connect-src 'none'; media-src 'none'; frame-src 'none'; form-action 'none'; base-uri 'none'\">";

/** CSP meta 를 문서 head 최상단에 주입(doctype 뒤 · 없으면 맨 앞). srcdoc 은 HTTP 헤더가 없어 CSP 는 meta 로만 건다. */
function injectCsp(html: string): string {
  const m = /<!doctype[^>]*>/i.exec(html);
  if (m) { const i = m.index + m[0].length; return html.slice(0, i) + SANDBOX_CSP + html.slice(i); }
...
interface AppUiData { html: string; title?: string; page_key?: string; app_id?: string; pages?: Array<{ key: string; title: string }> }
...
export async function openAppUi(appId: string, opts?: { page?: string; title?: string }): Promise<boolean> {
  try {
    ...
    const out = await api('/api/ui/apps/' + encodeURIComponent(appId) + '/ui' + q) as AppUiData;
    if (!out || typeof out.html !== 'string') throw new Error('UI 를 받지 못했습니다');
...
function mount(appId: string, data: AppUiData): void {
  closeAppUi();
  ...
  msgHandler = (ev: MessageEvent): void => {
    if (ev.source !== frame.contentWindow) return;
    const msg = ev.data as { method?: string; id?: unknown } | null;
  ...
  uiEl = el('div', { class: 'v2-appui-ov', role: 'dialog', 'aria-modal': 'true', 'aria-label': (data.title || appId) + ' 앱',
    ...
    el('div', { class: 'v2-appui' },
      el('div', { class: 'v2-appui-h' },
        el('b', { class: 'v2-appui-t', text: data.title || appId }),
