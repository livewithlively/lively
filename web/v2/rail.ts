...
export interface RailHooks {
  ...
  faces?: () => string[];
  /** 레일을 여닫았다 — 사이드바도 같이 다시 그린다(접힘에만 사이드바 머리에 워크스페이스 이름이 선다). */
  onLayout?: () => void;
}

// ── 구역 표 ──────────────────────────────────────────────────────────────────
interface SecDef { key: RailSection; label: string; route: string; tab: string | null; icon: string }
...
export function sectionRoute(sec: RailSection): string { return (SECTIONS.find((s) => s.key === sec) || SECTIONS[0]).route; }
...
function stackTile(): HTMLElement {
...
function onDocDown(e: MouseEvent): void { if (popEl && !popEl.contains(e.target as Node) && !(e.target as HTMLElement).closest('.v2-rail-stack')) closePopover(); }
...
function openPopover(anchor: HTMLElement): void {
  ...
  const pop = el('div', { class: 'v2-wspop', role: 'menu', 'aria-label': '워크스페이스' },
    ...
    el('button', { class: 'v2-wspop-row', type: 'button', role: 'menuitem', onclick: () => { closePopover(); toggleRail(); } },
      ...
      el('span', { class: 'v2-wspop-tt' }, el('b', { text: open ? '레일 접기' : '레일 펼치기' }), el('span', { text: '워크스페이스와 구역 이름을 늘 보이게' })),
...
function wsListOpen(): HTMLElement | null {
  ...
  return el('div', { class: 'v2-rail-wsl' },
    ...
    ...others.map((w) => el('button', {
      ...
      wsTile({ name: String(w.name || w.slug), kind: String(w.kind || 'team') }, 'v2-wscard-big'),
...
export function drawRail(): void {
  ...
  const top = el('div', { class: 'v2-rail-top' },
    open
      ...
      : el('div', { class: 'v2-rail-ws' }, stackTile()),
    open ? wsListOpen() : null);
  ...
  const recentEls = recents.map((a) => item('app:' + a.key, a.title, a.icon, false,
