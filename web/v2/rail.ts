...
function init(): void {
  ...
  try {
    ...
    if (Array.isArray(p)) pins = p.filter((x) => typeof x === 'string' && APPS.some((a) => a.key === x));
...
function recentForRail(n: number): AppDef[] {
  ...
  const take = (a: AppDef | undefined): void => {
    if (!a || SEC_APP_KEYS.has(a.key) || pins.includes(a.key) || pick.some((p) => p.key === a.key)) return;
...
function savePins(): void { try { localStorage.setItem(PIN_STORE, JSON.stringify(pins)); } catch (_) { /* 이번 화면은 된다 */ } }
function placePin(key: string, idx: number): void {
  const cur = pins.indexOf(key);
  const next = pins.filter((k) => k !== key);
  if (cur >= 0 && cur < idx) idx -= 1;
  next.splice(Math.max(0, Math.min(idx, next.length)), 0, key);
  pins = next; savePins(); drawRail();
}
function unpinApp(key: string): void { pins = pins.filter((k) => k !== key); savePins(); drawRail(); }
...
let dropped = false;
function clearOver(): void { host?.querySelectorAll('.over-top, .over-bot').forEach((x) => x.classList.remove('over-top', 'over-bot')); }
function wireDock(it: HTMLElement, a: AppDef, pinned: boolean): void {
  let hold: number | null = null; let sx = 0; let sy = 0;
  const cancelHold = (): void => { if (hold) { window.clearTimeout(hold); hold = null; } };
  it.addEventListener('dragstart', (e) => {
    cancelHold();
    dragKey = a.key; dropped = false;
    it.classList.add('dragging'); host?.classList.add('drag');
    if (e.dataTransfer) { e.dataTransfer.setData('text/plain', a.key); e.dataTransfer.effectAllowed = 'move'; }
  });
  it.addEventListener('dragend', (e) => {
    it.classList.remove('dragging'); host?.classList.remove('drag'); clearOver();
    ...
    if (!dropped && key && pinned && host) {
      ...
      if (out) { unpinApp(key); toast('독에서 뺐어요 — 최근에 열면 다시 아래에 떠요'); }
    }
  });
  if (pinned) {
    it.addEventListener('dragover', (e) => {
      if (!dragKey || dragKey === a.key) return;
      e.preventDefault();
      const r = it.getBoundingClientRect(); const before = e.clientY < r.top + r.height / 2;
      clearOver(); it.classList.add(before ? 'over-top' : 'over-bot');
    });
    ...
    it.addEventListener('drop', (e) => {
      ...
      dropped = true; placePin(dragKey, idx);
    });
  }
  ...
  it.addEventListener('pointermove', (e) => { if (hold && Math.hypot(e.clientX - sx, e.clientY - sy) > 6) cancelHold(); });
}
/** 고정 구간 자체도 놓을 자리다 — 비어 있을 때(끌기 중엔 점선 칸이 생긴다)와 맨 아래에 놓을 때. */
function wireZone(zone: HTMLElement): void {
  zone.addEventListener('dragover', (e) => { if (!dragKey) return; e.preventDefault(); zone.classList.add('over'); });
  zone.addEventListener('dragleave', (e) => { if (!zone.contains(e.relatedTarget as Node)) zone.classList.remove('over'); });
  zone.addEventListener('drop', (e) => {
    if (!dragKey || (e.target as HTMLElement).closest('.v2-rail-it')) return;
    e.preventDefault(); zone.classList.remove('over');
    dropped = true; placePin(dragKey, pins.length);
  });
}
...
export function drawRail(): void {
  ...
  const appItem = (a: AppDef, pinned: boolean): HTMLElement => {
    const it = item('app:' + a.key, a.title, a.icon, false,
  ...
  const pinnedEls = pins.map((k) => APPS.find((a) => a.key === k)).filter((a): a is AppDef => !!a && (!a.tab || navOn(a.tab))).map((a) => appItem(a, true));
  const recentEls = recentForRail(RECENT_N).map((a) => appItem(a, false));
  const pinsZone = el('div', { class: 'v2-rail-pins', 'aria-label': '독에 고정한 앱' }, ...pinnedEls) as HTMLElement;
  wireZone(pinsZone);

  const mid = el('div', { class: 'v2-rail-mid' },
    ...secEls, ...linkEls,
    el('div', { class: 'v2-rail-hr', role: 'presentation' }),
    pinsZone,
    pinnedEls.length && recentEls.length ? el('div', { class: 'v2-rail-hr v2-rail-hr--recent', role: 'presentation' }) : null,
    ...recentEls);
