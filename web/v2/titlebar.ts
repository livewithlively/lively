...
export interface Titlebar {
  /** 탭 줄이 들어갈 자리(그리드 1행 전폭). */
  host: HTMLElement;
  /** 창 맨 윗줄을 우리가 그리고 있는가 — false 면 이 모듈은 아무것도 안 한 것이다. */
  on: boolean;
}

interface DesktopBridge {
  platform?: string | null;
  titlebar?: { height?: number } | null;
  claimTitlebar?: (opts?: { selector?: string }) => boolean;
}
const desk = (): DesktopBridge | null => ((window as any).livelyDesktop as DesktopBridge) || null;

/**
 * 창 맨 윗줄을 셸이 가져간다. 데스크톱 frameless 창이 아니면 **null** — 웹(브라우저)에서는 아무 일도 일어나지 않고
 * 탭 줄은 종전대로 가운데 열 맨 위에 남는다.
 */
export function mountTitlebar(root: HTMLElement): Titlebar | null {
  const d = desk();
  const tb = d && d.titlebar;
  if (!d || !tb || typeof d.claimTitlebar !== 'function') return null;
  ...
  if (!d.claimTitlebar({ selector: '.v2-topbar' })) return null;

  const h = Math.max(24, Math.round(Number(tb.height) || 36));
  const host = el('div', { class: 'v2-topbar' },
    el('div', { class: 'v2-tb-grip', 'aria-hidden': 'true' })) as HTMLElement;
  ...
  applyInsets();
  if (wco && typeof wco.addEventListener === 'function') wco.addEventListener('geometrychange', applyInsets);
