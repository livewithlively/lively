...
export interface SseParse<T> {
  /** 이번 조각에서 **완성된** 프레임들. */
  events: T[];
  /** 아직 안 끝난 마지막 프레임 — 다음 조각 앞에 그대로 이어 붙인다. */
  rest: string;
}
 ...
 */
export function parseSse<T = unknown>(buffer: string): SseParse<T> {
  ...
  for (const frame of parts) {
    ...
    if (!data) continue;
    try { events.push(JSON.parse(data) as T); } catch { /* 깨진 프레임 하나가 스트림을 끊지 않는다 */ }
  }
  return { events, rest };
}
 ...
 */
export function stableConnection(connectedMs: number): boolean {
