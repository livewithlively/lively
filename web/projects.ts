  ...
  view.replaceChildren(
    ...
    el('div', { class: 'card-head', style: 'margin: 6px 0 14px' },
      el('div', {},
        ...
        el('p', { class: 'sub', style: 'margin: 5px 0 0', text: '리스트로 묶어서 봅니다 — 내가 참여한 리스트는 펼쳐지고, 나머지는 접혀 있어요.' }))),
...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId) {
  ...
  const newListBtn = el('button', { class: 'pjv-newlist-btn', type: 'button', title: '새 리스트 만들기' },
    el('span', { class: 'pjv-newlist-plus', text: '＋' }), el('span', { text: '새 리스트' }));
  ...
  const render = () => {
    ...
    if (pjvBoardGroupBy.mode === 'status') { renderStatus(); return; }
    ...
    for (const g of groups) body.append(pjvListGroup(g, reload, canDelete, fields, anchorId, meId, taskCtx, nested));
  };
  ...
  nestedSegBtn.onclick = (e) => { e.stopPropagation(); if (pjvBoardGroupBy.mode !== 'nested') { pjvBoardGroupBy.mode = 'nested'; syncToggles(); render(); } };
  listSegBtn.onclick = (e) => { e.stopPropagation(); if (pjvBoardGroupBy.mode !== 'list') { pjvBoardGroupBy.mode = 'list'; syncToggles(); render(); } };
  statusSegBtn.onclick = (e) => { e.stopPropagation(); if (pjvBoardGroupBy.mode !== 'status') { pjvBoardGroupBy.mode = 'status'; syncToggles(); render(); } };
  syncToggles();

  card.append(el('div', { class: 'card-head' },
    el('div', { class: 'pjv-tasks-head-left' }, el('h2', { text: '프로젝트' }), groupSeg, subtaskBtn),
    el('div', { class: 'card-head-actions' }, mineBtn, closedBtn, newListBtn)));
  card.append(body);
  render();
  return card;
}
...
function pjvListGroup(g, reload, canDelete, fields, anchorId, meId, taskCtx, nested?) {
  ...
  if (nested) {
    ...
    let any = false;
    if (inprog.length) { bodyEl.append(pjvProjGroup('진행 중', 'in_progress', inprog, reload, null, canDelete, false, fields, anchorId, meId, taskCtx, undefined, true)); any = true; }
    if (todo.length) { bodyEl.append(pjvProjGroup('할 일', 'todo', todo, reload, null, canDelete, false, fields, anchorId, meId, taskCtx, undefined, true)); any = true; }
    if (pjvProjClosedView.done && done.length) { bodyEl.append(pjvProjGroup('완료', 'done', done, reload, null, canDelete, false, fields, anchorId, meId, taskCtx, undefined, true)); any = true; }
    if (!any) bodyEl.append(el('div', { class: 'pjv-proj-empty', text: emptyText }));
...
function openListForm(reload, list?) {
  ...
  if (picker) rows.push(el('div', { class: 'field', style: 'margin-top:12px' },
    el('label', { class: 'field-label', text: '참여 멤버 (이 리스트를 펼쳐 보는 사람)' }), picker.box));
...
function pjvProjGroup(label, statusKey, list, reload, select, canDelete, withCols, fields, anchorId, meId, taskCtx?: any, sepTasks?: any, noAdd?: boolean) {
  fields = fields || [];
  sepTasks = sepTasks || [];
  ...
  if (!select && statusKey !== 'done' && !noAdd) body.append(pjvProjAddRow(statusKey, reload, body, countEl, fields, select, canDelete, anchorId, meId, taskCtx));
...
function projectKnowledgeSection(id, p, reload) {
  ...
  const reqCol = el('div', { class: 'pjk-col pjk-col-req' },
    el('div', { class: 'pjk-col-head' }, el('span', { class: 'pjk-col-title', text: '필요 지식' }), reqCount, mkActs('required', true)),
    reqList);
  const prodCol = el('div', { class: 'pjk-col pjk-col-prod' },
    el('div', { class: 'pjk-col-head' }, el('span', { class: 'pjk-col-title', text: '산출 지식' }), prodCount, mkActs('produced', false)),
    prodList);
...
function openKnowledgeRecommendPicker(id, relation, linkedNames, onLinked) {
  ...
  overlayBox('✨ 자동 추천 · 관련 지식', el('div', { class: 'ps-kn-pick' },
    el('div', { class: 'admin-hint', style: 'margin-bottom:8px', text: '의미가 비슷하거나(%), 같은 분류(📁)에 속한 지식입니다(관련도순). 필요한 것을 연결하세요.' }),
    results));
  (async () => {
    let recs: any;
    ...
    results.replaceChildren(...cand.map((m) => {
      ...
      const tags = el('span', { style: 'flex:none; display:inline-flex; gap:6px; align-items:baseline;' },
        m.shares_category ? el('span', { class: 'kn-chip', title: '프로젝트와 같은 분류', text: '📁 같은 분류' }) : null,
        pct > 0 ? el('span', { class: 'admin-hint', title: '의미 유사도(코사인)', text: pct + '%' }) : null);
      return el('div', { class: 'ps-kn-pick-row' },
        el('div', { class: 'ps-kn-pick-main' },
          el('div', { class: 'row-title', style: 'display:flex; justify-content:space-between; gap:8px; align-items:baseline;' },
            el('span', { text: m.title || m.name }), tags),
          el('div', { class: 'admin-hint ps-kn-pick-snip', text: (m.snippet || '').slice(0, 90) })),
        addBtn);
    }));
  })();
      ...
      if (depth === 0 && node && node.querySelector) {
        ...
        if (rowEl && titleEl && subBox && t && t.id != null && !rowEl.dataset.cfDbl) {
          ...
          rowEl.addEventListener('click', function (e) {
            ...
            if (e.target.closest('.pjv-trow-caret') || e.target.closest('.pjv-status-dot')) return;
