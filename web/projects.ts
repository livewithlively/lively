...
function pjvListFolderSubmenu(menu, close, list, reload) {
  ...
  api('/api/ui/v6/project-folders').then((d) => {
    ...
    for (const f of folders) if (!pjvFolderIsArchive(f)) menu.append(mkItem(f.name, f.id, f.color || 'var(--muted-2)'));
...
function pjvFolderTreeMenu(menu, close, folder, reload) {
  ...
  if (folder.parent_id != null && isSpace) menu.append(mk('최상위로 빼기', () => pjvMoveFolderToParent(folder.id, null, reload)));
  menu.append(el('div', { class: 'pjv-bulk-sep-h' }));
  menu.append(mk(kindLabel + ' 삭제', () => pjvDeleteFolder(folder, reload), true));
}

function pjvDeleteFolder(folder, reload) {
  if (!confirm('폴더 ‘' + folder.name + '’을(를) 삭제할까요?\n\n폴더만 사라지고, 속한 리스트는 보존됩니다(그 뒤 원하는 스페이스·폴더로 옮겨 주세요).')) return;
  (async () => {
...
function pjvPopover(anchor, content, opts?) {
  ...
  while (pjvPopStack.length) {
    ...
    if (top.pop.contains(anchor)) break;
    top.close();
  }
  ...
  const onDoc = (e) => {
    ...
    if (mine.some((x) => x.pop.contains(e.target) || (x.anchor && x.anchor.contains(e.target)))) return;
    close();
  };
  // Esc 는 맨 위 팝오버 하나만 닫는다(중첩 드롭다운에서 한 단계씩 빠져나오게).
  const onKey = (e) => {
    if (e.key !== 'Escape') return;
    if (pjvPopStack[pjvPopStack.length - 1] !== entry) return;
    e.stopPropagation();
    close();
  };
...
function openFolderGrid(id, startPath, base) {
  ...
  let lastData: any = null;          // 마지막 응답 — 정렬 변경 시 재요청 없이 재렌더(#1118)
  let sort = fileSortLoad();         // 열 정렬 상태(#1118) — 기기별 저장, 홈 팀 공유 폴더 모달과 공유
  const setSort = (s) => { sort = s; fileSortSave(s); if (lastData) render(lastData); };
  ...
  function render(data) {
    ...
    const head = el('div', { class: 'proj-file-lrow proj-file-lhead' },
      el('span', { class: 'proj-file-lic' }),
      el('span', { class: 'proj-file-lnm' }, fileSortBtn('이름', 'name', sort, setSort)),
      el('span', { class: 'proj-file-lsz' }, fileSortBtn('크기', 'size', sort, setSort)),
      el('span', { class: 'proj-file-ldt' }, fileSortBtn('수정 일시', 'mtime', sort, setSort)),
    ...
    crumb.replaceChildren(
      ...
      ...(data.path ? [el('span', { text: ' / ' + data.path })] : []));   // replaceChildren 은 null 을 'null' 텍스트로 넣는다 — 루트에서 'null' 노출 버그 수정
    const rows: any[] = [];
