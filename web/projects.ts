...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId, folders?, pageChrome?) {
  ...
  const alignBtn = el('button', { class: 'pjv-view-btn pjv-align-btn', type: 'button', title: '열 정렬 — 값과 헤더를 좌측/중앙으로 통일' },
    el('span', { class: 'pjv-view-btn-label', text: '정렬: 중앙' }));
  ...
  const applyAlign = () => {
    ...
    const l = alignBtn.querySelector('.pjv-view-btn-label'); if (l) l.textContent = left ? '정렬: 좌측' : '정렬: 중앙';
  };
  alignBtn.onclick = (e) => { e.stopPropagation();
    try { localStorage.setItem('pjv:colAlign', pjvIsAlignLeft() ? 'center' : 'left'); } catch (_) { /* noop */ }
    applyAlign();
  };
  ...
  const renderArea = (byStatus, noNav?) => {
    ...
    collapseBtn.onclick = (e) => { e.stopPropagation(); pjvBoardView.byArea = false; if (!pjvScopeKept()) pjvExitAreaMode(); pjvPersistSideOpen(); syncToggles(); render(); };
    ...
    const buildTree = () => {
      ...
      treeWrap.append(el('a', { class: 'pjv-side-navitem pjv-side-navfolder pjv-side-trash', href: '#/trash', title: '삭제한 프로젝트·지식·카테고리 복원 (휴지통)' },
    ...
    nav.append(navInner);
    if (noNav) { body.replaceChildren(main); return; } // #662 — 스코프 유지 + 사이드바 없음: 본문만
  ...
  sideBtn.onclick = (e) => { e.stopPropagation(); pjvBoardView.byArea = !pjvBoardView.byArea; if (pjvBoardView.byArea) { pjvBoardView.byFolder = false; } else if (!pjvScopeKept()) { pjvExitAreaMode(); } pjvPersistSideOpen(); syncToggles(); render(); };
  ...
  const toolbar = el('div', { class: 'card-head pjv-board-toolbar' },
    el('div', { class: 'pjv-tasks-head-left' }, el('h2', { text: '프로젝트' }), sideBtn, scopeChip, viewBtn, savedViewBtn, groupBtn, subtaskBtn, alignBtn),
...
function pjvViewMenu(anchor, onChange) {
  ...
  mkSwitch('byFolder', '리스트로 나누기', '리스트별로 묶어서 한눈에 보여줘요', (on) => { if (on) { pjvBoardView.byArea = false; pjvSidebarSel.key = '__all__'; pjvSidebarSel.explicit = false; pjvExitAreaMode(); pjvPersistSideOpen(); } }); // 스코프도 해제(#662) — 남겨두면 스코프 유지 렌더가 우선해 리스트 구역이 안 보인다
