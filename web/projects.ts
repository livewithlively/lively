...
function pjvViewTabsRow(ctx?) {
  ...
  for (const t of PJV_VIEW_TABS) {
    ...
    b.onclick = (e) => {
      e.stopPropagation();
      if (t.key === 'table' || t.key === 'timeline') { toast(t.label + ' 뷰는 준비 중이에요 — 곧 열립니다'); return; }
      if (pjvCurrentViewTab() === t.key) return;
      if (t.key === 'board') { pjvBoardView.kanban = true; pjvBoardView.byFolder = false; pjvBoardView.overview = false; }
      else { pjvBoardView.kanban = false; pjvBoardView.overview = false; if (!pjvBoardView.byFolder) pjvBoardView.byStatus = true; }
      syncs.forEach((s) => s());
