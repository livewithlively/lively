...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId, folders?) {
  ...
  const renderArea = (byStatus) => {
    ...
    else if (sel[0] === 'F') { const fid = sel.slice(1); shownProjects = (listsByFolder.get(Number(fid)) || []).flatMap((l) => (groupByList.get(l.id)?.projects) || []); }
    ...
    for (const f of folderList) {
      ...
      if (open) {
        ...
        if (fLists.length) for (const l of fLists) navInner.append(listNavItem(l, true));
        else navInner.append(el('button', { class: 'pjv-side-folder-empty', type: 'button', onclick: (e) => { e.stopPropagation(); openListForm(reload, undefined, { folderId: f.id }); } }, el('span', { class: 'pjv-newlist-plus', text: '＋' }), el('span', { text: '리스트 추가' })));
      }
    }
    // 최상위(폴더 없는) 리스트
...
function openProjectV2Form(reload, prefill?: any) {
  ...
  const back = overlayBox('새 프로젝트',
    el('div', { class: 'field' }, el('label', { class: 'field-label', text: '이름' }), nameIn),
    el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '분류 (리스트)' }), listPick.box),
...
function memberPicker(preselected, opts?) {
  ...
  function paintResults() {
    ...
    results.replaceChildren(...cand.map((m) => {
      ...
      return el('div', { class: 'proj-mp-row' + (on ? ' on' : ''), onclick: () => { if (on) selected.delete(m.id); else selected.add(m.id); paintResults(); searchIn.focus(); } },
