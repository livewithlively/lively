...
import { SOURCE_KIND_LABEL, SPACE_LABEL, buildSpacesNav, fetchAllSpaceCats, knInjectChip, knProvChip, knRow, knSideItem, myCatIdSet, spaceSubBar, srcRow } from './knowledge.js';
...
function pjvKanbanBoard(projects, selList, ctx) {
  ...
  for (const d of defs) {
    ...
    col.append(el('div', { class: 'pjv-kb-head' },
      ...
      el('span', { class: 'pjv-kb-label', text: d.label }),
...
async function renderProjectV2Board(view, scopeKey?) {
  ...
  if (scopeKey) { pjvSidebarSel.key = scopeKey; pjvBoardView.byArea = true; }
...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId, folders?, pageChrome?) {
  ...
  const sideBtn = el('button', { class: 'pjv-view-btn pjv-bundle-btn', type: 'button', title: '사이드바 — 스페이스·폴더·리스트 탐색 열기/닫기', 'aria-label': '사이드바 열기/닫기' },
    pjvSideToggleIcon(), el('span', { class: 'pjv-view-btn-label', text: '사이드바' }));
  // '뷰' — 저장된 뷰(ClickUp 이관 포함) 피커(#541). 현재 스코프(리스트/폴더)의 project_view 를 lazy 조회.
  const savedViewBtn = el('button', { class: 'pjv-view-btn pjv-savedview-btn', type: 'button', title: '뷰 — 저장된 보기(ClickUp 이관 포함)' },
    pjvViewIcon(), el('span', { class: 'pjv-view-btn-label', text: '뷰' }), el('span', { class: 'pjv-view-btn-caret', 'aria-hidden': 'true', text: '▾' }));
  // '그룹' — 그룹바이 필드+방향(#541 ClickUp group by 파리티). 기본값=ClickUp 뷰 grouping, 리스트별 로컬 오버라이드.
  const groupBtn = el('button', { class: 'pjv-view-btn pjv-groupby-btn', type: 'button', title: '그룹 — 필드와 방향으로 묶어 보기' },
    pjvViewIcon(), el('span', { class: 'pjv-view-btn-label', text: '그룹: 상태' }), el('span', { class: 'pjv-view-btn-caret', 'aria-hidden': 'true', text: '▾' }));
  ...
  const renderArea = (byStatus) => {
    ...
    if (groupBtn) pjvSyncGroupBtn(groupBtn, groupBy, !!selList);
    ...
    if (pjvBoardView.overview && selFolder) {
      ...
      if (!any) main.append(el('div', { class: 'pjv-proj-empty', text: scoped.length ? (mineOnly ? '내가 할당된 프로젝트가 없습니다.' : '아직 프로젝트가 없습니다.') : '이 폴더에 리스트가 없습니다.' }));
    } else if (byStatus) {
  ...
  const render = () => {
    ...
    if (byArea) {
      ...
      if (toolbar.parentElement === card) card.removeChild(toolbar);
      wrapper.replaceChildren(card);
      renderArea(byStatus);
      return;
    }
    // 사이드바 꺼짐 — 페이지크롬(전폭) 위, 카드(툴바 card-head + 본문).
    if (toolbar.parentElement !== card) card.insertBefore(toolbar, body);
  ...
  const rerenderScoped = () => { syncToggles(); if (pjvBoardView.byArea) pjvSaveScopeView(pjvSidebarSel.key, pjvSnapshotView()); render(); };
  ...
  sideBtn.onclick = (e) => { e.stopPropagation(); pjvBoardView.byArea = !pjvBoardView.byArea; if (pjvBoardView.byArea) { pjvBoardView.byFolder = false; } else { pjvExitAreaMode(); } pjvPersistSideOpen(); syncToggles(); render(); };
  ...
  const toolbar = el('div', { class: 'card-head pjv-board-toolbar' },
    el('div', { class: 'pjv-tasks-head-left' }, el('h2', { text: '프로젝트' }), sideBtn, viewBtn, savedViewBtn, groupBtn, subtaskBtn),
...
function openFolderForm(reload, folder?) {
  ...
  const back = overlayBox(editing ? '폴더 수정' : '새 폴더',
...
function pjvListStatusEditor(list, reload) {
  ...
  const paintRadios = () => radios.replaceChildren(
    radio('inherit', '기본 상태 사용', '할 일 · 진행 중 · 완료 (표준 3단계)'),
    radio('custom', '커스텀 상태 사용', '버킷 안에 중간 단계를 자유롭게'));
  paintRadios(); paint();

  const saveBtn = el('button', { class: 'btn btn-primary', text: '저장' });
  const cancelBtn = el('button', { class: 'btn btn-ghost', text: '취소', onclick: () => back.remove() });
  const layout = el('div', { class: 'pjv-statused' },
    el('div', { class: 'pjv-statused-left' }, el('div', { class: 'field-label', text: '상태 유형' }), radios),
...
function pjvSetStatusRegistry(lists) {
  ...
  for (const l of lists || []) if (pjvListIsCustomStatus(l)) pjvStatusReg.set(Number(l.id), pjvListStatusDefs(l));
}
...
function pjvSetGroupBy(selList, v) {
  try {
    if (v == null) localStorage.removeItem(pjvGroupByStoreKey(selList && selList.id));
...
function pjvSyncGroupBtn(btn, gb, enabled) {
  ...
  if (lbl) lbl.textContent = '그룹: ' + (f ? f.label : '상태') + (gb.dir === -1 ? ' ↓' : ' ↑');
...
function pjvGroupByMenu(anchor) {
  ...
  for (const f of PJV_GROUPBY_FIELDS) {
    const item = el('button', { class: 'pjv-menu-item', type: 'button' },
      ...
      ctx.groupBy.field === f.key ? el('span', { class: 'pjv-menu-check', text: '✓' }) : el('span', {}));
  ...
  const dirItem = el('button', { class: 'pjv-menu-item', type: 'button' },
    el('span', { text: ctx.groupBy.dir === -1 ? '내림차순 ↓' : '오름차순 ↑' }),
    el('span', { class: 'pjv-menu-check', text: '⇅' }));
...
function pjvViewMenu(anchor, onChange) {
  ...
  mkSwitch('byFolder', '리스트로 나누기', '리스트별로 묶어서 한눈에 보여줘요', (on) => { if (on) { pjvBoardView.byArea = false; pjvExitAreaMode(); pjvPersistSideOpen(); } });
  // 칸반 보드(#541) — 상태별 컬럼에 카드. 리스트 선택 시 그 리스트의 커스텀 상태 컬럼(ClickUp 보드 뷰 동형).
  mkSwitch('kanban', '칸반 보드', '상태별 컬럼에 카드로 보여줘요 (드래그로 상태 변경)', (on) => { if (on) pjvBoardView.byFolder = false; });
  syncAll();
}
...
function pjvGetColOrder(surface) { try { return JSON.parse(localStorage.getItem(pjvColOrderKey(surface)) || 'null'); } catch (_) { return null; } }
function pjvSetColOrder(surface, arr) { try { localStorage.setItem(pjvColOrderKey(surface), JSON.stringify(arr)); } catch (_) { /* noop */ } }
...
function pjvOrderFields(fields, surface) {
  ...
  if (!saved || !saved.length || !fields || fields.length < 2) return fields || [];
  const idx = new Map<string, number>(saved.map((id, i) => [String(id), i]));
  const rank = (id) => idx.has(String(id)) ? (idx.get(String(id)) as number) : Infinity;
...
function pjvWireColReorder(cells, fields, reload, surface) {
  ...
  const clearMarks = () => cells.forEach((c: any) => c.classList.remove('pjv-col-drop-before', 'pjv-col-drop-after'));
  cells.forEach((cell: any, i) => {
    ...
    cell.addEventListener('dragstart', (ev: any) => {
      const t = ev.target as Element;
      if (t && t.closest && t.closest('.pjv-thcol-menu, button, input')) { ev.preventDefault(); return; } // 메뉴/버튼에서 시작한 건 취소(클릭 유지)
      pjvColDrag.id = f.id; cell.classList.add('pjv-col-drag-src');
      try { ev.dataTransfer.effectAllowed = 'move'; ev.dataTransfer.setData('text/plain', 'col:' + f.id); } catch (_) { /* */ }
    });
    cell.addEventListener('dragend', () => { pjvColDrag.id = null; cell.classList.remove('pjv-col-drag-src'); clearMarks(); });
    const markSide = (ev) => {
      if (pjvColDrag.id == null || String(pjvColDrag.id) === String(f.id)) return false;
    ...
    cell.addEventListener('drop', (ev: any) => {
      ...
      ev.preventDefault(); ev.stopPropagation();
      const draggedId = pjvColDrag.id; pjvColDrag.id = null; clearMarks();
      const ids = fields.map((x) => String(x.id));
      const from = ids.indexOf(String(draggedId));
      if (from < 0 || String(draggedId) === String(f.id)) return;
      ids.splice(from, 1);
