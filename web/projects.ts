  ...
  const renderArea = (byStatus, noNav?) => {
    ...
    const buildTree = () => {
      ...
      treeWrap.append(el('div', { class: 'pjv-side-newrow' },
        el('button', { class: 'pjv-side-newlist', type: 'button', title: '새 스페이스 (최상위 구획)', onclick: (e) => { e.stopPropagation(); openFolderForm(reload, undefined, { kind: 'space' }); } }, el('span', { class: 'pjv-newlist-plus', text: '＋' }), el('span', { text: '새 스페이스' })),
...
function pjvFolderTreeMenu(menu, close, folder, reload) {
  ...
  if (!isSpace && folder.parent_id != null) menu.append(mk('최상위로 빼기', () => pjvMoveFolderToParent(folder.id, null, reload)));  // #766 중첩 해제
  menu.append(el('div', { class: 'pjv-bulk-sep-h' }));
...
function projectSessionLogSection(id) {
  ...
  api('/api/ui/v6/projects/' + id + '/session-logs')
    .then((d: any) => renderSessionListInto(body, (d && d.sessions) || [], '이 프로젝트에 기록된 세션이 없습니다. (관리 ▸ 세션 공유를 켜면 이 프로젝트에서 만든 세션 대화가 모입니다.)'))
    .catch((e: any) => body.replaceChildren(el('p', { class: 'admin-hint', text: (e && e.message) || '세션 기록을 불러오지 못했습니다.' })));
  return el('section', { class: 'card' },
    el('div', { class: 'card-head' }, el('h3', { text: '세션 기록' }),
      el('span', { class: 'admin-hint', text: '  이 프로젝트에서 만든 AI 세션 대화록(끝난 세션 포함)' })),
    body);
}
...
function pjvViewMenu(anchor, onChange) {
  ...
  const apply = (m) => {
    ...
    if (m === 'list' && !pjvBoardView.byArea) { pjvSidebarSel.key = '__all__'; pjvSidebarSel.explicit = false; pjvExitAreaMode(); pjvPersistSideOpen(); }
    items.forEach((s) => s());
    onChange();
  };
  const mkOpt = (key, label, hint) => {
    ...
    item.onclick = (e) => { e.stopPropagation(); if (curMode() !== key) apply(key); };
    items.push(sync);
    pop.append(item);
  };
