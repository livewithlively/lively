...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId) {
  ...
  const render = () => {
    ...
    for (const g of groups) body.append(pjvListGroup(g, reload, canDelete, fields, anchorId, meId, taskCtx));
  };
...
function pjvListGroup(g, reload, canDelete, fields, anchorId, meId, taskCtx) {
  ...
  if (!shown.length) bodyEl.append(el('div', { class: 'pjv-proj-empty', text: pjvBoardMineOnly.on ? '내가 할당된 프로젝트가 없습니다.' : '아직 프로젝트가 없습니다.' }));
  // 인라인 추가행 — 이 리스트(미분류면 list_id=null)에 바로 생성. '내 할당만' 읽기 모드에선 숨김.
  if (!pjvBoardMineOnly.on) bodyEl.append(pjvProjAddRow('in_progress', reload, bodyEl, null, fields, null, canDelete, anchorId, meId, taskCtx, isUn ? null : list.id));
...
function pjvProjGroup(label, statusKey, list, reload, select, canDelete, withCols, fields, anchorId, meId, taskCtx?: any, sepTasks?: any) {
  fields = fields || [];
  sepTasks = sepTasks || [];
