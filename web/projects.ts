...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId, folders?) {
    ...
    if (selList && selList.external_id) {
      ...
      if (cu === undefined) {
        ...
        api('/api/ui/v6/project-lists/' + selList.id + '/clickup-fields')
          .then((d) => { pjvCuFieldsCache.set(selList.id, d || { fields: [] }); if (((d || {}).fields || []).length || ((d || {}).view_columns || []).length) render(); })
  ...
  card.append(el('div', { class: 'card-head' },
    el('div', { class: 'pjv-tasks-head-left' }, el('h2', { text: '프로젝트' }), sideBtn, viewBtn, savedViewBtn, subtaskBtn),
