...
async function buildCategoryHome(slot: HTMLElement, cat: any, opts: any = {}) {
  ...
  let home: any = null;
  try { home = await api('/api/ui/knowledge/' + encodeURIComponent(homeDocName(cat))).then((r) => r && (r.knowledge || r)); }
  catch (_) { home = null; }
  const homeBody = () => {
  ...
  async function saveDecor(patch: any) {
    try {
      ...
      const r = await api('/api/ui/knowledge/' + encodeURIComponent(name) + '/props-ui',
  ...
  if (canCat) {
    ...
    titleEl.addEventListener('blur', async () => {
      ...
      try {
        ...
        if (opts.onCatChanged) opts.onCatChanged();
  ...
  let templateBar: HTMLElement | null = null;
  if (canDoc) {
    let timer: any = null;
    let saving = false;
    ...
    if (!hasContent) {
      const tplBtn = el('button', { class: 'cath-tpl-btn', type: 'button' },
        el('span', { text: '✨ 템플릿으로 대문 만들기' }),
        el('span', { class: 'cath-tpl-hint', text: '핵심 문서 카드 + 최근/결정 라이브 목록으로 시작' }));
  ...
  const tabBtn = (key: string, label: string, extra?: any) => {
    const b = el('button', { class: 'cath-tab-btn', type: 'button', 'data-tab': key },
      el('span', { text: label }), extra || null);
    b.onclick = () => { if (opts.onTab) opts.onTab(key); };
    return b;
  };
  const tabs = el('div', { class: 'cath-tabs' },
    tabBtn('home', '🏠 대문'),
    tabBtn('docs', '📄 문서', el('span', { class: 'cath-tab-count', text: opts.docsCount != null ? String(opts.docsCount) : '' })));
    ...
    el('div', { class: 'cath' },
      ...
      el('div', { class: 'cath-inner' },
        ...
        el('div', { class: 'cath-headrow' },
          el('div', { class: 'cath-headmain' }, titleEl, descEl),
          actionRow),
        tabs,
        bodyBox)));
  // 탭 초기 해석 — 'auto'(기본)면 대문 내용이 있을 때 대문, 없으면 문서(빈 대문으로 막지 않기).
  if (opts.onTab) opts.onTab(opts.tab === 'docs' ? 'docs' : (opts.tab === 'home' ? 'home' : (hasContent ? 'home' : 'docs')));
}
