...
function relayPolicyCard(data, detail) {
  ...
  save.addEventListener('click', async () => {
    ...
    try {
      ...
      await loadAdmin(true); toast('저장됨 — 구성원 다음 세션부터'); rerenderPanel(detail, 'custom-hooks', state.admin.data);
...
function hookForm(root, h, data, detail, isNew) {
  ...
  const harnessSel = el('select', {}, ...['all', 'claude', 'codex', 'openclaw', 'opencode', 'antigravity'].map((x) => el('option', { value: x, text: x })));
...
function harnessAssetEditor(detail, data) {
  ...
  for (const a of assets) {
    listCol.append(el('div', { class: 'mini-row' + (a.id === sel ? ' sel' : ''),
      onclick: () => { state.admin.assetSel = a.id; rerenderPanel(detail, 'harness-assets', data); } },
...
function assetForm(root, a, data, detail, isNew) {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      await loadAdmin(true); state.admin.assetSel = payload.id; toast('저장됨 — 구성원 다음 세션부터'); rerenderPanel(detail, 'harness-assets', state.admin.data);
    } catch (e) { toast(e.message, true); saveBtn.disabled = false; }
  });
  const actions = el('div', { class: 'admin-actions' }, saveBtn, status);
  ...
  if (isDraft) actions.append(el('button', { class: 'btn btn-primary', text: '승격 →', onclick: async () => {
    ...
    try {
      ...
      await loadAdmin(true); state.admin.assetSel = newId; toast('승격됨 — 구성원 다음 세션부터'); rerenderPanel(detail, 'harness-assets', state.admin.data);
    } catch (e) { toast(e.message, true); }
  } }));
  if (!isNew) actions.append(el('button', { class: 'btn-text', text: '제거', onclick: async () => {
    if (!confirm(`'${a.id}' 제거? 다음 세션부터 구성원 하네스에서 제거됩니다(미접속 머신은 직전 상태 유지).`)) return;
    try { await api('/api/ui/org/harness-asset/remove', { method: 'POST', body: JSON.stringify({ id: a.id }) }); await loadAdmin(true); state.admin.assetSel = null; toast('제거됨'); rerenderPanel(detail, 'harness-assets', state.admin.data); }
    catch (e) { toast(e.message, true); }
  } }));
  root.replaceChildren(
