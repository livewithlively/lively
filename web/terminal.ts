...
function openTermCreateForm(cfg, view, onCreated?) {
  ...
  const rootSeg = el('div', { class: 'term-seg' }, ...roots.map((r) => {
    ...
    b.onclick = () => { if (rootKey === r.key) return; rootKey = r.key; for (const k in rootBtns) rootBtns[k].classList.toggle('active', k === rootKey); pickerPath = ''; loadPicker(); };
  ...
  const back = overlay('새 세션',
    ...
    el('div', { class: 'ov-actions' },
      el('button', { class: 'btn btn-primary', 'data-tour': 'create', text: '생성하기', onclick: async (ev) => {
        ...
        const payload = { label: labelI.value, rootKey, subpath: pickerPath, harness: harnessSel.value, flags, autoApprove: autoCb.checked, invites: inviteBox.selected(), worktree: wtCb.checked };
        try {
...
function openTermEdit(s, cfg, view) {
  ...
  const lockNote = el('div', { class: 'term-lock-note' },
    el('span', { text: '🔒 작업 폴더 · 워크트리 · 하네스 · 모델 · 자동 승인은 세션을 처음 켤 때 정해집니다. 바꾸려면 실행 중인 LLM(또는 셸)을 닫고 새로 켜야 하므로 여기서는 수정할 수 없습니다 — 바꾸려면 새 세션을 만드세요.' }));
