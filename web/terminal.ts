...
function profileNoteEl(cfg) {
  ...
  if (p.provisioned) return el('div', { class: 'caption', text: '⚠ 내 프로필이 아직 로그인 안 돼 공유 계정으로 실행됩니다 — 터미널 상단 [내 계정 로그인] 버튼으로 한 번 로그인하세요.' });
...
function loginBannerEl(cfg, view) {
  ...
  return el('div', { class: 'card' },
    el('div', { class: 'caption', text: '⚠ 내 Claude 계정이 아직 로그인되지 않았습니다. 지금 만드는 AI 세션은 공유 계정으로 뜹니다. 한 번 로그인하면 이후 내가 만드는 세션은 내 계정으로 뜹니다.' }),
