...
async function renderTerminal(view) {
  ...
  function draw() {
    ...
    const counts: any = { waiting: 0, busy: 0, idle: 0, exited: 0, offline: 0 };
    ...
    const chips = el('div', { class: 'tsess-chips' }, allChip,
      ...([['waiting', '확인 필요'], ['busy', '작업 중'], ['idle', '대기 중'], ['exited', '종료됨'], ['offline', '연결 끊김']] as const)
...
function tsessCard(s, ctx) {
  ...
  if (s.owned) {
    ...
    acts.append(el('button', { class: 'tsess-icon danger', title: '이 세션을 끝낸다(대화록은 세션 기록에 남아 나중에 이어받을 수 있음)', text: '종료', onclick: async () => {
      if (!confirm(TSESS_END_CONFIRM(1, '세션 "' + s.label + '" 을(를) 종료할까요?'))) return;
      try { await api('/api/ui/terminal/sessions/' + encodeURIComponent(s.id) + (s.node ? '?node=' + encodeURIComponent(s.node.id) : ''), { method: 'DELETE' }); toast('세션을 종료했습니다 — 대화록은 📜 세션 기록에 남아 있어요'); reRender(); }
