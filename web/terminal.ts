...
function profileNoteEl(cfg) {
  ...
  if (p.provisioned) return el('div', { class: 'caption', text: '⚠ 내 프로필이 아직 로그인 안 돼 공유 계정으로 실행됩니다 — 상단 [내 계정 로그인] 버튼으로 한 번 로그인하세요.' });
...
function openTermCreateForm(cfg, view) {
  ...
  const wtWrap = el('label', { class: 'term-auto' }, wtCb,
    el('span', { text: ' git 워크트리에서 작업 — 선택한 폴더가 git 저장소면 새 브랜치의 워크트리를 만들어 격리 작업합니다(다른 세션·메인 체크아웃과 안 충돌). 저장소가 아니면 무시됩니다.' }));
...
function buildInvitePicker(cfg, current) {
  ...
  function paint() {
    ...
    if (!others.length) { results.replaceChildren(el('div', { class: 'proj-mp-empty', text: '초대할 다른 구성원이 없습니다 — 비공개 세션이 됩니다.' })); return; }
    ...
    if (!cand.length) { results.replaceChildren(el('div', { class: 'proj-mp-empty', text: '일치하는 사람이 없어요.' })); return; }
    results.replaceChildren(...cand.map((m) => {
      const on = selected.has(m.id);
      return el('div', { class: 'proj-mp-row' + (on ? ' on' : ''), role: 'button', 'aria-pressed': on ? 'true' : 'false',
        onclick: () => { if (on) selected.delete(m.id); else selected.add(m.id); paint(); searchIn.focus(); } },
        personFace(m.id, 'proj-mp-ava', m.name),
        el('span', { class: 'proj-mp-name', text: m.name + (m.kind === 'agent' ? ' (AI)' : '') }),
