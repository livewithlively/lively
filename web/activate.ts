...
export async function renderActivate(view: any): Promise<void> {
  ...
  view.replaceChildren(
    el('div', { class: 'card', style: 'max-width:560px;margin:24px auto' },
      ...
      // ⚠ 피싱 경고 — 양방향.
      el('div', { class: 'callout', style: 'border-left:3px solid #d9822b;padding:8px 12px;margin:8px 0;background:rgba(217,130,43,.08)' },
...
async function drawLookup(slot: any, raw: string): Promise<void> {
  ...
  const pwInput = el('input', { type: 'password', class: 'term-input', placeholder: '비밀번호 재확인', style: 'margin-top:6px;display:none' }) as HTMLInputElement;
  const cpBlock = canCp ? el('div', { style: 'margin:8px 0' },
