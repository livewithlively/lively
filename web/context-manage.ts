...
function findingRow(f: any, reload: () => void) {
  ...
  if (f.target_kind === 'knowledge') {
    ...
    card.append(el('a', { class: 'ctx-finding-target', href: '#/categories', text: `분류축 ${f.target_ref} →` }));
  }
...
function managerEditor(m: any | null, reload: () => void) {
  ...
  const syncSensitivity = () => {
    ...
    //  LLM 을 쓰는 종류에만 실행 AI 를 묻는다(서버 managers.needsLlm 과 같은 판정 — 갈리면 화면이 거짓말한다).
    runFields.hidden = !(k === 'contradiction' || k === 'code_drift');
