...
async function renderHomeSurface(box: HTMLElement, ctx: any) {
  ...
  const catRowEls = new Map<string, any>();   // cat.id → { icEl, metaEl } — 점진 보강 대상
  if (allCats.length) {
    ...
    for (const sk of ['business', 'product', 'system']) {
      ...
      for (const c of cats) {
        ...
        const metaEl = el('span', { class: 'wk-row-meta' },
          ...
          el('span', { class: 'wk-row-m wk-catrow-time' }));
        const row = el('div', { class: 'wk-catrow', role: 'link', tabindex: '0' },
          icEl,
          el('span', { class: 'wk-catrow-main' },
            el('span', { class: 'wk-catrow-name', text: c.name || c.key }),
            c.description ? el('span', { class: 'wk-catrow-desc', text: c.description }) : null),
          metaEl);
  ...
  if (!recent.length) {
    ...
    for (const e of recent) {
      ...
      if (day !== curDay) {
        if (hiddenInDay > 0) recentSec.body.append(el('div', { class: 'wk-day-more', text: '… ' + hiddenInDay + '개 더' }));
        curDay = day;
        dayCount = 0;
        hiddenInDay = 0;
  ...
  for (const c of allCats) {
    knFetchCategoryRows(c.id).then((rows) => {
      ...
      if (slot && slot.icEl.isConnected) {
        ...
        if (homeDoc && homeDoc.icon) { slot.icEl.textContent = homeDoc.icon; slot.icEl.classList.remove('letter'); }
        const latest = rows.filter((r) => !isCategoryHomeDoc(r.name))
          .reduce((m, r) => (String(r.updated_at || '') > m ? String(r.updated_at) : m), '');
        if (latest && slot.timeEl) slot.timeEl.textContent = relTime(latest);
      }
      for (const r of rows) {
