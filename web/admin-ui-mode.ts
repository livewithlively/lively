...
const LABEL: Record<string, string> = { v2: '새 화면 (베타) — 사이드바 · 리브 대화 · 앱(런치패드)', classic: '클래식 — 상단 탭(홈 · AI 세션 · 프로젝트 · WIKI · 맥락 관리 · 설정)' };
...
export function uiModeSection(detail: HTMLElement, data: any): void {
  ...
  detail.replaceChildren(
    sectionHead('화면', '라이블리를 어떤 화면으로 볼지 정합니다. 새 화면은 아직 베타입니다. 새 화면과 클래식은 같은 데이터·같은 주소를 쓰고, 클래식의 모든 페이지는 새 화면 안에서 [앱]으로 그대로 열립니다.'),
  ...
  function build(): void {
    ...
    body.append(
      el('div', { class: 'admin-field' }, el('label', { class: 'admin-field-label', text: '모든 구성원의 기본 화면' }), sel,
        el('p', { class: 'admin-hint' }, ...uiText('기본은 **클래식**입니다. 새 화면은 아직 **베타**라 완성 전까지는 원하는 조직만 여기서 켭니다 — 조직 전체로 켜기 전에 아래 \'이 브라우저에서만\'으로 먼저 써보는 것을 권합니다. 사람마다는 아래에서 따로 고를 수 있어요.'))),
  ...
  function mineBox(): HTMLElement {
    ...
    const draw = () => {
      ...
      box.replaceChildren(
        el('p', { class: 'admin-hint' }, ...uiText(o ? `이 브라우저는 조직 기본과 상관없이 **${o === 'v2' ? '새 화면 (베타)' : '클래식'}** 으로 봅니다.` : '이 브라우저는 조직 기본 화면을 따릅니다.')),
        el('div', { class: 'admin-actions' },
          el('button', { class: 'btn btn-sm', text: '새 화면(베타)으로 보기', onclick: () => { setUiModeOverride('v2'); location.hash = '#/'; location.reload(); } }),
