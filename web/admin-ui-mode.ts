...
const LABEL: Record<string, string> = { v2: '새 화면 — 사이드바 · 리브 대화 · 앱(런치패드)', classic: '클래식 — 상단 탭(홈 · AI 세션 · 프로젝트 · WIKI · 맥락 관리 · 설정)' };
...
export function uiModeSection(detail: HTMLElement, data: any): void {
  ...
  detail.replaceChildren(
    sectionHead('화면', '라이블리를 어떤 화면으로 볼지 정합니다. 새 화면과 클래식은 같은 데이터·같은 주소를 쓰고, 클래식의 모든 페이지는 새 화면 안에서 [앱]으로 그대로 열립니다.'),
  ...
  function build(): void {
    ...
    body.append(
      el('div', { class: 'admin-field' }, el('label', { class: 'admin-field-label', text: '모든 구성원의 기본 화면' }), sel,
        el('p', { class: 'admin-hint' }, ...uiText('새로 설치한 라이블리와 매니지드(app.lvly.io)는 **새 화면**이 기본입니다. 이미 클래식으로 쓰던 조직은 여기서 클래식으로 두면 종전과 같습니다. 사람마다는 아래에서 따로 고를 수 있어요.'))),
