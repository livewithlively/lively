...
async function renderKnowledgeSpace(view, _space, params) {
  ...
  const graphBtn = knGraphBtn(); graphBtn.classList.add('kn-side-actionitem');
  const sourcesBtn = knSourcesBtn(); sourcesBtn.classList.add('kn-side-actionitem');
  ...
  function buildSide() {
    ...
    side.replaceChildren(
      ...
      newPageBtn,   // (#req) ＋새 페이지 = 사이드바 상단 primary
      knMakeSideSearch(nav, knSideState), nav, mirrorBox,
      sideActions);   // (#req) 선택·지식 그래프·자료·휴지통 = 하단 유틸리티
  ...
  const layout = el('div', { class: 'kn-shell' },
    side,
    el('section', { class: 'kn-main' }, catBox, bulkBar, listBox, foot),   // (#req) 상단 툴바·필터바 제거 — 액션은 사이드바, 검색은 사이드바+⌘K
  );
...
function buildKnowledgeNav(nav, bySpace, selected, myIds: Set<string>, opts) {
  ...
  if (myIds && myIds.size) nav.append(el('div', { class: 'kn-star-legend' },
    el('span', { class: 'kn-star-legend-swatch', 'aria-hidden': 'true' }),
    el('span', { text: '파란 글씨 = 내 소유 카테고리' })));
}
