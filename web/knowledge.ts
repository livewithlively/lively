...
async function renderKnowledgeSpace(view, _space, params) {
  ...
  function applySideFilter() {
    ...
    nav.querySelectorAll('.kn-space-group').forEach((grp: any) => {
      let vis = 0;
      grp.querySelectorAll('.kn-nav-catwrap').forEach((w: any) => {
    ...
    const legend = nav.querySelector('.kn-star-legend') as HTMLElement | null;
    if (legend) legend.hidden = !!q;
    const anyVis = !q || Array.from(nav.querySelectorAll('.kn-nav-catwrap')).some((w: any) => !w.hidden);
    const note = nav.querySelector('.kn-side-noresult');
    if (q && !anyVis) { if (!note) nav.append(el('div', { class: 'kn-side-noresult kn-nav-note', text: '일치하는 분류 없음' })); }
    else if (note) note.remove();
  }
  function knSideSearchBox() {
    const input = el('input', { class: 'kn-side-search-input', type: 'text', placeholder: '분류 검색', 'aria-label': '분류 검색', value: knSideQ }) as HTMLInputElement;
    const clear = el('button', { class: 'kn-side-search-clear', type: 'button', title: '지우기', 'aria-label': '검색어 지우기', text: '×' });
    const box = el('div', { class: 'kn-side-search-box' + (knSideQ ? ' has-q' : ''), title: '전체 지식 검색은 ⌘K' },
      el('span', { class: 'kn-side-search-ic', 'aria-hidden': 'true', text: '🔍' }), input, clear);
...
function knHomeCatCard(c, home, count, isMine, onOpen, dragCtx?) {
  ...
  if (isMine) spaceEl.append(el('span', { class: 'kn-hcard-star', 'aria-hidden': 'true', text: '★ ' }));
...
async function renderKnowledgeDetail(view, name) {
  ...
  const nav = el('nav', { class: 'browse-tree', 'aria-label': '카테고리' });
  const mirrorBox = el('div', { class: 'kn-mirror-tree', hidden: true });
  side.append(el('div', { class: 'kn-side-head' }, el('div', { class: 'eyebrow', text: '카테고리' }), collapseBtn),
    knSideSearchBtn(), nav, mirrorBox);
  side.addEventListener('click', (ev) => {
    const item = ev.target.closest('[data-cat-val]');
    if (!item) return;
    ev.preventDefault();
