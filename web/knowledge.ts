...
async function renderKnowledgeSpace(view, _space, params) {
  ...
  const graphBtn = knGraphBtn(); graphBtn.classList.add('kn-side-toolitem');
  const sourcesBtn = knSourcesBtn(); sourcesBtn.classList.add('kn-side-toolitem');
  const trashBtn = el('a', { class: 'kn-side-toolitem', href: '#/trash', title: '삭제한 지식·카테고리 복원', text: '🗑 휴지통' });
  const sideTools = el('div', { class: 'kn-side-tools' },
  ...
  try { const fd = await api('/api/ui/v6/favorites'); for (const id of ((fd && fd.categories) || [])) favCatIds.add(String(id)); } catch (_) { /* 비로그인/실패 — 빈 즐겨찾기 */ }
  ...
  const toggleCatFav = async (id: string, next: boolean) => {
    ...
    catch (e: any) { if (next) favCatIds.delete(String(id)); else favCatIds.add(String(id)); buildSide(); toast('즐겨찾기 저장 실패 — ' + (e && e.message || e), true); }
  };
  ...
  function buildSide() {
    ...
    buildKnowledgeNav(nav, bySpace, selKey, myIds,
      { indexed: true, onOpen: (name) => openKnowledgePeek(name, { onRefresh: refetch }), favCatIds, onToggleFav: toggleCatFav });
    ...
    side.replaceChildren(
      ...
      knMakeSideSearch(nav, knSideState), nav, mirrorBox, sideTools);   // (#req) 도구 섹션 = Notion 페이지 트리 아래
  ...
  async function paintHome(quiet = false) {
    ...
    const secHead = (cls: string, title: string, sub: string | null, count: number, ctrl?: any) =>
      el('div', { class: 'kn-home-sechead2 ' + cls },
        el('div', { class: 'kn-home-sechead2-main' },
          ...
          el('span', { class: 'kn-home-sechead2-count', text: String(count) + '개 카테고리' }),
    ...
    if (mine.length) {
      ...
      if (rest.length) {
        ...
        const moreBtn = el('button', { class: 'kn-home-morecats', type: 'button' },
          moreLabel, el('span', { class: 'kn-home-morecats-arrow', 'aria-hidden': 'true', text: '→' }));
        const moreRow = el('div', { class: 'kn-home-morerow' }, moreBtn);
        moreBtn.onclick = () => {   // (#req) 토글 — 펼치기 ⇄ 접기
    ...
    const sechead = el('div', { class: 'kn-home-sechead' },
      el('span', { class: 'kn-home-sectitle', text: mine.length ? '내 소유 카테고리 최근 지식' : '최근 지식' }),
      // (#req) ＋새 페이지·선택을 최근 지식 섹션으로 이동(상단 툴바 비움).
      el('div', { class: 'kn-home-sechead-actions' },
        newPageBtn,
        selectBtn,
  ...
  const layout = el('div', { class: 'kn-shell' },
    side,
    el('section', { class: 'kn-main' }, catBox, bulkBar, listBox, foot),   // (#req) 상단 툴바 비움 — 액션은 사이드바 '도구'·최근 지식 섹션으로
  );
...
function buildKnowledgeNav(nav, bySpace, selected, myIds: Set<string>, opts) {
  ...
  // ⭐ 즐겨찾기(#670) — 즐겨찾기한 카테고리를 맨 위 고정 구역에(프로젝트 탭 사이드바와 동일 UI). 본래 위치에도 그대로 남고 별 표시.
  {
    ...
    if (favCats.length) {
      ...
      for (const c of favCats) nav.append(knNavCatNode(c, String(selected) === String(c.id), onOpen, myIds.has(String(c.id)), favOpts));
  ...
  for (const sk of ['business', 'product', 'system']) {
    ...
    for (const c of ordered) grp.append(knNavCatNode(c, String(selected) === String(c.id), onOpen, myIds.has(String(c.id)), favOpts));
    nav.append(grp);
  }
...
function knHomeCatCard(c, home, count, isMine, onOpen, dragCtx?) {
  ...
  if (isMine) cover.append(el('span', { class: 'kn-hcard-teambadge', text: '내 소유' }));
  ...
  const card = el('div', { class: 'kn-hcard' + (isMine ? ' kn-hcard-mine' : ''), role: 'link', tabindex: '0',
    title: (c.name || c.key) + ' 대문 열기' + (isMine ? ' · 내 소유 카테고리' : '') },
    cover,
