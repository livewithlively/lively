...
async function renderCategorySurface(box: HTMLElement, cat: any, ctx: any) {
  ...
  function paintLibrary() {
    ...
    if (!list.length) sec.body.append(wkEmpty(f.type ? '이 유형의 문서가 없어요.' : '문서가 없어요.'));
    else {
  ...
  function seeAllBtn(type: string) {
    return el('button', { class: 'wk-sec-act', type: 'button', text: '모두 보기 →',
      onclick: () => {
        f.type = type; ctx.syncHash(); paintLibrary();
  ...
  function buildOverview() {
    ...
    if (!shouldTxt && !introSlot && !cat.description) return null;
    const sec = bfSection('wk-ch-overview', '개요', null,
      canCat ? el('button', { class: 'wk-sec-act', type: 'button', title: '정의·범위·규칙(should)과 설명을 편집합니다',
    ...
    if (shouldTxt) {
      ...
      const col = el('div', { class: 'wk-bf-col' },
        el('div', { class: 'wk-bf-collabel', text: '정의 · 범위 · 규칙' }), bodyEl);
      ...
      grid.append(el('div', { class: 'wk-bf-col' },
        el('div', { class: 'wk-bf-collabel', text: '정의 · 범위 · 규칙' }),
        el('button', { class: 'wk-bf-invite', type: 'button',
          onclick: () => openCategoryForm(cat.space, cat, () => { ctx.onCatChanged(); ctx.repaint(); }) },
          el('span', { text: '이 카테고리의 정의와 범위를 적어두면, 새로 오는 팀원의 첫 지도가 됩니다.' }),
          el('span', { class: 'wk-bf-invite-act', text: '✎ 작성하기' }))));
    }
    if (introSlot) {
      grid.append(el('div', { class: 'wk-bf-col' },
        el('div', { class: 'wk-bf-collabel', text: '팀의 소개' }), curation));
    }
    sec.body.append(grid);
    return sec.el;
  }
  ...
  function courseDocs(): any[] {
    ...
    push(concepts[0]); push(decisions[0]); push(howtos[0]);
    for (const r of allDocs) { if (auto.length >= 4) break; push(r); }
    return auto;
  }
  function buildCourse() {
    ...
    const sec = bfSection('wk-ch-course', '처음이라면', null,
      el('span', { class: 'wk-course-progress' + (readN === docs.length ? ' done' : ''), text: readN + '/' + docs.length + ' 읽음' }));
    const track = el('div', { class: 'wk-course' });
    docs.forEach((d, i) => {
      const read = wkIsRead(d.name);
      const step = el('div', { class: 'wk-step' + (read ? ' read' : ''), role: 'link', tabindex: '0', title: d.title || d.name },
        el('span', { class: 'wk-step-num', 'aria-hidden': 'true', text: read ? '✓' : String(i + 1) }),
        el('div', { class: 'wk-step-main' },
          el('div', { class: 'wk-step-title', text: d.title || d.name }),
          el('div', { class: 'wk-step-meta' },
            d.type ? el('span', { class: 'wk-row-m', text: KN_TYPE_LABEL[d.type] || d.type }) : null,
            read ? el('span', { class: 'wk-row-m wk-step-readlbl', text: '읽음' }) : null)));
      const go = () => openDoc(d, step);
      step.addEventListener('click', go);
      step.addEventListener('keydown', (ev: any) => { if (ev.key === 'Enter') go(); });
      track.append(step);
    });
    sec.body.append(track,
      el('div', { class: 'wk-course-hint', text: '이 순서는 대문 소개글의 페이지 카드를 따라요 — 소개글에 카드를 넣어 코스를 직접 짤 수 있어요.' }));
    return sec.el;
  }
  ...
  function buildConcepts() {
    ...
    sec.body.append(el('div', { class: 'wk-doccard-grid wk-concept-grid' },
      ...concepts.slice(0, CAP).map((r) => wkDocCard(r, { open: (x, rEl) => openDoc(x, rEl), deckCap: 160, cls: 'concept' }))));
    return sec.el;
  }
  ...
  function buildDecisions() {
    ...
    for (const d of decisions.slice(0, CAP)) {
      ...
      const item = el('div', { class: 'wk-tl-item', role: 'link', tabindex: '0', title: d.title || d.name },
        el('span', { class: 'wk-tl-dot', 'aria-hidden': 'true' }),
        el('div', { class: 'wk-tl-main' },
          el('div', { class: 'wk-tl-title', text: d.title || d.name }),
          deck ? el('div', { class: 'wk-tl-deck', text: deck }) : null),
        el('span', { class: 'wk-row-m wk-tl-date', text: isNaN(dt.getTime()) ? '' : ((dt.getMonth() + 1) + '/' + dt.getDate()) }));
      const go = () => openDoc(d, item);
      item.addEventListener('click', go);
      item.addEventListener('keydown', (ev: any) => { if (ev.key === 'Enter') go(); });
      tl.append(item);
    }
    sec.body.append(tl);
    return sec.el;
  ...
  const chapterDefs = inDrill ? [] : [
    briefing.find((s: any) => s.id === 'wk-ch-overview') ? { id: 'wk-ch-overview', label: '개요', count: null } : null,
    briefing.find((s: any) => s.id === 'wk-ch-course') ? { id: 'wk-ch-course', label: '처음이라면', count: null } : null,
  ...
  box.replaceChildren(
    el('div', { class: 'wk-cat wk-cat-v2 wk-cat-v3' },
      cover,
