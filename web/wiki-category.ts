...
function openFolderForm(cat: any, parentFolder: string, done: () => void) {
  ...
  makeBtn.onclick = go;
  nameIn.addEventListener('keydown', (e: any) => { if (e.key === 'Enter') go(); });
}
...
async function renderCategorySurface(box: HTMLElement, cat: any, ctx: any) {
  ...
  moreBtn.onclick = () => {
    ...
    pop.append(item(sel.mode ? '선택 모드 끄기' : '☑ 여러 개 선택', () => { sel.mode = !sel.mode; if (!sel.mode) sel.names.clear(); paintLibrary(); }));
  ...
  if (canDoc) {
    ...
    if (!hasContent) {
      const tplBtn = el('button', { class: 'wk-tpl-btn', type: 'button', title: '핀 문서 카드 + 최근/결정 라이브 목록으로 시작' },
        el('span', { text: '✨ 템플릿으로 시작' }));
      tplBtn.onclick = async () => {
        ...
        } catch (e) { toast('템플릿 생성 실패 — ' + e.message, true); tplBtn.disabled = false; }
      };
...
async function buildHomeTemplate(cat: any): Promise<string> {
  let rows: any[] = [];
  try { rows = await knFetchCategoryRows(cat.id); } catch (_) { /* 목록 실패 — 카드 없는 템플릿 */ }
