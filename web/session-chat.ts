...
export function mountSessionChat(host: HTMLElement, first: SessionChatTarget, opts: SessionChatOpts): SessionChatHandle {
  ...
  // codex 실시간 층(#2055)의 승인·사용량이 앉는 자리 — 같은 층(입력칸 바로 위)이되 **다른 칸**이다.
  //  처방전은 자기 칸을 replaceChildren 로 비운다 — 한 칸을 나눠 쓰면 그때 승인 카드가 통째로 사라진다.
  ...
  const view: ChatView = createChatView(chatHost, {
    ...
    askHost: el('div', { class: 'sc-ask-host' }, liveDock, rxHost),   // 승인(#2055)·처방전(#1719) — 스크롤에 떠내려가지 않는 입력칸 바로 위
