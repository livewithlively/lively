...
export function mountSessionChat(host: HTMLElement, first: SessionChatTarget, opts: { terminalSrc?: string | null; openHref?: string | null; firstPrompt?: string | null; trail?: TrailWidget | null; onPickProject?: (anchor: HTMLElement) => void }): SessionChatHandle {
  ...
  let obsModel = ''; let obsEffort = '';       // 대화 파일이 말한 **실제** 값 — 드롭다운의 '지금'은 이걸 따른다
  let switching = false;
  ...
  function paintAxis(a: Axis, box: HTMLSelectElement, span: HTMLElement, observed: string,
    ...
    if (!canSwitch(a)) { box.hidden = true; chip(span, shown); return; }
    span.hidden = true;
    const choices = flagChoices(hcat, AXIS[a].flag);
    // 관측값이 선택지 중 하나를 품고 있으면(claude 는 'claude-opus-4-5-…' 안에 'opus') 그 칸을 고른 것으로 본다.
    const hit = choices.find((c) => observed && observed.toLowerCase().includes(c.toLowerCase())) || '';
  ...
  function setObserved(a: Axis, v: string): void {
    if (a === 'model') { if (obsModel === v) return; obsModel = v; } else { if (obsEffort === v) return; obsEffort = v; }
    paintRun();
  }
  ...
  function applyLine(o: any): void {
    ...
    if (o.type === 'assistant') {
      if (o.message?.model) setObserved('model', String(o.message.model));
