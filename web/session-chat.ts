...
export function mountSessionChat(host: HTMLElement, first: SessionChatTarget, opts: SessionChatOpts): SessionChatHandle {
  ...
  function nameNode(cls: string): HTMLElement {
    const t = titleText || '(이름 없음)';
    return canRename()
      ? el('button', { class: cls + ' sc-title-btn', type: 'button', title: '세션 이름 — 눌러서 바꿉니다', onclick: () => startRename() },
  ...
  paintTitle();
  const idxBtn = el('button', { class: 'btn-text sc-act', type: 'button', text: '목차', title: '이 세션에 보낸 질문 목차 — 누르면 그 자리로', onclick: () => openIndex() }) as HTMLButtonElement;
  ...
  const head = el('div', { class: 'sc-head' },
    el('div', { class: 'sc-head-l' },
      ...
      el('span', { class: 'sc-meta' },
        ...
        nameHost, el('span', { class: 'sc-sep sc-sep-name', text: '·' }),
        projEl,
