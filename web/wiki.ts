...
async function renderWikiSpace(view, params) {
  const f = (state as any).wiki = (state as any).wiki || { category: '', folder: '', type: '', q: '', indexed: false, all: false, home: false };
  ...
  if (freshEntry) {
    f.category = ''; f.folder = ''; f.type = ''; f.q = ''; f.indexed = false; f.all = false; f.home = false;
  }
  else if (params) {
    ...
    if (f.indexed) { f.category = ''; f.folder = ''; f.all = false; f.home = false; }
    else if (f.all) { f.category = ''; f.folder = ''; f.home = false; }
    else if (f.category) f.home = false;
  }
  ...
  function syncHash() {
    ...
    else if (f.home) p.set('home', '1');   // 사용자가 고른 홈 — 새로고침해도 기본 카테고리로 안 튕기게
  ...
  function selectCategory(v: string) {
    if (v === KN_INDEXED) { f.indexed = true; f.category = ''; f.home = false; }
    else { f.indexed = false; f.category = v || ''; f.home = !v; }   // '전체' = 홈을 명시적으로 고름
  ...
  if (freshEntry) {
    ...
    if (top) { f.category = top; sideCtl.rebuild(); }
  }
