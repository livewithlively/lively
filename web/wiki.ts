...
async function renderWikiSpace(view, params) {
  ...
  if (params) {
    if (params.has('category')) { f.category = params.get('category') || ''; f.indexed = false; f.all = false; }
  ...
  function repaint() {
    if (f.q || f.all || f.indexed) return renderFilterList(main, ctx);
    if (f.category) {
      const cat = sideCtl.findCat(f.category);
      if (cat) return renderCategorySurface(main, cat, ctx);
