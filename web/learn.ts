...
function tabsGuideCard() {
  const chapters = GUIDE_CHAPTERS.map((c) => {
    ...
    for (const t of c.tabs) grid.append(tabCard(t));
  ...
  return el('div', { class: 'card' },
    ...
    el('p', { class: 'guide-lead', text: '위쪽 메뉴는 성격에 따라 네 묶음이에요 — ① 시작, ② 실무(터미널·프로젝트), ③ 저장소(코드구조·WIKI), ④ 설정·도움말(관리·사용 가이드). 묶음별로 한 번만 훑어두면 길을 잃지 않아요.' }),
    ...chapters);
}
