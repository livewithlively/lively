...
function knCatOrderSaved(): string[] {
  try { const v = JSON.parse(localStorage.getItem(KN_CAT_ORDER) || '[]'); return Array.isArray(v) ? v.map(String) : []; }
  catch (_) { return []; }
}
function knCatOrderSave(ids: string[]) { try { localStorage.setItem(KN_CAT_ORDER, JSON.stringify(ids)); } catch (_) { /* noop */ } }
function knCatOrderClear() { try { localStorage.removeItem(KN_CAT_ORDER); } catch (_) { /* noop */ } }
function knSortByCatOrder(cats: any[]): any[] {
...
function knApplyCatReorder(allCats: any[], src: string, target: string, before: boolean): boolean {
