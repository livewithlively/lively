...
function wkOnLeave(ed: any, fn: () => Promise<any>) { ed.__wkLeave = fn; return ed; }
async function wkConfirmLeave() {
  for (const ed of Array.from(wkLiveEditors)) {
    try {
      if (!ed || !ed.el || !ed.el.isConnected) continue;
      if (typeof ed.__wkLeave !== 'function') continue;
      if (!(ed.isDirty && ed.isDirty())) continue;
      await ed.__wkLeave();
...
function knCatOrderSaved(): string[] {
  try { const v = JSON.parse(localStorage.getItem(WK_CAT_ORDER_KEY) || '[]'); return Array.isArray(v) ? v.map(String) : []; }
  catch (_) { return []; }
}
function knCatOrderSave(ids: string[]) {
  try { localStorage.setItem(WK_CAT_ORDER_KEY, JSON.stringify(ids)); } catch (_) { /* 용량 초과·프라이빗 모드 — 이번 세션만 미저장 */ }
}
function knCatOrderClear() { try { localStorage.removeItem(WK_CAT_ORDER_KEY); } catch (_) { /* noop */ } }
//  저장 순서를 앞에, 미지정(신설)은 원래 순서대로 뒤에 — 안정 병합(신규 카테고리가 사라지지 않는다).
function knSortByCatOrder(cats: any[], order?: string[]): any[] {
  const ord = order || knCatOrderSaved();
  if (!ord.length) return cats.slice();
...
function knApplyCatReorder(allCats: any[], srcId: string, targetId: string, before: boolean): boolean {
