// lib/person-name.ts — **사람 이름을 어떻게 부르나**의 단일 판정 (#1813).
//  화면은 전부 이 함수를 부른다. 테스트는 빌드 산출물(public/app/lib/person-name.js)을 잰다 —
//  러너가 web/ 를 수집하지 않아서다(scripts/person-name.test.mjs).
//
...
/** 이름을 가진 최소 형태 — 멤버 레코드든 API 응답이든 이 네 칸만 있으면 된다. */
...
export function personName(p: NamedPerson | null | undefined): string {
  ...
  if (p.use_nickname && nick) return nick;
