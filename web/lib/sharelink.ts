...
const SHARE_ROOTS: Record<string, string> = { shared: '공유 워크스페이스', personal: '개인 폴더' };

/** 루트 키의 사람용 이름. 모르는 키는 그대로(방어적). */
function shareRootLabel(root: string): string {
  return SHARE_ROOTS[String(root)] || String(root || '');
}

/** 경로 조각을 잇는다(빈 조각 무시) — 'a' + 'b/c' → 'a/b/c'. */
function joinRel(...parts: Array<string | null | undefined>): string {
  return parts.map((p) => String(p ?? '').replace(/^\/+|\/+$/g, '')).filter(Boolean).join('/');
}

/** 앱 해시 경로(라우터가 받는 형태). */
function fileLinkHash(root: string, rel: string): string {
 ...
 */
function fileLinkUrl(root: string, rel: string): string {
...
function shareFallbackBox(url: string): void {
  const input = el('input', { class: 'sharelink-input', type: 'text', readonly: '', value: url, spellcheck: 'false' });
  const back = el('div', { class: 'sharelink-back', onclick: (e: any) => { if (e.target === back) back.remove(); } },
    el('div', { class: 'sharelink-box', role: 'dialog', 'aria-label': '공유 링크' },
      el('div', { class: 'sharelink-title', text: '공유 링크' }),
      el('p', { class: 'sharelink-hint', text: '브라우저가 자동 복사를 막았어요 — 아래 주소를 복사해 보내세요.' }),
      input,
      el('div', { class: 'sharelink-acts' },
        el('button', { class: 'btn btn-primary btn-sm', type: 'button', text: '전체 선택', onclick: () => { input.focus(); input.select(); } }),
        el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '닫기', onclick: () => back.remove() }))));
 ...
 */
async function copyFileLink(root: string, rel: string, kind?: 'file' | 'dir'): Promise<void> {
...
function shareLinkIcon(size?: number): any {
  const s = size || 14;
  ...
  n.append(
    sv('path', { d: 'M10 13a5 5 0 0 0 7.07 0l2.12-2.12a5 5 0 0 0-7.07-7.07L10.7 5.24' }),
    sv('path', { d: 'M14 11a5 5 0 0 0-7.07 0L4.81 13.12a5 5 0 0 0 7.07 7.07l1.41-1.41' }));
  return n;
}
