...
const PREFS: Array<{ key: ThemePref; label: string; title: string }> = [
...
export function themeApplied(): 'light' | 'dark' {
  const p = themePref();
...
export function setThemePref(p: ThemePref): void {
  try { p === 'system' ? localStorage.removeItem(THEME_KEY) : localStorage.setItem(THEME_KEY, p); } catch { /* 시크릿 창 등 — 이번 페이지에만 적용 */ }
  apply();
}
...
export function themeControl(cls?: string): HTMLElement {
  const wrap = el('div', { class: 'theme-seg' + (cls ? ' ' + cls : ''), role: 'group', 'aria-label': '화면 테마' });
  const draw = () => {
    const cur = themePref();
    wrap.replaceChildren(...PREFS.map((p) =>
      el('button', {
