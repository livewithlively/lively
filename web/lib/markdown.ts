...
function renderContainer(type, rest, bodyLines) {
  ...
  switch (type) {
    ...
    case 'shot': {
      ...
      const hls: any[] = [], marks: any[] = [], rows: any[] = [];
      ...
      const enter = (i: number) => { if (sticky < 0) setOn(i); };
      const leave = () => { if (sticky < 0) setOn(-1); };
      ...
      if (attrs.caption || summary) fig.append(el('figcaption', { class: 'md-shot-cap' },
        ...
        items.length ? el('span', { class: 'md-shot-hint', 'aria-hidden': 'true', text: '항목을 짚으면 화면에서 위치가 밝혀집니다' }) : null));
