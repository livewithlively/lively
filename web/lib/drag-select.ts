...
// ── 체크박스 드래그 범위 선택(#1140) ──────────────────────────────────────────────
//  체크박스를 누른 채 위아래로 끌면 앵커~포인터 아래 행까지가 한 번에 칠해진다(되돌아오면 칠하기 전 상태로 복구).
//  프로젝트 보드(#366, pjvDrag)의 상호작용을 **input[type=checkbox] 목록**(대시보드 '내 AI 세션' · AI 세션 탭)에
//  맞춰 일반화한 것 — 두 화면이 같은 동작을 갖도록 한 벌만 둔다.
//  · 칠할 때 change 이벤트를 실제로 쏜다 → 호출부의 기존 onchange(선택집합·하단 플로팅 바 갱신)를 그대로 재사용.
//  · Shift+클릭도 같은 범위 선택(직전에 누른 체크박스~현재).
//  · 행(row) 요소를 훑어 그 안의 체크박스를 찾는다 — 체크박스는 평소 폭 0 으로 숨어 있어 포인터가 직접 닿지 않는다.
type DragSelCfg = { row: string; check: string };
const _dragSelCfgs: DragSelCfg[] = [];
let _dragSel: any = null;          // 드래그 중 상태
let _dragSelSuppress: any = null;  // 드래그 직후 앵커에 따라오는 click 1회 무시
const _dragSelLast = new Map<string, any>(); // cfg.check → 마지막으로 누른 체크박스(Shift+클릭 앵커)

function _dragSelCfgOf(cb: any): DragSelCfg | null {
  if (!cb || cb.tagName !== 'INPUT' || cb.type !== 'checkbox') return null;
  return _dragSelCfgs.find((c) => cb.matches(c.check) && cb.closest(c.row)) || null;
}
// 같은 목록(앵커 행의 부모) 안의 체크박스들을 DOM(=시각) 순서로.
function _dragSelBoxes(cfg: DragSelCfg, anchor: any): any[] {
  const row = anchor.closest(cfg.row);
  const scope = (row && row.parentElement) || document;
  return [...scope.querySelectorAll(cfg.check)].filter((c: any) => c.closest(cfg.row));
}
// 체크박스를 특정 상태로(멱등) — 바뀔 때만 change 를 쏴 호출부 상태를 갱신한다.
function _dragSelSet(cb: any, on: boolean) {
  if (!!cb.checked === !!on) return;
  cb.checked = !!on;
  cb.dispatchEvent(new Event('change', { bubbles: true }));
}
// 앵커~over 범위를 mode 로 칠하고, 범위 밖은 드래그 시작 시점(base)으로 되돌린다.
function _dragSelPaint(over: any) {
  const st = _dragSel; if (!st) return;
...
function _dragSelScroller(anchor: any): any {
...
function _dragSelRowAt(st: any): any {
  const boxes = _dragSelBoxes(st.cfg, st.anchor);
  let best: any = null, bestD = Infinity;
  for (const c of boxes) {
    const row = c.closest(st.cfg.row); if (!row) continue;
    const r = row.getBoundingClientRect();
    if (st.px < r.left - 80 || st.px > r.right + 80) continue;
...
function _dragSelTick() {
  const st = _dragSel; if (!st) return; // 드래그가 끝나면 루프도 끝
  if (st.px != null) {
    ...
    let dy = 0;
    if (st.py < r.top + EDGE) dy = -Math.ceil(MAX * Math.min(1, (r.top + EDGE - st.py) / EDGE));
    else if (st.py > r.bottom - EDGE) dy = Math.ceil(MAX * Math.min(1, (st.py - (r.bottom - EDGE)) / EDGE));
    if (dy) { if (sc) sc.scrollTop += dy; else window.scrollBy(0, dy); }
    const cb = _dragSelRowAt(st);
    if (cb && cb !== st.last) { st.last = cb; if (cb !== st.anchor) st.moved = true; _dragSelPaint(cb); }
  }
  requestAnimationFrame(_dragSelTick);
}
function _dragSelEnd(e: any) {
  const st = _dragSel; if (!st) return;
...
function initDragRangeSelect(row: string, check: string) {
  if (_dragSelCfgs.some((c) => c.row === row && c.check === check)) return;
  _dragSelCfgs.push({ row, check });
  if (_dragSelCfgs.length > 1) return; // 리스너는 최초 1회만
  document.addEventListener('pointerdown', (e: any) => {
    ...
    _dragSel = { cfg, anchor: cb, mode: !cb.checked, moved: false, last: cb, px: e.clientX, py: e.clientY,
      scroller: _dragSelScroller(cb), base: new Map(boxes.map((c: any) => [c, !!c.checked])) };
  ...
  document.addEventListener('pointermove', (e: any) => {
    const st = _dragSel; if (!st) return;
    if (e.buttons === 0) { _dragSelEnd(null); return; } // 창 밖에서 손을 뗀 경우 등 — 끊김 방지
    st.px = e.clientX; st.py = e.clientY;
  }, true);
  document.addEventListener('pointerup', (e: any) => { if (_dragSel) _dragSelEnd(e); }, true);
  document.addEventListener('click', (e: any) => {
    ...
    if (e.shiftKey && prev && prev !== cb && prev.isConnected) {
      ...
      if (ai >= 0 && ci >= 0) {
        const lo = Math.min(ai, ci), hi = Math.max(ai, ci);
        boxes.forEach((c, i) => { if (i >= lo && i <= hi && c !== cb) _dragSelSet(c, !!cb.checked); });
      }
    }
