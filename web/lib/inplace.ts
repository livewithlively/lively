...
const PINS = new WeakMap<any, { mo: MutationObserver; prev: string }>();

function unpin(host: any): void {
...
function busy(host: any, ...nodes: any[]): void {
  if (!host) return;
  ...
  if (held) { held.mo.disconnect(); PINS.delete(host); }

  const h = host.isConnected ? host.offsetHeight : 0;     // box-sizing:border-box 전역(01-base.css) → min-height 와 같은 축
  if (h >= 1) host.style.minHeight = h + 'px';
  host.setAttribute('aria-busy', 'true');
  host.replaceChildren(...nodes.flat(Infinity).filter((n: any) => n != null));
