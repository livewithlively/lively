...
//
// ★ 2026-09-03(#2599 T1) 이후 허용목록의 **주인은 이 파일이 아니다** — `src/terminal/session-env-contract.ts`
//  가 목록을 소유하고 sudoers 의 env_keep 두 줄은 거기서 **생성**된다(동일성·완전성은 그 모듈의 시험이 잰다).
//  여기 남은 단언들은 그 위에 얹는 **회귀 백스톱**이다: 이 파일은 빌드 없이(dist 불요) 도는 배포면 시험이라,
//  생성 파이프라인이 통째로 깨져도(모듈이 안 불리거나 마커가 지워져도) 리스 보존만은 독립적으로 지켜본다.
//  ⚠ 그래서 아래 E5 의 13개는 «또 하나의 목록» 이 아니라 **최소 하한선**이다 — 늘리지 말고, 새 변수는 계약에 선언하라.
