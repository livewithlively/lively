...
# #688 우선순위 안내 — 관리탭에서 '명시적으로 끈'(DB explicit off) 박스는 .env 시드보다 관리탭이 우선이라 이 스크립트로도 안 켜진다.
log "참고: 관리탭에서 임베딩을 명시적으로 꺼둔 상태(설정 출처 db-off)면 .env 시드가 무시됩니다 — 그 경우 관리탭 '임베딩(벡터검색)'에서 켜기 저장."
