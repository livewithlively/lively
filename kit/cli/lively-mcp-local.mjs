...
registerTool({
  ...
  description: "등록된 레포의 최신 코드를 이 머신에 확보(로컬에 없으면 clone·있으면 fetch)한 뒤, 격리 브랜치로 git worktree 를 만든다. "
    ...
    + "프로젝트 밖 세션의 기본 경로는 <cwd>/<repo> 인데, 그 자리를 **다른 세션이 쓰고 있으면 옆자리(<repo>-2 …)로 비켜서 새로 만든다** "
    + "— 같은 cwd 에서 뜬 세션들이 한 워킹트리·한 브랜치를 공유하던 자리다(자리 임자는 admin 의 lively-owner.json 으로 판정). "
    + "path 를 명시하면 그 자리를 그대로 주되 남의 자리면 warning 을 단다. "
