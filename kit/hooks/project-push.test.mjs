...
try {
  ...
  // ── 🔴 맥 NFD 로컬 이름 ↔ 서버 NFC (#1278b) — **같은 파일**로 봐야 한다 ──
  //  맥은 파일명을 NFD(자모 분해)로 저장하고 readdir 로도 NFD 를 돌려준다(dev 맥미니 실측: NFC 로 써도 반환은 NFD).
  //  서버는 NFC 로 저장하므로 비교를 바이트로 하면 로컬 파일이 '서버에 없는 새 문서'로 보인다 →
  //   ① 사본을 올려 **중복**이 생기고 ② 원장의 NFC 키가 '로컬에 없음'으로 보여 **서버 원본이 지워진다**.
  //  둘 다 파괴적이라, 비교 키는 반드시 NFC 로 접어야 한다.
  {
    reset();
    const NFC = "제품지식.md".normalize("NFC"), NFD = "제품지식.md".normalize("NFD");
    assert.notEqual(NFC, NFD, "테스트 전제가 깨졌다 — 두 정규화형이 같다");
    const M = Date.now() + 60_000, BODY = "본문";
    SERVER = { [NFC]: { body: BODY, mtime: M } };
    const dir = await mkProj("nfd-vs-nfc", { project_id: PROJECT_ID, sync: "both", last_pull: M },
      { [NFD]: BODY }, { [NFC]: { mtime: M, size: Buffer.byteLength(BODY) } });
    fs.utimesSync(path.join(dir, NFD), new Date(M), new Date(M));   // pull 이 받아둔 그대로(= 손대지 않음)
    await runHook(dir);
    assert.deepEqual(puts.map((x) => x.path), [], `NFD 로컬 이름을 새 문서로 보고 사본을 올렸다: ${JSON.stringify(puts.map((x) => x.path))}`);
    assert.deepEqual(deletes, [], `원장의 NFC 키를 '로컬에 없음'으로 오판해 서버 문서를 지웠다: ${JSON.stringify(deletes)}`);
    assert.deepEqual(upOf(dir)?.conflicts ?? [], [], "정규화만 다른 같은 파일을 충돌로 봤다");
    assert.equal(SERVER[NFC]?.body, BODY, "서버 원본이 손상됐다");
    ok("🔴 맥 NFD 로컬 이름 ↔ 서버 NFC — 같은 파일로 보고 올리지도 지우지도 않는다");
  }
