...
async function localFiles(base) {
  ...
  async function walk(dir, rel, depth) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        try { await fsp.stat(path.join(child, ".git")); skipped.push(childRel + "/"); continue; } catch { /* 레포 아님 → 들어간다 */ }
...
(async () => {
  ...
  for (const f of local.files) {
    ...
    if (!s) {
      ...
      if (ledger[f.path]) { conflicts.push({ path: f.path, why: "받아둔 문서인데 서버에서 사라짐(중앙에서 삭제된 듯) — 되살리지 않고 로컬에 그대로 둠", last_pull: lastPull }); continue; } // ②
      push.push(f);                                                  // ① 진짜 새 문서
      continue;
    }
  ...
  for (const f of push.slice(0, MAX_PUSH)) {
    ...
    try {
      ...
      if (typeof meta2.mtime === "number") {
        ...
        ledgerSet[f.path] = { mtime: meta2.mtime, size: typeof meta2.size === "number" ? meta2.size : f.size };
      } else {
        // 구 게이트웨이(mtime 미반환) → 기준선을 **모른다**. 거짓 기준선을 적느니 지운다(원장 없음 = fail-safe).
        //  다음 pull 이 종전 휴리스틱으로 받아가며 기준선을 다시 세운다.
        ledgerDropAfterPush.push(f.path);
      }
    } catch { failed++; }
  }
  for (const p of del.slice(0, MAX_DELETE)) {
    if (left() <= 0) break;
    try {
      const r = await jfetch(`/api/ui/v6/projects/${projectId}/file?path=${encodeURIComponent(p)}`, { method: "DELETE" });
      if (r.ok) { deleted++; ledgerDrop.push(p); } else failed++;
    } catch { failed++; }
  }
