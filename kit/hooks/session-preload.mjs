...
const managedAutoApproveParts = (full) => {
  for (const server of ["lively", "lively-local"]) {
    const prefix = `mcp__${server}__`;
    if (!String(full || "").startsWith(prefix)) continue;
    const tool = String(full).slice(prefix.length);
...
export function reconcileCodexAutoApprove(want) {
  try {
    ...
    const stripped = block.replace(/\[mcp_servers\.(?:lively|lively-local)\.tools\.[A-Za-z0-9_-]+\]\napproval_mode = "[a-z]+"\n\n?/g, "");
    const lines = [];
    const hasLocalServer = /\n\[mcp_servers\.lively-local\]\n/.test(block);
    for (const full of want) {
      const part = managedAutoApproveParts(full);
      if (!part) continue;                                  // 관리 서버 + TOML 키 안전한 이름만
      if (part.server === "lively-local" && !hasLocalServer) continue;
      lines.push(`[mcp_servers.${part.server}.tools.${part.tool}]`, 'approval_mode = "approve"', "");
    }
...
async function refreshRuntimeConfig() {
  ...
  try {
    ...
    //  permissions.allow 에 reconcile(매 세션 → admin 변경이 재설치 없이 다음 세션 반영). Lively 두 서버만 통과.
