...
//  게이트가 stdout 에 낸 것을 돌려준다. LIVELY_READONLY 값은 인자로 주입.
function runGate(readonlyEnv) {
  const id = `s${++sid}`;
  ...
  const out = execFileSync(process.execPath, [HOOK], {
    ...
    env: { ...process.env, HOME, TMPDIR: TMP, LIVELY_OFF: "", LIVELY_HOOKS_OFF: "", LIVELY_READONLY: readonlyEnv },
    encoding: "utf8",
  });
  return out;
}
...
try {
  ...
  t("대조군(readonly 미설정) — 넛지가 뜬다", () => {
    assert.equal(nagged(runGate("")), true, "readonly 아니면 block 넛지가 나와야");
  });
  // 읽기전용 — 억제: block 넛지가 없어야 하고, 게이트는 조용히 exit 0(execFileSync 가 안 던짐).
  for (const v of ["1", "true", "on", "yes"]) {
    t(`읽기전용(LIVELY_READONLY=${v}) — 넛지 억제`, () => {
      assert.equal(nagged(runGate(v)), false, `LIVELY_READONLY=${v} 면 넛지 없어야`);
    });
  }
  // 거짓값은 억제 안 함(값 판정이지 단순 존재 아님) — '0' 이면 대조군처럼 넛지.
  t("LIVELY_READONLY=0 은 억제 아님(넛지 유지)", () => {
    assert.equal(nagged(runGate("0")), true, "'0' 은 정상 → 넛지 유지");
  });
