...
//  (= 어드민 오버라이드 || 서버 DEFAULT_WRITEBACK_NOTICE, src/org/delivery/hook-defaults.ts)에서 온다 — readHooksConfig()?.writeback_notice
...
try {
  ...
  //  세션 pane 에 -e LIVELY_MODE=readonly 로 주입된다(createSession). incognito 는 LIVELY_OFF=1 이라 위에서 이미 exit.
  if ((process.env.LIVELY_MODE || "").trim().toLowerCase() === "readonly") process.exit(0);
