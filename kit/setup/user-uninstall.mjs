...
function uninstallRcBlock() {
  ...
  for (const rc of candidates) {
    ...
    const bi = cur.indexOf(RC_BEGIN); if (bi === -1) continue;
    const ei = cur.indexOf(RC_END, bi);
    let next;
    if (ei === -1) { const eol = cur.indexOf("\n", bi); const after = eol === -1 ? "" : cur.slice(eol + 1); next = (cur.slice(0, bi) + after).replace(/\n{3,}/g, "\n\n"); }
    else next = (cur.slice(0, bi) + cur.slice(ei + RC_END.length)).replace(/\n{3,}/g, "\n\n");
    next = next.replace(/\s+$/, "") + "\n";
...
function detect() {
  ...
  for (const f of [".zshrc", ".bashrc", ".bash_profile", ".profile"]) { const rc = join(HOME, f); if (existsSync(rc)) { try { if (readFileSync(rc, "utf8").includes(RC_BEGIN)) { out.rc = true; break; } } catch { /* */ } } }
  return out;
}
...
function main() {
  ...
  if (othersStillInstalled) {
    ...
    log("\n[2] 셸 rc(LIVELY_TOKEN export) 정리"); uninstallRcBlock();
