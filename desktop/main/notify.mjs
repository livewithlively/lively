...
export const NOTIFY = {
  ...
  PERSON: "person",             // 사람이 나를 불렀다(멘션·댓글) — 판정은 서버(v6/notify-store.ts)가 한다
  // 세션 종료(정상·강제)는 **알리지 않는다**(원준 2026-08-24). 끝난 세션은 목록에서 확인하면 되고,
  //  배너로 부를 만큼 사람이 지금 해야 할 일이 없다. 알림은 '지금 나를 부르는 것'만 남긴다.
};

/** 기본 설정 — 사람이 끄기 전까지 셋 다 켠다. */
export const NOTIFY_DEFAULTS = { [NOTIFY.WAITING]: true, [NOTIFY.DONE]: true, [NOTIFY.PERSON]: true };
 ...
 * @returns {Map<string, {id:string,name:string,awaiting:boolean,working:boolean}>}
 */
