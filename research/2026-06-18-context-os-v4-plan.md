...
- **V4-P4 ✅ (커밋 f1c5030)·V4-P5 ✅ (커밋 10c2348+8efa51f, workflow-std b206f6b)** — 상세는 위 페이즈 절 참조. P5 후속: stop-writeback-gate REASON v4 정정(distill→ctx_save, 소스+설치본·훅 preview 즉시 반영)·app.js 12→4 주석.
- **V4-P6 최종 종합검증 (워크플로 wtkje2auo, 5차원 병렬+적대종합)** — backend(taxonomy/injection/ingest)·web 4차원 전부 PASS(severityHigh 0): 4축 R/K/H/W가 DB·registry·CHECK·주입·웹·learn 6표면 일관(legacy active 0)·observed 3중동작(주입제외/ctx_grep검색/팀메모리제외) 라이브 동시성립·provenance 컬럼 미rename·distill 전면제거·redact choke-point·WYSIWYG md5 동일·parity65/36툴·test 무실패·v3자산 비파괴. **그러나 migration 차원 HIGH 2건 적발 → ship=false**: ①신규 셀프호스트 fresh-boot 가 4축 중 AREA(domain.space) 통째 누락(src/index.ts 부팅이 domainmap init() 미호출, CLI 전용) ②business 시드6이 'productivity' 레포명 하드코딩(고객 DB엔 없어 skip). 라이브 단일테넌트는 적재완료라 회귀 없음. 잔여(비차단): runbook §3 데이터소스표 옛'A'·H/W authored active 0 데이터관찰·areaMapForIndex 표시카운트 proposed 포함·confidence_chk rule값유지(행0)·멀티바이트 grep -a·P-V3-7 pgvector.
- **V4-P7 셀프호스트 fresh-boot 픽스 ✅ (커밋 efb3955, 재배포·실증)** — P6 HIGH 2건 교정. ①src/index.ts 부팅에 `DOMAINMAP_DATABASE_URL` 게이트로 domainmap init() 배선(item/org 동일평면·비치명·멱등). ②core/schema.ts business 시드 고객불변: 'productivity' 종속 제거→key가 어느 repo에도 space=business로 없을 때만(NOT EXISTS) sentinel repo '_org' 아래 적재·sentinel은 적재필요시만 생성. **실증**: 빈 postgres 컨테이너 fresh-boot→domain/repo/mapping 생성·_org·business6@_org·space CHECK·product0·중복0·init x2 멱등. **라이브 PRE=POST 무변화**(business6@productivity·_org 미생성·product30·total37·부팅로그 3스키마 ready). 게이트 build·test177·parity65/36툴. (독립 적대 재검증 별도 에이전트로 _org 다운스트림 영향 확인 중.)
