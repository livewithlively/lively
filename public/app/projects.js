...
function pjvProjRow(p, reload, select, canDelete, fields, anchorId, taskCtx) {
    ...
    const titleCell = el('div', { class: 'pjv-trow-title-cell' }, select ? null : pjvRowCheck('project', p, { reload }), caret, lead, title, canExpand ? el('span', { class: 'pjv-trow-subcount pjv-subcount-ico', title: nTasks + '개 태스크' }, pjvSubtaskIcon(), el('span', { text: String(nTasks) })) : null, ptagsEl, select ? null : pjvRowActions([
...
function pjvProjTaskRow(projectId, t, members, reload, depth, boardFields) {
    ...
    caret, pjvStatusControl(t, reload), el('span', { class: 'pjv-trow-title' + (isDone ? ' done' : ''), text: t.name || t.title || '(제목 없음)' }), subs.length ? el('span', { class: 'pjv-trow-subcount pjv-subcount-ico', title: subs.length + '개 하위' }, pjvSubtaskIcon(), el('span', { text: String(subs.length) })) : null, tagsEl);
...
function pjvProjMetaPanel(p, members, reload) {
    ...
    return el('div', { class: 'pjv-tm-fields pjv-proj-meta' }, row('◎', '상태', pjvProjStatusPill(p, reload)), 
...
async function renderProjectV2Detail(view, idStr) {
    ...
    el('div', { class: 'proj-detail-titlebox' }, titleEl), el('div', { class: 'proj-detail-actions' }, localWorkBtn, settingsBtn)));
    ...
    view.replaceChildren(head, projectBodySection(id, p, reload), projectCommentsSection(id, members), pjvTasksSection(id, p.tasks || [], members, reload, p.fields || []), projectFolderSection(id, V6_BASE), projectTerminalSection(id, members, meId, V6_BASE, p.name), projectTimelineSection(id, members, V6_BASE));
...
function openProjectComments(id, members) {
    ...
    function renderFeed() {
        ...
        for (const c of comments) {
            ...
            feedBox.append(el('div', { class: 'cmt-card' }, el('span', { class: 'cmt-ava', style: 'background:' + avatarColor(c.actor || who), text: initials(who) }), el('div', { class: 'cmt-body' }, el('div', { class: 'cmt-meta' }, el('span', { class: 'cmt-name', text: who }), el('span', { class: 'cmt-time', text: c.ts ? '· ' + relTime(c.ts) : '' })), el('div', { class: 'cmt-text md-rendered' }, renderMarkdown(c.body || '')), el('div', { class: 'cmt-foot' }, reactRow, replyBtn))));
        }
    ...
    const head = el('div', { class: 'cmt-head' }, el('h3', { text: '코멘트' }), el('div', { class: 'cmt-head-actions' }, sortBtn, searchBtn, el('button', { class: 'cmt-close', type: 'button', title: '닫기 (Esc)', text: '✕', onclick: close })));
...
function pjvTaskRow(projectId, t, members, reload, depth, fields) {
    ...
    const tagsEl = pjvRowTagsEl(t, reload);
    const titleCell = el('div', { class: 'pjv-trow-title-cell' }, pjvRowCheck('task', t, { reload, projectId, members }), caret, pjvStatusControl(t, reload), el('span', { class: 'pjv-trow-title' + (isDone ? ' done' : ''), text: t.name || t.title || '(제목 없음)' }), subs.length ? el('span', { class: 'pjv-trow-subcount pjv-subcount-ico', title: subs.length + '개 하위' }, pjvSubtaskIcon(), el('span', { text: String(subs.length) })) : null, tagsEl);
    if (depth)
        titleCell.style.paddingLeft = (depth * 22) + 'px';
