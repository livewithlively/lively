...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId, folders, pageChrome, favData) {
    ...
    const renderArea = (byStatus, noNav) => {
        ...
        for (const c of PJV_STD_COLS.proj) {
            if (c.defaultHidden && !pjvGetShownCols('proj').has(c.key))
                card.style.removeProperty(PJV_STD_COL_VAR[c.key]);
        }
                ...
                if (vcols.length) {
                    ...
                    for (const c of vcols) {
                        ...
                        if (std) {
                            ...
                            if (def && def.defaultHidden && !pjvGetShownCols('proj').has(std))
                                card.style.setProperty(PJV_STD_COL_VAR[std], PJV_STD_COL_W[std] || '92px');
                        }
                    }
...
function pjvApplyColWidths(card, surface) {
    ...
    for (const colKey of Object.keys(m)) {
        if (PJV_STD_COL_VAR[colKey] && !pjvStdColVisible(surface, colKey))
            continue;
...
function pjvGetShownCols(surface) { try {
    return new Set(JSON.parse(localStorage.getItem(pjvShownColsKey(surface)) || '[]'));
}
catch (_) {
    return new Set();
} }
function pjvSetShownCols(surface, set) { try {
...
function pjvApplyHiddenCols(card, surface) {
    ...
    for (const c of PJV_STD_COLS[surface]) {
        if (c.defaultHidden) {
            if (shown.has(c.key))
                card.style.setProperty(PJV_STD_COL_VAR[c.key], PJV_STD_COL_W[c.key] || '92px');
        }
...
function projectTerminalSection(id, members, meId, base, projectName, project) {
    ...
    newBtn.onclick = (e) => {
        ...
        menu.append(mkItem('💻', '내 PC에서 열기', '개발자용 · 직접 설치해 실행', () => openLocalWorkModal(id, project || { id, name: projectName, repos: projectRepos })), mkItem('☁️', '웹에서 바로 열기', '설치 불필요 · 팀 공용', () => openProjectSessionForm(id, load, B, projectName, projectRepos)));
    };
