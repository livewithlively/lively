...
async function renderProjectV2Board(view) {
    ...
    function repaint() {
        ...
        const opts = Object.assign({}, OPTS_BASE, { select: sel.mode ? { ids: sel.ids, canSelect: canDelete, onToggle: repaintBulk } : null });
        sectionsBox.replaceChildren(projectSection('진행 중', active, '아직 진행 중인 프로젝트가 없습니다. ‘+ 새 프로젝트’로 시작하세요.', reload, false, opts), projectSection('완료', done, '완료한 프로젝트가 아직 없습니다.', reload, true, opts));
        repaintBulk();
    }
...
async function renderProjectV2Detail(view, idStr) {
    ...
    // 제목줄 — 이름+상태칩(좌), 세부설정(우).
    head.append(el('div', { class: 'proj-detail-titlebar' }, el('div', { class: 'proj-detail-titlebox' }, el('h1', { class: 'proj-detail-title' }, p.name), el('span', { class: 'pill-state' + (isDone ? '' : ' confirmed'), text: PJV_STATUS_LABEL[p.status] || p.status })), el('div', { class: 'proj-detail-actions' }, settingsBtn)));
    if (p.description)
        head.append(el('p', { class: 'sub proj-detail-desc', text: p.description }));
    ...
    view.replaceChildren(head, pjvTasksSection(id, p.tasks || [], members, reload, p.fields || []), projectFolderSection(id, V6_BASE), projectTerminalSection(id, members, meId, V6_BASE), projectTimelineSection(id, members, V6_BASE));
...
function pjvStatusGroup(projectId, key, list, members, reload, fields, withCols) {
    ...
    if (withCols) {
        ...
        head = el('div', { class: 'pjv-tgroup-head pjv-tgroup-head-cols ' + m.cls }, el('div', { class: 'pjv-trow-title-cell' }, dot, labelEl, countEl, gcaret), el('div', { class: 'pjv-tcell pjv-colhead', text: '담당자' }), el('div', { class: 'pjv-tcell pjv-colhead', text: '마감일' }), el('div', { class: 'pjv-tcell pjv-colhead', text: '우선순위' }), ...(fields || []).map((f) => pjvColumnHead(f, projectId, reload)), el('div', { class: 'pjv-tcell pjv-tcell-add' }, pjvAddColumnButton(projectId, reload)));
...
function memberPicker(preselected, opts) {
    ...
    function paintChips() {
        ...
        chips.replaceChildren(...sel.map((m) => el('span', { class: 'proj-mp-chip' }, el('span', { text: m.display_name || m.id }), el('button', { class: 'proj-mp-chip-x', type: 'button', text: '×', onclick: () => { selected.delete(m.id); paintChips(); paintResults(); } }))));
    }
            ...
            if (depth === 0 && node && node.querySelector) {
                ...
                if (rowEl && titleEl && subBox && t && t.id != null && !rowEl.dataset.cfDbl) {
                    ...
                    rowEl.addEventListener('click', function (e) {
                        if (!e.target.closest('.pjv-trow-title'))
                            return; // 제목 클릭만 가로챔(셀/컨트롤은 통과)
