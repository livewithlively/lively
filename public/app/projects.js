...
function pjvLoadScopeView(k) { try {
    return JSON.parse(localStorage.getItem(pjvScopeViewKey(k)) || 'null');
}
catch (_) {
    return null;
} }
function pjvSaveScopeView(k, v) { try {
    if (v == null)
        localStorage.removeItem(pjvScopeViewKey(k));
    else
        localStorage.setItem(pjvScopeViewKey(k), JSON.stringify(v));
}
catch (_) { /* noop */ } }
...
function pjvScopeHash(key) {
    if (!key || key === '__all__')
        return '#/projects2';
    if (key === '__none__')
        return '#/projects2/none';
    if (key[0] === 'L')
        return '#/projects2/l/' + key.slice(1);
    if (key[0] === 'F')
        return '#/projects2/f/' + key.slice(1);
    return '#/projects2';
}
...
function pjvPersistSideOpen() { try {
    localStorage.setItem('pjv:sideOpen', pjvBoardView.byArea ? '1' : '0');
}
catch (_) { /* noop */ } }
...
function pjvKanbanBoard(projects, selList, ctx) {
    ...
    if (ctx.groupDir === -1)
        defs = defs.slice().reverse(); // 그룹 방향(#541) — 상태 그룹바이 내림차순이면 컬럼 역순
    const colOf = (p) => {
        if (custom) {
...
async function renderProjectV2Board(view, scopeKey) {
    ...
    if (scopeKey) {
        pjvSidebarSel.key = scopeKey;
        pjvSidebarSel.explicit = true;
    }
    try {
        const s = localStorage.getItem('pjv:sideOpen');
        if (s === '0')
            pjvBoardView.byArea = false;
        else if (s === '1' || scopeKey)
            pjvBoardView.byArea = true;
    }
    catch (_) {
        if (scopeKey)
            pjvBoardView.byArea = true;
    ...
    let allProjects, mineProjects, lists, folders, favData;
    try {
    ...
    view.replaceChildren(pjvProjectListBoard(allProjects, lists, mineIds, reload, canDelete, board.fields || [], board.anchorId, meId, folders, pageChrome, favData));
...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId, folders, pageChrome, favData) {
    ...
    const archivedListIds = new Set((lists || [])
    ...
    const savedViewBtn = el('button', { class: 'pjv-view-btn pjv-savedview-btn', type: 'button', title: '뷰 — 저장된 보기(ClickUp 이관 포함)', style: 'display:none' }, pjvViewIcon(), el('span', { class: 'pjv-view-btn-label', text: '뷰' }));
    // '그룹' — 그룹바이 필드+방향(#541 ClickUp group by 파리티). 기본값=ClickUp 뷰 grouping, 리스트별 로컬 오버라이드.
    const groupBtn = el('button', { class: 'pjv-tb-btn pjv-tb-pill pjv-groupby-btn', type: 'button', title: '그룹 — 필드와 방향으로 묶어 보기' }, pjvTbIcon('group'), el('span', { class: 'pjv-view-btn-label', text: '상태' }));
    ...
    const setAlignLeft = (v) => { try {
        localStorage.setItem('pjv:colAlign', v ? 'left' : 'center');
    }
    catch (_) { /* noop */ } applyAlign(); };
    ...
    const syncToggles = () => {
        ...
        if (searchInput.value !== pjvBoardSearch.q)
            searchInput.value = pjvBoardSearch.q;
    ...
    const renderArea = (byStatus, noNav) => {
        ...
        const favListIds = new Set(((favData && favData.project_lists) || []).map((x) => Number(x)));
        // 리스트 즐겨찾기 토글 — 낙관적 갱신(즉시 트리 재렌더) 후 서버 저장, 실패 시 롤백. 바깥 favData 도 갱신해 renderArea 재호출 간 유지.
        const toggleListFav = async (id, next) => {
            if (next)
                favListIds.add(id);
            else
                favListIds.delete(id);
            favData.project_lists = [...favListIds];
            buildTree();
            try {
                await pjvSetFavorite('project_list', id, next);
            }
            catch (e) {
                if (next)
                    favListIds.delete(id);
        ...
        if (selList && selList.external_id) {
            ...
            if (cu === undefined) {
                ...
                api('/api/ui/v6/project-lists/' + selList.id + '/clickup-fields')
                    .then((d) => { pjvCuFieldsCache.set(selList.id, d || { fields: [] }); if (((d || {}).fields || []).length || ((d || {}).view_columns || []).length || (d || {}).view_grouping || ((d || {}).view_sorting || []).length)
                    render(); })
                ...
                if (vcols.length) {
                    ...
                    for (const c of vcols) {
                        ...
                        if (std) {
                            ...
                            if (def && def.defaultHidden && !pjvGetShownCols('proj', selList.id).has(std))
                                card.style.setProperty(PJV_STD_COL_VAR[std], PJV_STD_COL_W[std] || '92px'); // #710 이 리스트 shown-set 기준
                        }
                    }
        ...
        if (pjvBoardView.overview && selFolder) {
            ...
            const brk = (arr0) => {
                const arr = vis(arr0);
                return { total: arr.length, done: arr.filter((p) => p.status === 'done').length,
            ...
            const statRow = el('div', { class: 'pjv-ov-stats' }, el('span', { class: 'pjv-ov-stat', text: '전체 ' + top.total }), el('span', { class: 'pjv-ov-stat', text: '진행 ' + top.prog }));
            if (pjvProjClosedView.done)
                statRow.append(el('span', { class: 'pjv-ov-stat', text: '완료 ' + top.done }));
            const ov = el('div', { class: 'pjv-overview' });
            ov.append(el('div', { class: 'pjv-ov-head' }, el('div', { class: 'pjv-ov-title' }, el('span', { class: 'pjv-ov-kind', text: isSpace ? '스페이스' : '폴더' }), el('h3', { class: 'pjv-ov-name', text: selFolder.name })), statRow));
            ...
            const ovCard = (key, glyph, name, arr, sub) => {
                ...
                c.append(el('div', { class: 'pjv-ov-card-head' }, glyph, el('span', { class: 'pjv-ov-card-name', text: name }), sub ? el('span', { class: 'pjv-ov-card-sub', text: sub }) : el('span', { class: 'pjv-ov-card-sub' })));
            ...
            for (const l of childLists)
                grid.append(ovCard('L' + l.id, pjvListGlyph(l), l.name, (groupByList.get(l.id)?.projects) || [], null));
            if (!childFolders.length && !childLists.length)
                grid.append(el('div', { class: 'pjv-proj-empty', text: '이 ' + (isSpace ? '스페이스' : '폴더') + '에 리스트가 없습니다.' }));
            ov.append(grid);
            main.append(ov);
        }
        ...
        const listNavItem = (list, sub, depth = 0, opts) => {
            const key = 'L' + list.id;
            ...
            const it = el('div', { class: 'pjv-side-navitem pjv-side-navlist' + (sub ? ' sub' : '') + (active ? ' active' : '') + (favListIds.has(list.id) ? ' is-fav' : ''), role: 'button', tabindex: '0', 'aria-pressed': String(active), ...(noDrag ? {} : { draggable: 'true' }) }, pjvListGlyph(list), el('span', { class: 'pjv-side-navlabel', text: list.name }), catBadge, el('span', { class: 'pjv-side-navcount', text: String(grp ? visCount(grp.projects) : 0) }));
            if (depth > 1)
            ...
            if (!noDrag)
                pjvSideNavDrop(it, {
                    reorderList: (lid) => { if (String(lid) === String(list.id))
                        return false; const d = lists.find((x) => String(x.id) === String(lid)); return !!(d && String(d.folder_id ?? '') === String(list.folder_id ?? '')); },
                    onList: (lid, after) => {
                        if (String(lid) === String(list.id))
                            return;
        ...
        sideResize.addEventListener('mousedown', (e) => {
            e.preventDefault();
            e.stopPropagation();
            const wrap = sideResize.closest('.pjv-side-wrap');
            ...
            const onMove = (ev) => {
                if (ev.buttons === 0) {
                    onUp();
                    return;
                } // 창 밖에서 손 뗀 경우 등 mouseup 유실 방지(#541 리뷰)
                let w = startW + (ev.clientX - startX);
                w = Math.max(150, Math.min(440, w));
                if (wrap)
                    wrap.style.setProperty('--pjv-side-w', Math.round(w) + 'px');
            };
            document.addEventListener('mousemove', onMove);
            document.addEventListener('mouseup', onUp);
            window.addEventListener('blur', onUp);
        });
        sideResize.addEventListener('dblclick', (e) => {
            e.preventDefault();
            e.stopPropagation();
            const wrap = sideResize.closest('.pjv-side-wrap');
            if (wrap)
                wrap.style.removeProperty('--pjv-side-w');
            try {
        ...
        addBtn.onclick = (e) => {
            ...
            const mk = (label, hint, fn) => {
                const b = el('button', { class: 'pjv-menu-item', type: 'button' }, el('span', { class: 'pjv-view-item-main' }, el('span', { class: 'pjv-view-item-label', text: label }), el('span', { class: 'pjv-view-item-hint', text: hint })));
        ...
        const renderFolderNode = (f, depth) => {
            ...
            if (sideSearchActive() && !folderMatchesDeep(f))
                return;
            ...
            fit.addEventListener('dragstart', (ev) => { pjvSideDrag.kind = 'folder'; pjvSideDrag.id = f.id; document.body.classList.add('pjv-side-dragging'); try {
                ev.dataTransfer.effectAllowed = 'move';
                ev.dataTransfer.setData('text/plain', 'F' + f.id);
            }
            catch (_) { /* */ } });
            ...
            if (open) {
                ...
                if (fLists.length)
                    for (const l of fLists) {
                        treeWrap.append(listNavItem(l, true, depth + 1));
                        appendProjMatches((groupByList.get(l.id)?.projects) || [], depth + 2);
                    }
                else if (!childFolders.length && !sideSearchActive())
                    treeWrap.append(el('button', { class: 'pjv-side-folder-empty', type: 'button', onclick: (e) => { e.stopPropagation(); openListForm(reload, undefined, { folderId: f.id }); } }, el('span', { class: 'pjv-newlist-plus', text: '＋' }), el('span', { text: '리스트 추가' })));
            }
        };
        ...
        function folderMatchesDeep(f) {
            if (!sideSearchActive())
                return true;
            if (folderSelfMatch(f))
                return true;
            if (folderListsDeep(f.id).some(listMatchesDeep))
                return true;
        ...
        function sideProjRow(p, depth) {
            ...
            const it = el('div', { class: 'pjv-side-navitem pjv-side-navproj' + (p.status === 'done' ? ' done' : ''), role: 'link', tabindex: '0', title: p.name }, ic, el('span', { class: 'pjv-side-navlabel', text: p.name }));
            ...
            it.addEventListener('click', go);
            it.addEventListener('keydown', (e) => { if (e.key === 'Enter') {
                e.preventDefault();
                go(e);
            } });
            return it;
        ...
        function appendProjMatches(projArr, depth) {
            ...
            const MAX = 8;
            for (const p of ms.slice(0, MAX))
                treeWrap.append(sideProjRow(p, depth));
            if (ms.length > MAX)
                treeWrap.append(el('div', { class: 'pjv-side-navproj-more', text: '＋' + (ms.length - MAX) + '개 더 — 리스트를 눌러 보기', style: `padding-left:${8 + Math.max(depth, 1) * 14 + 6}px` }));
        }
        ...
        const renderArchiveNode = () => {
            ...
            caret.addEventListener('click', (e) => { e.stopPropagation(); if (!archiveFolder)
                return; pjvFolderOpen.set(archiveFolder.id, !open); render(); });
            ...
            const it = el('div', { class: 'pjv-side-navitem pjv-side-navfolder pjv-side-archive' + (key && sel === key ? ' active' : ''), role: 'button', tabindex: '0',
                title: '아카이브 — 다 지난 리스트·폴더·프로젝트를 여기로 끌어 치워둡니다(삭제 아님). 보드 목록에서 빠지고, 다시 끌어내면 원래대로 돌아옵니다.' }, el('span', { class: 'pjv-side-archive-ico', 'aria-hidden': 'true', text: '📦' }), el('span', { class: 'pjv-side-navlabel', text: PJV_ARCHIVE_FOLDER_NAME }), el('span', { class: 'pjv-side-navcount', text: n ? String(n) : '' }), archiveFolder ? caret : null);
            const go = (e) => {
                e.stopPropagation();
                if (!archiveFolder) {
                    toast('아카이브가 비어 있어요 — 리스트·폴더·프로젝트를 여기로 끌어다 놓으세요');
                    return;
                }
                pjvFolderOpen.set(archiveFolder.id, true);
                // 아카이브엔 완료 프로젝트가 대부분 — 들어갈 때 Closed 를 켜서 '있는데 안 보이는' 상황을 막는다(툴바에서 다시 끌 수 있음).
                pjvProjClosedView.done = true;
                syncToggles();
                selectArea(key);
            };
            it.addEventListener('click', go);
            ...
            if (archiveFolder && open) {
                for (const c of (foldersByParent.get(archiveFolder.id) || []))
                    renderFolderNode(c, 1);
                for (const l of (listsByFolder.get(archiveFolder.id) || [])) {
                    if (sideSearchActive() && !listMatchesDeep(l))
                        continue;
        ...
        const buildTree = () => {
            ...
            if (!sideSearchActive()) {
                ...
                if (favLists.length) {
                    ...
                    for (const l of favLists)
                        treeWrap.append(listNavItem(l, false, 0, { noDrag: true }));
            ...
            for (const l of topLists)
                if (listMatchesDeep(l)) {
                    treeWrap.append(listNavItem(l, false));
                    appendProjMatches((groupByList.get(l.id)?.projects) || [], 1);
                }
            ...
            if (!sideSearchActive() || archiveHasMatch())
                renderArchiveNode();
            if (sideSearchActive()) {
                if (!treeWrap.querySelector('.pjv-side-navitem'))
        ...
        searchInput.addEventListener('keydown', (e) => { if (e.key === 'Escape') {
            searchInput.value = '';
            sideSearchQ = '';
            searchBox.classList.remove('has-q');
        ...
        nav.append(navInner);
        if (noNav) {
            shellHost.replaceChildren(main);
            return;
        } // #662 — 스코프 유지 + 사이드바 없음: 같은 main 셸만(nav 제외)
        const sideWrap = el('div', { class: 'pjv-side-wrap' }, nav, main);
        try {
            const sw = localStorage.getItem('pjv:sideW');
            if (sw)
                sideWrap.style.setProperty('--pjv-side-w', sw.indexOf('px') >= 0 ? sw : sw + 'px');
        }
        catch (_) { /* noop */ }
    ...
    const render = () => {
        ...
        if (byArea) {
            renderArea(byStatus);
            return;
        } // 페이지크롬·툴바는 renderArea 가 우측 컬럼 상단에 넣는다
    ...
    const scopeName = (key) => {
        if (key === '__none__')
            return '기타 (미분류)';
        if (typeof key === 'string' && key[0] === 'L') {
            const l = lists.find((x) => String(x.id) === key.slice(1));
            return l ? l.name : null;
        }
        if (typeof key === 'string' && key[0] === 'F') {
            const f = (folders || []).find((x) => String(x.id) === key.slice(1));
    ...
    const rerenderScoped = () => { syncToggles(); if (pjvBoardView.byArea || (pjvSidebarSel.key && pjvSidebarSel.key !== '__all__'))
        pjvSaveScopeView(pjvSidebarSel.key, pjvSnapshotView()); render(); };
    ...
    searchBtn.onclick = (e) => {
        e.stopPropagation();
        if (searchOpen && !pjvBoardSearch.q.trim()) {
            searchOpen = false;
            syncToggles();
            return;
        }
    ...
    addProjBtn.onclick = (e) => {
        e.stopPropagation();
        const trig = card.querySelector('.pjv-addrow-trigger');
        if (trig) {
    ...
    const syncCrumbs = () => {
        ...
        const chain = [];
        let leafList = null, leafFolder = null;
        if (typeof sel === 'string' && sel[0] === 'L') {
            ...
            while (f) {
                chain.unshift(f);
                f = f.parent_id != null ? folderById(f.parent_id) : null;
            }
        }
        ...
        if (leafList)
            items.push(mkCrumb(pjvListGlyph(leafList), leafList.name, 'L' + leafList.id, 'is-leaf'));
        if (sel === '__none__')
            items.push(mkCrumb(pjvBundleIcon(null, 'none'), '기타 (미분류)', '__none__', 'is-leaf'));
        // 구분자(/)를 끼워 넣고, 잎 옆에 ⌄(설정 메뉴)·☆(즐겨찾기)를 붙인다.
        const nodes = [];
        items.forEach((it, i) => { if (i)
            nodes.push(el('span', { class: 'pjv-crumb-sep', 'aria-hidden': 'true', text: '/' })); nodes.push(it); });
        ...
        if (leaf) {
            ...
            menuBtn.onclick = (e) => {
                ...
                if (leafList)
                    pjvListSettingsMenu(menu, close, leafList, reload);
                else
                    pjvFolderTreeMenu(menu, close, leafFolder, reload);
            };
            nodes.push(menuBtn);
            if (leafList) {
                const favIds = new Set(((favData && favData.project_lists) || []).map((x) => Number(x)));
                ...
                star.onclick = async (e) => {
                    ...
                    const next = !isFav;
                    if (next)
                        favIds.add(Number(leafList.id));
                    else
                        favIds.delete(Number(leafList.id));
                    if (favData)
                        favData.project_lists = [...favIds];
                    render();
                    try {
                        await pjvSetFavorite('project_list', leafList.id, next);
                    }
                    catch (err) {
                        toast('즐겨찾기 저장 실패 — ' + (err && err.message || err), true);
                        reload();
                    }
                };
                nodes.push(star);
...
function pjvListColHead(fields, anchorId, reload, listId) {
    const headEl = el('div', { class: 'pjv-tgroup-head pjv-tgroup-head-cols pjv-list-colhead' }, el('div', { class: 'pjv-trow-title-cell' }, el('span', { class: 'pjv-list-colhead-name', text: '프로젝트' }), pjvNameResizeHandle()), pjvStdColHead('proj', 'team', '팀원'), pjvStdColHead('proj', 'due', '마감일'), pjvStdColHead('proj', 'start', '시작일'), pjvStdColHead('proj', 'created', '생성일'), pjvStdColHead('proj', 'updated', '갱신일'), pjvStdColHead('proj', 'priority', '우선순위'), pjvStdColHead('proj', 'sess', '내 세션'), ...(fields || []).map((f) => pjvColumnHead(f, anchorId, reload)), el('div', { class: 'pjv-tcell pjv-tcell-add' }, anchorId ? pjvAddColumnButton(anchorId, reload, listId) : el('span', {})));
...
function pjvFavStar(isFav, onToggle) {
...
function openListForm(reload, list, opts) {
    ...
    const visRow = el('div', { class: 'pjv-visrow' + (visibility === 'members' ? ' on' : ''), role: 'switch', tabindex: '0',
        'aria-checked': visibility === 'members' ? 'true' : 'false' }, el('span', { class: 'pjv-visrow-txt' }, el('span', { class: 'pjv-visrow-title', text: '공개범위를 멤버로 제한' }), el('span', { class: 'pjv-visrow-hint', text: '켜면 멤버가 아닌 사람에겐 이 리스트와 프로젝트가 보이지 않아요.' })), visSw);
    const toggleVis = () => {
...
function pjvStatusIconStd(status, size) {
    ...
    if (status === 'done')
        return pjvStatusIcon('done', color('done', '#22c55e'), undefined, size);
    if (status === 'todo')
        return pjvStatusIcon('active', color('todo', '#94a3b8'), 0, size);
...
function pjvGrpOpenGet(listId, gid) { try {
    return localStorage.getItem(pjvGrpOpenKey(listId, gid)) !== '0';
}
catch (_) {
    return true;
} }
function pjvGrpOpenSet(listId, gid, open) {
    try {
        const k = pjvGrpOpenKey(listId, gid);
        if (open)
...
const pjvLocalSortOverride = new Map();
...
function pjvColSortCmp(sortSpec) {
    ...
    const val = (p) => {
        ...
        if (key === 'team') {
            const m = (p.members || [])[0];
            return m ? String(m.display_name || m.member_id || '') : null;
        }
        if (key === 'due')
            return p.due_date || null;
        if (key === 'start')
            return p.start_date || null;
        if (key === 'created')
...
function pjvRenderStatusGroups(main, shownProjects, selList, opts) {
    ...
    if (selList && pjvListIsCustomStatus(selList)) {
        ...
        for (const p of shownProjects) {
            ...
            // 미스매치는 네이티브 status 로 흡수 — done 은 Done(없으면 Closed), 그 외는 Active 첫 상태.
            if (!d)
                d = (p.status === 'done' ? (firstOfCat('done') || firstOfCat('closed')) : firstOfCat('active')) || defs[0];
        ...
        for (const cat of PJV_STATUS_CATS) {
            for (const d of defs.filter((x) => x.category === cat.key)) {
                if ((cat.key === 'done' || cat.key === 'closed') && !pjvProjClosedView.done)
                    continue;
    ...
    if (!mineOnly || inprog.length)
        subs.push(['진행 중', 'in_progress', inprog]);
    if (!mineOnly || todo.length)
        subs.push(['할 일', 'todo', todo]);
    if (pjvProjClosedView.done && (!mineOnly || done.length))
        subs.push(['완료', 'done', done]);
    if (gb.dir === -1)
        subs.reverse();
    for (const [label, key, arr] of subs)
        main.append(pjvProjGroup(label, key, sortArr(arr), reload, null, canDelete, takeCols(), fields, anchorId, meId, taskCtx, undefined, mineOnly, listIdForAdd));
}
...
function pjvRenderFieldGroups(main, shownProjects, selList, opts, gb, sortArr, takeCols) {
    ...
    for (const p of shownProjects) {
        if (gb.field === 'assignee') {
            const ms = (p.members || []);
            if (!ms.length)
                put(NONE, '담당자 없음', null, p);
            else
                for (const m of ms)
                    put('m:' + m.member_id, String(m.display_name || m.member_id), String(m.display_name || m.member_id), p);
        }
        else if (gb.field === 'priority') {
            const r = PJV_PRIORITY_RANK[p.priority];
            if (r === undefined)
                put(NONE, '우선순위 없음', null, p);
            else
                put('p:' + p.priority, ({ urgent: '긴급', high: '높음', normal: '보통', low: '낮음' })[p.priority] || p.priority, r, p);
        }
        else if (gb.field === 'due') {
            if (!p.due_date)
                put(NONE, '마감일 없음', null, p);
            else
                put('d:' + p.due_date, pjvFmtDate(p.due_date), p.due_date, p);
        }
        else if (gb.field === 'tag') {
            const tags = (p.tags || []);
            if (!tags.length)
                put(NONE, '태그 없음', null, p);
            else
                for (const t of tags)
                    put('t:' + String(t.name).toLowerCase(), String(t.name), String(t.name).toLowerCase(), p);
        }
    }
    ...
    if (!entries.length) {
        main.append(el('div', { class: 'pjv-proj-empty', text: mineOnly ? '내가 할당된 프로젝트가 없습니다.' : '아직 프로젝트가 없습니다.' }));
        return;
    }
...
function pjvProjStatusDot(p, reload) {
    ...
    btn.onclick = (e) => {
        ...
        for (const st of ['todo', 'in_progress', 'done']) {
            ...
            const item = el('button', { class: 'pjv-menu-item' + (cur ? ' sel' : ''), type: 'button' }, pjvStatusIconStd(st, 'sm'), el('span', { text: m.label }));
...
async function pjvBulkRunDefaultsModal(ctx) {
    ...
    const wtRow = el('label', { class: 'proj-sess-auto', style: 'margin-top:2px' }, wtChk, el('span', { text: ' 이 프로젝트 전용 작업 공간에서 격리 실행 — 매번 자동으로 준비되고(있으면 재사용) 다른 작업과 안 섞여요 (권장)' }));
    const wtHint = el('div', { class: 'caption', style: 'margin-top:2px' }, '작업 공간은 프로젝트에 맞춰 자동으로 준비돼요 — 이름을 따로 정할 필요 없어요. (개발자용: git worktree · 브랜치 project/' + pid + ')');
...
function pjvBulkStatus(anchor) {
    ...
    for (const [key, label] of [['todo', '할 일'], ['in_progress', '진행 중'], ['done', '완료']]) {
        const item = el('button', { class: 'pjv-menu-item', type: 'button' }, pjvStatusIconStd(key, 'sm'), el('span', { text: label }));
        item.onclick = () => {
            close();
            if (pjvSel.kind === 'task')
...
async function pjvBulkList(anchor) {
    ...
    for (const l of lists)
        menu.append(mkItem(l.name, l.id, l.color || avatarColor('list' + l.id)));
    if (!lists.length)
        menu.append(el('div', { class: 'pjv-menu-empty', text: '폴더가 없습니다 — 상단 ‘폴더’ 버튼을 켜면 왼쪽에서 ‘＋ 새 폴더’로 만들 수 있어요' }));
}
...
function pjvGroupCheck(kind, bodyEl) {
    const cb = el('button', { class: 'pjv-row-check pjv-group-check', type: 'button', title: '이 그룹 전체 선택/해제', 'aria-label': '그룹 전체 선택/해제' });
    const rowChecks = () => [...bodyEl.querySelectorAll('.pjv-row-check')].filter((c) => c._pjvKind === kind);
    const allOn = (checks) => checks.length > 0 && checks.every((c) => pjvSel.kind === kind && pjvSel.ids.has(c._pjvItem.id));
...
function pjvGroupReorderTarget(body, _reload) {
    let marker = null;
    const rows = () => [...body.children].filter((c) => c.classList && c.classList.contains('pjv-proj-wrap'));
    const clear = () => { if (marker) {
        marker.remove();
        marker = null;
    } };
    body.addEventListener('dragover', (ev) => {
        if (pjvSortCtx && pjvSortCtx.colSort)
            return; // 정렬 중엔 수동 순서 의미 없음
        const dragId = pjvFolderDrag.id;
        if (dragId == null)
            return;
        const dragged = rows().find((w) => String(w.getAttribute('data-proj-id')) === String(dragId));
        if (!dragged)
            return; // 이 그룹의 행이 아님 — 리스트/폴더 이동 등 기존 드롭 타깃에 맡긴다
        ev.preventDefault();
        ev.stopPropagation();
        try {
            ev.dataTransfer.dropEffect = 'move';
        ...
        if (!marker)
            marker = el('div', { class: 'pjv-proj-insert-marker', 'aria-hidden': 'true' });
        let before = null;
        for (const w of rows()) {
            if (w === dragged)
                continue;
        ...
        if (before)
            body.insertBefore(marker, before);
        else {
            const rs = rows();
            const last = rs[rs.length - 1];
            if (last)
                body.insertBefore(marker, last.nextSibling);
        }
    });
    body.addEventListener('dragleave', (ev) => { if (!body.contains(ev.relatedTarget))
        clear(); });
    body.addEventListener('drop', (ev) => {
        const dragId = pjvFolderDrag.id;
        if (dragId == null || !marker) {
            clear();
            return;
        }
        const dragged = rows().find((w) => String(w.getAttribute('data-proj-id')) === String(dragId));
        ...
        clear();
        const ids = rows().map((w) => Number(w.getAttribute('data-proj-id'))).filter(Boolean);
        if (ids.length > 1) {
...
function pjvProjRow(p, reload, select, canDelete, fields, anchorId, taskCtx) {
    ...
    if (!select) {
        ...
        wrap.addEventListener('dragstart', (ev) => {
            ...
            if (t && t.closest && t.closest('.pjv-row-check, .pjv-trow-caret, .pjv-row-actions, .pjv-subcount-ico, .pjv-cell-btn, .pjv-trow-subs, button, input, a')) {
                ev.preventDefault();
                return;
            }
...
function pjvProjGroup(label, statusKey, list, reload, select, canDelete, withCols, fields, anchorId, meId, taskCtx, sepTasks, noAdd, listId, statusDef) {
    ...
    if (!select && cat !== 'done' && cat !== 'closed' && !noAdd)
    ...
    let head;
    if (withCols) {
        head = el('div', { class: 'pjv-tgroup-head pjv-tgroup-head-cols ' + meta.cls }, el('div', { class: 'pjv-trow-title-cell' }, headCheck(), dot, labelEl, countEl, gcaret, pjvNameResizeHandle()), pjvStdColHead('proj', 'team', '팀원'), pjvStdColHead('proj', 'due', '마감일'), pjvStdColHead('proj', 'start', '시작일'), pjvStdColHead('proj', 'created', '생성일'), pjvStdColHead('proj', 'updated', '갱신일'), pjvStdColHead('proj', 'priority', '우선순위'), pjvStdColHead('proj', 'sess', '내 세션'), ...(fields || []).map((f) => pjvColumnHead(f, anchorId, reload)), el('div', { class: 'pjv-tcell pjv-tcell-add' }, anchorId ? pjvAddColumnButton(anchorId, reload, listId) : el('span', {}))); // #607/D 리스트별 필드 — 이 그룹의 리스트로 컬럼 추가
...
function pjvProjAddRow(statusKey, reload, body, countEl, fields, select, canDelete, anchorId, meId, taskCtx, listId, statusDef) {
    ...
    let indentParent = null; // Tab 들여쓰기(#663) — 바로 위 프로젝트의 '태스크'로 만들 때 그 부모 {id,name}. Shift+Tab 해제.
    ...
    el('span', { class: 'pjv-row-check-spacer', 'aria-hidden': 'true' }), el('span', { class: 'pjv-addrow-plus', text: '＋' }), el('span', { text: '프로젝트' }));
    ...
    const buildTitleCell = () => {
        ...
        const tc = el('div', { class: 'pjv-trow-title-cell' }, el('span', { class: 'pjv-row-check-spacer', 'aria-hidden': 'true' }), el('span', { class: 'pjv-trow-caret empty', 'aria-hidden': 'true' }), dotEl, input);
        if (indentParent)
            tc.style.paddingLeft = '22px'; // 하위(태스크) 위치 — 태스크 추가행(pjvAddRow)과 동일 톤
        return tc;
    };
    ...
    const applyIndent = () => {
        ...
        if (old)
            old.replaceWith(buildTitleCell());
    ...
    const commitAsTask = async () => {
        ...
        try {
            ...
            const patch = {};
            if (draft.memberIds && draft.memberIds[0])
                patch.assignee = draft.memberIds[0];
            if (draft.due_date)
            ...
            if (created && Object.keys(patch).length)
                await api('/api/ui/v6/tasks/' + created.id, { method: 'POST', body: JSON.stringify(patch) }).catch(() => { });
            if (taskCtx && taskCtx.invalidate)
                taskCtx.invalidate(indentParent.id);
...
function npTaskEditor() {
    ...
    let indentTask = null; // Tab 들여쓰기 대상(바로 위 태스크). null=상위 태스크로 추가.
    const applyIndent = () => {
    ...
    addInput.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Tab') {
            ...
            if (!indentTask && model.length) {
                indentTask = model[model.length - 1];
                applyIndent();
            } // 위에 태스크가 없으면 무시
            return;
        }
        ...
        if (indentTask) {
            ...
            const sub = { name };
            indentTask.subs.push(sub);
            const subAdd = indentTask.subBox && indentTask.subBox.querySelector('.np-subadd');
            if (subAdd)
                indentTask.subBox.insertBefore(buildSubRow(indentTask, sub), subAdd);
            else if (indentTask.subBox)
                indentTask.subBox.append(buildSubRow(indentTask, sub));
        }
        else {
            const task = { name, subs: [] };
            model.push(task);
    ...
    const go = async (withRun) => {
        ...
        try {
            ...
            if (withRun && np && np.id) {
                ...
                let sid = '';
                try {
                    const sr = await api('/api/ui/v6/projects/' + np.id + '/sessions', { method: 'POST', body: JSON.stringify(sbody) });
                    sid = (sr && sr.session && sr.session.id) || '';
                }
                catch (e) {
                    toast('프로젝트는 만들었지만 세션 실행 실패 — ' + (e.message || e), true);
                }
                back.remove();
                if (reload)
                    reload();
...
function pjvProjMetaPanel(p, members, reload) {
    ...
    row('👤', '팀원', pjvProjTeamControl(members, (ids) => pjvSaveProjMembers(p.id, ids))), row('🗓', '기간', pjvProjDatesField(p, reload)), row('⚑', '우선순위', pjvPriorityControl(p, (patch) => projPatch(p.id, patch, reload))), 
...
function renderProjectDemo(view) {
    ...
    head.append(el('div', { class: 'proj-detail-titlebar' }, el('div', { class: 'proj-detail-titlebox' }, titleEl), el('div', { class: 'proj-detail-actions' }, settingsBtn)));
    head.append(pjvProjMetaPanel(P, members, noop));
    view.replaceChildren(head, projectBodySection('__demo__', P, noop), projectKnowledgeSection('__demo__', P, noop), pjvTasksSection('__demo__', P.tasks, members, noop, []), demoFolderCard(), demoTerminalCard(members, meId));
...
function demoFolderCard() {
    ...
    card.append(el('div', { class: 'card-head' }, el('h3', { text: '공유 폴더' }), el('div', { class: 'card-head-actions' }, el('button', { class: 'btn btn-ghost btn-sm', text: '＋ 새 폴더' }), el('button', { class: 'btn btn-ghost btn-sm', text: '＋ 업로드' }))));
    const file = (ic, nm) => el('div', { class: 'proj-file-card' }, el('div', { class: 'proj-file-card-ic', text: ic }), el('div', { class: 'proj-file-card-nm', text: nm }));
    card.append(el('div', { style: 'display:flex;gap:14px;flex-wrap:wrap;padding:6px 2px' }, file('📊', '요금제_비교표.xlsx'), file('📄', '출시_공지_초안.docx'), file('🖼', '결제화면_시안.png')));
    return card;
}
...
function pjvOpenProjectModal(projectId, pageReload) {
    ...
    function closeModal(mode = 'user') {
        if (closed)
            return;
        closed = true;
        if (_pjvPmOpen && _pjvPmOpen.close === closeModal)
            _pjvPmOpen = null;
        ...
        if (pageReload && mode === 'user' && !popped)
    ...
    fullLink.onclick = (e) => {
        ...
        if (location.hash === url)
            ...
            location.hash = url; // URL 동기화가 안 된 예외(pushState 실패 등) — 평소대로 해시 이동
    };
...
async function renderProjectV2Detail(view, idStr) {
    if (idStr === '__demo__')
        return renderProjectDemo(view); // Lively 둘러보기 전용 예시 프로젝트(#761)
    pjvSelReset(); // 화면 진입/재렌더 시 다중선택·하단 바 초기화
    pjvSortCtx = null;
    pjvGroupCtx = null; // 보드의 정렬/그룹 컨텍스트 잔존 차단(#541 리뷰 — 상세 헤더가 보드 리스트 설정을 오염)
...
function projectKnowledgeSection(id, p, reload) {
    ...
    el('a', { href: '#/learn/docs/wiki?focus=required', target: '_blank', rel: 'noopener', title: '새 탭에서 사용 가이드 열기',
        style: 'color:var(--blue); text-decoration:none; white-space:nowrap;', text: '자세히' }))), knAddBtn));
...
const pjvSidebarSel = { key: '__all__', explicit: false };
...
function pjvKeepScopeOnCollapse() {
    if (pjvSidebarSel.key && pjvSidebarSel.key !== '__all__')
        pjvSidebarSel.explicit = true;
    else
        pjvExitAreaMode();
}
...
function pjvTbIcon(kind, cls) {
    ...
    if (kind === 'group') { // 레이어(그룹) — 겹쳐 쌓은 판 3장
        n.append(sv('path', { d: 'M12 3.4 3.6 7.6 12 11.8l8.4-4.2L12 3.4Z' }), sv('path', { d: 'M3.6 12 12 16.2 20.4 12' }), sv('path', { d: 'M3.6 16.4 12 20.6l8.4-4.2' }));
        return n;
    }
    ...
    if (kind === 'people') { // 담당자 — 두 사람
        n.append(sv('circle', { cx: 9.5, cy: 8.4, r: 3.1 }), sv('path', { d: 'M3.8 19a5.7 5.7 0 0 1 11.4 0' }), sv('circle', { cx: 16.9, cy: 7.4, r: 2.2 }), sv('path', { d: 'M16.5 12.7c2.3 0 4.2 1.9 4.2 4.2' }));
        return n;
    }
    if (kind === 'search') {
    ...
    if (kind === 'plus') {
        n.append(sv('path', { d: 'M12 5.2v13.6M5.2 12h13.6' }));
        return n;
    }
    if (kind === 'trash') {
        n.append(sv('path', { d: 'M4.6 6.8h14.8' }), sv('path', { d: 'M9.6 6.8V5.3c0-.72.58-1.3 1.3-1.3h2.2c.72 0 1.3.58 1.3 1.3v1.5' }), sv('path', { d: 'M6.7 6.8l.85 12.05A1.35 1.35 0 0 0 8.9 20.1h6.2a1.35 1.35 0 0 0 1.35-1.25L17.3 6.8' }));
        return n;
    }
    if (kind === 'x') {
        n.append(sv('path', { d: 'M6.6 6.6l10.8 10.8M17.4 6.6 6.6 17.4' }));
        return n;
    }
...
function pjvTabIcon(kind) {
    const n = sv('svg', { class: 'pjv-vtab-ic', viewBox: '0 0 24 24', width: 16, height: 16,
        fill: 'none', stroke: 'currentColor', 'stroke-width': 1.7, 'stroke-linecap': 'round', 'stroke-linejoin': 'round', 'aria-hidden': 'true' });
    if (kind === 'board') {
        n.append(sv('rect', { x: 3.4, y: 4.6, width: 4.8, height: 14.8, rx: 1.6 }), sv('rect', { x: 9.6, y: 4.6, width: 4.8, height: 9.6, rx: 1.6 }), sv('rect', { x: 15.8, y: 4.6, width: 4.8, height: 12.4, rx: 1.6 }));
    }
    else if (kind === 'timeline') {
        n.append(sv('rect', { x: 3.2, y: 5, width: 10.5, height: 3.8, rx: 1.9 }), sv('rect', { x: 7.4, y: 10.1, width: 13.4, height: 3.8, rx: 1.9 }), sv('rect', { x: 5, y: 15.2, width: 9.2, height: 3.8, rx: 1.9 }));
    }
    else if (kind === 'table') {
        n.append(sv('rect', { x: 3.2, y: 4.8, width: 17.6, height: 14.4, rx: 2.2 }), sv('path', { d: 'M3.2 9.6h17.6M9.6 9.6v9.6' }));
    }
    else { // list
        n.append(sv('circle', { cx: 5.2, cy: 7.2, r: 1.15, fill: 'currentColor', stroke: 'none' }), sv('circle', { cx: 5.2, cy: 12, r: 1.15, fill: 'currentColor', stroke: 'none' }), sv('circle', { cx: 5.2, cy: 16.8, r: 1.15, fill: 'currentColor', stroke: 'none' }), sv('path', { d: 'M9 7.2h11M9 12h11M9 16.8h11' }));
    }
    return n;
}
// ══ 툴바 필터 엔진 ══════════════════════════════════════════════════════════
...
function pjvBoardSettingsPopover(anchor, ctx) {
    ...
    pop.append(pjvSwitchRow('값을 왼쪽 정렬', () => ctx.isAlignLeft(), (v) => ctx.setAlignLeft(v), () => { }));
    ...
    const sv2 = el('button', { class: 'pjv-menu-item', type: 'button' }, el('span', { text: '저장된 뷰' + (pjvSavedView.id != null && pjvSavedView.name ? ' · ' + pjvSavedView.name : '') }), el('span', { class: 'pjv-menu-caret', text: '›' }));
...
let pjvBoardFieldsCur = []; // #710 확장 — 현재 보드의 커스텀 필드(anchor board.fields). 커스텀 컬럼 리스트별 숨김 재조정(pjvApplyHiddenCols)·되살리기 패널이 참조.
...
function fmtFileDate(ms) {
    ...
    if (sameYMD(d, now))
        return '오늘 ' + p(d.getHours()) + ':' + p(d.getMinutes());
    if (sameYMD(d, yst))
        return '어제 ' + p(d.getHours()) + ':' + p(d.getMinutes());
...
function projFileCardEl(id, it, rel, onDir, reload, base, select) {
    ...
    const c = el('div', { class: 'proj-file-card' + (select ? ' select-mode' : ''), title: it.name }, el('div', { class: 'proj-file-card-ic' }, fileThumb(id, it, rel, base)), el('div', { class: 'proj-file-card-nm', text: it.name }), el('div', { class: 'proj-file-card-sz', text: isDir ? '폴더' : fmtSize(it.size) }), it.mtime ? el('div', { class: 'proj-file-card-dt', text: fmtFileDate(it.mtime), title: fmtFileDateFull(it.mtime) }) : null);
    if (select) {
