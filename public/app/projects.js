...
async function renderProjectV2Board(view) {
    ...
    // 태스크 리스트와 동일한 3버킷 — 할 일(todo) / 진행 중(in_progress·레거시 active 흡수) / 완료(done).
    const todo = projects.filter((p) => p.status === 'todo');
    const inprog = projects.filter((p) => p.status !== 'done' && p.status !== 'todo');
    ...
    function repaintBulk() {
        ...
        const allBtn = el('button', { class: 'btn btn-ghost btn-sm', text: allOn ? '전체 해제' : '전체 선택',
            onclick: () => { if (allOn)
                ...
                deletable.forEach((p) => sel.ids.add(p.id)); repaint(); } });
        const delBtn = el('button', { class: 'btn btn-sm btn-danger', text: '선택 삭제', disabled: n === 0,
            onclick: () => bulkDelete(delBtn) });
        bulkBar.hidden = false;
        bulkBar.replaceChildren(el('span', { class: 'bulk-bar-count', text: n ? n + '개 선택됨' : '삭제할 프로젝트를 고르세요' }), el('div', { class: 'bulk-bar-actions' }, allBtn, delBtn));
    }
    function repaint() {
        // 헤더 우측 버튼 제거 — '새 프로젝트'(파란)·'선택'(일괄삭제) 없앰. 프로젝트 추가는 목록 하단 '＋ 프로젝트' 인라인 행
        //  (이름 입력 후 Enter → '새 프로젝트' 팝업 프리필), 삭제는 각 행 ⋯ 메뉴에서. 선택(일괄) 모드는 비활성.
        headActions.replaceChildren();
        sectionsBox.replaceChildren(pjvProjectListCard(todo, inprog, done, reload, null, canDelete, board.fields || [], board.anchorId, meId));
        repaintBulk();
    }
    async function bulkDelete(btn) {
        ...
        if (!ids.length)
            return;
        if (!confirm(ids.length + '개 프로젝트를 삭제할까요?\n\n각 프로젝트와 그 작업(태스크·하위)이 함께 사라집니다(되돌릴 수 없음). 연결된 지식은 보존됩니다.'))
            return;
        btn.disabled = true;
        // 병렬 삭제 — 일부 실패해도 나머지는 진행(성공/실패 건수 보고). 서버가 비소유분은 403.
        const results = await Promise.allSettled(ids.map((pid) => api('/api/ui/v6/projects/' + pid + '/delete', { method: 'POST' })));
    ...
    view.replaceChildren(head, projectSubBar('dashboard'), el('div', { class: 'card-head', style: 'margin: 6px 0 14px' }, el('div', {}, el('span', { class: 'eyebrow', text: '내 프로젝트' }), el('p', { class: 'sub', style: 'margin: 5px 0 0', text: '내가 속한 프로젝트 — 진행 중과 완료.' })), headActions), bulkBar, sectionsBox, el('div', { class: 'card-head', style: 'margin: 24px 0 14px' }, el('div', {}, el('span', { class: 'eyebrow', text: '회사 전체' }), el('p', { class: 'sub', style: 'margin: 5px 0 0', text: '회사에서 지금 진행 중인 모든 작업.' }))), companyTimelineSection());
    repaint();
}
...
function pjvProjTaskRow(projectId, t, members, reload, depth, boardFields) {
    ...
    const caret = subs.length
        ...
        : el('span', { class: 'pjv-trow-caret empty', 'aria-hidden': 'true' });
    const ttags = t.tags || [];
    const tagsEl = ttags.length ? el('span', { class: 'pjv-trow-tags' }, ...ttags.slice(0, 2).map((tg) => el('span', { class: 'pjv-trow-tag', style: '--tag:' + (tg.color || PJV_TAG_NONE), title: tg.name, text: tg.name })), ttags.length > 2 ? el('span', { class: 'pjv-trow-tag-more', text: '+' + (ttags.length - 2) }) : null) : null;
    const titleCell = el('div', { class: 'pjv-trow-title-cell' }, caret, pjvStatusControl(t, reload), el('span', { class: 'pjv-trow-title' + (isDone ? ' done' : ''), text: t.name || t.title || '(제목 없음)' }), subs.length ? el('span', { class: 'pjv-trow-subcount', title: subs.length + '개 하위', text: String(subs.length) }) : null, tagsEl);
    ...
    if (subs.length && depth < 4) {
        ...
        caret.onclick = () => { open = !open; caret.textContent = open ? '▾' : '▸'; caret.setAttribute('aria-expanded', open ? 'true' : 'false'); subBox.hidden = !open; };
    }
...
function pjvProjGroup(label, statusKey, list, reload, select, canDelete, withCols, fields, anchorId, meId, taskCtx, sepTasks) {
    fields = fields || [];
    sepTasks = sepTasks || [];
    ...
    for (const s of sepTasks)
        body.append(pjvProjTaskRow(s.projId, s.task, s.members, reload, 0, fields));
    // 클릭업식 인라인 추가행 — 각 그룹(완료 제외) 맨 아래. 빈 그룹에선 이 행이 '시작하기' CTA. 선택(일괄삭제) 모드에선 숨김.
    if (!select && statusKey !== 'done')
...
function pjvProjAddRow(statusKey, reload, body, countEl, fields, select, canDelete, anchorId, meId, taskCtx) {
    fields = fields || [];
    ...
    // 제목 칸 — 실제 프로젝트 행과 동일 구조(캐럿 자리 + 그룹 상태 동그라미 + 입력)로 그려 픽셀 정렬 일치.
    const buildTitleCell = () => {
        const meta = pjvProjStatusMeta(statusKey);
        return el('div', { class: 'pjv-trow-title-cell' }, el('span', { class: 'pjv-trow-caret empty', 'aria-hidden': 'true' }), el('span', { class: 'pjv-status-dot ' + meta.cls, 'aria-hidden': 'true' }, meta.glyph ? el('span', { class: 'pjv-status-glyph', text: meta.glyph }) : null), input);
    };
...
function openProjectV2Form(reload, prefill) {
    ...
    const back = overlayBox('새 프로젝트', el('div', { class: 'field' }, el('label', { class: 'field-label', text: '이름' }), nameIn), el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '설명 (선택)' }), descIn), el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '함께하는 팀원' }), picker.box), el('div', { class: 'ov-actions' }, saveBtn, cancelBtn));
...
function pjvProjTagsField(p, reload) {
    ...
    const render = () => {
        ...
        for (const tg of tags) {
            ...
            x.onclick = async (e) => { e.stopPropagation(); if (await save({ tag_id: tg.id, remove: true }))
                render(); };
            wrap.append(chip);
        }
    ...
    const openPicker = async (anchor) => {
        ...
        let all = [];
        try {
            all = await api('/api/ui/v6/tags').then((d) => d.tags || []);
        }
        catch (_) { /* graceful */ }
        const renderList = () => {
            ...
            for (const t of all.filter((t) => !qy || (t.name || '').toLowerCase().includes(qy))) {
                ...
                const item = el('button', { class: 'pjv-menu-item' + (on ? ' sel' : ''), type: 'button' }, el('span', { class: 'pjv-tm-tagdot', style: 'background:' + (t.color || PJV_TAG_NONE) }), el('span', { text: t.name }), on ? el('span', { class: 'pjv-asg-check', text: '✓' }) : null);
                item.onclick = async (e) => { e.stopPropagation(); if (await save(on ? { tag_id: t.id, remove: true } : { tag_id: t.id })) {
                    render();
                    renderList();
                } };
                list.append(item);
            ...
            if (qv && !all.some((t) => (t.name || '').toLowerCase() === qv.toLowerCase())) {
                ...
                create.onclick = async (e) => { e.stopPropagation(); if (await save({ name: qv })) {
                    try {
                        all = await api('/api/ui/v6/tags').then((x) => x.tags || []);
                    }
                    catch (_) { /* noop */ }
                    input.value = '';
                    render();
            ...
            if (!list.children.length)
                list.append(el('div', { class: 'pjv-menu-empty', text: '이름을 입력해 새 태그를 만들어보세요.' }));
        };
...
function pjvProjMetaPanel(p, members, reload) {
    ...
    return el('div', { class: 'pjv-tm-fields pjv-proj-meta' }, row('◎', '상태', pjvProjStatusPill(p, reload)), row('👤', '담당자', pjvAssigneeControl(p, members, (patch) => projSaveQuiet(p.id, patch))), row('🗓', '기간', pjvProjDatesField(p, reload)), row('⚑', '우선순위', pjvPriorityControl(p, (patch) => projPatch(p.id, patch, reload))), row('⏱', '시간 추적', pjvProjTimeField(p, reload)), row('🏷', '태그', pjvProjTagsField(p, reload)));
}
async function renderProjectV2Detail(view, idStr) {
    ...
    head.append(el('div', { class: 'proj-detail-titlebar' }, el('div', { class: 'proj-detail-titlebox' }, titleEl, el('span', { class: 'pill-state' + (isDone ? '' : ' confirmed'), text: PJV_STATUS_LABEL[p.status] || p.status })), el('div', { class: 'proj-detail-actions' }, localWorkBtn, settingsBtn)));
    ...
    if (members.length) {
        for (const m of members)
            teamRow.append(el('span', { class: 'proj-team-chip' }, el('span', { class: 'proj-team-ava', style: 'background:' + avatarColor(m.member_id), text: initials(m.display_name || m.member_id) }), el('span', { text: m.display_name || (m.member_id + (m.role ? ' · ' + m.role : '')) })));
    }
    else {
    ...
    view.replaceChildren(head, pjvTasksSection(id, p.tasks || [], members, reload, p.fields || []), projectFolderSection(id, V6_BASE), projectTerminalSection(id, members, meId, V6_BASE, p.name), projectTimelineSection(id, members, V6_BASE));
...
function openProjectSettings(id, p, reload, meId, base) {
    ...
    back.querySelector('.proj-settings').append(projectStatusBlock(id, p, closeAndReload), projectMembersBlock(id, p, closeAndReload, B), projectRulesBlock(id), projectRefsBlock(id, B), projectKnowledgeBlock(id, p.knowledge || { required: [], produced: [] }), projectDangerBlock(id, p, meId, back));
}
...
function openLocalWorkModal(id, p) {
    ...
    wtChk.checked = true;
    const wtBlock = el('section', { class: 'ps-block' }, el('h3', { class: 'ps-block-title', text: '코드 작업 방식' }), el('label', { style: 'display:flex;gap:8px;align-items:center;cursor:pointer' }, wtChk, el('span', { text: '워크트리 생성 — 프로젝트 전용 브랜치로 격리(같은 레포 동시 작업 권장)' })));
    ...
    const syncDeps = () => {
        ...
        if (has && !repoPathInp.value.trim())
            repoPathInp.value = '~/lively/repos/' + repoSel.value;
    };
    ...
    (async () => {
        try {
            ...
            if (r && r.repos && r.repos.length === 1)
                repoSel.value = r.repos[0];
        }
    ...
    genBtn.onclick = async () => {
        ...
        try {
            ...
            toast('가이드 생성 실패 — ' + e.message, true);
        }
        genBtn.disabled = false;
    };
    form.append(el('p', { class: 'ps-block-hint', text: '이 프로젝트를 내 컴퓨터에서 작업하기 위한 설정이에요. 아래를 채우고 ‘가이드 만들기’를 누르면, 내 PC 터미널에서 그대로 따라 할 명령을 단계별로 만들어 드립니다.' }), block('AI 코딩 에이전트', '내 PC에서 사용할 하네스를 고르세요.', harnessSel), block('사용할 레포', '이 프로젝트가 작업할 코드 레포. 한 레포를 여러 프로젝트가 공유합니다.', repoSel), repoPathBlock, wtBlock, branchBlock, el('div', { class: 'ps-rules-actions' }, genBtn), guideWrap);
}
...
function renderLocalWorkGuide(wrap, g) {
    ...
    const steps = (g.steps || []).map((s) => {
        ...
        return el('section', { class: 'ps-block' }, el('h3', { class: 'ps-block-title', text: s.title + (s.optional ? '  (선택)' : '') }), s.desc ? el('p', { class: 'ps-block-hint', text: s.desc }) : null, cmdText ? el('div', { style: 'display:flex;gap:8px;align-items:flex-start' }, el('pre', { style: 'flex:1;margin:0;padding:8px 10px;background:rgba(127,127,127,.1);border-radius:6px;overflow:auto;font-family:ui-monospace,Menlo,monospace;font-size:12px;white-space:pre;line-height:1.5' }, el('code', { text: cmdText })), copyBtn(cmdText)) : null);
    });
    wrap.replaceChildren(el('div', { style: 'margin-top:14px;border-top:1px solid rgba(127,127,127,.18);padding-top:12px' }, el('h3', { class: 'ps-block-title', text: '내 PC에서 따라 하기' }), el('p', { class: 'ps-block-hint', text: '아래 명령을 내 컴퓨터 터미널에서 순서대로 실행하세요. 공유 폴더(' + g.sharedBox + ')만 박스와 동기화되고, 코드 레포는 각자 PC에서 git 으로 관리됩니다.' })), ...steps, el('p', { class: 'ps-block-hint', text: '※ ‘' + g.boxHost + '’, ‘' + g.repoRemote + '’ 같은 자리표시자는 환경 설정이 연결되면 자동으로 채워집니다.' }));
}
...
function projectRulesBlock(id) {
    ...
    (async () => {
        ...
        try {
            ...
            ta.value = splitClaudeMd(res.ok ? await res.text() : '').manual;
        }
        catch (_) {
            ta.value = '';
        }
    ...
    return el('section', { class: 'ps-block' }, el('h3', { class: 'ps-block-title', text: '프로젝트 규칙' }), el('p', { class: 'ps-block-hint', text: '이 프로젝트에서 터미널 세션을 열면, 여기 적은 규칙이 그 AI(Claude)에게 자동으로 주입됩니다. (프로젝트 폴더의 CLAUDE.md 로 저장)' }), ta, el('div', { class: 'ps-rules-actions' }, saveBtn, status));
}
...
function projectRefsBlock(id, base) {
    ...
    async function fetchFiles() {
        try {
            const d = await api(listsUrl);
            return ((d && d.items) || []).filter((x) => x.type === 'file');
        }
        catch (_) {
            return [];
        } // 폴더 없음 = 아직 파일 없음
    ...
    function paint(files) {
        if (!files.length) {
            listEl.replaceChildren(el('div', { class: 'pjv-kn-empty', text: '아직 참고 파일이 없어요. 올리면 매 터미널 세션에서 AI가 작업 전 반드시 읽습니다.' }));
            return;
        }
        listEl.replaceChildren(...files.map((f) => el('div', { class: 'ps-refs-row' }, el('span', { class: 'ps-refs-ic' }, fileIconSvg(f.name, false)), el('span', { class: 'ps-refs-nm', text: f.name, title: f.name }), el('span', { class: 'ps-refs-sz', text: fmtSize(f.size) }), el('button', { class: 'proj-file-iconbtn danger', type: 'button', title: '삭제', text: '✕', onclick: () => removeRef(f.name) }))));
    }
    ...
    async function sync() {
        ...
        let cur = '';
        try {
            const r = await fetch(claudeUrl, { headers: token ? { Authorization: 'Bearer ' + token } : {} });
            cur = r.ok ? await r.text() : '';
        }
        catch (_) { /* */ }
    ...
    fileInput.onchange = async () => {
        ...
        try {
            for (const f of files)
                await authUpload(refPath(f.name), f);
            await sync();
            status.textContent = '올림 · 다음 세션부터 적용';
            toast('참고 파일을 추가했습니다');
        }
        catch (e) {
            status.textContent = '';
    ...
    async function removeRef(name) {
        if (!confirm('참고 파일 ‘' + name + '’을(를) 삭제할까요?'))
            return;
        try {
    ...
    reload();
    return el('section', { class: 'ps-block' }, el('h3', { class: 'ps-block-title', text: '참고 파일' }), el('p', { class: 'ps-block-hint', text: '여기 올린 파일은 이 프로젝트에서 터미널 세션을 열 때마다 AI가 작업 전 반드시 읽도록 강제됩니다. (프로젝트 폴더의 참고자료/ 에 저장 · CLAUDE.md 에 자동 등록)' }), listEl, fileInput, el('div', { class: 'ps-rules-actions' }, uploadBtn, status));
}
...
function projectKnowledgeBlock(id, knowledge) {
    ...
    function paintList(boxEl, list, emptyText, removable) {
        if (!list.length) {
            boxEl.replaceChildren(el('div', { class: 'pjv-kn-empty', text: emptyText }));
            return;
        }
        boxEl.replaceChildren(...list.map((k) => {
            const name = knName(k);
            const row = el('div', { class: 'row pjv-kn-row ps-kn-row' }, el('a', { class: 'row-title', href: '#/k/' + encodeURIComponent(name), text: k.title || name }), el('div', { class: 'row-meta' }, k.injection ? knInjectChip(k.injection) : null, k.provenance ? el('span', {}, ' ', knProvChip(k.provenance)) : null, k.lifecycle ? el('span', {}, '  ', lifecycleDot(k.lifecycle)) : null));
            if (removable)
                row.append(el('button', { class: 'proj-file-iconbtn danger', type: 'button', title: '연결 해제', text: '✕',
                    onclick: async (ev) => {
                        ev.preventDefault();
                        try {
                            await api('/api/ui/v6/projects/' + id + '/knowledge', { method: 'POST', body: JSON.stringify({ name, relation: 'required', unlink: true }) });
                            toast('연결을 해제했습니다');
                            refresh();
                        }
                        catch (e) {
...
function pjvAddRow(projectId, status, members, reload, body, countEl, fields) {
    ...
    const buildTitleCell = () => {
        const tc = el('div', { class: 'pjv-trow-title-cell' }, el('span', { class: 'pjv-trow-caret empty', 'aria-hidden': 'true' }), statusDotPlaceholder(indentParent ? 'todo' : status), input);
        if (indentParent)
...
function pjvTaskRow(projectId, t, members, reload, depth, fields) {
    ...
    const ttags = t.tags || [];
    const tagsEl = ttags.length ? el('span', { class: 'pjv-trow-tags' }, ...ttags.slice(0, 2).map((tg) => el('span', { class: 'pjv-trow-tag', style: '--tag:' + (tg.color || PJV_TAG_NONE), title: tg.name, text: tg.name })), ttags.length > 2 ? el('span', { class: 'pjv-trow-tag-more', text: '+' + (ttags.length - 2) }) : null) : null;
    const titleCell = el('div', { class: 'pjv-trow-title-cell' }, pjvRowCheck('task', t, { reload, projectId, members }), caret, pjvStatusControl(t, reload), el('span', { class: 'pjv-trow-title' + (isDone ? ' done' : ''), text: t.name || t.title || '(제목 없음)' }), subs.length ? el('span', { class: 'pjv-trow-subcount', title: subs.length + '개 하위', text: String(subs.length) }) : null, tagsEl);
    if (depth)
        titleCell.style.paddingLeft = (depth * 22) + 'px';
...
function projectTerminalSection(id, members, meId, base, projectName) {
    ...
    function editStatus(m) {
        ...
        const saveBtn = el('button', { class: 'btn btn-primary', text: '저장' });
        const back = overlayBox('내 상태 메시지', el('p', { class: 'admin-hint', text: '프로필 밑에 보이는 ‘현재 상태’예요 — 팀원에게 지금 무엇을 하는지 공유됩니다.' }), el('div', { class: 'field' }, input), el('div', { class: 'ov-actions' }, saveBtn, el('button', { class: 'btn btn-ghost', text: '취소', onclick: () => back.remove() })));
        setTimeout(() => input.focus(), 0);
        const go = async () => {
            saveBtn.disabled = true;
            try {
...
function projectTimelineSection(id, members, base) {
    ...
    paintChips();
    card.append(el('div', { class: 'card-head' }, el('h3', { text: '작업 타임라인' })), el('p', { class: 'proj-tl-note' }, el('span', { class: 'proj-tl-note-ic', text: 'ⓘ' }), el('span', {}, '여기엔 ', el('b', { text: 'AI와 함께 남긴 작업' }), '이 자동으로 모여요 (AI 밖에서 진행한 모든 작업은 빠질 수 있어요). ', el('b', { text: '확실하게 진행이 된 일을 위주로' }), ' 회사 업무 진행의 큰 맥락을 확인하는 용도로 사용해주세요.')), chipsBar, body);
    load();
    return card;
    async function load() {
        body.replaceChildren(skeletonRows(3));
...
export { PJV_PRIORITY, PJV_PRIORITY_ORDER, PJV_STATUS_ORDER, PJV_TASK_STATUS, authDownload, authUpload, avatarColor, debounce, fileIconSvg, fmtDateTime, fmtSize, initials, openFileViewer, pjvAssigneeControl, pjvAssignees, pjvAssigneeWrite, pjvCheckMini, pjvDueControl, pjvFmtDate, pjvIsOverdue, pjvPatchTask, pjvPopover, pjvPriorityControl, pjvSaveTask, pjvStatusMeta, renderProjectV2Detail, renderProjectsV2, };
