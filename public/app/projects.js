        ...
        results.replaceChildren(...cand.map((m) => {
            ...
            return el('div', { class: 'proj-mp-row' + (on ? ' on' : ''), role: 'button', 'aria-pressed': on ? 'true' : 'false',
                onclick: () => { if (on)
                    ...
                    selected.add(m.id); paintResults(); searchIn.focus(); fire(); } }, el('span', { class: 'proj-mp-ava', style: 'background:' + avatarColor(m.id), text: initials(m.display_name || m.id) }), el('span', { class: 'proj-mp-name', text: m.display_name || m.id }), el('span', { class: 'proj-mp-check' + (on ? ' on' : ''), 'aria-hidden': 'true', text: on ? '✓' : '' }));
        }));
    }
