...
async function openProjectSessionForm(id, reload, base, projectName) {
    ...
    const wtCb = el('input', { type: 'checkbox' });
    wtCb.checked = true;
    ...
    const repoDetail = el('div', { style: 'display:none;margin-top:8px;padding-left:8px;border-left:2px solid rgba(127,127,127,.2)' }, el('div', { class: 'field' }, el('label', { class: 'field-label', text: '박스 레포 경로 (선택)' }), repoPathIn), el('label', { class: 'proj-sess-auto', style: 'margin-top:8px' }, wtCb, el('span', { text: ' 워크트리 생성 — 전용 브랜치로 격리하고 그 폴더에서 세션 (끄면 프로젝트 폴더에서)' })), branchRow);
    ...
    saveBtn.onclick = async () => {
        ...
        try {
            ...
            if (repo) {
                ...
                if (prov && prov.worktree && prov.subpath)
                    subpath = prov.subpath; // 워크트리는 프로젝트 폴더 안 → 세션 cwd 로
            }
