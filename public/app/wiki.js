    ...
    async function refetch() {
        ...
        try {
            ...
            listBox.replaceChildren(...entries.map((s) => {
                const row = el('div', { class: 'wk-row', role: 'button', tabindex: '0' }, el('span', { class: 'wk-tick' + (s.provenance === 'observed' ? ' mirror' : '') }), el('span', { class: 'wk-row-title', text: s.title || ('자료 #' + s.id) }), el('span', { class: 'wk-row-meta' }, el('span', { class: 'wk-row-m', text: SOURCE_KIND_LABEL[s.kind] || s.kind }), el('span', { class: 'wk-row-m', text: relTime(s.occurred_at || s.updated_at) })));
