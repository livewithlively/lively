...
function openRepoForm(repo, reload) {
    ...
    checkBtn.onclick = async () => {
        ...
        try {
            ...
            checkNote.replaceChildren(el('span', { style: 'color:var(--danger)', text: '✗ 확인 실패 — ' + e.message }));
        }
    ...
    const form = el('div', { class: 'proj-settings' }, isNew ? el('div', { class: 'conn-pick-row', style: 'margin-bottom:10px' }, el('span', { class: 'admin-hint', style: 'margin:0; flex:1',
        text: '등록된 git 자격으로 레포 목록을 불러와 고를 수 있어요 — 이름·주소·기본 브랜치가 함께 채워집니다.' }), pickBtn) : null, psBlock('레포 이름', isNew ? '실제 git 레포 이름 — code_unit 이 이 이름으로 매핑됩니다. (경로 컴포넌트라 슬래시 불가 — GitLab 서브그룹은 마지막 조각만 들어갑니다)' : '이름을 바꿔도 매핑·도메인은 보존됩니다.', nameInp), psBlock('git 주소 (clone URL)', '도메인맵 스캔과 로컬 작업 클론이 이 주소를 씁니다. 비우면 git 미연결. HTTPS 가 막힌 셀프호스팅(GitLab 등)은 SSH 형(git@호스트:그룹/레포.git)으로 넣으세요.', el('div', {}, el('div', { class: 'conn-pick-row' }, urlInp, checkBtn), checkNote)), psBlock('기본 브랜치', '비우면 main. [연결 확인]으로 원격의 실제 기본 브랜치를 확인할 수 있어요.', branchInp), el('div', { class: 'ps-rules-actions' }, saveBtn));
