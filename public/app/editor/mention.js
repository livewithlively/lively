...
export function createMention(ctx) {
    const { st } = ctx;
    ...
    function paintMention() {
        ...
        if (st.mention.sel >= items.length)
            st.mention.sel = Math.max(0, items.length - 1);
        st.mention.menu.replaceChildren(el('div', { class: 'be-slash-head', text: '페이지 링크' }), ...(!items.length ? [el('div', { class: 'be-slash-empty', text: st.mention.query ? '결과 없음' : '검색어를 입력하세요' })]
            : items.map((it, idx) => {
                const row = el('button', { class: 'be-slash-item' + (idx === st.mention.sel ? ' on' : ''), type: 'button' }, el('span', { class: 'be-slash-ic', text: it.icon || '📄' }), el('span', { class: 'be-slash-label', text: it.title || it.name }));
                row.addEventListener('mousedown', (e) => e.preventDefault());
                row.onclick = () => applyMention(it);
                return row;
            })));
        const r = caretRange();
        let rect = r ? r.getBoundingClientRect() : st.mention.block.getBoundingClientRect();
        if (!rect.width && !rect.height && !rect.top)
            rect = st.mention.block.getBoundingClientRect();
    ...
    function scheduleMentionSearch() {
        ...
        st.mention.timer = setTimeout(async () => {
            ...
            try {
                ...
                if (!st.mention)
                    return;
                st.mention.items = ((r && r.entries) || []).filter((e) => !String(e.name || '').startsWith('category-home-'));
                st.mention.sel = 0;
                paintMention();
            }
    ...
    function syncMention(target) {
        ...
        if (st.mention.query !== m[1] || !st.mention.items.length) {
            st.mention.query = m[1];
