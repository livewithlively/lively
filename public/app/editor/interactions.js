...
export function createInteractions(ctx) {
    ...
    const bselEdge = (last) => (bsel.length ? bsel[last ? bsel.length - 1 : 0] : null);
    function bselClear() {
        if (!bsel.length)
            return;
        for (const b of bsel)
            b.classList.remove('be-bsel');
        bsel = [];
    ...
    function bselSet(a, b) {
        ...
        for (const x of bsel)
            if (next.indexOf(x) < 0)
                x.classList.remove('be-bsel');
        for (const x of next)
            x.classList.add('be-bsel');
        bsel = next;
    ...
    root.addEventListener('mousedown', (e) => {
        ...
        if (e.target && e.target.closest && e.target.closest('.be-gutter')) {
            bselAnchor = null;
            bDragging = false;
            return;
        }
