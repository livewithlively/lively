...
export function createKeys(ctx) {
    ...
    const prevTextBlock = (block) => ctx.prevTextBlock(block);
    const nextTextBlock = (block) => ctx.nextTextBlock(block);
    ...
    const applySlash = (item) => ctx.applySlash(item);
    ...
    const applyMention = (it) => ctx.applyMention(it);
    ...
    root.addEventListener('keydown', (e) => {
        ...
        if (ctx.bselActive()) {
            ...
            if (e.metaKey || e.ctrlKey || e.altKey)
                return; // 복사·잘라내기·붙여넣기·⌘Z 는 각자 경로가 처리
            if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
                e.preventDefault();
                const f = ctx.bselEdge(false);
                ctx.bselClear();
                if (f)
                    focusBlock(f, true);
                return;
        ...
        if (st.mention) {
            if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                ...
                if (n) {
                    st.mention.sel = (st.mention.sel + (e.key === 'ArrowDown' ? 1 : n - 1)) % n;
                    paintMention();
                }
                return;
            }
        ...
        if (st.slash) {
            if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                ...
                if (n) {
                    st.slash.sel = (st.slash.sel + (e.key === 'ArrowDown' ? 1 : n - 1)) % n;
                    paintSlash();
                }
                return;
            }
        ...
        if (e.key === '/' && !st.composing && !st.slash) {
            setTimeout(() => {
                ...
                if (st.slash) {
                    st.slash.anchorNode = rr.startContainer;
                    st.slash.anchorOffset = rr.startOffset;
                }
            }, 0);
            return;
        }
