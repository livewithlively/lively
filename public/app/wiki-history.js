...
function khEnsureStyles() {
    rqEnsureStyles(); // .rq-diff/.rq-dl — diff 본체
    if (document.getElementById('kh-styles'))
        return;
...
function mdChunk(t, text) {
    const wrap = el('div', { class: 'kh-md' + (t === '+' ? ' add' : t === '-' ? ' del' : '') });
    if (t !== ' ')
        wrap.append(el('span', { class: 'kh-md-sig', 'aria-hidden': 'true', text: t }));
    wrap.append(renderMarkdown(text));
    return wrap;
}
function diffViewMd(before, after) {
...
export function openKnHistory(name, opts = {}) {
    khEnsureStyles();
    const body = el('div', { class: 'kh-panel' }, skeleton('이력을 불러오는 중'));
    const back = overlayBox('변경 이력', body);
    ...
    let viewMode = 'side'; // #1600 기본 = 전/후 나란히(사람이 가장 먼저 묻는 것이 '뭐가 달라졌나')
    ...
    async function load() {
        ...
        if (!rows.length)
            body.replaceChildren(bar, skeleton('이력을 불러오는 중'));
        try {
    ...
    function rowEl(r) {
        ...
        if (r.added)
            delta.append(el('span', { class: 'add', text: '+' + r.added }));
        if (r.added && r.removed)
            delta.append(document.createTextNode(' '));
        if (r.removed)
            delta.append(el('span', { class: 'del', text: '−' + r.removed }));
        ...
        const main = el('button', { class: 'kh-main', type: 'button',
            title: absTime(r.at) }, el('span', { class: 'kh-when', text: relTime(r.at) }), 
        // 무변경 update(에디터 자동저장 등)는 서버가 kind='meta' 로 준다 — '본문 수정'이라 쓰면 거짓말이 된다.
        el('span', { class: 'kh-op' + (r.kind === 'meta' ? ' meta' : ''),
            text: r.op === 'update' && r.kind === 'meta' ? '속성만 변경' : (OP_LABEL[r.op] || r.op) }), who, cat, delta, ver);
    ...
    function expandedView(r, d, exp) {
        ...
        const where = el('div', { class: 'kh-where' }, el('span', { text: (r.actor_display || r.actor || '알 수 없음') + '님이 ' + absTime(r.at) + '에 '
        ...
        if (bBody === null)
            out.push(el('p', { class: 'kh-exp-note', text: '이 시점에 문서가 처음 만들어졌습니다.' }));
        if (aBody === null)
            out.push(el('p', { class: 'kh-exp-note', text: '이 변경으로 문서가 삭제됐습니다.' }));
        // 서식 ↔ 원문 토글. 다시 그리는 건 이 행뿐이고 요청은 없다(전문은 _d 에 이미 있다).
        const seg = el('div', { class: 'kh-seg', role: 'group', 'aria-label': '본문 표시 방식' });
        const mk = (label, mode) => el('button', {
            class: viewMode === mode ? 'on' : '', type: 'button', text: label, 'aria-pressed': String(viewMode === mode),
            onclick: () => { if (viewMode === mode)
                return; viewMode = mode; exp.replaceChildren(...expandedView(r, d, exp)); },
        });
        ...
        if (viewMode === 'side') {
            const col = (label, body, cls) => {
                const b = el('div', { class: 'kh-side-b' });
                if (body === null)
                    b.append(el('div', { class: 'kh-side-empty', text: '이 시점엔 문서가 없었습니다.' }));
                else if (!body.trim())
                    b.append(el('div', { class: 'kh-side-empty', text: '(빈 문서)' }));
                else
                    b.append(renderMarkdown(body));
                return el('div', { class: 'kh-side-col ' + cls }, el('div', { class: 'kh-side-h', text: label }), b);
            };
            out.push(el('div', { class: 'kh-side' }, col('고치기 전', bBody, 'before'), col('고친 뒤', aBody, 'after')));
        }
        else {
        ...
        if (isCurrent) {
            ...
            acts.append(el('button', {
                ...
                onclick: async (ev) => {
                    ...
                    btn.disabled = true;
                    try {
                        await api('/api/ui/knowledge/' + encodeURIComponent(name) + '/revert', { method: 'POST', body: JSON.stringify({ audit_id: r.audit_id, to: 'after' }) });
