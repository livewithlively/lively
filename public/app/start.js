...
// #/start/harness — 로컬 하네스 ↔ 라이블리 하네스 한눈에 + 껐다 켰다 (#891 온보딩 C).
//  세션훅이 push 한 **머신별** 로컬 관측(me/harness)과 라이블리 배포(me_assets)를 나란히 + 중복 대조 + 토글.
//  ⚠ 라이블리 토글은 멤버 단위(me_asset_pref, 모든 머신), 로컬 토글은 그 머신 파일만(harness-local-pref).
const KIND_LABEL = { subagent: '서브에이전트', command: '커맨드', hook: '훅', skill: '스킬' };
function kindLabel(k) { return KIND_LABEL[k] || '스킬'; }
// 한 줄 = 이름 + 종류 + 상태뱃지 + (선택)토글 버튼.
function harnessRow(name, kind, tag, toggle, sub) {
    return el('div', { class: 'ob-done-row' }, el('span', { class: 'ob-done-check', text: '•' }), el('span', { class: 'ob-done-label' }, el('b', { text: name }), sub ? el('span', { class: 'ob-step-desc', text: ' ' + sub }) : null), el('span', { class: 'ob-done-tag', text: kindLabel(kind) }), tag ? el('span', { class: 'ob-step-badge ' + tag.cls, text: tag.text }) : null, toggle || null);
}
// 라이블리 자산 끄기/켜기 — 멤버 단위(me_asset_pref). 모든 머신에 반영.
async function toggleLively(targetKind, id, turnOn, view) {
    await api('/api/ui/me/asset-pref', { method: 'POST', body: JSON.stringify({ target_kind: targetKind, ref_id: id, state: turnOn }) });
    toast(turnOn ? '켰습니다' : '껐습니다 — 다음 세션부터 적용');
    await renderStartHarness(view);
}
// 로컬 파일 끄기/켜기 — 그 머신만(harness-local-pref). 다음 세션에 세션훅이 .disabled rename(비파괴).
async function toggleLocal(machineId, kind, id, disable, view) {
    await api('/api/ui/me/harness-local-pref', { method: 'POST', body: JSON.stringify({ machine_id: machineId, kind, id, disabled: disable }) });
    toast(disable ? '끄기로 표시 — 그 컴퓨터에서 다음 세션에 반영' : '켜기로 표시 — 다음 세션에 복원');
    await renderStartHarness(view);
}
function toggleBtn(text, onclick) {
    return el('button', { class: 'btn btn-ghost btn-sm', text, onclick });
}
export async function renderStartHarness(view) {
    const head = [
        docsEyebrow('start'),
        el('h1', { class: 'docs-title', text: '내 하네스 설정' }),
        el('p', { class: 'docs-lead', text: '라이블리가 배포하는 스킬·훅과, 내 컴퓨터에 깔린 로컬 하네스 자산을 한눈에 보고 껐다 켰다 합니다. 겹치는 건 조직이 함께 관리·개선하는 라이블리 쪽을 쓰시길 권합니다.' }),
    ];
    const slot = el('div', { class: 'guide-cards' });
    ...
    // ── 라이블리 배포 (멤버 단위, 모든 머신) ── effective=false 면 내가 끈 것.
    const livelyRow = (x, targetKind, sub) => harnessRow(x.label || x.id, x.kind || 'hook', x.effective ? null : { text: '꺼짐', cls: 'off' }, toggleBtn(x.effective ? '끄기' : '켜기', () => toggleLively(targetKind, x.id, !x.effective, view)), sub);
    const livelyRows = [
        ...d.lively.skills.map((a) => livelyRow(a, 'harness_asset', a.description ? String(a.description).slice(0, 56) : '')),
        ...d.lively.hooks.map((h) => livelyRow(h, 'org_hook')),
    ];
    const livelyCard = el('div', { class: 'ob-sec' }, el('div', { class: 'ob-sec-head' }, el('span', { class: 'ob-sec-title', text: '라이블리가 준 것' }), el('span', { class: 'ob-sec-count', text: String(livelyRows.length) })), el('p', { class: 'ob-step-desc', text: '조직이 관리하는 스킬·훅입니다. 여기서 끄면 내 모든 컴퓨터에 반영됩니다(다음 세션부터).' }), el('div', { class: 'ob-done-list' }, ...(livelyRows.length ? livelyRows : [el('p', { class: 'admin-hint', text: '아직 없습니다.' })])));
    // ── 내 컴퓨터들 (머신별) ── 세션훅이 아직 push 안 했으면 machines 빈 배열.
    ...
    const machineCards = machines.map((m) => {
        const rows = (m.assets || []).map((a) => {
            if (a.overlap === 'shadow')
                shadowTotal++;
            const tag = a.disabled ? { text: '꺼짐', cls: 'off' }
                : a.overlap === 'managed' ? { text: '라이블리가 설치', cls: 'muted' }
                    : a.overlap === 'shadow' ? { text: '라이블리 것을 가림', cls: 'warn' }
                        : { text: '내가 만든 것', cls: 'own' };
            // managed(라이블리가 깐 파일)는 로컬에서 끄지 않는다 — 그건 '라이블리가 준 것' 섹션에서 멤버 단위로.
            const canToggle = a.overlap !== 'managed';
            const btn = canToggle ? toggleBtn(a.disabled ? '켜기' : '끄기', () => toggleLocal(m.machine_id, a.kind, a.id, !a.disabled, view)) : null;
            return harnessRow(a.id, a.kind, tag, btn);
        });
        const title = m.host || '내 컴퓨터';
        const fresh = m.at ? `마지막 확인 ${new Date(m.at).toLocaleString('ko-KR')}` : '';
        return el('div', { class: 'ob-sec' }, el('div', { class: 'ob-sec-head' }, el('span', { class: 'ob-sec-title', text: title }), el('span', { class: 'ob-sec-count', text: String((m.assets || []).length) })), fresh ? el('p', { class: 'admin-hint', text: fresh }) : null, el('div', { class: 'ob-done-list' }, ...(rows.length ? rows : [el('p', { class: 'admin-hint', text: '내가 직접 만든 로컬 자산은 없습니다(라이블리가 준 것만).' })])));
    });
    const localWrap = machines.length
        ? machineCards
        : [el('div', { class: 'ob-sec' }, el('div', { class: 'ob-sec-head' }, el('span', { class: 'ob-sec-title', text: '내 컴퓨터에 있는 것' })), el('p', { class: 'admin-hint', text: '아직 내 컴퓨터의 하네스를 못 봤습니다. 내 컴퓨터에서 claude(또는 codex)를 한 번 켜면, 다음 세션에 자동으로 여기 나타납니다. 컴퓨터가 여러 대면 각각 따로 보입니다. (웹 [터미널] 세션은 회사 서버라 로컬이 안 보입니다.)' }))];
    // shadow(가림)가 있으면 라이블리 채택 권고 배너.
    const shadowBanner = shadowTotal
        ? el('div', { class: 'ob-banner amber' }, el('div', { class: 'ob-banner-title', text: `내 로컬 자산 ${shadowTotal}개가 라이블리 것과 이름이 같아 라이블리 배포를 가리고 있어요.` }), el('p', { class: 'ob-banner-sub', text: '같은 이름이면 조직이 함께 관리·개선하는 라이블리 쪽을 쓰시길 권합니다(위에서 로컬 것을 끄면 라이블리 것이 살아납니다). 내 것이 더 낫다면 AI 에게 "이 스킬 라이블리에 올리게 정리해줘" 라고 해서 관리자에게 전달하세요.' }))
        : null;
    slot.replaceChildren(...[shadowBanner, livelyCard, ...localWrap].filter(Boolean));
}
