...
function currentWorkspace() {
    try {
        return (localStorage.getItem(WORKSPACE_KEY) || '').trim().toLowerCase();
    }
    catch (_) {
        return '';
    }
}
function setCurrentWorkspace(slug) {
    const s = (slug || '').trim().toLowerCase();
    try {
        if (!s || s === 'primary')
            localStorage.removeItem(WORKSPACE_KEY);
        else
            localStorage.setItem(WORKSPACE_KEY, s);
    }
    catch (_) { /* 프라이빗 모드 등 — 선택이 세션 한정이 될 뿐 */ }
}
...
async function api(path, opts = {}) {
    ...
    if (res.status === 401) {
        ...
        if (onUnauthorized)
            onUnauthorized(); // 배선돼 있으면 앱이 state 를 비우고 로그인 게이트를 띄운다
    ...
    if (res.status === 404 && ws && data && typeof data.message === 'string' && data.message.indexOf('워크스페이스') === 0) {
        setCurrentWorkspace('');
        location.reload();
        const eGone = new Error(data.message);
        eGone.status = 404;
        throw eGone;
    }
    if (!res.ok) {
...
export { TOKEN_KEY, api, apiUrl, appUrl, setUnauthorizedHandler, currentWorkspace, setCurrentWorkspace, };
