...
function visAxisOn(axis) {
    const ax = state.me && state.me.vis_axes;
    if (!ax)
        return true;
...
function uiModeOverride() {
    try {
        const o = localStorage.getItem(UI_MODE_KEY);
        return o === 'classic' || o === 'v2' ? o : null;
    }
    catch (_) {
        return null;
    }
}
export { state, visAxisOn, navOn, uiMode, setUiModeOverride, uiModeOverride, };
