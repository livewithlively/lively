...
export const isMdTableSep = (l) => /^\s*\|?[\s:|-]*-[\s:|-]*\|?\s*$/.test(String(l)) && String(l).indexOf('-') >= 0;
export function mdTableSplitRow(l) {
    let t = String(l).trim();
    if (t.startsWith('|'))
        t = t.slice(1);
...
export function parseTableMd(md) {
    ...
    if (body.length < 2)
        return null;
    if (body[0].indexOf('|') < 0 || !isMdTableSep(body[1]))
        return null;
    ...
    const fit = (r) => Array.from({ length: cols }, (_, i) => (r[i] == null ? '' : r[i]));
    return {
        head: fit(head),
...
export function tableToMd(t) {
    const cell = (s) => String(s == null ? '' : s).replace(/\r?\n/g, ' ').replace(/\|/g, '\\|').trim();
    const row = (r) => '| ' + r.map(cell).map((c) => c || ' ').join(' | ') + ' |';
    const out = [row(t.head), '| ' + t.align.map(alignBar).join(' | ') + ' |'];
    for (const r of t.rows)
        out.push(row(r));
    return out.join('\n');
}
export function isTableMd(md) { return !!parseTableMd(md); }
