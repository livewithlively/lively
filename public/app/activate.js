...
export async function renderActivate(view) {
    ...
    const codeInput = el('input', {
        type: 'text', class: 'term-input', autocomplete: 'off', spellcheck: 'false',
...
async function drawLookup(slot, raw) {
    ...
    const pwInput = el('input', { type: 'password', class: 'term-input', placeholder: '비밀번호 재확인', style: 'margin-top:6px;display:none' });
    const cpBlock = canCp ? el('div', { style: 'margin:8px 0' }, el('label', { class: 'admin-check', style: 'cursor:pointer' }, cpChk, el('span', { text: ' 관리 권한(admin/runtime) 포함 — 이 CLI 세션이 관리탭 기능(구성원·토큰·훅·DB소스)을 MCP로 다룹니다.' })), pwInput) : null;
    if (canCp)
