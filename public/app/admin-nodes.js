...
function versionBadge(n) {
    if (!n.online)
        return null; // 꺼져 있으면 버전 이야기는 소음이다
    if (n.agent_latest === true)
        return null; // 최신이면 조용한 게 맞다
...
function harnessLine(n) {
    ...
    if (!hs.length)
        return null;
    return el('span', { class: 'wikicat-should' }, el('span', { class: 'wikicat-should-label', text: '쓸 수 있는 AI' }), hs.join(' · '));
}
// 노드 한 행. acts 는 호출부가 정한다 — 같은 노드라도 '내 화면'과 '관리자 화면'에서 할 수 있는 일이 다르다.
function nodeRow(n, ownerLabel, acts) {
    const main = el('div', { class: 'wikicat-row-main' }, el('span', { class: 'wikicat-name', text: n.name || n.id }), el('span', { class: 'wikicat-key mono', text: n.id }), n.shared ? el('span', { class: 'dm-tag', text: '공유' }) : null, ownerLabel ? el('span', { class: 'wikicat-should' }, el('span', { class: 'wikicat-should-label', text: '연결한 사람' }), ownerLabel) : null, harnessLine(n));
...
function nodeGroup(title, hint, nodes, rowFn, empty) {
    ...
    if (!nodes.length)
        rows.append(el('div', { class: 'wikicat-empty' }, empty));
    else
...
const ownerActs = (act, n) => el('div', { class: 'wikicat-row-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: n.enabled === false ? '연결 허용' : '연결 차단',
    title: n.enabled === false ? '이 컴퓨터가 다시 연결되도록 허용합니다.' : '이 컴퓨터의 연결을 막습니다(작업은 못 들어오고, 등록은 남습니다).',
    onclick: () => act.setEnabled(n, n.enabled === false) }), el('button', { class: 'btn btn-ghost btn-sm', text: '접속 열쇠 재발급', onclick: () => act.rotate(n) }), el('button', { class: 'btn btn-ghost btn-sm', text: '지우기', onclick: () => act.remove(n) }));
// 사용가이드로 보내는 한 줄 — 설명은 문서가 갖고, 화면은 링크만 준다.
const guideLine = (text) => el('p', { class: 'admin-hint' }, document.createTextNode(text + ' '), el('a', { href: GUIDE, text: '사용가이드에서 연결하는 법 보기 →' }));
// ─────────────────────────────────────────────────────────────────────────────
...
export async function myNodesPanel(detail, data) {
    ...
    detail.replaceChildren(...[
        sectionHead('내 컴퓨터 · ' + mine.length + '대', '내 노트북이나 서버를 라이블리에 연결하면, 웹에서 그 컴퓨터의 AI 세션을 열고 오래 걸리는 작업을 맡길 수 있습니다.'),
        el('div', { class: 'card' }, ...[
            el('div', { class: 'wikicat' }, ...[
                nodeGroup('내가 연결한 컴퓨터', '기본적으로 나만 쓸 수 있습니다.', mine, (n) => nodeRow(n, null, ownerActs(act, n)), el('span', {}, document.createTextNode('아직 연결한 컴퓨터가 없습니다 — '), el('a', { href: GUIDE, text: '연결하는 법을 보세요' }), document.createTextNode('.'))),
                nodeGroup('함께 쓸 수 있는 공유 컴퓨터', '관리자가 조직 전체에 열어 둔 컴퓨터입니다. 여기에도 작업을 맡길 수 있습니다.', sharedOthers, (n) => nodeRow(n, null, null), '아직 공유로 지정된 컴퓨터가 없습니다.'),
            ]),
            ...
            sharedMine ? el('p', { class: 'admin-hint' }, document.createTextNode('내 목록 중 ' + sharedMine + '대가 공유 컴퓨터입니다 — 구성원 누구나 그 컴퓨터를 씁니다. 공유 해제는 '), el('a', { href: '#/system/nodes', text: '설정 ▸ 컴퓨터(노드)' }), document.createTextNode(' 에서 합니다(관리자). 연결 차단·지우기는 여기서 직접 할 수 있습니다.')) : null,
...
function openSharedNodeForm(reload) {
    ...
    const body = el('div', {}, el('p', { class: 'admin-hint' }, '여럿이 함께 쓸 컴퓨터의 자리를 먼저 만듭니다. 등록한 뒤 그 컴퓨터에서 연결 명령을 실행하면 목록에 나타납니다. '
        + '구성원 누구나 여기에 AI 세션을 열고 작업을 맡길 수 있게 되니, 개인 노트북이 아니라 공용 서버에만 쓰세요.'), field('아이디', idInp), el('p', { class: 'admin-hint', style: 'margin:-8px 0 12px' }, '영문 소문자·숫자·하이픈. 연결 명령에 그대로 들어갑니다.'), field('이름 (선택)', nameInp), msg, el('div', { class: 'admin-actions' }, save));
    ...
    save.onclick = async () => {
        ...
        try {
            ...
            body.replaceChildren(el('p', { class: 'install-ok', text: '✓ 자리를 만들었습니다. 이제 그 컴퓨터에서 연결하세요.' }), el('p', { class: 'admin-hint' }, '공유할 컴퓨터에서 관리자 계정으로 로그인한 뒤 아래를 실행합니다.'), el('pre', { class: 'admin-preview', text: cmd }), el('div', { class: 'admin-actions' }, el('button', { class: 'btn btn-primary', text: '명령 복사',
            ...
            el('button', { class: 'btn btn-ghost', text: '완료 — 목록으로', onclick: () => { back.remove(); reload(); } })), el('p', { class: 'admin-hint' }, document.createTextNode('라이블리 CLI 가 그 컴퓨터에 아직 없다면 '), el('a', { href: '#/start', text: '시작하기' }), document.createTextNode('의 한 줄 설치를 먼저 실행하세요. 연결되면 이 목록에 나타납니다.')));
...
export async function orgNodesPanel(detail, data) {
    ...
    let nodes;
    try {
        const r = await api('/api/ui/nodes');
        nodes = (r && r.nodes) || [];
    }
    catch (e) {
        detail.replaceChildren(el('div', { class: 'card' }, errorNote(e, '공유 컴퓨터를 불러오지 못했습니다')));
        return;
    }
    const memberName = (id) => {
    ...
    const adminActs = (n) => el('div', { class: 'wikicat-row-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: n.enabled === false ? '연결 허용' : '연결 차단',
        title: n.enabled === false ? '이 컴퓨터가 다시 연결되도록 허용합니다.' : '이 컴퓨터의 연결을 막습니다(작업은 못 들어오고, 등록은 남습니다).',
        onclick: () => act.setEnabled(n, n.enabled === false) }), el('button', { class: 'btn btn-ghost btn-sm', text: '접속 열쇠 재발급', onclick: () => act.rotate(n) }), el('button', { class: 'btn btn-ghost btn-sm', text: '공유 해제',
        title: '공유를 거두고 등록한 사람 전용으로 되돌립니다. 다시 공유하려면 지우고 공유용으로 새로 등록하세요.',
        onclick: () => act.unshare(n) }), el('button', { class: 'btn btn-ghost btn-sm', text: '지우기', onclick: () => act.remove(n) }));
    detail.replaceChildren(...[
        sectionHead('컴퓨터(노드) · 공유 ' + shared.length + '대', '조직이 함께 쓰는 컴퓨터입니다. 구성원 누구나 여기에 AI 세션을 열고 오래 걸리는 작업을 맡길 수 있습니다.'),
        el('div', { class: 'admin-actions', style: 'margin:0 0 14px' }, el('button', { class: 'btn btn-primary', text: '+ 공유 컴퓨터 등록', onclick: () => openSharedNodeForm(reload) })),
        el('div', { class: 'card' }, ...[
            el('div', { class: 'wikicat' }, nodeGroup('공유 컴퓨터', null, shared, (n) => nodeRow(n, memberName(n.owner_member), adminActs(n)), '아직 공유 컴퓨터가 없습니다 — [+ 공유 컴퓨터 등록]으로 공용 서버의 자리를 먼저 만드세요.')),
            // 이 화면이 '조직 전체 컴퓨터 목록'이 아니라는 걸 분명히 한다 — 안 보이는 게 결함이 아니라 설계다.
            el('p', { class: 'admin-hint' }, '구성원이 각자 연결한 개인 컴퓨터는 여기 보이지 않습니다. 그 컴퓨터들은 연결한 본인만 쓰고, 각자 [내 설정 ▸ 내 컴퓨터]에서 관리합니다. '
