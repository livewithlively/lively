...
export async function myNodesPanel(detail, data) {
    ...
    detail.replaceChildren(...[
        ...
        el('div', { class: 'card' }, ...[
            ...
            sharedMine ? el('p', { class: 'admin-hint' }, '내 컴퓨터 중 ' + sharedMine + '대가 공유로 지정돼 있습니다 — 구성원 누구나 그 컴퓨터를 씁니다. '
...
export async function orgNodesPanel(detail, data) {
    ...
    try {
        ...
        detail.replaceChildren(el('div', { class: 'card' }, errorNote(e, '조직의 컴퓨터를 불러오지 못했습니다')));
        return;
    }
    const memberName = (id) => {
    ...
    const adminActs = (n) => el('div', { class: 'wikicat-row-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: n.shared ? '공유 해제' : '공유로 지정',
        title: n.shared ? '다시 연결한 사람만 쓰도록 되돌립니다.' : '조직 구성원 전체가 이 컴퓨터를 쓸 수 있게 합니다.',
        onclick: () => act.setShared(n, !n.shared) }), el('button', { class: 'btn btn-ghost btn-sm', text: n.enabled === false ? '연결 허용' : '연결 차단',
        onclick: () => act.setEnabled(n, n.enabled === false) }), el('button', { class: 'btn btn-ghost btn-sm', text: '접속 열쇠 재발급', onclick: () => act.rotate(n) }), el('button', { class: 'btn btn-ghost btn-sm', text: '지우기', onclick: () => act.remove(n) }));
    detail.replaceChildren(...[
        sectionHead('컴퓨터(노드) · ' + nodes.length + '대', '조직에 연결된 컴퓨터 전체입니다. 기본은 연결한 사람만 쓰고, 여기서 공유로 지정한 컴퓨터만 구성원 전체가 함께 씁니다.'),
        el('div', { class: 'card' }, ...[
            el('div', { class: 'wikicat' }, ...[
                nodeGroup('공유 컴퓨터', '구성원 누구나 여기에 AI 세션을 열고 작업을 맡길 수 있습니다.', shared, (n) => nodeRow(n, memberName(n.owner_member), adminActs(n)), '아직 공유로 지정된 컴퓨터가 없습니다 — 아래에서 공용 서버를 골라 [공유로 지정]하세요.'),
                nodeGroup('구성원 개인 컴퓨터', '연결한 본인만 씁니다. 공유로 지정하면 조직 전체가 그 컴퓨터에서 작업을 실행하게 되니, 개인 노트북은 그대로 두세요.', personal, (n) => nodeRow(n, memberName(n.owner_member), adminActs(n)), '연결된 개인 컴퓨터가 없습니다.'),
            ]),
