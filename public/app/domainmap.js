...
function domainmapBody(data, firstSectionHint) {
    ...
    const statCard = el('div', { class: 'card' }, el('div', { class: 'stat-row' }, stat(fmtNum(domains.length), '도메인', '개'), stat(fmtNum(withShould), '의도(should) 설정됨', '/ ' + domains.length), stat(fmtNum(debts.length), '괴리·이슈(debt)', gapCount ? ('· 괴리 ' + gapCount) : '건')));
    // ── 도메인별 should | is | debt ──
    const tone = (d) => (d.debts > 0 ? ' has-debt' : '');
    function domainRow(d) {
        const hasShould = !!(d.should && d.should.trim());
        return el('div', { class: 'dm-dom' + tone(d) }, el('div', { class: 'dm-dom-head' }, el('span', { class: 'mono dm-dom-key', text: d.key }), d.name && d.name !== d.key ? el('span', { class: 'dm-dom-name', text: d.name }) : null, d.cross_cutting ? el('span', { class: 'dm-tag', text: '횡단' }) : null, d.space === 'business' ? el('span', { class: 'dm-tag', text: '비즈니스' }) : null, d.debts > 0 ? el('span', { class: 'dm-debt-chip', text: '괴리 ' + fmtNum(d.debts) }) : null), el('div', { class: 'dm-axes' }, el('div', { class: 'dm-axis dm-should' }, el('span', { class: 'dm-axis-label', text: '의도 · should' }), hasShould
            ? el('span', { class: 'dm-axis-val', text: d.should })
            : el('span', { class: 'dm-axis-empty', text: '아직 설정 안 됨' })), el('div', { class: 'dm-axis dm-is' }, el('span', { class: 'dm-axis-label', text: '구조 · is' }), el('span', { class: 'dm-axis-val' }, el('strong', { text: fmtNum(d.units) }), ' 코드', d.entities ? el('span', {}, ' · ', el('strong', { text: fmtNum(d.entities) }), ' 엔티티') : null, d.proposed ? el('span', { class: 'dm-prop', text: ' · 제안 ' + fmtNum(d.proposed) }) : null, (!d.units && !d.entities) ? el('span', { class: 'dm-axis-empty', text: '  매핑된 코드 없음' }) : null))));
    }
    ...
    function debtRow(dt) {
        ...
        return el('div', { class: 'dm-debt dm-sev-' + sev }, el('div', { class: 'dm-debt-top' }, el('span', { class: 'dm-sev-tag', text: DM_SEV_LABEL[sev] }), el('span', { class: 'dm-debt-title', text: dt.title }), dt.status && dt.status !== 'open' ? el('span', { class: 'dm-debt-status', text: dt.status }) : null), dt.detail ? el('div', { class: 'dm-debt-detail', text: dt.detail }) : null);
    }
    // ── 의도(should) 변경 이력 — 누가·어떤 작업으로 의도를 어떻게 바꿨나(before→after) ──
    const valOr = (s, fallback) => (s && s.trim() ? s : fallback);
    function shouldChangeRow(c) {
        return el('div', { class: 'dm-change' }, el('div', { class: 'dm-change-top' }, el('span', { class: 'mono', text: c.domain_key || ('#' + c.domain_id) }), c.domain_name ? el('span', { class: 'dm-change-dom', text: c.domain_name }) : null, el('span', { class: 'dm-change-when', text: relTime(c.at) })), el('div', { class: 'dm-diff' }, el('div', { class: 'dm-diff-side dm-before' }, el('span', { class: 'dm-diff-label', text: '이전' }), el('span', { class: 'dm-diff-val', text: valOr(c.should_before, '(없음)') })), el('span', { class: 'dm-diff-arrow', 'aria-hidden': 'true', text: '→' }), el('div', { class: 'dm-diff-side dm-after' }, el('span', { class: 'dm-diff-label', text: '이후' }), el('span', { class: 'dm-diff-val', text: valOr(c.should_after, '(없음)') }))), el('div', { class: 'dm-change-by' }, c.activity_id
            ? el('span', { class: 'dm-by-act' }, actTypeTag(c.activity_type), el('span', { class: 'dm-act-title', text: c.activity_title || '' }))
            : el('span', { class: 'dm-change-noact', text: '작업 귀속 없음' }), el('span', { class: 'dm-change-who', text: (c.author_person || c.actor_id || '미상') + (c.author_agent ? ' · ' + c.author_agent : '') })));
    }
    ...
    return [
        statCard,
        section('도메인별 의도(should) · 구조(is) · 괴리(debt)', firstSectionHint || null, domains.map(domainRow), '이 레포에 도메인이 없습니다.'),
        section('괴리(debt) — 의도와 구조의 간극', gapCount ? ('should↔is 괴리 ' + fmtNum(gapCount) + '건 포함') : null, debtSorted.map(debtRow), '표면화된 괴리·이슈가 없습니다.'),
        section('의도(should) 변경 이력', '누가 · 어떤 작업으로 의도를 바꿨나', shoulds.map(shouldChangeRow), '아직 의도(should) 변경 기록이 없습니다. 의도를 설정·수정하면 여기 쌓입니다.'),
        section('커밋 → 구조(is) 변경 이력', '어떤 커밋이 코드 구조를 바꿨나', isChanges.map(isChangeRow), '아직 커밋이 구조(is)를 바꾼 기록이 없습니다. commit 작업이 코드를 건드리면 여기 쌓입니다.'),
    ].filter(Boolean);
}
