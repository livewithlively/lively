...
function sectionHidden(key, data) {
    ...
    if (key === 'me-logins' && uiMode() === 'v2')
        return true;
    if (uiProfilePersonal() && PERSONAL_HIDDEN.includes(key))
        return true; // #1454 S4 — 심플 어드민
...
async function renderAdmin(view, sub) {
    ...
    if (sel === 'me-logins' && uiMode() === 'v2') {
        try {
            (window.top || window).location.hash = '#/connect';
        }
        catch (_) {
