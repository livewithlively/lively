...
async function fetchAllSpaceCats() {
    ...
    const lists = await Promise.all(['business', 'product', 'system'].map((sk) => api('/api/ui/categories?' + new URLSearchParams({ space: sk })).then((d) => (d && d.categories) || []).catch(() => [])));
...
function knOwnedGroup(countEl) {
    const caret = el('span', { class: 'pjv-side-folder-caret kn-space-caret', 'aria-hidden': 'true', text: '▾' });
    const grp = el('details', { class: 'kn-space-group kn-owned-group', open: '' }, el('summary', { class: 'pjv-side-navitem pjv-side-navfolder pjv-side-navspace kn-space-head kn-owned-head' }, el('span', { class: 'pjv-side-space-avatar kn-owned-ava', title: '내가 담당하는 카테고리', text: '★' }), el('span', { class: 'pjv-side-navlabel', text: '내 소유 카테고리' }), countEl || null, caret));
    grp.addEventListener('toggle', () => { caret.textContent = grp.open ? '▾' : '▸'; });
    return grp;
}
...
let knDragId = null;
let knDragGroup = null;
function knClearDropHints(nav) {
    nav.querySelectorAll('.kn-nav-catwrap.drop-before, .kn-nav-catwrap.drop-after')
        .forEach((n) => n.classList.remove('drop-before', 'drop-after'));
}
function knWireCatDrag(wrap, catId, group, nav, onReorder) {
    wrap.setAttribute('draggable', 'true');
    wrap.dataset.dragGroup = group;
    wrap.addEventListener('dragstart', (ev) => {
        knDragId = String(catId);
        knDragGroup = group;
        wrap.classList.add('drag-src');
        try {
            ev.dataTransfer.effectAllowed = 'move';
            ev.dataTransfer.setData('text/plain', String(catId));
        }
        catch (_) { /* 일부 브라우저 — setData 없이도 진행 */ }
    });
    ...
    wrap.addEventListener('dragover', (ev) => {
        if (!knDragId || knDragGroup !== group || knDragId === String(catId))
            return; // 다른 그룹·자기 자신 — 받지 않는다
        ev.preventDefault();
...
function knNavCatNode(c, on, onOpen, isMine, favOpts) {
    ...
    const row = el('a', { class: 'pjv-side-navitem kn-side-item kn-side-item-sub kn-nav-cat' + (on ? ' active' : '') + (isFav ? ' is-fav' : ''), href: '#',
        ...
        ...(c.hint ? { title: c.hint } : isMine ? { title: '내 소유 카테고리 — ' + (c.name || c.key) } : {}) }, tw, isMine ? knTeamChip() : null, el('span', { class: 'pjv-side-navlabel', text: c.name || c.key }), Number.isFinite(cnt) ? el('span', { class: 'pjv-side-navcount' + (cnt === 0 ? ' kn-count-zero' : ''), title: '지식 ' + cnt + '개', text: String(cnt) }) : null);
...
function buildKnowledgeNav(nav, bySpace, selected, myIds, opts) {
    ...
    const reorder = (src, target, before) => {
        if (knApplyCatReorder(allCats, src, target, before))
            onReorder();
    };
    const catNode = (c, group) => knWireCatDrag(knNavCatNode(c, String(selected) === String(c.id), onOpen, false, favOpts), String(c.id), group, nav, reorder);
    ...
    if (favCats.length) {
        ...
        if (knCatOrderSaved().length) {
            ...
            undo.addEventListener('click', (ev) => { ev.preventDefault(); ev.stopPropagation(); knCatOrderClear(); onReorder(); });
            grp.firstChild.insertBefore(undo, grp.firstChild.lastChild);
        }
        for (const c of favCats)
            grp.append(catNode(c, 'mine'));
        nav.append(grp);
    ...
    if (uncat > 0) {
        nav.append(knNavCatNode({ id: KN_UNCAT, name: '미분류', knowledge_count: uncat,
            hint: '카테고리가 없는 지식 — 소환(recall)에 안 잡히니 문서를 열어 분류해 주세요' }, String(selected) === KN_UNCAT, onOpen));
    }
}
...
async function knSideFilterNav(nav, q) {
    ...
    nav.querySelectorAll('.kn-side-hits').forEach((n) => n.remove());
    const applyGroups = () => {
...
function knMakeSideSearch(nav, st) {
    const input = el('input', { class: 'pjv-side-search-input', type: 'text', placeholder: '카테고리·지식 검색', 'aria-label': '카테고리·지식 검색', value: st.q || '' });
...
function knSideTools() {
    ...
    return el('div', { class: 'kn-side-tools' }, el('div', { class: 'eyebrow kn-side-tools-eyebrow', text: '도구' }), graphBtn, sourcesBtn, reviewBtn, trashBtn);
}
...
function createWikiSide(opts) {
    ...
    function buildSide() {
        buildKnowledgeNav(nav, bySpace, opts.selected ? opts.selected() : '', myIds, { indexed: true, onOpen: opts.onOpen, favCatIds, onToggleFav: toggleCatFav, uncatCount, onReorder: buildSide });
    ...
    const ready = (async () => {
        ...
        catch (_) { /* graceful: 사이드바 생략(콘텐츠는 계속) */ }
        await peopleP;
        if (opts.uncategorized) {
            try {
                uncatCount = await knFetchUncategorizedCount();
            }
            catch (_) { /* graceful: '미분류' 노드만 생략 */ }
        }
        try {
    ...
    function ownedFirst() {
        const allCats = ['business', 'product', 'system'].flatMap((sk) => bySpace[sk] || []);
        const owned = new Set([...favCatIds, ...Array.from(myIds)]);
        const list = knSortByCatOrder(allCats.filter((c) => owned.has(String(c.id))));
...
function wireSideCollapse(shell, sideCtl) {
    if (!sideCtl.collapseBtn || !sideCtl.reopenBtn)
        return;
    let collapsed = false;
    try {
