...
function knOwnedGroup(countEl) {
    const caret = el('span', { class: 'pjv-side-folder-caret kn-space-caret', 'aria-hidden': 'true', text: '▾' });
    const grp = el('details', { class: 'kn-space-group kn-owned-group', open: '' }, el('summary', { class: 'pjv-side-navitem pjv-side-navfolder pjv-side-navspace kn-space-head kn-owned-head' }, el('span', { class: 'pjv-side-space-avatar kn-owned-ava', title: '내가 담당하는 카테고리' }, knOwnedStar()), el('span', { class: 'pjv-side-navlabel', text: '내 소유 카테고리' }), countEl || null, caret));
    grp.addEventListener('toggle', () => { caret.textContent = grp.open ? '▾' : '▸'; });
    return grp;
}
