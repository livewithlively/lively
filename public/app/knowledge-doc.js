...
function knEffectiveVisible(hidden, propsUi) {
    ...
    for (const p of KN_PROP_CATALOG) {
        if ((!hidden.includes(p.key) || show.has(p.key)) && !hide.has(p.key))
            out.add(p.key);
    }
    return out;
}
...
function knPropValue(k, key) {
    switch (key) {
        ...
        case 'confidence':
            ...
            if (!k.confidence || k.confidence === 'observed')
                return null;
        ...
        case 'external': {
            ...
            if (!k.external_system && !k.external_url)
                return null;
            const parts = [];
            if (k.external_system)
                parts.push(el('span', { text: k.external_system + (k.external_instance ? ' · ' + k.external_instance : '') }));
            if (k.external_url) {
...
function knPropRow(label, value, hint) {
    ...
    v.append(value && value.nodeType ? value : el('span', { text: String(value) }));
    if (hint)
        v.append(infoDot(hint));
...
function openKnPropsSettings(foot, gear, k, hidden, onChanged) {
    ...
    function paintList() {
        list.replaceChildren(...KN_PROP_CATALOG.map((p) => {
            ...
            return el('div', { class: 'kn-propopt' + (on ? '' : ' off'), title: p.key }, el('span', { class: 'kn-propopt-label', text: p.label }), eye);
        }));
    }
    paintList();
    ...
    recBtn.onclick = () => { for (const p of KN_PROP_CATALOG)
        toggles.set(p.key, !KN_PROP_RECOMMENDED_HIDDEN.includes(p.key)); paintList(); };
    ...
    const busy = (b) => { docBtn.disabled = b; allBtn.disabled = b; };
    ...
    allBtn.onclick = async () => {
        ...
        try {
            ...
            for (const p of KN_PROP_CATALOG) {
                const on = !!toggles.get(p.key);
                if (on === vis.has(p.key))
                    continue; // 초기 토글(효과 노출)에서 안 바뀐 속성 = 전역 불변
                if (on)
                    nextHidden.delete(p.key);
                else
                    nextHidden.add(p.key);
            }
    ...
    const pop = el('div', { class: 'kn-props-pop' }, el('div', { class: 'kn-props-pop-head' }, el('span', { text: '속성 표시' }), recBtn), list, el('div', { class: 'kn-props-pop-actions' }, docBtn, allBtn));
    foot.append(pop);
    ...
    const onDoc = (ev) => { if (pop.contains(ev.target) || gear.contains(ev.target))
        return; close(); };
...
function openKnMetaPicker(anchor, field, k, save) {
    ...
    const opt = (label, value, on) => {
        const b = el('button', { class: 'kn-metapop-item' + (on ? ' on' : ''), type: 'button' }, el('span', { text: label }), on ? el('span', { class: 'kn-metapop-check', 'aria-hidden': 'true', text: '✓' }) : null);
        b.onclick = async () => { close(); if (!on) {
            try {
    ...
    if (field === 'type') {
        for (const [v, label] of Object.entries(KN_TYPE_LABEL))
            pop.append(opt(label, v, k.type === v));
    }
    else {
        ...
        api('/api/ui/categories').then((d) => {
            ...
            for (const sp of ['business', 'product', 'system']) {
                ...
                for (const c of inSp)
                    parts.push(opt(c.name || c.key, c.key, cur.has(c.key)));
            }
...
function buildKnPropsBlock(k, hidden, opts) {
    const vis = knEffectiveVisible(hidden, k.props_ui);
    const rows = [], hiddenRows = [];
    for (const p of KN_PROP_CATALOG) {
        const v = knPropValue(k, p.key);
        if (!v)
            continue; // 빈 값 자동 생략(노션 동일)
    ...
    const mkRow = ({ p, v }, dim) => {
        ...
        if (opts.editMeta && (p.key === 'category' || p.key === 'type')) {
            ...
            const val = r.querySelector('.kn-prop-v');
            ...
            val.addEventListener('keydown', (ev) => { if (ev.key === 'Enter')
                openIt(); });
        }
        return r;
    };
    ...
    const paint = () => {
        body.replaceChildren(...rows.map((rv) => mkRow(rv)), ...(revealed ? hiddenRows.map((rv) => mkRow(rv, true)) : []));
        const footKids = [];
        if (hiddenRows.length)
            footKids.push(el('button', { class: 'kn-props-more', type: 'button',
...
function knChildrenPanel(k) {
    ...
    }, el('span', { class: 'kn-link-rel kn-link-child', text: c.icon || (c.is_folder ? '📁' : knTreeIcon(c.notion_kind)) }), el('span', { class: 'kn-linkrow-title', text: (c.title || c.name) + (c.lifecycle === 'archived' ? ' (보관됨)' : '') })));
...
function knLinksPanel(k, reload) {
    ...
    const head = el('div', { class: 'sec-label sec-label-row' }, el('span', { text: '연결된 지식' }), hasMemoryScope() ? el('button', { class: 'btn btn-ghost btn-sm', text: '＋ 지식 연결',
    ...
    const dirGroup = (label, arr, incoming) => el('div', { class: 'kn-linkdir' }, el('span', { class: 'kn-linkdir-head', text: label }), el('div', { class: 'kn-linkrows' }, ...arr.map((e) => knLinkRow(e, k, reload, incoming))));
...
function openKnowledgeMoveTo(k, reload) {
    ...
    const back = overlayBox('이동 · ' + (k.title || k.name), el('p', { class: 'admin-hint', text: '옮길 폴더를 고르세요. (최상위)는 폴더 밖(트리 루트)으로 꺼냅니다.' }), list);
    const pick = (parentName, label) => el('div', { class: 'row', role: 'button', tabindex: '0', style: 'cursor:pointer',
        onclick: async () => {
            try {
                await api('/api/ui/knowledge/' + encodeURIComponent(k.name) + '/move', { method: 'POST', body: JSON.stringify({ parent_name: parentName }) });
    ...
    (async () => {
        ...
        if (catId != null) {
            try {
                folders = (await knFetchCategoryRows(catId)).filter((r) => r.is_folder && r.name !== k.name);
            }
...
function knCommentsSection(name) {
    ...
    let feed = [];
    const openThreads = new Set(); // 펼쳐진 최상위 댓글 id
    const meName = (state.me && (state.me.display_name || state.me.userId)) || '나';
    ...
    function card(c, isReply) {
        ...
        if (!isReply) {
            ...
            replyBtn.onclick = () => {
                if (reps.length && !openThreads.has(c.id))
                    openThreads.add(c.id);
                else if (reps.length && openThreads.has(c.id))
                    openThreads.delete(c.id);
                else
                    openThreads.add(c.id); // 답글 없으면 작성칸만 펼침
                paint();
                if (openThreads.has(c.id))
                    setTimeout(() => { const t = feedBox.querySelector('[data-reply-for="' + c.id + '"] textarea'); if (t)
                        t.focus(); }, 0);
            };
            foot.append(replyBtn);
        }
        const kids = [
            el('div', { class: 'kn-cmt-meta' }, el('span', { class: 'kn-cmt-name', text: who }), el('span', { class: 'kn-cmt-time', title: c.ts ? absTime(c.ts) : '', text: c.ts ? relTime(c.ts) : '' })),
            el('div', { class: 'kn-cmt-text md-rendered' }, renderMarkdown(c.body || '')),
            foot,
        ];
        return el('div', { class: 'kn-cmt-card' + (isReply ? ' kn-cmt-reply' : '') }, personFace(c.actor || who, 'kn-cmt-ava', who), el('div', { class: 'kn-cmt-body' }, ...kids));
    }
    ...
    function replyComposer(pid) {
        ...
        if (!hasMemoryScope())
            return wrap;
        const ta = el('textarea', { class: 'kn-cmt-input kn-cmt-input-sm', placeholder: '답글…', rows: '1' });
        ...
        const go = async () => {
            ...
            ta.disabled = true;
            try {
                const d = await api('/api/ui/knowledge/' + encodeURIComponent(name) + '/comments', { method: 'POST', body: JSON.stringify({ text, parent_id: pid }) });
                feed = (d && d.feed) || [];
                openThreads.add(pid);
                paint();
            }
            catch (e) {
                toast('답글 실패 — ' + e.message, true);
                send.disabled = false;
                ta.disabled = false;
            }
        };
    ...
    function paint() {
        ...
        if (!tops.length) {
            feedBox.append(el('div', { class: 'kn-cmt-empty', text: '아직 댓글이 없어요.' }));
            return;
        }
        for (const c of tops) {
    ...
    if (hasMemoryScope()) {
        const ta = el('textarea', { class: 'kn-cmt-input', placeholder: '댓글을 입력하세요…  (Enter 전송 · Shift+Enter 줄바꿈)', rows: '1' });
        ...
        const go = async () => {
            ...
            ta.disabled = true;
            try {
                const d = await api('/api/ui/knowledge/' + encodeURIComponent(name) + '/comments', { method: 'POST', body: JSON.stringify({ text }) });
                feed = (d && d.feed) || [];
                ta.value = '';
                grow();
                paint();
        ...
        send.onclick = go;
        composer.append(personFace((state.me && state.me.userId) || 'me', 'kn-cmt-ava', meName), el('div', { class: 'kn-cmt-composer-in' }, ta, el('div', { class: 'kn-cmt-composer-foot' }, send)));
    }
...
async function buildKnowledgeDetail(container, name, opts) {
    ...
    try {
        ...
        if (e.status === 404) {
            container.replaceChildren(el('div', { class: 'page-head' }, el('h1', { text: '없는 지식' })), el('div', { class: 'note', text: "'" + name + "' 을(를) 찾을 수 없습니다." }), el('a', { class: 'btn btn-ghost btn-sm', href: '#/knowledge', text: '← 지식으로' }));
            return;
        }
    ...
    if (!k) {
        container.replaceChildren(el('div', { class: 'note', text: '지식을 찾을 수 없습니다.' }));
        return;
    }
    ...
    const crumbs = el('div', { class: 'crumbs' }, el('a', { class: 'crumb-link', href: '#/knowledge', text: '지식' }));
    const ancestors = Array.isArray(k.ancestors) ? k.ancestors : [];
    for (const a of ancestors) {
        crumbs.append(el('span', { class: 'crumb-sep', text: ' / ' }), el('a', { class: 'crumb-link', href: '#/k/' + encodeURIComponent(a.name), text: a.title || a.name }));
    }
    crumbs.append(el('span', { class: 'crumb-sep', text: ' / ' }), ancestors.length
    ...
    const toggleFull = async () => {
        ...
        container.classList.toggle('full', full);
        if (!canEdit)
            return; // 읽기전용 — 화면에서만 토글(저장 생략)
        try {
            await saveKnPropsUi(k, { full_width: full });
        }
        catch (e) {
            toast('전폭 설정 저장 실패 — ' + e.message, true);
        }
    };
    ...
    moreBtn.onclick = () => {
        ...
        const item = (ic, label, fn, danger) => {
            const b = el('button', { class: 'kn-morepop-item' + (danger ? ' danger' : ''), type: 'button' }, el('span', { class: 'kn-morepop-ic', 'aria-hidden': 'true', text: ic }), el('span', { text: label }));
            b.onclick = () => { close(); fn(); };
            return b;
        };
    ...
    if (k.lifecycle === 'archived')
        badges.push(el('span', { class: 'kn-chip kn-archived',
            title: '원본(외부 시스템)에서 보관/삭제되어 미러만 남았습니다 — 내용은 보존됩니다.', text: '📦 보관됨' }));
    if (k.is_wiki)
        badges.push(el('span', { class: 'kn-chip kn-pin',
    ...
    if (k.is_folder) {
        ...
        const rawToggle = el('button', { class: 'btn btn-ghost btn-sm md-raw-toggle', text: '원문 보기',
            onclick: () => {
                ...
                if (showingRaw)
                    rawView.textContent = editor.getMarkdown();
                editor.el.hidden = showingRaw;
                rawView.hidden = !showingRaw;
                rawToggle.textContent = showingRaw ? '편집으로' : '원문 보기';
            } });
        body = el('div', { class: 'kn-doc-body kn-doc-editable' }, editor.el, rawView, el('div', { class: 'kn-doc-rawrow' }, rawToggle));
        body._getMd = () => editor.getMarkdown(); // 제목 저장 등 메타 upsert 가 편집 중 본문을 잃지 않게
    }
    else {
        ...
        const rawToggle = rawText
            ...
            : null;
        body = el('div', { class: 'kn-doc-body' }, rendered, rawView, rawToggle ? el('div', { class: 'kn-doc-rawrow' }, rawToggle) : null);
    }
    ...
    const panels = el('div', { class: 'kn-doc-panels' }, k.is_folder ? null : knChildrenPanel(k), knNotionPropsPanel(k), knLinksPanel(k, reload), knProjectLinks(k.name), relatedBox);
    ...
    const bodyMdNow = () => (body._getMd ? body._getMd() : (k.body_md || '')); // 편집 모드면 에디터 스냅샷이 truth
    ...
    function paintDecor() {
        ...
        coverEl.replaceChildren(canEdit && cv ? el('div', { class: 'kn-cover-btns' }, el('button', { class: 'ke-coverbtn', type: 'button', text: '커버 변경',
            onclick: (e) => openCoverPicker(e.target, { current: cv, onPick: (v) => saveDecor({ cover: v }) }) }), el('button', { class: 'ke-coverbtn', type: 'button', text: '제거', onclick: () => saveDecor({ cover: null }) })) : null);
        // 아이콘
        iconBtn.hidden = !ic;
        iconBtn.textContent = ic;
        iconBtn.classList.toggle('on-cover', !!cv);
        // 추가 버튼(호버 노출) — 권한자 & 비어있는 축만.
        decorAdd.replaceChildren(canEdit && !ic ? el('button', { class: 'ke-decor-btn', type: 'button', text: '☺ 아이콘 추가',
            onclick: (e) => openEmojiPicker(e.target, { onPick: (em) => saveDecor({ icon: em }) }) }) : null, canEdit && !cv ? el('button', { class: 'ke-decor-btn', type: 'button', text: '🖼 커버 추가',
...
async function knFolderChildrenBlock(k) {
    ...
    return el('div', { class: 'kn-links kn-folder-children' }, el('div', { class: 'sec-label sec-label-row' }, el('span', { text: '폴더 항목' }), el('span', { class: 'kn-sim-hint', text: (k.children_total || children.length) + '개' })), el('div', { class: 'kn-linkrows' }, ...rows));
}
// ════════════════════════════════════════════
...
// ════════════════════════════════════════════
let knPeekCur = null; // { back, name, pushed, baseHash, onPop, onKey, onRefresh }
...
function reanchorKnowledgePeek() { if (knPeekCur)
    knPeekCur.baseHash = knHashWithPeek(null); }
...
function openKnowledgePeek(name, opts) {
    ...
    const panel = el('aside', { class: 'kn-peek', role: 'dialog', 'aria-label': '지식 미리보기' }, el('div', { class: 'kn-peek-head' }, fullBtn, tabBtn, xBtn), body);
    ...
    const onPop = () => {
        ...
        if (!cur || cur.back !== back)
            return;
        if (knHashWithPeek(null) !== cur.baseHash) {
            dismissKnowledgePeek();
            return;
    ...
    if (!fromUrl) {
        history.pushState(null, '', knHashWithPeek(name));
        knPeekCur.pushed = true;
    }
    window.addEventListener('popstate', onPop);
    document.addEventListener('keydown', onKey);
    buildKnowledgeDetail(body, name, { mode: 'peek',
        onDeleted: () => { requestClose(); if (onRefresh)
            onRefresh(); } });
}
// ════════════════════════════════════════════
...
function openQuickSearch() {
    ...
    const panel = el('div', { class: 'qk-panel', role: 'dialog', 'aria-label': '빠른 검색' }, el('div', { class: 'qk-inrow' }, el('span', { class: 'qk-glass', 'aria-hidden': 'true', text: '🔍' }), input, el('span', { class: 'qk-esc', text: 'esc' })), list);
    ...
    const openEntry = (e, fullPage) => {
        ...
        if (page === 'k' || page === 'k-edit')
            location.hash = '#/k/' + encodeURIComponent(e.name);
        else
            openKnowledgePeek(e.name);
    };
    function paint() {
        if (!entries.length) {
        ...
        list.replaceChildren(...entries.map((e, i) => {
            ...
            const row = el('button', { class: 'qk-row' + (i === sel ? ' on' : ''), type: 'button' }, el('span', { class: 'qk-row-ic', text: knPageIcon(e) }), el('div', { class: 'qk-row-main' }, el('div', { class: 'qk-row-title', text: e.title || e.name }), snip ? el('div', { class: 'qk-row-snip', text: snip }) : null));
...
document.addEventListener('keydown', (e) => {
    if (!(e.metaKey || e.ctrlKey) || (e.key !== 'k' && e.key !== 'K'))
        return;
    const page = location.hash.replace(/^#\//, '').split(/[/?]/)[0] || '';
    if (!['knowledge', 'k', 'k-edit', 'trash'].includes(page))
        return;
    e.preventDefault();
    openQuickSearch();
});
export { SPACE_LABEL, KN_INJECTION_LABEL, KN_INJECTION_HINT, KN_PROVENANCE_LABEL, KN_PROVENANCE_HINT, KN_AUTHOR_LABEL, KN_AUTHOR_HINT, KN_TYPE_LABEL, KN_REL_LABEL, KN_LINK_REL_LABEL, KN_SOURCE_REL_LABEL, SOURCE_KIND_LABEL, buildKnowledgeDetail, consumeKnPeekNavGuard, dismissKnowledgePeek, reanchorKnowledgePeek, infoDot, knAuthorChip, knChildrenPanel, knFetchAuthoredTree, knFetchCategoryRows, knFolderFirstSort, knInjectChip, knInvalidateTreeCaches, knLinksPanel, knNotionPropsPanel, knPageIcon, knProjectLinks, knProvChip, knSimilarItem, knTreeIcon, knTypeChip, openKnowledgePeek, openProjectChooser, openQuickSearch, openSourceDetail, saveKnPropsUi, };
