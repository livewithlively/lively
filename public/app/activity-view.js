...
function actGlyph(paths, box, size, cls) {
    return sv('svg', { class: cls, viewBox: '0 0 ' + box + ' ' + box, width: size, height: size, fill: 'none', 'aria-hidden': 'true' }, ...paths.map((d) => sv('path', { d, stroke: 'currentColor', 'stroke-width': '1.7', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })));
}
...
function actLinkRow(icon, kind, name, sub, href, newTab) {
    ...
    const attrs = { class: 'act-lnk' + (href ? '' : ' act-lnk-static') };
    ...
    return el(href ? 'a' : 'div', attrs, el('span', { class: 'act-lnk-ico' }, actGlyph(ACT_LNK_ICON[icon], 24, '17', null)), el('span', { class: 'act-lnk-main' }, el('span', { class: 'act-lnk-kind', text: kind }), el('span', { class: 'act-lnk-name' + (icon === 'code' ? ' mono' : ''), text: name }), sub ? el('span', { class: 'act-lnk-sub', text: sub }) : null), go);
}
...
function activityDetailView(a, nameOf, opts) {
    ...
    if (headed) {
        box.append(el('div', { class: 'act-doc-head' }, el('div', { class: 'act-doc-kicker' }, actTypeTag(a.type), ...changed.map((c) => el('span', { class: 'act-doc-tag', text: c }))), el('h3', { class: 'act-doc-title', text: a.summary || a.title || '(제목 없음)' }), el('div', { class: 'act-doc-by' }, el('span', { class: 'act-doc-who', text: nameOf(a.author_person) || '미상' }), a.author_agent ? el('span', { class: 'act-doc-agent', text: a.author_agent }) : null, el('span', { class: 'act-doc-when', title: relTime(when), text: fmtDateTime(when) }))));
    }
    ...
    if (a.body)
        readable.append(el('div', { class: 'md-rendered act-doc-md' }, renderMarkdown(a.body)));
    ...
    const linkSec = el('div', { class: 'act-doc-sec', hidden: true }, el('div', { class: 'act-doc-label', text: '연결된 기록' }), lnks);
    ...
    for (const t of (a.tasks || [])) {
        ...
        addLink(actLinkRow('project', t.level === 'project' ? '진척시킨 프로젝트' : '진척시킨 태스크', s.name, s.sub, '#/projects2/p/' + t.id, false));
    }
    // 코드 — 갈 곳이 없는 사실이라 링크가 아니다.
    if (a.commit_sha) {
        addLink(actLinkRow('code', '코드 커밋', [a.repo, a.commit_sha.slice(0, 7)].filter(Boolean).join(' · '), a.touchCount ? '코드 ' + fmtNum(a.touchCount) + '곳을 건드렸습니다.' : ''));
    }
    box.append(linkSec);
...
async function attachSessionLink(addLink, sessionId) {
    ...
    try {
        ...
        return;
    } // 비공개(403) 또는 종료됨 → 들어갈 수 없으니 항목 없음
    if (!s || !s.id)
        return;
