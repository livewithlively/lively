...
function openCategoryForm(space, existing, reload, opts) {
    ...
    if (repoOptions.length) {
        ...
        repoField = el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '연결 레포 (선택)' }), wrap, el('p', { class: 'admin-hint', style: 'margin:6px 0 0' }, '이 분류가 사는 코드 레포입니다. 코드 스캔이 추정하는 것과 별개로, 여기서 직접 정합니다.'));
    }
    const back = overlayBox(editing ? '분류 수정' : ('새 분류 · ' + (SPACE_LABEL[space] || space)), el('div', { class: 'field' }, el('label', { class: 'field-label', text: '이름' }), nameIn), el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '키' }), keyIn), el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '정의·범위·규칙 (should)' }), shouldIn), el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '설명 (선택)' }), descIn), repoField, el('div', { class: 'ov-actions' }, saveBtn, cancelBtn));
    ...
    const go = async () => {
        ...
        try {
            if (editing) {
                ...
                if (repoField)
                    await saveRepos(existing.id, pickedRepos);
                toast('저장했습니다');
            }
            else {
                ...
                if (repoField && newId && pickedRepos.length)
                    await saveRepos(newId, pickedRepos);
                toast('분류를 만들었습니다');
            }
            back.remove();
            reload();
