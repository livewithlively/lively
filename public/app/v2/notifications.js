...
export async function loadNotifications(opts = {}) {
    ...
    const out = await api('/api/ui/me/notifications' + q);
...
function seen() {
    try {
        return new Set(JSON.parse(localStorage.getItem(SEEN_STORE) || '[]'));
    }
    catch {
        return new Set();
    }
 ...
 * @returns 실제로 띄운 건수
 */
export function raiseBanners(list) {
...
export function startNotificationBanners(intervalMs = 30_000) {
    if (bannerTimer)
        return;
    if (window.livelyDesktop)
        return;
    const tick = () => {
        if (notificationPermission() !== 'granted')
            return; // 권한이 없으면 서버를 부를 이유도 없다
        void loadNotifications({ limit: 20 }).then((feed) => raiseBanners(feed.notifications)).catch(() => { });
    };
    tick();
...
export function notificationRow(n) {
    const inner = [
        ...
        el('span', { class: 'tw' }, el('span', { class: 't', text: n.title }), n.body ? el('span', { class: 'p', text: n.body }) : null),
