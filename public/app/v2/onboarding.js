...
<div class="ob-toast" id="toast"></div>`;
    ...
    const SCENES = {
        ...
        terminal: {
            html: () => qHead('terminal', S.ai === '아직 없어요' ? `AI 구독이 아직 없으셔도 괜찮아요. 자료 쌓기·정리·검색은 지금부터 됩니다. 첫 질문 <b>3회</b>는 라이블리 계정으로 열어 드릴게요.` : (S.aiConnected ? '연결됐어요. 거의 끝났습니다.' : '거의 끝났습니다.'), '터미널에서 Claude Code나 Codex 등을 쓰시나요?', '쓰신다면 그 컴퓨터를 라이블리에 이어 두시길 권합니다. 한 줄 설치로 끝납니다.')
    ...
    function startReading() {
        if (!S.read.total)
            S.read.total = S.upN || 41; // #1881 — 실제로 올린 파일이 있으면 그 수가 곧 읽을 자료 수다(가짜 41 금지)
        if (S.read.finished)
            return;
