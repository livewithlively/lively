...
<div class="ob-toast" id="toast"></div>`;
    ...
    const SCENES = {
        ...
        claude: {
            html: () => qHead('claude', `이어 두시면 제(리브)가 일할 때도 ${esc(nick() || '당신')}님의 구독을 씁니다. 라이블리가 따로 요금을 매기지 않습니다.`, S.aiConnected ? '이어졌어요.' : `${esc(S.ai || 'AI')} 계정을 이어 주세요.`, S.aiConnected ? '' : '터미널을 열고 아래 한 줄을 치면 로그인 창이 열립니다.')
                + (S.aiConnected
           ...
           <button class="ob-btn ob-btn-sub" data-skip>나중에 할게요</button>`,
            bind: (el) => {
                ...
                go.onclick = async () => {
                    ...
                    if (!WS || !WS.ai_ready) {
                        go.disabled = false;
                        go.textContent = '다시 확인';
                        if (err)
                            err.textContent = '아직 로그인이 안 보여요. 터미널에서 로그인을 끝내고 다시 눌러 주세요.';
                        return;
                    }
                    S.aiConnected = true;
