...
function applyTabChrome(tab) {
    ...
    if (mobile)
        mobile.setTitle(titleFor(tab.route).title);
...
function projectPath(id) {
    ...
    if (!p)
        return { id, anc: '', ancSegs: [], name: `프로젝트 #${id}` };
...
function drawSide() {
    ...
    drawSideTree(treeHost, data, activeKey, {
        ...
        onActivateInstance: (id) => { const t = tabsApi?.tabs.find((x) => x.id === id); if (t)
            tabsApi?.activate(t); },
        onCloseInstance: (id) => { const t = tabsApi?.tabs.find((x) => x.id === id); if (t)
            tabsApi?.close(t); },
