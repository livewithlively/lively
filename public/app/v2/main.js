...
import { bySeen, drawSide as drawSideTree, isAppPinned, loadFavLists, markNav, projLandingRoute, projectOrder, sessText } from './side.js';
...
async function loadData(opts) {
    ...
    void instsP.then((rows) => {
        ...
        if (data.sessions.length)
            return; // 이미 목록이 있는 판이면 아래 정상 경로가 그린다
        drawSide();
        tabsApi?.paint();
    });
        ...
        if (page === '' || page === 'dashboard') {
            ...
            if (segs[1] === 'projects2' && !segs[2]) {
                ...
                if (landing !== '#/app/projects2') {
                    ...
                    if (tabsApi?.current() === tab && location.hash !== landing) {
                        suppressHash++;
...
const lastSideRows = new Map(); // 방금 그린 행 — 활성 자리 유지(activeHold)가 '누르기 직전 어디였나'를 읽는다
const sideRowInstance = new Map();
const dismissBasis = new Map(); // 행 키 → 치움 판정의 기준값(dismissKey) — × 와 put 이 같은 자를 쓴다
...
const seenSentAt = new Map(); // 세션 id → 마지막으로 도장을 보낸 시각(ms)
...
function markViewedSessionSeen() {
    const sid = viewedSessionId();
    if (!sid)
        return;
    const now = Date.now();
    if (now - (seenSentAt.get(sid) || 0) < SEEN_EVERY_MS)
        return;
...
function sideInstances() {
    ...
    const put = (key, route, at, stateKey, force, draft) => {
        ...
        if (!prev)
            dismissBasis.set(key, basis); // 아래 return 보다 먼저 — 치워진 행도 × 가 다시 읽을 수 있어야 한다
        //  치운 행은 **그 상태 그대로인 동안** 숨는다. 창이 열려 있으면(force) 늘 보인다 — 보고 있는 화면이 목록에 없으면 그게 고장이다.
        if (!force && !prev && dismissed[key] !== undefined && dismissed[key] === basis)
            return;
        ...
        if (key === activeKey && !pin) {
            if (st)
                activeHold = { key, group: PRIORITY_GROUP, rank: st.rank };
            else if (activeHold && activeHold.key === key) {
                group = activeHold.group;
                rank = activeHold.rank;
            }
        }
    ...
    for (const inst of appInstances) { // ② 세션 아닌 활성 인스턴스
        ...
        if (inst.subject_kind === 'session')
            continue; // 세션 인스턴스는 아래 ④ 에서 그 세션 행에 붙인다
    ...
    for (const inst of appInstances) {
        if (inst.status !== 'active' || inst.subject_kind !== 'session' || !inst.subject_ref)
            continue;
...
function openSideRow(key, fallback) {
...
let homeRoute = (() => { try {
    return localStorage.getItem(HOME_ROUTE_STORE) || '';
}
catch (_) {
    return '';
} })();
/** 지금 보는 화면이 [홈] 구역의 것이면 그 자리를 적어 둔다. 구역은 사람이 고를 때만 바뀌므로 이 판정이 곧 '어느 장소인가'다. */
function noteHomeRoute(route) {
    if (railSection() !== 'home' || !route || route === homeRoute)
        return;
    homeRoute = route;
    try {
        localStorage.setItem(HOME_ROUTE_STORE, route);
    }
    catch (_) { /* 이번 화면은 된다 */ }
}
/** [홈] 착지 — 두고 간 세션이 아직 있으면 그 세션으로, 아니면 종전대로 새 작업 화면. */
function homeLandingRoute() {
