...
export async function bootV2() {
    ...
    root.replaceChildren(sideEl = el('nav', { class: 'v2-side stu-side', 'aria-label': '탐색' }), centerEl = el('div', { class: 'v2-main', id: 'v2-main' }), makeSplitter({ axis: 'x', key: 'aside-w', cssVar: '--v2-aside-w', target: root, def: 316, min: 240, max: 720, grow: -1, label: '우패널 너비' }), asideEl = el('aside', { class: 'v2-aside', 'aria-label': '이 선택의 맥락' }));
...
function railIcon(d) { const g = sv('svg', { viewBox: '0 0 24 24', class: 'stu-rail-i', 'aria-hidden': 'true' }); g.innerHTML = d; return g; }
const RAIL_P = {
...
function drawSide() {
    if (!sideEl)
        return;
    const key = activeKey();
    const item = (k, ic, label, href, onclick) => el(href ? 'a' : 'button', { class: 'stu-rail-it' + (key === k ? ' on' : ''), href: href || null, type: href ? null : 'button', 'data-nav': k, title: label, onclick }, railIcon(RAIL_P[ic]), el('span', { text: label }));
