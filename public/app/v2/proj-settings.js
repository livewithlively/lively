...
export function openProjSettings(opts) {
    ...
    const descSave = el('button', { class: 'btn btn-primary btn-sm', type: 'button', text: '본문 저장', onclick: () => {
            ...
            void api('/api/ui/v6/projects/' + id, { method: 'POST', body: JSON.stringify({ description: md || null }) })
                ...
                .catch((e) => toast('저장하지 못했어요 — ' + (e?.message || e), true))
    ...
    const archBtn = el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: archived ? '보관 해제' : '아카이브로 보내기',
        title: archived ? '원래 자리(사이드바·보드)로 되돌립니다' : '사이드바·보드에서 빼고 [아카이브]에 둡니다' });
    archBtn.onclick = () => {
        void (async () => {
            if (!archived && !await confirmProjectArchive({ name: String(p.name || ''), liveN: 0 }))
                return;
            ...
            try {
                ...
                toast((archived ? '보관을 해제하지' : '아카이브로 보내지') + ' 못했어요 — ' + (e?.message || e), true);
            }
        })();
    };
    ...
    const trashBtn = el('button', { class: 'btn btn-danger btn-sm', type: 'button', text: '휴지통으로 보내기',
        title: '프로젝트와 그 안의 내 세션을 함께 휴지통으로 보냅니다 — 휴지통에서 복원할 수 있어요' });
    trashBtn.onclick = () => {
        void (async () => {
            if (!await confirmProjectTrash({ name: String(p.name || ''), sessN: Number(p.session_count ?? p.my_session_count ?? 0) || 0, liveN: 0, othersLive: 0 }))
                return;
            trashBtn.disabled = true;
            try {
                const res = await api('/api/ui/v6/projects/' + id + '/trash', { method: 'POST', body: JSON.stringify({ trashed: true }) });
                ...
                toast('휴지통으로 보내지 못했어요 — ' + (e?.message || e), true);
            }
        })();
    };
    panel.replaceChildren(el('header', { class: 'pn-modal-h' }, el('h2', { text: '프로젝트 정보' }), el('button', { class: 'pn-modal-x', type: 'button', title: '닫습니다', 'aria-label': '닫기', onclick: close }, pnIcon('x', 'pn-i'))), el('div', { class: 'pn-modal-b' }, sec('이름', '사이드바·탭에 보이는 이름입니다.', nameIn), sec('상태', '프로젝트가 지금 어느 단계인지 알려 줍니다.', stateRow), sec('본문', '무엇을 하는 프로젝트인지 적어 둡니다. 세션과 리브가 이 글을 읽고 일합니다.', desc, el('div', { class: 'pn-set-foot' }, descSave)), sec('할 일', '큰 덩어리만 적어 두면 충분합니다. 자세한 것은 세션이 만들어 줍니다.', taskIn, taskList), sec('보관', archived ? '지금 아카이브에 있어요. 해제하면 사이드바·보드에 다시 보입니다.' : '끝났거나 한동안 안 볼 프로젝트는 통째로 치워 둘 수 있어요. 태스크·세션·지식 연결은 그대로 남습니다.', el('div', { class: 'pn-set-foot' }, archBtn)), sec('삭제', '프로젝트를 폴더째 휴지통으로 보냅니다 — 그 안의 내 세션도 함께 가고, 도는 세션은 멈춥니다. 휴지통에서 복원하면 함께 돌아와요.', el('div', { class: 'pn-set-foot' }, trashBtn))), el('footer', { class: 'pn-modal-f' }, el('a', { class: 'btn-text', href: '#/projects/' + id, onclick: () => close(), text: '전체 프로젝트 화면 열기 ↗' }), el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '닫기', onclick: close })));
