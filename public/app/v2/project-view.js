...
const stateWord = (cat) => cat === 'done' ? { word: '끝남', dot: 'done' } : cat === 'unstarted' ? { word: '시작 전', dot: '' } : { word: '진행 중', dot: 'busy' };
...
export function mountProjectView(host, opts) {
    ...
    let chat = null;
    let dead = false;
    let fileTotal = null; // 공유 폴더 재귀 파일 수(자료 n) — 마운트 때 한 번 잰다
    ...
    function paintHead() {
        ...
        const stat = (label, val, extra) => el('span', { class: 'pv-stat' }, el('span', { class: 'k', text: label }), el('b', { text: val }), extra ?? null);
        const stats = el('div', { class: 'pv-stats' }, busy ? el('span', { class: 'pv-stat live' }, el('span', { class: 'v2-dot busy', 'aria-hidden': 'true' }), el('b', { text: `지금 ${busy}개 작업 중` })) : null, stat('세션', String(sess.length)), tasks.length
            ? el('a', { class: 'pv-stat', href: '#/app/projects2/p/' + id, title: '보드에서 열기' }, el('span', { class: 'k', text: '할 일' }), el('b', { text: `${tdone}/${tasks.length}` }), el('span', { class: 'pv-minibar', 'aria-hidden': 'true' }, el('span', { class: 'f', style: 'width:' + Math.round((tdone / tasks.length) * 100) + '%' })))
            : stat('할 일', '0'), stat('자료', fileTotal == null ? '…' : String(fileTotal)), stat('지식', String(knN)), members.length ? el('span', { class: 'pv-stat' }, el('span', { class: 'k', text: '사람' }), el('b', { text: String(members.length) })) : null, el('span', { class: 'pv-stats-sp' }), p.updated_at ? el('span', { class: 'pv-when', text: '수정 ' + relTime(p.updated_at) }) : null);
        headEl.replaceChildren(el('div', { class: 'pv-eyebrow' }, el('span', { class: 'pv-idchip mono', text: '#' + p.id }), el('span', { class: 'pv-state' }, el('span', { class: 'v2-dot ' + st.dot, 'aria-hidden': 'true' }), el('span', { text: st.word })), p.list && p.list.name ? el('span', { class: 'pv-crumbsep', text: p.list.name }) : null), el('h1', { class: 'pv-title', text: p.name }), stats);
    }
    ...
    function zone(label) {
        ...
        const root = el('section', { class: 'pv-panel', 'aria-label': label }, el('div', { class: 'pv-panel-h' }, el('span', { class: 'pv-panel-k', text: label }), count, acts), sub, body, foot);
    ...
    function paintBody() {
        ...
        if (desc.trim()) {
            ...
            bz.body.replaceChildren(el('div', { class: 'pv-blank' }, el('p', { text: '아직 본문이 없어요.' }), el('p', { class: 'sub' }, el('button', { class: 'pv-link', type: 'button', text: '리브에게 써 달라고 하기', onclick: () => chat?.say(SAY_TIDY_NEW) }), el('span', { text: ' 또는 ' }), el('button', { class: 'pv-link', type: 'button', text: '직접 쓰기', onclick: () => startEdit() }))));
        }
    }
    ...
    const cz = zone('코멘트');
    const cIn = el('textarea', { class: 'pv-cmt-in', rows: '1', placeholder: '코멘트 남기기', 'aria-label': '코멘트 남기기' });
    const cSend = el('button', { class: 'pv-btn', type: 'button', text: '남기기' });
    ...
    function paintComments(list) {
        ...
        if (!list.length) {
            cz.body.replaceChildren(el('div', { class: 'pv-blank' }, el('p', { text: '아직 코멘트가 없어요.' }), el('p', { class: 'sub', text: '결정·부탁·메모를 여기 남기면 이 방의 모두가 봅니다.' })));
            return;
        }
        cz.body.replaceChildren(...list.map((c) => el('div', { class: 'pv-cmt' }, el('div', { class: 'pv-cmt-h' }, el('b', { text: c.display_name || c.actor || '?' }), el('span', { class: 'pv-cmt-ts', text: relTime(c.ts) })), el('div', { class: 'pv-cmt-b', text: c.body }))));
    ...
    async function loadComments() {
        try {
            ...
            if (!dead)
                cz.body.replaceChildren(el('div', { class: 'pv-blank' }, el('p', { text: '코멘트를 불러오지 못했어요.' })));
        }
    }
    ...
    const fst = { path: '' };
    let folderItems = []; // 현재 폴더 목록 — 업로드 덮어쓰기 사전확인(#877)·되그리기용
    ...
    function paintFolder(items) {
        ...
        if (!folderItems.length) {
            fz.body.replaceChildren(el('div', { class: 'pv-blank' }, el('p', { text: fst.path ? '빈 폴더예요.' : '아직 파일이 없어요.' }), el('p', { class: 'sub', text: '파일이나 폴더를 이 칸에 끌어다 놓으면 올라갑니다.' })));
            return;
        }
        ...
        fz.body.replaceChildren(...sorted.map((it) => {
            ...
            return el('button', { class: 'pv-row pv-frow', type: 'button', title: it.name,
                ...
                } } }, el('span', { class: 'pv-fic', 'aria-hidden': 'true' }, fileIconSvg(it.name, isDir)), el('span', { class: 'pv-row-main' }, el('span', { class: 't', text: it.name })), el('span', { class: 'pv-fmeta', text: isDir ? '폴더' : fmtSize(it.size), title: it.mtime ? fmtFileDateFull(it.mtime) : '' }), isDir ? el('span', { class: 'pv-row-go', 'aria-hidden': 'true', text: '›' }) : el('span', { class: 'pv-row-go', 'aria-hidden': 'true' }));
        }));
    }
    ...
    function paintSessions() {
        ...
        if (!sess.length) {
            sz.body.replaceChildren(el('div', { class: 'pv-blank' }, el('p', { text: '아직 이 방에서 돈 세션이 없어요.' }), el('p', { class: 'sub', text: '[＋ 새 세션]으로 시작하거나, 아래에서 리브에게 먼저 계획을 시켜 보세요.' })));
            return;
        }
        ...
        sz.body.replaceChildren(...sess.slice(0, 60).map((s) => {
            ...
            return el('a', { class: 'pv-row' + (s.live && s.alive ? '' : ' dim'), href: '#/s/' + encodeURIComponent(s.id), title: s.label }, el('span', { class: 'v2-dot ' + dotCls(s.stateKey), 'aria-hidden': 'true' }), el('span', { class: 'pv-row-main' }, el('span', { class: 't', text: tx.main }), el('span', { class: 'm', text: [tx.sub, s.stateLabel + (s.live ? '' : ' · 기록만'), when(s.lastSeen), owner].filter(Boolean).join(' · ') })), el('span', { class: 'pv-row-go', 'aria-hidden': 'true', text: '›' }));
        }));
    }
