...
export function filesPart(ctx) {
    ...
    const head = el('div', { class: 'pn-fhead' }, el('div', { class: 'pn-frow1' }, crumbs, count), el('div', { class: 'pn-ftools' }, el('span', { class: 'pn-upgrp' }, upBtn, upDirBtn), mkBtn, el('span', { class: 'pn-fsp' }), viewBtn, sizeIn, sortBtn));
    ...
    function menuFor(f, e) {
        ...
        if (f) {
            ...
            if (f.type !== 'dir')
                rows.push({ label: '뷰어에서 보기', run: () => {
                        ...
                        if (!ctx.paneRoot().querySelector('.pn-ed')) {
        ...
        if (upDirSupported())
            rows.push({ label: '폴더째 올리기…', run: () => upDirIn.click() });
        rows.push({ sep: true, label: '' });
