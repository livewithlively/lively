...
const KIND_LABEL = { proj: '프로젝트', know: '지식', src: '자료', sess: '세션', hist: '세션 이력', app: '화면' };
...
// ════════════════════════════════════════════
let box = null;
export function omniOpen(seed) {
    if (box) {
        const i = box.querySelector('.v2-omni-in');
        i?.focus();
        i?.select();
        return;
    }
    ...
    function localHits(q) {
        ...
        const sess = d.sessions
            .map((s) => ({ s, t: sessText(s, projName(d, s.projectId)) }))
        ...
        const proj = d.projects.filter((p) => String(p.name || '').toLowerCase().includes(nq)).slice(0, 6)
    ...
    function run() {
        ...
        api('/api/ui/v6/projects/semantic?limit=6&q=' + qs).then((r) => put('proj', ((r && r.projects) || []).map((p) => ({
        ...
        api('/api/ui/knowledge/semantic?limit=6&q=' + qs).then((r) => put('know', ((r && r.entries) || []).map((e) => ({
    ...
    function recent() {
        ...
        hits = [...d.sessions].sort((a, b) => b.lastSeen - a.lastSeen).slice(0, 6).map((s) => {
            ...
            return { kind: 'sess', key: 's:' + s.id, title: t.main || t.sub || s.id, sub: projName(d, s.projectId), href: '#/s/' + encodeURIComponent(s.id) };
        });
    }
    input.addEventListener('input', () => { window.clearTimeout(timer); timer = window.setTimeout(run, 200); });
    input.addEventListener('keydown', (e) => {
        if (e.isComposing)
            return; // 한글 조합 중 Enter 는 확정이지 열기가 아니다
        if (e.key === 'ArrowDown' || (e.key === 'n' && e.ctrlKey)) {
            e.preventDefault();
            if (rowNodes.length) {
                sel = (sel + 1) % rowNodes.length;
                mark();
            }
        }
            ...
            e.preventDefault();
            if (rowNodes.length) {
                sel = (sel + rowNodes.length - 1) % rowNodes.length;
                mark();
            }
        }
        else if (e.key === 'Enter') {
            e.preventDefault();
            go(sel, e.metaKey || e.ctrlKey || e.altKey);
        }
...
function onEsc(e) { if (e.key === 'Escape' && box) {
    e.stopPropagation();
    omniClose();
} }
export function omniClose() {
    if (!box)
        return;
    box.remove();
    box = null;
