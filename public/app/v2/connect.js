...
function stateOf(v, svc) {
    if (v.connected.some((s) => s.key === svc.key))
        return 'on';
    if (v.blockedOAuth.some((s) => s.key === svc.key))
        return 'blocked';
    return 'off';
}
// ══ 목록 (#/connect) ═══════════════════════════════════════════════════════════
export async function renderConnect(host) {
    ...
    function group(title, note, items, st) {
        ...
        if (!hit.length)
            return null;
        return el('section', { class: 'cn-group' }, el('div', { class: 'cn-group-h' }, el('span', { class: 'v2-k', text: `${title} · ${hit.length}` }), el('span', { class: 'cn-group-note', text: note })), el('div', { class: 'cn-list' }, ...hit.map((s) => row(s, st))));
    }
    function row(svc, st) {
...
function connMeta(v, svc) {
    const cred = svc.token ? v.credMap.get(svc.token) : null;
    const bits = [svc.oauth && v.oauthMap.get(svc.oauth)?.connected ? '계정 로그인' : '토큰'];
    if (cred?.has_secret) {
        if (cred.scope_key)
            bits.push(cred.scope_key);
        if (cred.last_used_at)
            bits.push('마지막 사용 ' + relTime(cred.last_used_at));
        else if (cred.updated_at)
            bits.push('연결 ' + relTime(cred.updated_at));
    }
    return bits.join(' · ');
}
// ══ 앱 상세 (#/connect/<key>) ══════════════════════════════════════════════════
...
export async function renderConnectApp(host, key) {
    ...
    if (!svc) {
        host.replaceChildren(el('div', { class: 'v2-center' }, backLink(), el('p', { class: 'v2-empty', text: '그런 앱이 없어요.' })));
        return;
    }
    ...
    const spec = svc.token ? CRED_KINDS.find((x) => x.kind === svc.token) : null;
    // ── 동작 줄 ──
    const acts = [];
    if (st === 'on') {
        if (viaOAuth)
            acts.push(el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '다시 연결', onclick: () => void startOAuth(svc, reload) }));
        if (viaToken)
            acts.push(el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '토큰 교체', onclick: () => openToken(svc, reload) }));
        acts.push(el('button', {
    ...
    if (viaToken && cred?.scope_key)
        facts.push(['대상', String(cred.scope_key)]);
    if (viaToken && cred?.last_used_at)
        facts.push(['마지막 사용', relTime(cred.last_used_at)]);
    ...
    if (svc.key === 'google' && org && org.admin)
        settings.push(googleTeamCollectCard());
...
function googleTeamCollectCard() {
    const body = el('div', { class: 'cn-help' }, ...uiText('불러오는 중…'));
    const box = el('section', { class: 'cn-sec' }, el('div', { class: 'cn-sec-h' }, el('span', { class: 'v2-k', text: '팀 자료로 모으기' })), body);
    const openConsent = (url, after) => {
        window.open(url, '_blank', 'noopener');
    ...
    const paint = async () => {
        let s;
        try {
            s = await api('/api/ui/org/google/collect');
        }
        catch (e) {
            body.replaceChildren(errorNote(e, '수집 상태를 불러오지 못했습니다'));
            return;
        }
        const cols = (s && s.collectors) || [];
        const of = (k) => cols.find((c) => c.service === k) || {};
        ...
        const picked = () => [...(dChk.checked ? ['drive'] : []), ...(gChk.checked ? ['gmail'] : [])];
        const chk = el('input', { type: 'checkbox' });
        chk.checked = anyOn;
        ...
        if (anyOn) {
            ...
            notes.push('연결은 돼 있어요 — 모을 서비스를 고르고 켜면 바로 시작합니다.');
        else if (s && s.ready)
            notes.push('켜면 구글 화면이 열려요 — [허용]만 누르면 바로 시작됩니다. 토큰이나 설정을 만질 일은 없어요.');
        else
            notes.push('구글 연결 준비가 아직 안 됐어요 — 아래에 구글 OAuth 클라이언트 값 두 개를 넣으면 열립니다.');
        // ★ 비용 고지 — Gmail 을 고른 순간(또는 이미 켜진 순간) 무슨 대가가 붙는지 먼저 말한다.
        const costLine = () => gChk.checked
        ...
        dChk.onchange = repaintCost;
        gChk.onchange = repaintCost;
        const svcRow = el('div', { class: 'cn-sec-form' }, el('label', { class: 'cn-toggle' }, dChk, el('span', { text: ' Google Drive 문서' })), el('label', { class: 'cn-toggle' }, gChk, el('span', { text: ' Gmail 메일' })));
        const extra = [];
        ...
        if (connected) {
            extra.push(el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '권한 넓히기',
                onclick: async () => {
                    try {
                        const r = await api('/api/ui/org/google/collect/connect', { method: 'POST', body: JSON.stringify({ services: picked() }) });
                        if (r && r.authorization_url)
                            openConsent(r.authorization_url, () => void paint());
                        else
                            toast('구글 화면을 열지 못했습니다', true);
                    }
                    catch (e) {
                        toast((e && e.message) || '구글 화면을 열지 못했습니다', true);
                    }
                } }));
        }
        if (!(s && s.ready) && !connected) {
            ...
            //  client_secret 은 이 사이트 계정의 비밀번호가 아니다 — type=password 금지(#1250), 텍스트칸+CSS 가림.
            const idIn = el('input', { type: 'text', placeholder: 'OAuth 클라이언트 ID', autocomplete: 'off', style: 'width:100%' });
            const secIn = el('input', { type: 'text', class: 'secret-input', placeholder: 'OAuth 클라이언트 보안 비밀', autocomplete: 'off', style: 'width:100%' });
            const saveBtn = el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '클라이언트 값 저장' });
            saveBtn.onclick = async () => {
                const cid = idIn.value.trim(), sec = secIn.value.trim();
                if (!cid || !sec) {
                    toast('ID 와 보안 비밀을 모두 넣어 주세요', true);
                    return;
                }
                saveBtn.disabled = true;
                try {
            ...
            extra.push(el('div', { class: 'cn-sec-form' }, idIn, secIn, saveBtn, el('p', { class: 'cn-help' }, ...uiText('Google Cloud 콘솔 ▸ API 및 서비스 ▸ 사용자 인증 정보에서 만든 **웹 애플리케이션** 클라이언트의 ID·보안 비밀입니다. 그 클라이언트의 승인된 리디렉션 URI 에 ' + location.origin + '/oauth/callback 이 등록돼 있어야 하고, OAuth 동의 화면은 반드시 게시(In production) 상태여야 합니다 — 테스트 상태면 7일마다 연결이 끊깁니다.'))));
        }
        chk.onchange = async () => {
            chk.disabled = true;
            const services = picked();
            if (chk.checked && services.length === 0) {
                toast('모을 서비스를 하나 이상 골라 주세요', true);
                chk.checked = false;
                chk.disabled = false;
                return;
            }
            try {
                const r = await api('/api/ui/org/google/collect', { method: 'POST', body: JSON.stringify({ enabled: chk.checked, services }) });
                if (r && r.needs_connect && r.authorization_url) {
                    ...
                    if (skipped.length)
                        toast(`${skipped.map((x) => x.service === 'gmail' ? 'Gmail' : 'Google Drive').join(' · ')} 는 아직 허용 전이라 켜지 못했어요 — [권한 넓히기]를 눌러 주세요`, true);
                    else
                        toast(chk.checked ? '팀 자료 모으기를 켰어요 — 첫 수집은 잠시 뒤 시작됩니다' : '팀 자료 모으기를 껐어요');
                }
            }
