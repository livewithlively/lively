...
export const ASIDE_MSG = { ping: 'lively-aside-ping', pong: 'lively-aside-pong', open: 'lively-aside-open' };
let opener = null;
let parentReady = false; // 부모(껍데기)가 '곁칸 내줄 수 있다'고 답했다
/** 껍데기(v2/main)가 자기 우패널을 내주며 한 번 채운다. */
export function setAsideGuestOpener(fn) { opener = fn; }
/** 지금 이 화면에서 곁칸을 쓸 수 있나 — 버튼을 그릴 때 문구·동작을 고르는 데 쓴다(막다른 버튼 금지). */
export function canOpenInAside() { return !!opener || parentReady; }
/** 열었으면 true. 곁칸을 못 쓰면 false — 부르는 쪽이 그 자리에서(제스처가 살아 있을 때) 새 창으로 폴백한다. */
export function openInAside(g) {
    if (opener)
        return opener(g);
    if (!parentReady)
        return false;
    try {
        window.parent.postMessage({ type: ASIDE_MSG.open, guest: g }, location.origin);
        return true;
    }
    catch {
        return false;
...
if (typeof window !== 'undefined' && window.parent && window.parent !== window) {
    window.addEventListener('message', (ev) => {
        if (ev.origin !== location.origin || ev.source !== window.parent)
            return;
        if (ev.data && ev.data.type === ASIDE_MSG.pong)
            parentReady = true;
    });
    const ping = () => { try {
        window.parent.postMessage({ type: ASIDE_MSG.ping }, location.origin);
    }
    catch { /* 부모가 닫혔다 */ } };
    ping();
