...
export async function loadOrgMcp() {
    if (!hasScope('admin'))
        return null;
...
export const orgServerOf = (org, key) => (org && org.servers.find((s) => s.name === key)) || null;
export const presetOf = (org, key) => (org && org.presets.find((p) => p.name === key)) || null;
/** 이 게이트웨이의 OAuth 콜백 — 상류 콘솔(구글·슬랙)의 '허용된 redirect URI' 에 그대로 넣는 값. */
const callbackUrl = () => location.origin.replace(/\/mcp$/, '').replace(/\/$/, '') + '/oauth/callback';
function copyRow(label, value) {
    return el('div', { class: 'cn-copy' }, el('div', { class: 'cn-copy-k', text: label }), el('code', { class: 'cn-copy-v', text: value }), el('button', {
...
async function openForOrg(preset, clientId, clientSecret) {
    ...
    if (clientId.trim()) {
        const seed = { client_id: clientId.trim() };
        if (clientSecret.trim())
            seed.client_secret = clientSecret.trim();
    ...
    try {
        const r = await api('/api/ui/org/mcp-server/refresh', { method: 'POST', body: JSON.stringify({ name: preset.name }) });
...
function guideBlock(preset) {
    const cb = callbackUrl();
    const out = [];
    const g = preset.guide;
    if (g && Array.isArray(g.steps) && g.steps.length) {
        const steps = el('ol', { class: 'cn-steps' }, ...g.steps.map((s) => {
            const li = el('li', {});
            String(s).split('{callback}').forEach((part, i, arr) => {
                li.append(...uiText(part));
                if (i < arr.length - 1)
                    li.append(el('code', { text: cb }));
            });
            return li;
        }));
        out.push(el('details', { class: 'cn-steps-wrap' }, el('summary', { class: 'cn-steps-sum', text: `${preset.label} 콘솔 설정 절차 ${g.steps.length}단계 — 처음이면 펼쳐 보세요` }), ...(g.intro ? [el('p', { class: 'cn-help' }, ...uiText(String(g.intro)))] : []), steps, ...(g.url ? [el('a', { class: 'btn btn-ghost btn-sm', href: g.url, target: '_blank', rel: 'noopener noreferrer', text: '설정 페이지 열기 ↗' })] : [])));
    }
    out.push(copyRow('콜백 주소(redirect URI)', cb));
    if (preset.oauth_scope)
        out.push(copyRow('필요한 허용범위(scope)', String(preset.oauth_scope)));
    if (preset.note)
        out.push(el('details', { class: 'cn-steps-wrap' }, el('summary', { class: 'cn-steps-sum', text: '이 앱에 대해 알아 둘 것' }), el('p', { class: 'cn-help' }, ...uiText(String(preset.note)))));
    return out;
}
/**
 ...
 */
export function orgAdminSection(svc, org, reload) {
    if (!org || !org.admin)
        return null;
    const server = orgServerOf(org, svc.key);
    const preset = presetOf(org, svc.key);
    const head = el('div', { class: 'cn-sec-h' }, el('span', { class: 'v2-k', text: '조직 설정' }), el('span', { class: 'cn-adm-badge', text: '관리자만 보여요' }));
    // ── 이미 열려 있다 ──
    if (server) {
        const n = (server.tools_snapshot && server.tools_snapshot.length) || 0;
        const acts = el('div', { class: 'cn-acts' }, el('button', {
            class: 'btn btn-ghost btn-sm', type: 'button', text: n ? '도구 목록 다시 받기' : '도구 발행하기',
            onclick: async (e) => {
                const b = e.currentTarget;
                b.disabled = true;
                try {
                    const r = await api('/api/ui/org/mcp-server/refresh', { method: 'POST', body: JSON.stringify({ name: server.name }) });
                    toast(`도구 ${r && r.tool_count != null ? r.tool_count : '?'}개를 받았어요`);
                    reload();
                }
                catch (err) {
                    toast((err && err.message) || '받지 못했습니다', true);
                    b.disabled = false;
                }
            },
        }), el('button', {
            ...
            onclick: async () => {
                if (!await confirmDialog({
                    ...
                    return;
                try {
                    await api('/api/ui/org/mcp-server/remove', { method: 'POST', body: JSON.stringify({ name: server.name }) });
                    toast('조직에서 닫았어요');
                    reload();
                }
                catch (e) {
                    toast((e && e.message) || '닫지 못했습니다', true);
                }
            },
        }));
        return el('section', { class: 'cn-sec cn-adm' }, head, el('p', { class: 'cn-help' }, ...uiText(n
            ? `**조직에 열려 있어요** — 팀 누구나 자기 계정으로 연결할 수 있습니다. AI가 쓸 수 있는 도구 ${n}개가 발행돼 있어요.`
            : '**조직에 열려 있지만 아직 도구가 발행되지 않았어요** — 발행해야 세션에서 실제로 호출할 수 있습니다.')), acts, el('p', { class: 'cn-help cn-muted' }, ...uiText('전송 방식·권한 등급·PII 마스킹 같은 세부 설정은 [관리 ▸ AI 능력 ▸ AI 도구 ▸ 외부 도구 서버(MCP)] 에 있어요.')));
    }
    // ── 아직 안 열렸는데 프리셋도 없다 — 여기서 대충 만들지 않고 관리탭으로 보낸다 ──
    if (!preset) {
        return el('section', { class: 'cn-sec cn-adm' }, head, el('p', { class: 'cn-help' }, ...uiText('이 앱은 기본 카탈로그에 없어서 여기서 한 번에 열 수 없어요 — 주소·인증 방식을 직접 정해야 합니다.')), el('p', { class: 'cn-help cn-muted' }, ...uiText('[관리 ▸ AI 능력 ▸ AI 도구 ▸ 외부 도구 서버(MCP)] 에서 등록하세요.')));
    }
    // ── 프리셋으로 한 번에 열 수 있다 ──
    const dcr = !!preset.dcr;
    const idIn = el('input', { class: 'cn-in', type: 'text', autocomplete: 'off', spellcheck: 'false', placeholder: '콘솔에서 발급받은 client_id' });
    const secIn = el('input', { class: 'cn-in', type: 'password', autocomplete: 'new-password', placeholder: 'client_secret (있으면)' });
    const openBtn = el('button', { class: 'btn btn-primary', type: 'button', text: '조직에 열기' });
    openBtn.addEventListener('click', async () => {
        if (!dcr && !idIn.value.trim()) {
            toast('이 앱은 콘솔에서 만든 client_id 가 필요해요', true);
            idIn.focus();
            return;
        }
        openBtn.disabled = true;
        try {
            await openForOrg(preset, dcr ? '' : idIn.value, dcr ? '' : secIn.value);
            reload();
        }
        catch (e) {
            toast((e && e.message) || '열지 못했습니다', true);
            openBtn.disabled = false;
        }
    });
    return el('section', { class: 'cn-sec cn-adm' }, head, el('p', { class: 'cn-help' }, ...uiText(dcr
        ? '**아직 조직에 열려 있지 않아요.** 이 앱은 별도 준비 없이 바로 열 수 있습니다 — 열면 팀 누구나 자기 계정으로 연결할 수 있어요.'
        : '**아직 조직에 열려 있지 않아요.** 이 앱은 먼저 그 서비스 콘솔에서 앱을 하나 만들어야 합니다(아래 절차) — 조직당 한 번이면 됩니다.')), ...(dcr ? [] : [
        el('div', { class: 'cn-guide' }, ...guideBlock(preset)),
        el('div', { class: 'cn-fields' }, el('label', { class: 'cn-f' }, el('span', { class: 'cn-f-k', text: 'client_id' }), idIn), el('label', { class: 'cn-f' }, el('span', { class: 'cn-f-k', text: 'client_secret' }), secIn)),
