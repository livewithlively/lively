...
function tailUrls(s, maxBytes) {
    const urls = [];
    if (s.live && !s.node)
        urls.push('/api/ui/terminal/sessions/' + encodeURIComponent(s.id) + '/transcript?tail=' + maxBytes);
    const raw = s.raw || {};
    const uuid = String(s.logId || raw.claudeSessionId || raw.chatId || '');
    const node = String(s.logNode ?? s.node ?? '');
    const log = (sid, nd) => '/api/ui/v6/sessions/' + encodeURIComponent(sid) + '/log?fmt=chat&tail=' + maxBytes + '&node=' + encodeURIComponent(nd);
    if (uuid)
        urls.push(log(uuid, node));
    if (uuid && node)
        urls.push(log(uuid, '')); // 노드 표식이 어긋나 있어도 게이트웨이 박스 좌표('')에 있을 수 있다
    if (!uuid && s.id)
        urls.push(log(s.id, node));
    return urls;
}
/** 대화 꼬리(ChatLine ndjson → 나/AI 턴). 사이드바 둘째 줄(last-ask.ts)도 이 길로 '내 마지막 말'을 받는다. */
export async function fetchTurns(s, maxBytes) { return (await fetchTail(s, maxBytes)).turns; }
...
export async function fetchLastAsk(s, maxBytes) {
    const t = await fetchTail(s, maxBytes);
    //  "y"·"r" 같은 한두 글자 답은 질문이 아니다 — 그 앞의 제대로 된 말을 쓴다(없으면 그거라도).
    const pick = (arr) => [...arr].reverse().find((x) => x.length >= 4) || arr[arr.length - 1] || null;
    const text = pick(t.prompts) || pick(t.turns.filter((x) => x.who === 'me').map((x) => x.text));
    return { text, ok: t.ok };
}
async function fetchTail(s, maxBytes) {
    const hit = tailCache.get(s.id);
    if (hit && Date.now() - hit.at < 7000 && hit.turns.length)
        return hit;
    let got = { turns: [], prompts: [] };
    let ok = false;
    for (const url of tailUrls(s, maxBytes)) {
        try {
            ...
            if (got.turns.length || got.prompts.length)
                break; // 글이 나온 자리가 정답 — 빈 200(좌표 어긋남)은 다음 후보로
        }
...
function parseTurns(text) {
    const turns = [];
    const prompts = []; // 클로드 코드 last-prompt 레코드들 — 파일 순서 = 시간 순서
    for (const line of text.split('\n')) {
        ...
        if (o.type === 'last-prompt') {
            const lp = String(o.lastPrompt || '').replace(/\s+/g, ' ').trim();
            if (lp && !INJ_RE.test(lp))
                prompts.push(lp);
            continue;
        }
...
export function sessTitle(s, projectName) {
...
export async function lookupSessNames(list, projectName, onFound) {
    ...
    let got = false;
    for (const s of list) {
        if (askedName.has(s.id) || nameCache.has(s.id))
            continue;
        if (norm(sessText(s, projectName).main) !== norm(projectName))
            continue;
        askedName.add(s.id);
...
export function seedSessName(sid, text) {
...
function sessionsPart(ctx) {
    ...
    function mountStage() {
        if (composing || !sel) {
            ...
            window.setTimeout(() => { grow(); if (fresh && !handIsElsewhere())
                ta.focus(); }, 0);
            return;
        }
...
// ══ 지식 — 이 프로젝트에 연결된 문서 ══════════════════════════════════════════
function knowledgePart(ctx) {
    const root = el('div', { class: 'pn-part pn-kn' });
    const note = pnNote('lively_pn_kn_note', '지식은 세션들이 일하면서 계속 쓰고 고치는 글이고, 이 워크스페이스의 모든 세션이 함께 봅니다. 쌓일수록 맥락이 촘촘해져요.');
    const list = el('div', { class: 'pn-knlist' });
    root.append(note, list);
    let sig = '';
    function paint() {
        const kn = (ctx.detail()?.project || {}).knowledge || {};
        const pick = (arr, rel) => (arr || []).map((k) => ({
            name: String(k.name), title: String(k.title || ''), rel,
            type: k.type ?? null, lifecycle: k.lifecycle ?? null,
        }));
        const all = [...pick(kn.required, '필요'), ...pick(kn.produced, '산출')];
        const s2 = all.map((k) => k.rel + k.name + k.title).join('|');
        if (s2 === sig)
            return;
        sig = s2;
        if (!all.length) {
            list.replaceChildren(el('div', { class: 'pn-empty' }, pnIcon('doc', 'pn-i big'), el('b', { text: '연결된 지식이 아직 없어요.' }), el('p', { class: 'pn-fine', text: '세션이 만든 결론을 지식으로 남기면 여기 모입니다.' })));
            return;
        }
        list.replaceChildren(...all.map((k) => {
            ...
            }, pnIcon('doc', 'pn-i sm'), el('span', { class: 'n ell1', text: knTitle(k.title, k.name) }), el('span', { class: 'pn-knrel' + (k.rel === '산출' ? ' prod' : ''), text: k.rel }));
            return row;
        }));
    }
    ...
    async function openKnModal(k) {
        ...
        const onKey = (e) => { if (e.key === 'Escape')
            close(); };
        const box = el('div', { class: 'pn-modal pn-knm', role: 'dialog', 'aria-label': '지식 요약' }, el('div', { class: 'pn-modal-h' }, el('h2', { text: knTitle(k.title, k.name) }), el('button', { class: 'pn-modal-x', type: 'button', 'aria-label': '닫기', onclick: () => close() }, pnIcon('x', 'pn-i sm'))), el('div', { class: 'pn-modal-b' }, bodyEl), el('div', { class: 'pn-modal-f' }, el('span', { class: 'pn-fine ell', text: k.name }), el('a', { class: 'btn btn-primary btn-sm', href: '#/k/' + encodeURIComponent(k.name), onclick: () => close() }, el('span', { text: '전체 보기' }))));
        back.onclick = () => close();
        document.addEventListener('keydown', onKey);
        document.body.append(back, box);
        const d = await api('/api/ui/knowledge/' + encodeURIComponent(k.name)).catch(() => null);
        if (!box.isConnected)
            return;
        const kd = (d && (d.knowledge || d)) || null;
        if (!kd) {
            bodyEl.replaceChildren(el('p', { class: 'pn-fine', text: '내용을 불러오지 못했어요 — [전체 보기]로 열어 주세요.' }));
            return;
        }
        const meta = el('div', { class: 'pn-knm-meta' }, el('span', { class: 'pn-knrel' + (k.rel === '산출' ? ' prod' : ''), text: k.rel }), ...(kd.type ? [el('span', { class: 'pn-knm-tag', text: String(kd.type) })] : []), ...((kd.categories || []).slice(0, 2).map((c) => el('span', { class: 'pn-knm-tag', text: String(c.name || c.key) }))), ...(kd.updated_at ? [el('span', { class: 'pn-fine', text: relTime(String(kd.updated_at)) + ' 갱신' })] : []));
        ...
        bodyEl.replaceChildren(...(full && full !== short ? [el('p', { class: 'pn-knm-full', text: full })] : []), meta, ...(kd.summary ? [el('p', { class: 'pn-knm-sum', text: String(kd.summary) })] : []), el('div', { class: 'pn-md' }, renderMarkdown(md.slice(0, 6000))), ...(md.length > 6000 ? [el('p', { class: 'pn-fine', text: '…여기까지만 보여요. 전문은 [전체 보기]에서.' })] : []));
    }
    paint();
...
function tasksPart(ctx) {
    ...
    function paint() {
        ...
        sig = s2;
        if (!tasks.length) {
            root.replaceChildren(el('div', { class: 'pn-empty' }, pnIcon('task', 'pn-i big'), el('b', { text: '할 일이 아직 없어요.' }), el('p', { class: 'pn-fine', text: '프로젝트 정보에서 더하거나, 세션에 "태스크로 나눠 줘"라고 시켜 보세요.' })));
            return;
        }
        const done = tasks.filter((t) => t.status_category === 'done').length;
        root.replaceChildren(el('div', { class: 'pn-head' }, el('span', { class: 'pn-fine', text: `${done}/${tasks.length} 끝냈어요` })), el('div', { class: 'pn-tlist' }, ...tasks.map((t) => {
            const isDone = t.status_category === 'done';
            return el('div', { class: 'pn-trow' + (isDone ? ' done' : '') }, el('button', {
...
function timelinePart(ctx) {
    const root = el('div', { class: 'pn-part pn-tl' });
    const empty = el('div', { class: 'pn-empty' }, pnIcon('clock', 'pn-i big'), el('b', { text: '아직 보고 있는 세션이 없어요.' }), el('p', { class: 'pn-fine', text: '세션을 하나 열면 그 세션에 시킨 일과 남긴 것이 여기 시간순으로 쌓입니다.' }));
    // 발자취는 **한 벌뿐**이라 자리도 하나다. 같은 타임라인을 두 칸에 놓으면 보이는 칸이 임자고, 나머지 칸은
    //  뺏지 않고 그렇다고 말한다 — 서로 뺏으면 8초마다 발자취가 이 칸 저 칸으로 튄다.
    const elsewhere = el('div', { class: 'pn-empty' }, pnIcon('clock', 'pn-i big'), el('b', { text: '이 세션의 발자취는 다른 칸에 떠 있어요.' }), el('p', { class: 'pn-fine', text: '발자취는 한 벌이라 한 칸에만 뜹니다. 그 칸을 닫거나 다른 탭으로 바꾸면 이 자리로 옵니다.' }));
    const show = (node) => { if (root.firstChild !== node)
    ...
    function paint() {
        ...
        if (host.parentElement === root)
            return; // 내가 들고 있다
        if (host.isConnected && host.offsetParent) {
            show(elsewhere);
            return;
        } // 보이는 칸이 임자다
        show(host);
...
function livPart(ctx) {
    const root = el('div', { class: 'pn-part pn-liv' });
    let chat = null;
    if (ctx.id > 0)
        chat = mountProjectChat(root, { projectId: ctx.id, onTurnDone: () => ctx.onChanged?.() });
    else
...
function archivePart(ctx) {
    ...
    function paint() {
        ...
        sig = s2;
        if (!ss.length) {
            root.replaceChildren(el('div', { class: 'pn-empty' }, pnIcon('box', 'pn-i big'), el('b', { text: '보관한 세션이 아직 없어요.' }), el('p', { class: 'pn-fine', text: '세션 머리줄 [⋯ ▸ 이 세션 보관]으로 담깁니다 — 대화는 그대로 남고, 언제든 되살릴 수 있어요.' })));
            return;
        }
        const rows = ss.map((s) => {
...
function webPart(ctx) {
    ...
    const pushTrail = (u) => {
        const t = trail();
        if (t.urls[t.at] === u)
            return; // 같은 주소를 다시 = 새로고침이지 이동이 아니다
    ...
    const step = (d) => {
        if (live) {
            const v = frame;
            try {
                if (d < 0 ? v.canGoBack?.() : v.canGoForward?.()) {
                    if (d < 0)
                        v.goBack();
                    else
                        v.goForward();
    ...
    const reload = () => {
        if (asleep) {
            const u0 = asleep;
            asleep = '';
            frame.setAttribute('src', u0);
            return;
        } // 자고 있었다 → 깨우는 것이 곧 다시 싣기다
    ...
    const openTab = el('a', { class: 'pn-web-btn ic', target: '_blank', rel: 'noopener', title: '지금 주소를 새 탭에서 엽니다.', 'aria-label': '새 탭에서 열기', href: '#' }, pnIcon('ext', 'pn-i sm'));
    ...
    const backBtn = el('button', { class: 'pn-web-btn ic', type: 'button', 'aria-label': '뒤로',
        ...
        onclick: () => step(-1) }, pnIcon('chev', 'pn-i sm'));
    ...
    const zoomOut = el('button', { class: 'pn-web-btn ic', type: 'button', title: '축소합니다.', 'aria-label': '축소', onclick: () => stepZoom(-1) });
    zoomOut.textContent = '−';
    const zoomIn = el('button', { class: 'pn-web-btn ic', type: 'button', title: '확대합니다.', 'aria-label': '확대', onclick: () => stepZoom(1) });
    zoomIn.textContent = '+';
    const zoomLbl = el('button', { class: 'pn-web-btn zl', type: 'button', onclick: () => { setZoom(zooms.get(keyOf()) == null ? 1 : null); } });
    const stage = el('div', { class: 'pn-webstage' }, frame);
    root.append(el('div', { class: 'pn-web-bar' }, el('span', { class: 'pn-web-navs' }, backBtn, fwdBtn, el('button', { class: 'pn-web-btn ic', type: 'button', title: '이 칸만 다시 불러옵니다 — ⌘R(윈도는 Ctrl+R)도 같습니다.', 'aria-label': '다시 불러오기', onclick: () => reload() }, pnIcon('undo', 'pn-i sm'))), input, el('span', { class: 'pn-web-navs' }, zoomOut, zoomLbl, zoomIn), openTab), stage, live ? null : el('p', { class: 'pn-web-note pn-fine', text: '빈 화면인가요? 그 사이트가 창 안에 뜨는 걸 막은 거예요 — 오른쪽 ↗ 로 새 탭에서 여세요. 데스크톱 앱에서는 이 칸 안에 그대로 뜹니다.' }));
    ...
    function applyZoom() {
        ...
        if (live) {
            try {
                frame.setZoomFactor?.(z);
            }
            catch (_) { /* 아직 안 붙었다 — dom-ready 에서 다시 건다 */ }
    ...
    function setZoom(z) { zooms.set(keyOf(), z); applyZoom(); }
    function stepZoom(d) {
        const cur = effZoom();
    ...
    if (live) {
        ...
        for (const ev of ['did-navigate', 'did-navigate-in-page', 'did-finish-load', 'dom-ready'])
            frame.addEventListener(ev, sync);
    }
    ...
    function adopt() {
        ...
        const t = trail();
        if (u && t.urls[t.at] !== u)
            pushTrail(u); // 이 세션의 길이 비어 있으면 지금 주소를 첫 걸음으로
        if (u === (frame.getAttribute('src') || '')) {
            input.value = u;
            syncNav();
            return;
        }
    ...
    const ro = typeof ResizeObserver === 'function' ? new ResizeObserver(() => { if (zooms.get(keyOf()) == null)
        applyZoom(); }) : null;
    ro?.observe(stage);
    if (live)
        frame.addEventListener('dom-ready', () => applyZoom());
    ...
    const watch = window.setInterval(() => {
        ...
        if (Date.now() - hiddenSince >= SLEEP_AFTER_MS)
            sleep();
    }, 5000);
...
function viewerPart(ctx) {
    ...
    const remembered = () => {
        try {
            return (JSON.parse(localStorage.getItem(KEY) || '{}') || {})[ctx.memKey()] || '';
        }
        catch (_) {
            return '';
        }
    ...
    const upBtn = (withText) => el('button', {
    ...
    function paintBar() {
        ...
        bar.replaceChildren(el('button', { class: 'pn-web-btn', type: 'button', text: '← 자료', title: '파일 목록으로 돌아갑니다', onclick: () => void open('') }), el('b', { class: 'pn-ed-name ell', text: base(path), title: path }), upBtn(false), el('a', { class: 'pn-web-btn', href: fileUrl(path) + '&download=1', download: base(path), title: '내려받기' }, pnIcon('drop', 'pn-i sm')));
    }
    ...
    function paintPicker() {
        ...
        const draw = () => {
            ...
            if (!hit.length) {
                rows.replaceChildren(el('div', { class: 'pn-empty' }, pnIcon(list.length ? 'doc' : 'up', 'pn-i big'), el('b', { text: list.length ? '찾는 자료가 없어요.' : '아직 자료가 없어요.' }), el('p', { class: 'pn-fine', text: list.length
                        ? '이름 일부로 다시 찾아보세요.'
                        : '파일을 이 칸에 끌어다 놓거나 [올리기]를 누르세요 — 올린 파일은 자료에도 그대로 쌓입니다.' }), ...(list.length ? [] : [upBtn(true)])));
                return;
            }
            rows.replaceChildren(...hit.slice(0, 300).map((f) => {
                const k = kindOf(f.path);
                return el('button', { class: 'pn-frow2', type: 'button', title: f.path, onclick: () => void open(f.path) }, el('span', { class: 'pn-fic sm ' + k.kind }, pnIcon(k.kind === 'img' || k.kind === 'video' ? 'img' : 'doc', 'pn-i')), el('b', { class: 'pn-fname1', text: base(f.path) }), el('span', { class: 'pn-fcol k', text: k.type }), el('span', { class: 'pn-fcol s', text: fmtSize(f.size || 0) }), el('span', { class: 'pn-fcol d', text: f.mtime ? relTime(new Date(f.mtime).toISOString()) : '' }));
            }));
        };
        search.addEventListener('input', () => { q = search.value; draw(); });
        draw();
        body.replaceChildren(el('div', { class: 'pn-ed-pick2' }, search, rows));
        window.setTimeout(() => { if (q)
            search.focus(); }, 0);
    }
    async function open(p2) {
        path = p2;
        remember(p2);
        paintBar();
        if (!p2) {
        ...
        if (k.kind === 'text' || k.kind === 'page') {
            ...
            if (!r || !r.ok) {
                body.replaceChildren(el('p', { class: 'pn-fine', style: 'padding:14px', text: '파일을 읽지 못했어요.' }));
                return;
            }
            ...
            if (k.kind === 'page') {
                ...
                const f = el('iframe', { class: 'pn-ed-pv full', sandbox: '', tabindex: '-1' });
        ...
        body.replaceChildren(el('div', { class: 'pn-empty' }, pnIcon('doc', 'pn-i big'), el('b', { text: '이 형식은 브라우저에서 볼 수 없어요.' }), el('p', { class: 'pn-fine', text: '파워포인트·워드·엑셀·한글은 브라우저가 그릴 방법이 없습니다. 위 [내려받기]로 원래 앱에서 여세요.' })));
    }
    if (!(ctx.id > 0)) {
    ...
    const onSend = (e) => {
        const d = e.detail;
        if (!d || Number(d.id) !== ctx.id || !d.path)
            return;
        if (!list.some((f) => f.path === d.path))
            void loadList();
        void open(d.path);
    };
    ...
    return {
        root,
        tick: () => { void loadList().then(() => { if (!path)
            paintPicker(); paintBar(); }); },
...
function appsPart(ctx) {
    ...
    const list = async () => {
        ...
        if (ctx.dead())
            return;
        if (!apps.length) {
            root.replaceChildren(el('div', { class: 'pn-empty' }, el('b', { text: '설치된 앱이 없어요.' }), el('p', { class: 'pn-fine', text: '관리자가 앱을 설치하면 여기서 열 수 있어요.' })));
            return;
        }
        root.replaceChildren(el('div', { class: 'pn-apps-grid' }, ...apps.map((a) => {
            ...
            return el('button', { class: 'pn-app', type: 'button',
                ...
                onclick: () => { if (hasUi)
                    void openInstalledApp(a, projectId);
                else
                    void openAppSession(a.id, { title: a.title, projectId }); } }, el('span', { class: 'pn-app-ic' }, pnIcon(hasUi ? 'grid' : 'chat', 'pn-i')), el('b', { text: a.title }), el('span', { class: 'pn-fine', text: hasUi ? '앱 탭' : '앱 세션 탭' }));
        })));
    };
    void list();
    return { root };
...
export function makePart(type, ctx) {
    ...
    if (type === 'knowledge')
        return knowledgePart(ctx);
    if (type === 'tasks')
        return tasksPart(ctx);
    if (type === 'timeline')
        return timelinePart(ctx);
    if (type === 'apps')
        return appsPart(ctx);
    return livPart(ctx);
}
