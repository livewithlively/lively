...
export function hiddenSessions() {
    try {
        const a = JSON.parse(localStorage.getItem(HIDE_KEY) || '[]');
...
function saveHidden(set) {
    try {
        localStorage.setItem(HIDE_KEY, JSON.stringify([...set].slice(-800)));
    }
    catch (_) { /* noop */ }
}
export function hideSession(id) { const s = hiddenSessions(); s.add(id); saveHidden(s); }
export function unhideSession(id) { const s = hiddenSessions(); s.delete(id); saveHidden(s); }
/** 탭 줄을 쥔 쪽(panes.ts)에 '세션 보기가 바뀌었다'를 알린다 — 8초 틱을 기다리지 않고 그 자리에서 다시 그리게. */
export function sessionsViewChanged() { window.dispatchEvent(new Event('pn:sessions-view')); }
// ── 아이콘(스트로크 SVG) ──────────────────────────────────────────────────────
const ICON_PATHS = {
...
function archivePart(ctx) {
    ...
    function paint() {
        ...
        const rows = ss.map((s) => {
            ...
            return el('div', { class: 'pn-arow' + (busy ? ' busy' : '') }, el('span', { class: 'v2-dot ' + (live ? 'idle' : keep ? 'idle' : ''), 'aria-hidden': 'true' }), el('a', { class: 'n ell', href: '#/s/' + encodeURIComponent(s.id), title: t + ' — 대화를 봅니다', text: t }), el('span', { class: 'pn-fine w', text: live ? '치운 탭 · 도는 중' : keep ? '보관됨' : '끝난 세션' }), ...(hid.has(s.id) ? [el('button', {
                    class: 'btn-text', type: 'button', text: '탭에 꺼내기', title: '세션 탭 줄에 다시 세웁니다.',
                    onclick: () => { unhideSession(s.id); sessionsViewChanged(); sig = ''; paint(); },
                })] : []), ...(s.lastSeen ? [el('span', { class: 'pn-fine w', text: relTime(new Date(s.lastSeen).toISOString()) })] : []), ...(keep && !live ? [el('button', {
