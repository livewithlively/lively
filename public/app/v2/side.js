...
const grpAuto = new Set(); // 자동으로 펴진 것 — 이 창 수명만(정산에서 걷힌다)
const grpOrder = new Map();
let grpSeq = 0;
 ...
 */
export function settleSideGroups() { grpSettle = true; }
let openSet = new Set();
...
let appPinned = new Set(); // ★고정 = 사람이 고른 앱 인스턴스를 맨 위로(#1954)
...
let tidyOn = false;
const tidySel = new Set(); // 고른 프로젝트 키('p:123')
let showDone = false;
let mineOnly = false;
let sideFilter = '';
...
function init() {
    ...
    try {
        ...
        binsPinned = localStorage.getItem(BINS_KEY) === '1';
    }
    catch (_) { /* noop */ }
...
const isMine = (s) => !!s.owned || (!!meId() && String((s.raw && s.raw.owner) || '') === meId());
 ...
 *   정보가 줄었으니 줄도 줄인다. 그때 사라지는 것(소속·시각·상태어)은 툴팁이 받는다. */
function appRowEl(inst, one = false) {
    const tip = one
        ? [inst.title, [inst.status ? inst.status.label : '', inst.at ? when(inst.at) : ''].filter(Boolean).join(' · ')].filter(Boolean).join('\n')
        : inst.title;
    return el('div', { class: 'v2-app-inst' + (one ? ' v2-app-inst--1' : '') + (inst.active ? ' on' : '') + (inst.status ? ' st-' + inst.status.key : ''), role: 'listitem', 'data-instance': inst.id }, el('button', { class: 'v2-app-inst-open', type: 'button', title: tip, 'aria-current': inst.active ? 'page' : null,
...
const PROMOTES = { waiting: true, done: true };
const rowRank = (r) => (r.status ? (SESS_STATES[r.status.key] ? SESS_STATES[r.status.key].rank : 9) : 9);
const rowAt = (r) => Number(r.at) || 0;
/**
...
function projGroups(rest, searching) {
    ...
    for (const g of groups) {
        ...
        let ent;
        if (live)
            ent = prev && prev.tier === 1 ? prev : { tier: 1, seq: ++grpSeq, at: g.at };
        else if (!prev)
            ent = { tier: 2, seq: 0, at: g.at };
        else if (prev.tier === 1)
            ent = (settle && !g.active) ? { tier: 2, seq: 0, at: g.at } : prev;
        ...
        if (searching)
            ...
            g.open = true;
        else if (g.active || g.tier === 1) {
            g.open = true;
            grpAuto.add(g.key);
        }
        else if (grpAuto.has(g.key)) {
            if (settle) {
                grpAuto.delete(g.key);
                g.open = false;
            }
            else
                g.open = true;
...
function projGrpHead(g) {
    ...
    g.rows.length > 1 ? el('span', { class: 'v2-cnt', text: String(g.rows.length) }) : null), g.id ? newSessBtn(g.id) : el('span', { class: 'v2-newb', 'aria-hidden': 'true' }));
}
...
function projListKids(shown, q) {
    ...
    if (pinned.length) {
        kids.push(el('div', { class: 'v2-app-group', role: 'presentation', text: PINNED_LABEL }));
        for (const r of pinned)
            kids.push(appRowEl(r));
    }
    ...
    for (const g of groups) {
        if (both && !q && g.tier !== lastTier) {
            kids.push(el('div', { class: 'v2-app-group', role: 'presentation', text: g.tier === 1 ? PRIORITY_LABEL : REST_LABEL }));
            lastTier = g.tier;
        }
        //  ★펼친 묶음 = 흰 카드 그릇 — 세션이 프로젝트의 **안**에 산다는 걸 면(面)이 말한다.
        //   들여쓰기+세로선만으로는 "목록 둘이 이웃한 그림"으로 읽혔다(상민님 2026-08-18, 트리 .v2-pj.open 과 같은 처방).
        kids.push(el('div', { class: 'v2-pg' + (g.open ? ' open' : '') }, projGrpHead(g), g.open ? el('div', { class: 'v2-pg-list' }, ...g.rows.map((r) => appRowEl(r, true))) : null));
    }
    return kids;
}
...
function appListKids(shown, q) {
    const kids = groupProj ? projListKids(shown, q) : [];
    if (!groupProj) {
...
function findInput(ph) {
    return el('input', { class: 'v2-find-in', type: 'search', placeholder: ph, 'aria-label': ph, value: sideFilter,
        ...
        onblur: () => { findComposing = false; if (!sideFilter && findOpen)
...
/** 발치의 '갈 곳' 한 줄(세션 이력 등) — 목록의 항목이 아니라 같은 급의 문이다. */
...
function renderLegacy() {
    ...
    navSplitter(navEl), el('div', { class: 'v2-side-sec' }, countEl, findBtn(), filterBtn(fltN, liveAll, doneCount), 
    ...
    ...(findShown() ? [el('div', { class: 'v2-find' }, findIn)] : []), ...(fltN ? [filterSummary(fltN)] : []), 
    // 새 프로젝트 줄은 트리 **밖**·바로 위다 — 안에 두면 목록을 스크롤할 때 치던 칸이 위로 사라진다.
    ...(newOpen ? [newProjRow()] : []), treeEl, ...(tidyOn ? [tidyBar()] : []), 
    // 아카이브·휴지통 두 행(#1851) — 기본은 트리 맨 아래(renderTree 가 붙인다). 사람이 [아래 고정]을 켜면 여기(트리 밖,
    //  스크롤과 무관한 자리)에 선다 — 목록이 수백 행이어도 늘 닿는다.
    ...(binsPinned ? [el('div', { class: 'v2-bins v2-bins-fixed' }, ...binRows(data))] : []), el('div', { class: 'v2-side-foot' }, 
    ...
    const newIn = host.querySelector('.v2-npj-in');
    if (newIn) {
        if (newHad) {
            newIn.focus();
            if (newSel && newSel[0] != null)
                newIn.setSelectionRange(newSel[0], newSel[1]);
        }
        else if (newFocusWanted) {
            newFocusWanted = false;
            newIn.focus();
        }
    }
    bindFindKey();
...
function tidyBtn() {
    ...
    }, sv('svg', { viewBox: '0 0 24 24', class: 'v2-flt-ic', 'aria-hidden': 'true' }, sv('path', { d: 'M13 5h8m-8 7h8m-8 7h8' }), sv('path', { d: 'M3 17l2 2l4-4' }), sv('path', { d: 'M3 7l2 2l4-4' })));
    b.onclick = (e) => { e.preventDefault(); setTidy(!tidyOn); };
    return b;
}
function setTidy(on) {
    tidyOn = on;
    if (!on)
        tidySel.clear(); // 끄면 고른 것도 비운다 — 안 보이는 선택이 남아 있으면 다음에 켤 때 놀란다
    redraw();
}
/** 고른 프로젝트를 한 번에 아카이브로. 확인은 **한 번만** 받는다 — N번 물으면 그건 '한 번에'가 아니다. */
async function tidyArchive() {
    if (!last || !tidySel.size)
        return;
    ...
    for (const r of rows) {
        ...
        try {
            ...
            for (const sx of r.live.filter(isMine)) {
                ...
                try {
                    await api('/api/ui/terminal/sessions/' + encodeURIComponent(sx.id) + q, { method: 'DELETE' });
                }
                catch (_) { /* 아래 보관은 계속 */ }
            }
...
let tidyBarEl = null;
function tidyBar() {
    const bar = el('div', { class: 'v2-tidybar' });
    tidyBarEl = bar;
    paintTidyBar();
    return bar;
}
function paintTidyBar() {
    if (!tidyBarEl)
        return;
    const n = tidySel.size;
    const go = el('button', { class: 'btn btn-primary btn-sm', type: 'button', text: n > 1 ? `${n}개를 아카이브로` : '아카이브로' });
    if (!n)
        go.disabled = true;
    go.onclick = () => void tidyArchive();
    tidyBarEl.replaceChildren(el('span', { class: 'v2-tidybar-k', text: n ? `${n}개 선택됨` : '정리할 프로젝트를 고르세요' }), go, el('button', { class: 'btn-text', type: 'button', text: '끝내기', onclick: () => setTidy(false) }));
}
...
const navNatural = (nav) => (nav.scrollHeight > 0 ? nav.scrollHeight : 149);
const navSnap = (px, nat) => (px <= NAV_MIN + NAV_SNAP ? NAV_MIN : px >= nat - 12 ? nat : px);
...
function navSplitter(nav) {
    ...
    const h = makeSplitter({
        ...
        onDrag: (px) => { const v = navSnap(px, navNatural(nav)); if (v !== px)
            nav.style.setProperty('--v2-navh', v + 'px'); },
...
function newBtn() {
    const b = el('button', {
        ...
        onclick: (e) => { e.preventDefault(); if (newOpen)
            closeNew();
        else
            openNew(); },
...
function axisBtn() {
    ...
    }, groupProj ? glyph('folder', 'v2-axisbtn-ic') : icon('list', 'v2-axisbtn-ic'));
}
function findBtn() {
    const on = findShown();
...
function renderTree(rowsIn) {
    ...
    if (selectedPk && !closedSelected.has(selectedPk) && !openSet.has(selectedPk)) {
        ...
        if (r && (r.live.length || r.past.length)) {
            openSet.add(selectedPk);
...
function binPinBtn() {
    ...
    return el('button', { class: 'v2-pinb v2-bin-pin' + (on ? ' on' : ''), type: 'button', 'aria-pressed': String(on),
        ...
        onclick: (e) => { e.preventDefault(); e.stopPropagation(); binsPinned = !binsPinned; saveFlag(BINS_KEY, binsPinned); redraw(); } }, sv('svg', { viewBox: '0 0 24 24', class: 'v2-pinb-ic', 'aria-hidden': 'true' }, sv('path', { d: PIN_NEEDLE }), sv('path', { d: PIN_BODY })));
}
function projRow(r, sess, past, activeKey, selectedPk) {
    ...
    if (p && tidyOn) {
        ...
        row.insertBefore(el('span', { class: 'v2-ck', 'aria-hidden': 'true' }, sv('svg', { viewBox: '0 0 24 24', class: 'v2-ck-ic' }, sv('path', { d: 'M5 12.5l4.5 4.5L19 7.5' }))), row.firstChild);
        row.addEventListener('click', (e) => {
            if (e.metaKey || e.ctrlKey)
                return; // ⌘클릭으로 새 탭에 여는 길은 남긴다
            e.preventDefault();
            e.stopPropagation();
            if (tidySel.has(pk))
                tidySel.delete(pk);
...
function trashBtn(s) {
    ...
    }, sv('svg', { viewBox: '0 0 24 24', class: 'v2-ss-x-ic', 'aria-hidden': 'true' }, sv('path', { d: 'M4 7h16' }), sv('path', { d: 'M9 7V4h6v3' }), sv('path', { d: 'M6 7l1 13h10l1-13' })));
    btn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); void doTrash(s); });
    return btn;
}
async function doTrash(s) {
    ...
    try {
        ...
        if (r.skipped.length && !r.done.length) {
            toast(r.skipped[0].why || '휴지통으로 보내지 못했습니다', true);
            return;
        }
...
async function setArchived(p, archived, myLiveN) {
    if (archived && !await confirmProjectArchive({ name: p.name, liveN: myLiveN }))
        return;
    try {
        if (archived && last) {
            ...
            for (const s of mine) {
                ...
                try {
                    ...
                    toast(`「${sessText(s, p.name).main}」을(를) 멈추지 못했어요 — ${(e && e.message) || e}`, true);
                }
            }
        }
        ...
        toast((e && e.message) || (archived ? '아카이브로 보내지 못했습니다' : '보관을 해제하지 못했습니다'), true);
    }
}
