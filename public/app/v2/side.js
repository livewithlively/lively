...
function renderHomeApps() {
    ...
    const rowEl = (inst) => el('div', { class: 'v2-app-inst' + (inst.active ? ' on' : '') + (inst.status ? ' st-' + inst.status.key : ''), role: 'listitem', 'data-instance': inst.id }, el('button', { class: 'v2-app-inst-open', type: 'button', title: inst.title, 'aria-current': inst.active ? 'page' : null,
    ...
    for (const inst of shown) {
        const g = inst.group || '';
        if (g && g !== lastGroup) {
            rows.push(el('div', { class: 'v2-app-group', role: 'presentation', text: g }));
            lastGroup = g;
        }
        rows.push(rowEl(inst));
    }
    const listEl = el('div', { class: 'v2-app-list', role: 'list', 'aria-label': '열린 앱' }, ...rows, ...(!shown.length ? [el('div', { class: 'v2-app-empty' }, el('p', { text: q ? '찾는 열린 앱이 없어요.' : '열린 앱이 없어요.' }), q ? el('button', { class: 'btn-text', type: 'button', text: '검색 지우기', onclick: () => { sideFilter = ''; redraw(); } })
    ...
    host.replaceChildren(...(navHost ? [] : [navEl]), el('div', { class: 'v2-fixed' }, el('a', { class: 'v2-nav' + (last.activeKey() === 'inbox' ? ' on' : ''), href: '#/inbox' }, glyph('inbox', 'v2-nav-ic'), el('span', { class: 'n', text: '확인할 것' }), inboxN ? el('span', { class: 'v2-nav-cnt', text: String(inboxN) }) : null), ...(navOn('liv') !== false ? [el('a', { class: 'v2-nav' + (last.activeKey() === 'liv' ? ' on' : ''), href: '#/liv' }, el('span', { class: 'v2-nav-lm', 'aria-hidden': 'true', text: 'L' }), el('span', { class: 'n', text: '리브' }))] : []), el('a', { class: 'v2-nav' + (last.activeKey() === 'connect' ? ' on' : ''), href: '#/connect' }, glyph('link', 'v2-nav-ic'), el('span', { class: 'n', text: '외부 앱 연결' }))), el('section', { class: 'v2-app-space', 'aria-label': '앱' }, secHead('앱', instances.length, 
