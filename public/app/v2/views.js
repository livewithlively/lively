...
export function renderHome(host, data) {
    ...
    const tod = h < 12 ? '오전이에요' : h < 18 ? '오후예요' : '저녁이에요';
    ...
    const ta = el('textarea', { class: 'v2-compose-in', rows: '2', placeholder: '무엇이든 시키세요 — Enter 로 새 세션이 열립니다', 'aria-label': '무엇이든 시키기' });
    const send = el('button', { class: 'btn btn-primary btn-sm v2-compose-send', type: 'button', text: '시키기' });
    ...
    const submit = async () => {
        ...
        send.disabled = true;
        ta.disabled = true;
        send.textContent = '여는 중…';
        const ok = await openQuickSession(text);
        if (!ok) {
            send.disabled = false;
            ta.disabled = false;
            send.textContent = '시키기';
            ta.focus();
        }
    ...
    host.replaceChildren(el('section', { class: 'v2-home v2-home-compose' }, el('div', { class: 'v2-home-lead' }, el('h1', { class: 'v2-h1', text: lead })), el('div', { class: 'v2-compose' }, ta, el('div', { class: 'v2-compose-row' }, hint, send)), nowList(data)));
