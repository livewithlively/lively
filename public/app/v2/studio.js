...
const GRID = 8;
const snap = (n) => Math.max(0, Math.round(n / GRID) * GRID);
const LS_KEY = (id) => 'lively_studio3_' + id;
...
function sizeOf(spec) {
    if (spec.w < 280 || spec.h < 200)
        return 's';
    if (spec.w >= 540 || spec.h >= 460)
        return 'l';
    return 'm';
}
export function mountStudio(host, opts) {
    const id = opts.id;
    ...
    let lastSpawnTs = 0;
    const els = new Map();
    const sigs = new Map(); // 위젯 본문 서명 — 같으면 DOM 을 안 건드린다(스크롤·타이핑 보호)
    function save() { try {
        localStorage.setItem(LS_KEY(id), JSON.stringify({ widgets, seq, lastSpawnTs }));
    }
    catch (_) { /* noop */ } }
    function load() {
        try {
            const s = JSON.parse(localStorage.getItem(LS_KEY(id)) || 'null');
            if (s && Array.isArray(s.widgets)) {
                widgets = s.widgets;
                seq = Number(s.seq) || widgets.length + 1;
                lastSpawnTs = Number(s.lastSpawnTs) || 0;
                return true;
            }
        }
        catch (_) { /* noop */ }
    ...
    const mySessions = () => opts.data().sessions.filter((s) => (loose ? !s.projectId : Number(s.projectId) === id));
    // ── 골격 ──
    ...
    function paintDoor() {
        ...
        door.replaceChildren(el('div', { class: 'stu-door-l' }, el('div', { class: 'stu-door-top' }, loose ? el('div', { class: 'stu-eyebrow' }, el('span', { text: `세션 ${ss.length}` + (live.length ? ` · 지금 ${live.length}` : '') }), el('span', { class: 'sep', text: '·' }), el('span', { text: '아직 어느 프로젝트에도 붙지 않음' }))
    ...
    // ── 위젯 프레임 ──
    function addWidget(type, at, data, o2) {
        const d = wdef(type);
        const spec = { id: 'w' + seq++, type, x: at ? snap(at.x) : 40 + ((seq * 48) % 280), y: at ? snap(at.y) : 32 + ((seq * 36) % 200), w: d.w, h: d.h, data: data || {} };
        widgets.push(spec);
        if (!o2?.noSave) {
            save();
            paintAll();
    ...
    function removeWidget(spec) {
        if (loose && spec.type === 'session' && spec.data?.sid)
            looseHide(String(spec.data.sid)); // 자투리 판: 치운 카드는 다시 안 올린다
        widgets = widgets.filter((w) => w !== spec);
        sigs.delete(spec.id);
        save();
        paintAll();
    }
    function widgetEl(spec) {
        if (spec.type === 'sticky')
            return stickyEl(spec);
        const d = wdef(spec.type);
        const body = el('div', { class: 'stu-w-body' });
        const titleEl2 = el('b', { class: 'stu-w-t', text: widgetTitle(spec) });
        const szBtn = el('button', { class: 'stu-w-btn', type: 'button', title: '크기 단계 — S/M/L 로 돌려요(끌어서도 조절)', text: sizeOf(spec).toUpperCase(), onclick: (e) => { e.stopPropagation(); cycleSize(spec); } });
        const head = el('div', { class: 'stu-w-head' }, el('span', { class: 'stu-w-ic', style: 'color:' + d.ac + ';background:' + d.ac + '1a' }, icon(d.ic, 'stu-i sm')), titleEl2, d.shell ? el('span', { class: 'stu-shell-badge', text: '껍데기' }) : null, el('span', { class: 'stu-w-acts' }, szBtn, el('button', { class: 'stu-w-btn', type: 'button', title: spec.min ? '펼치기' : '접기', text: spec.min ? '▸' : '▾', onclick: (e) => { e.stopPropagation(); spec.min = !spec.min; sigs.delete(spec.id); save(); paintAll(); } }), focusPrev.has(spec.id)
            ? el('button', { class: 'stu-w-btn', type: 'button', title: '작게 — 원래 자리·크기로', text: '⤡', onclick: (e) => { e.stopPropagation(); unfocusWidget(spec); } })
            : null, spec.type === 'session' && spec.data?.sid
            ? el('button', { class: 'stu-w-btn', type: 'button', title: '전체 화면(대화창)', text: '⤢', onclick: (e) => { e.stopPropagation(); location.hash = '#/s/' + encodeURIComponent(String(spec.data.sid)); } })
            : null, el('button', { class: 'stu-w-btn', type: 'button', title: '판에서 치우기', text: '×', onclick: (e) => { e.stopPropagation(); removeWidget(spec); } })));
        const w = el('div', { class: 'stu-w stu-w-' + spec.type + (spec.min ? ' min' : '') + (focusId === spec.id ? ' focused' : ''), 'data-sz': sizeOf(spec), style: `left:${spec.x}px;top:${spec.y}px;width:${spec.w}px;${spec.min ? '' : 'height:' + spec.h + 'px;'}z-index:${spec.z || 1};--wac:${d.ac}` }, head, body, el('div', { class: 'stu-w-rsz', title: '크기 조절 — 문턱을 넘으면 구성이 바뀌어요' }));
    ...
    const SIZES = { s: { w: 240, h: 160 }, m: { w: 0, h: 0 }, l: { w: 620, h: 520 } };
    function cycleSize(spec) {
    ...
    function widgetTitle(spec) {
        ...
        if (spec.type === 'preview')
            return String(spec.data?.name || '프리뷰');
        if (spec.type === 'doc')
            return String(spec.data?.title || '문서');
        return wdef(spec.type).label;
    }
    function wireDrag(spec, w, handle) {
        handle.addEventListener('mousedown', (e) => {
            ...
            const move = (ev) => { spec.x = snap(ox + ev.clientX - sx); spec.y = snap(oy + ev.clientY - sy); w.style.left = spec.x + 'px'; w.style.top = spec.y + 'px'; };
            const up = () => { document.removeEventListener('mousemove', move); document.removeEventListener('mouseup', up); w.classList.remove('drag'); save(); growCanvas(); };
            document.addEventListener('mousemove', move);
            document.addEventListener('mouseup', up);
        });
    }
    function wireResize(spec, w, body, szBtn) {
        const h = w.querySelector('.stu-w-rsz');
        if (!h)
            return;
        h.addEventListener('mousedown', (e) => {
            ...
            const move = (ev) => {
                spec.w = Math.max(216, snap(ow + ev.clientX - sx));
                spec.h = Math.max(140, snap(oh + ev.clientY - sy));
                w.style.width = spec.w + 'px';
                if (!spec.min)
                    w.style.height = spec.h + 'px';
                const sz = sizeOf(spec);
                if (w.dataset.sz !== sz) {
                    w.dataset.sz = sz;
                    szBtn.textContent = sz.toUpperCase();
                    sigs.delete(spec.id);
                    fillBody(spec, body, true);
                } // 문턱 통과 → 그 자리에서 구성 전환
            };
            const up = () => { document.removeEventListener('mousemove', move); document.removeEventListener('mouseup', up); save(); if (sizeOf(spec) !== szWas) {
                sigs.delete(spec.id);
                fillBody(spec, body, true);
            } };
            document.addEventListener('mousemove', move);
    ...
    function growCanvas() {
        ...
        cmLayer.style.width = mx + 'px';
        cmLayer.style.height = my + 'px';
    }
    // ── 본문 라우팅 — force=구성 재생성 · 아니면 서명이 같을 때 손대지 않는다 ──
    function fillBody(spec, body, force) {
        const t = spec.type;
        if (t === 'session') {
            void fillSession(spec, body, !!force);
            return;
        }
        if (t === 'coach') {
            fillCoach(spec, body, !!force);
            return;
        }
        if (t === 'automation')
            fillAutomation(spec, body);
        else if (t === 'tasks')
            fillTasks(spec, body);
        else if (t === 'knowledge')
            void fillKnowledge(spec, body);
        else if (t === 'folder')
            void fillFolder(spec, body);
        else if (t === 'desc')
            fillDesc(body);
        else if (t === 'timeline')
            fillTimeline(body);
        else if (t === 'preview')
            fillPreview(spec, body);
        else if (t === 'doc')
            fillDoc(spec, body);
        else if (t === 'app-meeting')
            fillMeeting(body);
        else if (t === 'app-calendar')
            fillCalendar(body);
        else if (t === 'app-slides')
            fillSlides(body);
    }
    const tailCache = new Map();
    async function fetchTurns(s, maxBytes) {
        const hit = tailCache.get(s.id);
        if (hit && Date.now() - hit.at < 7000)
            return hit.turns;
        const turns = [];
        try {
            const headers = {};
            ...
            if (res.ok) {
                for (const line of (await res.text()).split('\n')) {
                    ...
                    if (!o || o.isSidechain || o.isMeta)
                        continue;
                    const c = o.message?.content;
                    if (o.type === 'user') {
                        const txt = (typeof c === 'string' ? c : Array.isArray(c) ? c.filter((b) => b?.type === 'text').map((b) => String(b.text || '')).join(' ') : '').replace(/\s+/g, ' ').trim();
                        if (txt && !INJ_RE.test(txt))
                            turns.push({ who: 'me', text: txt });
                    }
                    else if (o.type === 'assistant') {
                        const txt = (Array.isArray(c) ? c.filter((b) => b?.type === 'text').map((b) => String(b.text || '')).join('\n') : '').trim();
                        if (!txt)
                            continue;
                        const last = turns[turns.length - 1];
                        if (last && last.who === 'ai')
                            last.text = txt; // 이어 쓰는 답은 마지막 것으로 갱신(도구 사이 조각 합침)
                        else
                            turns.push({ who: 'ai', text: txt });
                    }
                }
            }
        }
    ...
    async function fillSession(spec, body, force) {
        const sid = spec.data?.sid ? String(spec.data.sid) : '';
        const s = sid ? mySessions().find((x) => x.id === sid) : null;
        const sz = sizeOf(spec);
        if (!s) {
            const list = mySessions();
            body.replaceChildren(el('p', { class: 'stu-fine', text: list.length ? '판에 올릴 세션을 고르세요.' : '아직 세션이 없어요 — 아래 컴포저로 시키면 자동으로 생깁니다.' }), el('div', { class: 'stu-pick' }, ...list.slice(0, 8).map((x) => el('button', { class: 'stu-pick-row', type: 'button', onclick: () => { spec.data = { sid: x.id }; sigs.delete(spec.id); save(); paintAll(); } }, el('span', { class: 'v2-dot ' + dotCls(x.stateKey), 'aria-hidden': 'true' }), el('span', { class: 'n', text: x.label || x.id }), el('span', { class: 'm', text: x.stateLabel })))));
            return;
        }
        const raw = s.raw || {};
        ...
        if (root)
            root.style.setProperty('--wac', s.stateKey === 'busy' ? '#2D6BF0' : waiting ? '#D9A32B' : s.stateKey === 'done' ? '#16C79A' : '#A6B2C8');
        ...
        if (sz === 's') { // S — Mail 위젯 문법: 상태 + 지금 하는 일 한 줄. 조작 없음.
            ...
            if (!force && sigs.get(spec.id) === sig)
                return;
            sigs.set(spec.id, sig);
            body.replaceChildren(el('div', { class: 'stu-sess-s' }, el('div', { class: 'st' }, el('span', { class: 'v2-dot ' + dotCls(s.stateKey), 'aria-hidden': 'true' }), el('span', { text: s.stateLabel }), el('span', { class: 'stu-fine', style: 'margin-left:auto', text: s.lastSeen ? relTime(new Date(s.lastSeen).toISOString()) : '' })), el('div', { class: 'now', text: workOk ? work : '조용해요 — 크기를 키우면 대화가 보여요' })));
            return;
        }
        const turns = (s.live && !s.node) ? await fetchTurns(s, sz === 'l' ? 60000 : 30000) : [];
        if (dead || !body.isConnected)
            return;
        const shown = sz === 'l' ? turns : turns.slice(-6);
        const sig = sz + '|' + s.stateKey + '|' + (workOk ? work : '') + '|' + shown.map((t) => t.who + t.text.length).join(',');
        if (!force && sigs.get(spec.id) === sig)
            return; // 변화 없음 — 스크롤·입력 그대로
        sigs.set(spec.id, sig);
        const conv = el('div', { class: 'stu-conv' }, shown.length ? null : el('p', { class: 'stu-fine', style: 'padding:8px 2px', text: s.live ? '아직 주고받은 말이 없어요.' : '기록만 남은 세션 — 열어서 보세요.' }), ...shown.map((t) => el('div', { class: 'stu-msg ' + t.who }, el('span', { class: 'who', text: t.who === 'me' ? '나' : 'AI' }), el('div', { class: 'tx', text: t.text.length > (sz === 'l' ? 2000 : 420) ? t.text.slice(0, sz === 'l' ? 2000 : 420) + '…' : t.text }))));
        ...
        const key = (action, label, primary) => el('button', { class: 'btn btn-sm ' + (primary ? 'btn-primary' : 'btn-ghost'), type: 'button', text: label, onclick: () => {
        ...
        body.replaceChildren(el('div', { class: 'stu-sess-st' }, el('span', { class: 'v2-dot ' + dotCls(s.stateKey), 'aria-hidden': 'true' }), el('span', { text: s.stateLabel }), raw.harness ? el('span', { class: 'stu-hbadge mono', text: String(raw.harness) }) : null, workOk && sz === 'l' ? el('span', { class: 'stu-fine ell', style: 'margin-left:8px', title: work, text: work }) : null, el('a', { class: 'stu-fine', style: 'margin-left:auto;color:var(--blue)', href: '#/s/' + encodeURIComponent(s.id), text: '열기 →' })), conv, ...(waiting ? [el('div', { class: 'stu-sess-keys' }, el('span', { class: 'stu-wait-k', text: '확인 필요' }), key('approve', '승인', true), key('deny', '거부'), key('interrupt', '멈춤'))] : []), el('div', { class: 'stu-sess-footrow' }, inp));
        conv.scrollTop = conv.scrollHeight; // 새 내용 = 바닥으로(메시지 앱)
    }
    // ══ 리브 제안 — Siri 제안 문법: 큰 아이콘 칩 + 한 문장 + 실행 하나 ═══════════════
    function coachRow(txt, say) {
        return el('button', { class: 'stu-sugg', type: 'button', title: '누르면 리브 대화로 들어가요', onclick: () => { openLiv(); chat?.say(say); } }, el('span', { class: 'stu-sugg-ic' }, icon('spark', 'stu-i sm')), el('span', { class: 'n', text: txt }), el('span', { class: 'go' }, icon('arrow', 'stu-i sm')));
    }
    function coachRows() {
        const ss = mySessions();
        const waiting = ss.filter((s) => s.stateKey === 'waiting');
        const idle = ss.filter((s) => s.live && s.alive && s.stateKey === 'idle');
        const tasks = Array.isArray(pj().tasks) ? pj().tasks : [];
        const noMeta = tasks.filter((tk) => tk.status_category !== 'done' && !tk.due_date && !(tk.assignees && tk.assignees.length)).length;
        const rows = [];
        if (waiting.length)
            rows.push([`세션 ${waiting.length}개가 답을 기다려요 — 물음 모아 보기`, '답을 기다리는 세션들의 질문을 모아 요약해 줘. 한 번에 결정할 수 있게.']);
        if (idle.length >= 2)
            rows.push([`조용한 세션 ${idle.length}개 — 결과를 지식으로 정리하고 닫기`, '조용한 세션들이 남긴 결과를 지식으로 정리하고, 닫아도 되는 세션을 알려 줘.']);
        if (noMeta >= 3)
            rows.push([`담당·마감 없는 할 일 ${noMeta}개 — 정리하고 병렬로`, '상태·마감·담당을 점검하고 비거나 어긋난 걸 정리해 줘.']);
    ...
    function fillCoach(spec, body, force) {
        const sz = sizeOf(spec);
        ...
        if (!force && sigs.get(spec.id) === sig)
            return;
        sigs.set(spec.id, sig);
        body.replaceChildren(el('div', { class: 'stu-suggs' }, ...rows.map(([t, s2]) => coachRow(t, s2))), sz !== 's' ? el('p', { class: 'stu-foot-note', text: '판의 상태에서 계산한 제안이에요.' }) : null);
    }
    // ══ 자동화 — 단축어 갤러리 문법: 색 타일 + 이름 + 설명 + [켜기] ═══════════════════
    function fillAutomation(spec, body) {
        const sz = sizeOf(spec);
        const row = (t, d2, say) => el('div', { class: 'stu-auto' }, el('span', { class: 'stu-auto-ic' }, icon('bolt', 'stu-i sm')), el('span', { class: 'b' }, el('b', { text: t }), sz !== 's' ? el('span', { class: 'd', text: d2 }) : null), el('button', { class: 'stu-pillbtn violet', type: 'button', text: '켜기', onclick: () => { openLiv(); chat?.say(say); } }));
        const rows = [
        ...
        body.replaceChildren(el('div', { class: 'stu-autos' }, ...rows.slice(0, sz === 's' ? 1 : 3)), sz !== 's' ? el('p', { class: 'stu-foot-note', text: '켜기 = 리브가 설계·생성(예약 붙박이는 다음 단계)' }) : null);
    }
    // ══ 태스크 — S: 진행 링 · M: Things 식 목록 · L: 마감 포함 ═══════════════════════
    function fillTasks(spec, body) {
        const sz = sizeOf(spec);
        const tasks = Array.isArray(pj().tasks) ? pj().tasks : [];
        ...
        if (sz === 's') { // 활동 링(Apple 링 문법)
            ...
            body.replaceChildren(el('div', { class: 'stu-task-s' }, ring, el('div', { class: 'k' }, el('b', { text: `${done}/${tasks.length}` }), el('span', { text: '끝냄' }), open.length ? el('span', { class: 'stu-fine', text: '다음 — ' + String(open[0].name).slice(0, 22) }) : null)));
            return;
        }
        const row = (tk) => el('a', { class: 'stu-task', href: '#/projects2/t/' + tk.id, title: tk.name }, el('span', { class: 'stu-ck' + (tk.status_category === 'started' ? ' run' : ''), 'aria-hidden': 'true' }), el('span', { class: 'n', text: tk.name }), tk.due_date ? el('span', { class: 'due' + (String(tk.due_date) < new Date().toISOString().slice(0, 10) ? ' over' : ''), text: String(tk.due_date).slice(5, 10) }) : null);
        body.replaceChildren(el('div', { class: 'stu-taskbar' }, el('i', { style: 'width:' + Math.round(pct * 100) + '%' }), el('span', { text: `${done}/${tasks.length}` })), open.length ? el('div', { class: 'stu-scroll' }, ...open.slice(0, sz === 'l' ? 30 : 9).map(row)) : el('p', { class: 'stu-fine', text: '남은 할 일이 없어요.' }), el('div', { class: 'stu-w-foot' }, el('a', { class: 'btn-text', href: '#/projects2/p/' + id, text: '보드에서 전체 →' }), el('button', { class: 'btn-text', type: 'button', text: '리브에게 나누기', onclick: () => { openLiv(); chat?.say('이 프로젝트를 태스크로 나눠 줘. 만들기 전에 목록 먼저.'); } })));
    }
    // ══ 지식 — M: 문서 목록 · L: 본문 미리보기 카드(Notes 그리드) ═══════════════════
    const knPeekCache = new Map();
    async function knSnippet(name) {
        if (knPeekCache.has(name))
            return knPeekCache.get(name);
        let out = '';
        try {
            const r = await api('/api/ui/knowledge/' + encodeURIComponent(name) + '?limit=8');
    ...
    async function fillKnowledge(spec, body) {
        const sz = sizeOf(spec);
        const kn = pj().knowledge || {};
        const all = [
            ...(kn.required || []).map((k) => ({ name: k.name, rel: '필요' })),
            ...(kn.produced || []).map((k) => ({ name: k.name, rel: '산출' })),
        ];
        if (!all.length) {
            body.replaceChildren(el('p', { class: 'stu-fine', text: '연결된 지식이 아직 없어요.' }));
            return;
        }
        if (sz === 's') {
            body.replaceChildren(el('div', { class: 'stu-kn-s' }, el('b', { text: String(all.length) }), el('span', { text: '연결된 지식' }), el('span', { class: 'stu-fine ell', text: all[all.length - 1].name })));
            return;
        }
        if (sz === 'l') {
            const cards = await Promise.all(all.slice(0, 8).map(async (k) => {
                const snip = await knSnippet(k.name);
                return el('button', { class: 'stu-kncard', type: 'button', title: k.name, onclick: () => openPeek('#/k/' + encodeURIComponent(k.name), k.name) }, el('div', { class: 'h' }, icon('doc', 'stu-i sm'), el('span', { class: 'stu-kn-rel' + (k.rel === '산출' ? ' prod' : ''), text: k.rel })), el('b', { class: 'ell2', text: k.name }), el('span', { class: 'snip', text: snip || '…' }));
            }));
            if (dead || !body.isConnected)
                return;
        ...
        body.replaceChildren(el('div', { class: 'stu-scroll' }, ...all.slice(0, 14).map((k) => el('button', { class: 'stu-docrow', type: 'button', title: k.name, onclick: () => openPeek('#/k/' + encodeURIComponent(k.name), k.name) }, icon('doc', 'stu-i sm'), el('span', { class: 'n', text: k.name }), el('span', { class: 'stu-kn-rel' + (k.rel === '산출' ? ' prod' : ''), text: k.rel })))));
    }
    ...
    async function fillFolder(spec, body) {
        ...
        const head = el('div', { class: 'stu-w-foot stu-asset-head' }, el('span', { class: 'stu-fine', text: files.length ? files.length + '개 · 최근 먼저' : '' }), el('button', { class: 'btn-text', type: 'button', text: '＋ 올리기', title: '컴퓨터에서 파일 고르기 — 판에 끌어다 놓아도 올라가요', onclick: () => upIn.click() }));
        if (!files.length) {
            body.replaceChildren(upIn, el('div', { class: 'stu-asset-empty' }, icon('drop', 'stu-i big'), el('b', { text: '아직 자료가 없어요' }), el('p', { class: 'stu-fine', text: '컴퓨터에서 파일을 판에 끌어다 놓거나 [＋ 올리기]를 누르세요. 세션이 만든 결과물도 여기 쌓입니다.' }), el('button', { class: 'btn-text', type: 'button', text: '＋ 올리기', onclick: () => upIn.click() })));
            return;
        }
        if (sz2 === 's') {
            const f = files[0];
        ...
        const row = (f) => {
            ...
            const thumb = k.kind === 'img' ? el('img', { alt: '', loading: 'lazy' }) : null;
            if (thumb)
                fillImg(thumb, f.path);
            return el('div', { class: 'stu-asset' }, el('button', { class: 'stu-asset-b', type: 'button', title: f.path + ' — 열기', onclick: () => openDeskItem({ kind: k.kind, path: f.path, label: base(f.path), type: k.type }) }, el('span', { class: 'stu-asset-ic ' + k.kind }, thumb || icon(k.kind === 'page' ? 'deck' : 'doc', 'stu-i')), el('span', { class: 'stu-asset-t' }, el('b', { text: base(f.path) }), el('span', { class: 'stu-asset-m' }, el('span', { text: k.type }), el('span', { class: 'sep', text: '·' }), el('span', { text: fmtSize(f.size || 0) }), dir ? el('span', { class: 'sep', text: '·' }) : null, dir ? el('span', { class: 'stu-asset-dir', text: dir }) : null))), el('button', { class: 'stu-w-btn stu-asset-x', type: 'button', text: '×', title: '휴지통으로', onclick: () => void trashItems([{ kind: k.kind, path: f.path, label: base(f.path), type: k.type }]).then(() => { sigs.delete(spec.id); fillBody(spec, body, true); }) }));
        };
        body.replaceChildren(upIn, head, el('div', { class: 'stu-scroll stu-assets' }, ...files.slice(0, sz2 === 'l' ? 60 : 20).map(row)), el('div', { class: 'stu-w-foot' }, el('button', { class: 'btn-text', type: 'button', text: '원본 폴더 열기 ↗', title: '폴더 구조 그대로 보는 파일 브라우저', onclick: () => openPeek('#/f?root=shared&path=' + encodeURIComponent('project/' + id), '공유 폴더 — 원본') })));
    }
    function fillDesc(body) {
        const md = String(pj().description || '').trim();
        body.replaceChildren(el('div', { class: 'stu-scroll' }, md ? el('div', { class: 'stu-md' }, renderMarkdown(md)) : el('p', { class: 'stu-fine', text: '본문이 아직 없어요.' })), el('div', { class: 'stu-w-foot' }, el('button', { class: 'btn-text', type: 'button', text: '리브에게 정돈 맡기기', onclick: () => { openLiv(); chat?.say('본문을 정돈된 형식으로 다시 써 줘. 사실·결정·링크는 하나도 잃지 말고.'); } })));
    }
    function fillTimeline(body) {
        const tl = createTimeline(body, { scope: '프로젝트 #' + id, showActors: true, outcomes: true, empty: '아직 남은 것이 없어요.' });
        void loadProjectTimeline(id, detail).then((items) => { if (!dead)
            tl.addAll(items); });
    }
    function fillPreview(spec, body) {
        ...
        if (/\.(png|jpe?g|gif|webp|svg)$/i.test(path)) {
            const im = el('img', { class: 'stu-prev-img', alt: path, loading: 'lazy' });
            fillImg(im, path); // 헤더 인증(쿠키 없는 세션에서도 뜨게)
            body.replaceChildren(el('div', { class: 'stu-prevwrap' }, im));
        }
        else
            body.replaceChildren(el('iframe', { class: 'stu-prev-f', src: /\.pdf$/i.test(path) ? url + '#toolbar=0&view=FitH' : url, title: path }));
        body.append(el('div', { class: 'stu-w-foot' }, el('span', { class: 'stu-fine ell', text: path }), el('button', { class: 'btn-text', type: 'button', text: '크게', onclick: () => void openFileViewer(id, path, path, () => { }, '/api/ui/v6/projects/') })));
    }
    function fillDoc(spec, body) {
    ...
    function shellCard(bodyText, rows) {
        return [el('p', { class: 'stu-shell-p', text: bodyText }), el('div', { class: 'stu-suggs' }, ...rows)];
    }
    function fillMeeting(body) {
        body.replaceChildren(...shellCard('회의록을 넣으면 결정·태스크·미결이 추출되어 이 프로젝트에 연결되는 자리예요.', [coachRow('회의록 추출을 지금 맡겨 보기', '공유 폴더의 회의록을 찾아 결정·태스크·미결을 추출하고, 태스크로 만들지 목록 먼저 보여 줘.')]));
    }
    function fillCalendar(body) {
        const tasks = (Array.isArray(pj().tasks) ? pj().tasks : []).filter((tk) => tk.due_date && tk.status_category !== 'done');
        body.replaceChildren(el('p', { class: 'stu-shell-p', text: '마감·회의·세션 예약(정기 실행)이 한 달력에 놓일 자리예요.' }), tasks.length ? el('div', { class: 'stu-scroll' }, ...tasks.slice(0, 6).map((tk) => el('a', { class: 'stu-task', href: '#/projects2/t/' + tk.id }, el('span', { class: 'due', text: String(tk.due_date).slice(5, 10) }), el('span', { class: 'n', text: tk.name })))) : el('p', { class: 'stu-fine', text: '마감 있는 할 일이 아직 없어요.' }), el('div', { class: 'stu-suggs' }, coachRow('마감 점검 맡기기', '마감이 지난·임박한 태스크를 점검하고 조정안을 제안해 줘.')));
    }
    function fillSlides(body) {
        body.replaceChildren(...shellCard('이 프로젝트의 지식·산출물로 보고용 장표(HTML)를 만들어 판에 놓는 자리예요.', [coachRow('산출지식으로 장표 초안', '산출지식으로 보고용 장표(HTML) 초안을 만들어 줘. 목차 먼저.')]));
    }
    // 포스트잇
    function stickyEl(spec) {
        const ta = el('textarea', { class: 'stu-sticky-ta', placeholder: '메모…', 'aria-label': '포스트잇' });
        ta.value = String(spec.data?.text || '');
        ta.addEventListener('input', () => { spec.data = { ...(spec.data || {}), text: ta.value }; save(); });
        const w = el('div', { class: 'stu-w stu-sticky', style: `left:${spec.x}px;top:${spec.y}px;width:${spec.w}px;height:${spec.h}px;z-index:${spec.z || 1}` }, el('span', { class: 'stu-tape', 'aria-hidden': 'true' }), el('button', { class: 'stu-w-btn stu-sticky-x', type: 'button', text: '×', title: '치우기', onclick: () => removeWidget(spec) }), ta, el('div', { class: 'stu-w-rsz' }));
        ...
        h.addEventListener('mousedown', (e) => {
            ...
            e.stopPropagation();
            const sx = e.clientX, sy = e.clientY, ow = spec.w, oh = spec.h;
            const move = (ev) => { spec.w = Math.max(160, snap(ow + ev.clientX - sx)); spec.h = Math.max(120, snap(oh + ev.clientY - sy)); w.style.width = spec.w + 'px'; w.style.height = spec.h + 'px'; };
    ...
    function setCommentMode(on) {
        ...
        if (on)
            toast('코멘트 모드 — 판 위를 드래그해 영역을 잡으세요.');
    }
    cmLayer.addEventListener('mousedown', (e) => {
        if (!commentMode)
            return;
        e.preventDefault();
        ...
        const up = (ev) => {
            ...
            if (w2 < 24 || h2 < 18)
                return;
            const spec = { id: 'w' + seq++, type: 'sticky', x: 0, y: 0, w: 0, h: 0, data: { cm: { x: snap(x), y: snap(y), w: snap(w2), h: snap(h2) }, text: '' } };
            widgets.push(spec);
            save();
            paintAll();
            setCommentMode(false);
            window.setTimeout(() => { canvas.querySelector('[data-cm="' + spec.id + '"] textarea')?.focus(); }, 30);
        };
        document.addEventListener('mousemove', move);
        document.addEventListener('mouseup', up);
    });
    function commentEl(spec) {
        const cm = spec.data?.cm || { x: 40, y: 40, w: 160, h: 80 };
        const ta = el('textarea', { class: 'stu-cmt-ta', placeholder: '이 영역에 대해…', 'aria-label': '코멘트' });
        ta.value = String(spec.data?.text || '');
        ta.addEventListener('input', () => { spec.data = { ...(spec.data || {}), text: ta.value }; save(); });
        const card = el('div', { class: 'stu-cmt-card' }, el('div', { class: 'stu-cmt-h' }, icon('pin', 'stu-i sm'), el('b', { text: '코멘트' }), el('button', { class: 'stu-w-btn', type: 'button', text: '×', title: '지우기', onclick: () => removeWidget(spec) })), ta, el('div', { class: 'stu-cmt-acts' }, el('button', { class: 'stu-pillbtn mint', type: 'button', text: '리브에게', onclick: () => {
                const t = ta.value.trim();
                if (!t) {
                    toast('코멘트를 먼저 써 주세요.', true);
                    return;
                }
                openLiv();
                chat?.say('[캔버스 코멘트 · 판 좌표 ' + cm.x + ',' + cm.y + ' 근처] ' + t);
            } })));
        const g = el('div', { class: 'stu-cmt', 'data-cm': spec.id, style: `left:${cm.x}px;top:${cm.y}px;z-index:${spec.z || 5}` }, el('div', { class: 'stu-cmt-region', style: `width:${cm.w}px;height:${cm.h}px` }), card);
        const head = g.querySelector('.stu-cmt-h');
        head.addEventListener('mousedown', (e) => {
            if (e.target.closest('button'))
                return;
            e.preventDefault();
            const sx = e.clientX, sy = e.clientY, ox = cm.x, oy = cm.y;
            const move = (ev) => { cm.x = snap(ox + ev.clientX - sx); cm.y = snap(oy + ev.clientY - sy); g.style.left = cm.x + 'px'; g.style.top = cm.y + 'px'; };
            const up = () => { document.removeEventListener('mousemove', move); document.removeEventListener('mouseup', up); spec.data = { ...(spec.data || {}), cm }; save(); };
            document.addEventListener('mousemove', move);
            document.addEventListener('mouseup', up);
        });
        return g;
    ...
    // ── 피크 ──
    let peekEl = null;
    function closePeek() { if (peekEl) {
        peekEl.remove();
        peekEl = null;
        document.removeEventListener('keydown', peekEsc, true);
    } }
    function peekEsc(e) { if (e.key === 'Escape') {
        e.stopPropagation();
        closePeek();
    } }
    function openPeek(hash, title) {
        closePeek();
        ...
        const pe = el('aside', { class: 'stu-peek', role: 'dialog', 'aria-label': title }, el('button', { class: 'stu-peek-grip', type: 'button', title: '닫기 — 오른쪽으로 밀어 치웁니다 (Esc)', 'aria-label': '닫기', onclick: closePeek }, el('span', { class: 'ch', text: '›', 'aria-hidden': 'true' })), el('div', { class: 'stu-peek-h' }, el('b', { text: title }), el('span', { class: 'stu-peek-acts' }, el('button', { class: 'btn-text', type: 'button', text: '판에 놓기', onclick: () => { addWidget('doc', { x: 120, y: 80 }, { hash, title }); closePeek(); } }), el('a', { class: 'btn-text', href: hash, target: '_blank', rel: 'noopener', text: '새 탭 ↗' }), el('button', { class: 'stu-peek-x', type: 'button', title: '닫기 (Esc)' }, icon('x', 'stu-i sm'), el('span', { text: '닫기' })))), el('iframe', { class: 'stu-peek-f', src: location.pathname + '?embed=1' + hash, title }));
        pe.querySelector('.stu-peek-x').onclick = closePeek;
        peekEl = pe;
    ...
    const destChip = el('button', { class: 'stu-dest', type: 'button', 'aria-haspopup': 'listbox', title: '행선지 — 글의 내용으로 추정해요. 눌러서 바꿀 수 있어요' });
    const destWhy = el('span', { class: 'stu-dest-why' });
    const input = el('textarea', { class: 'stu-comp-in', rows: '1', placeholder: '무엇이든 시키세요 — 정리는 리브가, 일은 세션이 받아요', 'aria-label': '시키기' });
    const sendBtn = el('button', { class: 'stu-comp-send', type: 'button', title: '보내기(Enter)' }, icon('send', 'stu-i'));
    const livToggle = el('button', { class: 'stu-liv-toggle', type: 'button', title: '리브 대화 열기/닫기' }, el('span', { class: 'lm', text: 'L' }), el('span', { class: 'stu-liv-n', hidden: true }));
    ...
    const launchBtn = el('button', { class: 'stu-dock-btn', type: 'button', 'aria-expanded': 'false', title: '런치패드 — 위젯·앱을 판에 꺼냅니다', onclick: (e) => { e.stopPropagation(); toggleLaunch(); } }, icon('grid', 'stu-i'));
    // 도크(원준 2026-08-19): 왼쪽 = 부르는 것(리브·런치패드), 가운데 = 시키는 것. [+ 새 세션]은 뺐다 —
    //  처음 온 사람에게 '세션'은 뜻 모를 말이고, 새 일을 시키면 어차피 세션이 열린다(행선지 칩이 그 사실을 말해 준다).
    const composer = el('div', { class: 'stu-composer stu-dock' }, livToggle, launchBtn, el('div', { class: 'stu-comp-box' }, el('div', { class: 'stu-comp-row1' }, input, sendBtn), el('div', { class: 'stu-comp-row2' }, el('span', { class: 'k', text: '행선지' }), destChip, destWhy)));
    stage.append(composer, livPanel);
    const tokensOf = (t) => String(t || '').toLowerCase().split(/[^0-9a-z가-힣]+/).filter((x) => (/[가-힣]/.test(x) ? x.length >= 2 : x.length >= 3));
    function guessDest(text) {
        if (destOverride)
            return { dest: destOverride, why: '직접 골랐어요' };
        const t = String(text || '').toLowerCase();
        if (!t.trim())
            return { dest: { kind: 'liv' }, why: '' };
        let best = null;
        for (const s of mySessions().filter((x) => x.live && x.alive)) {
            const name = (s.label || '') + ' ' + String(s.raw?.title || '');
            let score = 0, hit = '';
            for (const tok of tokensOf(name))
                if (t.includes(tok)) {
                    score += tok.length;
        ...
        if (best && best.score >= 4)
            return { dest: { kind: 'session', sid: best.s.id, label: sessText(best.s, pj().name || '').main || best.s.label }, why: `「${best.hit}」가 닿았어요` };
        if (/(본문|명세|태스크|할 일|할일|상태|정리|요약|지식|연결|이름|마감|담당|우선순위|프로젝트)/.test(t))
            return { dest: { kind: 'liv' }, why: '정리 명령으로 읽혀요' };
        return { dest: { kind: 'new' }, why: '새 일로 읽혀요' };
    }
    function paintDest() {
        const g = guessDest(input.value);
        const d = g.dest;
        destChip.replaceChildren(el('span', { class: 'stu-dest-dot ' + (d.kind === 'liv' ? 'liv' : d.kind === 'new' ? 'new' : 'sess'), 'aria-hidden': 'true' }), el('b', { text: d.kind === 'liv' ? '리브(워크스페이스)' : d.kind === 'new' ? '새 세션' : d.label }), el('span', { class: 'car', text: '▾', 'aria-hidden': 'true' }));
        destWhy.textContent = g.why;
    }
    destChip.onclick = (e) => {
        e.stopPropagation();
        const pop = el('div', { class: 'stu-dest-pop' }, el('button', { class: 'stu-dest-opt', type: 'button', onclick: () => { destOverride = null; paintDest(); pop.remove(); } }, el('b', { text: '자동 — 글에서 추정' })), el('button', { class: 'stu-dest-opt', type: 'button', onclick: () => { destOverride = { kind: 'liv' }; paintDest(); pop.remove(); } }, el('span', { class: 'stu-dest-dot liv' }), el('span', { text: '리브(정리·본문·태스크)' })), el('button', { class: 'stu-dest-opt', type: 'button', onclick: () => { destOverride = { kind: 'new' }; paintDest(); pop.remove(); } }, el('span', { class: 'stu-dest-dot new' }), el('span', { text: '새 세션 열어서' })), ...mySessions().filter((s) => s.live && s.alive).slice(0, 6).map((s) => el('button', { class: 'stu-dest-opt', type: 'button', onclick: () => { destOverride = { kind: 'session', sid: s.id, label: sessText(s, pj().name || '').main || s.label }; paintDest(); pop.remove(); } }, el('span', { class: 'v2-dot ' + dotCls(s.stateKey) }), el('span', { text: sessText(s, pj().name || '').main || s.label }))));
        composer.append(pop);
        const off = (ev) => { if (!ev.target.closest('.stu-dest-pop, .stu-dest')) {
            pop.remove();
    ...
    input.addEventListener('keydown', (e) => { if (e.key === 'Enter' && !e.shiftKey && !e.isComposing) {
        e.preventDefault();
        void sendComposer();
    } });
    ...
    async function sendComposer() {
        ...
        if (dest.kind === 'session') {
            try {
                ...
                focusWidget(card);
                const ref2 = els.get(card.id);
                ref2?.root.classList.add('flash');
                window.setTimeout(() => ref2?.root.classList.remove('flash'), 1600);
            }
            catch (e) {
    ...
    async function spawnSession(firstPrompt, at) {
        const p = runPrefs();
        ...
        try {
            ...
            if (!loose) {
                try {
                    await api('/api/ui/terminal/sessions/' + encodeURIComponent(sid) + '/project', { method: 'POST', body: JSON.stringify({ projectId: id }) });
                }
                catch (_) { /* noop */ }
            }
            ...
            popIn(spec);
            if (firstPrompt)
                focusWidget(spec); // 시켰으면 그 대화를 크게 — 세션 화면으로 넘어가지 않아도 되게
    ...
    function toggleLiv(on) {
        const want = on != null ? on : livPanel.hidden;
        livPanel.hidden = !want;
        livToggle.classList.toggle('on', want);
    }
    function openLiv() { toggleLiv(true); }
    livToggle.onclick = () => { if (loose) {
        location.hash = '#/liv';
        return;
    } toggleLiv(); };
    if (loose)
        livToggle.title = '리브 — 워크스페이스 리브 화면으로';
    function mountChat() {
        if (loose)
            return; // 프로젝트 리브는 프로젝트의 것 — 자투리 판에서는 L 이 워크스페이스 리브로 데려간다
        chat = mountProjectChat(livBody, {
            projectId: id,
            onTurnDone: () => { void refreshDetail(); opts.onProjectChanged?.(); },
            onHasTurns: (has) => { livToggle.querySelector('.stu-liv-n').hidden = !has; },
        });
    }
    ...
    function refreshDataWidgets() {
        for (const spec of widgets) {
            if (spec.min || spec.type === 'session')
                continue;
            if (!['tasks', 'knowledge', 'desc', 'coach', 'automation', 'app-calendar'].includes(spec.type))
                continue;
            const ref = els.get(spec.id);
            if (ref && ref.root.isConnected && !(document.activeElement && ref.root.contains(document.activeElement))) {
                sigs.delete(spec.id);
                fillBody(spec, ref.body, true);
            }
        }
    }
    // ── 새 산출물 자동 등장 ──
    async function autoSpawn() {
        ...
        let data = null;
        try {
            data = await api('/api/ui/v6/projects/' + id + '/files?path=');
        }
        catch (_) {
            return;
        }
        if (dead)
            return;
        const items = ((data && data.items) || []).filter((it) => it.type !== 'dir' && /\.(png|jpe?g|gif|webp|svg|pdf|html?)$/i.test(it.name));
        const fresh = items.filter((it) => Number(it.mtime || 0) * (String(it.mtime).length > 11 ? 1 : 1000) > lastSpawnTs)
            .filter((it) => !widgets.some((w) => w.type === 'preview' && w.data?.path === it.name));
        if (!lastSpawnTs) {
    ...
    function startGuide() {
        const step = (n, t, d2, act) => el('div', { class: 'stu-guide-s' }, el('span', { class: 'stu-guide-n', text: n }), el('div', {}, el('b', { text: t }), ...(typeof d2 === 'string' ? [el('p', { class: 'stu-fine', text: d2 })] : d2), act ? el('button', { class: 'btn-text', type: 'button', text: act.label, onclick: act.run }) : null));
        // 안내 속 행선지 칩은 **그림이 아니라 진짜 손잡이**다 — 누르면 아래 도크의 그 칩이 열리고 잠깐 뛴다.
        //  "밑에 이거 건드려라"를 글로만 설명하는 것보다 눌러 보게 하는 쪽이 한 번에 통한다(상민님 2026-08-19).
        const chipDemo = () => {
            const c = el('button', { class: 'stu-guide-chip', type: 'button', title: '행선지 — 어디로 보낼지 여기서 고릅니다' }, el('span', { class: 'stu-dest-dot liv', 'aria-hidden': 'true' }), el('b', { text: '리브(워크스페이스)' }), el('span', { class: 'car', text: '▾', 'aria-hidden': 'true' }));
            c.onclick = () => {
        ...
        if (loose)
            return el('div', { class: 'stu-guide' }, el('h2', { text: '프로젝트에 붙지 않은 세션들' }), el('p', { class: 'stu-guide-l', text: '아직 어느 프로젝트에도 매이지 않은 세션이 여기 모입니다. 지금은 하나도 없네요.' }), el('div', { class: 'stu-guide-g' }, el('div', { class: 'stu-guide-s' }, el('span', { class: 'stu-guide-n', text: '＋' }), el('div', {}, el('b', { text: '아래 칸에 시켜 보세요' }), el('p', { class: 'stu-fine', text: '프로젝트를 정하지 않고도 바로 일을 시킬 수 있어요. 열린 세션은 이 판에 카드로 모이고, 나중에 프로젝트로 옮길 수 있습니다.' })))));
        ...
        return el('div', { class: 'stu-guide' }, el('h2', { text: '여기가 이 프로젝트의 작업대예요' }), el('p', { class: 'stu-guide-l', text: '빈 캔버스로 시작해서, 이 프로젝트에 맞게 화면을 조정해보세요.' }), el('div', { class: 'stu-guide-g' }, step('1', '아래 칸에 시켜 보세요', [
        ...
        ]), step('2', '파일을 끌어다 놓아 보세요', '컴퓨터에서 파일·폴더를 이 판에 떨어뜨리면 이 프로젝트에서 AI랑 대화할 때 AI가 참고해서 작업합니다. AI가 작업하면서 생성한 파일도 여기에 쌓입니다.'), step('3', '위젯을 통해서 커스터마이즈 해보세요', '태스크·캘린더·메모 같은 건 ⊞ 에서 앱으로 열거나 위젯으로 캔버스에 둘 수 있어요. 지금 당장은 필요 없습니다.', { label: '⊞ 열어 보기', run: () => toggleLaunch(true) })));
    }
    ...
    const appTile = (d, after, asApp) => {
        const c = el('button', { class: 'stu-app' + (asApp ? ' as-app' : ''), type: 'button', title: (asApp ? '앱 — 열어서 하는 일. ' : '위젯 — 캔버스에 둡니다. ') + d.hint, draggable: String(!asApp),
            onclick: () => { if (asApp)
                openApp(d.type);
            else
                addWidget(d.type); after?.(); } }, el('span', { class: 'stu-app-ic', style: 'background:' + d.ac }, icon(d.ic, 'stu-i')), el('span', { class: 'stu-app-n', text: d.label }), d.shell ? el('span', { class: 'stu-shell-dot', title: '껍데기' }) : null);
        if (!asApp)
    ...
    const sessionAppTile = (a) => {
        ...
        return el('button', { class: 'stu-app as-app stu-app-inst', type: 'button',
            ...
            onclick: () => { toggleLaunch(false); if (hasUi)
                void openAppUi(a.id, { title: a.title });
            else
                void openSessionAppCard(a); } }, el('span', { class: 'stu-app-ic', style: 'background:' + (hasUi ? '#2D6BF0' : '#0FA37E') }, icon(hasUi ? 'grid' : 'chat', 'stu-i')), el('span', { class: 'stu-app-n', text: a.title }), el('span', { class: 'stu-inst-dot', title: hasUi ? '앱' : '세션 앱' }));
    };
    ...
    let appEl = null;
    function closeApp() { if (appEl) {
        const id2 = appEl.dataset.spec || '';
        sigs.delete(id2);
        els.delete(id2);
        appEl.remove();
        appEl = null;
    ...
    function openApp(type) {
        ...
        const win = el('section', { class: 'stu-appwin', role: 'dialog', 'aria-label': d.label }, el('div', { class: 'stu-appwin-h' }, el('span', { class: 'stu-w-ic', style: 'color:' + d.ac + ';background:' + d.ac + '1a' }, icon(d.ic, 'stu-i sm')), el('b', { text: d.label }), d.shell ? el('span', { class: 'stu-shell-badge', text: '껍데기' }) : null, el('span', { class: 'stu-peek-acts' }, el('button', { class: 'btn-text', type: 'button', text: '＋ 캔버스에 위젯으로', title: '이 앱의 내용을 판에 두고 곁눈질합니다', onclick: () => { addOrFocus(type); closeApp(); } }), el('button', { class: 'stu-peek-x', type: 'button', onclick: closeApp }, icon('x', 'stu-i sm'), el('span', { text: '닫기' })))), body);
        appEl = win;
        win.dataset.spec = 'app-' + type;
        if (type === 'desc')
            fillDescApp(body);
        else if (type === 'knowledge')
            fillKnowledgeApp(body);
        else {
            const spec = { id: 'app-' + type, type, x: 0, y: 0, w: 900, h: 560, data: {} }; // L 등급 = 앱은 넓게
    ...
    function fillDescApp(body) {
        const ta = el('textarea', { class: 'stu-descedit', spellcheck: 'false' });
        ta.value = String(pj().description || '');
        const status = el('span', { class: 'stu-fine' });
        body.replaceChildren(el('p', { class: 'stu-fine', text: '이 프로젝트가 무엇인지 — 세션과 리브가 일할 때 먼저 읽는 글입니다. 마크다운으로 쓰세요.' }), ta, el('div', { class: 'stu-appwin-f' }, status, el('button', { class: 'stu-pillbtn mint', type: 'button', text: '저장', onclick: async () => {
                ...
                try {
                    ...
                    status.textContent = '저장하지 못했어요 — ' + (e?.message || e);
                }
    ...
    /** 지식 앱 = **찾는** 자리(위젯은 목록만). */
    function fillKnowledgeApp(body) {
        const kn = (pj().knowledge || {});
        const all = [
            ...(kn.required || []).map((k) => ({ name: String(k.name), rel: '필요', title: String(k.title || '') })),
            ...(kn.produced || []).map((k) => ({ name: String(k.name), rel: '산출', title: String(k.title || '') })),
        ];
        const list = el('div', { class: 'stu-scroll stu-assets' });
        const q = el('input', { class: 'stu-appsearch', type: 'search', placeholder: '지식 이름·제목으로 찾기', 'aria-label': '지식 검색' });
        const draw = () => {
            const t = q.value.trim().toLowerCase();
            const hit = all.filter((k) => !t || k.name.toLowerCase().includes(t) || (k.title || '').toLowerCase().includes(t));
            list.replaceChildren(...(hit.length ? hit.map((k) => el('div', { class: 'stu-asset' }, el('button', { class: 'stu-asset-b', type: 'button', onclick: () => { closeApp(); openPeek('#/k/' + encodeURIComponent(k.name), k.name); } }, el('span', { class: 'stu-asset-ic' }, icon('book', 'stu-i')), el('span', { class: 'stu-asset-t' }, el('b', { text: k.name }), el('span', { class: 'stu-asset-m' }, el('span', { text: k.rel }), k.title ? el('span', { class: 'sep', text: '·' }) : null, k.title ? el('span', { class: 'stu-asset-dir', text: k.title }) : null)))))
    ...
    function toggleNotif(on) {
        ...
        if (want) {
            ...
            body.querySelector('.tl-wrap > .v2-aside-h')?.append(el('span', { class: 'stu-w-acts stu-notif-acts' }, el('button', { class: 'btn-text', type: 'button', text: '판에 고정', title: '위젯으로 꺼내 바탕화면에 둡니다', onclick: () => { addOrFocus('timeline'); toggleNotif(false); } }), el('button', { class: 'stu-w-btn', type: 'button', text: '×', title: '닫기(Esc)', onclick: () => toggleNotif(false) })));
            void loadProjectTimeline(id, detail).then((items) => { if (!dead && !notif.hidden)
                tl.addAll(items); });
            toggleLaunch(false);
            notif.hidden = false;
    ...
    function onDocDown(e) {
        ...
        const t = e.target;
        if (!notif.hidden && !notif.contains(t) && !streamEl.contains(t) && !tlBtn.contains(t))
            toggleNotif(false);
        if (appEl && !appEl.contains(t) && !launch.contains(t))
            closeApp();
    ...
    function onDocKey(e) {
        ...
        if (e.key === 'Escape') {
            ...
            else if (!notif.hidden) {
                toggleNotif(false);
                e.stopPropagation();
            }
            else if (!launch.hidden) {
                toggleLaunch(false);
                e.stopPropagation();
            }
        ...
        if ((e.key === 'Delete' || e.key === 'Backspace') && selected().length) {
            e.preventDefault();
            void trashItems(selected().map((x) => x.it));
        }
    }
    ...
    function openDeskItem(it, at) {
        if (it.kind === 'stack-folder') {
            addOrFocus('folder');
            return;
        } // 원시 파일 브라우저가 아니라 자료함 위젯(그 안에 '원본 폴더 열기' 가 있다)
        if (it.kind === 'stack-kn') {
            addOrFocus('knowledge');
            return;
        }
        ...
        void openFileViewer(id, it.path, base(it.path), () => { }, '/api/ui/v6/projects/');
    }
    // ── 선택 ──
    const iconEls = new Map();
    const selected = () => [...iconEls].filter(([e2]) => e2.classList.contains('sel')).map(([e2, it]) => ({ el: e2, it }));
    function selectOnly(b) {
        for (const [e2] of iconEls)
            e2.classList.toggle('sel', e2 === b);
        paintDeskBar();
    }
    ...
    function macFolder(tone) {
        const u = 'f' + (tone || 'b') + '-' + id;
        return sv('svg', { viewBox: '0 0 56 46', class: 'stu-folder-sv', 'aria-hidden': 'true' }, sv('defs', {}, sv('linearGradient', { id: u + '-back', x1: '0', y1: '0', x2: '0', y2: '1' }, sv('stop', { offset: '0', 'stop-color': tone ? '#5AD1CF' : '#7CB6F7' }), sv('stop', { offset: '1', 'stop-color': tone ? '#2FB3B4' : '#4E93EC' })), sv('linearGradient', { id: u + '-front', x1: '0', y1: '0', x2: '0', y2: '1' }, sv('stop', { offset: '0', 'stop-color': tone ? '#8BE6E2' : '#A8D3FC' }), sv('stop', { offset: '1', 'stop-color': tone ? '#3DC2C0' : '#5FA2F0' }))), 
        // 뒤판 — 왼쪽 위에 탭이 솟은 판
    ...
    function deskIcon(it) {
        ...
        if (it.kind === 'img') {
            ...
            let front;
            if (top && top.kind === 'img') {
                const th = el('img', { alt: '', loading: 'lazy' });
                fillImg(th, top.path);
            ...
            g = el('span', { class: 'stu-ico-g stack cat-' + (it.cat || it.kind) }, el('span', { class: 'stu-stk s3', 'aria-hidden': 'true' }), el('span', { class: 'stu-stk s2', 'aria-hidden': 'true' }), front);
        }
        else {
        ...
        const b = el('button', { class: 'stu-ico' + (stack ? ' is-stack' : '') + (openPopFor && openPopFor === it.path ? ' open' : ''), type: 'button', role: 'option', 'aria-selected': 'false', draggable: String(!stack),
        ...
        b.addEventListener('dblclick', (e) => { e.preventDefault(); openDeskItem(it); });
        b.addEventListener('keydown', (e) => { if (e.key === 'Enter') {
            e.preventDefault();
            openDeskItem(it);
        } });
    ...
    board.addEventListener('mousedown', (e) => {
        if (commentMode || e.button !== 0)
            return;
        const t = e.target;
        if (t.closest('.stu-w, .stu-cmt, .stu-composer, .stu-launch, .stu-notif, .stu-peek'))
            return;
        ...
        let moved = false;
        const move = (ev) => {
            if (!moved && Math.abs(ev.clientX - sx) + Math.abs(ev.clientY - sy) < 5)
                return;
            if (!moved)
                stage.classList.add('marq-on');
            moved = true;
            marq.hidden = false;
            const x = Math.min(sx, ev.clientX), y = Math.min(sy, ev.clientY);
            const w2 = Math.abs(ev.clientX - sx), h2 = Math.abs(ev.clientY - sy);
            marq.style.left = (x - sr.left) + 'px';
            marq.style.top = (y - sr.top) + 'px';
            marq.style.width = w2 + 'px';
            marq.style.height = h2 + 'px';
        ...
        const up = () => {
            ...
            document.removeEventListener('mouseup', up);
            marq.hidden = true;
            stage.classList.remove('marq-on');
            if (!moved)
                clearSel(); // 그냥 클릭 = 바탕 클릭 = 선택 해제(맥)
        };
        document.addEventListener('mousemove', move);
        document.addEventListener('mouseup', up);
    });
    ...
    trashEl.addEventListener('drop', (e) => {
        ...
        if (!raw.startsWith('stu-desk:'))
            return;
        try {
            void trashItems(JSON.parse(raw.slice(9)));
        }
        catch (_) { /* noop */ }
    });
    ...
    async function trashItems(items) {
        ...
        if (!list.length)
            return;
        try {
            await api('/api/ui/v6/projects/' + id + '/folder?path=' + encodeURIComponent(TRASH_DIR), { method: 'POST' });
        }
        catch (_) { /* 이미 있음 */ }
        let ok = 0;
        for (const it of list) {
            try {
                await moveFile(it.path, TRASH_DIR + '/' + base(it.path));
                ok++;
            }
            catch (e) {
                toast(base(it.path) + ' — ' + (e?.message || e), true);
            }
        }
        if (ok)
            toast(ok + '개를 휴지통으로 옮겼어요 — 휴지통을 두 번 누르면 되돌릴 수 있어요.');
        clearSel();
        deskSig = '';
        await paintDesk();
    }
    async function duplicateItems(items) {
        let ok = 0;
        for (const it of items) {
            ...
            try {
                ...
                if (!up.ok)
                    throw new Error('복제 실패(' + up.status + ')');
                ok++;
            }
            catch (e) {
                toast(nm + ' — ' + (e?.message || e), true);
            }
        }
        if (ok)
            toast(ok + '개를 복제했어요.');
    ...
    async function openTrash() {
        let items = [];
        try {
            const d = await api('/api/ui/v6/projects/' + id + '/files?path=' + encodeURIComponent(TRASH_DIR));
            items = (d && d.items) || [];
        }
        catch (_) {
            items = [];
        ...
        const panel = el('aside', { class: 'stu-trashpanel', role: 'dialog', 'aria-label': '휴지통' }, el('div', { class: 'stu-notif-h2' }, el('b', { text: '휴지통' }), el('span', { class: 'stu-fine', text: items.length + '개' }), el('span', { class: 'stu-peek-acts' }, items.length ? el('button', { class: 'btn-text danger', type: 'button', text: '비우기', onclick: () => void emptyTrash(panel) }) : null, el('button', { class: 'stu-peek-x', type: 'button', onclick: () => panel.remove() }, icon('x', 'stu-i sm'), el('span', { text: '닫기' })))), host);
        host.replaceChildren(...(items.length ? items.map((f) => el('div', { class: 'stu-trashrow' }, el('span', { class: 'n', text: String(f.name) }), el('span', { class: 'm', text: fmtSize(f.size || 0) }), el('button', { class: 'btn-text', type: 'button', text: '되돌리기', onclick: async () => {
                try {
                    await moveFile(TRASH_DIR + '/' + f.name, String(f.name));
                    toast(f.name + ' — 제자리로 돌려놨어요.');
                    panel.remove();
                    deskSig = '';
                    await paintDesk();
                }
    ...
    async function emptyTrash(panel) {
        try {
            await api('/api/ui/v6/projects/' + id + '/file?path=' + encodeURIComponent(TRASH_DIR), { method: 'DELETE' });
            toast('휴지통을 비웠어요.');
        }
        catch (e) {
    ...
    const looseHidden = (() => { try {
        return new Set(JSON.parse(localStorage.getItem(LOOSE_HIDE_KEY) || '[]'));
    }
    catch (_) {
        return new Set();
    } })();
    function looseHide(sid) { looseHidden.add(sid); try {
        localStorage.setItem(LOOSE_HIDE_KEY, JSON.stringify([...looseHidden]));
    }
    catch (_) { /* noop */ } }
    function syncLooseCards(first) {
        if (!loose)
            return;
    ...
    async function assetFiles() {
        ...
        const m = await api('/api/ui/v6/projects/' + id + '/shared/manifest').catch(() => null);
        const mf = (m && m.files) || [];
        trashN = mf.filter((f) => String(f.path).startsWith(TRASH_DIR + '/')).length;
        return mf
            .filter((f) => {
            const p2 = String(f.path);
            if (p2.startsWith(TRASH_DIR + '/'))
                return false;
            if (MACHINE_FILES.has(base(p2)))
                return false;
    ...
    async function paintDesk() {
        if (loose) {
            ...
            trashEl.hidden = true;
            return;
        } // 공유 폴더가 없는 판 — 바탕 아이콘·휴지통도 없다
        const assets = await assetFiles().catch(() => []);
        if (dead)
            return;
    ...
    let streamSeen = null; // null = 첫 로드(기준선만 잡고 알리지 않는다 — 들어오자마자 폭탄이면 소음이다)
    async function pollStream() {
        let items = [];
        try {
            items = await loadProjectTimeline(id, detail);
        }
        catch (_) {
            return;
        }
        if (dead)
            return;
        const key = (it) => String(it.id ?? '') + ':' + String(it.at ?? it.time ?? '') + ':' + String(it.title ?? it.text ?? '');
        if (!streamSeen) {
            streamSeen = new Set(items.slice(0, 60).map(key));
            return;
        }
        const fresh = items.filter((it) => !streamSeen.has(key(it))).slice(0, 3);
        for (const it of fresh)
            streamSeen.add(key(it));
        for (const it of fresh)
            pushStream(it);
    }
    function pushStream(it) {
        const title = String(it.title || it.text || it.label || '새 소식');
        const sid = String(it.sessionId || it.session_id || it.raw?.session_id || '');
        const card = el('button', { class: 'stu-streamcard', type: 'button', title: '이 일이 있던 자리로' }, el('span', { class: 'stu-stream-dot', 'aria-hidden': 'true' }), el('span', { class: 'stu-stream-t' }, el('b', { text: title }), el('span', { class: 'stu-fine', text: it.kind || it.type || '방금' })));
        card.onclick = () => {
        ...
        while (streamEl.children.length > 3)
            streamEl.firstElementChild?.remove();
    ...
    function tipDone(k) { tipsDone.add(k); try {
        localStorage.setItem(TIP_KEY, JSON.stringify([...tipsDone]));
    }
    catch (_) { /* noop */ } }
    function showTip(k, text, act) {
        if (tipsDone.has(k) || !tip.hidden)
            return;
        tip.replaceChildren(el('p', { class: 'stu-tip-t', text }), el('div', { class: 'stu-tip-a' }, act ? el('button', { class: 'stu-pillbtn mint', type: 'button', text: act.label, onclick: () => { tipDone(k); tip.hidden = true; act.run(); } }) : null, el('button', { class: 'btn-text', type: 'button', text: '괜찮아요', onclick: () => { tipDone(k); tip.hidden = true; } })));
        tip.hidden = false;
    }
    function thinkTip() {
        if (!tip.hidden || dead)
        ...
        if ((assetCount ?? 0) > 0 && !widgets.some((w) => w.type === 'folder')) {
            showTip('assets', `올려 둔 자료가 ${assetCount}개 있어요. 자주 볼 것 같으면 판에 두고 곁눈질할 수 있어요.`, { label: '자료함 올리기', run: () => addOrFocus('folder') });
            return;
        }
        if (live.length && !hasSessionCard) {
            showTip('live', `지금 ${live.length}개가 돌아가는 중이에요. 판에 올려 두면 대화가 오가는 걸 그대로 볼 수 있어요.`, { label: '세션 올리기', run: () => { const s0 = live[0]; const w = addWidget('session', { x: 80, y: 96 }, { sid: s0.id }); w.z = ++zTop; popIn(w); } });
            return;
        }
        if (widgets.length >= 2 && !widgets.some((w) => w.type === 'timeline')) {
            showTip('tl', '무슨 일이 있었는지 늘 보고 싶으면 타임라인을 판에 두세요. 필요 없으면 그때그때 알림으로만 와요.', { label: '타임라인 올리기', run: () => addOrFocus('timeline') });
        }
    }
    ...
    function ensureScrim() {
        ...
        if (!scrimEl) {
            sc.addEventListener('pointerdown', (e) => { e.stopPropagation(); unfocusWidget(); });
            scrimEl = sc;
        }
        if (sc.parentElement !== canvas)
            canvas.append(sc);
        return sc;
    }
    /** 전환 동안만 transition 을 켠다 — 상시로 켜면 끌어 옮길 때 손가락을 따라오지 못한다. */
    function animate(node, apply) {
        node.classList.add('anim');
        apply();
        window.setTimeout(() => node.classList.remove('anim'), ANIM_MS + 40);
    }
    function focusWidget(spec) {
        if (focusId && focusId !== spec.id)
            unfocusWidget();
        ...
        if (!focusPrev.has(spec.id))
            focusPrev.set(spec.id, { x: spec.x, y: spec.y, w: spec.w, h: spec.h, min: spec.min });
    ...
    function unfocusWidget(spec) {
        ...
        animate(ref.root, () => {
            ref.root.style.left = prev.x + 'px';
            ref.root.style.top = prev.y + 'px';
    ...
    const ghost = el('div', { class: 'stu-ghost', hidden: true }, el('div', { class: 'stu-w-head' }, el('span', { class: 'stu-w-ic', style: 'color:#2D6BF0;background:#2D6BF01a' }, icon('chat', 'stu-i sm')), el('b', { class: 'stu-w-t', text: '새 세션' }), el('span', { class: 'stu-ghost-tag', text: '보내면 여기서 시작돼요' })), el('div', { class: 'stu-ghost-b' }, el('p', { class: 'stu-ghost-p' }), el('div', { class: 'stu-ghost-cur', 'aria-hidden': 'true' })));
    stage.append(ghost);
    function paintAim() {
        ...
        if (isNew) {
            const p = runPrefs();
            ghost.querySelector('.stu-ghost-p').textContent = text.trim().replace(/\s+/g, ' ').slice(0, 140);
            ghost.querySelector('.stu-ghost-tag').textContent = (p.harness && p.harness !== 'shell' ? p.harness : 'claude') + ' · 보내면 여기서 시작돼요';
        }
        if (ghost.hidden === isNew)
            ghost.hidden = !isNew;
    ...
    function ghostBusy(on, prompt) {
        ghost.classList.toggle('busy', on);
        if (on) {
            ghost.hidden = false;
            ghost.querySelector('.stu-ghost-p').textContent = prompt ? prompt.replace(/\s+/g, ' ').slice(0, 140) : '빈 세션';
            ghost.querySelector('.stu-ghost-tag').textContent = '세션을 여는 중…';
        }
        else
            ghost.hidden = true;
    }
    /** 유령 창의 판 좌표(스크롤 반영) — 진짜 카드가 그 자리에서 태어난다. */
    function ghostCanvasPos() {
        if (ghost.hidden)
            return null;
    ...
    board.addEventListener('drop', (e) => {
        ...
        if (raw.startsWith('tl:')) {
            e.preventDefault();
            try {
                const p = JSON.parse(raw.slice(3));
                if (p && p.href)
                    addWidget('doc', at, { hash: String(p.href), title: String(p.label || '문서') });
            }
            catch (_) { /* noop */ }
        }
    });
    upDropZone(stage, stage, (items, emptyDirs) => { void uploadDropped(items, emptyDirs); });
    async function uploadDropped(items, emptyDirs) {
        if (!items.length && !emptyDirs.length)
            return;
        if (loose) {
            toast('이 판은 프로젝트 폴더가 없어 파일을 둘 곳이 없어요 — 세션을 프로젝트에 연결한 뒤 그 작업대에 올려 주세요.', true);
            return;
        }
        const at = lastDropAt;
        ...
        lastSpawnTs = Date.now();
        save(); // 자동 등장(autoSpawn)이 같은 파일을 한 번 더 올리지 않게
        for (const spec of widgets)
            if (spec.type === 'folder') {
                const ref = els.get(spec.id);
                if (ref) {
    ...
    function armLive() {
        liveTimer = window.setInterval(() => {
            ...
            for (const spec of widgets) {
                if (spec.min || (spec.type !== 'session' && spec.type !== 'coach'))
                    continue;
                const ref = els.get(spec.id);
                if (!ref || !ref.root.isConnected)
                    continue;
                if (focused && ref.root.contains(focused) && focused.tagName === 'INPUT')
                    continue; // 입력 중 — 건드리지 않는다(서명 비교가 있어도 안전하게)
                fillBody(spec, ref.body, false);
                if (spec.type === 'session')
                    ref.title.textContent = widgetTitle(spec);
            }
            if (tick % 4 === 0) {
                void autoSpawn();
                void paintDesk();
    ...
    // ── 시작 ──
    if (!load())
        widgets = []; // 첫 방문 = 빈 판(위젯 자동 배치 폐지, #1719 원준 2026-08-19)
    if (loose)
        syncLooseCards(true); // 다만 자투리 판은 '그 세션들' 자체가 내용이다 — 카드로 전부 올린다
    paintDoor();
    paintAll();
    mountChat();
    paintDest();
    ...
    if (!detail)
        void refreshDetail();
    return {
        destroy() { dead = true; if (liveTimer)
            clearInterval(liveTimer); chat?.destroy(); chat = null; closePeek(); for (const u of blobUrls)
            URL.revokeObjectURL(u); document.removeEventListener('pointerdown', onDocDown); document.removeEventListener('keydown', onDocKey); },
        renderAside,
    };
}
