...
export function mountStudio(host, opts) {
    ...
    function startGuide() {
        const step = (n, t, d2, act) => el('div', { class: 'stu-guide-s' }, el('span', { class: 'stu-guide-n', text: n }), el('div', {}, el('b', { text: t }), el('p', { class: 'stu-fine', text: d2 }), act ? el('button', { class: 'btn-text', type: 'button', text: act.label, onclick: act.run }) : null));
        if (loose)
        ...
        return el('div', { class: 'stu-guide' }, el('h2', { text: '여기가 이 프로젝트의 작업대예요' }), el('p', { class: 'stu-guide-l', text: '빈 캔버스로 시작해서, 이 프로젝트에 맞게 화면을 조정해보세요.' }), el('div', { class: 'stu-guide-g' }, step('1', '아래 칸에 시켜 보세요', '“랜딩 문구 3안 써 줘” 처럼 적으면 AI가 일을 시작하고, 그 대화가 이 판에 카드로 올라옵니다.'), step('2', '파일을 끌어다 놓아 보세요', '컴퓨터에서 파일·폴더를 이 판에 떨어뜨리면 이 프로젝트에서 AI랑 대화할 때 AI가 참고해서 작업합니다. AI가 작업하면서 생성한 파일도 여기에 쌓입니다.'), step('3', '위젯을 통해서 커스터마이즈 해보세요', '태스크·캘린더·메모 같은 건 ⊞ 에서 앱으로 열거나 위젯으로 캔버스에 둘 수 있어요. 지금 당장은 필요 없습니다.', { label: '⊞ 열어 보기', run: () => toggleLaunch(true) })));
    }
