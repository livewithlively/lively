...
export function mountStudio(host, opts) {
    ...
    function paintDoor() {
        ...
        door.replaceChildren(el('div', { class: 'stu-door-l' }, el('div', { class: 'stu-door-top' }, el('a', { class: 'stu-homelogo', href: '#/', title: '홈으로' }, el('span', { text: 'L' })), el('button', { class: 'stu-projbtn', type: 'button', title: '프로젝트 목록 열기/닫기', onclick: () => window.dispatchEvent(new Event('stu:toggle-projects')) }, icon('folder', 'stu-i sm'), el('span', { text: '프로젝트' }), el('span', { class: 'car', text: '▾', 'aria-hidden': 'true' })), el('span', { class: 'stu-door-sep', 'aria-hidden': 'true' }), el('div', { class: 'stu-eyebrow' }, el('span', { class: 'mono', text: '#' + p.id }), el('span', { class: 'sep', text: '·' }), el('span', { class: 'stu-state ' + st.c, text: st.t }), el('span', { class: 'sep', text: '·' }), el('span', { text: `세션 ${ss.length}` + (live.length ? ` · 지금 ${live.length}` : '') }), el('span', { class: 'sep', text: '·' }), el('span', { text: `할 일 ${tasks.length - doneN}/${tasks.length}` }), el('span', { class: 'sep', text: '·' }), el('span', { text: `지식 ${knN}` }))), el('h1', { class: 'stu-title', text: p.name || '프로젝트 #' + id })), el('div', { class: 'stu-door-r' }, el('span', { class: 'stu-faces' }, ...members.slice(0, 5).map((m) => personFace(String(m.member_id || m), 'stu-face', String(m.display_name || m.member_id || '')))), 
        // 이 줄은 **캔버스보다 위**에 있는 것들만 — 판 설정(코멘트 모드·정리)뿐이다(원준 2026-08-19).
        //  타임라인·명세는 기능이라 앱/위젯으로 내려갔다(⊞ 런치패드).
        cmBtn, el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '정리', title: '위젯 위치만 격자에 맞춰 정렬합니다(크기는 그대로)', onclick: () => { autoArrange(); paintAll(); save(); } })));
    ...
    function widgetEl(spec) {
        ...
        const w = el('div', { class: 'stu-w stu-w-' + spec.type + (spec.min ? ' min' : ''), 'data-sz': sizeOf(spec), style: `left:${spec.x}px;top:${spec.y}px;width:${spec.w}px;${spec.min ? '' : 'height:' + spec.h + 'px;'}z-index:${spec.z || 1};--wac:${d.ac}` }, head, body, el('div', { class: 'stu-w-rsz', title: '크기 조절 — 문턱을 넘으면 구성이 바뀌어요' }));
    ...
    function onDocDown(e) {
        ...
        const t = e.target;
        if (!notif.hidden && !notif.contains(t) && !streamEl.contains(t))
            toggleNotif(false);
    ...
    function focusWidget(spec) {
        ...
        if (!focusPrev.has(spec.id))
            focusPrev.set(spec.id, { x: spec.x, y: spec.y, w: spec.w, h: spec.h });
        ...
        const ref = els.get(spec.id);
        if (ref) {
            ref.root.classList.add('focused');
            ref.root.scrollIntoView({ block: 'nearest', inline: 'nearest' });
        }
    }
    function unfocusWidget(spec) {
        ...
        if (!prev)
            return;
        Object.assign(spec, prev);
        focusPrev.delete(spec.id);
        sigs.delete(spec.id);
        save();
        paintAll();
    }
