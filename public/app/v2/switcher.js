...
export function switcherTile() {
    ...
    const btn = el('button', { class: 'v2-ws v2-rail-tile', type: 'button', 'aria-haspopup': 'menu',
        title: `${w.name} · ${kindText} 워크스페이스 — 누르면 전환·연결` }, face);
    btn.onclick = (e) => { e.preventDefault(); if (openPanel) {
        closeMenu();
        return;
    } void openMenu(btn); };
...
export function switcherName() {
    ...
    const btn = el('button', { class: 'v2-ws v2-side-wsn', type: 'button', 'aria-haspopup': 'menu',
        title: `${w.name} · ${kindText} 워크스페이스 — 누르면 전환·연결` }, el('span', { class: 'v2-side-wsn-t', text: w.name }), el('span', { class: 'v2-ws-car', 'aria-hidden': 'true', text: '▾' }));
    btn.onclick = (e) => { e.preventDefault(); if (openPanel) {
        closeMenu();
        return;
    } void openMenu(btn); };
...
export function activeWorkspaceSlug() { return currentWorkspace() || 'primary'; }
/** 그 워크스페이스로 전환(헤더 선택 + 리로드). 메뉴 안의 전환과 **같은 경로**다. */
export function switchWorkspace(slug) { switchTo(slug); }
/** 지금 워크스페이스의 이름·종류 — 레일의 스택 타일·팝오버가 쓴다. */
export function workspaceInfo() { return ws(); }
/** 종전 전환·만들기·연결 메뉴를 그대로 연다 — 레일 팝오버의 [＋ 워크스페이스 추가]가 여기로 온다(폼을 두 벌 만들지 않는다). */
export function openWorkspaceMenu(anchor) { if (openPanel) {
    closeMenu();
    return;
