// v2/run-picker.ts — '무엇으로 열까' 세 칸(**제공자 · 모델 · 추론강도**)과 그 기억(#1758).
//
//  ── 왜 제공자인가 ──
...
//
//  ── 기본값 = 직전 세팅 ──
//  세 칸의 기본은 **내가 지난번에 고른 값**이다 — 클래식 '새 AI 세션' 폼이 쓰는 것과 **같은 localStorage 키**
...
//  거기선 제공자가 고정이고 모델·추론강도만 바꾼다: 이미 뜬 프로세스는 다른 회사 모델로 갈아탈 수 없다).
...
export const providerLabel = (h) => (h && h.provider && h.provider.label) || (h ? (PROVIDER_FALLBACK[h.key] || '그 밖의 제공자') : '');
// ── 카탈로그 — 한 번만 받아 세션 내내 나눠 쓴다(홈·프로젝트·대화창이 각자 부른다) ──
let cached = null;
let inflight = null;
export function runCatalog() {
    ...
    if (!inflight) {
        inflight = api('/api/ui/terminal/config')
            .then((cfg) => {
            const hs = Array.isArray(cfg && cfg.harnesses) ? cfg.harnesses : [];
            cached = hs;
            return hs;
        })
            .catch(() => []) // 못 받으면 빈 목록 — 부르는 쪽이 칸을 안 그리고 지난번 설정 그대로 연다
...
const prefsKey = () => PREFS_KEY + '::' + ((state.me && (state.me.userId || state.me.email)) || 'anon');
export function runPrefs() {
    try {
...
export function saveRunPrefs(patch) {
    try {
        localStorage.setItem(prefsKey(), JSON.stringify({ ...runPrefs(), ...patch }));
    }
    catch { /* localStorage 불가 — 기억만 못 할 뿐 */ }
}
/**
 * 제공자·모델·추론강도 세 칸을 만든다. 카탈로그는 비동기로 오므로 **먼저 빈 자리를 반환하고** 도착하면 채운다 —
 * 그 사이에 사람이 Enter 를 쳐도 value() 가 저장된 직전 설정을 그대로 돌려주므로 세션은 정상적으로 열린다.
 */
export function createRunPicker(opts) {
    const remember = opts?.remember !== false;
    const prefs = runPrefs();
    const savedFlags = (prefs.flags && typeof prefs.flags === 'object') ? { ...prefs.flags } : {};
    let harnesses = [];
    ...
    const root = el('div', { class: 'v2-run' }, provSel, modelSel, effortSel);
    provSel.replaceChildren(el('option', { value: '' }, '불러오는 중…'));
    provSel.disabled = true;
    modelSel.hidden = true;
    effortSel.hidden = true;
    const cur = () => findHarness(harnesses, harnessKey);
    const AXES = ['--model', '--effort'];
    const boxOf = (name) => (name === '--model' ? modelSel : effortSel);
    ...
    function paint() {
        // 셸은 AI 가 아니라 여기 후보가 아니다 — 이 칸은 '무엇에게 시킬까'를 묻는다.
        const list = harnesses.filter((h) => h.key !== 'shell');
        provSel.disabled = !list.length;
        if (!list.length) {
            provSel.replaceChildren(el('option', { value: '' }, '지난번 설정 그대로'));
            modelSel.hidden = true;
            effortSel.hidden = true;
            return;
        }
        if (!list.some((h) => h.key === harnessKey))
            harnessKey = list[0].key;
    ...
    const changed = () => {
        for (const n of AXES)
            if (!boxOf(n).hidden)
                savedFlags[n] = boxOf(n).value;
        if (remember)
            saveRunPrefs({ harness: harnessKey, flags: { ...savedFlags } });
        opts?.onChange?.(pick());
    };
    ...
    const pick = () => {
        ...
        return { harness: harnessKey, flags };
    };
    void runCatalog().then((hs) => { harnesses = hs; paint(); });
    return {
        el: root,
        value: pick,
        disable(on) { provSel.disabled = on || !harnesses.length; modelSel.disabled = on; effortSel.disabled = on; },
    };
}
