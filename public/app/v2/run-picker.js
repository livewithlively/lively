...
function loadConfig() {
    ...
    if (!inflight) {
        inflight = api('/api/ui/terminal/config')
            ...
            .catch(() => ({ harnesses: [], nodes: [] })) // 못 받으면 빈 목록 — 부르는 쪽이 칸을 안 그리고 지난번 설정 그대로 연다
            .finally(() => { inflight = null; });
    }
    return inflight;
}
export function runCatalog() { return loadConfig().then((c) => c.harnesses); }
/** 실행 노드 목록(#1744) — 내가 세션을 만들 수 있는 노드(서버가 소유·공유로 이미 필터). 온라인만 실제 생성 가능(폼이 게이트). */
export function runNodes() { return loadConfig().then((c) => c.nodes); }
...
export function createRunPicker(opts) {
    ...
    function paintNode() {
        ...
        nodeSel.replaceChildren(el('option', { value: '' }, '중앙 컴퓨터 (기본)'), ...nodes.map((n) => {
            ...
            if (!n.online)
                o.disabled = true; // 연결돼 있어야 생성 가능(서버도 409 재검증)
            return o;
        }));
    ...
    const changed = () => {
        ...
        if (remember)
            saveRunPrefs({ harness: harnessKey, flags: { ...savedFlags }, node: nodeKey });
        opts?.onChange?.(pick());
    };
    ...
    return {
        el: root,
        value: pick,
        disable(on) { nodeSel.disabled = on || !nodes.length; provSel.disabled = on || !harnesses.length; modelSel.disabled = on; effortSel.disabled = on; },
    };
}
