...
export function hasBrowserSurface() {
    const d = window.livelyDesktop;
 ...
 */
export function browserSurface(opts) {
    ...
    const view = live
        ? el('webview', { class: 'v2-bs-view', src: start, allowpopups: null })
        : null;
    const nav = (raw) => {
        const u = toUrl(raw);
        if (!u)
    ...
    const btn = (label, title, fn) => el('button', { class: 'v2-bs-btn', type: 'button', title, 'aria-label': title, text: label, onclick: fn });
    const back = btn('‹', '뒤로', () => { if (view && view.canGoBack && view.canGoBack())
        view.goBack(); });
    const fwd = btn('›', '앞으로', () => { if (view && view.canGoForward && view.canGoForward())
        view.goForward(); });
    const reload = btn('↻', '새로고침', () => { if (view)
        view.reload();
    else
        nav(addr.value); });
    const ext = extBridge();
    const head = el('div', { class: 'v2-bs-bar' }, el('span', { class: 'v2-bs-navs' }, back, fwd, reload), addr, el('span', { class: 'v2-bs-acts' }, ext ? el('button', { class: 'btn-text', type: 'button', text: '확장', title: '이 브라우저에 설치된 확장',
        onclick: (e) => openExtensions(e.currentTarget, ext) }) : null, outLink));
    ...
    function fallbackUrl(u) {
        ...
        body.replaceChildren(el('div', { class: 'v2-bs-fallback' }, el('p', { class: 'v2-bs-fb-h', text: '이 화면 안에서는 열 수 없습니다.' }), el('p', { class: 'v2-bs-fb-p', text: '많은 사이트가 다른 페이지 안에 끼워 넣는 것을 스스로 막습니다. 브라우저에서는 그 제한을 넘을 수 없어요.' }), el('p', { class: 'v2-bs-fb-p', text: '라이블리 데스크톱 앱에서 열면 이 자리에 그대로 뜹니다.' }), el('a', { class: 'btn btn-primary btn-sm', href: u, target: '_blank', rel: 'noopener noreferrer', text: host + ' 새 탭에서 열기 ↗' })));
    }
    if (view) {
        ...
        view.addEventListener('did-fail-load', (e) => {
            ...
            if (!e || e.isMainFrame === false || e.errorCode === -3)
                return;
            body.replaceChildren(el('div', { class: 'v2-bs-fallback' }, el('p', { class: 'v2-bs-fb-h', text: '페이지를 열지 못했습니다.' }), el('p', { class: 'v2-bs-fb-p', text: (e.errorDescription || '알 수 없는 오류') + ' — ' + (e.validatedURL || addr.value) }), el('button', { class: 'btn btn-sm', type: 'button', text: '다시 시도', onclick: () => { body.replaceChildren(view); view.reload(); } })));
        });
    }
    else {
        fallbackUrl(start);
...
async function openExtensions(anchor, ext) {
    ...
    const draw = async () => {
        ...
        try {
            ...
            if (!r.ok)
                throw new Error(r.error || '목록을 읽지 못했습니다');
            items = r.items || [];
        }
        catch (e) {
        ...
        for (const it of items) {
            body.append(el('div', { class: 'v2-bs-ext-row' }, el('span', { class: 'v2-bs-ext-nm', text: it.name + (it.version ? ' ' + it.version : '') }), el('button', {
                class: 'btn-text', type: 'button', text: '제거',
                onclick: async () => {
                    const r = await ext.remove(it.id);
                    if (!r.ok) {
                        toast(r.error || '제거하지 못했습니다');
                        return;
                    }
