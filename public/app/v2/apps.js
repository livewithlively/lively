        ...
        const session = sApps.filter((a) => !q || a.title.toLowerCase().includes(q) || a.id.toLowerCase().includes(q)).map((a) => {
            ...
            return el('button', { class: 'v2-pad-item v2-pad-item--app', role: 'listitem', type: 'button',
                ...
                onclick: () => { closeLaunchpad(); if (hasUi)
                    ...
                    void openAppSession(a.id, { title: a.title }); } }, el('span', { class: 'v2-pad-ico' }, appIcon(hasUi ? 'apps' : 'term')), el('b', { text: a.title }), el('span', { class: 'v2-pad-desc', text: hasUi ? '앱 화면 — 창으로 열려요' : '앱 세션 — 열면 이 앱으로 대화가 시작돼요' }), el('span', { class: 'v2-pad-badge', text: hasUi ? '앱' : '세션 앱' }));
        });
