...
function init() {
    ...
    void listWorkspaces().then((rows) => { if (rows.length > 1) {
        spaces = rows;
        drawRail();
    } });
}
...
const ln = (k, cls = 'v2-rail-ic') => sv('svg', { viewBox: '0 0 24 24', class: cls, 'aria-hidden': 'true' }, ...D[k].map((d) => sv('path', { d })));
const secIcon = (s) => (s.icon === 'home' ? ln('home') : appIcon(s.icon, 'v2-rail-ic'));
...
export function drawRail() {
    ...
    const secEls = railSections().map((s) => {
        ...
        }, secIcon(s), el('span', { class: 'v2-rail-t', text: s.label }), 
    ...
    }, appIcon(a.icon, 'v2-rail-ic'), el('span', { class: 'v2-rail-t', text: a.title }), running.has(a.key) ? el('span', { class: 'v2-rail-run', role: 'img', 'aria-label': '실행 중' }) : null));
    const mid = el('div', { class: 'v2-rail-mid' }, ...secEls, recentEls.length ? el('div', { class: 'v2-rail-hr', role: 'presentation' }) : null, recentEls.length && open ? el('div', { class: 'v2-rail-k', text: '최근 앱' }) : null, ...recentEls);
    // ③ 발치 — 접기/펼치기 · 앱 · 나
    const me = state.me || {};
    const myName = String(me.display_name || me.email || me.userId || '');
    const foot = el('footer', { class: 'v2-rail-foot' }, el('button', {
        class: 'v2-rail-tg', type: 'button', 'aria-expanded': String(open),
        'data-tip': open ? '레일 접기' : '레일 펼치기',
        title: (open ? '레일 접기' : '레일 펼치기') + ' — ⌘⇧S',
        onclick: () => toggleRail(),
    }, ln(open ? 'chevL' : 'chevR'), el('span', { class: 'v2-rail-t', text: open ? '레일 접기' : '레일 펼치기' })), el('button', {
        class: 'v2-rail-it', type: 'button', 'data-tip': '앱 — 모든 앱', title: '모든 앱',
        onclick: () => openLaunchpad(),
    }, ln('grid'), el('span', { class: 'v2-rail-t', text: '앱' })), el('button', {
    ...
    }, profileAvatar(me.avatar, myName, me.userId, 'v2-ava', { char: me.avatar_char, color: me.avatar_color }), el('span', { class: 'v2-rail-t', text: myName }), ln('gear', 'v2-rail-gear')));
...
function wsListOpen() {
    ...
    }, el('span', { class: 'v2-wscard-big' + (w.kind === 'personal' ? ' round' : ''), text: String(w.name || w.slug).trim().slice(0, 1) }), el('span', { class: 'v2-rail-t', text: String(w.name || w.slug) }), el('span', { class: 'v2-rail-kd', text: w.kind === 'personal' ? '개인' : '팀' }))));
}
