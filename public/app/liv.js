...
export async function renderLiv(view) {
    ...
    const chatWrap = el('div', { class: 'liv-chat' }, el('div', { class: 'liv-chat-head' }, el('span', { text: '리브와 대화' })), el('div', { class: 'liv-chat-body', id: 'liv-chat-body' }, el('div', { class: 'liv-chat-boot', text: '세션을 준비하고 있습니다…' })));
...
async function fillLivCards(host) {
    ...
    host.replaceChildren(el('div', { class: 'liv-cards-title', text: `지금 손볼 것 ${st.total}가지` }), ...st.findings.map((f) => livCard(f, host)));
}
...
async function bootLivSession(host, cards) {
    ...
    if (fresh)
        void sendWhenReady(s.id, LIV_OPENING);
}
/**
 ...
 */
async function sendWhenReady(sessionId, text) {
    for (let i = 0; i < 40; i++) { // 최대 ~80초
        ...
        try {
            ...
            const p = await api('/api/ui/terminal/sessions/' + encodeURIComponent(sessionId) + '/prompts')
                .catch(() => null);
            if (p && Array.isArray(p.prompts) && p.prompts.length)
                return;
