...
export async function renderLiv(view, opts = {}) {
    ...
    const head = el('header', { class: 'liv-top' }, el('div', { class: 'liv-top-row' }, el('span', { class: 'liv-face', 'aria-hidden': 'true', text: 'L' }), el('div', { class: 'liv-top-name' }, el('b', { text: '리브' }), el('span', { class: 'liv-top-role', text: '워크스페이스 담당자' })), opts.embedded ? null : el('a', {
        class: 'btn btn-sm btn-ghost liv-top-out', href: '#/dashboard', text: '← 라이블리',
        title: '라이블리 홈으로 돌아갑니다.',
    })), el('p', { class: 'liv-top-say' }, el('b', { text: '이 워크스페이스를 대신 손보는 담당자예요.' }), document.createTextNode(' 설명서를 드리지 않습니다 — 말씀하시면 제가 직접 합니다.')));
    ...
    const chatWrap = el('div', { class: 'liv-chat' }, el('div', { class: 'liv-chat-body', id: 'liv-chat-body' }, el('div', { class: 'liv-chat-boot', text: '준비하고 있습니다…' })));
    ...
    view.append(el('div', { class: 'liv-wrap' }, head, el('div', { class: 'liv-body liv-body-solo' }, letter, chatWrap)));
 ...
 */
export async function fillLivCards(host, askHost) {
    ...
    try {
        ...
        host.replaceChildren(el('div', { class: 'liv-letter-quiet' }, el('p', { class: 'liv-quiet-say', text: '지금 상태를 읽지 못했습니다.' }), el('p', { class: 'liv-quiet-sub', text: '아래에서 저에게 직접 물어보셔도 됩니다.' })));
        return;
    }
 ...
 */
function livLetter(st, host) {
    const n = st.findings.length;
    const lead = n === 1
        ? `${meName()}지금 한 가지가 걸립니다.`
        : `${meName()}지금 ${n}가지가 걸립니다.`;
    return el('article', { class: 'liv-letter-body' }, el('p', { class: 'liv-lead', text: lead }), ...st.findings.map((f, i) => livPara(f, i, n, host)), el('p', { class: 'liv-sign' }, el('span', { text: '— 리브' })));
}
/** 편지의 한 문단 = 제안 하나. 무엇을 봤는지(제목·설명) + 제가 하겠다(버튼)까지가 한 덩어리다. */
function livPara(f, i, total, host) {
    ...
    if (f.href)
        acts.append(el('a', { class: 'btn btn-sm', href: f.href, text: f.prompt ? '직접 볼게요' : '보러 가기' }));
    acts.append(el('button', {
        ...
        onclick: (ev) => {
            ev.currentTarget.closest('.liv-para')?.remove();
            if (!host.querySelector('.liv-para'))
                refreshLiv();
        },
    }));
    return el('div', { class: 'liv-para liv-para-' + f.severity }, total > 1 ? el('span', { class: 'liv-para-no', text: ORDINAL[i] ?? String(i + 1) }) : null, el('div', { class: 'liv-para-b' }, el('p', { class: 'liv-para-t', text: f.title }), f.detail ? el('p', { class: 'liv-para-d', text: f.detail }) : null, acts));
}
/**
 ...
 */
function livQuiet() {
    const say = (t) => el('button', {
        class: 'liv-can-row', type: 'button', onclick: () => livChatFill(t),
    }, el('span', { class: 'liv-can-mark', 'aria-hidden': 'true', text: '·' }), el('span', { text: t }));
    return el('div', { class: 'liv-letter-quiet' }, el('p', { class: 'liv-quiet-say', text: '오늘은 드릴 말씀이 없습니다.' }), el('p', { class: 'liv-quiet-sub', text: '새로 눈에 띄는 게 생기면 제가 먼저 말씀드릴게요. 그동안 무엇이든 시키셔도 됩니다.' }), el('div', { class: 'liv-can' }, el('div', { class: 'liv-can-h', text: '이런 것도 부탁하실 수 있어요' }), say('쓰던 노션이랑 연결해 주세요'), say('매주 회의록을 여기 쌓고 싶어요'), say('지금 워크스페이스에서 손볼 게 있는지 봐 주세요')));
}
/**
 * 객관식 질문 — **사람은 고르기만 한다.**
 *
