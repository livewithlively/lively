...
export async function ingestPolicyPanel(detail, data) {
    ...
    try {
        ...
        detail.replaceChildren(el('div', { class: 'card' }, errorNote(e, '검토 게이트 설정을 불러오지 못했습니다')));
        return;
    }
    let obs = null;
    try {
    ...
    detail.replaceChildren(el('div', {}, el('div', { class: 'section-title' }, el('h2', { text: '지식 검토 정책' })), el('p', { class: 'admin-hint', text: 'AI가 기록한 지식을 사람이 확인한 뒤에 유효해지도록 할지 정합니다.' }), el('p', { class: 'admin-hint ctx-perm-line' }, canEdit ? null : el('span', { class: 'pill', text: '읽기 전용' }), el('span', { class: 'admin-only-badge', text: '관리자' }), ' 이 정책을 바꾸는 일은 관리자 전용입니다 — 조직 전체의 지식 인입 게이트라, 규칙 하나가 모든 구성원의 기록에 걸립니다. ', canEdit ? '설정 내용은 모든 구성원이 볼 수 있습니다.'
        : '지금 어떤 규칙이 걸려 있는지는 아래에서 그대로 보실 수 있습니다 — 내 지식이 왜 검토 대기로 갔는지 확인할 수 있습니다.')), el('div', { class: 'card' }, cardHead('검토 게이트', '켜면 에이전트가 기록한 지식은 [WIKI ▸ 검토 대기]로 이동하고, 승인 전까지는 검색·세션주입·목록에 표시되지 않습니다. 사람이 웹에서 직접 쓴 지식은 영향을 받지 않습니다.'), 
    // 비-admin 에겐 읽기 전용 요약만 — 누를 수 없는 스위치·버튼은 안내가 아니라 미끼다(#1419).
    canEdit ? gateCard(preset, obs, reload) : gateSummaryReadonly(preset), canEdit ? rulesSection(rules, reload) : rulesSummaryReadonly(rules)));
}
/** 게이트 상태 읽기 전용 — 스위치 없이 '지금 켜져 있나'만. */
function gateSummaryReadonly(preset) {
    const on = !!(preset && preset.enabled);
    return el('div', { class: 'mini-meta' }, el('span', { class: 'pill' + (on ? ' pill-ok' : ''), text: on ? '켜짐' : '꺼짐' }), el('span', { text: on
            ? '  에이전트가 기록한 지식은 승인 전까지 검색·세션주입에 나오지 않습니다.'
            : '  에이전트가 기록한 지식이 곧바로 유효해집니다.' }));
}
/** 세부 규칙 읽기 전용 — 무엇이 걸려 있는지만(편집·추가 없음). */
function rulesSummaryReadonly(rules) {
    const box = el('div', { style: 'margin-top:12px' }, el('div', { class: 'section-title' }, el('h2', { text: '세부 규칙' })));
    if (!rules.length) {
        box.append(el('p', { class: 'admin-hint', text: '세부 규칙이 없습니다 — 위 게이트 설정만 적용됩니다.' }));
        return box;
    }
    for (const p of rules) {
        box.append(el('div', { class: 'mini-meta' }, el('span', { class: 'pill' + (p.enabled ? ' pill-ok' : ''), text: p.enabled ? '켜짐' : '꺼짐' }), el('span', { class: 'pill', text: '신규 ' + (p.action || 'auto') }), el('span', { class: 'pill', text: '수정 ' + (p.action_update || 'auto') }), el('span', { text: '  ' + (p.note || describeMatch(p)) })));
    }
    return box;
}
...
function describeMatch(p) {
    const bits = [];
    if (p.match_category)
        bits.push(`분류 ${p.match_category}`);
    if (p.match_system)
        bits.push(`출처 ${p.match_system}`);
    if (p.match_channel)
        bits.push(`채널 ${p.match_channel}`);
    if (p.match_actor_kind)
        bits.push(p.match_actor_kind === 'ai' ? 'AI 작성' : '사람 작성');
    if (p.match_agent)
        bits.push(`하네스 ${p.match_agent}`);
    if (p.match_type)
        bits.push(`종류 ${p.match_type}`);
    if (p.match_provenance)
        bits.push(`출처유형 ${p.match_provenance}`);
    if (p.match_sensitive)
        bits.push(`민감 ${p.match_sensitive}`);
...
function gateCard(preset, obs, reload) {
    ...
    const createSel = el('select', { class: 'rq-sel', disabled: !on });
    for (const [v, t] of CREATE_ACTS)
        createSel.append(el('option', { value: v, text: t }));
    ...
    const updSel = el('select', { class: 'rq-sel', disabled: !on });
    for (const [v, t] of UPDATE_ACTS)
        updSel.append(el('option', { value: v, text: t }));
...
function rulesSection(rules, reload) {
    ...
    for (const p of rules) {
        ...
        const main = el('div', { class: 'rq-row-main', style: 'cursor:default' }, p.is_exception ? el('span', { class: 'rq-badge edit', text: '예외' }) : null, el('span', { class: 'rq-title', text: axes }), el('span', { class: 'rq-meta' }, el('span', { text: '신규: ' + (ACT_SHORT[p.action] || p.action) }), el('span', { text: '수정: ' + (ACT_SHORT[p.action_update] || p.action_update || '즉시 반영') }), p.enabled ? null : el('span', { class: 'rq-warn', text: '꺼짐' })), el('span', { class: 'rq-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: p.enabled ? '끄기' : '켜기', onclick: () => ruleToggle(p, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: '수정', onclick: () => openIngestPolicyForm(p, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: '삭제', onclick: () => ruleDelete(p.id, reload) })));
        rows.append(el('div', { class: 'rq-row' }, main));
    }
    const det = el('details', { style: 'margin-top:18px' }, el('summary', { style: 'cursor:pointer;font-size:13px;font-weight:700;color:var(--ink-sub)', text: `세부 규칙 (고급) — ${rules.length}개` }), el('p', { class: 'admin-hint', style: 'margin-top:10px', text: '도메인·하네스·출처·지식 종류별로 다르게 적용하려면 규칙을 추가하세요. 여러 규칙에 걸리면 가장 보수적인 쪽이 적용됩니다(신규 금지>검토>즉시 · 수정 금지>승인 후>사후검토>즉시). “예외” 규칙은 그 누적을 건너뛰고 그 규칙대로 확정합니다 — “전부 검토하되 이 도메인만 자동통과” 같은 완화는 예외로만 가능합니다.' }), el('div', { style: 'margin:8px 0 10px' }, el('button', { class: 'btn btn-ghost btn-sm', text: '+ 규칙 추가', onclick: () => openIngestPolicyForm(null, reload) })), rows);
    return det;
}
async function ruleToggle(pol, reload) {
    try {
...
async function ruleDelete(id, reload) {
...
// 규칙 폼 — 8축 + 신규/수정 액션 + 예외.
export async function openIngestPolicyForm(pol, reload) {
    const isNew = !pol;
    ...
    const sel = (opts, cur) => {
        const s = el('select', { class: 'rq-sel' });
        for (const [v, t] of opts)
            s.append(el('option', { value: v, text: t }));
        if (cur) {
            if (!opts.some((o) => o[0] === cur))
                s.append(el('option', { value: cur, text: cur + ' (미등록)' }));
            s.value = cur;
        }
        return s;
    };
    ...
    const prioInp = el('input', { type: 'number', class: 'rq-sel', value: String((pol && pol.priority) || 0) });
...
export async function reviewQueuePanel(detail, data) {
    ...
    try {
        const r = await api('/api/ui/org/ingest-policy');
        const p = ((r && r.policies) || []).find((x) => x.preset === GATE_PRESET);
        gateOn = !!(p && p.enabled);
    }
    catch {
        gateOn = null;
    ...
    const paint = () => {
        const vis = visible();
        if (rqUi.cur >= vis.length)
            rqUi.cur = Math.max(0, vis.length - 1);
...
function renderGroups(vis, paint, drop) {
    if (!vis.length) {
        return [el('div', { class: 'rq-rows' }, el('div', { class: 'rq-empty', text: '검토 대기 중인 항목이 없습니다.' }))];
    }
    ...
    for (const [gname, gitems] of groups) {
        ...
        for (const it of gitems)
            rows.append(renderRow(it, idx++, paint, drop));
        out.push(el('div', { class: 'rq-group' }, el('div', { class: 'rq-grouphead' }, el('span', { text: gname }), el('span', { class: 'rq-groupcount', text: String(gitems.length) })), rows));
    }
    return out;
}
function renderRow(it, idx, paint, drop) {
    ...
    const chk = el('input', {
        ...
        onclick: (e) => {
            e.stopPropagation();
            if (rqUi.sel.has(it.key))
                rqUi.sel.delete(it.key);
            else
                rqUi.sel.add(it.key);
            paint();
        },
    ...
    const meta = el('span', { class: 'rq-meta' }, it.kind === 'edit'
        ? el('span', { class: 'rq-delta' }, el('span', { class: 'add', text: '+' + (it.added ?? 0) }), el('span', { text: ' ' }), el('span', { class: 'del', text: '−' + (it.removed ?? 0) }))
        : null, it.conflict ? el('span', { class: 'rq-warn', title: '제안 이후 라이브 본문이 또 바뀌었습니다 — 승인하면 그 변경을 덮어씁니다', text: '⚠ 충돌' }) : null, it.type ? el('span', { text: TYPE_LABEL[it.type] || it.type }) : null, el('span', { text: whoText }), el('span', { text: relTime(it.at) }));
    const main = el('div', {
        class: 'rq-row-main',
        onclick: () => { if (rqUi.open.has(it.key))
            rqUi.open.delete(it.key);
        else
            rqUi.open.add(it.key); rqUi.cur = idx; paint(); },
    }, chk, badge, el('span', { class: 'rq-title', text: it.title }), meta, el('span', { class: 'rq-acts' }, el('a', {
        class: 'btn btn-ghost btn-sm', href: '#/knowledge/' + encodeURIComponent(it.name), text: '열기',
        onclick: (e) => e.stopPropagation(),
    }), el('button', { class: 'btn btn-ghost btn-sm', text: '✓ 승인', onclick: (e) => { e.stopPropagation(); approve(it, drop); } }), el('button', { class: 'btn btn-ghost btn-sm', text: '✕ 반려', onclick: (e) => { e.stopPropagation(); reject(it, drop); } })));
...
async function fillExpand(exp, it) {
    try {
        if (it.kind === 'new') {
        ...
        if (!cur) {
            exp.replaceChildren(el('p', { class: 'rq-exp-note rq-warn', text: '대상 지식이 삭제되었습니다 — 반려로 정리하세요.' }));
            return;
        }
        ...
        exp.replaceChildren(el('p', { class: 'rq-exp-note', text: note + (rev.edits > 1 ? ` (에이전트가 ${rev.edits}번 저장 — 마지막 검토 이후 누적 변화)` : '') }), diffView(beforeTxt, afterTxt));
    }
    catch (e) {
...
export function diffView(before, after) {
    ...
    d.forEach((l, i) => {
        if (l.t === ' ')
            return;
        for (let k = Math.max(0, i - CTX); k <= Math.min(d.length - 1, i + CTX); k++)
            keep[k] = true;
    });
    ...
    const flush = () => {
        if (skipped)
            box.append(el('div', { class: 'rq-dl skip', text: `⋯ ${skipped}줄 생략` }));
        skipped = 0;
    };
    d.forEach((l, i) => {
        if (!keep[i]) {
        ...
        box.append(el('div', { class: cls }, el('span', { class: 'sig', text: l.t === ' ' ? '' : l.t }), el('span', { text: l.s })));
    });
    flush();
    return box;
}
// ── 액션 ──
async function approve(it, drop) {
    try {
        if (it.kind === 'new') {
...
function bulkBar(items, paint, drop) {
    const n = rqUi.sel.size;
    const go = el('button', { class: 'btn btn-primary btn-sm', text: `선택 ${n}건 승인` });
    go.onclick = async () => {
        if (!confirm(`${n}건을 승인할까요?`))
            return;
        go.disabled = true;
        const keys = [...rqUi.sel];
        let ok = 0, fail = 0;
    ...
    return el('div', { class: 'rq-bulk' }, el('b', { text: `${n}건 선택됨` }), go, el('button', { class: 'btn btn-ghost btn-sm', text: '선택 해제', onclick: () => { rqUi.sel.clear(); paint(); } }));
}
// ── 키보드 — 손이 마우스로 안 떠나게. 입력창 안에서는 절대 가로채지 않는다(undo.ts inEditableOwner 관례). ──
function installKeys(listBox, visible, paint, drop) {
    if (rqKeys)
        rqKeys.abort();
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Enter' || e.key === ' ') {
            ...
            if (!it)
                return;
            if (rqUi.open.has(it.key))
                rqUi.open.delete(it.key);
            else
                rqUi.open.add(it.key);
            paint();
            return;
        }
        if (e.key === 'x') {
