...
async function renderHomeSurface(box, ctx) {
    ...
    const arrived = new Map(); // 도착한 보강값(재렌더에도 재적용)
    let dragId = null;
    let dragGroup = null;
    function catCard(c, group, compact) {
        ...
        const card = el('div', { class: 'wk-ccard' + (compact ? ' compact' : ''), role: 'link', tabindex: '0', title: c.name || c.key }, wkAurora(String(c.key || c.id), c.space, { cls: 'wk-ccard-cover', watermark: seedIcon || (Array.from(String(c.name || c.key || '?').trim())[0] || '').toUpperCase() }), el('div', { class: 'wk-ccard-body' }, icEl, el('div', { class: 'wk-ccard-main' }, el('div', { class: 'wk-ccard-name', text: c.name || c.key }), c.description ? el('div', { class: 'wk-ccard-desc', text: c.description }) : null), el('div', { class: 'wk-ccard-meta' }, otherTeam ? el('span', { class: 'wk-row-m wk-otherteam', title: '담당 팀', text: otherTeam }) : null, Number.isFinite(cnt) ? el('span', { class: 'wk-row-m', text: cnt + '개' }) : null, timeEl)));
        ...
        slot.addEventListener('dragover', (ev) => {
            if (!dragId || dragGroup !== group || dragId === String(c.id))
                return;
            ev.preventDefault();
    ...
    function clearDropHints() {
        home.querySelectorAll('.wk-hslot.drop-before, .wk-hslot.drop-after').forEach((n) => n.classList.remove('drop-before', 'drop-after'));
    }
    function enrichIconOf(c) {
    ...
    if (pinned.length) {
        ...
        sec.body.append(el('div', { class: 'wk-doccard-grid wk-pinned-grid' }, ...show.map((e) => wkDocCard(e, { open: openFrom(names), deckCap: 110 }))));
        home.append(sec.el);
    }
    ...
    if (!recent.length) {
        ...
        let grid = null;
        const flushHidden = () => {
            if (hiddenInDay > 0 && grid)
                grid.after(el('div', { class: 'wk-day-more', text: '… ' + hiddenInDay + '개 더' }));
            hiddenInDay = 0;
        };
        ...
        for (const [id, got] of arrived) {
            ...
            if (slot && slot.icEl && slot.icEl.isConnected) {
                if (got.icon) {
                    ...
                    if (slot.wm)
                        slot.wm.textContent = got.icon;
                }
                if (got.latest && slot.timeEl)
                    slot.timeEl.textContent = relTime(got.latest);
            }
        }
    }
