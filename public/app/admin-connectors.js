...
function connectorEditor(detail, data) {
    ...
    for (const c of connectors) {
        ...
        listCol.append(el('div', { class: 'mini-row' + (c.system === sel ? ' sel' : ''),
            onclick: () => { state.admin.connectorSel = c.system; rerenderPanel(detail, 'connectors', data); } }, el('div', { class: 'mini-title', text: c.label }, c.enabled ? el('span', { class: 'pill', text: '자동 싱크' }) : null), el('div', { class: 'mini-meta', text: secTotal ? `토큰 ${setCount}/${secTotal} 등록됨` : '토큰 불필요' })));
    }
    const right = el('div', {});
