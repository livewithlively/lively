...
async function renderTerminal(view, teamId) {
    ...
    if (teamId) {
        renderTeamView(view, cfg, teams, sessions, teamId);
        return;
    }
    ...
    function repaint() {
        ...
        headActions.replaceChildren(el('button', { class: 'btn btn-ghost', text: '+ 새 팀', onclick: () => openTeamCreateForm(cfg, view) }), el('button', { class: 'btn btn-primary', text: '+ 새 공개 세션', onclick: () => openTermCreateForm(cfg, view, null, 'public') }), el('button', { class: 'btn btn-ghost', text: '+ 새 비공개 세션', onclick: () => openTermCreateForm(cfg, view, null, 'private') }));
        ...
        if (teams.length) {
            ...
            for (const t of teams)
                tlist.append(teamRow(t, cfg, view));
        ...
        sections.forEach(([secHead, secList], i) => {
            if (i > 0)
                secHead.classList.add('term-section--div'); // 첫 섹션 빼고 위에 구분선
...
function termSessionList(items, cfg, view, emptyText, sel) {
    ...
    for (const s of items)
        list.append(termRow(s, cfg, view, null, sel));
    return list;
}
...
function termPublicSection(pubMine, pubOthers, cfg, view, sel) {
    const wrap = el('div', {});
    if (pubMine.length)
        wrap.append(termSessionList(pubMine, cfg, view, '', sel));
    else if (!pubOthers.length)
        wrap.append(termSessionList([], cfg, view, '공개 세션이 없습니다. "새 세션"으로 만드세요.'));
    else
        wrap.append(el('div', { class: 'caption', style: 'margin-top:10px', text: '내가 만든 공개 세션은 없습니다.' }));
    if (pubOthers.length) {
        const list = termSessionList(pubOthers, cfg, view, '');
        list.style.display = 'none';
        const caret = el('span', { class: 'term-fold-caret', text: '▾' });
        const toggle = el('button', { class: 'term-fold', type: 'button' }, caret, el('span', { text: '다른 멤버의 공개 세션 ' + pubOthers.length + '개' }));
...
function teamRow(t, cfg, view) {
    ...
    const meta = el('div', { class: 'term-row-meta' }, el('div', { class: 'term-row-title' }, el('span', { text: '📁 ' + t.label }), t.owned ? el('span', { class: 'term-badge', text: '내 팀' }) : null), el('div', { class: 'caption', text: '구성원 ' + count + '명' + ((t.memberNames && t.memberNames.length) ? ' · ' + t.memberNames.join(', ') : '') }));
    ...
    if (t.owned) {
        ...
        actions.push(el('button', { class: 'btn btn-ghost btn-sm', text: '해제', onclick: async () => {
                if (!confirm('팀 "' + t.label + '"을(를) 해제할까요? 폴더와 파일은 그대로 두고 팀 묶음(접근 제한)만 풉니다.'))
                    return;
                try {
                    await api('/api/ui/terminal/teams/' + encodeURIComponent(t.id), { method: 'DELETE' });
                    toast('팀 해제됨');
                    renderTerminal(view, null);
                }
                catch (e) {
...
function renderTeamView(view, cfg, teams, sessions, teamId) {
    ...
    if (!team || team.accessible === false) {
        ...
        view.replaceChildren(el('div', { class: 'page-head' }, el('div', { class: 'term-head-actions' }, back), el('h1', { text: '팀 세션' })), el('div', { class: 'empty', text: msg }));
        return;
    }
    const ownerName = team.owned ? myName(cfg) : (memberName(cfg, team.owner) || '소유자');
    const memberBadges = el('div', { class: 'term-members' }, el('span', { class: 'term-badge owner', text: '👑 ' + ownerName }), ...((team.memberNames || []).map((n) => el('span', { class: 'term-badge', text: '👤 ' + n }))));
    const head = el('div', { class: 'page-head' }, el('div', { class: 'term-head-actions' }, back, el('button', { class: 'btn btn-primary', text: '+ 새 세션', onclick: () => openTermCreateForm(cfg, view, team) }), team.owned ? el('button', { class: 'btn btn-ghost', text: '팀원 관리', onclick: () => openTeamManageForm(team, cfg, view) }) : null), el('h1', { text: '📁 ' + team.label }), memberBadges, el('div', { class: 'caption', text: '이 팀 세션은 같은 팀원끼리만 자유롭게 보고 열 수 있습니다.' }));
    const mine = sessions.filter((s) => s.team === teamId);
    const list = el('div', { class: 'term-list' });
    if (!mine.length)
        list.append(el('div', { class: 'empty', text: '이 팀에 세션이 없습니다. "새 세션"으로 만드세요.' }));
    for (const s of mine)
        list.append(termRow(s, cfg, view, team));
...
function termRow(s, cfg, view, team, sel) {
    ...
    const meta = el('div', { class: 'term-row-meta' }, el('div', { class: 'term-row-title' }, el('span', { class: 'term-row-name', title: s.label, text: s.label }), el('span', { class: 'term-badge author', text: '👤 ' + author }), el('span', { class: 'term-badge', text: s.visibility === 'private' ? '비공개' : '공개' }), s.autoApprove ? el('span', { class: 'term-badge danger', text: '자동승인' }) : null, s.attached ? el('span', { class: 'term-badge', text: '접속중' }) : null), el('div', { class: 'caption', text: harnessLabel + (created ? ' · ' + created : '') + (s.dir ? ' · ' + s.dir : '') }));
...
function openTermCreateForm(cfg, view, team, fixedVis) {
    ...
    const visSel = el('select', { class: 'term-input' }, el('option', { value: 'public' }, '공개 — 모든 멤버가 열람 가능한 세션 (팀 내부 세션은 팀원만 열람가능)'), el('option', { value: 'private' }, '비공개 — 나에게만 보이고 나만 열 수 있는 세션'));
    // 헤더의 '새 공개/비공개 세션' 버튼으로 들어오면 공개여부가 정해져 있으므로 자동선택+비활성(회색)으로 둔다.
    if (fixedVis) {
        visSel.value = fixedVis;
        visSel.disabled = true;
    }
    const flagsBox = el('div', { class: 'term-flags' });
    const autoCb = el('input', { type: 'checkbox' });
    const autoWrap = el('label', { class: 'term-auto' }, autoCb, el('span', { text: ' 자동 승인 (위험) — 에이전트가 확인 없이 파일 수정·명령 실행. 공유 폴더에선 특히 주의.' }));
    ...
    const back = overlay(team ? ('새 세션 · ' + team.label) : newTitle, field('이름', labelI), field('작업자', authorI), locFields, field('하네스', harnessSel), field('공개 범위', visSel), flagsBox, autoWrap, el('div', { class: 'ov-actions' }, el('button', { class: 'btn btn-primary', text: '생성하기', onclick: async (ev) => {
...
function openTermEdit(s, cfg, view, team) {
    ...
    const visSel = el('select', { class: 'term-input' }, el('option', { value: 'public', selected: s.visibility !== 'private' ? '' : null }, '공개 — 모든 멤버가 열람 가능한 세션 (팀 내부 세션은 팀원만 열람가능)'), el('option', { value: 'private', selected: s.visibility === 'private' ? '' : null }, '비공개 — 나에게만 보이고 나만 열 수 있는 세션'));
    // ── 생성 시 고정(비활성 표시) ──
    ...
    const back = overlay('세션 수정', field('이름', labelI), field('공개 범위', visSel), lockNote, field('작업자', ro(author)), field('작업 폴더', ro(s.dir || '(기본)')), field('하네스', ro(harness.label || s.harness || '?')), flagFields, field('자동 승인', ro(autoShown)), el('div', { class: 'ov-actions' }, el('button', { class: 'btn btn-primary', text: '저장', onclick: async (ev) => {
            ev.currentTarget.disabled = true;
            try {
                await api('/api/ui/terminal/sessions/' + encodeURIComponent(s.id), { method: 'POST', body: JSON.stringify({ label: labelI.value, visibility: visSel.value }) });
                back.remove();
                toast('수정됨');
                renderTerminal(view, team ? team.id : null);
            }
...
function openTeamCreateForm(cfg, view) {
    ...
    const back = overlay('새 팀', field('팀 이름', labelI), field('소유자', el('input', { class: 'term-input', type: 'text', value: myName(cfg), disabled: '' })), field('팀원 (선택한 구성원만 이 팀 세션에 접근)', listBox), el('div', { class: 'ov-actions' }, el('button', { class: 'btn btn-primary', text: '팀 만들기', onclick: async (ev) => {
            if (!labelI.value.trim()) {
                toast('팀 이름을 입력하세요', true);
                return;
            }
            ev.currentTarget.disabled = true;
            ...
            try {
                ...
                toast('팀 생성됨');
                if (out && out.team)
                    location.hash = '#/terminal?team=' + encodeURIComponent(out.team.id);
                else
                    renderTerminal(view, null);
            }
            catch (e) {
                ev.currentTarget.disabled = false;
                toast('팀 생성 실패 — ' + e.message, true);
            }
        } })));
}
...
function openTeamManageForm(team, cfg, view) {
    ...
    const back = overlay('팀원 관리 · ' + team.label, field('팀 이름', labelI), field('소유자', el('input', { class: 'term-input', type: 'text', value: memberName(cfg, team.owner), disabled: '' })), field('팀원 (선택한 구성원만 이 팀 세션에 접근)', listBox), el('div', { class: 'ov-actions' }, el('button', { class: 'btn btn-primary', text: '저장', onclick: async (ev) => {
            if (!labelI.value.trim()) {
                toast('팀 이름을 입력하세요', true);
                return;
            }
...
export { renderTerminal, teamRow, teardownTerminal, };
