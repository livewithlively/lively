...
function openTermCreateForm(cfg, view, onCreated) {
    ...
    const roWrap = el('label', { class: 'term-auto' }, roCb, el('span', { text: ' 읽기전용 — 라이블리에서 읽기만 하고 지식·프로젝트·작업 등 쓰기는 안 합니다(기밀 작업용).' }));
    ...
    el('div', { 'data-tour': 'folder' }, field('어디서 실행할까요', el('div', { class: 'term-loc' }, rootSeg, el('div', { class: 'term-loc-folder' }, pickerBox)))), el('div', { class: 'term-checks', 'data-tour': 'options' }, autoWrap, roWrap), presetToggle, presetBody, el('div', { 'data-tour': 'invite' }, field('초대 (비우면 나만 보는 비공개 세션)', inviteBox.box)), el('div', { class: 'ov-actions' }, el('button', { class: 'btn btn-primary', 'data-tour': 'create', text: '생성하기', onclick: async (ev) => {
