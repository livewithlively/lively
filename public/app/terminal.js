...
async function renderTerminal(view) {
    ...
    async function bulkDelete(btn) {
        const items = [...sel.ids].map((id) => sessions.find((s) => s.id === id)).filter(Boolean);
        if (!items.length)
            return;
    ...
    if (s.owned) {
        if (!s.node) { // #1015 공개/비공개 빠른 토글(노드 세션은 공개 모델 미지원 — 소유자·초대자만 유지)
            acts.append(el('button', { class: 'tsess-icon', title: s.private ? '지금 비공개 — 눌러서 공개로(모든 멤버가 목록에서 봄)' : '지금 공개 — 눌러서 비공개로(나·초대한 멤버만 봄)', text: s.private ? '🔒 비공개' : '🌐 공개', onclick: async (ev) => {
                    ...
                    try {
                        await api('/api/ui/terminal/sessions/' + encodeURIComponent(s.id), { method: 'POST', body: JSON.stringify({ private: !s.private }) });
                        toast(s.private ? '공개로 전환됨' : '비공개로 전환됨');
                        reRender();
                    }
                    catch (e) {
...
function openTermCreateForm(cfg, view, onCreated) {
    ...
    const modeSel = el('select', { class: 'term-input' }, el('option', { value: 'normal' }, '일반 — 라이블리 읽기·쓰기'), el('option', { value: 'readonly' }, '읽기전용 — 읽되 쓰지 않음(기밀 작업)'), el('option', { value: 'incognito' }, '인코그니토 — 라이블리 전혀 안 씀(클린룸)'));
    const modeWrap = el('label', { class: 'term-auto' }, el('span', { text: ' 라이블리 모드 ' }), modeSel);
    // #1015 공개 범위 — 기본 공개(모든 멤버가 목록에서 봄). 끄면 비공개(나·초대한 멤버만). 입장·조작은 어느 경우든 소유자·초대자만.
    const pubCb = el('input', { type: 'checkbox' });
    pubCb.checked = true;
    const pubWrap = el('label', { class: 'term-auto' }, pubCb, el('span', { text: ' 공개 — 모든 멤버가 세션 목록에서 이 세션을 봅니다(무슨 작업인지). 끄면 비공개(나·초대한 멤버만). 입장·조작은 어느 경우든 소유자·초대자만.' }));
    ...
    el('div', { 'data-tour': 'folder' }, field('어디서 실행할까요', el('div', { class: 'term-loc' }, rootSeg, el('div', { class: 'term-loc-folder' }, pickerBox)))), el('div', { class: 'term-checks', 'data-tour': 'options' }, autoWrap, modeWrap, pubWrap), presetToggle, presetBody, el('div', { 'data-tour': 'invite' }, field('입장 허용 (초대한 멤버는 이 세션에 들어와 조작·대화 열람 가능 · 공개여도 입장은 이들만)', inviteBox.box)), el('div', { class: 'ov-actions' }, el('button', { class: 'btn btn-primary', 'data-tour': 'create', text: '생성하기', onclick: async (ev) => {
...
function openTermEdit(s, cfg, view) {
    ...
    const back = overlay('세션 수정', field('이름', labelI), field('공개 범위', el('label', { class: 'term-check-row' }, publicChk, el('span', { class: 'term-check-note', text: pubNote }))), field('입장 허용 (초대한 멤버는 이 세션에 들어와 조작·대화 열람 가능 · 나는 항상 가능)', inviteBox.box), lockNote, field('작업자', ro(author)), field('작업 폴더', ro(s.dir || '(기본)')), field('하네스', ro(harness.label || s.harness || '?')), flagFields, field('자동 승인', ro(autoShown)), el('div', { class: 'ov-actions' }, el('button', { class: 'btn btn-primary', text: '저장', onclick: async (ev) => {
            ev.currentTarget.disabled = true;
            try {
                await api('/api/ui/terminal/sessions/' + encodeURIComponent(s.id), { method: 'POST', body: JSON.stringify({ label: labelI.value, invites: inviteBox.selected(), private: isNode ? undefined : !publicChk.checked, node: (s.node && s.node.id) || undefined }) });
                back.remove();
                toast('수정됨');
                renderTerminal(view);
            }
