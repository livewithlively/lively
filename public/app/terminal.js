...
function tsessOwnFilter() { try {
    ...
    return ['all', 'mine', 'invited', 'project'].includes(v) ? v : 'all';
}
catch {
    return 'all';
} }
...
function tsessIsProject(s) { return !!(Number(s.projectId) || 0); }
...
function buildSessProjFilter(projName, projIds, current, onPick) {
    ...
    const btn = el('button', { type: 'button', class: 'tsess-projfilter-btn' + (current !== 0 ? ' active' : ''), title: '프로젝트로 필터' }, el('span', { text: current === 0 ? '프로젝트' : cur.label }), el('span', { class: 'tsess-projfilter-chev', text: '▾' }));
    const dd = el('div', { class: 'tsess-projfilter-dd', hidden: true });
    const search = el('input', { type: 'search', class: 'tsess-projfilter-search', placeholder: '프로젝트 검색…' });
    const listBox = el('div', { class: 'tsess-projfilter-list' });
    const renderList = (q) => {
        const ql = (q || '').trim().toLowerCase();
        const matches = opts.filter((o) => !ql || o.label.toLowerCase().includes(ql));
        listBox.replaceChildren(...(matches.length ? matches : [{ v: current, label: '(일치 없음)' }]).map((o) => el('button', { class: 'tsess-projfilter-opt' + (o.v === current ? ' active' : ''), type: 'button',
            onmousedown: (e) => { e.preventDefault(); close(); if (o.v !== current)
                onPick(o.v); } }, o.label)));
    };
    let docHandler = null;
...
async function renderTerminal(view) {
    ...
    function repaintBulk() {
        ...
        const allBtn = scope.length
            ? el('button', { class: 'dash-bulkbar-btn', type: 'button', text: allOn ? '전체 해제' : '보이는 것 전체 (' + scope.length + ')',
                ...
                onclick: () => { if (allOn)
                    scope.forEach((s) => sel.ids.delete(s.id));
                else
                    scope.forEach((s) => sel.ids.add(s.id)); draw(); } })
            : null;
        ...
        endBtn.disabled = n === 0;
        bulkBar.replaceChildren(el('span', { class: 'dash-bulkbar-n', text: n ? (n + '개 선택') : '세션을 골라 그리드로 열거나 종료하세요' }), ...[allBtn, deadBtn].filter(Boolean), openBtn, endBtn, el('button', { class: 'dash-bulkbar-btn', type: 'button', title: sel.mode ? '선택 모드 종료' : '선택 해제', text: '완료',
    ...
    function draw() {
        ...
        const counts = { waiting: 0, busy: 0, idle: 0, exited: 0, offline: 0 };
        for (const s of sessions)
            counts[s._st]++;
        ...
        // 상태 칩 — 5단계. 해당 세션이 0개인 상태는 칩을 숨긴다(빈 칩이 늘어나 필터 줄이 시끄러워지므로).
        const chips = el('div', { class: 'tsess-chips' }, allChip, ...[['waiting', '확인 필요'], ['busy', '작업 중'], ['idle', '대기 중'], ['exited', '종료됨'], ['offline', '연결 끊김']]
            .filter(([k]) => counts[k] > 0 || statusOn.has(k))
            .map(([k, lbl]) => stChip(k, lbl, k)));
        ...
        if (projIds.length)
            right.append(buildSessProjFilter(projName, projIds, projF, (v) => { projF = v; draw(); }));
        ...
        if (ownF === 'invited' && !invitedCount)
            ownF = 'all'; // 해당 버킷이 비면 필터 해제(줄이 숨겨져 갇히지 않게)
        if (ownF === 'project' && !projCount)
            ownF = 'all';
        if (ownF === 'mine' && !othersCount)
            ownF = 'all';
        ...
        if (othersCount) {
            ...
            rows.push(el('div', { class: 'tsess-filter-row' }, el('span', { class: 'tsess-filter-lbl', text: '소유' }), ownChip('mine', '내 세션', mineCount), ...(invitedCount ? [ownChip('invited', '초대받은 세션', invitedCount)] : []), ...(projCount ? [ownChip('project', '남의 프로젝트 세션', projCount)] : [])));
        }
    ...
    async function bulkEnd(btn) {
        ...
        if (!confirm(TSESS_END_CONFIRM(items.length, items.length + '개 세션을 종료할까요?')
            ...
            + (skipped ? '\n남의 세션 ' + skipped + '개는 제외됩니다(소유자만 종료 가능).' : '')))
            return;
        btn.disabled = true;
...
function tsessCard(s, ctx) {
    ...
    if (sel && s.owned) {
        ...
        cb.addEventListener('change', () => {
            ...
            if (sel.listEl)
                sel.listEl.classList.toggle('has-sel', sel.ids.size > 0);
            if (sel.onToggle)
                sel.onToggle();
        });
...
function openTermCreateForm(cfg, view, onCreated) {
    ...
    el('div', { 'data-tour': 'folder' }, field('어디서 실행할까요', el('div', { class: 'term-loc' }, rootSeg, el('div', { class: 'term-loc-folder' }, pickerBox)))), modeField, // 라이블리 모드 — '어디서 실행할까요'와 같은 카드 언어로, 그 아래 독립 필드(#1007+ 디자인 통일)
...
function openNodeManager(view) {
    ...
    async function load() {
        ...
        const howBody = el('div', { style: 'display:none' }, el('div', { class: 'caption', text: '내 PC·서버(macOS·Linux)를 노드로 붙이면 그 머신의 터미널 세션을 여기서 만들고 관리하며, 위탁 워커로도 씁니다.' }), codeEl, el('div', {}, el('button', { class: 'btn btn-ghost btn-sm', text: '명령 복사', onclick: () => { try {
        ...
        for (const n of nodes) {
            ...
            list.append(el('div', { class: 'card', style: 'display:flex;align-items:center;gap:10px;padding:10px 12px' }, el('span', { text: '🖥' }), el('div', { style: 'min-width:0' }, el('div', { style: 'font-weight:600' }, document.createTextNode((n.name || n.id) + ' '), el('span', { class: 'caption', text: n.kind === 'worker' ? '워커' : '멤버' })), el('div', { class: 'caption', text: n.id })), badge, acts));
        }
...
termCreatePrefs, saveTermCreatePrefs, termAutoApprovePref, // 체크박스 없는 원클릭 실행(대시보드 '만들고 Claude로 실행' 등)이 쓰는 자동 승인 기본값(#782)
 };
