...
function tsessView() { try {
    const v = localStorage.getItem(TSESS_VIEW_KEY) || 'all';
    return TSESS_VIEWS.some((x) => x.key === v) ? v : 'all';
}
catch {
    return 'all';
} }
function saveTsessView(v) { try {
    localStorage.setItem(TSESS_VIEW_KEY, v);
}
catch { /* noop */ } }
const TSESS_MINE_KEY = 'lively_term_mine_only_v2';
function tsessMineOnly() { try {
    return localStorage.getItem(TSESS_MINE_KEY) === '1';
}
catch {
    return false;
} }
function saveTsessMineOnly(v) { try {
    localStorage.setItem(TSESS_MINE_KEY, v ? '1' : '0');
}
catch { /* noop */ } }
...
async function renderTerminal(view) {
    ...
    function repaintBulk() {
        ...
        endBtn.disabled = n === 0;
        bulkBar.replaceChildren(el('span', { class: 'dash-bulkbar-n', text: n ? (n + '개 선택') : '세션을 골라 그리드로 열거나 종료하세요' }), ...pickBtns, openBtn, endBtn, el('button', { class: 'dash-bulkbar-btn', type: 'button', title: sel.mode ? '선택 모드 종료' : '선택 해제', text: '완료',
    ...
    function draw() {
        ...
        if (mineOnly && !othersCount)
            mineOnly = false; // 남의 세션이 없으면 토글이 사라지므로 갇히지 않게 해제
        if (othersCount) {
    ...
    async function bulkEnd(btn) {
        ...
        if (!await tsessConfirmEnd(items.length + '개 세션을 종료할까요?', lines))
            return;
        btn.disabled = true;
    ...
    if (s.restorable) {
        ...
        rb.onclick = async () => {
            ...
            try {
                ...
                toast(restoreVerb + ' 완료 — 새 터미널에서 대화를 이어받아요(정확한 대화를 못 찾으면 이어보기 목록에서 고르세요).');
                reRender();
            }
            catch (e) {
                toast(restoreVerb + ' 실패 — ' + (e && e.message || e), true);
                rb.disabled = false;
