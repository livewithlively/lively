    ...
    function repaintBulk() {
        ...
        const allBtn = el('button', { class: 'btn btn-ghost btn-sm', text: allOn ? '전체 해제' : '전체 선택',
            onclick: () => { if (allOn)
                ...
                ownedSessions.forEach((s) => sel.ids.add(s.id)); draw(); } });
...
function tsessCard(s, ctx) {
    ...
    // 액션 — 열기(항상) + 소유자면 수정·삭제.
    const acts = el('div', { class: 'tsess-acts' }, el('button', { class: 'tsess-open', text: '열기', onclick: () => window.open('/ui/terminal.html?session=' + encodeURIComponent(s.id) + '&label=' + encodeURIComponent(s.label || ''), '_blank') }));
    if (s.owned) {
