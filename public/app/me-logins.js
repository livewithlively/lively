...
const CH_TYPE_LABEL = { public: '공개', private: '비공개', group_dm: '그룹 DM', dm: 'DM', unknown: '목록 밖' };
// 규칙 설명 — 기술 용어 없이. '무엇이 기본인지 · 왜 그런지 · 새로 생기는 대화는 어떻게 되는지' 세 가지를 답한다.
function channelRuleExplainer() {
    const rule = (icon, title, body) => el('div', { style: 'display:flex; gap:9px; align-items:flex-start; padding:7px 0' }, el('span', { style: 'font-size:15px; line-height:1.35', text: icon }), el('span', { style: 'display:flex; flex-direction:column; gap:1px' }, el('span', { class: 'mini-title', text: title }), el('span', { class: 'mini-meta' }, ...uiText(body))));
    return el('div', { style: 'margin:0 0 12px' }, el('p', { class: 'admin-hint', style: 'margin:0 0 4px' }, ...uiText('슬랙은 “이 채널만 빼고” 같은 권한을 주지 않습니다 — 한 번 연결하면 내가 볼 수 있는 대화가 통째로 열려요. '
        + '그래서 라이블리가 대신 걸러 줍니다. 아무것도 설정하지 않아도 아래 규칙이 자동으로 적용됩니다.')), rule('🌐', '공개 채널 — 처음부터 열려 있어요', 'AI 가 읽고 보낼 수 있습니다. 워크스페이스 사람이면 누구나 볼 수 있는 대화라 기본을 열어 둡니다. 보여주고 싶지 않으면 체크를 끄세요.'), rule('🔒', '비공개 채널 · 그룹 DM · 1:1 DM — 처음부터 닫혀 있어요', 'AI 가 읽지도, 보내지도 못합니다. 필요한 대화만 직접 체크를 켜서 여세요.'), el('p', { class: 'admin-hint', style: 'margin:6px 0 0' }, ...uiText('나중에 새로 만들어지거나 새로 초대된 대화도 같은 규칙을 따릅니다 — 비공개 대화는 내가 직접 켜기 전까지 계속 닫혀 있어요. '
...
function slackChannelPolicyCard() {
    ...
    const notice = el('div');
    let all = [];
    const label = (c) => (c.type === 'dm' || c.type === 'group_dm' ? c.name : '#' + String(c.name || '').replace(/^#/, ''));
    const isDefault = (c) => c.allow_read === c.default_read && c.allow_write === c.default_write;
    let connected = true;
    const paint = () => {
        // '목록이 비었다'를 '전부 허용'이라고 말하면 거짓 안심이 된다 — 연결이 없어서 못 읽은 것과 구분한다.
        if (!connected) {
            summary.replaceChildren(...uiText('슬랙 연결을 확인하지 못했습니다 — 위에서 [연결]을 마치면 내가 속한 대화가 여기에 뜹니다.'));
            return;
        }
        if (!all.length) {
            summary.replaceChildren(...uiText('내가 속한 대화를 찾지 못했습니다. 슬랙에서 채널에 참여한 뒤 [새로고침]하세요.'));
            return;
        }
        ...
        summary.replaceChildren(...uiText(`대화 ${all.length}개 중 AI 가 읽을 수 있는 건 ${open}개입니다`
            + (closed ? ` — 나머지 ${closed}개는 닫혀 있어요.` : '.')));
    };
    const rowOf = (c) => {
        const mk = (checked) => { const b = el('input', { type: 'checkbox' }); b.checked = checked; return b; };
        ...
        const paintRow = () => {
            badges.replaceChildren(el('span', { class: 'pill', text: CH_TYPE_LABEL[c.type] || c.type }), 
        ...
        const save = async () => {
            ...
            try {
                ...
                c.allow_read = readChk.checked;
                c.allow_write = writeChk.checked;
                paintRow();
                paint();
            }
            catch (e) {
                readChk.checked = c.allow_read;
                writeChk.checked = c.allow_write; // 저장 실패 = 화면도 되돌린다(거짓 안심 방지)
        ...
        const box = (chk, text) => el('label', { style: 'display:inline-flex; align-items:center; gap:5px; font-size:12.5px; color:var(--ink-sub); cursor:pointer' }, chk, el('span', { text }));
        paintRow();
        return el('div', { class: 'svc-item' }, el('span', { class: 'svc-item-main' }, el('span', { class: 'mini-title' }, el('span', { text: label(c) }), badges), c.from_policy ? el('span', { class: 'mini-meta' }, ...uiText('지금 내 대화 목록엔 없습니다(나갔거나 권한 밖) — 설정만 남아 있어요.')) : null), el('span', { class: 'svc-item-actions' }, resetWrap, box(readChk, '열람'), box(writeChk, '발송')));
    };
    const draw = () => {
    ...
    const load = async () => {
        let d;
        try {
            d = await api('/api/ui/me/slack/channels');
        }
        catch (e) {
            summary.replaceChildren();
            listBox.replaceChildren(errorNote(e, '대화 목록을 불러오지 못했습니다'));
            return;
        }
        all = d.channels || [];
        ...
        if (d.error)
            notes.push(el('p', { class: 'admin-hint', style: 'margin:0 0 8px' }, ...uiText('슬랙에서 대화 목록을 받지 못했습니다(' + d.error + '). 아래는 이미 저장해 둔 설정입니다.')));
        if (d.warning)
            notes.push(el('p', { class: 'admin-hint', style: 'margin:0 0 8px' }, ...uiText(d.warning)));
