...
const SOURCE_KIND_LABEL = { transcript: '전사록', minutes: '회의록', email: '이메일', slack: '슬랙', discord: '디스코드', notion_doc: '노션', clickup_doc: '클릭업', drive_file: '구글드라이브', local_file: '내 컴퓨터', other: '기타' };
...
const knCatRowsCache = new Map(); // 전량(본문 포함)
const knCatIndexCache = new Map(); // 경량(본문 제외, #1091)
...
function knFetchCategoryIndex(catId) { return knCachedCatRows(knCatIndexCache, String(catId), true); }
function knFetchCategoryRows(catId) { return knCachedCatRows(knCatRowsCache, String(catId), false); }
function knCachedCatRows(cache, key, light) {
    let p = cache.get(key);
    if (!p) {
...
async function knLoadCategoryRows(key, light) {
    const out = [];
...
function openKnPropsSettings(foot, gear, k, hidden, onChanged) {
    ...
    const setBusy = (b) => { docBtn.disabled = b; allBtn.disabled = b; }; // ⚠ 배럴의 busy(높이 예약)와 다른 것 — 이름을 갈라 둔다
...
function openKnowledgeMoveTo(k, reload) {
    ...
    (async () => {
        ...
        if (catId != null) {
            try {
                folders = (await knFetchCategoryIndex(catId)).filter((r) => r.is_folder && r.name !== k.name);
            }
