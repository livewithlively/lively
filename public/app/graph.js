...
const adj = new Map();
let svg, gViewport, gHulls, gLinks, gNodes, gRegion, gNlab, stage;
let hoverName = null, soloCat = null, query = '';
let alpha = 0, raf = 0, running = false, dragNode = null;
main();
async function main() {
    let data;
    try {
...
function paint() {
    ...
    for (const c of CATS) {
        ...
        c.elName.setAttribute('y', (sy - 11 - f * 0.5).toFixed(1));
        c.elName.setAttribute('font-size', f.toFixed(1));
        c.elCount.setAttribute('x', sx.toFixed(1));
        c.elCount.setAttribute('y', (sy - 12).toFixed(1));
...
function centerOn(n, k) {
    if (k)
        view.k = clamp(k, K_MIN, K_MAX);
    view.tx = root.clientWidth / 2 - n.x * view.k;
    view.ty = root.clientHeight / 2 - n.y * view.k;
}
function bbox(pts) {
