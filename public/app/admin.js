...
import { api, applyReveal, el, errorNote, pageHead, profileAvatar, relTime, renderMarkdown, state, toast } from './core.js';
...
async function profilesEditor(detail) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, el('h3', { text: '중앙박스 계정(프로필)' }), el('p', { class: 'caption', text: '멤버별로 다른 Claude Code 계정을 쓰게 합니다. [프로필 만들기]로 그 멤버의 config·MCP·훅을 깔고, 그 멤버가 웹터미널에서 한 번 로그인(claude → /login)하면 그 멤버가 띄우는 중앙박스 세션이 자기 계정으로 뜹니다. 로그인 안 된 멤버는 공유 계정으로 폴백돼요(무회귀).' })), ...items);
}
