...
const ADMIN_SECTIONS = [
    ...
    { key: 'ingest-policy', label: '지식 검토 게이트', meaning: null, group: 'ai' },
    { key: 'review-queue', label: '검토 큐', meaning: null, group: 'ai' },
...
function sectionHidden(key, data) {
    ...
    if (RUNTIME_ONLY.includes(key) && !data.canRuntime)
        return true;
    if (MEMORY_SCOPE_SECTIONS.includes(key) && !(data.canEdit || hasScope('memory')))
        return true; // #783 지식 쓰기 권한자면 검토 가능
    return false;
}
...
function meaningCard(m0) {
    ...
    const trigger = el('button', { class: 'meaning-trigger', type: 'button', 'aria-haspopup': 'dialog' }, el('span', { class: 'meaning-trigger-icon', 'aria-hidden': 'true', text: 'ⓘ' }), el('span', { text: '이게 뭐예요?' }));
    trigger.addEventListener('click', () => {
        overlay(m.label || '이게 뭐예요?', el('div', { class: 'meaning meaning-' + m.tone + ' meaning-pop' }, el('div', { class: 'meaning-head' }, el('span', { class: 'meaning-dot', 'aria-hidden': 'true' }), el('span', { class: 'meaning-title', text: '구성원에게 미치는 효과' }), tag ? el('span', { class: 'meaning-tag', text: tag }) : null), el('p', { class: 'meaning-what', text: m.what }), el('div', { class: 'meaning-grid' }, meaningRow('누가 보나', m.reach), meaningRow('언제 적용되나', m.when), meaningRow('어디에 쓰이나', m.where)), el('div', { class: 'meaning-ex' }, el('span', { class: 'meaning-ex-label', text: '예를 들면' }), el('span', { text: m.example }))));
    });
    return trigger;
}
...
async function renderAdmin(view, sub) {
    ...
    if (sel && SECTION_REMAP[sel])
        sel = SECTION_REMAP[sel]; // 흡수된 구 섹션 URL → 새 섹션
...
function renderAdminDetail(detail, sel, data) {
    ...
    if (sel === 'org-defaults' || SCAFFOLD_SECTIONS.includes(sel))
        return sectionEditor(detail, sel, data);
    if (sel === 'members')
        return membersEditor(detail, data);
    if (sel === 'teams')
        return teamsPanel(detail, data);
    if (sel === 'member-add')
        return memberAddPanel(detail, data);
    if (sel === 'tokens')
        return tokensPanel(detail, data);
    if (sel === 'profiles')
        return profilesEditor(detail);
    if (sel === 'profile')
...
async function managedSessionsPanel(detail, data) {
    ...
    try {
        ...
        detail.replaceChildren(el('div', { class: 'card' }, errorNote(e, '상시 세션을 불러오지 못했습니다')));
        return;
    }
    try {
    ...
    const rows = el('div', { class: 'wikicat-rows' });
    if (!sessions.length)
        rows.append(el('div', { class: 'wikicat-empty', text: '아직 상시 세션이 없습니다. ‘+ 상시 세션 추가’로 등록하면 keep-alive 가 항상 띄워둡니다.' }));
    for (const m of sessions) {
        ...
        const acts = el('div', { class: 'wikicat-row-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: '띄우기/재생성', onclick: () => managedEnsure(m.id, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: m.enabled ? '끄기' : '켜기', onclick: () => managedToggle(m, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: '수정', onclick: () => openManagedSessionForm(m, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: '삭제', onclick: () => managedDelete(m.id, reload) }));
        rows.append(el('div', { class: 'wikicat-row' }, main, acts));
    }
    const head = el('div', { class: 'wikicat-grouphead' }, el('span', { class: 'wikicat-grouptitle', text: '상시 세션' }), el('span', { class: 'wikicat-groupcount', text: String(sessions.length) }), el('button', { class: 'btn btn-ghost btn-sm wikicat-add', text: '+ 상시 세션 추가', onclick: () => openManagedSessionForm(null, reload) }));
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '상시 세션 (에이전트)' })), el('p', { class: 'admin-hint', text: '항상 떠 있는 에이전트 세션입니다. 격리 워크스페이스(공유폴더)에서 돌고, keep-alive 가 죽으면 재생성합니다. 크론(미매핑 분류 등)이 이 세션에 작업을 쏩니다 — 팀플랜 과금. account = 어떤 라이블리 계정(클로드 로그인)으로 띄울지.' }), el('div', { class: 'wikicat' }, el('div', { class: 'wikicat-group' }, head, rows)));
    detail.replaceChildren(card);
}
...
async function reposPanel(detail, data) {
    ...
    if (!repos.length) {
        ...
        for (const r of repos) {
            ...
            const main = el('div', { class: 'wikicat-row-main' }, el('span', { class: 'wikicat-name', text: r.name }), el('span', { class: 'wikicat-key mono', text: (t.code_units || 0) + ' code · ' + (t.domains || 0) + ' dom' }), deprecated ? el('span', { class: 'dm-tag', text: '폐기됨' }) : null, meta);
            const acts = canEdit ? el('div', { class: 'wikicat-row-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: '⟳ 최신화', title: '이 레포의 공유 클론을 upstream 으로 최신화(fetch + fast-forward). 게이트웨이가 당겨오므로 모든 멤버가 최신 코드를 읽게 됩니다.', onclick: (e) => repoRefreshShared(r.name, e) }), el('button', { class: 'btn btn-ghost btn-sm', text: '수정', onclick: () => openRepoForm(r, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: deprecated ? '복귀' : '폐기', onclick: () => repoSetDeprecated(r.name, deprecated, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: '삭제', onclick: () => repoHardDelete(r.name, reload) })) : null;
            rows.append(el('div', { class: 'wikicat-row' }, main, acts));
        }
    }
...
async function wikiCategoriesPanel(detail, data) {
        ...
        if (!items.length) {
            ...
            for (const c of items) {
                ...
                const shouldLine = should
                    ...
                    : el('span', { class: 'wikicat-should wikicat-should-empty' }, el('span', { class: 'wikicat-should-label', text: '정의·범위·규칙' }), canEdit ? '미설정 — 수정에서 추가할 수 있어요' : '미설정');
    ...
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '분류 체계 관리' }), canEdit ? null : el('span', { class: 'admin-sub' }, el('span', { class: 'pill', text: '읽기 전용' }), ' 편집은 context 권한 필요')), el('p', { class: 'admin-hint', text: '지식(위키)의 분류축입니다. 사업·제품·시스템 카테고리를 한 화면에서 추가·수정·삭제하며, 변경은 지식·프로젝트 탭 좌측 카테고리에 그대로 반영됩니다. (제품 카테고리=도메인)' }), list);
    detail.replaceChildren(card);
}
// WIKI 카테고리 삭제(확인 후) — categoryCard 의 삭제 로직과 동일 엔드포인트. reload 로 패널 갱신.
async function deleteWikiCategory(c, reload) {
    if (!confirm('‘' + (c.name || c.key) + '’ 카테고리를 삭제할까요? (연결된 매핑·엣지도 함께 정리됩니다)'))
        return;
    try {
    ...
    function render(editing, prefill) {
        ...
        const resetToDefault = () => {
            ...
            if (editing) {
                ...
                render(true, defaultBody); // 보기 모드면 기본값 채운 수정 모드로 진입
        };
        // 미리보기 — 가이드 섹션은 '이 편집 부분만'(${area}/${rules} 채워진 인덱스), 그 외는 전체 멤버 컨텍스트.
        const headBtns = el('div', { class: 'card-head-actions' }, el('button', { class: 'btn btn-ghost btn-sm', text: '미리보기',
            onclick: isScaffold ? (() => showGuidePreview(ta.value)) : showMemberPreview }), (canEdit && isScaffold)
            ...
            : null);
        const body = [
            el('div', { class: 'card-head' }, el('div', { class: 'section-title' }, el('h2', { text: title }), meaningCard(meaning)), headBtns),
        ];
        // 골격 섹션 — 비개발자용 경고 배너(무엇인지 한 줄 + 공통 위험 안내). 읽기/수정 모두 항상 표시.
        if (isScaffold) {
            body.push(el('div', { class: 'admin-warn' }, el('div', { text: SCAFFOLD_WARN[key] || '' }), el('div', { style: 'margin-top:5px;font-weight:500', text: SCAFFOLD_WARN_COMMON })));
        }
        ...
        if (editing) {
            ...
            saveBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    sec.body_md = r.section.body_md;
                    sec.version = r.section.version; // [보기] 전환 시 최신본 노출
...
async function showMemberPreview() {
    try {
        ...
        overlay('구성원의 AI가 매 세션 실제로 읽는 내용', el('p', { class: 'admin-hint', text: '아래가 모든 구성원의 대화 첫머리에 주입되는 정적 컨텍스트입니다(라이브 현황은 별도로 매 세션 자동 추가).' }), el('div', { class: 'md-rendered admin-md-box', style: 'max-height:70vh; overflow:auto' }, renderMarkdown(r.context || '(비어 있음)')));
    }
    catch (e) {
        toast(e.message, true);
    }
...
async function showGuidePreview(bodyMd) {
    try {
        ...
        overlay('이 가이드가 실제 주입되는 모습 (지식·주제·핀 자동 채움)', el('p', { class: 'admin-hint', text: '아래는 이 편집 내용의 ${rules}(항상-주입 지식)·${categories}(카테고리 지도)·${wiki}(WIKI 인덱스 핀) 가 실제 데이터로 채워져 매 대화에 주입되는 부분입니다(회사 소개·규칙 섹션 등 다른 부분은 제외).' }), el('div', { class: 'md-rendered admin-md-box', style: 'max-height:70vh; overflow:auto' }, renderMarkdown(r.context || '(비어 있음)')));
    }
    catch (e) {
        toast(e.message, true);
    }
...
async function profilesEditor(detail) {
    ...
    const items = profiles.length ? profiles.map((p) => {
        ...
        if (hasCtrl) {
            ...
            kids.push(el('label', { class: 'caption', style: 'display:block;margin:3px 0 7px;cursor:pointer' }, cpChk, el('span', { text: '관리 권한(admin/runtime) 포함 — 이 계정으로 뜬 세션이 관리탭 기능(구성원·토큰·훅·DB소스)을 MCP로 직접 다룹니다. 변경은 감사에 AI로 남습니다(#549).' })));
        }
    ...
    detail.replaceChildren(el('div', { class: 'card' }, el('h3', { text: '중앙박스 계정 격리' }), el('p', { class: 'caption', text: '각 구성원은 자기 OS 계정(box_<slug>, 홈 700)으로 완전 격리됩니다 — 구성원 간 Claude 자격증명(.credentials.json) 상호열람 차단(외부상주·금융권). 격리 인프라(deploy/linux/install-isolation.sh)가 깔린 박스에서 구성원이 웹터미널을 처음 열면 격리 유저가 자동 생성되고(별도 버튼 불요), 그 세션부터 본인 Claude 로그인으로 뜹니다. 미프로비저닝 멤버는 공유 계정으로 폴백(무회귀). 첫 세션 지연 없이 미리 깔려면 [지금 미리 만들기], 끄려면 게이트웨이 env LIVELY_MEMBER_ISOLATION=off.' })), ...items);
}
...
function membersEditor(detail, data) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('구성원 관리', meaning), members.length ? searchBar : null, listCol));
}
...
function memberRead(root, m, data, detail, opts = {}) {
    ...
    const kids = [
        el('div', { class: 'member-read-head' }, el('h3', { text: m.display_name || m.id }), canEdit ? (m.hasToken ? el('span', { class: 'pill pill-ok', text: '설치됨' }) : el('span', { class: 'pill', text: '미설치' })) : null),
    ];
    if (canEdit) {
        ...
        kids.push(roRow('아이디', m.id), roRow('종류', m.kind || 'human'), roRow('대표 이메일', m.email), roRow('상태', (m.state || 'active') === 'active' ? '활성' : '비활성'), roRow('권한 (이 구성원 토큰의 scope)', scopeText), field('외부 계정 연결 (신원 매칭 키)', el('div', { class: 'admin-ro admin-ro-pre', text: idnText || '—' })), field('개인 레이어', el('div', { class: 'admin-ro admin-ro-pre', text: (m.body_md && m.body_md.trim()) || '—' })));
    }
    else {
    ...
    if (canEdit) {
        const acts = el('div', { class: 'admin-actions' }, el('button', { class: 'btn btn-primary', text: '수정',
            ...
            onclick: () => { if (opts.onEdit) {
                opts.onEdit();
                return;
            } state.admin.memberEditing = true; renderAdminDetail(detail, 'members', data); } }));
...
function memberForm(root, m, data, detail, isNew, opts = {}) {
    ...
    function addIdn(idn) {
        ...
        rm.addEventListener('click', () => { row.remove(); const i = idnRows.indexOf(rec); if (i >= 0)
            idnRows.splice(i, 1); });
        idnRows.push(rec);
        idnWrap.append(row);
    }
    ...
    root.replaceChildren(isNew ? el('span', { hidden: '' }, idIn) : field('아이디 (내부 식별자 · 변경 불가)', idIn), field('표시 이름', nameIn), field('종류', kindSel), field('대표 이메일', emailIn), field('상태', stateSel), field('권한 (이 구성원 토큰의 scope)', scopeWrap), el('div', { class: 'field' }, el('label', { class: 'field-label', text: '외부 계정 연결 (신원 매칭 키)' }), idnWrap, el('button', { class: 'btn-text', text: '+ 계정 추가', onclick: () => addIdn(null) })), field('개인 레이어', bodyTa), actions);
}
...
async function teamsPanel(detail, data) {
    ...
    if (canEdit)
        listCol.append(el('button', { class: 'btn btn-ghost btn-sm', text: '+ 새 팀',
    ...
    for (const t of teams) {
        listCol.append(el('div', { class: 'mini-row' + (String(t.id) === String(sel) ? ' sel' : ''),
            onclick: () => { state.admin.teamSel = t.id; state.admin.teamEditing = false; teamsPanel(detail, data); } }, el('div', { class: 'mini-title', text: (t.name || t.key) }), el('div', { class: 'mini-meta', text: (t.member_count || 0) + '명 · 카테고리 ' + (t.category_count || 0) })));
    }
    if (!teams.length)
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('팀 관리', { key: 'team' }), el('p', { class: 'admin-hint', text: '구성원을 팀으로 묶고, 팀이 맡는 카테고리(도메인)를 정합니다. 카테고리 오너 배정은 [분류 체계 관리]에서, 여기선 팀과 팀원을 관리합니다. 팀 소유 카테고리는 팀원의 프로젝트·위키 탭과 AI 세션에 먼저 노출됩니다(오너십=우선순위, 접근제한 아님).' }), el('div', { class: 'admin-two admin-two-cols' }, listCol, right)));
}
// 팀 보기(수정 전 읽기 요약).
    ...
    const tokenRow = (t, isActive) => {
        ...
        const right = isActive
            ? el('button', { class: 'btn btn-ghost btn-sm', text: '접속 해제', onclick: async (e) => {
                    if (!confirm(`'${t.label || t.user_id}' 님의 접속을 해제할까요? 이 열쇠는 즉시 무효화됩니다(되돌릴 수 없음).`))
                        return;
...
function injectionMap(detail, data) {
    ...
    function momentToggle(hookKey) {
        ...
        chk.addEventListener('change', async () => {
            try {
                await saveRuntime({ hooks: { ...hooks, [hookKey]: chk.checked } }, chk.checked ? '주입 켜짐' : '주입 꺼짐');
    ...
    function writebackEditor() {
        ...
        const ta = el('textarea', { rows: '6', placeholder: def });
        ta.value = cur || def;
        ta.disabled = !canEdit;
        const btn = el('button', { class: 'btn btn-primary btn-sm', text: '저장' });
        if (!canEdit)
...
function hooksOverview(detail, data) {
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '훅 (Hooks)' })), el('p', { class: 'admin-hint', text: '훅은 구성원의 AI 세션 중 특정 시점(이벤트)에 게이트웨이가 자동으로 끼어들어 실행하는 코드·설정입니다. 세션이 시작될 때 조직 컨텍스트를 주입하거나, 지금 작업 중인 폴더를 라이블리 작업으로 인식하거나, 세션이 끝날 때 기록을 남기도록 너지하는 식으로 동작합니다 — 사람이 매번 챙기지 않아도 AI가 조직의 방식대로 일하게 만드는 자동 장치입니다.' }), el('p', { class: 'admin-hint', text: '구성원 머신은 매 세션 게이트웨이에서 훅을 내려받아 실행하므로(runner fetch), 여기서 바꾸면 재설치 없이 다음 세션부터 자동 반영됩니다. 주요 주입 시점: 세션 시작(SessionStart) · 프롬프트 제출(UserPromptSubmit) · 도구 사용 전후(Pre/PostToolUse) · 세션 종료(Stop). 아래 세 가지로 관리합니다.' }));
    const items = [
    ...
    for (const [key, title, desc] of items) {
        if (sectionHidden(key, data))
            continue; // 권한으로 숨은 하위는 안내에서도 제외(404 유도 방지).
        list.append(el('a', { class: 'hooks-ov-card', href: '#/system/' + key }, el('div', { class: 'hooks-ov-title', text: title }), el('div', { class: 'hooks-ov-desc', text: desc })));
    }
...
function runtimeEditor(detail, data) {
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            const hooks = {};
            for (const [k] of HOOK_OPTS)
                hooks[k] = chks[k].checked;
...
function storageEditor(detail, data) {
    ...
    async function loadAlert() {
        ...
        const minSel = el('select', { class: 'input' }, el('option', { value: 'warn', text: '경고부터 (85% 도달 시에도 알림)' }), el('option', { value: 'critical', text: '위험만 (95% 도달·DB 다운만 알림)' }));
    ...
    async function loadWorkspace() {
        ...
        let ws;
        try {
            ws = await api('/api/ui/org/workspace');
        }
        catch (e) {
        ...
        const rows = (ws.projects || []).map((p) => {
            ...
            function renderPlan(plan) {
                ...
                for (const r of plan.results || []) {
                    ...
                    parts.push(el('div', { class: 'ws-plan' }, el('p', { class: 'storage-calc', text: `회수 가능 ${fmtBytes(r.reclaimable_bytes)} — ${derived.map((d) => d.path + ' ' + fmtBytes(d.bytes)).join(' · ') || '없음'}` }), ...(unc.length ? [el('p', { class: 'storage-calc', text: `미분류(지우지 않음): ${unc.map((d) => d.path).join(', ')}` })] : []), ...(r.protected_kept?.length ? [el('p', { class: 'storage-calc', text: `보호(절대 안 지움): ${r.protected_kept.join(', ')}` })] : []), el('p', { class: 'storage-calc', text: r.worktree_removable ? '워크트리: 제거 가능(푸시 완료·변경 없음)' : `워크트리: 유지 — ${r.worktree_reason}` })));
                }
                ...
                detailBox.replaceChildren(...parts, el('div', { class: 'ws-actions' }, el('label', { class: 'storage-toggle' }, wtChk, el('span', { text: allRemovable ? ' 워크트리도 제거 (push 완료 — provision 으로 복구 가능)' : ' 워크트리 제거 불가 (위 사유 참조)' })), doBtn));
            }
            const badges = [];
            if (p.active_session)
                badges.push(el('span', { class: 'storage-lv storage-lv-warn', text: '작업 중' }));
            if (p.kind === 'archived')
                badges.push(el('span', { class: 'storage-lv storage-lv-ok', text: '완료 보관' }));
            return el('div', { class: 'storage-item' }, el('div', { class: 'storage-head' }, el('span', {}, el('strong', { text: p.name || String(p.project_id) }), el('span', { class: 'storage-calc', text: `  ${fmtBytes(p.bytes ?? 0)} · ${p.last_used ? relTime(p.last_used * 1000) : '—'}` })), el('span', { class: 'ws-badges' }, ...badges, analyzeBtn)), detailBox);
        });
        wsRegion.replaceChildren(el('p', { class: 'storage-calc', text: `${(ws.projects || []).length}개 프로젝트 · 합계 ${fmtBytes(ws.total_bytes)} — ${ws.root}` }), el('p', { class: 'admin-hint', text: '프로젝트별로 [분석]을 눌러 무엇이 회수 가능한지 먼저 확인하세요(아무것도 지우지 않습니다). 회수 대상은 다시 만들 수 있는 것뿐입니다 — node_modules·빌드 산출물 등. 소스·커밋·설정(.env)·데이터는 절대 지우지 않으며, 작업 중인 세션이 있거나 push 안 한 커밋이 있으면 거부합니다. 자동 삭제는 하지 않습니다.' }), ...(rows.length ? rows : [el('p', { class: 'admin-hint', text: '워크스페이스가 비어 있습니다.' })]));
    }
    load();
}
function embeddingsEditor(detail, data) {
    const canEdit = !!data.canEdit;
    const body = el('div');
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('임베딩 (벡터검색)', null), el('p', { class: 'admin-hint', text: '지식 의미검색·유사도·중복감지가 쓰는 벡터 임베딩을 켜고 끕니다. 기본은 꺼짐(정확 grep 검색으로 폴백). 켜면 OpenAI-compatible /v1/embeddings 엔드포인트(로컬 Ollama 사이드카 또는 외부 API)로 임베딩합니다. 설정은 게이트웨이 재시작 없이 즉시 반영됩니다.' }), body));
    body.append(el('p', { class: 'admin-hint', text: '불러오는 중…' }));
    let pollTimer = null;
...
function hooksPreviewPanel(detail, data) {
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '훅 주입 미리보기 (읽기 전용)' })), el('p', { class: 'admin-hint', text: '구성원 머신에 설치된 세션 훅이 각자 세션 컨텍스트에 실제로 무엇을 주입하는지, 그 최종 메시지를 보여줍니다. 훅을 고르면 그 훅이 넣는 메시지 전문을 미리볼 수 있습니다. exact=게이트웨이/설치파일이 단일 출처(드리프트 없음), approximate=멤버 머신이 세션마다 동적으로 덧붙이는 부분이 있어 일부만 재현됩니다.' }));
    detail.replaceChildren(card);
    ...
    api('/api/ui/org/hooks/preview').then((r) => {
        ...
        if (!hooks.length) {
            card.append(el('p', { class: 'empty', text: '미리볼 세션 훅이 없습니다.' }));
            return;
        }
        let sel = state.admin.hookPreviewSel;
        if (!hooks.some((h) => h.id === sel))
            sel = hooks[0].id;
        ...
        for (const h of hooks) {
            ...
            listCol.append(el('div', { class: 'mini-row' + (h.id === sel ? ' sel' : ''),
                onclick: () => { state.admin.hookPreviewSel = h.id; renderHookPreviewDetail(); } }, el('div', { class: 'mini-title', text: h.title || h.id }, el('span', { class: 'pill', text: h.event })), el('div', { class: 'mini-meta' }, el('span', { class: 'src-status' }, el('span', { class: 'dot6 ' + (h.fidelity === 'exact' ? 'ok' : 'dim'), 'aria-hidden': 'true' }), fidLabel), ' · ' + h.id)));
        }
        const right = el('div', {});
        card.append(el('div', { class: 'admin-two' }, listCol, right));
        function renderHookPreviewDetail() {
            for (const row of listCol.querySelectorAll('.mini-row'))
            ...
            for (const [k, v] of metaRows)
                metaTable.append(el('tr', {}, el('td', { text: k }), el('td', { text: v })));
            const msg = h.message || '';
            const detailBody = el('div', {}, el('h3', { text: h.title || h.id }), metaTable, el('div', { class: 'sec-label', style: 'margin-top:14px' }, '세션에 주입되는 최종 메시지'));
            if (!msg) {
                ...
                const raw = el('pre', { class: 'admin-preview' });
                raw.textContent = msg;
                raw.hidden = true;
...
function mcpForm(root, s, data, detail, isNew) {
    ...
    let catalog = [];
    api('/api/ui/org/connector-catalog').then((r) => {
        catalog = r.catalog || [];
...
async function renderClickupMemberPanel(panel) {
    ...
    let res;
    try {
        res = await api('/api/ui/org/connector/clickup/members');
    }
    catch (e) {
        panel.replaceChildren(el('p', { class: 'admin-hint', text: 'ClickUp 멤버 로드 실패: ' + e.message }));
        return;
    }
    ...
    if (!users.length) {
        panel.replaceChildren(head, intro, el('p', { class: 'admin-hint', text: 'ClickUp 팀 멤버가 없습니다.' }));
        return;
    }
    ...
    for (const r of users) {
        ...
        if (/^#[0-9a-fA-F]{3,8}$/.test(cu.color || '')) {
            dot.style.background = cu.color;
            dot.style.color = '#fff';
        }
        const userCell = el('td', {}, el('div', { class: 'cu-user' }, dot, el('div', {}, el('div', { class: 'mini-title', text: cu.username || ('id ' + cu.id) }), el('div', { class: 'mini-meta', text: cu.email || ('id ' + cu.id) }))));
        if (r.mapped_via === 'identity') {
            // ① 매핑됨(identities 명시 행) — 표시명 + 해제.
            const unlink = el('button', { class: 'btn-text', text: '해제' });
            unlink.addEventListener('click', async () => {
                if (!confirm(`'${cu.username || cu.id}' ↔ '${nameOf(r.mapped_member_id)}' 연결을 해제할까요?`))
                    return;
                unlink.disabled = true;
                try {
                    await postIdentities(r.mapped_member_id, cu, false);
            ...
            saveB.addEventListener('click', async () => {
                ...
                saveB.disabled = true;
                try {
                    await postIdentities(selBox.value, cu, true);
                    toast('연결됨 — 다음 싱크부터 담당자에 반영');
                    void renderClickupMemberPanel(panel);
                }
                catch (e) {
                    toast(e.message, true);
...
function dbSourceEditor(detail, data) {
    ...
    const dbSafety = allowlistCard(data, 'DB 접속 안전범위 (allowlist)', 'db_query/db_schema 데이터소스가 접속할 수 있는 사설/내부 host(이 목록 밖의 사설/localhost 는 차단 — SSRF 방어. 외부 공인 DB 는 등록 불요)와, auth_ref 가 참조할 수 있는 비번 env 이름(값 아님 — 게이트웨이 프로세스 env 에 있어야 함)입니다.', [
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('DB 데이터소스', data.meaning['db-source']), el('div', { class: 'admin-two' }, listCol, right)), dbSafety);
}
...
function customHookEditor(detail, data) {
    ...
    if (editing)
        ...
        right.append(el('p', { class: 'admin-hint', text: '구성원 머신에서 특정 시점에 자동 실행되는 코드입니다. 본문은 멤버 디스크에 저장되지 않고 매 세션 게이트웨이에서 받아 실행됩니다(끄면 다음 세션부터 무효).' }));
...
function toolsEditor(detail, data) {
    ...
    for (const t of proxyTools) {
        listCol.append(el('div', { class: 'mini-row' + (t.name === sel ? ' sel' : ''),
            onclick: () => { state.admin.toolSel = t.name; renderAdminDetail(detail, 'tools', data); } }, el('div', { class: 'mini-title', text: t.name }, t.enabled === false ? el('span', { class: 'pill', text: '비활성' }) : null, t.auto_approve ? el('span', { class: 'pill pill-warn', text: '자동승인' }) : null), el('div', { class: 'mini-meta', text: (t.method || 'GET') + ' · ' + (t.scope || '-') })));
    }
    const right = el('div', {});
    const editing = sel === '__new__'
    ...
    if (editing)
        ...
        right.append(el('p', { class: 'admin-hint', text: '사내 API를 AI 도구로 래핑합니다. 저장 즉시(재설치 없이) 구성원 AI가 씁니다. 호출은 런타임 설정의 화이트리스트 안에서만, 인증은 환경변수 이름으로만.' }), builtinToggles(data));
    const pol = data.toolPolicy || { url_allowlist: [], allowed_auth_envs: [] };
    const toolsSafety = allowlistCard(data, '외부 호출 안전범위 (allowlist)', 'AI 도구(http_proxy)가 외부를 호출할 수 있는 범위 — 이 목록 밖은 차단(SSRF 방어). 사내 API 도구를 안 쓰면 비워둬도 됩니다.', [
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('AI 도구(MCP)', data.meaning['tool']), el('div', { class: 'admin-two' }, listCol, right)), toolsSafety);
}
...
function toolForm(root, t, data, detail, isNew) {
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            renderAdminDetail(detail, 'tools', state.admin.data);
        }
        catch (e) {
            toast(e.message, true);
            saveBtn.disabled = false;
...
function deployPanel(detail, data) {
    ...
    const blocks = deployCommands(gw, os).map((c) => c.kind === 'install' ? installSelfBlock(gw, os) : staticBlock(c));
    detail.replaceChildren(el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '설치 · 업데이트 · 제거' }), osTabs), el('p', { class: 'admin-hint', text: '본인 머신에 설치/업데이트/제거하는 명령입니다. 업데이트·제거는 설치된 토큰을 자동으로 읽어 토큰 재입력이 필요 없습니다. (다른 구성원에게 배포할 토큰은 [구성원 추가] 탭에서.)' }), ...blocks));
}
// OS별 설치 명령(토큰 박음).
...
function installSelfBlock(gw, os) {
    ...
    const cpLabel = canCp ? el('label', { class: 'caption', style: 'display:block;margin:3px 0 7px;cursor:pointer' }, cpChk, el('span', { text: '관리 권한(admin/runtime) 포함 — 이 토큰으로 설치한 로컬 세션이 관리탭 기능(구성원·토큰·훅·DB소스)을 MCP로 직접 다룹니다. 변경은 감사에 AI로 남습니다(#632).' })) : null;
    ...
    go.addEventListener('click', async () => {
        ...
        try {
            ...
            result.replaceChildren(el('p', { class: 'admin-hint', text: '✓ 본인 토큰 발급됨(scope: ' + (r.scopes || []).join('/') + '). 본인 머신에서 아래를 실행하세요 — 토큰은 지금만 보입니다.' }), el('div', { class: 'deploy-head' }, el('span', {}), copyButton(() => cmd, '명령 복사')), el('pre', { class: 'admin-preview', text: cmd }));
        }
        catch (e) {
            toast(e.message, true);
        }
    ...
    return el('div', { class: 'deploy-block' }, el('h3', { text: '설치 (본인 머신)' }), el('p', { class: 'admin-hint', text: '본인 토큰을 발급해 본인 머신에 설치합니다(git 불필요). 새 기기/재설치 시 사용.' }), cpLabel, el('div', { class: 'install-minter' }, go), result);
}
...
function fieldWithHelp(label, control, m) {
    return el('div', { class: 'field' }, el('div', { class: 'field-label-row' }, el('label', { class: 'field-label', text: label }), meaningCard(m)), control);
}
...
function parseMyProfile(md) {
    ...
    r.len = (PROF_LEN.find((l) => len.startsWith(l.v)) || {}).v || '';
    r.area = grab(/^[-*\s]*\**\s*담당[^:：\n]*\**\s*[:：]\s*(.+)$/m);
    r.tools = grab(/^[-*\s]*\**\s*자주[^:：\n]*\**\s*[:：]\s*(.+)$/m);
    return r;
}
...
function credVaultCard(owner, title, intro, creds, encReady, reload) {
    ...
    rows.push(el('div', { class: 'card', style: 'padding:12px; margin-top:10px;' }, el('div', { class: 'field-label', style: 'margin-bottom:8px', text: '+ 자격 추가' }), field('서비스', kindSel), scopeField, secretField, helpP, docLink, el('div', { class: 'admin-actions', style: 'margin-top:10px' }, submit, status)));
...
function awsRoleCard(creds, reload) {
    ...
    const rows = [el('p', { class: 'admin-hint', style: 'margin:0 0 6px', text: 'AWS 는 토큰 대신 "역할(role)"을 등록해요. 게이트웨이가 이 역할을 각 구성원 이름으로 가정(assume)해 15분짜리 단기 자격을 발급합니다 — 장기 키가 새어나갈 일이 없고, AWS CloudTrail 에 누가 무엇을 했는지 남아요. "전원 기본"은 조회(readonly) 역할로 두고, 쓰기가 필요한 구성원만 개별 오버라이드하세요. (역할·신뢰관계는 AWS 관리자가 먼저 만들어야 해요.)' })];
    ...
    rows.push(el('div', { class: 'card', style: 'padding:12px; margin-top:10px;' }, el('div', { class: 'field-label', style: 'margin-bottom:8px', text: '+ AWS 역할 등록 / 오버라이드' }), field('적용 대상', targetSel), memberField, field('역할 ARN (role ARN)', arnIn), field('리전 (region)', regionSel), field('서명 service(선택 · 기본 execute-api)', serviceIn), field('ExternalId(선택)', extIn), field('구분 이름(선택)', scopeIn), el('div', { class: 'admin-actions', style: 'margin-top:10px' }, submit, status)));
...
async function dbAuditEditor(detail, data) {
    ...
    const apply = () => {
        ...
        if (f.source)
            void renderSubjectKeyPanel(subjPanel, f.source, data);
        else
            subjPanel.replaceChildren(el('p', { class: 'admin-hint', text: "특정 소스를 고르면 '누구의 정보를 조회했는지' 기록할 식별자 컬럼을 지정할 수 있습니다." }));
    };
    ...
    const card = el('div', { class: 'card' }, sectionTitle('DB 접근 감사', '구성원(과 그들의 AI)이 db_query·db_schema 로 어떤 데이터를 조회했는지 전부 기록됩니다 — 조회자·시각·소스·테이블·마스킹/열람 컬럼·대상 식별자. 위변조 방지(해시체인)이며, 신용정보법상 조회 기록 보존에 씁니다.'), bar, el('div', { class: 'audit-verify' }, verifyBtn, verifyOut), body);
    if (f.source)
        void renderSubjectKeyPanel(subjPanel, f.source, data);
    else
        subjPanel.append(el('p', { class: 'admin-hint', text: "위 '모든 소스'를 특정 소스로 바꾸면, 그 소스에서 '누구의 정보를 조회했는지' 기록할 식별자 컬럼(예: 고객ID)을 지정할 수 있습니다." }));
...
async function loadAuditRows(body, f) {
    ...
    body.replaceChildren(el('p', { class: 'admin-hint', text: `${rows.length}건${r.total > rows.length ? ` (전체 ${r.total}건 중 최근 100건)` : ''} · '열람(raw)'은 마스킹 우회(언마스크) 조회 — 붉은 표시는 주목 대상` }), el('div', { class: 'audit-scroll' }, tbl));
}
...
async function openMyProfile() {
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        if (l)
            lines.push('- 응답 길이: ' + l.v + ' — ' + l.hint);
        if (areaIn.value.trim())
            lines.push('- 담당 영역: ' + areaIn.value.trim());
        if (toolsIn.value.trim())
            lines.push('- 자주 쓰는 도구·레포: ' + toolsIn.value.trim());
