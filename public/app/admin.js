...
function renderAdminDetail(detail, sel, data) {
    ...
    if (sel === 'managed-policy' || sel === 'org-defaults' || SCAFFOLD_SECTIONS.includes(sel))
...
async function cronPanel(detail, data) {
    ...
    for (const j of jobs) {
        ...
        const acts = el('div', { class: 'wikicat-row-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: '지금 실행', onclick: () => cronRunNow(j.id, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: j.enabled ? '끄기' : '켜기', onclick: () => cronToggle(j, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: '수정', onclick: () => openCronForm(j, reload) }), el('button', { class: 'btn btn-ghost btn-sm', text: '삭제', onclick: () => cronDelete(j.id, reload) }));
        rows.append(el('div', { class: 'wikicat-row' }, main, acts));
    }
    const head = el('div', { class: 'wikicat-grouphead' }, el('span', { class: 'wikicat-grouptitle', text: '스케줄 잡' }), el('span', { class: 'wikicat-groupcount', text: String(jobs.length) }), el('button', { class: 'btn btn-ghost btn-sm wikicat-add', text: '+ 잡 추가', onclick: () => openCronForm(null, reload) }));
...
async function openCronForm(job, reload) {
    ...
    for (const [v, l] of actions)
        actionSel.append(el('option', { value: v, text: l, ...((job && job.action === v) ? { selected: true } : {}) }));
    ...
    try {
        ...
        for (const s of ((r && r.sessions) || []))
            sessSel.append(el('option', { value: s.id, text: (s.label || s.id) + ' (' + s.id + ')', ...((params.session === s.id) ? { selected: true } : {}) }));
    }
    catch { /* 세션 목록 실패해도 폼은 연다 */ }
    const saveBtn = el('button', { class: 'btn btn-primary btn-sm', text: isNew ? '잡 추가' : '저장' });
    const form = el('div', { class: 'proj-settings' }, block('잡 id', isNew ? '소문자 슬러그(a-z0-9_-). 잡의 고유 키.' : 'id 는 변경 불가.', idInp), block('이름', '관리 목록에 보일 이름.', labelInp), block('액션', '게이트웨이가 실행할 작업(허용 목록).', actionSel), sessBlock, repoBlock, systemBlock, block('주기 (초)', '이 간격마다 실행(최소 60). cron식이 있으면 그게 우선.', intervalInp), block('cron식 (선택)', '벽시계 스케줄. 예: 0 9 * * 1-5 = 평일 09:00. 비우면 주기초.', cronInp), block('켬', '', el('label', { class: 'inline' }, enabledChk, el('span', { text: ' 활성화' }))), el('div', { class: 'ps-rules-actions' }, saveBtn));
    syncVis();
    ...
    saveBtn.onclick = async () => {
        ...
        const p = {};
        if (action === 'map_unmapped' && sessSel.value)
            p.session = sessSel.value;
        if (action === 'refresh_repo' && repoInp.value.trim())
            p.repo = repoInp.value.trim();
        if (action === 'connector_sync' && systemInp.value.trim())
            p.system = systemInp.value.trim();
...
function openManagedSessionForm(m, reload) {
    ...
    const form = el('div', { class: 'proj-settings' }, block('세션 id', isNew ? '소문자 슬러그(a-z0-9_-). 고유 키.' : 'id 는 변경 불가.', idInp), block('이름', '관리 목록·세션 탭에 보일 이름.', labelInp), block('라이블리 계정/프로필', '어떤 클로드 로그인(시드)으로 띄울지. 지금은 단일 프로필 — 곧 프로필별 로그인.', accountInp), block('격리 워크스페이스(하위경로)', '공유폴더 아래 이 세션 전용 작업폴더. 비우면 managed/<id>.', wsInp), block('하네스', '', harnessSel), block('자동 승인', '도구 실행을 묻지 않고 진행(무인 작업에 필요).', el('label', { class: 'inline' }, autoChk, el('span', { text: ' --dangerously-skip-permissions' }))), block('항상 켬(keep-alive)', '죽으면 재생성.', el('label', { class: 'inline' }, enabledChk, el('span', { text: ' enabled' }))), el('div', { class: 'ps-rules-actions' }, saveBtn));
...
function membersEditor(detail, data) {
    ...
    for (const m of data.members) {
        ...
        listCol.append(el('div', { class: 'mini-row' + (m.id === sel ? ' sel' : ''),
            ...
            onclick: () => { state.admin.memberSel = m.id; state.admin.memberEditing = false; renderAdminDetail(detail, 'members', data); } }, el('div', { class: 'mini-title', text: (m.display_name || m.id) }, canEdit ? (m.hasToken ? el('span', { class: 'pill pill-ok', text: '설치됨' }) : el('span', { class: 'pill', text: '미설치' })) : null), el('div', { class: 'mini-meta', text: meta })));
    }
    const right = el('div', {});
    ...
    if (member && state.admin.memberEditing)
        memberForm(right, member, data, detail, false);
    else if (member)
        memberRead(right, member, data, detail);
    else
        right.append(el('p', { class: 'admin-hint', text: canEdit ? '왼쪽에서 구성원을 고르세요. 새 구성원은 [구성원 추가] 탭에서.' : '읽기 전용 — 이름·종류만 표시됩니다(이메일·계정·권한은 관리자만).' }));
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('구성원 관리', meaning), el('div', { class: 'admin-two admin-two-cols' }, listCol, right))); // 이 탭만 좌우 2단 유지
}
...
function injectionMap(detail, data) {
    ...
    const ssBlock = momentBlock('세션 시작 — SessionStart', '대화가 열릴 때 조직 컨텍스트를 자동으로 깔아준다 — 맨 위 조직 헤더(자동) 다음, 아래 순서로 조립.', momentToggle('session_preload'), el('div', { class: 'inj-pieces' }, pieceRow('1', '회사 소개·규칙·AI 성격', '회사 배경 + 항상 지킬 규칙 + AI 말투 (섹션)', editBtn('org-defaults')), pieceRow('2', '컨텍스트 온톨로지 가이드', '아래 ${ } 자리표시자를 담는 전체 골격 — 주입 위치·순서는 이 템플릿이 정함 (고급)', editBtn('context-ontology-guide')), el('div', { class: 'inj-subpieces' }, el('div', { class: 'admin-hint inj-sub', text: '└ 가이드 본문의 ${ } 자리에 매 세션 실제 데이터로 자동 채워짐:' }), subPieceRow('${rules}', '항상-주입 지식', 'injection=always 로 표시한 지식 전문(규칙·페르소나) · 가이드 맨 위', jump('WIKI(always) →', '#/knowledge?injection=always')), subPieceRow('${categories}', '카테고리 지도', '전 카테고리(주제) 목록 — 자동 생성(편집 불가)', null), subPieceRow('${wiki}', 'WIKI 인덱스 핀', '핀(is_wiki)한 지식의 제목·소환키만(본문 제외) · 가이드 맨 끝', jump('WIKI 인덱스 →', '#/knowledge/pinned')))), previewExpander(), customList('SessionStart'));
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('세션 주입 지도', null), el('p', { class: 'admin-hint', text: '이 조직의 AI가 매 세션 자동으로 [무엇을·언제] 받고 수행하나를 한곳에 모았습니다. 각 맥락 조각의 “편집”은 그 조각의 정식 위치(규칙·소개 섹션 또는 WIKI 탭)로 이동합니다 — 여기서 새로 만드는 게 아니라 한 지도에서 전부 도달합니다.' }), !rc ? el('p', { class: 'admin-hint', text: '※ 주입 시점 ON/OFF·너지 편집은 관리자만 가능합니다. 아래는 보기 전용 + 편집 위치로의 이동만 동작합니다.' }) : null, el('div', { class: 'inj-moments' }, ssBlock, ptuBlock, stopBlock, otherBlock)));
}
...
function installSelfBlock(gw, os) {
    ...
    return el('div', { class: 'deploy-block' }, el('h3', { text: '설치 (본인 머신)' }), el('p', { class: 'admin-hint', text: '본인 토큰을 발급해 본인 머신에 설치합니다(git 불필요). 새 기기/재설치 시 사용.' }), el('div', { class: 'install-minter' }, go), result);
}
...
// ════════════════════════════════════════════
export { copyButton, deployCommands, field, hasScope, installCmd, loadAdmin, overlay, renderSystem, };
