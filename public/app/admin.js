    ...
    const items = profiles.length ? profiles.map((p) => {
        ...
        if (!os.ready) {
            ...
            kids.push(el('button', { class: 'btn btn-ghost btn-sm', text: '지금 미리 만들기', onclick: async (ev) => {
                    ...
                    try {
                        await api('/api/ui/terminal/members/provision-os', { method: 'POST', body: JSON.stringify({ member: p.id }) });
                        toast('OS 격리 유저 생성됨 — 이 멤버 세션이 본인 계정으로 격리됩니다');
                        reload();
                    }
                    catch (e) {
                        btn.disabled = false;
            ...
            kids.push(el('button', { class: 'btn btn-ghost btn-sm', text: '재프로비저닝(격리·토큰 갱신)', onclick: async (ev) => {
                    ...
                    try {
                        await api('/api/ui/terminal/members/provision-os', { method: 'POST', body: JSON.stringify({ member: p.id }) });
                        toast('재프로비저닝됨 — 로그인·실행중 세션 유지, 새 세션부터 새 토큰');
                        reload();
                    }
                    catch (e) {
                        btn.disabled = false;
