...
function oauthConnectorsCard(conns, reload) {
    ...
    return el('div', { class: 'card' }, el('h3', { class: 'admin-subhead', text: 'OAuth 로그인' }), el('p', { class: 'admin-hint', style: 'margin:0 0 10px', text: 'Notion·Slack·Google 처럼 OAuth 로그인이 필요한 외부 도구 서버예요. [연결]을 누르면 새 탭에서 로그인·동의하고, 그 뒤 AI가 나로서 그 서비스를 씁니다(토큰은 게이트웨이가 안전 보관·자동 갱신).' }), el('div', { class: 'admin-actions' }, el('button', { class: 'btn btn-ghost btn-sm', text: '새로고침', onclick: reload })), ...rows);
}
...
async function myCredentialsSection(host) {
    ...
    kids.push(credVaultCard('me', '토큰·API 키 (직접 등록)', 'GitLab PAT 처럼 서비스에서 발급한 토큰·API 키를 직접 넣어 둡니다 — AI가 나로서 그 서비스를 씁니다. (OAuth로 연결하는 서비스는 아래 [OAuth 로그인]에서.)', (mine.credentials || []).filter((c) => c.kind !== 'aws_role_arn'), encReady, () => myCredentialsSection(host)));
