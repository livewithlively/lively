...
async function renderAdmin(view, sub) {
    ...
    for (const s of ADMIN_SECTIONS) {
        ...
        list.append(el('a', { class: 'row' + (s.key === sel ? ' sel' : '') + indentCls, href: '#/system/' + s.key }, el('div', { class: 'row-title', text: s.label }), el('div', { class: 'row-meta', text: adminRowMeta(s.key, data) })));
    }
    ...
    view.replaceChildren(el('div', {}, el('div', { class: 'card-head admin-head' }, el('div', { class: 'admin-head-l' }, el('h2', { text: '관리' })), canEdit
        ...
        : el('span', { class: 'admin-sub' }, el('span', { class: 'pill', text: '읽기 전용' }), ' ' + (data.profile.display_name || '조직') + ' · 보기 전용(편집은 관리자)')), groupBar, split));
...
function renderAdminDetail(detail, sel, data) {
    ...
    if (sel === 'hooks-group')
        return hooksOverview(detail, data);
    if (sel === 'runtime')
        return runtimeEditor(detail, data);
    if (sel === 'hooks-preview')
        return hooksPreviewPanel(detail, data);
    if (sel === 'custom-hooks')
...
async function wikiCategoriesPanel(detail, data) {
    ...
    let bySpace;
    try {
        const lists = await Promise.all(SPACE_SUBS.map((s) => api('/api/ui/categories?' + new URLSearchParams({ space: s.key })).then((d) => (d && d.categories) || [])));
        bySpace = {};
    ...
    for (const s of SPACE_SUBS) {
        ...
        if (!items.length) {
            ...
            for (const c of items) {
                ...
                const main = el('div', { class: 'wikicat-row-main' }, el('span', { class: 'wikicat-name', text: c.name || c.key }), el('span', { class: 'wikicat-key mono', text: c.key }), c.cross_cutting ? el('span', { class: 'dm-tag', text: '횡단' }) : null, shouldLine);
    ...
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '카테고리 설정' }), canEdit ? null : el('span', { class: 'admin-sub' }, el('span', { class: 'pill', text: '읽기 전용' }), ' 편집은 context 권한 필요')), el('p', { class: 'admin-hint', text: '지식(위키)의 분류축입니다. 사업·제품·시스템 카테고리를 한 화면에서 추가·수정·삭제하며, 변경은 지식·프로젝트 탭 좌측 카테고리에 그대로 반영됩니다. (제품 카테고리=도메인)' }), list);
    detail.replaceChildren(card);
}
...
async function showMemberPreview() {
    try {
        ...
        overlay('구성원의 AI가 매 세션 실제로 읽는 내용', el('p', { class: 'admin-hint', text: '아래가 모든 구성원의 대화 첫머리에 주입되는 정적 컨텍스트입니다(라이브 현황은 별도로 매 세션 자동 추가).' }), el('pre', { class: 'admin-preview', text: r.context || '(비어 있음)' }));
    }
    catch (e) {
        toast(e.message, true);
    }
...
async function showGuidePreview(bodyMd) {
    try {
        ...
        overlay('이 가이드가 실제 주입되는 모습 (주제·강제규칙 자동 채움)', el('p', { class: 'admin-hint', text: '아래는 이 편집 내용의 ${area}·${rules} 가 실제 데이터로 채워져 매 대화에 주입되는 부분입니다(회사 규칙·소개 등 다른 부분은 제외).' }), el('pre', { class: 'admin-preview', text: r.context || '(비어 있음)' }));
    }
    catch (e) {
        toast(e.message, true);
    }
