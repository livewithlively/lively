...
function sectionTitle(titleText, m) {
    ...
    return el('div', {}, el('div', { class: 'section-title' }, el('h2', { text: titleText }), isText ? null : meaningCard(m)), isText ? el('p', { class: 'admin-hint' }, ...inlineBold(m)) : null);
}
...
async function renderAdmin(view, sub) {
    ...
    //  #837 추가: 그룹명 아래 한 줄 힌트("누가 쓰나"·"AI에게 뭘 가르치나"…) — 새 분류축이 무엇인지 이름만으론 안 보인다.
    const side = el('nav', { class: 'docs-side admin-side', 'aria-label': '관리 섹션' });
    for (const g of ADMIN_GROUPS) {
        const items = visibleSections.filter((s) => s.group === g.key);
        if (!items.length)
            continue;
        const box = el('div', { class: 'docs-side-group' }, el('div', { class: 'docs-side-title', text: g.label }, g.hint ? el('span', { class: 'docs-side-hint', text: g.hint }) : null));
        for (const s of items) {
            // 회색 부제 없이 라벨만(#613). 예외는 '지식 검토 정책'(#802 계승) — 검토 대기 건수 배지(0건이면 안 그린다).
            //  큐 자체는 WIKI 로 갔지만(#837), 게이트를 켠 관리자가 "내 밸브에 N건이 밀려 있다"를 여기서 보게 둔다.
            box.append(el('a', { class: 'docs-item' + (s.key === sel ? ' active' : ''), href: '#/system/' + s.key,
                'aria-current': s.key === sel ? 'page' : null }, el('span', { text: s.label }), s.key === 'ingest-policy' ? reviewNavBadge() : null));
        }
        side.append(box);
    }
    const detail = el('div', {});
...
function renderAdminDetail(detail, sel, data) {
    // ── ① nav 섹션 ──
    if (sel === 'me-profile')
        return void myProfileSection(detail);
    if (sel === 'me-ai')
...
function injectionMap(detail, data) {
    ...
    const updBlock = momentBlock('키트 자동 업데이트 — SessionStart(백그라운드)', '구성원 컴퓨터의 라이블리 키트(훅 코드·배선)를 게이트웨이 최신본과 맞춘다. 세션 시작에 버전만 비교하고, 다르면 백그라운드로 내려받아 재설치 → 다음 세션부터 적용(현재 세션은 방해하지 않음). 회사맥락·스킬은 원래도 매 세션 자동이라 이 토글과 무관.', momentToggle('self_update', ' 자동 업데이트 켜기', '자동 업데이트 켜짐 — 구성원 다음 세션부터', '자동 업데이트 꺼짐 — 구성원이 직접 업데이트 명령을 실행해야 합니다'), el('p', { class: 'admin-hint inj-sub', text: '끄면 훅·배선 변경이 구성원에게 전달되지 않습니다(구성원이 [내 AI 세션 생성] 화면의 업데이트 명령을 직접 실행해야 함). 구성원 개인이 끄려면 환경변수 LIVELY_NO_AUTO_UPDATE=1.' }));
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('세션 주입', null), el('p', { class: 'admin-hint', text: '이 조직의 AI가 매 세션 자동으로 [무엇을·언제] 받고 수행하나를 한곳에 모았습니다. 항상-주입 섹션 문서(맨 위 자동 헤더 다음에 sort 순으로 깔림)는 여기서 직접 추가·편집·삭제·재정렬합니다.' }), !rc ? el('p', { class: 'admin-hint', text: '※ 주입 시점 ON/OFF·너지 편집은 관리자만 가능합니다. 아래는 보기 전용 + 편집 위치로의 이동만 동작합니다.' }) : null, el('div', { class: 'inj-moments' }, ssBlock, ptuBlock, stopBlock, updBlock, otherBlock)));
}
...
function connectorEditor(detail, data) {
    ...
    const banner = (editing && editing.secrets_enabled === false)
        ...
        : null;
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('외부 자료 수집', data.meaning && data.meaning['connector']), banner, el('div', { class: 'admin-two admin-two-cols' }, listCol, right)));
}
...
async function renderConnectorMemberPanel(panel, system) {
    ...
    const head = sectionTitle('멤버 매핑 · ' + label, label + ' 의 사람을 우리 구성원과 연결합니다 — 연결하면 다음 싱크부터 작성자·담당자가 그 구성원으로 매칭돼요. 이메일이 같으면 자동매치 후보가 미리 선택됩니다. **여기가 매핑을 편집하는 곳입니다** — 구성원 화면에서는 결과만 보여요.');
    if (res.error) {
...
function dbSourceEditor(detail, data) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('DB 데이터소스', data.meaning['db-source']), el('div', { class: 'admin-two admin-two-cols' }, listCol, right)), dbSafety);
}
...
function toolsEditor(detail, data) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, el('div', { class: 'admin-two admin-two-cols' }, listCol, right)), toolsSafety);
}
...
async function myProfileSection(detail) {
    ...
    try {
        ...
        detail.replaceChildren(el('div', { class: 'card' }, errorNote(e, '내 정보를 불러오지 못했습니다')));
        return;
    }
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('내 정보', '이름·사진은 프로젝트·작업 기록·팀 화면 어디에서나 나를 가리키는 얼굴이에요.'), field('프로필 사진', ava.node), field('표시 이름', nameIn), data.email ? field('이메일 (로그인 아이디 · 변경은 관리자)', el('div', { class: 'admin-ro', text: data.email })) : null, data.email ? field('비밀번호', el('div', { style: 'display:flex; align-items:center; gap:10px; flex-wrap:wrap;' }, el('button', { type: 'button', class: 'btn btn-ghost btn-sm', text: '비밀번호 변경', onclick: () => changePasswordModal() }), el('span', { class: 'admin-hint', style: 'margin:0', text: '현재 비밀번호를 확인한 뒤 새 비밀번호로 바꿔요.' }))) : null, el('div', { class: 'admin-actions' }, saveBtn, status)));
}
...
async function myAiSection(detail) {
    ...
    function previewExpander() {
        ...
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                box.replaceChildren(el('p', { class: 'admin-hint', style: 'margin:0 0 8px' }, ...inlineBold('내 AI 가 매 세션 첫머리에 **실제로 읽는 전문**입니다 — 조직 맥락 + 우리 팀 + 아래 개인 규칙. 다른 구성원에겐 그 사람의 개인 규칙이 실립니다.')), ctx
                    ? el('div', { class: 'md-rendered admin-md-box', style: 'max-height:420px; overflow:auto' }, renderMarkdown(ctx))
                    : el('p', { class: 'admin-hint', text: '미리볼 내용이 없습니다.' }));
            }
            catch (e) {
                box.replaceChildren(errorNote(e, '미리보기를 불러오지 못했습니다'));
            }
        });
        return el('div', {}, el('div', { class: 'admin-actions' }, btn), box);
    }
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('AI 개인화', '여기서 고르면 **내** AI 가 매 세션 첫머리에 `내 개인 규칙` 으로 읽습니다 — 나에게만 적용되고 팀에 공유되지 않아요. 비밀번호·토큰 같은 시크릿은 넣지 마세요(자동 차단).'), field('역할', roleIn), field('개발 이해도', el('div', {}, devChips, devHint)), field('호칭 (AI가 나를 부르는 말)', addressIn), field('말투', toneChips), field('추가 메모', memoTa), el('div', { class: 'admin-actions' }, saveBtn, status), previewExpander()));
}
// ── [내 설정 ▸ 내 서비스 로그인] — member_secret vault + OAuth 연결 + git 인증 ──
async function myLoginsSection(detail) {
    const head = el('div', { class: 'card' }, sectionTitle('내 서비스 로그인', 'AI 가 **나로서** 외부 서비스(슬랙·깃랩·노션 등)에 접근할 때 쓸 내 로그인입니다. 암호화 저장되고 다른 사람에게 보이지 않아요. — 우리 게이트웨이에 접속하는 [접속 열쇠]와는 다른 것입니다.'), field('git 인증 (레포 접근)', el('div', { style: 'display:flex; align-items:center; gap:10px; flex-wrap:wrap;' }, el('button', { type: 'button', class: 'btn btn-ghost btn-sm', text: 'git 인증 관리', onclick: () => openGitCredentialManager('me') }), el('span', { class: 'admin-hint', style: 'margin:0', text: 'private 레포 클론·세션(shell·Claude) 안 git 에 쓸 SSH 키/토큰을 등록해요.' }))));
    const vault = el('div', {});
...
async function showHarnessDetail(kind, id, name) {
    ...
    try {
        ...
        if (d.body)
            parts.push(el('div', { class: 'md' }, ...[(() => { const m = el('div', {}); m.innerHTML = renderMarkdown(String(d.body)); return m; })()]));
        if (!parts.length)
...
async function myAssetsSection(detail) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('내 스킬 · 훅', '라이블리가 배포한 스킬·훅과 **내 컴퓨터에 깔린 로컬 하네스**를 한눈에 보고 켜고 꺼요. 겹치면 조직이 함께 관리·개선하는 라이블리 쪽을 권합니다. 다음 세션부터 반영돼요.'), bodyBox));
    const reload = async () => {
        ...
        const livelyRow = (targetKind, it) => {
            ...
            const nameLink = el('a', { href: '#', class: 'mini-title', style: 'text-decoration:none' }, el('span', { text: it.label || it.id }), el('span', { class: 'pill', text: it.effective ? '적용 중' : '미적용' }));
            nameLink.addEventListener('click', (ev) => { ev.preventDefault(); showHarnessDetail(it.kind || 'hook', it.id, it.label || it.id); });
            return el('div', { class: 'mini-row', style: 'display:flex; align-items:center; gap:12px;' }, el('div', { style: 'flex:1; min-width:0;' }, nameLink, el('div', { class: 'mini-meta', text: kindLabel + (desc ? ' · ' + (desc.length > 90 ? desc.slice(0, 90) + '…' : desc) : '') })), seg);
        };
        const rows = [];
        ...
        if (lskills.length)
            lskills.forEach((sk) => rows.push(livelyRow('harness_asset', sk)));
        else
            rows.push(el('p', { class: 'admin-hint', text: '배포된 스킬이 없어요.' }));
        if (lhooks.length) {
            rows.push(el('h4', { style: 'margin:14px 0 6px', text: '커스텀 훅' }));
            lhooks.forEach((h) => rows.push(livelyRow('org_hook', h)));
        }
        ...
        if (machines.length) {
            for (const m of machines) {
                ...
                const head = el('div', { style: 'display:flex; align-items:center; gap:10px; margin:18px 0 6px' }, el('h4', { style: 'margin:0', text: title }), m.at ? el('span', { class: 'mini-meta', text: '· ' + new Date(m.at).toLocaleString('ko-KR') }) : null);
                ...
                del.addEventListener('click', async () => {
                    if (!confirm(`'${title}' 의 하네스 관측을 목록에서 지울까요? (그 컴퓨터에서 다시 세션을 열면 자동으로 다시 나타납니다.)`))
                        return;
                    try {
                ...
                const list = (m.assets || []);
                if (!list.length) {
                    rows.push(el('p', { class: 'admin-hint', text: '내가 직접 만든 로컬 자산은 없어요(라이블리가 준 것만).' }));
                    continue;
                }
                for (const a of list) {
                    if (a.overlap === 'shadow')
                    ...
                    const nameEl = a.overlap === 'managed'
                        ? (() => { const l = el('a', { href: '#', class: 'mini-title', style: 'text-decoration:none', text: a.id }); l.addEventListener('click', (ev) => { ev.preventDefault(); showHarnessDetail(a.kind, a.id, a.id); }); return l; })()
                        : el('span', { class: 'mini-title', text: a.id });
                    rows.push(el('div', { class: 'mini-row', style: 'display:flex; align-items:center; gap:12px;' }, el('div', { style: 'flex:1; min-width:0;' }, nameEl, el('div', { class: 'mini-meta', text: kindLabel + ' · ' + tagText })), seg));
                }
            }
        }
        else {
        ...
        if (shadowTotal) {
            rows.unshift(el('div', { class: 'admin-hint', style: 'padding:10px 12px; background:#fff6e6; border:1px solid #f0d59a; border-radius:8px; margin-bottom:10px' }, el('b', { text: `내 로컬 자산 ${shadowTotal}개가 라이블리 것과 이름이 같아 라이블리 배포를 가리고 있어요. ` }), '같은 이름이면 라이블리 쪽을 권합니다(로컬 것을 끄면 라이블리 것이 살아나요). 내 것이 더 낫다면 AI 에게 “이 스킬 라이블리에 올리게 정리해줘” 라고 해서 관리자에게 전달하세요.'));
        }
