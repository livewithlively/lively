...
function membersEditor(detail, data) {
    ...
    const memberRow = (m) => {
        ...
        return el('div', { class: 'mini-row member-row' + (m.id === state.admin.memberSel ? ' sel' : ''),
            ...
            onclick: () => { state.admin.memberSel = m.id; state.admin.memberEditing = false; renderAdminDetail(detail, 'members', data); } }, profileAvatar(m.avatar || null, m.display_name || m.id, m.id, 'member-ava', { char: m.avatar_char, color: m.avatar_color }), el('div', { class: 'member-row-body' }, el('div', { class: 'mini-title' }, el('span', { class: 'member-name', text: (m.display_name || m.id) }), canEdit ? (m.hasToken ? el('span', { class: 'pill pill-ok', text: '설치됨' }) : el('span', { class: 'pill', text: '미설치' })) : null), el('div', { class: 'mini-meta', text: meta })));
    };
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('구성원 관리', meaning), members.length ? searchBar : null, body));
}
