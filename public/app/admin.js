...
function customHookEditor(detail, data) {
    ...
    if (editing)
        ...
        right.append(el('p', { class: 'admin-hint', text: '구성원 머신에서 특정 시점에 자동 실행되는 코드입니다. 본문은 멤버 디스크에 저장되지 않고 매 세션 게이트웨이에서 받아 실행됩니다(끄면 다음 세션부터 무효).' }), relayPolicyCard(data, detail));
...
async function myAssetsSection(detail) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('내 스킬 · 훅', '라이블리가 배포한 스킬·훅이 **어느 컴퓨터에 깔렸는지** 한눈에 보고 켜고 꺼요. 켜져 있는데 아직 안 깔린 PC는 ‘미설치’로 표시돼요. 아래에는 컴퓨터마다 **내가 직접 만든** 로컬 하네스가 따로 나오고, 별명도 붙일 수 있어요. 다음 세션부터 반영돼요.'), bodyBox));
    const reload = async () => {
        ...
        const livelyRow = (targetKind, it, kind) => {
            ...
            const left = el('div', { class: 'harness-click', style: 'flex:1; min-width:0;', title: '눌러서 내용 보기' }, titleEl, el('div', { class: 'mini-meta', text: kindLabel + (desc ? ' · ' + (desc.length > 90 ? desc.slice(0, 90) + '…' : desc) : '') }), el('div', { style: 'margin-top:6px' }, chipRow));
        ...
        if (machines.length) {
            ...
            for (const m of machines) {
                ...
                if (m.at)
                    head.append(el('span', { class: 'mini-meta', text: '· ' + new Date(m.at).toLocaleString('ko-KR') + ' 관측' }));
