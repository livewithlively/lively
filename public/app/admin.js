...
const ADMIN_GROUPS = [
    ...
    { key: 'data', label: '데이터 연결', hint: 'AI가 어디에 닿나' },
    { key: 'ops', label: '운영·감사', hint: '무슨 일이 있었나' },
];
...
function sectionTitle(titleText, m) {
    ...
    return el('div', {}, el('div', { class: 'section-title' }, el('h2', { text: titleText }), isText ? null : meaningCard(m)), isText ? el('p', { class: 'admin-hint', text: m }) : null);
}
...
async function renderAdmin(view, sub) {
    ...
    state.admin.sel = sel;
    const detail = el('div', { class: 'admin-body' });
    ...
    view.replaceChildren(el('div', {}, pageHead('관리', '조직·권한, 분류 체계, 연결·데이터 등 시스템 전반을 설정합니다.', [statusEl], '리'), el('div', { class: 'docs-layout admin-layout' }, adminSidebar(visibleSections, sel), detail)));
    applyReveal([detail]);
}
// ── 좌측 사이드바 — 그룹 머리글 + 항목. 가로 중분류 바를 대체한다(#837).
//  사용 가이드(#/learn)의 `.docs-side` 문법을 그대로 재사용: 한국어에선 uppercase·자간으로 위계를 못 주므로
//  '굵은 잉크 머리글 + 항목 들여쓰기'로 준다(styles.css 의 해당 주석 참조). 새 시각 언어를 만들지 않는다.
//  16개 화면이 항상 한눈에 보이므로 "이게 어느 분류였지"를 뒤질 일이 없고, 섹션마다 화면 골격이 바뀌지 않는다.
function adminSidebar(visibleSections, sel) {
    const side = el('nav', { class: 'docs-side', 'aria-label': '관리 설정' });
    for (const g of ADMIN_GROUPS) {
        const items = visibleSections.filter((s) => s.group === g.key);
        if (!items.length)
            continue; // 권한으로 그룹이 통째로 비면 머리글도 안 그린다
        const box = el('div', { class: 'docs-side-group' }, el('div', { class: 'docs-side-title', text: g.label }, el('span', { class: 'docs-side-hint', text: g.hint })));
        for (const s of items) {
            const on = s.key === sel;
            // 배지는 '지식 검토 게이트'에만(#802 계승) — 대기 건수가 0이면 안 그린다. 큐 자체는 WIKI 로 갔지만,
            //  게이트를 켠 관리자가 "내 밸브에 N건이 밀려 있다"를 여기서 보게 둔다.
            box.append(el('a', { class: 'docs-item' + (on ? ' active' : ''), href: '#/system/' + s.key,
                'aria-current': on ? 'page' : null }, el('span', { text: s.label }), s.key === 'ingest-policy' ? reviewNavBadge() : null));
        }
        side.append(box);
    }
    return side;
...
function membersSection(detail, data) {
    ...
    detail.replaceChildren(sectionHead('구성원', '이 조직을 누가 쓰는지, 각자 무엇으로 접속하는지, AI 를 어떤 계정으로 돌리는지.', data.meaning['member']), segTabs('members', [
        { key: 'list', label: '구성원', render: (h) => membersEditor(h, data) },
        // 접속 열쇠 = 우리 게이트웨이에 접속하는 bearer 토큰(auth_token). 외부 서비스 로그인과 다른 것 — 용어 사전 참조.
        { key: 'tokens', label: '접속 열쇠', show: admin, render: (h) => tokensPanel(h, data) },
...
function toolsSection(detail, data) {
    ...
    detail.replaceChildren(sectionHead('AI 도구', 'AI 가 호출할 수 있는 것 전부 — 기본 제공 도구, 우리가 감싼 사내 API, 외부 도구 서버(MCP).', data.meaning['tool']), segTabs('tools', [
...
function automationSection(detail, data) {
    detail.replaceChildren(sectionHead('자동화', '사람 없이 도는 것 — 정해진 시각에 실행하는 스케줄과, 그 작업을 수행할 상시 에이전트.'), segTabs('automation', [
...
function auditSection(detail, data) {
    detail.replaceChildren(sectionHead('감사 로그', '누가·언제·무엇을 했나. 관리 항목 변경, DB 조회, AI 의 도구 호출 — 셋 다 읽기 전용입니다.'), segTabs('audit', [
...
async function toolUsagePanel(detail) {
    ...
    const card = el('div', { class: 'card' }, el('p', { class: 'admin-hint', text: '하네스(Claude·Codex 등)가 어떤 MCP 툴을 어떤 인자로 어느 빈도로 호출했는지입니다. 모든 호출이 기록되며(시크릿 마스킹·큰 값 절단), AI에게 묻거나 db_query 로 mcp_call_log 를 직접 조회할 수도 있습니다.' }), controls, stats, daysEl, el('div', { class: 'tu-sub', text: '툴별 호출' }), toolTable, byHarness.length ? el('div', { class: 'tu-sub', text: '하네스별' }) : null, byHarness.length ? harnessChips : null, el('div', { class: 'tu-sub', text: '최근 호출' + (total ? ' (' + total.toLocaleString() + ')' : '') }), calls, pagerBox);
...
async function orgAuditPanel(detail) {
    ...
    const card = el('div', { class: 'card' }, el('p', { class: 'admin-hint', text: '구성원·접속 열쇠·런타임·외부 자료 수집·DB소스·훅·도구 등 관리 항목이 누구에 의해(사람/AI) 어떤 경로(웹/MCP)로 언제 어떻게 바뀌었는지 기록입니다. 각 줄을 펼치면 바뀐 필드의 이전→이후를 볼 수 있어요(시크릿은 마스킹). AI에게 묻거나 org_audit_list(MCP)로도 조회할 수 있습니다.' }), controls, el('div', { class: 'oa-sub', text: '변경 이력' + (total ? ' (' + total.toLocaleString() + ')' : '') }), list, pagerBox);
    detail.replaceChildren(card);
}
...
async function cronPanel(detail, data) {
    ...
    const card = el('div', { class: 'card' }, el('p', { class: 'admin-hint', text: '게이트웨이가 주기 실행하는 잡입니다. is 신선화(refresh)·미매핑 코드 LLM 분류(map_unmapped → 타깃 상시 에이전트에 주입, 팀플랜 과금)·외부 자료 수집 싱크 등. 주기는 “초” 또는 cron식.' }), el('p', { class: 'admin-hint', text: 'cron식의 시각은 ' + tz + ' 기준입니다(서버 머신의 OS 시간대와 무관 — [조직 · 연결] ▸ 조직 시간대에서 바꿉니다).' }), el('div', { class: 'wikicat' }, el('div', { class: 'wikicat-group' }, head, rows)));
    detail.replaceChildren(card);
}
...
async function managedSessionsPanel(detail, data) {
    ...
    const card = el('div', { class: 'card' }, el('p', { class: 'admin-hint', text: '항상 떠 있는 에이전트 세션입니다. 격리 워크스페이스(공유폴더)에서 돌고, keep-alive 가 죽으면 재생성합니다. 크론(미매핑 분류 등)이 이 세션에 작업을 쏩니다 — 팀플랜 과금. account = 어떤 라이블리 계정(클로드 로그인)으로 띄울지.' }), el('div', { class: 'wikicat' }, el('div', { class: 'wikicat-group' }, head, rows)));
    detail.replaceChildren(card);
}
...
async function reposPanel(detail, data) {
    ...
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '레포(git)' }), state.admin.canEdit ? el('button', { class: 'btn btn-ghost btn-sm', text: '게이트웨이 git 계정', onclick: () => openGitCredentialManager('gateway') }) : null, canEdit ? null : el('span', { class: 'admin-sub' }, el('span', { class: 'pill', text: '읽기 전용' }), ' 편집은 context 권한 필요')), el('p', { class: 'admin-hint', text: '코드 레포(실제 git 레포)를 등록·연결합니다. 여기 설정한 git 주소·기본 브랜치는 도메인맵 스캔과 ‘내 컴퓨터에서 작업’ 클론이 함께 씁니다. 레포는 code_unit 이 매핑되는 단위예요. private 레포 클론 인증은 [게이트웨이 git 계정] 또는 각 구성원의 [내 프로필 ▸ git 인증]에서 설정합니다. HTTPS 가 막힌 셀프호스팅(GitLab 등)은 git 주소를 SSH 형(git@호스트:그룹/레포.git)으로 넣으세요.' }), el('div', { class: 'wikicat' }, el('div', { class: 'wikicat-group' }, head, rows)));
    detail.replaceChildren(card);
}
...
async function wikiCategoriesPanel(detail, data) {
    ...
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '카테고리 (분류 체계)' }), canEdit ? null : el('span', { class: 'admin-sub' }, el('span', { class: 'pill', text: '읽기 전용' }), ' 편집은 context 권한 필요')), el('p', { class: 'admin-hint', text: '지식(위키)의 분류축입니다. 사업·제품·시스템 카테고리를 한 화면에서 추가·수정·삭제하며, 변경은 지식·프로젝트 탭 좌측 카테고리에 그대로 반영됩니다. (제품 카테고리=도메인)' }), list);
    detail.replaceChildren(card);
}
...
function membersEditor(detail, data) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, (members.length || canEdit) ? bar : null, listCol));
}
...
async function teamsPanel(detail, data) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('팀', { key: 'team' }), el('p', { class: 'admin-hint', text: '구성원을 팀으로 묶고, 팀이 맡는 카테고리(도메인)를 정합니다. 카테고리 오너 배정은 [분류 체계 관리]에서, 여기선 팀과 팀원을 관리합니다. 팀 소유 카테고리는 팀원의 프로젝트·위키 탭과 AI 세션에 먼저 노출됩니다(오너십=우선순위, 접근제한 아님).' }), el('div', { class: 'admin-two admin-two-cols' }, listCol, right)));
}
// 팀 보기(수정 전 읽기 요약).
...
function tokensPanel(detail, data) {
    ...
    const children = [
        el('p', { class: 'admin-hint', text: '구성원이 우리 게이트웨이에 접속할 때 쓰는 열쇠입니다. 외부 서비스(슬랙·깃랩 등) 로그인은 [AI 능력 ▸ 서비스 로그인]으로 — 다른 것입니다.' }),
        installMinterBlock(data, gw, { title: '접속 열쇠 발급' }),
...
function storageEditor(detail, data) {
    ...
    //
    // ⚠ 권한(#837 결함 수정) — 이 화면은 admin 전용인데, 버튼이 부르는 POST /api/ui/v6/projects/:id/reclaim 은
    //  **memory** scope 를 요구한다(workspace_reclaim capability — 프로젝트 생애주기 도구라 그렇게 설계됨).
    //  admin 은 memory 를 함의하지 않으므로(web.ts mw 가 엄격 includes) admin 만 가진 관리자는 눌러도 403 이었다.
    //  서버 권한모델은 손대지 않는다 — reclaim 은 파일을 지우는 파괴적 도구라 선언적 2중 게이트(mw + 핸들러 경계)를
    //  유지하는 게 맞다. 대신 클라가 **서버가 실제로 요구하는 권한**대로 버튼을 켜고, 없으면 이유를 말한다.
    const canReclaim = canEdit && hasScope('memory');
    ...
    async function loadWorkspace() {
        ...
        const rows = (ws.projects || []).map((p) => {
            ...
            analyzeBtn.disabled = !canReclaim || p.project_id == null;
...
function embeddingsEditor(detail, data) {
    const canEdit = !!data.canEdit;
    const body = el('div');
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('의미 검색 (임베딩)', null), el('p', { class: 'admin-hint', text: '지식 의미검색·유사도·중복감지가 쓰는 벡터 임베딩을 켜고 끕니다. 기본은 꺼짐(정확 grep 검색으로 폴백). 켜면 OpenAI-compatible /v1/embeddings 엔드포인트(로컬 Ollama 사이드카 또는 외부 API)로 임베딩합니다. 설정은 게이트웨이 재시작 없이 즉시 반영됩니다.' }), body));
    body.append(el('p', { class: 'admin-hint', text: '불러오는 중…' }));
    let pollTimer = null;
...
function toolsEditor(detail, data) {
    ...
    // 빌트인 토글은 #837 에서 [기본 제공 도구] 서브탭으로 분리 — 여기서 또 그리면 같은 화면에 두 번 나온다.
        ...
        right.append(el('p', { class: 'admin-hint', text: '사내 API를 AI 도구로 래핑합니다. 저장 즉시(재설치 없이) 구성원 AI가 씁니다. 호출은 런타임 설정의 화이트리스트 안에서만, 인증은 환경변수 이름으로만.' }));
    ...
    detail.replaceChildren(el('div', { class: 'card' }, el('div', { class: 'admin-two' }, listCol, right)), toolsSafety);
}
...
function oauthConnectorsCard(conns, reload) {
    ...
    return el('div', { class: 'card' }, sectionTitle('OAuth 로그인', 'Notion·Slack·Google 처럼 OAuth 로그인이 필요한 외부 도구 서버예요. [연결]을 누르면 새 탭에서 로그인·동의하고, 그 뒤 AI가 나로서 그 서비스를 씁니다(토큰은 게이트웨이가 안전 보관·자동 갱신).'), el('div', { class: 'admin-actions' }, el('button', { class: 'btn btn-ghost btn-sm', text: '새로고침', onclick: reload })), ...rows);
}
// 커넥터 현황(#746 imp#4·#5) — 기본 카탈로그 각 커넥터의 등록/설정 상태 개관(관리자 온보딩 지도).
function catalogStatusCard(catalog, servers) {
    const byName = new Map((servers || []).map((s) => [s.name, s]));
    const rows = [el('p', { class: 'admin-hint', style: 'margin:0 0 8px', text: '기본 제공되는 외부 도구 서버(MCP) 프리셋의 현재 상태입니다 — 외부 자료 수집(미러)과는 다른 것이에요. 추가·발행은 [AI 도구 ▸ 외부 도구 서버]에서, 구성원 연결은 각자 [연결].' })];
    for (const c of (catalog || [])) {
...
async function credentialsEditor(detail) {
    ...
    const cards = [
        el('div', { class: 'card' }, sectionTitle('서비스 로그인 (조직)', 'AI 가 외부 서비스(GitLab·Slack·AWS 등)에 접속할 때 쓰는 조직 공용 로그인입니다. — 우리 게이트웨이에 접속하는 [접속 열쇠]와는 다른 것이고(그건 [구성원 ▸ 접속 열쇠]), 개인 로그인은 우측 상단 [내 프로필]에 있습니다.'), encReady ? null : el('p', { class: 'gate-error', text: '⚠ 서버에 암호화 키(CONNECTOR_SECRET_KEY)가 없어 자격을 저장할 수 없습니다 — 관리자에게 요청하세요.' })),
        catalogStatusCard(catalog.catalog || [], mcpServers.servers || []),
        credVaultCard('org', '통합 자격', '개인 로그인이 없는 구성원이 조회(비-PII read)할 때 공용으로 쓰는 로그인이에요. 쓰기·외부발신·민감정보에는 쓰이지 않아요(그건 개인 로그인 필수).', (org.credentials || []).filter((c) => c.kind !== 'aws_role_arn'), encReady, () => credentialsEditor(detail)),
...
async function myCredentialsSection(host) {
    ...
    kids.push(credVaultCard('me', '내 서비스 로그인', 'AI 가 나로서 그 서비스에 접근할 때 씁니다(예: GitLab MR 올리기, Slack 검색). 암호화 저장되고 다른 사람에게 보이지 않아요.', (mine.credentials || []).filter((c) => c.kind !== 'aws_role_arn'), encReady, () => myCredentialsSection(host)));
...
async function dbAuditEditor(detail, data) {
    ...
    const card = el('div', { class: 'card' }, sectionTitle('DB 조회 기록', "구성원(과 그들의 AI)이 db_query·db_schema 로 어떤 데이터를 조회했는지 전부 기록됩니다 — 조회자·시각·소스·테이블·마스킹/열람 컬럼·대상 식별자. 위변조 방지(해시체인)이며, 신용정보법상 조회 기록 보존에 씁니다. — '누구의 정보인지' 기록할 식별자 컬럼 지정은 설정이므로 [데이터 연결 ▸ DB 데이터소스]에 있습니다."), bar, el('div', { class: 'audit-verify' }, verifyBtn, verifyOut), body);
    detail.replaceChildren(card);
    void loadAuditRows(body, f);
}
