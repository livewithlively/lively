...
function myAiAccountsCard() {
    ...
    const load = async () => {
        ...
        try {
            ...
            body.replaceChildren(anyShared ? el('p', { class: 'aiacct-note' }, ...inlineBold('이 서버는 구성원별 AI 계정 격리가 없습니다 — 아래 **‘서버 공용’ 표시가 붙은 AI 는 서버 전체가 같은 계정**을 씁니다. 내가 연결한 것이 아닐 수 있고, 여기서 로그인하면 다른 구성원에게도 그대로 적용됩니다.')) : null, ...(accounts.length
