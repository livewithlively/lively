...
async function teamsPanel(detail, data) {
    ...
    detail.replaceChildren(sectionHead('팀', '구성원을 팀으로 묶고, 팀이 맡는 카테고리를 정합니다. 팀이 맡은 카테고리는 팀원의 화면과 AI 세션에 먼저 나옵니다.', { key: 'team' }), el('div', { class: 'card' }, cardHead('팀 목록과 담당 카테고리'), el('div', { class: 'admin-two admin-two-cols' }, listCol, right)));
}
// 팀 보기(수정 전 읽기 요약).
    ...
    const reload = async () => {
        ...
        const onOffSeg = (targetKind, it) => {
            ...
            const note = following
                ...
                : el('span', { class: 'sw-note' }, el('span', { text: '내가 바꿈 · 기본값은 ' + (it.byDefault ? '켬' : '끔') }), el('button', { type: 'button', class: 'btn-text sw-reset', text: '되돌리기', onclick: () => void set(null) }));
