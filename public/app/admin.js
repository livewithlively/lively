...
function membersSection(detail, data) {
    ...
    detail.replaceChildren(sectionHead('구성원', '이 조직을 누가 쓰는지, 각자 무엇으로 접속하는지, AI를 어떤 계정으로 실행하는지.', data.meaning['member']), segTabs('members', [
...
function toolsSection(detail, data) {
    ...
    detail.replaceChildren(sectionHead('AI 도구', 'AI가 호출할 수 있는 것 전부 — 사내 API 도구, 기본 제공 도구, 외부 도구 서버(MCP).', data.meaning['tool']), segTabs('tools', [
...
function agentAssetsSection(detail, data) {
    detail.replaceChildren(sectionHead('스킬 · 훅', '구성원의 AI에 배포되는 스킬·서브에이전트·커맨드와, 이벤트에 반응하는 코드 훅을 관리합니다.', data.meaning['harness-asset']), segTabs('agent-assets', [
...
function automationSection(detail, data) {
    detail.replaceChildren(sectionHead('자동화', '사람 없이 자동으로 실행되는 작업입니다 — 정해진 시각에 실행하는 스케줄과, 그 작업을 수행할 상시 에이전트를 관리합니다.'), segTabs('automation', [
...
function auditSection(detail, data) {
    detail.replaceChildren(sectionHead('감사 로그', '누가 언제 무엇을 했는지 기록합니다. 관리 항목 변경·DB 조회·AI 도구 호출 세 가지이며, 모두 읽기 전용입니다.'), segTabs('audit', [
...
async function orgAuditPanel(detail) {
    ...
    const card = el('div', { class: 'card' }, el('p', { class: 'admin-hint', text: '구성원·접속 토큰·런타임·외부 자료 수집·DB 소스·훅·도구 등 관리 항목이 누구에 의해(사람/AI) 어떤 경로(웹/MCP)로 언제 어떻게 바뀌었는지 기록합니다. 각 줄을 펼치면 바뀐 필드의 이전→이후를 볼 수 있습니다(시크릿은 마스킹). AI에게 묻거나 org_audit_list(MCP)로도 조회할 수 있습니다.' }), controls, el('div', { class: 'oa-sub', text: '변경 이력' + (total ? ' (' + total.toLocaleString() + ')' : '') }), list, pagerBox);
    detail.replaceChildren(card);
}
...
async function cronPanel(detail, data) {
    ...
    const card = el('div', { class: 'card' }, el('p', { class: 'admin-hint', text: '게이트웨이가 정해진 주기마다 실행하는 잡입니다. 실제 코드 의존(is) 최신화(refresh), 미매핑 코드 유닛 LLM 분류(map_unmapped — 타깃 상시 에이전트에 주입, 팀플랜 과금), 외부 자료 수집 싱크 등이 있습니다. 주기는 초 단위 간격 또는 cron식으로 지정합니다.' }), el('p', { class: 'admin-hint', text: 'cron식의 시각은 ' + tz + ' 기준입니다(서버 머신의 OS 시간대와 무관 — [조직 · 연결] ▸ 조직 시간대에서 바꿉니다).' }), el('div', { class: 'wikicat' }, el('div', { class: 'wikicat-group' }, head, rows)));
    detail.replaceChildren(card);
}
...
async function managedSessionsPanel(detail, data) {
    ...
    const card = el('div', { class: 'card' }, el('p', { class: 'admin-hint', text: '항상 실행 상태로 유지되는 에이전트 세션입니다. 격리 워크스페이스(공유폴더)에서 실행되며, keep-alive 점검에서 세션이 없으면 자동으로 다시 만듭니다. 크론 잡(미매핑 분류 등)이 이 세션에 작업을 전달합니다 — 팀플랜 과금. account 는 이 세션을 실행할 라이블리 계정(클로드 로그인)입니다.' }), el('div', { class: 'wikicat' }, el('div', { class: 'wikicat-group' }, head, rows)));
    detail.replaceChildren(card);
}
...
async function reposPanel(detail, data) {
    ...
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '레포(git) · ' + repos.length + '개' }), canEdit ? el('button', { class: 'btn btn-ghost btn-sm', text: '+ 레포 추가', onclick: () => openRepoForm(null, reload) }) : null, state.admin.canEdit ? el('button', { class: 'btn btn-ghost btn-sm', text: '게이트웨이 git 계정 관리', onclick: () => openGitCredentialManager('gateway') }) : null, canEdit ? null : el('span', { class: 'admin-sub' }, el('span', { class: 'pill', text: '읽기 전용' }), ' 편집은 context 권한 필요')), el('p', { class: 'admin-hint', text: '코드 레포(실제 git 레포)를 등록·연결합니다. 여기 설정한 git 주소·기본 브랜치는 도메인맵 스캔과 ‘내 컴퓨터에서 작업’ 클론이 함께 씁니다. 레포는 code_unit 이 매핑되는 단위입니다. private 레포 클론 인증은 [게이트웨이 git 계정 관리] 또는 각 구성원의 [내 프로필 ▸ git 인증]에서 설정합니다.' }), el('div', { class: 'wikicat' }, el('div', { class: 'wikicat-group' }, rows)));
    detail.replaceChildren(card);
}
...
async function wikiCategoriesPanel(detail, data) {
    ...
    const card = el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '카테고리 (분류 체계)' }), canEdit ? null : el('span', { class: 'admin-sub' }, el('span', { class: 'pill', text: '읽기 전용' }), ' 편집은 context 권한 필요')), el('p', { class: 'admin-hint', text: '지식(위키)의 분류 체계(분류축)입니다. 사업·제품·시스템 카테고리를 한 화면에서 추가·수정·삭제하며, 변경은 지식·프로젝트 탭 좌측 카테고리에 그대로 반영됩니다. (제품 카테고리=도메인)' }), list);
    detail.replaceChildren(card);
}
...
function profileEditor(detail, data) {
    ...
    detail.replaceChildren(el('div', { class: 'card admin-form-narrow' }, ...body));
}
...
function tokensPanel(detail, data) {
    ...
    const children = [
        ...
        el('div', { class: 'meaning-grid', style: 'margin:16px 0 12px' }, meaningRow('발급된 토큰은', '지금 누가 회사 게이트웨이에 연결할 수 있는지 보여줍니다. 사용 현황을 확인하고, 필요할 때 특정 구성원의 접속을 해제(차단)할 수 있습니다.'), meaningRow('언제 해제하나', '퇴사·기기 분실 등 그 사람의 접속을 끊어야 할 때. 해제하면 서버를 다시 켤 필요 없이 그 즉시 막힙니다(되돌릴 수 없음).')),
    ];
    if (active.length)
        ...
        children.push(el('p', { class: 'admin-hint', text: '아직 발급된 접속 토큰이 없습니다 — 위에서 구성원을 골라 발급하세요.' }));
    if (revoked.length)
...
function injectionMap(detail, data) {
    ...
    canEdit ? listEditor('외부 인입 툴(pull_tools) — 외부 맥락을 가져온 세션을 종료 너지 대상에 넣습니다', rc.pull_tools, 'pull_tools', 'mcp__lively__ext__', '이 prefix 로 시작하는 MCP 툴을 사용하면 외부 맥락을 가져온 세션으로 봅니다(가져온 내용을 기록하지 않은 채 끝내는 것을 막음) · 줄당 툴이름 prefix · 비우면 이 기능이 꺼집니다. 기본 mcp__lively__ext__ = 라이블리 MCP 프록시 전체. ⚠ 훅은 mcp__lively__* 만 관측하므로 다른 서버 prefix 는 적어 두어도 감지되지 않습니다(구성원이 자기 하네스에 직접 추가한 MCP 커버는 후속 작업)') : null, customList('PostToolUse'));
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('세션 주입', null), el('p', { class: 'admin-hint', text: '이 조직의 AI 가 매 세션 무엇을 언제 자동으로 받고 수행하는지 한곳에 모았습니다. 항상 주입되는 섹션 문서(맨 위 자동 헤더 다음, sort 순으로 삽입)는 여기서 직접 추가·편집·삭제·재정렬합니다.' }), !rc ? el('p', { class: 'admin-hint', text: '※ 주입 시점 ON/OFF·너지 편집은 관리자만 가능합니다. 아래는 보기 전용 + 편집 위치로의 이동만 동작합니다.' }) : null, el('div', { class: 'inj-moments' }, ssBlock, ptuBlock, stopBlock, updBlock, otherBlock)));
}
...
function storageEditor(detail, data) {
    const canEdit = !!data.canEdit;
    const body = el('div');
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('저장소 · 로그', null), el('p', { class: 'admin-hint', text: '이 서버의 디스크가 얼마나 찼는지 확인하고, 로그가 무한히 쌓이지 않도록 상한을 정합니다. 디스크가 가득 차면 DB가 중단되어 로그인을 포함한 모든 기능이 멈춥니다 — 그 전에 알아채는 것이 이 화면의 목적입니다. 저장하면 게이트웨이 재시작 없이 즉시 반영됩니다.' }), body));
...
function embeddingsEditor(detail, data) {
    const canEdit = !!data.canEdit;
    const body = el('div');
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('의미 검색 (임베딩)', null), el('p', { class: 'admin-hint', text: '지식 의미검색·유사도·중복감지가 쓰는 벡터 임베딩을 켜고 끕니다. 기본은 꺼짐이며, 이때는 정확 일치(grep) 검색을 사용합니다. 켜면 OpenAI-compatible /v1/embeddings 엔드포인트(로컬 Ollama 사이드카 또는 외부 API)로 임베딩합니다. 설정은 게이트웨이 재시작 없이 즉시 반영됩니다.' }), body));
    body.append(el('p', { class: 'admin-hint', text: '불러오는 중…' }));
    let pollTimer = null;
...
function customHookEditor(detail, data) {
    ...
    if (editing)
        ...
        right.append(el('p', { class: 'admin-hint', text: '구성원 머신에서 특정 시점에 자동 실행되는 코드입니다. 본문은 구성원 디스크에 저장되지 않고 매 세션 게이트웨이에서 받아 실행됩니다(비활성화하면 다음 세션부터 실행되지 않습니다). 왼쪽 목록에서 항목을 선택하면 내용을 보고 편집할 수 있습니다.' }));
...
function catalogStatusCard(catalog, servers) {
    ...
    const rows = [el('p', { class: 'admin-hint', style: 'margin:0 0 8px', text: '기본 제공되는 외부 도구 서버(MCP) 프리셋의 현재 상태입니다 — 외부 자료 수집(미러)과는 별개 항목입니다. 추가·발행은 [AI 도구 ▸ 외부 도구 서버]에서 하고, 구성원은 각자 [연결]에서 자기 계정을 연결합니다.' })];
    for (const c of (catalog || [])) {
...
async function credentialsEditor(detail) {
    ...
    const cards = [
        el('div', { class: 'card' }, sectionTitle('서비스 로그인 (조직)', 'AI 가 외부 서비스(GitLab·Slack·AWS 등)에 접속할 때 쓰는 조직 공용 로그인입니다. 게이트웨이 접속에 쓰는 [접속 토큰]과는 별개이며(관리는 [구성원 ▸ 접속 토큰]에서), 개인 로그인은 우측 상단 [내 프로필]에서 관리합니다.'), encReady ? null : el('p', { class: 'gate-error', text: '⚠ 서버에 암호화 키(CONNECTOR_SECRET_KEY)가 없어 자격을 저장할 수 없습니다 — 관리자에게 요청하세요.' })),
...
function awsRoleCard(creds, reload) {
    ...
    const rows = [el('p', { class: 'admin-hint', style: 'margin:0 0 6px', text: 'AWS 는 토큰 대신 "역할(role)"을 등록합니다. 게이트웨이가 이 역할을 각 구성원 이름으로 가정(assume)해 15분 동안 유효한 단기 자격을 발급합니다 — 장기 키를 저장하지 않으므로 유출 위험이 없고, 누가 무엇을 했는지 AWS CloudTrail 에 기록됩니다. "전원 기본"은 조회(readonly) 역할로 두고, 쓰기가 필요한 구성원만 개별 오버라이드하세요. (역할·신뢰관계는 AWS 관리자가 먼저 만들어야 합니다.)' })];
...
async function dbAuditEditor(detail, data) {
    ...
    const card = el('div', { class: 'card' }, sectionTitle('DB 조회 기록', "구성원(과 그들의 AI)이 db_query·db_schema 로 어떤 데이터를 조회했는지 전부 기록됩니다 — 조회자·시각·소스·테이블·마스킹/열람 컬럼·대상 식별자. 기록은 해시체인으로 위변조를 방지하며, 신용정보법상 조회 기록 보존에 사용합니다. '누구의 정보인지'를 남길 식별자 컬럼은 [데이터 연결 ▸ DB 데이터소스]에서 지정합니다."), bar, body);
    detail.replaceChildren(card);
    void loadAuditRows(body, f);
}
...
async function myLoginsSection(detail) {
    ...
    const head = el('div', { class: 'card' }, sectionTitle('내 서비스 로그인', 'AI가 나로서(내 계정으로) 아래 서비스를 쓸 수 있게 미리 로그인해 두는 곳이에요. 로그인 정보는 암호화해 저장하고 다른 사람에게는 보이지 않아요. Lively 게이트웨이에 접속하는 [접속 토큰](관리 ▸ 구성원 ▸ 접속 토큰)과는 별개예요.'));
    const svcCard = el('div', { class: 'card' }); // 서비스별 탭이 여기 그려짐
    const gitCard = el('div', { class: 'card' }, el('h3', { class: 'admin-subhead', text: '레포 접근 (개발자용)' }), el('p', { class: 'admin-hint', style: 'margin:0 0 10px', text: '코드 저장소(GitHub·GitLab)에서 클론·푸시할 때 쓰는 SSH 키/토큰이에요. 코드 작업을 하지 않는다면 설정하지 않아도 돼요.' }), el('div', { class: 'admin-actions' }, el('button', { type: 'button', class: 'btn btn-ghost btn-sm', text: 'git 인증 관리', onclick: () => openGitCredentialManager('me') })));
...
async function myAssetsSection(detail) {
    ...
    detail.replaceChildren(el('div', { class: 'card' }, sectionTitle('내 스킬 · 훅', '라이블리가 배포한 스킬·훅이 **어느 컴퓨터에 설치됐는지** 한눈에 보고 켜고 끌 수 있어요. 켜져 있는데 아직 설치되지 않은 PC는 ‘미설치’로 표시돼요. 아래에는 컴퓨터마다 **내가 직접 만든** 로컬 하네스가 따로 나오고, 별명도 붙일 수 있어요. 켜고 끈 변경은 다음 세션부터 반영돼요.'), bodyBox));
    const reload = async () => {
