...
const TAXONOMY_PROMPT = "우리 조직의 분류체계(카테고리)를 처음부터 세워줘. `lively-taxonomy` 스킬을 따라서 — " +
 ...
 */
function emptyTaxonomyCard(canEdit) {
    const card = el('div', { class: 'card ctx-empty', style: 'margin:14px 0' }, el('p', { class: 'ctx-empty-t', text: '분류축이 아직 없습니다' }), el('p', { class: 'admin-hint' }, ...uiText('분류축은 지식과 프로젝트를 담는 갈래입니다. 갈래가 없으면 새로 들어오는 지식이 전부 「미분류」가 되고, 미분류 지식은 AI 가 검색해도 나오지 않습니다 — 쌓이기는 하는데 쓰이지 않습니다.')));
    if (!canEdit) {
    ...
    card.append(el('p', { class: 'admin-hint', style: 'margin-top:10px' }, el('b', { text: 'AI 에게 맡기기 (권장) — ' }), el('span', { text: '아래 문장을 그대로 주면 우리가 하는 일을 훑어 분류축을 제안합니다. 만들기 전에 목록을 보여주고 확인을 받습니다.' })), el('div', { class: 'mini-meta', style: 'gap:8px;margin-top:8px' }, copy, el('a', { class: 'btn btn-ghost btn-sm', href: '#/terminal', target: '_blank', rel: 'noopener', text: 'AI 세션 열기 ↗' })), pre, el('p', { class: 'admin-hint', style: 'margin-top:12px', text: '직접 만들려면 아래 공간별 [+ 추가] 를 쓰세요.' }));
    return card;
}
