...
async function buildWikiDoc(container, name, opts = {}) {
    ...
    try {
        ...
        if (e && e.status === 404) {
            container.replaceChildren(el('div', { class: 'wk-doc-missing' }, el('div', { class: 'wk-doc-missing-title', text: '문서를 찾을 수 없어요' }), el('div', { class: 'wk-doc-missing-sub', text: '삭제됐거나 파일명이 바뀌었을 수 있습니다. 휴지통에서 복원할 수 있어요.' }), el('div', { class: 'wk-doc-missing-actions' }, el('a', { class: 'btn btn-ghost btn-sm', href: '#/knowledge', text: '← 위키 홈' }), el('a', { class: 'btn btn-ghost btn-sm', href: '#/trash', text: '🗑 휴지통' }))));
            return;
        }
    ...
    const bodyMdNow = () => (body._getMd ? body._getMd() : (k.body_md || ''));
    ...
    if (cat) {
        crumbs.append(el('span', { class: 'wk-crumb-sep', 'aria-hidden': 'true', text: '›' }), el('a', { class: 'wk-crumb', href: '#/knowledge?category=' + encodeURIComponent(cat.category_id), text: cat.name || cat.key }));
    }
    for (const a of (Array.isArray(k.ancestors) ? k.ancestors : [])) {
        crumbs.append(el('span', { class: 'wk-crumb-sep', 'aria-hidden': 'true', text: '›' }), el('a', { class: 'wk-crumb', href: '#/k/' + encodeURIComponent(a.name), text: a.title || a.name }));
    }
    crumbs.append(el('span', { class: 'wk-crumb-sep', 'aria-hidden': 'true', text: '›' }), el('span', { class: 'wk-crumb wk-crumb-cur', title: k.title || '', text: String(k.short_title || '').trim() || k.title || k.name }));
    ...
    moreBtn.onclick = () => {
        ...
        pop.append(item(fullOn ? '⇤ 본문 폭 되돌리기' : '⤢ 전체 폭으로', async () => {
            ...
            if (canEdit) {
                try {
                    await api('/api/ui/knowledge/' + encodeURIComponent(k.name) + '/props-ui', { method: 'POST', body: JSON.stringify({ full_width: next }) });
                    k.props_ui = Object.assign({}, k.props_ui || {}, { full_width: next });
                }
                catch (_) { /* 화면만 */ }
            }
        }));
        if (canEdit)
            pop.append(item('✕ 삭제', () => knDelete(k.name, opts.onDeleted), true));
    ...
    function paintDecor() {
        ...
        coverEl.hidden = !cv;
        if (cv)
            applyCoverBg(coverEl, cv);
        coverEl.replaceChildren(...[canEdit && cv ? el('div', { class: 'wk-cover-btns' }, el('button', { class: 'wk-cover-btn', type: 'button', text: '커버 변경',
                onclick: (e) => openCoverPicker(e.target, { current: cv, onPick: (v) => saveDecor({ cover: v }) }) }), el('button', { class: 'wk-cover-btn', type: 'button', text: '제거', onclick: () => saveDecor({ cover: null }) })) : null].filter(Boolean));
        iconBtn.hidden = !ic;
        ...
        decorAdd.replaceChildren(...[
            canEdit && !ic ? el('button', { class: 'wk-decor-btn', type: 'button', text: '☺ 아이콘 추가',
                onclick: (e) => openEmojiPicker(e.target, { onPick: (em) => saveDecor({ icon: em }) }) }) : null,
            canEdit && !cv ? el('button', { class: 'wk-decor-btn', type: 'button', text: '🖼 커버 추가',
                onclick: (e) => openCoverPicker(e.target, { onPick: (v) => saveDecor({ cover: v }) }) }) : null,
        ].filter(Boolean));
    ...
    if (canRename) {
        ...
        titleEl.addEventListener('blur', async () => {
            ...
            try {
                const payload = { name: k.name, short_title: t, body_md: bodyMdNow() };
                if (k.is_folder)
    ...
    if (!canMeta && !k.summary)
        ledeEl.hidden = true;
    if (canMeta) {
        const oneLineGuard = (node) => {
            node.addEventListener('keydown', (e) => { if (e.key === 'Enter' && !e.isComposing && e.keyCode !== 229) {
                e.preventDefault();
                node.blur();
            } });
        ...
        ledeEl.addEventListener('blur', async () => {
            ...
            if (t === (k.summary || ''))
                return;
            try {
                const payload = { name: k.name, summary: t, body_md: bodyMdNow() };
                if (k.is_folder)
                    payload.is_folder = true;
    ...
    if (!String(k.short_title || '').trim())
        fullTitleEl.hidden = true; // 짧은 제목이 없으면 위 제목이 곧 원래 제목이다
    if (canRename) {
        fullTitleEl.addEventListener('keydown', (e) => { if (e.key === 'Enter' && !e.isComposing && e.keyCode !== 229) {
            e.preventDefault();
            fullTitleEl.blur();
        } });
    ...
    const editMetaSave = async (field, value) => {
        const payload = { name: k.name, body_md: bodyMdNow() };
        if (k.is_folder)
            payload.is_folder = true;
        payload[field] = value;
        const r = await api('/api/ui/knowledge', { method: 'POST', body: JSON.stringify(payload) });
        if (field === 'type') {
            ...
            try {
                const d = await api('/api/ui/knowledge/' + encodeURIComponent(k.name));
                k.categories = ((d && d.knowledge) || d).categories || k.categories;
            }
            catch (_) { /* 다음 리로드에서 정합 */ }
        }
    ...
    function proplineItem(text, opts2 = {}) {
        ...
        if (opts2.click)
            it.onclick = (ev) => opts2.click(ev.currentTarget);
        return it;
    }
    function paintPropline() {
        const parts = [wkTick(k)];
        if (k.confidence === 'human')
            parts.push(proplineItem('사람 저작', { cls: 'human', title: '사람이 직접 작성한 지식' }));
        const curCat2 = (Array.isArray(k.categories) ? k.categories : []).filter((c) => c.state !== 'rejected')[0];
        parts.push(proplineItem(KN_TYPE_LABEL[k.type] || k.type || '유형 없음', canMeta ? { title: '유형 — 클릭해서 변경', click: (a) => openKnMetaPicker(a, 'type', k, editMetaSave) } : {}));
        if (curCat2)
            parts.push(proplineItem(curCat2.name || curCat2.key, canMeta ? { title: '카테고리 — 클릭해서 변경', click: (a) => openKnMetaPicker(a, 'category', k, editMetaSave) } : {}));
        if (always)
            parts.push(proplineItem('⚡ 항상 주입', { cls: 'warn', title: '모든 세션 첫머리에 항상 주입되는 문서' }));
        if (k.lifecycle === 'archived')
            parts.push(proplineItem('📦 보관됨', { cls: 'dim' }));
        // #783 승인 전 지식 — 검색·주입에서 빠져 있다는 사실이 문서 자체에서 보여야 한다.
        if (k.lifecycle === 'pending')
            parts.push(proplineItem('🔎 검토 대기', { cls: 'warn', title: '승인 전까지 검색·세션주입·목록에 노출되지 않습니다' }));
    ...
    function openProplinePopover(anchor) {
    ...
    if (observed) {
        ...
        banner = el('div', { class: 'wk-banner mirror' }, el('span', { class: 'wk-banner-txt', text: (k.external_system === 'notion' ? '노션' : '외부') + ' 미러 — 원본에서 편집됩니다' + (k.last_synced_at ? ' · 동기화 ' + relTime(k.last_synced_at) : '') }), safe ? el('a', { class: 'wk-banner-link', href: safe, target: '_blank', rel: 'noopener noreferrer', text: '원본 열기 ↗' }) : null);
    }
    else if (always) {
        banner = el('div', { class: 'wk-banner always' }, el('span', { class: 'wk-banner-txt', text: '항상 주입 문서 — 본문은 관리 탭 · 세션 주입에서 편집합니다' }), el('a', { class: 'wk-banner-link', href: '#/system/injection-map', text: '세션 주입 열기 →' }));
    }
    ...
    if (k.lifecycle === 'pending') {
        ...
        if (canEdit) {
            ...
            acts.push(el('button', {
                ...
                onclick: async () => {
                    if (!confirm('이 지식을 반려할까요? 휴지통으로 이동하며 복원할 수 있습니다.'))
                        return;
                    try {
    ...
    const doSave = async (force, opts) => {
        if (!editor || saving || (!force && !editor.isDirty()))
            return;
        ...
        try {
            const payload = { name: k.name, body_md: md || HOME_EMPTY };
            ...
            if (!keepalive) { // 지수 백오프 자동 재시도(첫 실패에만 토스트 1회) — online 이벤트가 별도로 재플러시
                ...
                clearTimeout(retryTimer);
                retryTimer = setTimeout(() => { if (editor && editor.el.isConnected && editor.isDirty())
                    doSave(true); }, retryDelay);
                if (first)
                    toast('저장이 지연되고 있어요 — 자동 재시도 중', true);
            }
        }
        saving = false;
        if (!lastSaveFailed && editor && editor.isDirty())
            queueSave(); // 저장 중 추가 편집분 재큐잉(성공 시에만)
    };
    ...
    async function confirmAndSave() {
        if (!editor || !editor.isDirty() || confirming || rawOpen)
            return false;
        confirming = true;
        let ok = false;
        try {
    ...
    if (k.is_folder) {
        ...
        editor.el.addEventListener('focusout', (ev) => {
            const to = ev && ev.relatedTarget;
            if (to && editor.el.contains(to))
                return;
            if (document.querySelector('.ov-confirm-back'))
                return; // 이미 확인창이 떠 있다
            setTimeout(() => { if (editor && editor.isDirty() && !editor.el.contains(document.activeElement))
                confirmAndSave(); }, 0);
        });
        body._getMd = () => editor.getMarkdown();
    ...
    function toggleRaw() {
        if (rawOpen) {
            if (editableDoc && rawBox) {
                ...
                if (ta && editor) {
                    editor.setMarkdown(ta.value);
                    doSave(true);
                } // setMarkdown 이 dirty 리셋 → 강제 저장
            }
            if (rawBox)
                rawBox.remove();
            rawBox = null;
        ...
        rawBox = el('div', { class: 'wk-rawbox' }, el('div', { class: 'wk-rawbox-bar' }, el('span', { class: 'wk-rawbox-label', text: 'MD 원문' }), backBtn), editableDoc ? rawIn : el('pre', { class: 'wk-rawbox-pre', text: k.body_md || '' }));
        body.after(rawBox);
    }
...
// ════════════════════════════════════════════
let wkPeekCur = null; // { back, name, pushed, baseHash, onPop, onKey, onRefresh }
let wkPeekNavGuard = false;
let wkPeekListCtx = null; // 마지막 목록의 문서 name 순서 — ↑↓ 순회용
let wkPeekOriginEl = null;
function setWikiPeekList(names) { wkPeekListCtx = names && names.length ? names.slice() : null; }
function consumeWikiPeekGuard() { const v = wkPeekNavGuard; wkPeekNavGuard = false; return v; }
function wkHashWithPeek(name) {
...
function reanchorWikiPeek() { if (wkPeekCur)
    wkPeekCur.baseHash = wkHashWithPeek(null); }
function dismissWikiPeek() {
    ...
    if (cur.onClose) {
        ...
        catch (_) { /* 갱신 실패는 무해 — 다음 repaint 가 바로잡음 */ }
    }
}
function openWikiPeek(name, opts) {
    ...
    if (matchMedia('(max-width: 1100px)').matches) {
        dismissWikiPeek();
        if (fromUrl)
            location.replace(location.href.split('#')[0] + '#/k/' + encodeURIComponent(name));
        else
            location.hash = '#/k/' + encodeURIComponent(name);
        return;
    }
    const prevPushed = wkPeekCur && opts && opts._traverse ? wkPeekCur.pushed : false;
    if (opts && opts.originEl)
        wkPeekOriginEl = opts.originEl;
    dismissWikiPeek(); // 패널은 항상 1개 — 열려 있으면 교체
    if (opts && opts.originEl)
        wkPeekOriginEl = opts.originEl; // dismiss 가 초기화한 것 복원
    ...
    const panel = el('aside', { class: 'wk-peek', role: 'dialog', 'aria-label': '문서 미리보기' }, el('div', { class: 'wk-peek-head' }, navHint, fullBtn, tabBtn, xBtn), body);
    const back = el('div', { class: 'wk-peek-backdrop' }, panel);
    const requestClose = () => {
        const cur = wkPeekCur;
        if (!cur || cur.back !== back)
            return;
        if (cur.pushed) {
            history.back();
            return;
    ...
    const onPop = () => {
        ...
        if (!cur || cur.back !== back)
            return;
        if (wkHashWithPeek(null) !== cur.baseHash) {
            dismissWikiPeek();
            return;
        } // 필터/경로까지 변함 — route 가 재렌더
        const h = location.hash.replace(/^#\/?/, '');
        const qIdx = h.indexOf('?');
    ...
    const onKey = (ev) => {
        if (ev.key === 'Escape') {
            if (document.querySelector('.qk-back'))
                return; // ⌘K 팔레트가 위에 떠 있으면 그쪽이 먼저 닫힌다
            requestClose();
            return;
        }
        ...
        if (t && typeof t.closest === 'function' && (t.closest('.be') || t.isContentEditable || /^(INPUT|TEXTAREA|SELECT)$/.test(t.tagName)))
            return;
        if (traverse(ev.key === 'ArrowDown' ? 1 : -1)) {
            ev.preventDefault();
            ev.stopPropagation();
        }
    };
    back.addEventListener('mousedown', (ev) => { if (ev.target === back)
        requestClose(); });
    ...
    if (!fromUrl && !(opts && opts._traverse)) {
        history.pushState(null, '', wkHashWithPeek(name));
        wkPeekCur.pushed = true;
    }
    window.addEventListener('popstate', onPop);
    document.addEventListener('keydown', onKey, true);
    buildWikiDoc(body, name, { mode: 'peek', onDeleted: () => { requestClose(); if (onRefresh)
            onRefresh(); } });
}
// ════════════════════════════════════════════
...
function openWikiSearch() {
    ...
    const input = el('input', { class: 'qk-input', type: 'search',
        placeholder: '검색어 입력 — 자연어 의미검색', 'aria-label': '빠른 검색' });
    const list = el('div', { class: 'qk-results' }, el('div', { class: 'qk-hint', text: '전체 지식에서 의미로 찾습니다. ↑↓ 이동 · Enter 미리보기 · ⌘Enter 페이지로' }));
    const newRow = el('button', { class: 'qk-newrow', type: 'button', hidden: true });
    const panel = el('div', { class: 'qk-panel', role: 'dialog', 'aria-label': '빠른 검색' }, el('div', { class: 'qk-inrow' }, el('span', { class: 'qk-glass', 'aria-hidden': 'true', text: '🔍' }), input, el('span', { class: 'qk-esc', text: 'esc' })), list, newRow);
    ...
    function paint() {
        ...
        if (q)
            newRow.replaceChildren(el('span', { class: 'qk-newrow-plus', 'aria-hidden': 'true', text: '＋' }), el('span', { text: '"' + q + '" 제목으로 새 페이지' }));
        if (!entries.length) {
        ...
        list.replaceChildren(...entries.map((e, i) => {
            ...
            const row = el('button', { class: 'qk-row' + (i === sel ? ' on' : ''), type: 'button' }, wkTick(e), el('span', { class: 'qk-row-ic', text: knPageIcon(e) }), el('div', { class: 'qk-row-main' }, el('div', { class: 'qk-row-title', title: e.title || '', text: (e.short_title || '').trim() || e.title || e.name }), snip ? el('div', { class: 'qk-row-snip', text: snip }) : null));
            row.addEventListener('mousedown', (ev) => ev.preventDefault());
            row.onclick = (ev) => openEntry(e, ev.metaKey || ev.ctrlKey);
            return row;
        }));
    ...
    input.addEventListener('input', () => {
        ...
        t = setTimeout(async () => {
            ...
            try {
                ...
                if (seq !== qkSeq || closed)
                    return; // 그 사이 검색어가 바뀜/닫힘 — 이 결과는 폐기
    ...
    const onKey = (e) => {
        ...
        if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
            e.preventDefault();
            if (entries.length) {
                sel = (sel + (e.key === 'ArrowDown' ? 1 : entries.length - 1)) % entries.length;
                paint();
            }
            return;
        }
        if (e.key === 'Enter') {
            e.preventDefault();
            openEntry(entries[sel], e.metaKey || e.ctrlKey);
        }
    };
    back.addEventListener('mousedown', (ev) => { if (ev.target === back)
        close(); });
...
document.addEventListener('keydown', (e) => {
    if (!(e.metaKey || e.ctrlKey) || (e.key !== 'k' && e.key !== 'K'))
        return;
    const page = location.hash.replace(/^#\//, '').split(/[/?]/)[0] || '';
    if (!['knowledge', 'k', 'trash'].includes(page))
        return;
    e.preventDefault();
    openWikiSearch();
});
...
async function renderWikiDraft(view, params) {
    ...
    const decor = { icon: null, cover: null }; // 생성 전 스테이징 — 생성 직후 props-ui 로 반영
    let saving = false;
    let timer = null;
    ...
    function clearDraft() { try {
        localStorage.removeItem(draftKey);
    }
    catch (_) { /* 무시 */ } }
    function mirrorDraft() {
        if (created)
            return; // 생성됨 = 서버가 진실, 로컬 미러 불필요
        ...
        try {
            localStorage.setItem(draftKey, JSON.stringify({ title, body_md, catKey, typeVal, ts: Date.now() }));
        }
        catch (_) { /* 용량 초과 등 무시 */ }
    }
    function keepaliveSaveCreated() {
        if (!created)
            return;
        const title = (titleEl.textContent || '').trim();
        const payload = { name: created.name, title: title || created.title, body_md: editor.getMarkdown().trim() || HOME_EMPTY };
        if (typeVal)
            payload.type = typeVal;
        if (catKey)
            payload.category = catKey;
        try {
            api('/api/ui/knowledge', { method: 'POST', body: JSON.stringify(payload), keepalive: true });
        }
        catch (_) { /* 무시 */ }
    }
    const saveChip = el('span', { class: 'wk-save-chip', 'aria-live': 'polite' });
    ...
    const crumbs = el('nav', { class: 'wk-folio-crumbs' }, el('a', { class: 'wk-crumb', href: '#/knowledge', text: 'WIKI' }), el('span', { class: 'wk-crumb-sep', 'aria-hidden': 'true', text: '›' }), el('span', { class: 'wk-crumb wk-crumb-cur', text: '새 페이지' }));
    // 제목.
    ...
    function paintDecor() {
        ...
        if (decor.cover)
            applyCoverBg(coverEl, decor.cover);
        coverEl.replaceChildren(...[decor.cover ? el('div', { class: 'wk-cover-btns' }, el('button', { class: 'wk-cover-btn', type: 'button', text: '커버 변경',
                onclick: (e) => openCoverPicker(e.target, { current: decor.cover, onPick: (v) => { decor.cover = v; paintDecor(); syncDecor(); } }) }), el('button', { class: 'wk-cover-btn', type: 'button', text: '제거', onclick: () => { decor.cover = null; paintDecor(); syncDecor(); } })) : null].filter(Boolean));
        ...
        decorAdd.replaceChildren(...[
            !decor.icon ? el('button', { class: 'wk-decor-btn', type: 'button', text: '☺ 아이콘 추가',
                onclick: (e) => openEmojiPicker(e.target, { onPick: (em) => { decor.icon = em; paintDecor(); syncDecor(); } }) }) : null,
            !decor.cover ? el('button', { class: 'wk-decor-btn', type: 'button', text: '🖼 커버 추가',
                onclick: (e) => openCoverPicker(e.target, { onPick: (v) => { decor.cover = v; paintDecor(); syncDecor(); } }) }) : null,
        ].filter(Boolean));
    ...
    async function syncDecor() {
        if (!created)
            return;
        try {
            await api('/api/ui/knowledge/' + encodeURIComponent(created.name) + '/props-ui', { method: 'POST', body: JSON.stringify(decor) });
        }
    ...
    function paintPropline() {
        ...
        catBtn.onclick = () => openKnMetaPicker(catBtn, 'category', { categories: catKey ? [{ key: catKey, state: 'confirmed' }] : [] }, async (_f, v) => { catKey = v; catName = ''; await resolveCatName(); paintPropline(); queue(); });
        ...
        typeBtn.onclick = () => openKnMetaPicker(typeBtn, 'type', { type: typeVal || 'reference' }, async (_f, v) => { typeVal = v; paintPropline(); queue(); });
        const hint = created
    ...
    const editor = wkTrackEditor(createBlockEditor({
        ...
        keepaliveSaveCreated();
    else
        mirrorDraft(); }); // 언로드 flush — 생성 후 keepalive, 생성 전 로컬 미러
    ...
    const queue = () => {
        ...
        if (!draftFirstEditAt)
            draftFirstEditAt = Date.now();
        if (created)
            setChip('저장 중…', true);
        else
            setChip(editor && editor.getMarkdown().trim() ? '초안 보관 중' : '', false); // 생성 전 — 로컬에 안전 보관 중임을 명시
        clearTimeout(timer);
        const waited = Date.now() - draftFirstEditAt;
        if (created && waited >= 4000) {
            commit();
            return;
        } // maxWait — 연속 타이핑 중 강제 저장
    ...
    async function commit() {
        ...
        if (!created) {
            if (!canvas.isConnected)
                return; // 생성 전에 페이지를 떠남 — 지연 타이머가 유령 문서를 만들지 않게
            if (!title || !catKey)
                return; // 제목+분류가 서야 생성 — 유령 문서 0
            saving = true;
            setChip('만드는 중…', true);
            try {
                const payload = { title, body_md: editor.getMarkdown().trim() || HOME_EMPTY, category: catKey, type: typeVal || 'reference', provenance: 'authored' };
                if (preFolder)
                    payload.parent_name = preFolder;
                const r = await api('/api/ui/knowledge', { method: 'POST', body: JSON.stringify(payload) });
                created = (r && r.knowledge) || null;
                if (!created)
                    throw new Error('생성 응답이 비었습니다');
                knInvalidateTreeCaches();
                editor.resetDirty();
                ...
                crumbs.replaceChildren(el('a', { class: 'wk-crumb', href: '#/knowledge', text: 'WIKI' }), el('span', { class: 'wk-crumb-sep', 'aria-hidden': 'true', text: '›' }), el('a', { class: 'wk-crumb', href: '#/knowledge?category=' + encodeURIComponent(String((created.categories && created.categories[0] && created.categories[0].category_id) || '')), text: catName || catKey }), el('span', { class: 'wk-crumb-sep', 'aria-hidden': 'true', text: '›' }), el('span', { class: 'wk-crumb wk-crumb-cur', text: created.title || created.name }));
                canvas.classList.remove('wk-doc-draft');
                if (decor.icon || decor.cover)
                    syncDecor();
                if (preProject) {
                    api('/api/ui/v6/projects/' + encodeURIComponent(preProject) + '/knowledge', { method: 'POST', body: JSON.stringify({ name: created.name, relation: preRelation }) })
                        .then(() => toast('프로젝트에 연결했습니다')).catch(() => { });
                }
            ...
            saving = false;
            if (created && (editor.isDirty() || (titleEl.textContent || '').trim() !== created.title))
                queue();
            return;
        }
    ...
    if (savedDraft && (Date.now() - (savedDraft.ts || 0) < 7 * 864e5) && (savedDraft.title || (savedDraft.body_md || '').trim())) {
        if (savedDraft.title && !(titleEl.textContent || '').trim())
            titleEl.textContent = savedDraft.title;
        if (savedDraft.body_md)
            editor.setMarkdown(savedDraft.body_md);
        if (savedDraft.catKey && !catKey)
            catKey = savedDraft.catKey;
        if (savedDraft.typeVal && !typeVal)
            typeVal = savedDraft.typeVal;
        toast('이전에 작성 중이던 초안을 불러왔어요');
        setTimeout(() => queue(), 0); // 제목+분류가 서 있으면 곧 생성/저장
    }
    canvas.replaceChildren(coverEl, el('div', { class: 'wk-folio' }, crumbs, el('div', { class: 'wk-folio-actions' }, saveChip)), el('div', { class: 'wk-doc-head' }, iconBtn, decorAdd, titleEl), propline, el('div', { class: 'wk-doc-body' }, editor.el));
    await resolveCatName();
    paintPropline();
    setTimeout(() => titleEl.focus(), 0);
}
export { buildWikiDoc, consumeWikiPeekGuard, dismissWikiPeek, openWikiPeek, openWikiSearch, reanchorWikiPeek, renderWikiDocPage, renderWikiDraft, setWikiPeekList, };
