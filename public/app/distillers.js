...
export async function distillersPanel(detail, data) {
    ...
    if (creatingNew)
        body.append(editorCard(null, rerender));
    else {
...
function summaryCard(d, st, rerender) {
    ...
    card.append(el('div', { class: 'mini-title' }, el('span', { text: d.label || d.key }), el('span', { class: 'pill' + (d.enabled ? ' pill-ok' : ''), text: d.enabled ? '켜짐' : '꺼짐' }), el('span', { class: 'pill', text: '우선순위 ' + d.priority }), el('span', { class: 'pill', text: '남은 자료 ' + Number(st.backlog || 0).toLocaleString() + '건' }), st.reviewed ? el('span', { class: 'pill', text: '판정 ' + Number(st.reviewed).toLocaleString() }) : null, d.prefilter_level ? el('span', { class: 'pill', text: '사전필터 ' + d.prefilter_level }) : null));
    ...
    del.addEventListener('click', async () => {
        if (!confirm(`증류기 '${d.label || d.key}' 를 삭제할까요? (이미 만들어진 지식은 그대로 남습니다)`))
            return;
        try {
...
function editorCard(d, rerender) {
    ...
    const incCh = el('textarea', { style: S + ';min-height:70px;resize:vertical', placeholder: '한 줄에 채널 하나. 비우면 채널 무관(전체)' });
    incCh.value = listText(v('include_channels', null));
    const excCh = el('textarea', { style: S + ';min-height:52px;resize:vertical', placeholder: '제외할 채널(한 줄에 하나)' });
    ...
    const chPick = el('div', { style: 'display:flex;flex-wrap:wrap;gap:6px;max-height:150px;overflow:auto;margin-top:8px' }, el('span', { class: 'admin-hint', text: '채널 목록 불러오는 중…' }));
    void (async () => {
        try {
    ...
    const rIn = (k, ph) => {
        const i = el('input', { type: 'number', style: S, placeholder: ph });
        if (rules[k] != null)
    ...
    const rMatch = el('select', { style: S });
    ...
    const typeSel = el('select', { style: S });
    ...
    const modeSel = el('select', { style: S });
...
async function runJobCard(rerender) {
    ...
    card.append(el('p', { class: 'admin-hint' }, el('b', { text: '증류 스케줄 잡이 없습니다 — 증류기를 만들어도 아무것도 돌지 않습니다.' }), el('span', { text: ' 증류기는 "무엇을 어떻게"만 정하고, 실제로 자료를 집어 AI에게 넘기는 건 스케줄 잡입니다.' })));
    ...
    mk.addEventListener('click', async () => {
        ...
        try {
            ...
            toast('증류 잡을 만들었습니다 — 증류기를 설정한 뒤 켜세요');
            rerender();
        }
        catch (e) {
            toast('실패 — ' + e.message, true);
