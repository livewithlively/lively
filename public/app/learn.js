...
async function renderLearnDocs(view, slug) {
    ...
    if (slug === 'domainmap') {
        location.replace('#/learn/docs/context');
        return;
    }
...
async function renderLearn(view) {
    ...
    docsShell(view, 'overview', docsEyebrow('overview'), el('h1', { class: 'docs-title', text: page ? page.title : '라이블리 개요' }), docsBody(page ? page.md : '', heroCard())); // 히어로 = 같은 카드 흐름의 첫 장(간격 통일)
}
