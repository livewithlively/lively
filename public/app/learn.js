...
const GUIDE_KEY_GROUP = { 'start-project': 'begin', tour: 'begin', install: 'begin' };
function guideGroupOf(active) {
...
function guideHeader(active) {
    const grp = guideGroupOf(active);
    const crumbBar = el('div', { class: 'pjv-crumbbar' }, el('nav', { class: 'pjv-crumbs', 'aria-label': '현재 위치' }, el('span', { class: 'pjv-crumb is-leaf lg-crumb-leaf' }, tabIcon('book-open', 'pjv-crumb-ic lg-crumb-ic'), el('span', { class: 'pjv-crumb-label', text: '사용 가이드' })), el('span', { class: 'lg-crumb-sub', text: grp.hint })));
    const tabs = el('div', { class: 'pjv-vtabs lg-vtabs', role: 'tablist', 'aria-label': '문서 묶음' });
    for (const g of GUIDE_NAV) {
        const on = g.key === grp.key;
        tabs.append(el('a', { class: 'pjv-vtab lg-vtab' + (on ? ' active' : ''), href: g.items[0].href, role: 'tab', 'aria-selected': String(on) }, el('span', { class: 'lg-vtab-label', text: g.group })));
    }
    const left = el('div', { class: 'pjv-tasks-head-left' });
    for (const it of grp.items) {
        const on = it.key === active;
        left.append(el('a', { class: 'pjv-tb-btn pjv-tb-pill lg-pill' + (on ? ' active' : ''), href: it.href, 'aria-current': on ? 'page' : null }, el('span', { class: 'pjv-view-btn-label', text: it.label })));
    }
    const right = el('div', { class: 'card-head-actions' }, el('a', { class: 'pjv-tb-btn lg-tb-open', href: location.href.split('#')[0] + (location.hash || '#/learn'), target: '_blank', rel: 'noopener', title: '새 브라우저 탭에서 열기' }, tabIcon('external', 'pjv-tb-ic')));
...
export function docsShell(view, active, ...content) {
    ...
    view.replaceChildren(el('div', { class: 'pjv-board-wrap lg-wrap' }, el('div', { class: 'card pjv-listboard lg-board' }, guideHeader(String(active || 'overview')), el('div', { class: 'lg-body' }, article))));
...
function docsBody(md, ...lead) {
    ...
    for (const s of secs) {
        ...
        if (!s.title) {
            ...
            if (lead)
                cards.append(el('div', { class: 'md-rendered docs-lead' }, renderMarkdown(lead, { uiChips: true })));
            if (rest)
                cards.append(el('div', { class: 'card docs-card' }, el('div', { class: 'md-rendered docs-md' }, docsDecorate(renderMarkdown(rest, { uiChips: true })))));
            continue;
        }
        cards.append(el('div', { class: 'card docs-card' }, el('div', { class: 'card-head' }, el('h2', { text: s.title })), el('div', { class: 'md-rendered docs-md' }, docsDecorate(renderMarkdown(body, { uiChips: true })))));
    }
    return cards;
}
...
async function renderLearnDocs(view, slug) {
    ...
    const LEGACY = { plan: '', domainmap: 'context', home: 'sessions', terminal: 'sessions', admin: 'settings', cli: 'commands', 'how-it-works': '' };
    if (slug in LEGACY) {
        location.replace(LEGACY[slug] ? '#/learn/docs/' + LEGACY[slug] : '#/learn');
        return;
    }
...
async function renderLearn(view) {
    ...
    docsShell(view, 'overview', docsEyebrow('overview'), el('h1', { class: 'docs-title', text: page ? page.title : '라이블리란' }), docsBody(md, heroCard())); // 히어로 = 같은 카드 흐름의 첫 장(간격 통일)
}
// ── 히어로 — 이 서비스가 통째로 뭔지(한 문장) + 하루의 순환 4단계. 개요 문서의 첫 카드. ──
//  #1841: 모노캡 아이브로('LIVELY CONTEXT') 폐지 — 장식이지 정보가 아니었다. 제목·리드·순환·핵심 한 줄만.
function heroCard() {
    return el('div', { class: 'card guide-hero lg-hero' }, el('h2', { class: 'guide-hero-title' }, '회사가 쓰는 AI 를 위한 ', el('span', { class: 'accent', text: '공용 맥락 저장소' }), '.'), el('p', { class: 'guide-hero-lead', text: 'AI 는 세상 지식은 많지만 우리 회사가 무슨 일을 하는지, 어떤 규칙이 있는지, 지금 무엇이 진행 중인지는 모릅니다. 라이블리는 그 배경을 한곳에 모아 두고 구성원이 AI 를 켤 때마다 자동으로 건네줍니다 — 그래서 누가 켜든 회사를 이미 아는 상태에서 일이 시작됩니다.' }), el('div', { class: 'guide-cycle' }, el('div', { class: 'guide-flow' }, flowStep('layers', '모아두기', '규칙·지식·진행 상황이 WIKI 와 프로젝트에 쌓입니다.'), flowArrow(), flowStep('send', '자동 전달', '세션을 켜면 그 내용이 AI 에게 먼저 들어갑니다.'), flowArrow(), flowStep('zap', '바로 일 시작', '배경 설명 없이 한 줄만 시키면 됩니다.'), flowArrow(), flowStep('save', '다시 쌓기', '결정·결과를 AI 가 지식으로 남깁니다.')), el('div', { class: 'guide-loop' }, el('span', { class: 'guide-loop-label' }, el('span', { class: 'guide-loop-key', 'aria-hidden': 'true', text: '↻' }), el('b', { text: '남긴 지식은 다시 모아두기로' }), el('span', { class: 'guide-loop-sub', text: '쓸수록 AI 가 받는 맥락이 정확해집니다' })))), el('div', { class: 'guide-remember' }, el('span', { class: 'guide-remember-key', text: '핵심 한 줄' }), el('p', { text: '어디서 켜든 같습니다 — 라이블리 웹이든 내 터미널이든, 같은 회사 맥락이 들어갑니다.' })));
}
// 작동 3단계 — 아이콘 + 제목 + 한 줄.
...
async function renderLearnTour(view) {
    ...
    const courses = courseDefs.length ? el('div', { class: 'card' }, el('div', { class: 'card-head' }, el('h2', { text: '섹션만 골라 보기' })), el('p', { class: 'guide-lead', text: '급하면 필요한 것만 봐도 돼요. 각 섹션은 따로 시작하고 따로 끝나요.' }), el('div', { class: 'guide-path' }, courseDefs.map((c, i) => courseRow(String(i + 1), c.key, c.title, c.desc)))) : null;
...
function obProgress(pct) {
    return el('div', { style: 'height:8px;background:var(--line-row);border-radius:4px;overflow:hidden;margin:10px 0' }, el('div', { style: `height:100%;width:${pct}%;background:var(--ok-2);transition:width .3s` }));
}
...
async function renderOnboarding(view) {
    const head = el('div', { class: 'page-head' }, el('h1', {}, '온보딩 ', el('span', { class: 'accent', text: '진행상황' })), el('p', { class: 'sub', text: '이 인스턴스 셋업이 어디까지 됐는지 한눈에 봅니다. 진행률은 필수 항목만으로 셉니다 — 「선택」은 해당하는 조직만 하면 됩니다.' }));
    const slot = el('div', {});
    ...
    try {
        ...
        s.items.forEach((it, i) => {
            ...
            steps.append(el('div', { class: 'card', style: 'display:flex;gap:12px;align-items:flex-start;margin-bottom:10px;opacity:' + (it.done || it.optional ? '0.65' : '1') }, el('div', { style: `flex:0 0 28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;color:var(--on-fill);background:${it.done ? 'var(--ok-2)' : it.optional ? 'var(--muted-3)' : 'var(--muted-2)'}`, text: it.done ? '✓' : it.optional ? '–' : String(i + 1) }), el('div', { style: 'flex:1;min-width:0' }, el('div', { style: 'font-weight:600' }, it.label, it.optional ? el('span', { class: 'admin-only-badge', style: 'margin-left:6px', text: '선택' }) : null, it.count !== undefined ? el('span', { class: 'admin-hint', text: ` · 현재 ${it.count}` }) : null), el('div', { class: 'admin-hint', style: 'margin:2px 0 0', text: it.how }), it.href ? el('a', { class: 'accent', href: it.href, text: it.done ? '보기 →' : '바로가기 →', style: 'display:inline-block;margin-top:6px;text-decoration:none' }) : null)));
        });
...
export { checklist, openInstallModal, overlayBox, renderInstall, renderLearn, renderLearnDocs, renderLearnTour, renderOnboarding, skeleton, skeletonRows, };
