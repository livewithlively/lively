...
const undoneIds = []; // 이 탭에서 이미 되돌린 원본 감사 id (서버 픽 제외 목록)
const redoIds = []; // 다시 실행 후보(LIFO) — 마지막으로 되돌린 원본 감사 id
let busy = false;
// 텍스트 편집 소유자 안에 있으면 전역 undo 를 비켜선다 — 네이티브 입력 undo / 블록에디터 자체 스택(#657z) / xterm.
function inEditableOwner(t) {
...
async function doUndo() {
    try {
        ...
        if (e.status === 404)
            toast('되돌릴 내 작업이 없습니다');
        else if (e.status !== 401)
            toast('실행 취소 실패 — ' + e.message, true);
    }
}
async function doRedo() {
    ...
    try {
        ...
        if (i >= 0)
            undoneIds.splice(i, 1); // 다시 실행됐으니 이후 Z 가 이 작업을 다시 취소할 수 있게
        if (r && r.redone) {
            toast('↪︎ 다시 실행 — ' + r.redone.label);
            rerenderRoute();
        }
    }
    catch (e) {
        if (e.status !== 401)
            toast('다시 실행 실패 — ' + e.message, true);
    }
}
