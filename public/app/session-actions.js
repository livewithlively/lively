...
function keepNote(fate, policy) {
    ...
    if (fate === 'some')
        return `${keep} 대화 기록은 수집이 켜진 AI(하네스) 세션만 📜 「세션 기록」에 남아 💬 「이어 질문하기」로 이어받을 수 있어요 — 나머지는 이 박스 안에만 남습니다.${days}`;
    if (fate === 'none')
...
export function trashConfirmSkipped() {
    try {
        return localStorage.getItem(TRASH_SKIP_KEY) === '1';
    }
    catch (_) {
        return false;
    } // 스토리지 차단 — 그냥 묻는다
}
export function setTrashConfirmSkipped(on) {
    try {
        if (on)
            localStorage.setItem(TRASH_SKIP_KEY, '1');
        else
            localStorage.removeItem(TRASH_SKIP_KEY);
    }
    catch (_) { /* noop */ }
}
...
export async function confirmSessionTrash(opts) {
    if (trashConfirmSkipped())
        return true;
    const n = opts.n || 1;
    const skip = el('input', { type: 'checkbox', id: 'sess-trash-skip' });
    const ok = await confirmDialog({
        ...
        extra: el('label', { class: 'ov-confirm-opt', for: 'sess-trash-skip' }, skip, el('span', { class: 'n', text: '다음부터 묻지 않고 바로 보내기' }), el('span', { class: 'h', text: '휴지통 화면에서 되돌릴 수 있어요' })),
    });
    if (ok && skip.checked)
        setTrashConfirmSkipped(true);
    return ok;
}
...
export async function confirmSessionPurgeLocal(opts) {
    return confirmDialog({
...
function splitFootprint(fps) {
    ...
    for (const fp of fps) {
        for (const k of fp.knowledge_created) {
            ...
            seen.add(key);
            if (k.touched_after)
                keep.push({ label: k.title || k.name, why: '이 세션이 끝난 뒤에 수정된 문서라 남겨요' });
            else
                kc.push({ key: k.name, label: k.title || k.name });
        }
        for (const k of fp.knowledge_edited) {
            ...
            seen.add(key);
            if (k.touched_after)
                keep.push({ label: k.title || k.name, why: '이 세션이 끝난 뒤에 또 수정된 문서라 되돌리지 않아요' });
            else
                ke.push({ key: k.name, label: k.title || k.name });
        }
        for (const p of fp.projects) {
            ...
            seen.add(key);
            if (!p.created_here)
                continue; // 이 세션이 만들지 않은 프로젝트는 애초에 대상이 아니다(붙어만 있던 것)
            if (p.other_sessions)
                keep.push({ label: `#${p.id} ${p.name}`, why: `다른 세션 ${p.other_sessions}개가 붙어 있는 프로젝트라 남겨요` });
            else if (p.touched_after)
                keep.push({ label: `#${p.id} ${p.name}`, why: '이 세션이 끝난 뒤에 수정된 프로젝트라 남겨요' });
            else
                pj.push({ key: String(p.id), label: `#${p.id} ${p.name}` });
        }
    ...
    const groups = [];
    if (kc.length)
        groups.push({ kind: 'kc', title: `만든 지식 ${kc.length}건 지우기`, items: kc, count: kc.length });
    if (ke.length)
        groups.push({ kind: 'ke', title: `고친 지식 ${ke.length}건 이 세션 전 내용으로 되돌리기`, items: ke, count: ke.length });
    if (pj.length)
        groups.push({ kind: 'pj', title: `만든 프로젝트 ${pj.length}건 지우기`, hint: '그 안의 작업·첨부·폴더도 함께', items: pj, count: pj.length });
    // 자료는 세션이 '만든' 것이 아니다 — 올렸거나 수집된 원본이 이 세션의 지식에 붙어 있는 것이다(원준 지적). 붙어 있는 관계로 말한다.
    if (src.length)
        groups.push({ kind: 'src', title: `원본 자료 ${src.length}건 지우기`, hint: '위에서 지우는 지식에만 붙어 있는 자료예요', items: src, count: src.length });
    if (acts)
        groups.push({ kind: 'ac', title: `작업 기록 ${acts}건 지우기`, items: [], count: acts });
    return { groups, keep };
}
...
async function purgeDialog(opts) {
    ...
    if (opts.groups.length) {
        rows.push(el('p', { class: 'sess-purge-h' }, el('span', { text: '함께 지울 것' }), el('span', { class: 'sub', text: '풀면 그것만 남겨요' })));
        for (const g of opts.groups) {
            const r = checkRow('pg-' + g.kind, g.title, g.items.map((i) => i.label), g.hint);
            r.box.checked = true; // 기본 체크(원준 결정 ②) — "버린다 = 결과도 버린다"
            rows.push(r.el);
            boxes.push({ kind: g.kind, box: r.box });
        }
        ...
        only.addEventListener('click', () => { for (const b of boxes) {
            b.box.checked = false;
            b.box.dispatchEvent(new Event('change'));
        } });
        rows.push(only);
    }
        ...
        for (const g of opts.groups) {
            ...
            if (g.items.length)
                for (const it of g.items)
                    list.push({ label: it.label, why: g.kind === 'ke' ? '체크를 풀어서 이 세션이 고친 내용 그대로 남겨요' : '체크를 풀어서 남겨요' });
            else
                list.push({ label: g.title.replace(/ 지우기$/, ''), why: '체크를 풀어서 남겨요' });
        }
        keepBox.replaceChildren();
        if (!list.length)
            return;
        keepBox.append(el('p', { class: 'sess-purge-h' }, el('span', { text: `남는 것 · ${list.length}` })));
        for (const k of list.slice(0, 8))
            keepBox.append(el('p', { class: 'sess-purge-keep' }, el('span', { class: 'n', text: k.label }), el('span', { class: 'w', text: k.why })));
        if (list.length > 8)
            keepBox.append(el('p', { class: 'sess-purge-keep' }, el('span', { class: 'w', text: `외 ${list.length - 8}건` })));
    };
    for (const b of boxes)
        b.box.addEventListener('change', paintKeep);
    rows.push(keepBox);
    paintKeep();
    const ok = await confirmDialog({
        title: opts.title, danger: true, confirmText: '완전 삭제', cancelText: '취소',
...
function keptLines(opts) {
    ...
    if (opts.live)
        kept.push('이 세션은 계속 쓸 수 있지만, 앞으로의 대화도 중앙 기록에 남지 않아요.');
    return kept;
}
...
export async function confirmSessionPurgeMany(opts) {
    ...
    const { groups, keep } = splitFootprint([...fps.values()].filter((x) => !!x));
    const on = await purgeDialog({
        title: opts.title,
    ...
    for (const s of opts.sessions)
        if (s.sid)
            out.set(s.sid, choiceOf(fps.get(s.sid) ?? null, on));
    return out;
}
...
export function purgedToast(r) {
    ...
    if (r.projects_deleted)
        parts.push(`프로젝트 ${r.projects_deleted}건 삭제` + (r.folders_deleted ? ` (폴더 ${r.folders_deleted}개 포함)` : ''));
    if (r.sources_deleted)
        parts.push(`자료 ${r.sources_deleted}건 삭제`);
