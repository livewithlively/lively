...
function loginBannerEl(cfg, view) {
    ...
    if (os.ready) {
        ...
        return el('div', { class: 'card' }, el('button', { class: 'btn btn-primary btn-sm', text: '🔑 내 계정 로그인', onclick: () => openLoginSession(view) }), el('span', { class: 'caption', text: '  처음 한 번 — 개인 세션을 열어 claude 에서 /login (codex 는 codex 세션에서 codex login --device-auth). 이후 내가 만드는 세션은 내 격리 계정으로 뜹니다.' }));
    }
