...
function openNodeManager(view) {
    ...
    async function load() {
        ...
        for (const n of nodes) {
            ...
            const acts = el('div', { style: 'display:flex;gap:6px;margin-left:auto' }, ...(canShare ? [el('button', { class: 'btn btn-ghost btn-sm', text: n.shared ? '공유 해제' : '공유 노드로',
                    title: n.shared
                        ? '이 노드를 다시 등록자 전용으로 되돌립니다(진행 중인 남의 작업은 그대로 끝납니다).'
                        : '조직 전체가 이 노드에 위탁·세션을 열 수 있게 합니다. 개인 PC라면 남이 그 컴퓨터에서 코드를 실행하게 되는 것이니 주의하세요.',
                    onclick: () => share(n) })] : []), el('button', { class: 'btn btn-ghost btn-sm', text: n.enabled === false ? '활성화' : '비활성',
            ...
            ...(n.shared ? [document.createTextNode(' '), el('span', { class: 'tsess-badge', text: '🌐 공유' })] : [])), el('div', { class: 'caption', text: n.id })), badge, acts));
        }
