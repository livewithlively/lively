...
function tsessProjView() { try {
    return localStorage.getItem(TSESS_PROJVIEW_KEY) === 'recent' ? 'recent' : 'list';
}
catch {
    return 'list';
} }
function saveTsessProjView(v) { try {
    if (v === 'recent')
        localStorage.setItem(TSESS_PROJVIEW_KEY, 'recent');
    else
        localStorage.removeItem(TSESS_PROJVIEW_KEY);
}
catch { /* noop */ } }
function buildSessProjFilter(opts) {
    ...
    for (const id of idSet)
        if (!rows.some((p) => Number(p.id) === Number(id)))
            rows.push({ id, name: projName.get(id) || ('프로젝트 #' + id), list_id: 0, updated_at: null });
    ...
    const mkNode = (key, kind, name, sort) => ({ key, kind, name, sort, children: [], projects: [], count: 0 });
    const folderNode = new Map();
    const listNode = new Map();
    const roots = [];
    const orphanProjects = []; // 리스트 없는 프로젝트 = 최상위에 그대로 노출
    const ensureFolder = (fid) => {
    ...
    const countUp = (n) => { n.count = n.projects.length + n.children.reduce((a, c) => a + countUp(c), 0); return n.count; };
    for (const r of roots)
        countUp(r);
    const prune = (ns) => ns.filter((n) => n.count > 0).map((n) => ({ ...n, children: prune(n.children) }));
    const tree = prune(roots).sort(bySort);
    const hadSaved = (() => { try {
        return !!localStorage.getItem('lively_term_projtree_v1');
    }
    catch {
        return false;
    } })();
    ...
    const saveExp = (e) => { try {
        if (e.size)
            localStorage.setItem(EXP_KEY, JSON.stringify([...e]));
        else
    ...
    const seedExpanded = (ns) => { for (const n of ns)
        if (n.kind === 'folder')
            expanded.add(n.key); };
    if (!hadSaved)
        seedExpanded(tree);
    let view = tsessProjView();
    const nameOf = (id) => { const p = rows.find((x) => Number(x.id) === id); return (p && p.name) || projName.get(id) || ('프로젝트 #' + id); };
    const wrap = el('div', { class: 'tsess-projfilter' });
    const btn = el('button', { type: 'button', class: 'tsess-projfilter-btn' + (sel.size ? ' active' : ''), title: '프로젝트로 필터 — 여러 개 고를 수 있어요' }, el('span', { text: sel.size === 0 ? '프로젝트' : sel.size === 1 ? nameOf([...sel][0]) : '프로젝트 ' + sel.size }), el('span', { class: 'tsess-projfilter-chev', text: '▾' }));
    ...
    const projBtn = (p, depth, meta) => {
        ...
        return el('button', {
            ...
            onmousedown: (e) => {
                ...
                e.stopPropagation();
                if (on)
                    sel.delete(Number(p.id));
                else
                    sel.add(Number(p.id));
                onChange(new Set(sel));
            },
        }, el('span', { class: 'dash-pop-box', text: on ? '✓' : '' }), el('span', { class: 'tsess-opt-name', text: p.name }), meta ? el('span', { class: 'tsess-opt-meta', text: meta }) : null);
    };
    const renderList = (q) => {
        const ql = (q || '').trim().toLowerCase();
        const hit = (p) => !ql || String(p.name || '').toLowerCase().includes(ql);
        // 상단 — 선택이 있으면 '몇 개 골랐는지 + 초기화'(체크를 하나씩 끄지 않고 한 번에 전체로 되돌린다).
        const nodes = [sel.size
                ? el('div', { class: 'tsess-projfilter-head' }, el('span', { class: 'tsess-projfilter-headn', text: sel.size + '개 선택' }), el('button', {
                    class: 'dash-pop-clear', type: 'button', title: '프로젝트 조건을 지우고 전체 보기',
                    onmousedown: (e) => { e.preventDefault(); e.stopPropagation(); sel.clear(); onChange(new Set()); },
                }, '초기화'))
        ...
        if (view === 'recent') {
            ...
            if (!flat.length)
                nodes.push(el('div', { class: 'tsess-projfilter-none', text: '일치하는 프로젝트가 없어요' }));
            for (const p of flat)
                nodes.push(projBtn(p, 0, (listById.get(String(p.list_id)) || {}).name || ''));
        ...
        const walk = (ns, depth) => {
            for (const n of ns.slice().sort(bySort)) {
                ...
                const deepCount = (function cnt(x) { return x.projects.filter(hit).length + x.children.reduce((a, c) => a + cnt(c), 0); })(n);
                if (ql && !deepCount)
                ...
                const row = el('button', {
                    ...
                    onmousedown: (e) => {
                        e.preventDefault();
                        e.stopPropagation(); // 가지를 여닫아도 드롭다운은 닫지 않는다
                ...
                }, el('span', { class: 'tsess-tree-caret', text: open ? '▾' : '▸' }), el('span', { class: 'tsess-tree-name', text: n.name }), el('span', { class: 'tsess-tree-n', text: String(deepCount) }));
                nodes.push(row);
                shown++;
                if (open) {
                    walk(kids, depth + 1);
                    for (const p of mine) {
                        nodes.push(projBtn(p, depth + 1));
                        shown++;
                    }
                }
            }
        ...
        for (const p of orphans) {
            nodes.push(projBtn(p, 0));
            shown++;
        }
        if (!shown)
            nodes.push(el('div', { class: 'tsess-projfilter-none', text: ql ? '일치하는 프로젝트가 없어요' : '고를 프로젝트가 없어요' }));
        listBox.replaceChildren(...nodes);
    };
    for (const [k, label, hint] of [['list', '폴더별', '사이드바처럼 폴더·리스트를 눌러 안으로 들어갑니다'], ['recent', '최신순', '폴더를 무시하고 최근에 갱신된 순서로 봅니다']]) {
        ...
        b.onmousedown = (e) => {
            ...
            if (view === k)
                return;
            view = k;
            saveTsessProjView(view);
            for (const x of [...seg.children])
                x.classList.toggle('on', x.textContent === (view === 'list' ? '폴더별' : '최신순'));
            renderList(search.value);
        };
        seg.append(b);
    }
