...
async function renderTerminal(view) {
    ...
    function draw(reopen = '') {
        ...
        const selBtn = iconBtn('tsess-sel-btn' + (sel.mode ? ' active' : ''), sel.mode ? '선택 모드 끝내기' : '선택 — 여러 세션을 골라 한 탭 그리드로 열거나 한 번에 종료', pjvTbIcon('check'), () => { if (sel.mode) {
            sel.mode = false;
            sel.ids.clear();
        }
        else
            sel.mode = true; draw(); });
        if (!sessions.length)
            selBtn.setAttribute('aria-disabled', 'true');
        ...
        if (!sessions.length) {
            sel.ids.clear();
            listWrap.replaceChildren(el('div', { class: 'pjv-empty-hint', text: '아직 세션이 없습니다. [+ 새 세션]으로 만드세요.' }));
            repaintBulk();
            return;
        }
        if (!shown.length) {
            sel.ids.clear();
            listWrap.replaceChildren(el('div', { class: 'pjv-empty-hint', text: '이 필터에 해당하는 세션이 없습니다.' }));
            repaintBulk();
            return;
        }
        ...
        const rendered = []; // 실제로 그려진 행 = 벌크바의 선택 후보(접힌 섹션은 빠진다 — #1150)
        for (const sect of TSESS_SECTIONS) {
            ...
            caret.onclick = (e) => { e.stopPropagation(); if (off)
                collapsed.delete(sect.key);
            else
                collapsed.add(sect.key); saveTsessCollapsed(collapsed); draw(); };
            const head = el('div', { class: 'pjv-tgroup-head tsess-sect-head', title: sect.hint }, el('span', { class: 'pjv-row-check-spacer', 'aria-hidden': 'true' }), caret, el('span', { class: 'pjv-tgroup-label', text: sect.label }), el('span', { class: 'pjv-tgroup-count', text: String(items.length) }));
            head.onclick = () => caret.click();
            const body = el('div', { class: 'pjv-tgroup-body tsess-list' + (sel.mode ? ' selectmode' : '') + (sel.ids.size ? ' has-sel' : '') });
            if (!off) {
                body.append(tsessColHead());
                for (const s of items) {
                    body.append(tsessRow(s, ctx));
                    rendered.push(s);
                }
                sel.listEls.push(body);
            }
    ...
    const crumbBar = el('div', { class: 'pjv-crumbbar' }, el('nav', { class: 'pjv-crumbs', 'aria-label': '현재 위치' }, el('span', { class: 'pjv-crumb is-leaf tsess-crumb' }, tsessNodeIcon('pjv-crumb-ic'), el('span', { class: 'pjv-crumb-label', text: 'AI 세션' })), el('span', { class: 'tsess-crumb-sub', text: '박스와 노드에서 도는 AI 세션 전체' })));
    const viewTabs = el('div', { class: 'pjv-vtabs', role: 'tablist', 'aria-label': '뷰' }, el('button', { class: 'pjv-vtab active', type: 'button', role: 'tab', 'aria-selected': 'true' }, pjvTabIcon('list'), el('span', { text: '리스트' })));
...
function startTerminalTour(firstStep, opts) {
    startTour([
        ...
        },
        {
            ...
            target: () => {
                const t = document.querySelector('[data-tour="preset"]');
                if (!t)
                    return null;
                const body = t.parentElement?.querySelector('.term-adv-body');
