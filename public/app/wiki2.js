    ...
    async function paintBoard(box) {
        ...
        const zoneCard = (c, opts) => {
            ...
            for (const r of rows) {
                ...
                fu.append(el('a', { class: 'wk2-fur', href: '#/k/' + encodeURIComponent(r.name), target: '_blank', rel: 'noopener', title: r.title || r.name }, wkTick({ confidence: r.confidence === 'human' ? 'human' : null, provenance: r.provenance === 'observed' ? 'observed' : null }), el('span', { class: 'tx' }, el('b', { text: r.title || r.name }), el('span', { text: ' — ' + L.sum }), L.who ? el('span', { class: 'who', text: ' · ' + L.who }) : null), el('span', { class: 'm', text: relTime(r.activity_at || r.updated_at) })));
            }
            ...
            topLink.addEventListener('keydown', (ev) => { if (ev.key === 'Enter' || ev.key === ' ') {
                ev.preventDefault();
                drill();
            } });
