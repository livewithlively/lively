...
export async function renderWiki2(view, sub, params) {
    ...
    async function paintVerify(box) {
        ...
        const paint = () => {
            ...
            if (!ord.length) {
                kids.push(el('div', { class: 'wk2-empty' }, el('div', { class: 'big' }, el('span', { class: 'wk2-okdot' }), el('span', { text: '검증할 것이 없습니다' })), el('div', {}, el('span', { text: '최근 변화는 ' }), (() => { const a = el('a', { href: '#', text: '기록' }); a.onclick = (e) => { e.preventDefault(); syncHash('history'); void renderWiki2(view, 'history', new URLSearchParams('cat=' + f.cat)); }; return a; })(), el('span', { text: '에서 볼 수 있습니다.' }))));
            }
            else {
                if (waiting.length) {
