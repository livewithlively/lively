...
export async function refreshWiki2NavBadge(n) {
    ...
    if (total == null) {
        ...
        try {
            ...
            total = 0;
        } // memory scope 없음(403) 등 — 배지 없이 조용히
    }
    else
        wk2BadgeAt = Date.now();
...
async function fillCard(box, it, drop) {
    try {
        ...
        if (it.kind === 'edit' && detail) {
            ...
            nl.append(el('div', { class: 'wk2-sum' }, el('span', { text: rev.note || structSummary(blocks, it.added, it.removed) }), el('span', { class: 'lbl', text: rev.note ? '제안 요약' : '구조 요약(요약 미제출)' })));
            if (it.conflict) {
            ...
            const renderBlock = (b) => {
                ...
                if (b.op === '바뀜') {
                    kids.push(el('div', { class: 'wk2-ba' }, el('span', { class: 'lab', text: '이전' }), el('span', { class: 'prev' }, ws ? ws.prev : mdClip(b.before.join('\n'))), el('span', { class: 'lab', text: '이후' }), el('span', { class: 'next' }, ws ? ws.next : mdClip(b.after.join('\n')))));
                }
                else if (b.op === '추가') {
                    kids.push(mdClip(b.after.join('\n')));
                }
                else {
                    kids.push(el('div', { class: 'wk2-ba' }, el('span', { class: 'lab', text: '삭제' }), el('span', { class: 'prev' }, mdClip(b.before.join('\n')))));
                }
            ...
            nl.append(el('div', { class: 'wk2-sum' }, el('span', { text: it.note || (k.summary || '새 지식 제안') }), el('span', { class: 'lbl', text: '신규 제안' })));
            try {
            ...
            nl.append(el('div', { class: 'wk2-chg' }, el('div', { class: 'wk2-sect' }, el('span', { class: 'wk2-op', text: '추가' }), el('span', { text: '전체 본문' })), mdClip(k.body_md || '(본문 없음)')));
        }
        ...
        if (srcs.length) {
            srcs.slice(0, 3).forEach((s, i) => {
                if (i)
                    srcSpan.append(el('span', { text: ' · ' }));
                srcSpan.append(el('span', { text: ' ' + (s.title || s.name || s.kind || '자료') + ' ' }), el('span', { class: 'm', text: s.occurred_at ? relTime(s.occurred_at) : '' }));
            });
            if (srcs.length > 3)
                srcSpan.append(el('span', { class: 'm', text: ` 외 ${srcs.length - 3}` }));
        }
        else
            srcSpan.append(el('span', { text: ' 연결된 출처 없음', style: 'color:var(--muted-2)' }));
        if (k.external_url)
            srcSpan.append(el('span', { text: ' · ' }), el('a', { href: String(k.external_url), target: '_blank', rel: 'noopener', text: '원본 열기 ↗' }));
        evi.append(srcSpan);
        const links = (k.links || {});
        const backN = Array.isArray(links.incoming) ? links.incoming.length : 0;
        const contraN = [].concat(links.incoming || [], links.outgoing || []).filter((l) => l.relation === 'contradicts').length;
        evi.append(el('span', {}, el('span', { class: 'lbl', text: '영향' }), el('span', { text: ` 참조 ${backN}건 ` }), el('span', { class: 'm', text: '(자동 갱신되지 않음)' }), contraN ? el('span', { class: 'wk2-conf', text: ` · 충돌 ${contraN}` }) : null));
        ...
        const ok = el('button', { class: 'btn btn-primary btn-sm', text: it.kind === 'new' ? '승인 · 반영' : (applied ? '확인 (이상 없음)' : '승인 · 반영') });
        const no = el('button', { class: 'btn btn-ghost btn-sm', text: it.kind === 'new' ? '반려' : (applied ? '되돌리기' : '반려 · 폐기') });
        ok.onclick = () => void w2Approve(it, drop);
        no.onclick = () => void w2Reject(it, drop);
        const act = el('div', { class: 'wk2-act' }, ok, no, el('span', { class: 'more' }, el('a', { href: '#/k/' + encodeURIComponent(it.name), text: '⋯ 직접 수정으로 대체', title: 'WIKI 편집기에서 최종본을 직접 저장한 뒤, 이 제안은 반려로 닫으세요 — 제안본을 승인하면 직접 수정이 덮입니다.' })));
        box.replaceChildren(...[nl, evi, rawT, rawBox, act].filter(Boolean));
    }
    catch (e) {
...
// ── 판정(엔드포인트는 #783 그대로) ──
async function w2Approve(it, drop) {
    try {
        if (it.kind === 'new') {
...
async function w2Reject(it, drop) {
...
let wk2Keys = null;
function installWk2Keys(listBox, visible, ui, paint, drop) {
    if (wk2Keys)
        wk2Keys.abort();
    wk2Keys = new AbortController();
    const inEditable = (t) => {
        const e = (t && t.nodeType === 1 ? t : document.activeElement);
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'j' || e.key === 'ArrowDown') {
            e.preventDefault();
            ui.cur = Math.min(vis.length - 1, ui.cur + 1);
            paint();
            return;
        }
        if (e.key === 'k' || e.key === 'ArrowUp') {
            e.preventDefault();
            ui.cur = Math.max(0, ui.cur - 1);
            paint();
            return;
        }
        if (e.key === 'Enter' || e.key === ' ') {
            ...
            if (!it)
                return;
            if (ui.open.has(it.key))
                ui.open.delete(it.key);
            else
                ui.open.add(it.key);
            paint();
            return;
...
export async function renderWiki2(view, sub, params) {
    ...
    const sideCtl = createWikiSide({
        selected: () => f.cat,
        onSelect: (v) => { f.cat = v || ''; syncHash(mode); sideCtl.rebuild(); void repaint(); },
    ...
    function headEl(m, verifyN, canonN, proposalN, extra) {
        const bt = (key, label) => {
        ...
        return el('div', {}, el('div', { class: 'wk2-head' }, el('h2', { text: headTitle() }), el('span', { class: 'sp' }), extra || null, el('div', { class: 'wk2-vt' }, bt('verify', '검증' + (verifyN ? ' ' + verifyN : '')), bt('history', '기록'))), el('div', { class: 'wk2-layers' }, el('span', { class: 'hu' }, el('span', { text: '● 정본 ' }), el('b', { text: canonN == null ? '—' : String(canonN) })), el('span', { text: '  ·  제안 ' }), el('b', { text: String(proposalN) })));
    }
    // ── 검증 뷰 ──
    async function paintVerify(box) {
        ...
        const inCat = (i) => (!selKey ? true : i.cat === selKey);
        ...
        const row = (it, idx) => {
            ...
            const r = el('div', { class: 'wk2-row' + (ui.cur === idx ? ' cur' : ''), 'data-key': it.key }, wkTick({ confidence: it.whoKind === 'human' ? 'human' : null, provenance: it.whoKind === 'connector' ? 'observed' : null }), el('span', { class: 't', text: it.title }), el('span', { class: 'k', text: it.note || (it.kind === 'new' ? '신규 제안' : (it.mode === 'applied' ? '수정 반영됨' : '수정 제안')) }), it.conflict ? el('span', { class: 'wk2-conf', text: '경합' }) : null, (it.edits || 1) > 1 ? el('span', { class: 'm', text: '×' + it.edits }) : null, el('span', { class: 'sp' }), el('span', { class: 'm', text: (it.agent || it.who) + ' · ' + relTime(it.at) }), el('span', { class: 'm', text: open ? '▾' : '▸' }));
            r.onclick = () => { ui.cur = idx; if (ui.open.has(it.key))
                ui.open.delete(it.key);
            else
                ui.open.add(it.key); paint(); };
            const parts = [r];
            if (open) {
                const card = el('div', { class: 'wk2-xp' }, el('div', { class: 'wk2-xh' }, el('span', { class: 't', text: it.title }), el('span', { class: 'wk2-chip', text: it.kind === 'new' ? '신규 제안' : (it.mode === 'applied' ? '반영됨 · 사후확인' : '수정 제안') }), el('span', { class: 'm', text: (it.agent ? it.agent + ' · ' : '') + it.who + ' · ' + relTime(it.at) })), el('div', { class: 'wk2-cardbody' }, skeleton('불러오는 중')));
                const body = card.querySelector('.wk2-cardbody');
        ...
        const paint = () => {
            const ord = ordered();
            if (ui.cur >= ord.length)
                ui.cur = Math.max(0, ord.length - 1);
            const kids = [];
            kids.push(headEl('verify', waiting.length + applied.length, canonN, all.length));
            if (!ord.length) {
                const historyLink = () => { const a = el('a', { href: '#', text: '기록' }); a.onclick = (e) => { e.preventDefault(); syncHash('history'); void renderWiki2(view, 'history', new URLSearchParams('cat=' + f.cat)); }; return a; };
                if (gateOn === false) {
                    // 빈 상태 A — 게이트 전부 auto: 대기가 비는 게 '정상 상태'임을 말한다(결함이 아니라 선택된 다이얼).
                    kids.push(el('div', { class: 'wk2-empty' }, el('div', { class: 'big', text: '지금은 모든 AI 변경이 검증 없이 즉시 반영되고 있습니다' }), el('div', {}, el('span', { text: '사람 확인을 거치게 하려면 지식 검토 게이트를 켜세요 — ' }), el('a', { href: '#/system/ingest-policy', text: '게이트 정책 설정 →' })), el('div', { class: 'wk2-kbd', style: 'margin-top:8px' }, el('span', { text: '켜면 이후의 AI 신규·수정 제안이 여기 쌓이고, 승인해야 정본에 반영됩니다. 최근 변화는 ' }), historyLink(), el('span', { text: ' 에서.' }))));
                }
                else {
                    kids.push(el('div', { class: 'wk2-empty' }, el('div', { class: 'big' }, el('span', { class: 'wk2-okdot' }), el('span', { text: '검증할 것이 없습니다' })), el('div', {}, gateOn == null ? el('span', { text: '게이트가 꺼져 있으면 AI 변경이 검증 없이 바로 반영됩니다(관리자가 설정). ' }) : null, el('span', { text: '최근 변화는 ' }), historyLink(), el('span', { text: '에서 볼 수 있습니다.' }))));
                }
            }
            else {
                if (waiting.length) {
    ...
    async function paintHistory(box) {
        ...
        const kids = [headEl('history', queueTotal, canonN, pend.length, bySel)];
        ...
        for (const [dk, list] of days) {
            ...
            const day = el('div', { class: 'wk2-day' }, el('div', { class: 'wk2-dayh' }, el('span', { text: dk }), el('span', { class: 'm', text: `갱신 ${rest.length}${mirrors.length ? ' · 미러 싱크 ' + mirrors.length : ''}` })));
            const cats = new Map();
            for (const e of rest) {
                if (!cats.has(e.catName))
                    cats.set(e.catName, []);
            ...
            for (const [cn, ces] of cats) {
                ...
                for (const e of ces) {
                    day.append(el('div', { class: 'wk2-trow' }, el('span', { class: 'tm', text: tm(e.at) }), wkTick({ confidence: e.human ? 'human' : null, provenance: null }), el('a', { class: 't', href: '#/k/' + encodeURIComponent(e.name), text: e.title }), el('span', { class: 'k', text: e.text }), e.kind === 'pending' ? el('span', { class: 'wk2-pill', text: '검토 대기' }) : null, el('span', { class: 'sp' }), el('span', { class: 'm', text: e.who }), e.kind === 'pending' ? (() => { const a = el('a', { href: '#', text: '검증 →', style: 'font-size:12px' }); a.onclick = (ev) => { ev.preventDefault(); syncHash('verify'); void renderWiki2(view, '', new URLSearchParams(f.cat ? 'cat=' + f.cat : '')); }; return a; })() : null));
                }
            }
            if (mirrors.length) {
                const det = el('details', { class: 'wk2-mirror' }, el('summary', {}, el('span', { text: `○ 노션 미러 싱크 ${mirrors.length}건 — 미분류` }), el('span', { class: 'm', text: 'last_synced_at 기준 · 펼치기 ▸' })));
                for (const e of mirrors) {
                    det.append(el('div', { class: 'wk2-trow' }, el('span', { class: 'tm', text: tm(e.at) }), el('a', { class: 't', href: '#/k/' + encodeURIComponent(e.name), text: e.title }), el('span', { class: 'sp' }), el('span', { class: 'm', text: e.who })));
                }
                day.append(det);
            }
            kids.push(day);
    ...
    async function repaint() {
        ...
        if (mode === 'history')
            ...
            await paintVerify(box);
    }
    const shell = el('div', { class: 'kn-shell' }, sideCtl.side, main);
    knApplySideW(shell);
    shell.append(knSideResizeHandle(shell));
    await sideCtl.ready;
    if (document.body.dataset.route !== 'wiki2')
        return; // 로딩 중 탭 이탈 — 늦은 mount 가 남의 화면을 덮지 않게
