...
export function createBlockEditor(opts = {}) {
    ...
    let dirty = false;
    let composing = false;
    const markDirty = () => { dirty = true; if (opts.onChange)
        opts.onChange(); };
    // ── 블록 DOM 생성 ──
    ...
    function addRowDelete(block) {
        ...
        x.onclick = () => { const nb = nextTextBlock(block) || prevTextBlock(block); block.remove(); ensureOne(); renumber(); markDirty(); if (nb)
            focusBlock(nb, false); };
        block.append(x);
    }
    ...
    function paintSlash() {
        ...
        slash.menu.replaceChildren(el('div', { class: 'be-slash-head', text: '블록' }), ...(!items.length ? [el('div', { class: 'be-slash-empty', text: '결과 없음' })] : items.map((it, idx) => {
            const row = el('button', { class: 'be-slash-item' + (idx === slash.sel ? ' on' : ''), type: 'button', role: 'menuitem' }, el('span', { class: 'be-slash-ic', text: it.ic }), el('span', { class: 'be-slash-label', text: it.label }), el('span', { class: 'be-slash-hint', text: it.hint }));
    ...
    root.addEventListener('drop', (e) => {
        if (!dragging)
            return;
        e.preventDefault();
        const t = root.querySelector('.be-drop-above, .be-drop-below');
        if (t && t !== dragging) {
            if (t.classList.contains('be-drop-above'))
                t.before(dragging);
            else
                t.after(dragging);
            renumber();
    ...
    root.addEventListener('dragend', () => { if (dragging)
        dragging.classList.remove('be-dragging'); dragging = null; clearDrop(); });
    // ── 로드/공개 API ──
    function load(md) {
        closeSlashMenu();
    ...
    return {
        ...
        setMarkdown: (md) => { load(md); dirty = false; },
        isDirty: () => dirty,
