...
function pjvOpenTaskModal(taskId, pageReload) {
    ...
    function pjvtmStatusField(t, refresh) {
        ...
        const btn = el('button', { class: 'pjv-tm-statuspill ' + meta.cls, type: 'button' }, meta.glyph ? el('span', { class: 'pjv-status-glyph', text: meta.glyph }) : null, el('span', { text: meta.label.toUpperCase() }));
        btn.onclick = (e) => {
            e.stopPropagation();
            const menu = el('div', { class: 'pjv-menu' });
    ...
    async function pjvtmTagPop(anchor, d, t, renderField) {
        ...
        function showList(query) {
            ...
            const renderList = () => {
                ...
                if (qq && !all.some((x) => x.name.toLowerCase() === qq.toLowerCase())) {
                    ...
                    const cr = el('button', { class: 'pjv-tm-tagrow pjv-tm-tagcreate', type: 'button' }, el('span', { class: 'pjv-tm-tagcreate-label', text: 'Create' }), el('span', { class: 'pjv-tm-tag', style: '--tag:' + previewColor }, el('span', { class: 'pjv-tm-tag-name', text: qq })), el('span', { class: 'pjv-tm-tagcreate-enter', text: '↵' }));
            ...
            input.addEventListener('keydown', (e) => {
                ...
                if (exact) {
                    if (!selIds().has(exact.id))
                        persistAdd(exact);
                    input.value = '';
                    renderList();
                }
                else
                    createAndAdd(v, PJV_TAG_COLORS[all.length % PJV_TAG_COLORS.length]);
            });
            renderChips();
            renderList();
        }
        ...
        function showManageAll() {
            ...
            const list = el('div', { class: 'pjv-tm-tagresults' });
            pop.replaceChildren(el('div', { class: 'pjv-tm-tagcolor-top' }, back, el('div', { class: 'pjv-tm-tagmanage-title', text: '모든 태그 관리' })), el('div', { class: 'pjv-tm-tagpop-head' }, el('span', { text: all.length + '개 · 클릭해 이름·색상·삭제 (모든 태스크 반영)' })), list);
            if (!all.length) {
                list.append(el('div', { class: 'pjv-menu-empty', text: '아직 태그가 없습니다.' }));
                return;
            }
            for (const x of all) {
        ...
        if (links.length) {
            ...
            for (const type of ['blocking', 'waiting_on', 'linked']) {
                ...
                for (const l of byType[type]) {
                    ...
                    sec.append(el('div', { class: 'pjv-tm-link-row' }, el('span', { class: 'pjv-tm-link-type ' + type, text: lt.label }), el('span', { class: 'pjv-status-dot sm ' + lmeta.cls }, lmeta.glyph ? el('span', { class: 'pjv-status-glyph', text: lmeta.glyph }) : null), el('button', { class: 'pjv-tm-link-name', type: 'button', text: l.name, onclick: () => { dirty = true; pjvOpenTaskModal(l.id, pageReload); closeModal(); } }), el('button', { class: 'pjv-tm-link-x', type: 'button', title: '연결 해제', text: '✕',
                        onclick: async () => {
                            try {
        ...
        const run = (typeof debounce === 'function' ? debounce : (f) => f)(async () => {
            ...
            results.replaceChildren(...cand.slice(0, 10).map((x) => {
                ...
                return el('button', { class: 'pjv-menu-item', type: 'button',
                    ...
                    } }, el('span', { class: 'pjv-status-dot sm ' + m.cls }, m.glyph ? el('span', { class: 'pjv-status-glyph', text: m.glyph }) : null), el('span', { text: x.name }));
            }));
            if (!cand.length)
            ...
            const load = async () => {
                ...
                for (const it of items) {
                    ...
                    rowsBox.append(el('button', { class: 'pjv-files-row file', type: 'button', onclick: async () => {
                            close();
                            addBtn.disabled = true;
                            addBtn.textContent = '첨부 중…';
                            try {
                                const tok = localStorage.getItem(TOKEN_KEY);
                                const blob = await fetch(base + pid + '/file?download=1&path=' + encodeURIComponent(childPath), { headers: tok ? { Authorization: 'Bearer ' + tok } : {} }).then((r) => { if (!r.ok)
...
export { PJV_TAG_NONE, pjvOpenTaskModal, };
