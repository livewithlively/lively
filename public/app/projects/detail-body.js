...
function projectBodyCommentRow(id, p, reload, members) {
    let card = null;
    const wide = (on) => { if (card)
        card.classList.toggle('is-wide', !!on); };
...
function projectBodyCollapse(card, bodyWrap, onWide) {
    const wide = (on) => { if (onWide)
        onWide(on); }; // 본문이 전폭을 가져야 하는 순간을 바깥(본문+코멘트 행)에 알린다
    ...
    const remeasure = () => {
        if (box.contains(document.activeElement))
            return; // 편집 중 — 아래 focusin 이 펼쳐 둔 상태를 건드리지 않는다
    ...
    if (typeof ResizeObserver === 'function')
        new ResizeObserver(() => remeasure()).observe(bodyWrap);
    card.append(box, row);
}
