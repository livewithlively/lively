...
function pjvProjAddRow(statusKey, reload, body, countEl, fields, select, canDelete, anchorId, meId, taskCtx, listId, statusDef) {
    ...
    const commit = (keepOpen) => {
        ...
        const pending = el('div', { class: 'pjv-trow pjv-proj-row pjv-proj-row-pending' }, el('div', { class: 'pjv-trow-title-cell' }, el('span', { class: 'pjv-row-check-spacer', 'aria-hidden': 'true' }), el('span', { class: 'pjv-trow-caret empty', 'aria-hidden': 'true' }), dotEl, el('span', { class: 'pjv-trow-title', text: name })));
    ...
    input.addEventListener('blur', () => {
        setTimeout(() => {
            if (busyTask || !row.classList.contains('editing'))
                return;
...
export { PJV_CU_GROUP_MAP, PJV_GROUPBY_FIELDS, PJV_PRIORITY_RANK, pjvAlsoListKey, pjvColSortCmp, pjvContainerCmp, pjvGetAlsoList, pjvGetGroupBy, pjvGroupByStoreKey, pjvGroupScope, pjvGrpOpenGet, pjvGrpOpenKey, pjvGrpOpenSet, pjvManualCmp, pjvProjAddRow, pjvProjDelete, pjvProjGroup, pjvProjMore, pjvProjRename, pjvProjRow, pjvProjSessionCell, pjvProjStatusDot, pjvProjTaskRow, pjvProjTeamControl, pjvRenderFieldGroups, pjvRenderStatusGroups, pjvSetAlsoList, pjvSetGroupBy, pjvSetProjStatus, pjvSortProjects, projPatch, };
