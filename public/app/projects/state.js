...
let _pjvKeepScrollHost = null; // 그 위치를 가진 스크롤 주체(아래 pjvScrollHost) — 신호와 함께 소비된다
...
function pjvScrollHost() { return document.querySelector('.pjv-pm-body') || window; }
function pjvScrollTop(host) { return host === window ? (window.scrollY || window.pageYOffset || 0) : host.scrollTop; }
function pjvScrollSet(host, y) { if (host === window)
    window.scrollTo(0, y);
else
    host.scrollTop = y; }
function pjvScrollTopNow() { return pjvScrollTop(pjvScrollHost()); } // 지금 상세가 보고 있는 위치(모달이면 모달 안)
function pjvReloadKeepScroll(reload) {
    ...
    _pjvKeepScrollY = null;
    _pjvKeepScrollHost = null; // 미소비(비대상 재로드)면 여기서 즉시 해제 — 누수 방지
    return ret;
}
...
function pjvKeepScrollForNextRender(y) {
    ...
    _pjvKeepScrollHost = host;
    _pjvKeepScrollY = y;
    setTimeout(() => { if (_pjvKeepScrollY === y && _pjvKeepScrollHost === host) {
        _pjvKeepScrollY = null;
        _pjvKeepScrollHost = null;
    } }, 300);
}
...
let _pjvSkipRouteRenderFor = null;
function pjvSkipNextRouteRender(hash) {
    if (!hash)
        return;
    _pjvSkipRouteRenderFor = hash;
    setTimeout(() => { if (_pjvSkipRouteRenderFor === hash)
        _pjvSkipRouteRenderFor = null; }, 1000); // 미소비 시 회수
}
function pjvConsumeSkipRouteRender(hash) {
    if (_pjvSkipRouteRenderFor && _pjvSkipRouteRenderFor === hash) {
        _pjvSkipRouteRenderFor = null;
        return true;
    }
    return false;
}
...
function pjvRestoreScroll(y, host) {
    ...
    const tick = () => {
        if (stopped)
            return;
        if (Math.abs(pjvScrollTop(h) - y) > 2) {
            pjvScrollSet(h, y);
            settledAt = 0;
        }
        else if (!settledAt)
            settledAt = Date.now();
        if ((settledAt && Date.now() - settledAt > 500) || Date.now() - t0 > 4000) {
            cleanup();
            return;
        }
...
const pjvKnownFolderIds = new Set();
...
function pjvSidePrefsBody() {
    ...
    for (const [id, v] of pjvFolderOpen) {
        ...
        if (!Number.isFinite(n) || n <= 0)
            continue;
        if (pjvKnownFolderIds.size && !pjvKnownFolderIds.has(n))
            continue;
...
function pjvSidePrefsApply(p) {
    pjvFolderOpen.clear();
    for (const id of (p && p.folder_closed) || []) {
        const n = Number(id);
        if (Number.isFinite(n))
            pjvFolderOpen.set(n, false);
    }
    for (const id of (p && p.folder_open) || []) {
        const n = Number(id);
        if (Number.isFinite(n))
            pjvFolderOpen.set(n, true);
    }
}
function pjvSidePrefsCacheSave() { try {
    localStorage.setItem(pjvSidePrefsCacheKey(), JSON.stringify(pjvSidePrefsBody()));
}
catch (_) { /* noop */ } }
function pjvSidePrefsCacheLoad() { try {
    const v = JSON.parse(localStorage.getItem(pjvSidePrefsCacheKey()) || 'null');
    if (v)
        pjvSidePrefsApply(v);
}
catch (_) { /* noop */ } }
let pjvSidePrefsTimer = null;
// 접기/펼치기 직후 — 캐시는 즉시, 서버는 디바운스(연속 토글을 한 번의 POST 로).
function pjvSidePrefsPush() {
    pjvSidePrefsCacheSave();
    if (pjvSidePrefsTimer)
        clearTimeout(pjvSidePrefsTimer);
    pjvSidePrefsTimer = setTimeout(() => {
        pjvSidePrefsTimer = null;
        api(PJV_SIDE_PREFS_API, { method: 'POST', body: JSON.stringify(pjvSidePrefsBody()) }).catch(() => { });
    }, 400);
}
...
let pjvSidePrefsReady = null;
function pjvSidePrefsEnsure() { if (!pjvSidePrefsReady)
    pjvSidePrefsReady = pjvSidePrefsLoad(); return pjvSidePrefsReady; }
async function pjvSidePrefsLoad() {
    pjvSidePrefsCacheLoad(); // 캐시 먼저 — 서버가 느리거나 실패해도 접어둔 대로 그린다
    let server = null;
    try {
        server = await api(PJV_SIDE_PREFS_API);
    }
    catch (_) {
        return;
    } // 실패 시 캐시 그대로(무해)
    if (server && server.saved) {
        pjvSidePrefsApply(server);
        pjvSidePrefsCacheSave();
        return;
    }
    if (pjvFolderOpen.size)
        pjvSidePrefsPush(); // 서버 미저장 + 이 브라우저 캐시 있음 → 1회 이관(기존 사용자 보존)
}
