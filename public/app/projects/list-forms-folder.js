...
function openFolderForm(reload, folder, opts) {
    ...
    const spaceMemberPicker = compactPicker('공개 대상', (onChange) => memberPicker((editing && isSpace ? (folder.members || []) : []).map((m) => m.member_id), { includeMe: !editing, onChange }), { emptyText: '대상 없음 — 관리자만 볼 수 있어요', avatars: true, maxChips: 6 });
    ...
    const spaceVisRow = el('div', { class: 'pjv-visrow' + (spaceVis === 'members' ? ' on' : ''), role: 'switch', tabindex: '0',
        'aria-checked': spaceVis === 'members' ? 'true' : 'false' }, el('span', { class: 'pjv-visrow-txt' }, el('span', { class: 'pjv-visrow-title', text: '공개범위를 지정한 사람으로 제한' }), el('span', { class: 'pjv-visrow-hint', text: '켜면 이 스페이스와 그 안의 리스트·프로젝트·파일·AI 세션이 대상 외에는 보이지 않아요.' })), spaceVisSw);
    const toggleSpaceVis = () => {
        spaceVis = spaceVis === 'members' ? 'open' : 'members';
        const on = spaceVis === 'members';
        spaceVisRow.classList.toggle('on', on);
        spaceVisSw.classList.toggle('on', on);
        spaceVisRow.setAttribute('aria-checked', on ? 'true' : 'false');
        spaceMemberField.style.display = on ? '' : 'none';
    };
    spaceVisRow.onclick = (e) => { e.stopPropagation(); toggleSpaceVis(); };
    spaceVisRow.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        toggleSpaceVis();
    } });
    ...
    const back = overlayBox(editing ? (kindLabel + ' 수정') : ('새 ' + kindLabel), el('div', { class: 'field' }, el('label', { class: 'field-label', text: '이름' }), nameIn), el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '색' }), swatches), isSpace ? spaceVisField : null, isSpace ? spaceMemberField : null, isSpace ? null : parentField, el('div', { class: 'pjv-side-nav-hint', style: 'margin-top:10px', text: isSpace ? '스페이스는 최상위 구획이에요 — 안에 폴더·리스트를 담아요.' : '폴더는 정리용이에요 — 멤버·공개범위·상태는 리스트에서 설정해요.' }), el('div', { class: 'ov-actions' }, saveBtn, cancelBtn));
    ...
    const go = async () => {
        ...
        try {
            if (editing) {
                ...
                if (isSpace) {
                    body.visibility = spaceVis;
                    body.members = spaceVis === 'members' ? spaceMemberPicker.getSelected() : [];
                }
                ...
                if (newId != null && newSpaceVis) {
                    try {
                        await api('/api/ui/v6/project-folders/' + newId, { method: 'POST', body: JSON.stringify(newSpaceVis) });
                    }
                    catch (_) {
                        toast('스페이스는 만들었지만 공개범위 설정에 실패했어요 — 설정에서 다시 시도해 주세요', true);
                    }
                }
                if (newId != null) {
                    try {
