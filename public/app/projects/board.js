...
function pjvProjectListBoard(projects, lists, mineIds, reload, canDelete, fields, anchorId, meId, folders, pageChrome, favData) {
    ...
    const renderArea = (byStatus, noNav) => {
        ...
        const searchProjsByList = new Map();
        const searchProjsUn = [];
        for (const p of projects) {
            if (p.list_id == null) {
                searchProjsUn.push(p);
                continue;
            }
            if (!searchProjsByList.has(p.list_id))
                searchProjsByList.set(p.list_id, []);
        ...
        const listNavItem = (list, sub, depth = 0, opts) => {
            ...
            const it = el('div', { class: 'pjv-side-navitem pjv-side-navlist' + (sub ? ' sub' : '') + (active ? ' active' : '') + (favListIds.has(list.id) ? ' is-fav' : ''), role: 'button', tabindex: '0', 'aria-pressed': String(active), ...(noDrag ? {} : { draggable: 'true' }) }, pjvListGlyph(list), el('span', { class: 'pjv-side-navlabel', text: list.name }), pjvVisLock(list), catBadge, el('span', { class: 'pjv-side-navcount', text: String(grp ? visCount(grp.projects) : 0) }));
        ...
        navInner.append(el('div', { class: 'pjv-side-nav-head' }, el('span', { class: 'pjv-side-nav-head-label', text: '폴더 · 리스트' }), el('span', { class: 'pjv-side-head-btns' }, collapseBtn, addBtn)));
        ...
        const renderFolderNode = (f, depth) => {
            ...
            const glyph = isSpace
                ? el('span', { class: 'pjv-side-space-avatar', text: (String(f.name).trim()[0] || 'S').toUpperCase(), style: 'background:' + (f.color || avatarColor('space' + f.id)) })
                : pjvBundleIcon(f.color || 'var(--muted-2)');
            const fit = el('div', { class: 'pjv-side-navitem pjv-side-navfolder' + (isSpace ? ' pjv-side-navspace' : '') + (sel === fkey ? ' active' : ''), role: 'button', tabindex: '0', draggable: 'true' }, glyph, el('span', { class: 'pjv-side-navlabel', text: f.name }), pjvVisLock(f), caret);
            pjvSideIndent(fit, depth); // 들여쓰기 격자 + 위계 세로선(#1067)
            fit.addEventListener('click', (e) => { e.stopPropagation(); if (!isFolderOpen(f.id))
                pjvSetFolderOpen(f.id, true); selectArea(fkey); });
            ...
            if (open) {
                ...
                if (fLists.length)
                    for (const l of fLists) {
                        ...
                        appendProjMatches(searchProjsOf(l.id), depth + 2, 'L' + l.id);
                    }
        ...
        function appendProjMatches(projArr, depth, scopeKey) {
            ...
            if (scopeKey) {
                ...
                more.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    go(e);
                } });
            }
        ...
        const renderArchiveNode = () => {
            ...
            caret.addEventListener('click', (e) => { e.stopPropagation(); if (!archiveFolder)
                return; pjvSetFolderOpen(archiveFolder.id, !open); render(); });
        ...
        const buildTree = () => {
            ...
            if (!sideSearchActive()) {
                ...
                if (favLists.length) {
                    ...
                    const head = el('div', { class: 'pjv-side-navitem pjv-side-favsec', role: 'button', tabindex: '0', 'aria-expanded': String(open),
                        title: '즐겨찾기 — 리스트 ⋯ 메뉴에서 추가하면 여기 맨 위에 고정됩니다' }, pjvTbIcon('star-on', 'pjv-side-favsec-ic'), el('span', { class: 'pjv-side-navlabel', text: '즐겨찾기' }), el('span', { class: 'pjv-side-navcount', text: String(favLists.length) }), caret);
                    ...
                    if (open)
                        for (const l of favLists)
                            treeWrap.append(listNavItem(l, true, 1, { noDrag: true }));
            ...
            for (const l of topLists)
                if (listMatchesDeep(l)) {
                    ...
                    appendProjMatches(searchProjsOf(l.id), 1, 'L' + l.id);
                }
            ...
            if (sideSearchActive() ? unSearchHit : showUn) {
                ...
                const uit = el('div', { class: 'pjv-side-navitem pjv-side-navlist' + (sel === unKey ? ' active' : ''), role: 'button', tabindex: '0' }, pjvBundleIcon(null, 'none'), el('span', { class: 'pjv-side-navlabel', text: '기타 (미분류)' }), el('span', { class: 'pjv-side-navcount', text: String(unGroup ? visCount(unGroup.projects) : 0) }));
    ...
    const cancelBoardSearchTimer = () => { if (searchTimer) {
    ...
    const applyBoardSearch = () => {
        ...
        if (searchComposing) {
            scheduleBoardSearch();
            return;
        } // 아직 조합 중 — 렌더는 조합이 끝난 뒤로
        if (searchInput.value === searchRenderedQ)
            return; // 이미 이 값으로 그려져 있다
        ...
        searchInput.focus();
        try {
            searchInput.setSelectionRange(selS, selE);
        }
        catch (_) { /* 브라우저가 막으면 포커스만 */ }
    };
...
export { PJV_SUBTASK_BTNLABEL, pjvFolderDrag, pjvSaveProjMembers, pjvSavedViewMenu, pjvSetProjStatusCustom, pjvSideDrag, pjvSubtaskMenu, pjvSwitchRow, pjvTaskModalStatusField, renderProjectsV2, };
