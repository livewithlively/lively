...
function pjvListCategoryField(currentId) {
    ...
    const row = el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '카테고리' }), selectEl, el('div', { class: 'field-hint', text: '이 리스트의 프로젝트가 이 카테고리를 물려받아요. 하나는 반드시 골라야 해요 — 카테고리는 관리탭 ▸ 분류 체계에서 만들어요.' }));
...
function openListForm(reload, list, opts) {
    ...
    const picker = compactPicker('참여 멤버', (onChange) => memberPicker(editing ? (list.members || []).map((m) => m.member_id) : [], { includeMe: !editing, onChange }), { emptyText: '참여 멤버 없음', avatars: true, maxChips: 6 });
    // 공개 대상 팀(#1291 v2) — 사람을 한 명씩 고르는 대신 부서로 잠근다. 팀원이 바뀌면 대상도 따라간다.
    //  잠갔을 때만 의미가 있어 토글에 연동해 보였다 숨긴다(열려 있으면 '참여 멤버'가 표시용인 것과 같은 맥락).
    const teamPicker = compactPicker('공개 대상 팀', (onChange) => pjvTeamPicker(editing ? ((list.teams || []).map((t) => t.team_id)) : [], onChange), { emptyText: '팀 미지정', maxChips: 6 });
    ...
    const rows = [
        ...
        catField.row,
        el('div', { class: 'field', style: 'margin-top:12px' }, el('label', { class: 'field-label', text: '참여 멤버' }), picker.trigger),
        statusTmplField,
        ...
        try {
            if (editing) {
                ...
                if (created && created.id) {
                    ...
                    if (tsel.length)
                        await api('/api/ui/v6/project-lists/' + created.id + '/teams', { method: 'POST', body: JSON.stringify({ teams: tsel }) }).catch(() => { });
                    if (icon)
...
export { openListForm, pjvMoveProjectList, pjvSaveListMembers, };
