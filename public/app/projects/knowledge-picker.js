...
function openKnowledgePicker(id, relation, linkedNames, onLinked) {
    ...
    const runSearch = debounce(async () => {
        ...
        try {
            matches = await api('/api/ui/knowledge/semantic?q=' + encodeURIComponent(q) + '&limit=20').then((d) => (d && d.entries) || []);
        }
        catch (e) {
    ...
    const rerender = () => { if (searchIn.value.trim())
        runSearch();
    else
        loadRecs(); }; // 관계 전환 시 현재 화면 기준 재필터(#1133)
