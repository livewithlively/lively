...
async function pjvBulkRunDefaultsModal(ctx) {
    ...
    let whereVal = d.where === 'local' ? 'local' : 'web';
    const whereBtns = {};
    const whereSeg = el('div', { class: 'where-pill' }, ...[
        ...
        b.onclick = (e) => {
            ...
            for (const k in whereBtns)
                whereBtns[k].classList.toggle('on', k === whereVal);
            syncRepoField();
        };
        whereBtns[w.k] = b;
        return b;
    ...
    const wtRow = el('label', { class: 'term-auto' }, wtChk, el('span', { text: ' 이 프로젝트 전용 작업 공간에서 격리 실행 — 자동으로 준비되고(있으면 재사용) 다른 작업과 안 섞여요.' }));
    ...
    const repoBody = el('div', { class: 'term-adv-body', hidden: '' }, el('div', { class: 'caption', text: '내 PC에서 실행할 때 아래 레포를 준비합니다(있으면 재사용). 코드 작업이 아니면 그대로 두세요 — 세션이 필요해지면 스스로 가져옵니다.' }), el('div', { class: 'term-checks' }, wtRow), repoBox);
    const repoCaret = el('span', { class: 'term-fold-caret', text: '▸' });
    const repoToggle = el('button', { class: 'term-fold', type: 'button' }, repoCaret, el('b', { text: '코드 저장소 준비' }), el('span', { class: 'term-fold-sum', text: ' · 내 PC 실행에만 적용' }));
    repoToggle.onclick = (e) => {
        e.preventDefault();
        const o = repoBody.hasAttribute('hidden');
        if (o)
            repoBody.removeAttribute('hidden');
        else
            repoBody.setAttribute('hidden', '');
    ...
    const back = overlayBox('클로드로 실행 — 기본값', el('p', { class: 'admin-hint', text: '‘클로드로 실행’(플로팅 바)을 누를 때 쓰는 기본값이에요. 여기서 바꾸면 다음 실행부터 이 값으로 준비됩니다. (이 프로젝트에만 적용)' }), runGrid, el('div', { class: 'term-checks' }, autoRow), repoField, el('div', { class: 'ov-actions' }, saveBtn, cancelBtn));
    back.classList.add('ov-back--center'); // 세션 모달과 같은 세로 정렬(#1145)
    const head = back.querySelector('.ov-head');
    if (head)
        head.insertBefore(whereSeg, head.querySelector('.btn') || null); // 제목 줄 pill
    saveBtn.onclick = () => {
...
function pjvActIcon(kind) {
    ...
    if (kind === 'session')
        return svg(sv('rect', { x: '1.5', y: '4.5', width: '16', height: '14', rx: '2.4' }), sv('path', { d: 'M5.2 9.4l3 2.6-3 2.6' }), sv('path', { d: 'M10.6 15.4h3.8' }), sv('path', { d: 'M20.6 2.6v5' }), sv('path', { d: 'M18.1 5.1h5' }));
    return svg();
}
...
export { pjvActIcon, pjvBulkApply, pjvBulkAssignee, pjvBulkBarEl, pjvBulkDelete, pjvBulkDue, pjvBulkDuplicate, pjvBulkIcon, pjvBulkList, pjvBulkPriority, pjvBulkRunClaude, pjvBulkRunDefaultsModal, pjvBulkStatus, pjvBulkTags, pjvDrag, pjvDragChecks, pjvDragEnd, pjvDragInit, pjvDragPaint, pjvGroupCheck, pjvGroupReorderTarget, pjvReorder, pjvReorderEnd, pjvReorderInit, pjvReorderMove, pjvReorderSibs, pjvReorderStart, pjvRowActions, pjvRowCheck, pjvRowGrip, pjvRowTagsEl, pjvRunDefaults, pjvRunDefaultsKey, pjvRunDefaultsLegacyKey, pjvSaveRunDefaults, pjvSel, pjvSelDomClear, pjvSelIds, pjvSelLastEl, pjvSelPatchUrl, pjvSelReloadAfter, pjvSelRenderBar, pjvSelReset, pjvSelSilent, pjvSelToggle, pjvSetChecked, pjvTagPopover, };
