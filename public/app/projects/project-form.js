...
export function openProjectV2Form(reload, prefill) {
    ...
    const go = async (withRun) => {
        ...
        if (!listChoice.ok) {
            toast('리스트를 선택하세요 — 프로젝트는 반드시 리스트에 들어갑니다', true);
            return;
        }
...
function listPicker(selectedListId) {
    ...
    let loaded = []; // 로드된 리스트(하나도 없으면 미분류 허용 — 첫 프로젝트 부트스트랩)
    let folders = [];
    let picked = selectedListId != null ? Number(selectedListId) : null;
    const expanded = new Set();
    const box = el('div', { class: 'pjv-listpick tsess-projfilter' });
    const btn = el('button', { type: 'button', class: 'tsess-projfilter-btn', 'aria-haspopup': 'true', title: '리스트 고르기' }, el('span', { class: 'pjv-listpick-label', text: '불러오는 중…' }), el('span', { class: 'tsess-projfilter-chev', text: '▾' }));
    const dd = el('div', { class: 'tsess-projfilter-dd', hidden: true });
    const search = el('input', { type: 'search', class: 'tsess-projfilter-search', placeholder: '리스트 검색…' });
    ...
    box.append(btn, dd);
    const nameOf = (id) => { const l = loaded.find((x) => Number(x.id) === Number(id)); return l ? l.name : ''; };
    const paintBtn = () => {
        const lab = btn.querySelector('.pjv-listpick-label');
        if (!loaded.length) {
            lab.textContent = '리스트 없음 — 미분류로 생성';
    ...
    const open = () => {
        ...
        render('');
        search.focus();
        docHandler = (e) => { if (!box.contains(e.target))
            close(); };
        setTimeout(() => { if (!dd.hidden)
    ...
    const buildTree = () => {
        ...
        const cnt = (n) => { n.count = n.lists.length + n.children.reduce((a, c) => a + cnt(c), 0); return n.count; };
        for (const r of roots)
            cnt(r);
        const prune = (ns) => ns.filter((n) => n.count > 0).map((n) => ({ ...n, children: prune(n.children) }));
    ...
    const listRow = (l, depth) => {
        ...
        return el('button', {
            ...
            onmousedown: (e) => { e.preventDefault(); picked = Number(l.id); paintBtn(); close(); },
        }, el('span', { class: 'tsess-opt-name', text: l.name }));
    };
    const render = (q) => {
        const ql = (q || '').trim().toLowerCase();
        const hit = (l) => !ql || String(l.name || '').toLowerCase().includes(ql);
        if (!loaded.length) {
        ...
        const walk = (ns, depth) => {
            for (const n of ns) {
                const deep = (function c(x) { return x.lists.filter(hit).length + x.children.reduce((a, y) => a + c(y), 0); })(n);
                if (ql && !deep)
                    continue; // 검색 중 매치 없는 가지는 숨긴다
                ...
                nodes.push(el('button', {
                    ...
                    onmousedown: (e) => {
                        e.preventDefault();
                        e.stopPropagation(); // 가지를 여닫아도 팝오버는 닫지 않는다
                ...
                }, el('span', { class: 'tsess-tree-caret', text: isOpen ? '▾' : '▸' }), el('span', { class: 'tsess-tree-name', text: n.name }), el('span', { class: 'tsess-tree-n', text: String(deep) })));
                shown++;
                if (isOpen) {
                    walk(n.children, depth + 1);
                    for (const l of n.lists.filter(hit).sort(sortLists)) {
        ...
        walk(roots, 0);
        for (const l of orphans.filter(hit).sort(sortLists)) {
            nodes.push(listRow(l, 0));
            shown++;
        } // 폴더 밖 리스트는 최상위에
        if (!shown)
            nodes.push(el('div', { class: 'tsess-projfilter-none', text: '일치하는 리스트가 없어요' }));
        listBox.replaceChildren(...nodes);
    };
    const sortLists = pjvContainerCmp; // #541 — 사이드바와 동일 비교자(sort → ClickUp orderindex → 이름)
    const seedExpand = () => { for (const r of buildTree().roots)
        expanded.add(r.key); }; // 스페이스(최상위 폴더)는 펼친 채로 시작
    ...
    const ready = Promise.all([
        ...
        folders = folds || [];
        if (picked != null && !loaded.some((l) => Number(l.id) === Number(picked)))
            picked = null;
        seedExpand();
        paintBtn();
    });
    return {
        ...
        getSelected: () => {
            if (picked != null)
                return { ok: true, listId: Number(picked) };
