...
function listPicker(selectedListId) {
    ...
    const btn = el('button', { type: 'button', class: 'cf-trigger', 'aria-haspopup': 'true', title: '리스트 고르기' }, chips, el('span', { class: 'cf-caret', text: '▾' }));
    ...
    const paintBtn = () => {
        if (!loaded.length) {
            chips.replaceChildren(el('span', { class: 'cf-empty', text: '리스트 없음 — 미분류로 생성' }));
            return;
        }
        if (picked == null) {
            chips.replaceChildren(el('span', { class: 'cf-empty', text: '리스트를 선택하세요' }));
            return;
        }
