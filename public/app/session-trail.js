...
function classifyBash(cmd, desc) {
    const c = String(cmd || '');
    if (!c.trim())
        return null;
...
export function classifyToolUse(name, input) {
    ...
    switch (name) {
        ...
        case 'Edit':
        case 'MultiEdit':
        case 'NotebookEdit': return mk('file', '고침', tailPath(o.file_path ?? o.notebook_path));
        case 'Write': return mk('file', '씀', tailPath(o.file_path));
        case 'Bash': return classifyBash(o.command, o.description);
        case 'Task':
        case 'Agent': return mk('cmd', '맡김', short(o.description ?? o.prompt, 70));
    }
    if (!name.startsWith('mcp__lively__'))
        return null;
    const t = name.slice('mcp__lively__'.length);
    const kHref = (n) => (n ? '#/k/' + encodeURIComponent(String(n)) : null);
    // 지식
    if (t === 'knowledge_get')
        return mk('knowledge', '읽음', String(o.name || ''), { href: kHref(o.name) });
    if (t === 'knowledge_search' || t === 'knowledge_grep' || t === 'knowledge_similar')
        return mk('knowledge', '찾아봄', short(o.q ?? o.query ?? o.name, 80));
    if (t === 'knowledge_save')
        return mk('knowledge', o.mode === 'append' ? '덧붙임' : o.mode === 'edit' ? '고침' : '남김', String(o.title || o.name || ''), { detail: o.name ? String(o.name) : undefined, href: kHref(o.name) });
    if (/^knowledge_(link|link_category|set_lifecycle|set_title|move|set_wiki|propose_category)$/.test(t))
        return mk('knowledge', '고침', String(o.name || o.from || ''), { href: kHref(o.name) });
    ...
    if (t === 'activity_log')
        return mk('activity', '기록', short(o.summary || o.title, 100), { detail: o.commit_sha ? '커밋 ' + String(o.commit_sha).slice(0, 8) : undefined });
    if (t === 'activity_list')
        return mk('activity', '살펴봄', short(o.project_id ? `프로젝트 #${o.project_id}` : '작업 기록', 60));
    // 프로젝트
    if (t === 'project_get_v6')
        return mk('project', '불러옴', `#${o.id ?? o.projectId ?? ''}`, { href: o.id ? '#/p/' + o.id : null });
    ...
    if (t === 'task_set_status_v6')
        return mk('task', o.status === 'done' ? '끝냄' : '되돌림', `#${o.id ?? ''}`, { href: o.id ? '#/projects2/t/' + o.id : null });
    if (t === 'task_detail_v6')
        return mk('task', '봄', `#${o.id ?? ''}`, { href: o.id ? '#/projects2/t/' + o.id : null });
    if (/^task_/.test(t))
        return mk('task', '고침', `#${o.id ?? o.taskId ?? ''}`, { href: o.id ? '#/projects2/t/' + o.id : null });
    // 자료
    if (t === 'source_get' || t === 'source_artifact')
        return mk('source', '읽음', String(o.id ?? o.name ?? ''));
    if (t === 'source_save')
