...
function sessionShareEditor(detail, data) {
    ...
    function build() {
        ...
        retIn.value = String(ss.retention_days ?? 0);
        retIn.disabled = !canEdit; // 폴백도 DEF(0=무제한, #1752)와 일치
        ...
        body.append(el('div', { class: 'card-sub' }, enRow), field('수집할 하네스', harnessRows, null), field('수집 범위', scopeSel, null), field('저장 형태', storeSel, null), field('보존 기간(일)', retIn, '0 = 무제한(기본 — 기록이 계속 남습니다). 일수를 정하면 그보다 오래 쉰 세션의 기록은 자동 정리되고, 목록에서도 사라집니다.'), field('기록 열람 권한', viewSel, '중앙에 모인 대화를 누가 열람·이어받을 수 있는지.'), canEdit ? el('div', { class: 'admin-actions' }, saveBtn)
            : el('p', { class: 'admin-hint' }, ...uiText('읽기 전용 — 변경은 관리자(admin) 권한이 필요합니다.')));
    }
}
export { logsEditor, sessionsAdminEditor, sessionShareEditor, };
