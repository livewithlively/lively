...
function toolsEditor(detail, data) {
    ...
    for (const t of proxyTools) {
        listCol.append(el('div', { class: 'mini-row' + (t.name === sel ? ' sel' : ''),
            onclick: () => { state.admin.toolSel = t.name; rerenderPanel(detail, 'tools-proxy', data); } }, el('div', { class: 'mini-title', text: t.name }, t.enabled === false ? el('span', { class: 'pill', text: '비활성' }) : null, t.auto_approve ? el('span', { class: 'pill pill-warn', text: '자동승인' }) : null), el('div', { class: 'mini-meta', text: (t.method || 'GET') + ' · ' + (t.scope || '-') })));
    }
    const right = el('div', {});
...
function builtinToggles(data) {
    ...
    for (const cand of cands) {
        ...
        const detail = el('div', { style: 'display:none; margin:2px 0 8px 14px; padding:6px 10px; border-left:2px solid var(--line)' }, cand.description ? el('p', { class: 'admin-hint', style: 'white-space:pre-wrap; margin:0 0 6px' }, ...uiText(cand.description)) : null, el('div', { class: 'admin-subhead', text: '입력 필드 (MCP inputSchema)' }), mcpFieldsEl(cand.inputSchema));
...
function toolForm(root, t, data, detail, isNew) {
    ...
    const kindField = field('개인 자격 종류 (auth_kind)', el('div', {}, authKindSel, el('p', { class: 'admin-hint', style: 'margin:4px 0 0' }, ...uiText('L2(집행)면 개인 자격이 필수예요. L0/L1(읽기·제안)이면 개인 자격이 없을 때 「통합 자격」으로 대신 로그인해요. 구성원은 [내 설정 ▸ 외부 서비스 관리]에서 자기 로그인을 넣습니다.'))));
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            rerenderPanel(detail, 'tools-proxy', state.admin.data);
        }
        catch (e) {
            toast(e.message, true);
...
export { toolsEditor, builtinToggles, };
