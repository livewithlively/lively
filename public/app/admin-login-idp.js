...
function loginIdpEditor(detail, data) {
    const canEdit = !!data.canEdit;
    const body = el('div');
    detail.replaceChildren(sectionHead('로그인 · 회사 계정', '구성원이 회사 구글·SSO 계정으로 로그인하게 합니다. 켜도 이메일·비밀번호 로그인은 그대로 남습니다(IdP 장애 시 들어올 길).'), el('div', { class: 'card' }, cardHead('회사 계정 로그인(OIDC)'), body));
    ...
    (async () => {
        ...
        try {
            ...
            if (!cfg)
                throw new Error('설정을 읽지 못했습니다(관리자 권한이 필요합니다).');
        }
        catch (e) {
    ...
    function statusBanner(cfg) {
        ...
        if (cfg.enterprise_required) {
            return el('div', { class: 'box' }, ...uiText('설정은 갖춰졌지만, 이 배포에는 회사 계정 로그인을 처리할 Enterprise 모듈이 없어 아직 동작하지 않습니다. ' +
        ...
        if (cfg.source === 'env') {
            return el('div', { class: 'box' }, ...uiText('지금은 서버 설정(.env 의 OIDC_*)으로 켜져 있습니다. 아래에서 저장하면 이 화면의 설정이 우선합니다.'));
        }
    ...
    function render(cfg) {
        ...
        const issuer = el('input', { type: 'text', value: cfg.issuer || '', placeholder: 'https://accounts.google.com' });
        const clientId = el('input', { type: 'text', value: cfg.client_id || '', placeholder: 'xxxxx.apps.googleusercontent.com' });
        ...
        const label = el('input', { type: 'text', value: cfg.label || '', placeholder: '비우면 issuer 로 자동(예: Google 계정으로 로그인)' });
        ...
        saveBtn.addEventListener('click', async () => {
            ...
            if (secret.value.trim())
                payload.client_secret = secret.value.trim();
            try {
        ...
        const clearBtn = el('button', { class: 'btn btn-sm', text: '시크릿 지우기' });
        ...
        const copyBtn = el('button', { class: 'btn btn-sm', text: '복사' });
        copyBtn.addEventListener('click', async () => {
            try {
                ...
                toast('복사하지 못했습니다 — 주소를 직접 선택해 복사하세요', true);
            }
        });
        body.replaceChildren(statusBanner(cfg), el('p', { class: 'admin-hint' }, ...uiText('IdP 콘솔(구글 Cloud Console ▸ Google Auth Platform)에서 웹 애플리케이션 클라이언트를 만들고, ' +
            '아래 리디렉션 주소를 그대로 등록한 뒤 발급된 클라이언트 ID·시크릿을 여기 넣으세요. ' +
            '구글 워크스페이스라면 Audience 를 Internal 로 두면 조직 구성원만 로그인할 수 있고 구글 검증 절차도 없습니다.')), field('IdP 에 등록할 리디렉션 주소', el('div', { style: 'display:flex; gap:8px; align-items:center; flex-wrap:wrap' }, cbBox, copyBtn)), field('회사 계정 로그인 사용', el('label', { style: 'display:flex; gap:8px; align-items:center' }, enabled, el('span', { class: 'admin-hint', style: 'margin:0' }, ...uiText('끄면 이메일·비밀번호만 남습니다.')))), field('issuer (IdP 주소)', issuer), field('클라이언트 ID', clientId), field('클라이언트 시크릿', el('div', { style: 'display:flex; gap:8px; align-items:center; flex-wrap:wrap' }, secret, clearBtn)), field('자동 가입 허용 도메인 (쉼표로 구분)', el('div', {}, domains, el('p', { class: 'admin-hint', style: 'margin:6px 0 0' }, ...uiText('이 도메인의 회사 이메일이면 첫 로그인에 구성원이 자동으로 만들어집니다(권한은 지식·프로젝트 조회만). ' +
            '비우면 미리 등록된 구성원만 로그인할 수 있습니다.')))), field('로그인 버튼 문구', label), el('div', { class: 'admin-actions' }, saveBtn, status));
    }
}
