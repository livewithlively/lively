...
import { THEME_ORDER, harnessThemeSync, setHarnessThemeSync, setThemePref, themePref } from './theme.js'; // #1683 화면 테마 · AI 세션 동기화
...
function themePrefRow() {
    ...
    wrap.append(seg, el('label', { style: 'display:flex; align-items:center; gap:8px; cursor:pointer;' }, cb, el('span', { style: 'font-size:13px' }, ...uiText('AI 세션도 이 테마로 띄웁니다.'))), el('p', { class: 'admin-hint', style: 'margin:0' }, ...uiText('끄면 각 AI 하네스가 저장해 둔 테마를 그대로 씁니다. 이미 열려 있는 세션은 바뀌지 않아요 — 하네스는 시작할 때 테마를 정합니다.')));
    return wrap;
}
