...
function catalogStatusCard(catalog, servers) {
    ...
    for (const c of (catalog || [])) {
        ...
        if (s && s.enabled !== false) {
            ...
            chip = el('span', { class: 'pill', style: 'background:var(--amber-bg);color:var(--amber-mid)', text: '⚙ 설정 필요' });
...
function svcTokenForm(kind, reload) {
    ...
    return el('div', { class: 'card', style: 'padding:14px' }, spec.scope ? field(spec.scope + '(선택)', scopeIn) : null, field(spec.secretLabel, secretRow(secretIn)), spec.help ? el('p', { class: 'admin-hint', style: 'margin:2px 0 0' }, ...uiText(spec.help)) : null, spec.docUrl ? el('a', { class: 'admin-hint', href: spec.docUrl, target: '_blank', rel: 'noopener', style: 'display:inline-block; margin:6px 0 0', text: '토큰 발급 페이지 열기 ↗' }) : null, el('div', { class: 'admin-actions', style: 'margin-top:10px' }, submit, status));
}
...
export { AWS_REGIONS, CRED_KINDS, awsRoleCard, catalogStatusCard, credVaultCard, credentialsEditor, openGitCredentialManager, svcTokenForm, };
