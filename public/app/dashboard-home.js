...
async function renderMyDashboard(view) {
    ...
    view.replaceChildren(el('div', { class: 'dash' }, strip, el('div', { class: 'dash-zones' }, el('div', { class: 'dash-colleft' }, zoneNotif.box, zoneProj.box), el('div', { class: 'dash-colmid' }, zoneSess.box, zoneFold.box), zoneLog.box)));
...
async function fillProjects(zone, onCount, projectsP, listsP) {
    ...
    const projRow = (p) => el('a', { class: 'dash-row dash-projrow', href: '#/projects2/p/' + p.id }, el('span', { class: 'dash-dot', style: 'background:' + dotColor(p) }), el('span', { class: 'dash-nm', title: p.name, text: p.name }), p.my_session_count ? el('span', { class: 'dash-badge', text: '세션 ' + p.my_session_count }) : null, el('span', { class: 'dash-meta', text: p.updated_at ? relTime(p.updated_at) : '' }));
    ...
    const draw = () => {
        ...
        for (const listId of order) {
            ...
            listEl.append(el('div', { class: 'dash-ghead' }, el('span', { class: 'dash-ghead-dot', style: 'background:' + ((l && l.color) || 'var(--muted-3)') }), el('span', { text: (l && l.name) || '미분류' }), el('span', { class: 'dash-ghead-n', text: String(arr.length) })));
            for (const p of arr)
                listEl.append(projRow(p));
        }
...
async function fillNotifications(zone, projectsP) {
    ...
    if (due.length) {
        ...
        for (const { p, n } of due)
            frag.push(el('a', { class: 'dash-row dash-notif-row', href: '#/projects2/p/' + p.id }, el('span', { class: 'dash-notif-ic due', text: '⏰' }), el('span', { class: 'dash-nm' }, el('span', { class: 'dash-nm-line', title: p.name, text: p.name }), el('span', { class: 'dash-sub', text: '마감 ' + dueLabel(n) })), el('span', { class: 'dash-meta' + (n < 0 ? ' overdue' : ''), text: n < 0 ? '지남' : (n === 0 ? 'D-day' : 'D-' + n) })));
    }
    if (feedItems.length) {
        if (due.length)
            frag.push(el('div', { class: 'dash-ghead', text: '최근 활동' }));
        for (const it of feedItems)
            frag.push(notifRow(it));
    }
...
function notifRow(it) {
    ...
    let ind;
    if (it.kind === 'mention')
        ind = el('span', { class: 'dash-notif-ic mention', text: '@' });
    else if (it.kind === 'comment')
        ind = el('span', { class: 'dash-notif-ic cmt', text: '💬' });
    else if (it.kind === 'update')
        ind = el('span', { class: 'dash-notif-ic upd', text: '✎' });
    else
        ind = el('span', { class: 'dash-dot tn-' + (it.tone || 'mut') });
    return el('a', { class: 'dash-row dash-notif-row' + (it.kind === 'mention' ? ' is-mention' : ''), href: '#/projects2/p/' + it.pid }, ind, el('span', { class: 'dash-nm' }, el('span', { class: 'dash-nm-line', title: it.head, text: it.head }), el('span', { class: 'dash-sub', title: sub, text: sub })));
}
// 필드 변경 이벤트 → 한국어 한 줄.
