...
async function fillProjects(zone, onCount) {
    ...
    const draw = () => {
        ...
        for (const listId of order) {
            ...
            frag.push(el('div', { class: 'dash-ghead' }, el('span', { class: 'dash-ghead-dot', style: 'background:' + ((l && l.color) || 'var(--muted-3)') }), el('span', { text: (l && l.name) || '미분류' }), el('span', { class: 'dash-ghead-n', text: String(group.length) })));
            for (const p of group) {
                frag.push(el('a', { class: 'dash-row', href: '#/projects2/p/' + p.id }, el('span', { class: 'dash-dot', style: 'background:' + dotColor(p) }), el('span', { class: 'dash-nm', title: p.name, text: p.name }), p.my_session_count ? el('span', { class: 'dash-badge', text: '세션 ' + p.my_session_count }) : null, el('span', { class: 'dash-meta' + (dueTone(p) === 'over' ? ' overdue' : ''), text: projMeta(p) })));
            }
        }
...
async function fillFolders(zone) {
    ...
    zone.body.replaceChildren(...dirs.map((name) => el('div', { class: 'dash-row' }, foldIcon(), el('span', { class: 'dash-nm', title: name, text: name }))));
}
function foldIcon() {
    const NS = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(NS, 'svg');
    for (const [k, v] of [['viewBox', '0 0 24 24'], ['fill', 'none'], ['stroke', 'currentColor'], ['stroke-width', '2'], ['aria-hidden', 'true'], ['class', 'dash-fold-ic']])
