        ...
        for (const s of shown) {
            ...
            if (s.owned) { // 소유 세션만 일괄 선택 가능(비소유는 서버가 종료 403).
                ...
                cb.onchange = () => { if (cb.checked)
                    ...
                    selected.delete(s.id); box.classList.toggle('sel', cb.checked); list.classList.toggle('has-sel', selected.size > 0); renderBar(); };
...
function openSessMenu(anchor, s, onChange) {
    ...
    item('이름 수정', false, async () => {
        ...
        try {
            ...
            toast('이름 변경 실패 — ' + (e && e.message || e), true);
        }
    });
    item('삭제', true, async () => {
