...
function dashFolderBrowser(root, startPath) {
    ...
    const openItem = (it, isDir) => { if (isDir) {
        ...
        showPreview(relOf(it.name), it.name); };
...
const DASH_IMG_MIME = { png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', gif: 'image/gif', webp: 'image/webp', svg: 'image/svg+xml', bmp: 'image/bmp', ico: 'image/x-icon', avif: 'image/avif' };
...
async function openProjTasksPopover(anchor, p) {
    ...
    try {
        ...
        if (mode === 'active')
            tasks = tasks.filter((t) => !isTaskDone(t)); // 칩(진행 중만)과 일치 — 할 일도 active 라 포함
        if (!tasks.length) {
            box.replaceChildren(el('div', { class: 'dash-pop-desc', text: mode === 'active' ? '진행 중인 태스크가 없어요.' : '태스크가 없어요.' }));
            return;
        }
        box.replaceChildren(...tasks.map((t) => el('a', { class: 'dash-listpop-row', href: '#/projects2/p/' + p.id, title: t.name || '' }, el('span', { class: 'dash-listpop-dot', style: 'background:' + dashTaskDot(t) }), el('span', { class: 'dash-listpop-nm', text: t.name || '(제목 없음)' }))));
    }
    catch (e) {
