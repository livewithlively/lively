...
function actionCard(name, input) {
    ...
    const card = el('div', { class: 'livc-slip' }, el('div', { class: 'livc-slip-head' }, el('span', { class: 'livc-slip-mark livc-slip-run', 'aria-hidden': 'true', text: '●' }), el('span', { class: 'livc-slip-name', text: toolLabel(name) }), el('details', { class: 'livc-slip-raw' }, el('summary', { text: '자세히' }), rawPart('보낸 것', JSON.stringify(input ?? {}, null, 2))), el('span', { class: 'livc-slip-state', text: '하는 중' })));
    return card;
}
/** '자세히' 안의 한 토막 — 무엇을 보여주는지 작은 이름표를 달아 둘을 한 접힘에 담는다. */
function rawPart(cap, body) {
    return el('div', { class: 'livc-raw-part' }, el('div', { class: 'livc-raw-cap', text: cap }), el('pre', { text: String(body ?? '').slice(0, 4000) }));
}
