...
async function renderMineSurface(box, ctx) {
    ...
    const searchBtn = el('button', { class: 'wk-hero-search', type: 'button', title: '전체 지식 의미검색 (⌘K)' }, el('span', { class: 'wk-hero-glass', 'aria-hidden': 'true', text: '🔍' }), el('span', { class: 'wk-hero-ph', text: '무엇이든 검색 — 의미로 찾습니다' }), el('span', { class: 'wk-hero-kbd', text: '⌘K' }));
    searchBtn.onclick = () => openWikiSearch();
    mine.append(el('div', { class: 'wk-hero' }, el('div', { class: 'wk-hero-row' }, searchBtn, hasMemoryScope() ? el('a', { class: 'btn btn-primary wk-hero-new', href: '#/knowledge/new',
        title: '새 페이지 — 제목을 쓰면 바로 저장됩니다', text: '＋ 새 페이지' }) : null), el('div', { class: 'wk-hero-stats' }, el('span', { class: 'wk-row-m', text: '카테고리 ' + allOwned.length }), el('span', { class: 'wk-row-m', text: '지식 ' + totalDocs }), freshStat)));
    ...
    function fillLane(c, body, bits, rows) {
        ...
        if (homeDoc && homeDoc.icon) {
            ...
            if (bits.wm)
                bits.wm.textContent = homeDoc.icon;
            paintStrip();
        }
    ...
    function lane(c) {
        ...
        cover.addEventListener('keydown', (ev) => { if (ev.key === 'Enter')
            go(); });
        ...
        const foot = el('div', { class: 'wk-lane3-foot' }, el('button', { class: 'wk-sec-act', type: 'button', text: '전체 보기 →', onclick: go }), el('span', { class: 'wk-sec-sp' }), newLink);
        const card = el('section', { class: 'wk-lane3', id: 'wkl-' + id }, cover, el('div', { class: 'wk-lane3-body' }, el('div', { class: 'wk-lane3-headrow' }, ic, name, el('span', { class: 'wk-lane3-hm' }, fresh, cnt, time)), list, foot));
        laneEls.set(id, card);
        const bits = { ic, cnt, time, fresh, newLink, wm: cover.querySelector('.wk-aurora-wm') };
        const load = knFetchCategoryIndex(c.id)
            .then((rows) => { if (list.isConnected)
            fillLane(c, list, bits, rows); })
            .catch(() => { if (list.isConnected)
            list.replaceChildren(wkEmpty('불러오지 못했습니다.')); });
        return { el: card, load };
    }
    ...
    function paintStrip() {
        ...
        const slots = cats.map((c) => {
            ...
            const chip = el('button', { class: 'wk-mn-chip', type: 'button',
                title: (c.name || c.key) + ' — 눌러서 해당 레인으로, 끌어서 순서 변경' }, icon ? el('span', { class: 'wk-mn-chip-ic', 'aria-hidden': 'true', text: icon }) : null, el('span', { class: 'wk-mn-chip-name', text: c.name || c.key }), freshN ? el('span', { class: 'wk-row-m wk-m-new', text: '+' + freshN }) : null);
    ...
    function applyOrder() {
        paintStrip();
        lanesBox.append(...orderedCats().map((c) => laneEls.get(String(c.id))).filter(Boolean));
        if (ctx.onCatChanged)
