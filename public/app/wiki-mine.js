...
async function renderMineSurface(box, ctx) {
    ...
    if (!cats.length) {
        mine.append(el('div', { class: 'wk-mine-head' }, el('span', { class: 'wk-mine-star', 'aria-hidden': 'true', text: '★' }), el('h1', { class: 'wk-mine-title', text: '내 소유 카테고리' })), wkEmpty('아직 내 소유 카테고리가 없습니다. 사이드바에서 카테고리에 ★를 켜면 이 화면에 모입니다.', el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '카테고리 훑어보기',
    ...
    mine.append(el('div', { class: 'wk-mine-head' }, el('span', { class: 'wk-mine-star', 'aria-hidden': 'true', text: '★' }), el('h1', { class: 'wk-mine-title', text: '내 소유 카테고리' }), el('span', { class: 'wk-mine-sp' }), searchBtn, hasMemoryScope() ? el('a', { class: 'btn btn-primary wk-mine-new', href: '#/knowledge/new',
        title: '새 페이지 — 제목을 쓰면 바로 저장됩니다', text: '＋ 새 페이지' }) : null), el('div', { class: 'wk-mine-stats' }, el('span', { class: 'wk-row-m', text: '카테고리 ' + cats.length }), el('span', { class: 'wk-row-m', text: '지식 ' + totalDocs }), freshStat));
    ...
    function fillLane(c, body, headBits, rows) {
        ...
        if (homeDoc && homeDoc.icon) {
            ...
            if (headBits.wm)
                headBits.wm.textContent = homeDoc.icon;
        }
        headBits.cnt.textContent = docs.length + '개';
        if (docs[0] && docs[0].updated_at)
            headBits.time.textContent = relTime(docs[0].updated_at);
        const freshN = docs.filter(wkIsFresh).length;
        if (freshN) {
            headBits.fresh.textContent = '+' + freshN;
            freshTotal += freshN;
            paintFreshStat();
        }
        if (!docs.length) {
    ...
    function lane(c) {
        ...
        const head = el('div', { class: 'wk-lane-head', role: 'link', tabindex: '0', title: (c.name || c.key) + ' — 카테고리 페이지로 이동합니다' }, cover, el('div', { class: 'wk-lane-headrow' }, ic, el('span', { class: 'wk-lane-name', text: c.name || c.key }), el('span', { class: 'wk-lane-hm' }, fresh, cnt, time)));
        const go = () => ctx.selectCategory(String(c.id));
        head.addEventListener('click', go);
        head.addEventListener('keydown', (ev) => { if (ev.key === 'Enter')
            go(); });
        ...
        const foot = el('div', { class: 'wk-lane-foot' }, el('button', { class: 'wk-sec-act', type: 'button', text: '전체 보기 →', onclick: go }), el('span', { class: 'wk-sec-sp' }), newLink);
