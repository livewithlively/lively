...
function previewCard(p, reload) {
    ...
    const state = el('span', { class: 'pvg-state' }, el('i', { class: 'pvg-dot ' + status, 'aria-hidden': 'true' }), el('span', { text: statusText }), verb ? el('b', { class: 'pvg-verb', text: verb }) : null);
    ...
    const meta = (where || seen) ? el('span', { class: 'pvg-meta' }, where ? el('span', { class: 'pvg-meta-w', text: where }) : null, seen ? el('span', { class: 'pvg-when', text: seen }) : null) : null;
    // 화면 골격 — "이 카드는 화면이다"를 말하는 추상 미니어처(내용을 아는 척하지 않는다 — 스크린샷이 없다).
    const face = el('span', { class: 'pvg-face' }, el('i', { class: 'pvg-sk w1', 'aria-hidden': 'true' }), el('i', { class: 'pvg-sk w2', 'aria-hidden': 'true' }), el('i', { class: 'pvg-sk w3', 'aria-hidden': 'true' }), meta, state);
    const bar = el('span', { class: 'pvg-bar' }, el('span', { class: 'pvg-lights', 'aria-hidden': 'true' }, el('i'), el('i'), el('i')), el('span', { class: 'pvg-name', text: p.label || p.id }));
    ...
    const foot = el('span', { class: 'pvg-foot' }, p.repo ? el('code', { class: 'pvg-repo', text: p.repo }) : el('span'), el('span', { class: 'pvg-acts' }, running ? el('button', { class: 'btn-text', type: 'button', text: '새로 만들기', title: '최신 작업으로 다시 준비합니다', onclick: () => previewEnsure(p.id, reload) }) : null, (running || preparing) ? el('button', { class: 'btn-text', type: 'button', text: '끄기', onclick: () => previewStop(p.id, reload) }) : null, el('button', { class: 'btn-text', type: 'button', text: '설정', onclick: () => openPreviewEnvForm(p, reload) }), el('button', { class: 'btn-text pvg-del', type: 'button', text: '삭제', onclick: () => previewDelete(p, reload) })));
...
async function previewGallery(host, reload) {
    ...
    try {
        ...
        host.replaceChildren(el('div', { class: 'card' }, errorNote(e, '미리보기 목록을 불러오지 못했습니다')));
        return;
    }
    envs.sort((a, b) => {
    ...
    const newTile = el('button', { class: 'pvg-new', type: 'button', onclick: () => openPreviewEnvForm(null, reload) }, el('span', { class: 'pvg-new-plus', 'aria-hidden': 'true', text: '+' }), el('span', { class: 'pvg-new-t', text: '새 미리보기' }), el('span', { class: 'pvg-new-sub', text: '작업 화면을 따로 띄워 봅니다' }));
    ...
    if (envs.some((x) => x.enabled && x.status === 'preparing')) {
        previewPollTimer = setTimeout(() => { if (document.body.contains(host))
            void previewGallery(host, reload); }, 5000);
    }
}
...
async function renderPreviewPage(view) {
    const host = el('div', { class: 'pvg' });
    const reload = () => void previewGallery(host, reload);
    const head = el('div', { class: 'pvg-head' }, sectionHead('미리보기', PREVIEW_HINT), el('button', { class: 'btn btn-ghost btn-sm pvg-head-add', type: 'button', text: '+ 미리보기 만들기', onclick: () => openPreviewEnvForm(null, reload) }));
...
async function previewEnsure(id, reload) {
    try {
        ...
        const s = r && r.status;
        if (s === 'running')
            toast('준비됐습니다 — 카드를 누르면 열립니다');
        else if (s === 'preparing')
            toast('준비를 시작했습니다 — 끝나면 카드가 저절로 바뀝니다');
        else
...
async function previewDelete(p, reload) {
    ...
    if (!ok)
        return;
    try {
        await api('/api/ui/preview-envs/' + encodeURIComponent(p.id) + '/delete', { method: 'POST' });
        toast('삭제했습니다');
        reload();
    }
    catch (e) {
...
export { previewEnvsPanel, renderPreviewPage, };
