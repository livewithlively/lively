...
const AUDIT_PERIOD_DAYS = { '1d': 1, '7d': 7, '30d': 30, all: 0 };
// 현재 필터 → 쿼리스트링(페이징 제외). 표 조회와 CSV 내보내기가 같은 조건을 보도록 한 곳에서 만든다(#1309).
function dbAuditFilterQs(f) {
    ...
    if (f.user)
        qs.set('user', f.user);
    if (f.table)
        qs.set('table', String(f.table).toLowerCase());
...
async function dbAuditEditor(detail, data) {
    ...
    const bar = el('div', { class: 'aud-filters' }, perField, audField('소스', srcSel), audField('종류', opSel), audField('결과', resSel), audField('조회자', userIn), audField('테이블', tableIn), audPageSizeField('db', () => { f.page = 1; reload(); }), el('div', { class: 'aud-right' }, 
    // CSV 는 화면에 찍힌 텍스트 필터(조회자·테이블)까지 그대로 반영한다 — 아직 [조회]를 안 눌렀어도 입력값을 먼저 굳힌다.
    audExportBtn(() => { f.user = userIn.value.trim(); f.table = tableIn.value.trim(); void audExportCsv('db', dbAuditFilterQs(f)); }), el('button', { class: 'btn btn-ghost btn-sm', text: '조회', onclick: applyText })));
