...
function projOpenListFilter(ctx, anchor, listId, l, rawAll, onChange) {
    const panel = el('div', { class: 'dash-pop-panel dash-pop-panel--filter' });
    const head = el('div', { class: 'dash-pop-head' });
    const qIn = el('input', { class: 'dash-pop-search', type: 'text', placeholder: '프로젝트 이름…', 'aria-label': '프로젝트 이름으로 거르기', value: dashListFilter(listId).q });
    const secs = el('div');
    ...
    const render = () => {
        ...
        head.replaceChildren(el('strong', { text: '목록 거르기' }), dashListFilterOn(lf)
        ...
        const multiSec = (title, axis, opts) => {
            ...
            const pl = pool(axis);
            const allRow = el('button', { class: 'dash-pop-opt dash-pop-opt--all' + (cur.size === 0 ? ' sel' : ''), type: 'button', 'aria-pressed': String(cur.size === 0) }, el('span', { class: 'dash-pop-box', text: cur.size === 0 ? '✓' : '' }), el('span', { class: 'dash-pop-name', text: '전체' }), el('span', { class: 'dash-pop-cnt', text: String(pl.length) }));
            ...
            for (const o of opts) {
                ...
                const row = el('button', { class: 'dash-pop-opt' + (on ? ' sel' : '') + (n === 0 && !on ? ' zero' : ''), type: 'button', 'aria-pressed': String(on) }, el('span', { class: 'dash-pop-box', text: on ? '✓' : '' }), o.deco || null, el('span', { class: 'dash-pop-name', text: o.label }), el('span', { class: 'dash-pop-cnt', text: String(n) }));
                row.onclick = () => { const next = new Set(cur); if (on)
                    next.delete(o.key);
                else
                    next.add(o.key); save({ [axis]: [...next] }); apply(); };
                nodes.push(row);
            }
        };
        ...
        multiSec('상태', 'st', dashListStatusDefs(l).map((d) => ({
        ...
        multiSec('우선순위', 'pri', [
            ...PJV_PRIORITY_ORDER.map((k) => ({
                key: k, label: PJV_PRIORITY[k].label,
                deco: el('span', { class: 'pjv-flag ' + PJV_PRIORITY[k].cls, text: '⚑' }),
        ...
        for (const p of rawAll)
            for (const t of (p.tags || []))
                if (!tagMap.has(String(t.id)))
                    tagMap.set(String(t.id), t);
        if (tagMap.size)
            multiSec('태그', 'tags', [...tagMap.values()].map((t) => ({
...
function projSeedOvOpen(ctx) {
    const parentOf = new Map(ctx.folders.map((f) => [Number(f.id), f.parent_id]));
    const shownIds = new Set([...ctx.autoIds, ...dashOvPinned()]);
...
function projOpenOvPrefs(ctx, anchor) {
    ...
    panel.append(footEl);
    const renderTree = () => {
        if (ctx.foldersLoaded && !ctx.ovSeeded) {
            ctx.ovSeeded = true;
            projSeedOvOpen(ctx);
        }
        ...
        const listRow = (l, depth) => {
            ...
            const nm = el('span', { class: 'dash-pop-name' }, ctx.favLists.has(id) ? el('span', { class: 'dash-pop-fav', title: '즐겨찾기', text: '⭐ ' }) : null, l.name || '(이름 없음)');
        ...
        const folderNode = (f, depth, forced) => {
            ...
            const toggle = () => { if (q)
                return; if (ctx.ovOpen.has(Number(f.id)))
                ctx.ovOpen.delete(Number(f.id));
            else
                ctx.ovOpen.add(Number(f.id)); renderTree(); };
    ...
    if (!ctx.foldersLoaded)
        ctx.foldersP.then(() => { ctx.foldersLoaded = true; if (rowsWrap.isConnected)
            renderTree(); });
    dashPopover(anchor, panel);
}
