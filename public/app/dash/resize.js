...
function dashColsSig(colKeys) { return colKeys.map((c) => c.join('.')).join('|'); }
...
function dashColsSaved(sig, n) {
    ...
    try {
        ...
        if (o && o.sig === sig && ok(o.fr))
            return o.fr.slice();
    }
    catch { /* 파손된 값 무시 */ }
    if (sig !== DASH_COLS_SIG_V2)
        return null;
    try {
        const a = JSON.parse(localStorage.getItem(DASH_COLS_KEY_V2) || 'null');
        return ok(a) ? a.slice() : null;
    }
    catch {
        return null;
    }
}
function dashSaveCols(sig, fr) { try {
    localStorage.setItem(DASH_COLS_KEY, JSON.stringify({ sig, fr }));
}
catch { /* 무시 */ } }
...
const DASH_ROW_KEYS_LEGACY = { 'proj-fold': 'dash_rows_left_v1', 'notif-sess': 'dash_rows_mid_v1' };
function dashRowKey(keys) { const sig = keys.join('-'); return DASH_ROW_KEYS_LEGACY[sig] || ('dash_rows_' + sig + '_v1'); }
function dashRowsSaved(key, n) {
    try {
        const a = JSON.parse(localStorage.getItem(key) || 'null');
        return Array.isArray(a) && a.length === n && a.every((x) => typeof x === 'number' && x > 0.15) ? a.slice() : null;
    }
    catch {
        return null;
    }
}
function dashSaveRows(key, a) { try {
...
function dashInitRowResize(colEl, storeKey, weights, opts) {
    const n = weights.length;
    if (n < 2) {
        if (!(opts && opts.cssDefault))
            colEl.style.gridTemplateRows = 'minmax(0,1fr)';
        return;
    } // 박스 1개 = 조절할 경계가 없다
    ...
    const captureFr = () => {
        const h = kids.map((k) => Math.max(1, k.offsetHeight || 80));
    ...
    const mkHandle = (idx) => {
        ...
        const onUp = () => { if (!dragging)
            return; dragging = false; document.removeEventListener('pointermove', onMove); document.removeEventListener('pointerup', onUp); document.body.classList.remove('dash-row-resizing'); dashSaveRows(storeKey, rows); };
        h.addEventListener('pointerdown', (e) => { e.preventDefault(); if (!frMode)
            captureFr(); dragging = true; startY = e.clientY; a0 = rows[idx]; b0 = rows[idx + 1]; document.body.classList.add('dash-row-resizing'); document.addEventListener('pointermove', onMove); document.addEventListener('pointerup', onUp); apply(); });
        ...
        h.addEventListener('keydown', (e) => { const s = e.key === 'ArrowUp' ? -0.3 : e.key === 'ArrowDown' ? 0.3 : 0; if (!s)
            ...
            captureFr(); const a = rows[idx] + s, b = rows[idx + 1] - s; if (a >= MIN_FR && b >= MIN_FR) {
            rows[idx] = a;
            rows[idx + 1] = b;
            apply();
            dashSaveRows(storeKey, rows);
        } });
        return h;
    };
    for (let i = 0; i < n - 1; i++)
        colEl.insertBefore(mkHandle(i), kids[i + 1]);
    if (!cssDefault)
        apply(); // 배치가 사람마다 달라 CSS 기본 행 트랙이 맞을 수 없다 — auto/fr 어느 쪽이든 여기서 확정
}
export { DASH_COLS_KEY, DASH_COLS_KEY_V2, DASH_COLS_SIG_V2, dashColsSig, DASH_COL_PX, dashColsSaved, dashSaveCols, DASH_ROW_KEYS_LEGACY, dashRowKey, dashRowsSaved, dashSaveRows, dashInitRowResize, };
