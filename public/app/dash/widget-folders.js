...
async function fillFolders(zone) {
    ...
    let dirs = [];
    let locked = new Set(); // 일부공개 폴더(#1291) — 🔒 배지용. 서버가 items[].locked 로 알려준다.
    ...
    const paint = () => {
        ...
        if (dashFoldView() === 'list') {
            ...
            for (const name of dirs)
                list.append(dashFolderRow(name, () => openBrowser(name), locked.has(name)));
    ...
    const reload = async () => {
        ...
        locked = new Set(((data && data.items) || []).filter((i) => i.type === 'dir' && i.locked).map((i) => i.name));
...
function dashFolderRow(name, onOpen, locked) {
    const row = el('div', { class: 'dash-fold-row', role: 'button', tabindex: '0', title: name + ' 열기' }, el('span', { class: 'dash-fold-row-ic', 'aria-hidden': 'true' }, dashFolderThumb()), el('span', { class: 'dash-fold-row-nm' }, el('span', { text: name }), locked ? dashLockBadge() : null), el('span', { class: 'dash-fold-row-go', 'aria-hidden': 'true', text: '›' }));
...
function dashFolderCard(name, locked) {
    ...
    return el('div', { class: 'proj-file-card', title: name }, locked ? dashLockBadge() : null, el('div', { class: 'proj-file-card-ic' }, dashFolderThumb()), el('div', { class: 'proj-file-card-nm', text: name }), el('div', { class: 'proj-file-card-sz', text: '폴더' }));
}
// ── 공유폴더 공개범위(#1291 v2) ──────────────────────────────────────────────
...
async function openFolderVisibilityForm(root, rel, name, onSaved) {
    ...
    let cur;
    try {
        cur = await api(aclUrl);
    }
    catch (e) {
        toast('공개범위를 불러오지 못했습니다 — ' + e.message, true);
        return;
    }
    ...
    if (!cur.settable) {
        ...
        const backRO = overlay('공개범위 · ' + name, el('div', { class: 'dash-fbvis-path', text: rel }), el('div', { class: 'dash-fbvis-note' }, el('b', { text: '이 폴더는 프로젝트의 공개범위를 그대로 따라요' }), el('span', { text: '프로젝트 폴더와 그 안의 모든 파일은 그 프로젝트가 보이는 사람에게만 보여요. 범위를 바꾸려면 프로젝트(또는 그 리스트·스페이스)의 공개범위를 바꾸세요.' })), el('div', { class: 'ov-actions' }, okBtn));
    ...
    const picker = compactPicker('공개 대상', (onChange) => memberPicker(cur.members || [], { includeMe: vis !== 'members', onChange }), { emptyText: '대상 없음 — 관리자만 볼 수 있어요', avatars: true, maxChips: 6 });
    ...
    const visRow = el('div', { class: 'pjv-visrow' + (vis === 'members' ? ' on' : ''), role: 'switch', tabindex: '0',
        'aria-checked': vis === 'members' ? 'true' : 'false' }, el('span', { class: 'pjv-visrow-txt' }, el('span', { class: 'pjv-visrow-title', text: '공개범위를 지정한 사람으로 제한' }), el('span', { class: 'pjv-visrow-hint', text: '켜면 이 폴더와 그 안의 하위 폴더·파일 전부가 대상 외에는 보이지 않아요(목록·미리보기·다운로드·업로드 모두).' })), sw);
    const toggleVis = () => {
    ...
    const inherited = cur.inherited_from
        ? el('div', { class: 'dash-fbvis-note' }, el('b', { text: '상위 폴더에서 이미 제한돼 있어요' }), el('span', { text: '‘' + cur.inherited_from + '’ 이(가) 일부공개라, 이 폴더는 여기서 더 좁힐 수만 있고 넓힐 수는 없어요.' }))
        : null;
...
function dashFolderBrowser(root, startPath) {
    ...
    const mkActions = (it, isDir) => {
        ...
        if (isDir && root === 'shared' && visAxisOn('shared_folder'))
            actions.append(act('🔒', '공개범위', false, () => openFolderVisibilityForm(root, relOf(it.name), it.name, () => load())));
        else if (!isDir)
    ...
    const fbCard = (it) => {
        ...
        it.locked ? dashLockBadge() : null, el('div', { class: 'proj-file-card-ic' }, isDir ? dashFolderThumb() : dashFileThumb(it.name)), el('div', { class: 'proj-file-card-nm', text: it.name }), el('div', { class: 'proj-file-card-sz', text: isDir ? '폴더' : fmtSize(it.size) }), it.mtime ? el('div', { class: 'proj-file-card-dt', text: fmtFileDateFull(it.mtime) }) : null); // #1118 — 항상 풀 일시
    ...
    const fbRow = (it) => {
        const isDir = it.type === 'dir';
        const row = el('div', { class: 'dash-fb-row' + (isDir ? ' is-dir' : ''), title: it.name, role: 'button', tabindex: '0' }, el('span', { class: 'dash-fb-row-ic' }, isDir ? dashFolderThumb() : dashFileThumb(it.name)), el('span', { class: 'dash-fb-row-nm' }, el('span', { text: it.name }), it.locked ? dashLockBadge() : null), 
