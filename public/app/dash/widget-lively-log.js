...
async function fillLivelyLog(zone) {
    ...
    const draw = async () => {
        let d;
        try {
            d = await api(`/api/ui/me/lively-log?window=${encodeURIComponent(win)}&scope=${scope}&limit=${shown}`);
        }
        catch (e) {
        ...
        if (d.dbAccess && d.dbAccess.length)
            lead.push(`조직 DB 를 ${n(d.dbAccess.reduce((a, r) => a + Number(r.queries || 0), 0))}번 봤어요`);
        if (lead.length)
            box.append(el('div', { class: 'dash-lvl-lead', text: lead.join(' · ') }));
        // ── 범위 전환 — '내 것만'이 기본. 공용 박스 조직에선 이게 비므로 '전체'로 넘어갈 길을 항상 준다. ──
        box.append(el('div', { class: 'dash-lvl-scope' }, ...[['me', '내 작업'], ['all', '팀 전체']].map(([k, label]) => {
            const b = el('button', { class: 'dash-chip' + (k === scope ? ' on' : ''), type: 'button', text: label });
            b.onclick = () => { if (k !== scope) {
                scope = k;
                shown = PAGE;
                draw();
            } };
        ...
        if (!events.length) {
            ...
            const others = (d.actors || []).filter((a) => a.actor !== meId);
            const why = others.length
                ? `이 계정(${nameOf(meId) || meId})으로 남은 기록이 없어요. 이 기간 기록은 ${others.map((a) => `${nameOf(a.actor)} ${n(a.calls)}건`).join(' · ')} 입니다 — AI 세션은 그 세션이 도는 계정으로 기록돼요. [팀 전체]로 보세요.`
        ...
        for (const e of fold(events)) {
            ...
            if (day !== lastDay) {
                lastDay = day;
                box.append(el('div', { class: 'dash-task-gh' }, el('span', { text: day })));
            }
            const who = scope === 'all' && e.actor && e.actor !== meId ? nameOf(e.actor) : '';
            const main = el('span', { class: 'dash-lvl-main' }, el('span', { class: 'dash-lvl-verb' + (e.ok ? '' : ' fail'), text: verbOf(e.tool) }), e.label ? el('span', { class: 'dash-lvl-title', text: e.label }) : null, e.count > 1 ? el('span', { class: 'dash-lvl-x', text: '×' + e.count }) : null, who ? el('span', { class: 'dash-lvl-by', text: who }) : null);
            box.append(el('div', { class: 'dash-row dash-lvl-row', title: `${e.tool}${e.harness ? ' · ' + e.harness : ''}` }, main, el('span', { class: 'dash-lvl-when', text: hhmm(e.at) })));
        }
        ...
        if (bg) {
            ...
            if (bgLines.length) {
                ...
                for (const [label, v] of bgLines) {
                    box.append(el('div', { class: 'dash-row dash-lvl-row' }, el('span', { class: 'dash-lvl-main' }, el('span', { class: 'dash-lvl-verb', text: label })), el('span', { class: 'dash-lvl-when', text: `${n(v)}건` })));
                }
            }
        }
