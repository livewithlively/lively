...
const SESS_SRC_OPTS = [['myproj', '프로젝트'], ['private', '개인'], ['invited', '초대']];
// #1059 P1 — AI 세션 탭과 **같은 항목·같은 순서**. 종전엔 offline 이 exited 버킷에 흡수돼 '종료됨'을 고르면
//  오프라인 세션까지 딸려왔다(고객사 A 기준 3건 고르려다 45건).
const SESS_STATE_OPTS = SESS_STATE_KEYS.map((k) => [k, SESS_STATES[k].label]);
...
const sessIsClosedProjSess = (ctx, s) => { const pid = Number(s.projectId) || 0; return pid > 0 && ctx.closedPids.has(pid) && dashSessDead(s); };
// 축 필터를 걸기 전의 기준 집합 — 필터 칩의 카운트와 실제로 그려지는 카드 수가 항상 일치하도록 둘 다 이걸 쓴다.
const sessBaseSessions = (ctx) => ctx.sessions
    .filter((s) => ctx.showClosed || !sessIsClosedProjSess(ctx, s))
    .filter((s) => !ctx.onlineOnly || !dashSessDead(s)); // #670 온라인만 — 축 필터보다 앞(칩 카운트도 이 기준)
const sessMatchSrc = (ctx, s) => ctx.fSrc.size === 0 || ctx.fSrc.has(sessSrcBucket(s));
const sessMatchState = (ctx, s) => ctx.fState.size === 0 || ctx.fState.has(sessStateBucket(s));
// 칩 라벨 = 현재 조건 요약. 2개까지는 이름 그대로, 3개부터는 '조건 N개'(헤더 폭이 흔들리지 않게).
function sessFilterLabel(ctx) {
...
function sessOpenFilterMenu(ctx, anchor) {
    ...
    const render = () => {
        ...
        const section = (title, opts, set, bucketOf, otherOk) => {
            ...
            const allRow = el('button', { class: 'dash-pop-opt dash-pop-opt--all' + (set.size === 0 ? ' sel' : ''), type: 'button', 'aria-pressed': String(set.size === 0) }, el('span', { class: 'dash-pop-box', text: set.size === 0 ? '✓' : '' }), el('span', { class: 'dash-pop-name', text: '전체' }), el('span', { class: 'dash-pop-cnt', text: String(pool.length) }));
            ...
            for (const [k, label] of opts) {
                ...
                const row = el('button', { class: 'dash-pop-opt' + (on ? ' sel' : '') + (n === 0 && !on ? ' zero' : ''), type: 'button', 'aria-pressed': String(on) }, el('span', { class: 'dash-pop-box', text: on ? '✓' : '' }), el('span', { class: 'dash-pop-name', text: label }), el('span', { class: 'dash-pop-cnt', text: String(n) }));
                row.onclick = () => { if (on)
                    set.delete(k);
                else
                    set.add(k); apply(); };
...
function sessPopTxt(name, desc) {
    const descEl = el('span', { class: 'dash-pop-desc', hidden: true, text: desc });
    const info = el('button', { class: 'dash-pop-i', type: 'button', 'aria-label': '설명 보기', 'aria-expanded': 'false', title: '설명 보기', text: 'i' });
    ...
    return el('span', { class: 'dash-pop-txt' }, el('span', { class: 'dash-pop-nameline' }, el('span', { class: 'dash-pop-name', text: name }), info), descEl);
}
// 헤더 우상단 통일 컨트롤 — ⚙(세션 표시 설정) + →(터미널 딥링크). ⚙ = '완료·보관 프로젝트 세션 표시' 토글(#req, 기존 '기본 필터'는 칩과 중복이라 폐지).
function sessOpenPrefs(ctx, a) {
    ...
    cb.checked = ctx.showClosed;
    cb.onchange = () => setShowClosed(cb.checked);
    panel.append(el('label', { class: 'dash-pop-row' }, cb, sessPopTxt('완료·보관 프로젝트 세션 표시', 'done/closed 프로젝트의 종료된(오프라인) 세션도 표시. 작업 중·확인 필요·대기 중 세션은 이 설정과 무관하게 항상 보여요.')));
    // #670 온라인 세션만 — 지금 도는 세션만. 상태 필터로도 되지만 스위치 하나가 더 빠르다(상민님, #1098 후속으로 복구).
    const cbOnline = el('input', { type: 'checkbox' });
    cbOnline.checked = ctx.onlineOnly;
    cbOnline.onchange = () => setOnlineOnly(cbOnline.checked);
    panel.append(el('label', { class: 'dash-pop-row' }, cbOnline, sessPopTxt('온라인 세션만', '작업 중·확인 필요·대기 중만 표시 — 오프라인·종료됨·복원 가능 숨김')));
    // #req 카드 정렬 선택.
    ...
    for (const [k, label] of sortOpts) {
        const row = el('button', { class: 'dash-pop-opt' + (k === ctx.sortMode ? ' sel' : ''), type: 'button' }, el('span', { class: 'dash-pop-name', text: label }), k === ctx.sortMode ? el('span', { class: 'dash-pop-check', text: '✓' }) : null);
    ...
    for (const [k, label] of [['full', '자세히 (기본)'], ['compact', '간략히']]) {
        const row = el('button', { class: 'dash-pop-opt' + (k === ctx.density ? ' sel' : ''), type: 'button' }, el('span', { class: 'dash-pop-name', text: label }), k === ctx.density ? el('span', { class: 'dash-pop-check', text: '✓' }) : null);
...
function openSessMenu(anchor, s, onChange) {
    ...
    if (s.owned)
        item(s.restorable ? '복원 목록에서 지우기' : '종료', s.restorable ? '이 세션을 더는 복원하지 않습니다(대화록은 남아요)' : '이 세션을 끝냅니다(작업 폴더·대화록은 남아요)', true, async () => {
            ...
            try {
                ...
                toast((s.restorable ? '지우기' : '종료') + ' 실패 — ' + (e && e.message || e), true);
            }
        });
    close = dashPopover(anchor, panel);
}
export { openSessRename, openSessMenu, sessSrcBucket, sessStateBucket, SESS_SRC_OPTS, SESS_STATE_OPTS, sessIsProjClosed, sessIsClosedProjSess, sessBaseSessions, sessMatchSrc, sessMatchState, sessFilterLabel, sessOpenFilterMenu, sessOpenPrefs, };
