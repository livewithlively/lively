...
function sessRenderBar(ctx) {
    ...
    const delBtn = el('button', { class: 'dash-bulkbar-btn danger', type: 'button', text: '삭제' + (n ? ' (' + n + ')' : ''), onclick: () => sessKillSelected(ctx) });
    delBtn.disabled = n === 0;
    ...
    bar.replaceChildren(el('span', { class: 'dash-bulkbar-n', text: n ? (n + '개 선택') : '세션을 골라 열거나 복원·삭제하세요' }), ...(scope.length ? [pickBtn] : []), gridBtn, restoreBtn, delBtn, el('button', { class: 'dash-bulkbar-btn', type: 'button', title: '선택 해제', text: '완료', onclick: () => { ctx.selected.clear(); ctx.draw(); } }));
}
