...
function ensureAside() {
    ...
    asideOpenBtn = el('a', { class: 'dash-aside-btn', target: '_blank', rel: 'noopener', title: '새 탭에서 열기', text: '새 탭' });
    ...
    asideFrame = el('iframe', { class: 'dash-aside-frame', title: '미리보기 화면' });
    // 지역 변수로 만들고 마지막에 모듈 변수에 넣는다 — 모듈 변수(null 허용)를 바로 쓰면 이 아래가 전부 null 검사에 걸린다.
    const box = el('aside', { class: 'dash-aside', 'aria-label': '미리보기 곁칸' }, grip, el('header', { class: 'dash-aside-h' }, asideTitle, el('span', { class: 'dash-aside-sp' }), asideOpenBtn, reload, close), asideFrame);
    close.addEventListener('click', () => closeAside());
    reload.addEventListener('click', () => { if (asideFrame)
        asideFrame.src = asideFrame.src; });
    document.addEventListener('keydown', (e) => {
        // Esc 는 곁칸이 열려 있고, 입력 중이 아닐 때만 — 모달 위에서 Esc 를 뺏지 않는다.
        if (e.key !== 'Escape' || !document.body.classList.contains('has-dash-aside'))
            return;
        const t = e.target;
        if (t && /^(INPUT|TEXTAREA|SELECT)$/.test(t.tagName))
            return;
        if (document.querySelector('dialog[open], .modal.open'))
            return;
        closeAside();
    });
    ...
    window.addEventListener('resize', () => { if (document.body.classList.contains('has-dash-aside'))
...
function openAside(url, title) {
    ensureAside();
    applyAsideWidth(asideWidth());
    if (asideTitle)
        asideTitle.textContent = title || '미리보기';
    if (asideOpenBtn)
        asideOpenBtn.href = url;
    if (asideFrame && asideFrame.src !== url)
        asideFrame.src = url;
...
/** 미리보기 목록 위젯. */
async function fillPreviews(zone) {
    const body = zone.body;
    try {
        const res = await api('/api/ui/preview-envs');
        const list = (Array.isArray(res) ? res : (res && (res.envs || res.items || res.previews))) || [];
        // 켜져 있는 것 먼저, 그다음 최근에 쓴 순 — 지금 볼 수 있는 것이 위로 온다.
        const rank = (s) => (s === 'running' ? 0 : s === 'preparing' ? 1 : 2);
        ...
        body.textContent = '';
        if (zone.countEl)
            zone.countEl.textContent = list.length ? String(list.length) : '';
        if (!list.length) {
            body.append(dashEmpty('띄워 둔 미리보기가 없어요.'));
            return;
        }
        for (const p of list) {
            const st = PREV_STATE[String(p.status)] || { t: String(p.status || ''), k: 'off' };
            const name = p.label || p.project_name || p.id;
            const row = el('button', { class: 'dash-prev', type: 'button', title: '오른쪽 곁칸에서 열기' }, el('span', { class: 'dash-prev-st dash-prev-st--' + st.k, title: st.t }), el('span', { class: 'dash-prev-b' }, el('span', { class: 'dash-prev-t', text: name }), el('span', { class: 'dash-prev-m', text: [p.id, p.branch, p.last_active_at ? relTime(p.last_active_at) : ''].filter(Boolean).join(' · ') })), el('span', { class: 'dash-prev-go', text: '곁칸에서 보기' }));
            row.addEventListener('click', () => {
                if (!p.url)
                    return;
                openAside(p.url, name);
                // 지금 보고 있는 것을 목록에서도 표시 — 곁칸과 목록이 어긋나 보이지 않게.
                for (const r of Array.from(body.querySelectorAll('.dash-prev')))
