...
function storageEditor(detail, data) {
    ...
    sectionHead('컴퓨팅 리소스', '이 서버가 쓰는 자원 — 메모리 · PTY 슬롯 · 디스크 — 를 한 화면에서 보고, 바닥나기 전에 알림을 받도록 임계를 정합니다. 어느 하나가 고갈되면 로그인·세션·터미널이 함께 멈추고, PTY 는 고갈되면 ssh 접속까지 막혀 원격으로 고칠 수도 없게 됩니다.'), el('div', { class: 'card' }, cardHead('메모리 · PTY 슬롯 · 디스크 · 경보 알림'), body));
    ...
    function build(st) {
        ...
        if (mem) {
            ...
            const eoKills = Array.isArray(mem.earlyoom_kills) ? mem.earlyoom_kills : [];
            const recent = eoKills.slice().sort((a, b) => (b.at || 0) - (a.at || 0)).slice(0, 10);
            killBlock = el('div', { class: 'storage-block' }, el('strong', { text: '최근 강제 종료 (earlyoom)' }), ...(eoReadable === false
        ...
        if (ptyS) {
            ...
            const leaks = [];
            if (typeof fdLeak === 'number' && fdLeak > 0)
                leaks.push(`PTY fd ${fdLeak}개`);
            if (typeof procLeak === 'number' && procLeak > 0)
                leaks.push(`attach 프로세스 ${procLeak}개`);
            // 고아(PPID=1)는 위 둘과 출처가 다르다 — 부모가 죽어 장부에도, 자식 카운트에도 안 잡힌다(#687 버그B 축).
            //  안 세면 "0" 으로 보여 사람을 안심시키므로 별도 항목으로 노출한다.
            if (typeof orphan === 'number' && orphan > 0)
                leaks.push(`고아 attach ${orphan}개(이전 인스턴스 잔재)`);
            ...
            const winParts = [];
            if (upSec > 0)
                winParts.push(`게이트웨이 가동 ${fmtElapsed(upSec)}`);
            if (typeof oldest === 'number')
                winParts.push(`최고령 attach ${fmtElapsed(oldest)}`);
            else if (att === 0 && upSec > 0)
                winParts.push('열린 attach 없음');
            ...
            const windowHint = winParts.length
                ...
                : '';
            ptyBlock = el('div', { class: 'storage-item' }, el('div', { class: 'storage-head' }, el('strong', { text: 'PTY 슬롯' }), el('span', { class: 'storage-lv storage-lv-' + plvKey, text: '사용 ' + pPct + '% · ' + plvLabel })), el('div', { class: 'gauge' }, pfill), el('p', { class: 'storage-calc', text: `한도 ${pMax} 중 ${pUsed} 사용 · 이 게이트웨이의 웹터미널 attach ${att}개${gapHint}${windowHint}` }));
        }
        ...
        const pressureBtn = el('button', { class: 'btn btn-primary btn-sm', text: '압박 회수 정책 저장' });
        pressureBtn.disabled = !canEdit;
        ...
        if (!STALL_PRESETS.some(([v]) => v === curStall))
            stallSel.append(el('option', { value: curStall, text: `현재 설정: ${curStall}ms` }));
        stallSel.value = curStall;
        stallSel.disabled = !canEdit;
        const stallBtn = el('button', { class: 'btn btn-primary btn-sm', text: '무응답 상한 저장' });
        stallBtn.disabled = !canEdit;
        ...
        const authStopChk = el('input', { type: 'checkbox' });
        authStopChk.checked = dp.auth_fail_stop_cron !== false; // 컬럼 부재(구 DB)면 켬이 기본
        authStopChk.disabled = !canEdit;
        const failedBtn = el('button', { class: 'btn btn-primary btn-sm', text: '실패 뒤처리 저장' });
        failedBtn.disabled = !canEdit;
        failedBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                toast('저장됨 — 다음 감시 주기(5초)부터 적용됩니다.');
                load();
            }
            catch (e) {
                toast(e.message, true);
        ...
        const ptyAlertBtn = el('button', { class: 'btn btn-primary btn-sm', text: 'PTY 경보 임계 저장' });
        ptyAlertBtn.disabled = !canEdit;
        ...
        const memoryTab = (host) => host.replaceChildren(el('h3', { class: 'storage-h', text: '지금 상태' }), ...(memBlock
            ...
            : [el('div', { class: 'storage-block' }, el('p', { class: 'admin-hint' }, ...uiText('메모리 정보를 읽지 못했습니다.')))]), ...(killBlock ? [killBlock] : []), el('h3', { class: 'storage-h', text: '메모리 경보' }), el('div', { class: 'storage-block' }, el('strong', { text: '메모리 경보 임계' }), el('div', { class: 'storage-fields' }, el('label', {}, el('span', { text: '경고 임계(사용%, 0=끔)' }), memWarnIn), el('label', {}, el('span', { text: '위험 임계(사용%, 0=끔)' }), memCritIn)), el('p', { class: 'storage-calc', text: '디스크처럼, 메모리 사용%가 이 값을 넘으면 경보 웹훅으로 알립니다(OOM 임박 사전 경고). 위험 임계는 경고보다 커야 합니다. 0=끔. 제안: 경고 85 · 위험 95.' }), el('p', { class: 'admin-hint' }, ...uiText('경보를 받을 웹훅 채널은 [경보 알림] 탭에서 설정합니다(디스크·DB·메모리·PTY 공용). 위 게이지의 현재 사용%를 보고 임계를 정하세요.')), el('div', { class: 'storage-actions' }, memAlertBtn)), 
        // ── 세션 메모리·회수(#1059) — 박스 다운(OOM) 재발 방지의 두 축을 관리탭에서 조절 ──
        el('h3', { class: 'storage-h', text: '세션 메모리 · 회수' }), el('div', { class: 'storage-block' }, el('strong', { text: '세션 메모리 상한 (per-session)' }), ...(srcHint(st.session_memory_policy_source) ? [srcHint(st.session_memory_policy_source)] : []), el('div', { class: 'storage-fields' }, el('label', {}, el('span', { text: 'MemoryHigh(MB, 0=무제한)' }), memHighIn), el('label', {}, el('span', { text: 'MemoryMax(MB, 0=무제한)' }), memMaxIn)), el('p', { class: 'storage-calc', text: 'MemoryMax 를 넘은 세션은 그 세션 안에서만 OOM-kill 되고 박스는 생존합니다(폭주 1개만 죽음). MemoryHigh 는 그 아래 소프트 스로틀. High ≤ Max.' }), el('p', { class: 'admin-hint' }, ...uiText('claude 는 네이티브라 힙제한이 안 통해 cgroup 이 유일 수단입니다. 0/0=무제한(무회귀). 캡을 걸면 새 세션이 격리 scope 로 뜹니다(박스에 격리 인프라 배포 필요 — 미설치면 종전대로 무제한). 예: 16GB 박스 High 3072 · Max 4096.')), el('div', { class: 'storage-actions' }, memPolBtn)), el('div', { class: 'storage-block' }, el('strong', { text: 'idle 세션 자동 회수' }), ...(srcHint(st.session_reclaim_policy_source) ? [srcHint(st.session_reclaim_policy_source)] : []), el('div', { class: 'storage-fields' }, el('label', {}, el('span', { text: 'idle 임계(분, 0=끔)' }), idleTtlIn)), el('p', { class: 'storage-calc', text: '이 시간 넘게 idle 인 세션을 5분 주기로 회수합니다. 작업내용(대화·설정)은 보존돼 목록에 “복원 가능”으로 남고, 열면 이어집니다(admission control 대신 채택).' }), el('p', { class: 'admin-hint' }, ...uiText('0=끔(무회귀, 기본). 상시(managed)·접속 중·작업 중·확인 대기 세션은 절대 회수하지 않습니다. 예: 16GB 박스 180~1440분.')), el('div', { class: 'storage-actions' }, reclaimBtn)), el('div', { class: 'storage-block' }, el('strong', { text: '메모리 압박 회수' }), el('div', { class: 'storage-fields' }, el('label', {}, el('span', { text: '발동 임계(사용%, 0=끔)' }), pressurePctIn), el('label', {}, el('span', { text: '스왑 임계(사용%, 0=끔)' }), pressureSwapIn), el('label', {}, el('span', { text: '압박 시 idle 하한(분)' }), pressureIdleIn)), el('p', { class: 'storage-calc', text: '메모리 또는 스왑 사용률이 임계를 넘으면 위 idle 임계를 기다리지 않고, 실제 점유(RSS)가 큰 세션부터 걷어 임계 밑으로 내려가면 멈춥니다 — 필요한 만큼만 회수합니다.' }), el('p', { class: 'admin-hint' }, ...uiText('왜 필요한가: 이 자리를 종전엔 earlyoom 이 맡았는데, 그건 예고도 복원 신호도 없는 강제 종료라 사용자 눈엔 세션이 그냥 사라집니다. 게이트웨이가 먼저 개입하면 같은 메모리를 “복원 가능”한 방식으로 확보합니다. 0=끔(기본). 평시 회수를 꺼 둔 채(위 0) 이것만 켜도 됩니다. 제안: 메모리 85~90 · 스왑 90 · 압박 하한 60분.')), el('p', { class: 'admin-hint' }, ...uiText('**스왑 임계를 함께 켜세요.** 메모리 임계는 최대 90까지만 고를 수 있습니다 — earlyoom 이 94%에서 먼저 개입하므로 그보다 늦은 값은 영영 발동하지 못합니다(실제로 95로 켜 두고 한 번도 안 돈 박스가 있었습니다). 그리고 물리 메모리가 여유로워 보여도 스왑이 차 있으면 그 박스는 이미 벼랑입니다 — 전면 장애 시점의 실측이 물리 82% · **스왑 99.9%** 였습니다. earlyoom 은 스왑을 보지 않으므로 이 축은 언제나 게이트웨이가 먼저 잡습니다.')), el('div', { class: 'storage-actions' }, pressureBtn)), 
        // ── 위탁 작업 무응답 상한(#1101) — 세션 회수와 같은 결(언제 끊을지)이라 같은 섹션에 둔다. ──
        el('div', { class: 'storage-block' }, el('strong', { text: '위탁 작업 무응답 상한' }), ...(srcHint(st.delegate_policy_source) ? [srcHint(st.delegate_policy_source)] : []), el('div', { class: 'storage-fields' }, el('label', {}, el('span', { text: '이 시간 무응답이면 실패 처리' }), stallSel)), el('p', { class: 'storage-calc', text: '위탁한 작업이 시작된 뒤 이 시간 동안 출력을 한 줄도 내지 않으면 멈춘 것으로 보고 실패로 끝냅니다. 왜 멈췄는지 짐작되는 원인도 함께 남깁니다.' }), el('p', { class: 'admin-hint' }, ...uiText('가장 흔한 원인은 의뢰자의 Claude 토큰이 없거나 만료된 경우입니다(그러면 작업이 오류도 없이 멈춰 섭니다). 이 상한이 없으면 작업 제한시간(기본 1시간)까지 아무 단서 없이 매달립니다. 정상 작업은 시작하자마자 출력을 내므로 5분이면 넉넉합니다 — 레포 준비가 오래 걸리는 박스라면 늘려 잡으세요.')), el('div', { class: 'storage-actions' }, stallBtn)), 
        // ── 실패한 위탁의 뒤처리(#1675 ①③) — 무응답 상한과 같은 결(위탁 사고 처리)이라 바로 아래에 둔다. ──
        el('div', { class: 'storage-block' }, el('strong', { text: '실패한 위탁 뒤처리' }), el('div', { class: 'storage-fields' }, el('label', {}, el('span', { text: '검시용 세션 보존(건, 0=즉시 정리)' }), keepFailedIn), el('label', {}, el('span', { text: '보존 시간(분, 0=무제한)' }), failedTtlIn), el('label', {}, el('span', { text: '자격 오류면 그 예약작업 자동 중지' }), authStopChk)), el('p', { class: 'storage-calc', text: '실패한 위탁의 작업 화면은 원인을 살펴보라고 남겨둡니다. 다만 최근 몇 건까지만, 정해둔 시간까지만 남기고 나머지는 정리합니다. 실행 기록과 오류 원문은 화면을 정리해도 그대로 남습니다.' }), el('p', { class: 'admin-hint' }, ...uiText('왜 상한이 필요한가: 종전에는 실패한 작업 화면이 **무제한으로** 쌓였습니다. Claude 토큰이 폐기된 채 예약작업이 10분마다 계속 돌던 박스에서 하루 만에 2,300여 개가 누적됐고, 그 무게로 스왑이 바닥나 데이터베이스 응답이 멈추고 웹까지 간헐적으로 끊겼습니다.')), el('p', { class: 'admin-hint' }, ...uiText('자동 중지를 켜두면 토큰 폐기·만료 같은 자격 오류가 처음 나온 순간 그 예약작업을 멈추고 알림을 보냅니다(대기 중이던 같은 작업도 함께 취소). 토큰을 다시 등록한 뒤 예약작업을 다시 켜면 재개됩니다 — 끄면 알림만 가고 계속 실패합니다.')), el('div', { class: 'storage-actions' }, failedBtn)));
        // ── [PTY 슬롯] 탭(#687 후속) — 메모리 탭과 동형(지금 상태 게이지 + 경보 임계). ──
        const ptyTab = (host) => host.replaceChildren(el('h3', { class: 'storage-h', text: '지금 상태' }), ...(ptyBlock
            ? [el('div', { class: 'storage-block' }, ptyBlock, el('p', { class: 'admin-hint' }, ...uiText('웹터미널 탭 하나가 PTY 1개를 씁니다. 게이지는 시스템 전체 포화도(tmux 세션·ssh·멤버 세션 포함)이고, 누수 판정은 그것과 별개로 이 게이트웨이의 장부와 실제(열린 PTY fd · 살아있는 attach 프로세스)를 대조해서 합니다 — 그래서 박스 규모와 무관하게 0이 정상입니다. 누수가 잡히면 게이트웨이 재시작이 즉시 전부 회수합니다(머신 재부팅 불필요, tmux 세션도 안 죽습니다).')))]
            : [el('div', { class: 'storage-block' }, el('p', { class: 'admin-hint' }, ...uiText('이 플랫폼에서는 PTY 사용량을 읽지 못합니다 — 경보 감시도 건너뜁니다.')))]), el('h3', { class: 'storage-h', text: 'PTY 경보' }), el('div', { class: 'storage-block' }, el('strong', { text: 'PTY 경보 임계' }), el('div', { class: 'storage-fields' }, el('label', {}, el('span', { text: '경고 임계(사용%, 0=끔)' }), ptyWarnIn), el('label', {}, el('span', { text: '위험 임계(사용%, 0=끔)' }), ptyCritIn)), el('p', { class: 'storage-calc', text: 'PTY 사용%가 이 값을 넘으면 경보 웹훅으로 알립니다. 디스크·메모리와 달리 기본으로 켜 둡니다(70/85) — 고갈되면 웹터미널뿐 아니라 ssh 접속까지 막혀 원격으로 고칠 수 없게 되기 때문입니다.' }), el('p', { class: 'admin-hint' }, ...uiText('경보를 받을 웹훅 채널은 [경보 알림] 탭에서 설정합니다(디스크·DB·메모리·PTY 공용).')), el('div', { class: 'storage-actions' }, ptyAlertBtn)));
        const storageTab = (host) => {
            host.replaceChildren(...(banner ? [banner] : []), el('h3', { class: 'storage-h', text: '지금 상태' }), el('div', { class: 'storage-block' }, ...diskRows), el('h3', { class: 'storage-h', text: '정책' }), ...(srcNote ? [srcNote] : []), el('div', { class: 'storage-block' }, el('strong', { text: '디스크 경고' }), el('div', { class: 'storage-fields' }, el('label', {}, el('span', { text: '경고 임계(%)' }), warnIn), el('label', {}, el('span', { text: '위험 임계(%)' }), critIn)), el('p', { class: 'storage-calc', text: '경고 → /readyz 가 degraded 로 알립니다(서비스는 정상 동작). 위험 → 신규 세션·클론을 막습니다.' }), el('p', { class: 'admin-hint' }, ...uiText('경고 임계는 위험 임계보다 낮아야 합니다.'))), el('div', { class: 'storage-block' }, el('strong', { text: '공유 빌드 캐시' }), el('label', { class: 'storage-toggle' }, cacheChk, el('span', { text: ' 의존성 캐시를 서버의 공유 위치 한 곳에 모읍니다 (권장)' })), cacheState, el('p', { class: 'admin-hint' }, ...uiText('npm·pnpm·pip·uv·Go·Maven·Yarn·NuGet·Composer 의 다운로드 캐시를 세션마다 따로 받지 않고 공유합니다. 빌드가 빨라지고, 나중에 프로젝트의 빌드 산출물을 정리해도 금방 복구됩니다. 새로 만드는 세션부터 적용됩니다.')), el('label', { class: 'storage-toggle' }, homeChk, el('span', { text: ' Gradle · Cargo 홈까지 공유 (주의)' })), el('p', { class: 'admin-hint storage-warn' }, ...uiText('⚠ 이 옵션을 켜면 캐시뿐 아니라 설정·자격증명도 공유 위치로 옮겨갑니다 — ~/.gradle/gradle.properties(서명키·저장소 인증)와 ~/.cargo/credentials.toml(레지스트리 토큰)이 무시됩니다. 그 파일에 의존하는 빌드가 실패할 수 있으니, 해당 파일을 쓰지 않는 것이 확실할 때만 켜세요.')), el('div', { class: 'storage-actions' }, saveBtn)), el('h3', { class: 'storage-h', text: '워크스페이스' }), wsRegion);
            loadWorkspace();
        };
        ...
        //  대등한 탭으로 뺀다.
        const alertTab = (host) => {
            host.replaceChildren(el('h3', { class: 'storage-h', text: '경보 알림' }), el('p', { class: 'admin-hint' }, ...uiText('디스크 · DB · 메모리 · PTY 슬롯 경보가 모두 이 채널로 나갑니다. 각 임계는 해당 탭에서 정합니다.')), alertRegion);
            loadAlert();
        };
    ...
    async function loadAlert() {
        ...
        alertRegion.replaceChildren(el('div', { class: 'storage-block' }, status, ...(keyNote ? [keyNote] : []), el('div', { class: 'storage-fields' }, el('label', { style: 'flex:1 1 320px' }, el('span', { text: '웹훅 주소' }), secretRow(urlIn)), el('label', {}, el('span', { text: '알림 기준' }), minSel), el('label', {}, el('span', { text: '이름(선택)' }), labelIn)), el('p', { class: 'admin-hint' }, ...uiText('슬랙·디스코드의 incoming webhook 주소를 그대로 넣으면 됩니다. 전송되는 알림: 디스크 경고/위험 진입, DB 연결 불가, 그리고 각각의 복구. 저장된 주소는 암호화되어 다시 표시되지 않습니다(변경할 때만 다시 입력).')), el('div', { class: 'ws-actions' }, a.configured ? el('span', {}, testA, ' ', delA) : el('span', {}), saveA)));
    }
    // ── 워크스페이스(#813 T3-2 백스톱) ──
    ...
    function renderWorkspace() {
        ...
        const { selFolders, wtFolders, actFolders, derivedOnly, selBytes } = planReclaimBatches(selected, wtSelected, analyzed);
        ...
        reclaimSelBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (derivedOnly.length) {
                    res.push(...await runBatch('/api/ui/org/workspace/reclaim', derivedOnly, { remove_worktree: false }, (n) => { done = n; tick(); }));
                }
                // ② 워크트리까지 — 서버가 실행 직전에 재검사하고, 거부되면 사유를 담아 돌려준다.
                if (wtFolders.length) {
                    const base = derivedOnly.length;
                    res.push(...await runBatch('/api/ui/org/workspace/reclaim', wtFolders, { remove_worktree: true }, (n) => { done = base + n; tick(); }));
                }
                ...
                if (wtKept.length)
                    toast(`워크트리 ${wtKept.length}개는 남았습니다 — ${wtKept[0]}`, true);
                loadWorkspace();
            }
            catch (e) {
                toast(e.message, true);
        ...
        const rows = targets.map((p) => {
            ...
            if (!pr) {
                ...
                if (wtRemovable(pr)) {
                    const wtChk = el('input', { type: 'checkbox', title: '워크트리(체크아웃 폴더)까지 제거' });
                    wtChk.checked = wtSelected.has(p.folder);
                    wtChk.disabled = !canEdit;
                    wtChk.addEventListener('change', () => {
                        if (wtChk.checked)
                            wtSelected.add(p.folder);
                        else
                            wtSelected.delete(p.folder);
                        renderWorkspace(); // 상단 버튼 라벨·합계를 다시 그린다
                    });
                    right.unshift(el('label', { class: 'ws-wt-opt', title: '워크트리(체크아웃 폴더)까지 제거 — 푸시 완료·변경 없음으로 확인된 것만' }, wtChk, el('span', { text: '워크트리' })));
                }
                ...
                for (const r of pr.results || []) {
                    ...
                    detail.append(el('div', { class: 'ws-plan' }, el('p', { class: 'storage-calc', text: `${derived.map((d) => d.path + ' ' + fmtBytes(d.bytes)).join(' · ') || '정리할 파생물 없음'}` }), el('p', { class: 'storage-calc', text: r.worktree_removable
