...
async function cronPanel(detail, data) {
    ...
    const autoSystemOf = (j) => {
        if (j.action !== 'connector_sync')
            return null;
        if (/^collector-\d+(-full)?$/.test(String(j.id)))
            return '수집기';
    ...
    for (const j of jobs) {
        ...
        const main = el('div', { class: 'wikicat-row-main' }, el('span', { class: 'wikicat-name', text: j.label || j.id }), autoSys ? withTip(el('span', { class: 'pill', text: '자동' }), autoSys === '수집기'
            ? '[맥락 관리 ▸ 수집]에서 수집기를 켜서 자동 등록된 잡입니다. 싱크를 멈추려면 이 잡이 아니라 수집기를 끄세요.'
            : autoSys + ' 커넥터를 켜서 자동 등록된 구 방식 잡입니다. 지금은 [맥락 관리 ▸ 수집]의 수집기가 이 일을 합니다.') : null, el('span', { class: 'wikicat-key mono', text: j.action + sess }), owner
            ? withTip(owner.href
                ? el('a', { class: 'wikicat-owner', href: owner.href, text: owner.label + ' →' })
                : el('span', { class: 'wikicat-owner', text: owner.label }), '이 잡의 전용 화면입니다. ' + owner.why)
            : null, 
        ...
        j.enabled
            ...
            : (j.auto_disabled_at
                ? withTip(el('span', { class: 'pill pill-warn', text: '자동 중지됨' }), (j.auto_disabled_reason || '연속 실패로 자동 중지되었습니다.')
    ...
    const card = el('div', { class: 'card' }, cardHead('정기 실행 잡', '게이트웨이가 정해진 주기마다 실행하는 잡입니다. 실제 코드 의존(is) 최신화(refresh), 미매핑 코드 유닛 LLM 분류(map_unmapped — 타깃 상시 에이전트에 주입, 팀플랜 과금), 외부 자료 수집 싱크 등이 있습니다. 주기는 초 단위 간격 또는 cron식으로 지정합니다. cron식의 시각은 '), 
    // 두 종류가 한 표에 섞여 있다는 사실을 표보다 먼저 말한다(#1618 후속) — 행마다 붙는 링크만으로는
    //  "왜 어떤 줄에만 있지?"가 되고, 없는 줄이 '누락'인지 '원래 없는 것'인지 구분이 안 된다.
    el('p', { class: 'admin-hint', style: 'margin:0 0 10px' }, el('b', { text: '전용 화면이 있는 잡과 없는 잡이 섞여 있습니다. ' }), el('span', { text: '오른쪽에 화면 이름이 붙은 잡은 그곳이 정본입니다 — 거기서 켜고 끄면 주기·의뢰자 같은 짝이 함께 맞춰집니다(여기서 고칠 수도 있지만 어긋나기 쉽습니다). 이름이 없는 잡은 이 표가 곧 주인입니다.' })), el('div', { class: 'wikicat' }, el('div', { class: 'wikicat-group' }, head, rows)));
...
function openManagedSessionForm(m, reload) {
    ...
    const renderHarnessFlags = () => {
        ...
        for (const k of Object.keys(flagCtrls))
            delete flagCtrls[k];
        flagsWrap.replaceChildren(...(h.flags || []).filter((f) => f.type === 'select').map((f) => {
            const sel = el('select', { style: psInputStyle });
            for (const v of (f.choices || ['']))
                sel.append(el('option', { value: v, text: v || '(기본)', ...((mflags[f.name] === v) ? { selected: true } : {}) }));
    ...
    const fillHarness = () => {
        ...
        if (hcat.some((h) => h.key === want))
            harnessSel.value = want;
        renderHarnessFlags();
    };
    ...
    saveBtn.onclick = async () => {
        ...
        const flags = {};
        for (const [name, ctrl] of Object.entries(flagCtrls))
            if (ctrl && ctrl.value)
                flags[name] = ctrl.value;
