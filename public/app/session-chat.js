...
export function mountSessionChat(host, first, opts) {
    ...
    let pendingSent = null; // 낙관적으로 그린 내 말 — 파일에 나타나면 그 턴을 재사용
    ...
    async function open() {
        ...
        if (!chunk) {
            ...
            if (notYet && firstPrompt && canType() && !target.node) {
                ...
                pendingSent = { text: firstPrompt, t: rec.t };
                cur = rec;
                running = true;
                firstPrompt = null;
                view.running(rec.t);
                ...
                const t0 = rec.t;
                window.setTimeout(() => { if (!destroyed && pendingSent && pendingSent.t === t0)
                    view.setNote('아직 첫 지시가 기록에 나타나지 않았어요 — 로그인이나 확인 대화상자에 멈춰 있을 수 있어요(터미널을 확인해 보세요).'); }, 60000);
                paintState();
    ...
    function olderBar() {
        ...
        const btn = el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: `이전 대화 불러오기 (${kb >= 1024 ? (kb / 1024).toFixed(1) + 'MB' : kb + 'KB'} 더 있음)` });
    ...
    async function loadOlder(btn) {
        ...
        let chunk;
        try {
            chunk = await rawGet(srcPath(src, { from, to: loadedFrom }));
        }
        catch (e) {
            btn.disabled = false;
    ...
    return {
        update(t) {
            ...
            projLink.textContent = t.projectName;
            projLink.href = t.projectId ? '#/p/' + t.projectId : '#/';
            paintState();
            if (!wasDead && dead()) {
                running = false;
                if (cur)
        ...
        destroy() { destroyed = true; if (pollTimer)
            clearTimeout(pollTimer); view.destroy(); },
    };
}
