...
export function mountSessionChat(host, first, opts) {
    ...
    const normTxt = (x) => String(x || '').replace(/\s+/g, ' ').trim().toLowerCase();
    // 사람이 지은 이름만 남긴다 — 프로젝트명 되풀이(dev 실측 58%)·id 꼴은 이름이 아니다(사이드바 side.ts sessText 와 같은 규칙).
    function cleanName() {
        let n = String(titleText || '').trim();
        const proj = String(target.projectName || '').trim();
        if (proj && n.startsWith(proj))
            n = n.slice(proj.length).replace(/^[\s·:\-–—_/|]+/, '').trim();
        if (proj && n && normTxt(n) === normTxt(proj))
            n = '';
        if (idLabel(n))
            n = '';
        return n;
    }
    const penIc = () => sv('svg', { viewBox: '0 0 24 24', class: 'sc-title-pen', 'aria-hidden': 'true' }, sv('path', { d: 'M4 20h4L19 9a2.1 2.1 0 0 0-3-3L5 17z' }));
    ...
    function paintTitle() {
        ...
        if (name) {
            titleHost.replaceChildren(canRename()
                ? el('button', { class: 'sc-title sc-title-btn', type: 'button', title: '세션 이름 — 눌러서 바꿉니다', onclick: () => startRename() }, el('span', { class: 'sc-title-t', text: name }), penIc())
    ...
    paintTitle();
    const headR = el('div', { class: 'sc-head-r' }, termStatusEl, opts.onToggleFiles ? filesBtn : null, opts.terminalSrc && isBox ? [fixBtn, setBtn] : null, moreBtn);
    const head = el('div', { class: 'sc-head' }, el('div', { class: 'sc-head-l' }, dot, titleHost, chatBadge, el('span', { class: 'sc-meta' }, stateEl, runEl)), headR);
    ...
    const startFlag = (a) => {
        const f = (target.raw && target.raw.flags) || null;
    ...
    function paintAxis(a, box, span, observed, optLabel, showLabel) {
        ...
        if (!canSwitch(a)) {
            box.hidden = true;
            chip(span, shown, a === 'model' ? (obsModelTip || known) : undefined);
            return;
        }
        span.hidden = true;
        ...
        box.replaceChildren(el('option', { value: '' }, hit ? '그대로 두기' : shown ? `지금 · ${shown}` : `${AXIS[a].ko} · AI 기본값`), ...choices.map((c) => el('option', { value: c }, optLabel(c))));
    ...
    const paintState = () => {
        ...
        if (waiting && !waitBar.childElementCount) {
            waitBar.replaceChildren(el('span', { class: 'v2-dot wait', 'aria-hidden': 'true' }), el('div', { class: 'sc-wait-t' }, el('b', { text: '확인이 필요해요' }), el('span', { text: ' — 세션이 승인이나 선택을 기다리고 있어요. 무엇을 묻는지는 터미널에 떠 있습니다.' })), el('div', { class: 'sc-wait-acts' }, opts.terminalSrc && isBox ? el('button', { class: 'btn btn-sm btn-primary', type: 'button', text: '터미널에서 답하기', onclick: () => setMode('term') }) : null, canKeys() ? el('button', { class: 'btn btn-sm btn-ghost', type: 'button', text: '기본 선택으로 답하기', onclick: () => sendKey('approve') }) : null, canKeys() ? el('button', { class: 'btn btn-sm btn-ghost', type: 'button', text: '거부', onclick: () => sendKey('deny') }) : null));
        }
        if (dead())
            paintDeadFooter();
    };
    ...
    let mode = 'chat';
    function setMode(m) {
        if (m === 'term' && (!opts.terminalSrc || !isBox))
            m = 'chat';
        mode = m;
    ...
    function paintQState(pd, row) {
        ...
        if (stuck) {
            pd.state.replaceChildren(el('span', { text: '전달 대기 중 — 입력창이 아직 안 떠요. 로그인이 필요한 상태일 수 있어요. ' }), ...(opts.terminalSrc && isBox ? [el('button', { class: 'btn-text dt-qact', type: 'button', text: '터미널 열기', onclick: () => setMode('term') })] : []));
    ...
    async function openInner() {
        ...
        if (!chunk) {
            ...
            view.list.append(el('div', { class: 'livc-open sc-empty' }, el('p', { text: msg }), opts.terminalSrc && isBox ? el('button', { class: 'btn btn-ghost btn-sm', type: 'button', text: '터미널로 보기', onclick: () => setMode('term') }) : null));
            paintState();
    ...
    const rxDone = new Set(); // 이 묶음은 닫았다(× ) — 새 묶음이면 다시 뜬다
    let rxKey = ''; // 지금 그려 둔 묶음
    const rxOff = () => { try {
        return localStorage.getItem(RX_OFF_KEY) === '1';
    }
    catch {
        return false;
    } };
    const askAt = (t) => { const v = t.ts ? Date.parse(t.ts) : NaN; return Number.isFinite(v) ? v : t.startedAt; };
    /** 마지막 '한 묶음' = 끝에서부터 20분 이내로 이어진 사람 말들. 그 사이가 벌어지면 거기서 끊는다. */
    function lastAskCluster() {
        const asks = recs.map((r) => r.t).filter((t) => t.text && t.text.trim());
        if (asks.length < RX_MIN_ASKS)
            return [];
        const out = [asks[asks.length - 1]];
        for (let i = asks.length - 2; i >= 0; i--) {
            if (askAt(out[0]) - askAt(asks[i]) > RX_GAP_MS)
                break;
            out.unshift(asks[i]);
        }
        return out;
    ...
    function paintRx() {
        ...
        if (rxOff() || running) {
            if (!running)
                rxHost.replaceChildren();
            return;
        }
        const cluster = lastAskCluster();
        if (cluster.length < RX_MIN_ASKS) {
            rxHost.replaceChildren();
            rxKey = '';
            return;
        }
        ...
        if (key === rxKey)
            return; // 같은 묶음 — 다시 그리지 않는다(깜빡임 방지)
        rxKey = key;
        if (rxDone.has(key)) {
            rxHost.replaceChildren();
            return;
    ...
    function rxMerged(cluster) {
        const parts = cluster.map((t) => (t.text || '').trim()).filter(Boolean);
        const box = el('div', { class: 'sc-rx-prompt' }, el('span', { class: 'sc-rx-plabel', text: '나눠 말씀하신 것을 한 덩어리로 합치면' }), el('div', { class: 'sc-rx-ptext' }, ...parts.map((p, i) => i === 0
            ? el('span', { text: p })
            : el('span', { class: 'sc-rx-add', text: ' ' + p }))));
        return { el: box, text: parts.join(' ') };
    }
    function rxCard(cluster, key) {
        const n = cluster.length;
        const mins = Math.max(1, Math.round((askAt(cluster[n - 1]) - askAt(cluster[0])) / 60000));
        const merged = rxMerged(cluster);
        const close = () => { rxDone.add(key); rxHost.replaceChildren(); };
        // 4 → 1 — 숫자를 읽지 않아도 보이게. 뒤쪽(합쳐진 것)에는 예상 시간을 쓰지 않는다(우리는 모른다).
        const dots = el('div', { class: 'sc-rx-fold' }, el('span', { class: 'sc-rx-grp' }, ...Array.from({ length: Math.min(n, 6) }, () => el('i', { class: 'sc-rx-b' }))), el('span', { class: 'sc-rx-arrow', 'aria-hidden': 'true', text: '→' }), el('span', { class: 'sc-rx-grp' }, el('i', { class: 'sc-rx-b one' })), el('span', { class: 'sc-rx-cap' }, el('b', { text: `${n}번 · ${mins}분` }), document.createTextNode(' 이 한 번으로')));
        const put = canType()
        ...
        const copy = el('button', { class: 'btn-text sc-rx-alt', type: 'button', text: '복사',
            onclick: async () => { try {
                await navigator.clipboard.writeText(merged.text);
                toast('지시문을 복사했어요.');
            }
            catch {
                window.prompt('이 지시문을 복사하세요:', merged.text);
            } } });
        ...
        const never = el('button', { class: 'btn-text sc-rx-never', type: 'button', text: '이런 거 안 볼래요',
            onclick: () => { try {
                localStorage.setItem(RX_OFF_KEY, '1');
            }
            catch { /* 비치명 */ } rxHost.replaceChildren(); toast('앞으로 이 안내를 띄우지 않습니다.'); } });
        return el('div', { class: 'sc-rx' }, el('div', { class: 'sc-rx-h' }, el('span', { class: 'sc-rx-av', 'aria-hidden': 'true', text: 'L' }), el('span', { class: 'sc-rx-ht', text: '이번 건, 한 번에 끝낼 수 있었어요' }), el('button', { class: 'btn-text sc-rx-x', type: 'button', title: '닫기', 'aria-label': '닫기', text: '×', onclick: close })), el('div', { class: 'sc-rx-b-wrap' }, el('p', { class: 'sc-rx-said' }, document.createTextNode('나눠서 '), el('b', { text: `${n}번` }), document.createTextNode(' 말씀하시느라 '), el('b', { text: `${mins}분` }), document.createTextNode('이 걸렸어요. 아래처럼 처음부터 한 번에 주시면 됩니다.')), dots, merged.el, el('div', { class: 'sc-rx-acts' }, ...(put ? [put] : []), copy, keep, never)));
    }
