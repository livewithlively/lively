...
function mcpEditor(detail, data) {
    ...
    for (const s of servers) {
        listCol.append(el('div', { class: 'mini-row' + (s.name === sel ? ' sel' : ''),
            onclick: () => { state.admin.mcpSel = s.name; rerenderPanel(detail, 'mcp', data); } }, el('div', { class: 'mini-title', text: s.name }, s.enabled === false ? el('span', { class: 'pill', text: '비활성' }) : null), el('div', { class: 'mini-meta', text: (s.transport || 'http') + ' · ' + (s.transport === 'stdio' ? (s.command || '-') : (s.url || '-')) })));
    }
    const right = el('div', {});
...
function mcpForm(root, s, data, detail, isNew) {
    ...
    refreshBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            rerenderPanel(detail, 'mcp', state.admin.data);
        }
        catch (e) {
...
function mcpPresetField(f, data, isNew) {
    ...
    presetSel.addEventListener('change', () => {
        ...
        if (c.dcr) {
            ...
            if (c.guide.intro)
                presetHint.append(el('div', { class: 'caption', style: 'margin:0 0 6px' }, ...uiText(c.guide.intro)));
            presetHint.append(el('ol', { style: 'margin:0;padding-left:18px;display:flex;flex-direction:column;gap:4px' }, ...c.guide.steps.map((s) => {
                const parts = String(s).split('{callback}');
                const li = el('li', {});
                parts.forEach((p, i) => { li.append(...uiText(p)); if (i < parts.length - 1)
            ...
            if (c.name === 'slack') {
                ...
                mkBtn.addEventListener('click', async () => {
                    mkBtn.disabled = true;
                    try {
                        const r = await api('/api/ui/org/slack/app-manifest');
                        if (r && r.create_url)
                            window.open(r.create_url, '_blank', 'noopener');
                        else
                            toast('링크를 받지 못했습니다', true);
                    }
                    catch (e) {
                        toast((e && e.message) || '링크를 만들지 못했습니다', true);
                    }
                    finally {
            ...
            if (c.guide.url)
                presetHint.append(el('a', { class: 'admin-hint', href: c.guide.url, target: '_blank', rel: 'noopener', style: 'display:inline-block;margin-top:6px', text: '설정 페이지 열기 ↗' }));
        }
        else {
            presetHint.append(el('div', { style: 'font-weight:600;margin-bottom:4px', text: `${c.label}: 사전등록 OAuth client 필요 — provider 콘솔 셋업:` }), el('ol', { style: 'margin:0;padding-left:18px;display:flex;flex-direction:column;gap:3px' }, el('li', { text: 'provider 콘솔에서 OAuth 클라이언트 생성(유형은 서비스 문서 참조)' }), el('li', { text: '아래 [필요한 OAuth 허용범위] 를 그 콘솔의 스코프 설정에 추가' }), el('li', {}, '승인된 redirect URI 에 게이트웨이 콜백 등록 → ', el('code', { text: cb })), el('li', {}, '발급된 client_id/secret 를 아래 ', el('b', { text: 'OAuth 클라이언트' }), ' 필드에 입력'), el('li', {}, ...uiText('저장 → 본인 [연결] → [발행](막히면 스코프/콜백 재확인)'))));
        }
        ...
        if (c.oauth_scope) {
            ...
            presetHint.append(el('div', { style: 'margin-top:8px;font-weight:600' }, ...uiText('필요한 OAuth 허용범위')), scopeBox, copyBtn);
        }
        if (c.note)
            presetHint.append(el('div', { class: 'caption', style: 'margin-top:6px' }, ...uiText(c.note)));
