...
function clsEnsureStyles() {
    if (document.getElementById('cls-styles'))
        return;
...
export async function renderClassificationReview(view) {
    const host = el('div', {});
...
async function classificationPanel(detail) {
    clsEnsureStyles();
    rqEnsureStyles();
    ...
    try {
        ...
        detail.replaceChildren(el('div', { class: 'card' }, errorNote(e, '제안된 분류를 불러오지 못했습니다')));
        return;
    }
    ...
    const visible = () => items.filter((i) => (!clsUi.space || i.space === clsUi.space)
        && (!clsUi.cat || i.catKey === clsUi.cat)
        && (!clsUi.lowOnly || i.confidence == null || i.confidence < 0.5));
    // 낮은 신뢰도(<50% 또는 미측정) — 칩·통계에서 공유하는 판정.
    const isLow = (i) => i.confidence == null || i.confidence < 0.5;
    ...
    const catSel = el('select', { class: 'rq-sel', style: 'width:auto;min-width:170px' });
    function rebuildCatSel() {
        const seen = new Map(); // key → 표시명(space · 이름)
        for (const it of items) {
            if (clsUi.space && it.space !== clsUi.space)
                continue;
            if (it.catKey && !seen.has(it.catKey))
                seen.set(it.catKey, (SPACE_LABEL[it.space] || it.space || '') + ' · ' + it.catName);
        }
        if (clsUi.cat && !seen.has(clsUi.cat))
            clsUi.cat = ''; // 필터로 사라진 카테고리 선택은 해제
        catSel.replaceChildren(el('option', { value: '', text: '모든 카테고리' }), ...[...seen].map(([k, label]) => el('option', { value: k, text: label })));
        catSel.value = clsUi.cat;
    }
    catSel.onchange = () => { clsUi.cat = catSel.value; clsUi.cur = 0; paint(); };
    const spaceSel = el('select', { class: 'rq-sel', style: 'width:auto;min-width:120px' });
    for (const [v, t] of [['', '모든 영역'], ['business', SPACE_LABEL.business], ['product', SPACE_LABEL.product], ['system', SPACE_LABEL.system]]) {
    ...
    const drop = (key) => {
        ...
        if (i >= 0)
            items.splice(i, 1);
        clsUi.sel.delete(key);
        clsUi.open.delete(key);
        paint();
    };
    const paint = () => {
        const vis = visible();
        if (clsUi.cur >= vis.length)
            clsUi.cur = Math.max(0, vis.length - 1);
        ...
        countEl.replaceChildren(el('span', {}, '총 ', el('b', { text: String(items.length) }), '건 대기'), low ? el('span', { text: ` · 신뢰도 낮음(<50%) ${low}건` }) : null, el('span', { text: ' · 신뢰도 낮은순' }));
    };
    const card = el('div', { class: 'card' }, cardHead('분류 검토 대기', '분류기가 제안한 카테고리를 확정·재분류·반려합니다. 신뢰도 낮은순(가장 의심스러운 것 먼저)으로 정렬돼 있습니다.'), countEl, el('div', { class: 'rq-bar' }, chips, spaceSel, catSel, el('span', { class: 'rq-kbd' }, el('b', { text: 'j/k' }), el('span', { text: ' 이동 · ' }), el('b', { text: 'Enter' }), el('span', { text: ' 근거 · ' }), el('b', { text: 'a' }), el('span', { text: ' 확정 · ' }), el('b', { text: 'r' }), el('span', { text: ' 반려 · ' }), el('b', { text: 'x' }), el('span', { text: ' 선택' }))), listBox, bulkBox);
    detail.replaceChildren(card);
    paint();
    installKeys(listBox, visible, paint, drop);
}
// 평면 목록 — 서버가 신뢰도 낮은순으로 준 순서 그대로(그룹핑하면 그 순서가 깨진다).
function renderList(vis, paint, drop) {
...
function renderRow(it, idx, paint, drop) {
    ...
    const chk = el('input', {
        ...
        onclick: (e) => {
            e.stopPropagation();
            if (clsUi.sel.has(it.key))
                clsUi.sel.delete(it.key);
            else
                clsUi.sel.add(it.key);
            paint();
        },
    });
    ...
    const titleLink = el('a', {
        ...
        title: '문서 열기', onclick: (e) => e.stopPropagation(),
    });
    const cat = el('span', { class: 'cls-cat', title: (SPACE_LABEL[it.space] || it.space || '') + ' · ' + (it.catKey || '') }, el('span', { class: 'sp', text: SPACE_LABEL[it.space] || it.space || '—' }), el('span', { text: it.catName }));
    const meta = el('span', { class: 'rq-meta' }, cat, it.type ? el('span', { text: KN_TYPE_LABEL[it.type] || it.type }) : null, el('span', { text: relTime(it.at) }));
    const acts = el('span', { class: 'rq-acts' }, el('button', { class: 'btn btn-ghost btn-sm', text: '✓ 확정', title: '제안된 카테고리로 확정',
        onclick: (e) => { e.stopPropagation(); void confirmOne(it, drop); } }), el('button', { class: 'btn btn-ghost btn-sm', text: '↻ 재분류', title: '다른 카테고리로 바꿔 확정',
        onclick: (e) => { e.stopPropagation(); openReclassifyPicker(e.currentTarget, it, drop); } }), el('button', { class: 'btn btn-ghost btn-sm', text: '✕ 반려', title: '분류를 취소하고 미분류 인박스로 되돌림',
        onclick: (e) => { e.stopPropagation(); void rejectOne(it, drop); } }));
    const main = el('div', {
        class: 'rq-row-main',
        onclick: () => { if (clsUi.open.has(it.key))
            clsUi.open.delete(it.key);
        else
            clsUi.open.add(it.key); clsUi.cur = idx; paint(); },
    ...
    if (isOpen) {
        row.append(el('div', { class: 'rq-exp' }, it.evidence
...
function openReclassifyPicker(anchor, it, drop) {
    const old = document.querySelector('.kn-metapop');
    if (old) {
        old.remove();
        return;
    } // 열려 있으면 토글로 닫기(openKnMetaPicker 관례)
    ...
    api('/api/ui/categories').then((d) => {
        ...
        for (const sp of ['business', 'product', 'system']) {
            ...
            for (const c of inSp) {
                ...
                const b = el('button', { class: 'kn-metapop-item' + (on ? ' on' : ''), type: 'button' }, el('span', { text: c.name || c.key }), on ? el('span', { class: 'kn-metapop-check', 'aria-hidden': 'true', text: '✓ 현재 제안' }) : null);
...
async function confirmOne(it, drop) {
    try {
        await api('/api/ui/knowledge/' + encodeURIComponent(it.name) + '/category', { method: 'POST', body: JSON.stringify({ category_id: it.categoryId, state: 'confirmed' }) });
        toast('확정 — 분류를 확정했습니다');
        drop(it.key);
    }
    catch (e) {
...
async function reclassifyOne(it, categoryId, drop) {
    try {
        await api('/api/ui/knowledge/' + encodeURIComponent(it.name) + '/category', { method: 'POST', body: JSON.stringify({ category_id: categoryId, state: 'confirmed' }) });
...
async function rejectOne(it, drop) {
    if (!confirm('이 제안을 반려할까요? 카테고리 연결을 지우고 미분류 인박스로 되돌립니다(지식 본문은 그대로).'))
        return;
    try {
        await api('/api/ui/knowledge/' + encodeURIComponent(it.name) + '/category', { method: 'POST', body: JSON.stringify({ category_id: it.categoryId, unlink: true }) });
...
function bulkBar(items, paint, drop) {
    ...
    const go = el('button', { class: 'btn btn-primary btn-sm', text: `선택 ${n}건 확정` });
    go.onclick = async () => {
        if (!confirm(`${n}건을 제안된 카테고리로 확정할까요?`))
            return;
        go.disabled = true;
    ...
    return el('div', { class: 'rq-bulk' }, el('b', { text: `${n}건 선택됨` }), go, el('span', { style: 'font-size:11.5px;color:var(--muted)', text: '재분류·반려는 건별로' }), el('button', { class: 'btn btn-ghost btn-sm', text: '선택 해제', onclick: () => { clsUi.sel.clear(); paint(); } }));
}
...
function installKeys(listBox, visible, paint, drop) {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Enter' || e.key === ' ') {
            ...
            if (!it)
                return;
            if (clsUi.open.has(it.key))
                clsUi.open.delete(it.key);
            else
                clsUi.open.add(it.key);
            paint();
            return;
        }
        if (e.key === 'x') {
        ...
        if (e.key === 'r') {
            ...
            const it = cur();
            if (it)
                void rejectOne(it, drop);
            return;
        }
        if (e.key === 'Escape' && clsUi.sel.size) {
            e.preventDefault();
            clsUi.sel.clear();
            paint();
        }
