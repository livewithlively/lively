...
function myAiSection(detail) {
    detail.replaceChildren(sectionHead('내 AI 계정', '내 AI 세션이 어떤 AI 로, 누구 계정으로 실행되는지 보고 필요하면 로그인합니다.'), el('div', { class: 'admin-stack' }, myAiAccountsCard(), el('p', { class: 'admin-hint' }, ...uiText('역할·호칭·말투·사용 언어 같은 **AI 개인 규칙**은 화면 왼쪽 아래의 내 이름을 눌러 열리는 내 프로필 창 [AI 개인 규칙] 에서 편집합니다.'))));
}
export { 
//  #1898 — 내 프로필 창([내 AI 계정] 탭)이 **이 카드를 그대로** 쓴다. 로그인 세션을 여는 규칙
//  (claude 는 그 하네스로 · codex 는 셸 + device-auth)이 여기 한 곳에만 있어야 한다.
myAiAccountsCard, myAiSection, };
