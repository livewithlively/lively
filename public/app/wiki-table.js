...
function wkTrack(cols) { return [WK_TITLE_TRACK, ...cols.map((c) => c.width), WK_MORE_TRACK].join(' '); }
// 작성 주체 — 민트 틱과 같은 판정(사람 저작 / 외부 미러 / AI). 글자로 한 번 더 말한다(색만으로 뜻을 전하지 않는다).
function wkAuthorLabel(e) {
    if (e.confidence === 'human')
        return '사람';
...
export function wkDocCols(opts = {}) {
    const cols = [
        { key: 'type', label: '유형', width: '88px', render: (e) => el('span', { class: 'pjv-fval', text: e.type ? (KN_TYPE_LABEL[e.type] || e.type) : '' }) },
    ];
    if (opts.category)
        cols.push({ key: 'cat', label: '분류', width: '150px', align: 'left', render: (e) => el('span', { class: 'pjv-fval wk-tcat', text: (opts.catName && opts.catName(e)) || '' }) });
    cols.push({ key: 'who', label: '작성', width: '72px', render: (e) => el('span', { class: 'pjv-fval wk-twho' + (e.confidence === 'human' ? ' human' : ''), text: wkAuthorLabel(e) }) }, { key: 'updated', label: '갱신', width: '92px', render: (e) => el('span', { class: 'pjv-fval', title: e.updated_at || '', text: e.updated_at ? relTime(e.updated_at) : '' }) });
    return cols;
}
...
export function wkColHead(cols, titleLabel = '제목') {
    const head = el('div', { class: 'pjv-tgroup-head pjv-tgroup-head-cols pjv-list-colhead wk-colhead' }, el('div', { class: 'pjv-trow-title-cell' }, el('span', { class: 'pjv-list-colhead-name', text: titleLabel })), ...cols.map((c) => el('div', { class: 'pjv-tcell pjv-colhead pjv-stdcol' + (c.align === 'left' ? ' wk-col-left' : ''), 'data-col': c.key }, el('span', { class: 'pjv-thcol-name', text: c.label, title: c.label }))), el('div', { class: 'pjv-tcell pjv-tcell-add' }, el('span', {})));
    head.style.gridTemplateColumns = wkTrack(cols);
    return head;
}
// 한 행 — 프로젝트 행(.pjv-trow.pjv-proj-row)과 같은 높이·hover·제목 셀 문법. 제목 셀 = [체크] 틱 · 아이콘 · 제목 · 발췌.
export function wkTableRow(e, opts) {
    ...
    title.onclick = (ev) => { ev.stopPropagation(); if (opts.select && !isFolder) {
        check && check.click();
        return;
    } go(); };
    ...
    const titleCell = el('div', { class: 'pjv-trow-title-cell' }, check ? el('label', { class: 'wk-tcheckwrap' }, check) : el('span', { class: 'pjv-row-check-spacer', 'aria-hidden': 'true' }), wkTick(e), ic, title, snip ? el('span', { class: 'wk-tsnip', title: snip, text: snip }) : null);
    titleCell.addEventListener('click', (ev) => {
        if (ev.target.closest('button, input, label, a, .pjv-trow-title'))
            return;
        if (check) {
            check.click();
            return;
    ...
    more.onclick = (ev) => {
        ...
        if (!isFolder)
            menu.append(mk('새 탭에서 열기', () => window.open(location.pathname + '?ui=classic#/k/' + encodeURIComponent(e.name), '_blank')));
        for (const it of (opts.menu ? opts.menu(e) : []))
            menu.append(mk(it.label, it.fn, it.danger));
    };
    const row = el('div', { class: 'pjv-trow pjv-proj-row wk-trow' + (isFolder ? ' folder' : '') + (e.lifecycle === 'archived' ? ' archived' : '') + (check && check.checked ? ' sel' : ''), 'data-author': e.confidence || '', 'data-prov': e.provenance || '' }, titleCell, ...cells, el('div', { class: 'pjv-tcell pjv-tcell-add' }, more));
...
export function wkTableGroup(label, entries, opts) {
    const body = el('div', { class: 'pjv-tgroup-body wk-tbody' }, wkColHead(opts.cols, opts.titleLabel));
    for (const e of entries)
        body.append(wkTableRow(e, opts));
    if (!label)
        return el('div', { class: 'pjv-tgroup wk-tgroup' }, body);
    const caret = el('button', { class: 'pjv-tgroup-caret', type: 'button', text: '▾', 'aria-expanded': 'true', title: '접기' });
    const head = el('div', { class: 'pjv-tgroup-head wk-tgroup-head' }, el('span', { class: 'pjv-row-check-spacer', 'aria-hidden': 'true' }), caret, el('span', { class: 'pjv-tgroup-label', text: label }), el('span', { class: 'pjv-tgroup-count', text: String(opts.count ?? entries.length) }));
    let open = true;
...
export function wkBoardHeader(o) {
    const nav = el('nav', { class: 'pjv-crumbs', 'aria-label': '현재 위치' });
    o.crumbs.forEach((c, i) => {
        if (i)
            nav.append(el('span', { class: 'pjv-crumb-sep', 'aria-hidden': 'true', text: '/' }));
    ...
    if (o.sub)
        nav.append(el('span', { class: 'wk-crumb-sub', text: o.sub }));
    ...
    for (const t of o.tabs) {
        const b = el(t.href ? 'a' : 'button', { class: 'pjv-vtab' + (t.active ? ' active' : ''), role: 'tab', 'aria-selected': String(!!t.active), ...(t.href ? { href: t.href } : { type: 'button' }) }, t.iconNode || pjvTabIcon(t.icon || 'list'), el('span', { text: t.label }));
        if (t.onClick)
            b.onclick = (ev) => { ev.preventDefault(); t.onClick(); };
        tabs.append(b);
    }
    const hasTb = (o.left && o.left.length) || (o.right && o.right.length);
    const toolbar = hasTb ? el('div', { class: 'card-head pjv-board-toolbar' }, el('div', { class: 'pjv-tasks-head-left' }, ...(o.left || [])), el('div', { class: 'card-head-actions' }, ...(o.right || []))) : null; // 툴바에 실을 게 없으면 빈 줄을 남기지 않는다
...
export function wkTbIcon(cls, label, icon, fn) {
    const b = el('button', { class: 'pjv-tb-btn ' + cls, type: 'button', title: label, 'aria-label': label }, icon);
    b.onclick = (ev) => { ev.stopPropagation(); fn(b); };
    return b;
}
export function wkTbPill(label, opts = {}) {
    const b = el('button', { class: 'pjv-tb-btn pjv-tb-pill wk-tb-pill' + (opts.active ? ' active' : ''), type: 'button', title: opts.title || label }, opts.icon || null, el('span', { class: 'pjv-view-btn-label', text: label }));
    if (opts.onClick)
        b.onclick = (ev) => { ev.stopPropagation(); opts.onClick(b); };
    return b;
}
export function wkTbPrimary(label, fn, more) {
    ...
    if (more && more.length) {
        ...
        mb.onclick = (ev) => {
            ...
            for (const it of more) {
                const b = el('button', { class: 'pjv-menu-item', type: 'button' }, el('span', { text: it.label }));
                b.onclick = (e2) => { e2.stopPropagation(); close(); it.fn(); };
                menu.append(b);
            }
        };
        group.append(mb);
...
export function wkTbSearch(value, placeholder, onCommit) {
    const input = el('input', { type: 'text', class: 'pjv-tb-search-input', placeholder, 'aria-label': placeholder, value });
    const box = el('div', { class: 'pjv-tb-search' + (value ? ' open' : '') });
    const btn = wkTbIcon('pjv-search-btn' + (value ? ' active' : ''), '검색 — 제목·본문으로 좁혀 보기', pjvTbIcon('search'), () => { box.classList.toggle('open'); if (box.classList.contains('open'))
        input.focus();
    else if (input.value) {
        input.value = '';
        onCommit('');
...
export function wkSurfaceTabs(active, reviewCount) {
    return [
        { key: 'docs', label: '문서', icon: 'list', href: '#/knowledge?all=1', active: active === 'docs' },
        { key: 'sources', label: '자료', icon: 'table', href: '#/knowledge/sources', active: active === 'sources' },
