...
async function renderKnowledge(view, sub, params) {
    ...
    if (sub === 'graph')
        return renderKnowledgeGraph(view); // #290 지식 그래프
...
function knLinkRow(e, k, view, incoming) {
    const row = el('div', { class: 'row kn-link-row', style: 'display:flex; gap:8px; align-items:center;' }, el('a', { class: 'row-title', href: '#/k/' + encodeURIComponent(e.name),
        style: 'flex:1; text-decoration:none; display:flex; gap:8px; align-items:baseline;' }, el('span', { class: 'kn-chip kn-link-rel kn-link-' + e.relation, text: KN_LINK_REL_LABEL[e.relation] || e.relation }), el('span', { text: e.title || e.name })));
    if (hasScope('memory')) {
...
function knLinksPanel(k, view) {
    ...
    if (sources.length) {
        box.append(el('div', { class: 'sec-label', text: '출처 자료' }), el('div', { class: 'list-box' }, ...sources.map((s) => el('span', { class: 'kn-chip kn-source-chip',
...
async function renderKnowledgeGraph(view) {
    const head = el('div', { class: 'page-head' }, el('h1', {}, '지식 ', el('span', { class: 'accent', text: '그래프' })), el('p', { class: 'sub', text: '지식↔지식 링크 그래프 — 노드를 카테고리로 묶고 링크(관련·구체화·모순·의존)를 선으로 잇습니다. 노드를 누르면 상세로. 링크가 쌓일수록 풍부해집니다.' }));
    ...
    let data;
    try {
        data = await api('/api/ui/knowledge-graph?' + new URLSearchParams({ limit: '500' }));
    }
    catch (e) {
        holder.replaceChildren(errorNote(e, '그래프를 불러오지 못했습니다'));
        return;
    }
    const nodes = (data && data.nodes) || [], edges = (data && data.edges) || [];
    if (!nodes.length) {
        holder.replaceChildren(el('div', { class: 'empty', text: '표시할 지식이 없습니다.' }));
        return;
    }
    // 카테고리별 클러스터를 큰 원 둘레에 배치, 클러스터 내부는 작은 원에.
    const byCat = new Map();
    for (const n of nodes) {
        const c = n.category || '(미분류)';
        if (!byCat.has(c))
            byCat.set(c, []);
        byCat.get(c).push(n);
    }
    ...
    const SPACE_COLOR = { product: '#5b9cff', business: '#f0a84d', system: '#9b8cff' };
    const pos = new Map();
    cats.forEach((c, ci) => {
