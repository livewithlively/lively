...
function knGraphBtn() {
    return el('button', { class: 'btn btn-ghost btn-sm kn-graph-btn', type: 'button', role: 'link',
        title: '도메인으로 묶은 지식 지도 — 풀스크린 새 창에서 팬·줌으로 탐색', onclick: openKnowledgeAtlas }, sv('svg', { class: 'sub-graph-ic', viewBox: '0 0 24 24', width: '14', height: '14', 'aria-hidden': 'true' }, sv('circle', { cx: '6', cy: '7', r: '2.4', fill: 'currentColor' }), sv('circle', { cx: '17', cy: '6', r: '2', fill: 'currentColor', opacity: '0.7' }), sv('circle', { cx: '13', cy: '17', r: '2.2', fill: 'currentColor', opacity: '0.85' }), sv('path', { d: 'M7.8 8.2 11.4 15.4M15.2 7.3 13.7 14.9', stroke: 'currentColor', 'stroke-width': '1.3', 'stroke-linecap': 'round', opacity: '0.5' })), ' 지식 그래프');
}
...
async function renderKnowledgeSpace(view, _space, params) {
    ...
    if (f.catTab === undefined)
        f.catTab = 'auto'; // #657w 카테고리 탭 — 대문(home)|문서(docs). auto=대문 내용 있으면 대문
    ...
    if (params) {
        ...
        if (params.has('tab'))
            f.catTab = params.get('tab') === 'docs' ? 'docs' : 'home'; // #657w 딥링크 복원
        if (params.has('mode'))
    ...
    const selectBtn = el('button', { class: 'btn btn-ghost btn-sm', text: '선택', title: '여러 지식을 골라 한 번에 삭제',
        onclick: () => {
            ...
            if (!sel.mode)
                sel.names.clear();
            if (sel.mode && curCat() && f.catTab !== 'docs')
                setCatTab('docs'); // #657w 선택은 목록 작업 — 문서 탭으로
            refetch(); // 홈(#657h)에서도 전체 목록으로 전환되게 refetch
        } });
    // (#req) 상단 툴바 비움 — 액션 재배치: 지식 그래프·자료·휴지통 → 사이드바 '도구' 섹션(Notion 페이지 트리 아래),
    //  ＋새 페이지·선택 → 홈 '최근 지식' 섹션 헤더. (지식 그래프는 openKnowledgeAtlas=window.open 이라 새 탭에서 열림.)
    const newPageBtn = hasScope('memory') ? el('a', { class: 'btn btn-primary btn-sm', href: '#/knowledge/new',
        title: '새 페이지를 작성합니다 — 노션처럼 바로 타이핑', text: '＋ 새 페이지' }) : null;
    const graphBtn = knGraphBtn();
    graphBtn.classList.add('kn-side-toolitem');
    const sourcesBtn = knSourcesBtn();
    sourcesBtn.classList.add('kn-side-toolitem');
    const trashBtn = el('a', { class: 'kn-side-toolitem', href: '#/trash', title: '삭제한 지식·카테고리 복원', text: '🗑 휴지통' });
    const sideTools = el('div', { class: 'kn-side-tools' }, el('div', { class: 'eyebrow kn-side-tools-eyebrow', text: '도구' }), graphBtn, sourcesBtn, trashBtn);
    ...
    function setCatTab(t) {
        ...
        if (wrap)
            wrap.classList.toggle('cath-mode-docs', t === 'docs');
        catBox.querySelectorAll('.cath-tab-btn').forEach((b) => b.classList.toggle('on', b.dataset.tab === t));
        applyTabVisibility();
        syncHash();
    }
    async function paintHome(quiet = false) {
        ...
        if (mine.length) {
            ...
            if (rest.length) {
                ...
                const moreLabel = el('span', { class: 'kn-home-morecats-label', text: '모든 카테고리 다 보기 (' + rest.length + ')' });
                const moreBtn = el('button', { class: 'kn-home-morecats', type: 'button' }, moreLabel, el('span', { class: 'kn-home-morecats-arrow', 'aria-hidden': 'true', text: '→' }));
                ...
                moreBtn.onclick = () => {
                    ...
                    moreLabel.textContent = open ? '접기' : '모든 카테고리 다 보기 (' + rest.length + ')';
                };
        ...
        const pool = [];
        for (const rows of poolRows)
            for (const r of rows)
                if (!isCategoryHomeDoc(r.name) && !r.is_folder)
                    pool.push(r);
        pool.sort((a, b) => new Date(b.updated_at || 0).getTime() - new Date(a.updated_at || 0).getTime());
        const recent = pool.slice(0, HOME_RECENT_CAP);
        lastEntries = recent;
        const sechead = el('div', { class: 'kn-home-sechead' }, el('span', { class: 'kn-home-sectitle', text: mine.length ? '내 소유 카테고리 최근 지식' : '최근 지식' }), 
        // (#req) ＋새 페이지·선택을 최근 지식 섹션으로 이동(상단 툴바 비움).
        el('div', { class: 'kn-home-sechead-actions' }, newPageBtn, selectBtn, el('button', { class: 'kn-home-all', type: 'button', title: '카테고리 구분 없이 전체 지식을 최신순으로 봅니다',
    ...
    function syncHash() {
        ...
        if (f.indexed)
            ...
            if (f.catTab === 'docs')
                p.set('tab', 'docs'); // #657w 문서 탭 딥링크(대문=기본이라 생략)
        }
        if (f.provenance)
...
function knSideSearchBtn() {
    return el('button', { class: 'kn-side-search', type: 'button', title: '전체 지식 빠른 검색 (⌘K)',
        onclick: () => openQuickSearch() }, el('span', { class: 'kn-side-search-ic', 'aria-hidden': 'true', text: '🔍' }), el('span', { text: '검색' }), el('span', { class: 'kn-side-kbd', text: '⌘K' }));
}
...
async function knSideFilterNav(nav, q) {
    const query = String(q || '').trim().toLowerCase();
    const seq = ++knSideFilterSeq;
    const wraps = Array.from(nav.querySelectorAll('.kn-nav-catwrap'));
    nav.querySelectorAll('.kn-side-hits').forEach((n) => n.remove()); // 이전 검색 결과 행 제거
    const applyGroups = () => {
        ...
        if (query && !anyVis) {
            if (!note)
                nav.append(el('div', { class: 'kn-side-noresult kn-nav-note', text: '일치하는 카테고리·지식이 없습니다' }));
        }
        else if (note)
            note.remove();
    };
    ...
    const onOpen = nav._knOnOpen || ((name) => { location.hash = '#/k/' + encodeURIComponent(name); });
    await Promise.all(wraps.map(async (w) => {
        const catId = w.querySelector('[data-cat-val]')?.dataset?.catVal;
        if (!catId)
            return;
        let rows = [];
        try {
        ...
        if (seq !== knSideFilterSeq)
            return; // 그 사이 검색어가 바뀜 — 이 결과는 폐기
        ...
        if (hits.length > CAP)
            box.append(el('div', { class: 'kn-side-hitmore', text: '＋' + (hits.length - CAP) + '개 더 — 카테고리를 눌러 보기' }));
        w.append(box);
        applyGroups();
    }));
}
...
function knMakeSideSearch(nav, state) {
    const input = el('input', { class: 'pjv-side-search-input', type: 'text', placeholder: '카테고리·지식 검색', 'aria-label': '카테고리·지식 검색', value: state.q || '' });
...
function buildSpacesNav(nav, bySpace, selected, myIds, opts) {
    ...
    for (const sk of ['business', 'product', 'system']) {
        ...
        for (const c of ordered)
            grp.append(knSideItem(c.name || c.key, String(c.id), String(selected) === String(c.id), { star: myIds.has(String(c.id)), cls: 'kn-side-item-sub' }));
        nav.append(grp);
    }
}
// ════════════════════════════════════════════
...
function knTable(entries, open) {
    const titleCell = (e) => el('td', { class: 'kn-table-title' }, el('span', { class: 'row-ic', 'aria-hidden': 'true', text: knPageIcon(e) }), (e.title || e.name));
    const tr = (e) => {
        ...
        if (e.is_folder) {
            ...
            rowEl.append(titleCell(e), el('td', { class: 'kn-table-dim', text: e.type ? (KN_TYPE_LABEL[e.type] || e.type) : '' }), el('td', {}, knAuthorChip(e.confidence) || (e.provenance === 'observed' ? knProvChip(e.provenance) : null)), el('td', { class: 'kn-table-dim' }, e.updated_at ? relTime(e.updated_at) : ''));
        }
...
function knGallery(entries, open) {
    ...
    for (const e of entries) {
        ...
        if (!applyCoverBg(cover, e.cover)) {
            applyCoverBg(cover, defaultCoverFor(e.name));
            cover.classList.add('kn-gcard-cover-dim');
        }
        ...
        const card = el('div', { class: 'kn-gcard', role: 'link', tabindex: '0' }, cover, el('div', { class: 'kn-gcard-body' }, el('div', { class: 'kn-gcard-title', text: e.title || e.name }), preview ? el('div', { class: 'kn-gcard-prev', text: preview }) : null, el('div', { class: 'kn-gcard-meta' }, e.type ? knTypeChip(e.type) : null, el('span', { class: 'kn-gcard-time', text: relTime(e.updated_at) }))));
    ...
    if (!entries.length)
        grid.append(el('div', { class: 'empty', text: '표시할 페이지가 없습니다.' }));
    return grid;
}
...
function knCatGalleryOn(cat) {
    try {
        return localStorage.getItem('kn-cat-gallery-' + cat.id) === '1';
    }
    catch (_) {
        return false;
    }
...
function knCatGallerySet(cat, on) {
    try {
        if (on)
            localStorage.setItem('kn-cat-gallery-' + cat.id, '1');
        else
            localStorage.removeItem('kn-cat-gallery-' + cat.id);
    }
    catch (_) { /* noop */ }
}
...
function openCatViewMenu(anchorBox, btn, cat, onChanged) {
    ...
    const pop = el('div', { class: 'kn-viewpop' }, el('div', { class: 'kn-viewpop-head', text: '카테고리 보기' }), modeRow('list', '리스트', '기존 행 목록(폴더는 📁 드릴다운)'), modeRow('table', '테이블', '제목·유형·작성 주체·갱신 컬럼 테이블'), modeRow('gallery', '갤러리', '커버·아이콘 카드 그리드 — 이 브라우저에서만 저장'), modeRow('entry', '엔트리 문서', '지정한 문서를 본문 영역에 렌더(상단 바로 목록 전환)'), cat.entry_name ? el('div', { class: 'kn-viewpop-cur', text: '엔트리: ' + cat.entry_name }) : null, el('div', { class: 'kn-viewpop-hr' }), entrySet);
...
async function renderKnowledgeDetail(view, name) {
    ...
    const setSide = (off) => {
        ...
        try {
            localStorage.setItem(KN_SIDE_COLLAPSE_KEY, off ? '1' : '0');
        }
        catch (_) { /* noop */ }
    };
    ...
    const knSideState = { q: '' };
    side.append(el('div', { class: 'pjv-side-nav-head' }, el('span', { class: 'pjv-side-nav-head-label', text: '지식 카테고리' }), collapseBtn), knMakeSideSearch(nav, knSideState), nav, mirrorBox);
    side.addEventListener('click', (ev) => {
        const item = ev.target.closest('[data-cat-val]');
        if (!item)
            return;
...
async function renderSources(view, _params) {
    ...
    const head = el('div', { class: 'page-head' }, el('div', { class: 'page-head-row' }, el('h1', { class: 'page-title' }, '자', el('span', { class: 'accent', text: '료' })), el('a', { class: 'btn btn-ghost btn-sm', href: '#/knowledge', text: '← 지식' })), el('p', { class: 'sub', text: '회의록·이메일·슬랙처럼 아직 정리하기 전의 원본입니다. 지식 화면의 보조 입력층이며, 여기서 다듬으면 지식이 됩니다.' }));
...
export { SOURCE_KIND_LABEL, SPACE_LABEL, SPACE_SUBS, buildSpacesNav, fetchAllSpaceCats, knInjectChip, knProvChip, knRow, knSideItem, myCatIdSet, openCategoryForm, renderKnowledge, renderKnowledgeDetail, renderKnowledgeSpace, renderTrash, spaceSubBar, srcRow, };
