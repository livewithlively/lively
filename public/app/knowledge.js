...
function knSideItem(label, catVal, on, opts) {
    ...
    return el('a', { class: 'tree-item' + (on ? ' on' : '') + (opts && opts.cls ? ' ' + opts.cls : '') + (star ? ' kn-cat-mine' : ''),
        ...
        ...(title ? { title } : {}) }, el('span', { class: 'tree-glyph' + (star ? ' tree-glyph-star' : ' all'), 'aria-hidden': 'true', text: glyph }), el('span', { class: 'tree-label', text: label }));
}
// ★ 범례(#req) — 사이드바 하단. '별표가 왜 붙는지' 를 명시(우리 팀 소유 카테고리). 팀 소유가 하나라도 있을 때만.
function knStarLegend(myIds) {
    if (!myIds || !myIds.size)
        return null;
    return el('div', { class: 'kn-star-legend' }, el('span', { class: 'kn-cat-star', 'aria-hidden': 'true', text: '★' }), el('span', { text: '우리 팀이 담당하는 분류' }));
}
...
function buildSpacesNav(nav, bySpace, selected, myIds, opts) {
    ...
    for (const sk of ['business', 'product', 'system']) {
        ...
        const ordered = [...mine, ...rest];
        const grp = el('details', { class: 'tree-group kn-space-group', open: '' }, el('summary', { class: 'tree-grouphead' }, el('span', { class: 'tree-grouptitle', text: SPACE_LABEL[sk] })));
        for (const c of ordered)
            grp.append(knSideItem(c.name || c.key, String(c.id), String(selected) === String(c.id), { star: myIds.has(String(c.id)) }));
        nav.append(grp);
    }
