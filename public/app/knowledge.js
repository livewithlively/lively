...
function knNavDocNode(r, depth, onOpen, childN) {
    ...
    const row = el('div', { class: 'tree-item kn-nav-doc kn-nav-folder', role: 'button', tabindex: '0',
        style: 'padding-left:' + Math.max(4, pad - 16) + 'px', title: r.title || r.name }, tw, el('span', { class: 'tree-glyph kn-nav-glyph', 'aria-hidden': 'true', text: '📁' }), el('span', { class: 'tree-label', text: r.title || r.name }), folderCnt != null ? el('span', { class: 'kn-nav-count' + (folderCnt === 0 ? ' zero' : ''), title: '항목 ' + folderCnt + '개', text: String(folderCnt) }) : null);
    const kids = el('div', { class: 'kn-nav-kids' });
    kids.hidden = true;
    let opened = false, loaded = false;
