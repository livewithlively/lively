...
const bridge = () => {
    ...
    const u = d && d.update;
    return u && typeof u.get === 'function' && typeof u.apply === 'function' ? u : null;
};
/** 이 화면이 데스크톱 앱 안이고, 그 앱이 업데이트를 스스로 받아 두는 판인가. */
export function hasDesktopUpdate() { return !!bridge(); }
const hosts = new Map();
let cur = { ready: false, version: '', busy: false };
 ...
 */
export function mountDesktopUpdate(host, variant = 'bar') {
    const b = bridge();
    if (!b) {
        host.hidden = true;
        return;
    } // 브라우저·구 앱 — 자리만 접어 둔다(레이아웃에서 사라진다)
    // 같은 자리의 옛 등록 중 **문서에 붙지 못한 것**을 먼저 버린다 — 그리다 만 렌더(중간에 갈아엎힌 사이드바)가
    //  남기는 유일한 찌꺼기다. 붙어 있는 것은 건드리지 않는다(그게 지금 보이는 화면이다).
    for (const [h, sl] of hosts)
        if (sl.variant === variant && !h.isConnected)
            hosts.delete(h);
    ...
    if (!wired) {
        ...
        try {
            b.onChange((s) => { cur = norm(s); paint(); });
        }
        catch (_) { /* 다리가 낡았다 — get 만으로 산다 */ }
        void b.get().then((s) => { cur = norm(s); paint(); }).catch(() => { });
    }
    paint();
}
const norm = (s) => ({
...
function render(host, variant) {
    ...
    const btn = el('button', {
        ...
        onclick: () => void apply(),
    }, applying ? '다시 시작하는 중입니다…' : '다시 시작하여 반영하기');
    const why = cur.busy ? '지금 도는 작업이 끝나면 반영할 수 있습니다.' : '';
    if (variant === 'row') {
        // 사이드바 발치 — 폭이 좁아 두 줄로 쌓는다. 문장은 짧게 하되 어미는 남긴다.
        host.replaceChildren(el('div', { class: 'deskup-t' }, el('span', { class: 'deskup-dot', 'aria-hidden': 'true' }), el('span', { text: '업데이트 준비가 완료되었습니다.' })), ...(ver || why ? [el('p', { class: 'deskup-sub', text: why || ver })] : []), btn);
    }
    else {
        host.replaceChildren(el('span', { class: 'deskup-dot', 'aria-hidden': 'true' }), el('span', { class: 'deskup-txt', text: '업데이트 준비가 완료되었습니다. 다시 시작하면 반영됩니다.' }), ...(ver ? [el('span', { class: 'deskup-ver', text: ver })] : []), ...(why ? [el('span', { class: 'deskup-ver', text: why })] : []), btn);
    }
...
async function apply() {
    ...
    try {
        ...
        if (r && r.ok === false) {
            applying = false;
            paint();
            toast(r.error || '업데이트를 적용하지 못했습니다.', true);
        }
    }
    catch (e) {
        applying = false;
