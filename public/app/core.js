...
function renderMarkdown(md) {
    ...
    while (i < lines.length) {
        ...
        if (bullet || ordered) {
            ...
            const indentW = (ws) => { let w = 0; for (const c of ws)
                w += c === '\t' ? 4 : 1; return w; };
            ...
            const stack = [{ width: indentW(first[1]), list: rootList, ordered: !!ordered }];
            while (i < lines.length) {
                let m = bulletRe.exec(lines[i]);
                let isOrdered = false;
                if (!m) {
                    m = orderedRe.exec(lines[i]);
                    if (m)
                        isOrdered = true;
                }
                if (!m) {
                ...
                const w = indentW(m[1]);
                while (stack.length > 1 && w < stack[stack.length - 1].width)
                    stack.pop(); // 얕아짐 → 상위 레벨 복귀
                ...
                if (w > top.width) {
                    ...
                    if (stack.length === 1)
                        break; // 최상위 종류 전환(ul↔ol) → 별도 리스트 블록(기존 동작 보존)
                ...
                if (task) {
                    top.list.append(el('li', { class: 'md-task' }, el('input', { type: 'checkbox', disabled: true, checked: task[1] !== ' ' }), ' ', ...renderInline(task[2] || '')));
                }
                else {
