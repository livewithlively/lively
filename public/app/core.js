...
function renderContainer(type, rest, bodyLines) {
    ...
    switch (type) {
        ...
        case 'axes': {
            // 맥락의 세 축 — 카테고리(분류)를 정점으로, 아래를 '여러 필요 지식 → 프로젝트 → 여러 산출 지식' 흐름으로 편다.
            //  지식(정적)은 흐름 양끝(스택 카드 = 여러 개가 들고난다), 프로젝트(동적)는 가운데. 프로젝트 하나에 여러 지식이
            //  들어가 일하며 여러 지식이 새로 나온다는 걸 시각화(#762). 본문 3줄 `이모지 | 이름 | 부제`.
            ...
            return el('figure', { class: 'md-axes', role: 'group', 'aria-label': '맥락의 세 축' }, apex, el('span', { class: 'md-axes-stem', 'aria-hidden': 'true' }), el('div', { class: 'md-axes-flow' }, fnode(knIc, '필요 지식', '여러 개를 먼저 쥐고 시작 · 선택', 'is-req is-stack'), arrow(), fnode(pjIc, pj[1] || '프로젝트', pj[2] || '동적 · 지금 바꾸는 것', 'is-dynamic'), arrow(), fnode(knIc, '산출 지식', '일이 새로 남기는 지식들 · WIKI에 쌓임', 'is-prod is-stack')));
        }
        case 'synced': {
