...
async function boot() {
    ...
    if (userBtn) {
        ...
        if (!document.querySelector('.topbar-right .theme-seg'))
            userBtn.before(themeControl());
    }
...
async function renderAuthProviders() {
    ...
    try {
        ...
        const oidc = list.find((p) => p && p.kind === 'oidc' && p.enabled);
        if (!oidc)
            return;
        box.textContent = '';
