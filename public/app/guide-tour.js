...
const TOUR_ONLY = (c) => c.key !== 'project-do'; // 'Lively 둘러보기'(랜딩·완주) 대상 = project-do 제외 3섹션
...
const SCENES = [
    ...
    },
    {
        ...
        match: (h) => h.startsWith('#/categories'),
        ...
        build(ctx) {
            ...
            if (q('.wikicat-drift'))
                steps.push({ target: () => q('.wikicat-drift'), title: '정의가 낡았는지 보기',
                    body: el('p', { class: 'tour-p' }, '정의를 마지막으로 고친 날과, ', b('그 뒤 이 갈래에 쌓인 기록 수'), '예요. 숫자가 크면 정의가 현실을 못 따라가고 있다는 신호입니다.') });
            if (q('.wikicat-row-acts'))
                steps.push({ target: () => q('.wikicat-row-acts'), title: '정의 손보기',
            ...
            return [
                {
                    ...
                    placement: 'top', scrollIntoView: true,
                ...
                },
                {
                    ...
                    target: '[data-tour="pd-create-btn"]', placement: 'top',
    ...
    },
    {
        ...
        match: (h) => h.startsWith('#/projects2/p/') && h.indexOf('__demo__') < 0,
        ready: () => !!q('main .page-head'),
        build() {
            const steps = [];
            ...
            if (q('[data-tour="pd-link-kn"]')) {
                steps.push({ target: '[data-tour="pd-link-kn"]', scrollIntoView: true, advanceOn: 'click',
                    title: '① 지식 연결',
                    body: el('p', { class: 'tour-p' }, '이 일에 필요한 회사 지식(WIKI)을 붙여두면 ', b('AI 가 그걸 알고 시작'), '해요. ', b('[＋ 지식 연결]'), ' 을 눌러 보세요.') });
                steps.push({
                    target: () => { const bk = Array.from(document.querySelectorAll('.ov-back')).pop(); return bk ? (bk.querySelector('.ov-box') || bk) : null; },
                    advanceWhen: () => !q('.ov-back'),
                    title: '골라서 [＋ 연결]',
                    body: [
            ...
            if (q('.proj-term-card') && q('[data-tour="proj-new-session"]')) {
                steps.push({ target: '[data-tour="proj-new-session"]', placement: 'left', scrollIntoView: true, advanceOn: 'click',
...
const NAV_TAB_OF = { categories: 'context', domainmap: 'context' };
function sceneNavOn(key) {
    const sc = sceneByKey(key);
    if (!sc)
        return true; // 정의 없는 씬 키(구 플랜 잔재)는 종전처럼 보존 — resume 스캔이 어차피 건너뛴다
