...
async function fillCategorySelect(sel, preselectKey) {
...
export async function renderKnowledgeForm(view, params, editName) {
    ...
    if (isEdit) {
        ...
        if (k.injection === 'always') {
            toast('항상-주입 섹션은 관리 ▸ 세션 주입에서 편집합니다');
            location.hash = '#/system/injection-map';
            return;
        }
    }
    ...
    titleIn.addEventListener('paste', (e) => {
        e.preventDefault();
    ...
    rawToggle.onclick = () => {
        rawMode = !rawMode;
        if (rawMode) {
            rawTa.value = editor.getMarkdown();
            rawTa.hidden = false;
            editor.el.hidden = true;
            rawToggle.textContent = '블록 에디터';
            rawTa.focus();
        }
        else {
            editor.setMarkdown(rawTa.value);
            editor.resetDirty ? null : null;
            rawTa.hidden = true;
            editor.el.hidden = false;
            rawToggle.textContent = 'MD 원문';
        }
    };
    // ── 커버/아이콘 ──
    const coverBox = el('div', { class: 'ke-cover', hidden: true });
    const coverBtns = el('div', { class: 'ke-cover-btns' }, el('button', { class: 'ke-coverbtn', type: 'button', text: '커버 변경',
        onclick: (e) => openCoverPicker(e.target, { current: decor.cover, onPick: (v) => { decor.cover = v || ''; paintDecor(); } }) }), el('button', { class: 'ke-coverbtn', type: 'button', text: '제거', onclick: () => { decor.cover = ''; paintDecor(); } }));
    ...
    function paintDecor() {
        ...
        if (decor.cover)
            applyCoverBg(coverBox, decor.cover);
        iconBtn.hidden = !decor.icon;
        iconBtn.textContent = decor.icon || '';
        iconBtn.className = 'ke-icon' + (decor.cover ? ' ke-icon-oncover' : '');
        decorAdd.replaceChildren(!decor.icon ? el('button', { class: 'ke-decor-btn', type: 'button', text: '☺ 아이콘 추가',
            onclick: (e) => openEmojiPicker(e.target, { onPick: (em) => { decor.icon = em; paintDecor(); } }) }) : null, !decor.cover ? el('button', { class: 'ke-decor-btn', type: 'button', text: '🖼 커버 추가',
            onclick: (e) => openCoverPicker(e.target, { onPick: (v) => { decor.cover = v || ''; paintDecor(); } }) }) : null);
    }
    paintDecor();
    // ── 저장 ──
    ...
    async function save() {
        ...
        if (!body) {
            ...
            (rawMode ? rawTa : editor.el).focus();
            return;
        }
        if (!catSel.value) {
            toast('분류를 선택하세요 (1개 필수)', true);
            catSel.focus();
            return;
        }
        if (!typeSel.value) {
            toast('유형을 선택하세요', true);
            typeSel.focus();
            return;
        }
        ...
        try {
            const payload = { title: title || undefined, body_md: body, provenance: provSel.value, category: catSel.value, type: typeSel.value };
            if (isEdit)
                payload.name = k.name;
            ...
            if (decor.icon !== prevIcon || decor.cover !== prevCover) {
                try {
                    ...
                    toast('아이콘/커버 저장 실패 — ' + e.message, true);
                }
            }
