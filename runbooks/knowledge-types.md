...
**언제 이 종류인가(분류기준).** 강제 규범(R)·절차(H)·작업(W)이 아닌 거의 모든 저작 지식은 K. 배경·맥락·사실·도메인 지식·메모·외부 링크·산출물이 다 K 로 모인다(주제는 종류가 아니라 area=domain 으로 구분, 출처는 provenance 로 구분). 애매하면 K.
...
| K | Knowledge note | 강제 규범(R)·절차(H)·작업(W)이 아닌 거의 모든 저작 지식은 K. 배경·맥락·사실·도메인 지식·메모·외부 링크·산출물이 다 K 로 모인다(주제는 종류가… | 검색 회상(recalled): 인덱스(제목·요약)에 노출, 전문은 일에 맞춰 area+검색으로 그때 소환(on-demand). |
...
| Slack | 수집중(active) | Slack 커넥터가 지정 채널의 메시지/스레드를 활동으로 가져온다. | 동기화(run-sync/backfill) | K |
| Discord | 중단(dropped) | (수집 중단) Discord 커넥터로 채널 메시지를 가져오던 경로. 커넥터 코드는 유지하나 현재 수집하지 않는다. | — | K |
