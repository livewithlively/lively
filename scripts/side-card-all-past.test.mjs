...
//  ★ 6판에서 이 계약이 정밀해졌다(원준 2026-09-12 질문 "x 누르면 어디로 가는 거야?").
//   빼는 이유는 «이미 줄로 섰다» **하나**다 — 「도는 세션이니까」라는 별도 규칙은 없앴다.
//   그 별도 규칙이 있던 동안 **치운 도는 세션**은 줄로도 안 서고(dismissed 가 live 보다 먼저 판정된다)
//   접힘에서도 빠져 **어디에도 없었다.** × 툴팁이 「세션은 그대로 돌고」라고 말하니 그게 × 의 정상 경로다.
lay(projectPastRows([done("끝남", 1, DAY), run("도는중", 1, 0)], 1, new Set(["도는중"]), 12),
  ["끝남"], 1,
  "B1 줄로 선 도는 세션은 접힘에 안 들어온다 — 빼는 이유는 «이미 섰다» 다");

lay(projectPastRows([done("끝남", 1, DAY), run("치운도는세션", 1, 0)], 1, NONE, 12),
  ["치운도는세션", "끝남"], 2,
  "B1′ ★★ 줄로 **안** 선 도는 세션은 접힘이 받는다 — × 로 치운 도는 세션이 갈 곳이 여기다");
