...
ok(/splitSessAxis\(shown, projPinnedId\)/.test(sx) && /text: PINNED_BUCKET/.test(sx) && /kids\.push\(pinCard\(pinRows, o\)\)/.test(sx)
  && /projGroups\(inCards, false, false\)/.test(sx) && /projGrpCard\(g, o, false, standingSess\)/.test(sx) && /kids\.push\(dayCard\(d\.group, d\.rows, o\)\)/.test(sx),
  "W2b ★세션별 축도 같은 「고정」 층(세션 카드 + 고정한 프로젝트 카드, 자리 기억은 안 건드림) · 나머지는 날짜 카드");
ok(/const inCards = projCardRows\(cut\.pinnedProjRows\);/.test(sx), "W2c 세션별 축의 고정 프로젝트 카드에도 그 프로젝트 자신의 화면 줄은 안 넣는다(#3870)");
