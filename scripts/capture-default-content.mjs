...
//  읽고, '코드가 이름으로 전제하는' 지식은 src/org/delivery/seed-knowledge/ 의 각색 스냅샷에서 읽어
//  src/org/delivery/default-content.ts (신규 설치 시드 데이터)로 굳힌다. (#713)
//
...
//    → 시딩본은 고객 맥락으로 각색해 `src/org/delivery/seed-knowledge/<name>.md` 에 손으로 유지하고(SoT), 여기선
...
// 코드가 이름으로 전제하는 지식(댕글링 방지 대상). 근거: src/scheduler/index.ts (런북 2개를 부트스트랩 프롬프트에 주입).
...
const SEED_DIR = path.join(here, "..", "src", "org", "delivery", "seed-knowledge");
...
//   · 지식 본문: \`src/org/delivery/seed-knowledge/<name>.md\` 가 SoT(고객 맥락 각색본). 그 파일을 고친 뒤
