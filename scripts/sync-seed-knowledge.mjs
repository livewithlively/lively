// 시딩 지식 오프라인 sync — `src/org/delivery/seed-knowledge/<name>.md`(각색 SoT)를 편집한 뒤, DB 없이
//  src/org/delivery/default-content.ts 의 DEFAULT_KNOWLEDGE 만 다시 굳힌다. 훅·스킬은 기존 파일 그대로 재사용한다.
//  (전체 재생성=capture-default-content.mjs 는 훅·스킬을 위해 canonical DB 가 필요하다. 지식만 고칠 땐 이걸 써라.)
//
//  실행:  node scripts/sync-seed-knowledge.mjs
//  → seed-knowledge 편집 → 이 스크립트 → `git diff src/org/delivery/default-content.ts` 확인 → 커밋.
