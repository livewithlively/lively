...
const v = await import(path.join(DIST, "org/credentials/member-secret-store.js"));
...
try {
  ...
  await v.setMemberSecret(v.memberOwner("u1"), "gitlab_pat", "git.example.com",
    { secret: "glpat-SECRET-u1", meta: { username: "u1" }, label: "u1 PAT" }, "u1");
  const got = await v.getMemberSecret(v.memberOwner("u1"), "gitlab_pat", "git.example.com");
  ...
  await v.setMemberSecret(v.GATEWAY_OWNER, "gitlab_pat", "git.example.com", { secret: "glpat-GATEWAY" }, "admin");
  const r1 = await v.resolveMemberSecret("u1", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: true });
  if (r1?.secret !== "glpat-SECRET-u1") throw new Error("member 우선 실패 " + r1?.secret);
  ok(step);

  step = "⑤ 해소 체인 — member 없고 allowFallback=true → gateway 통합 폴백";
  const r2 = await v.resolveMemberSecret("u2", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: true });
  ...
  const r3 = await v.resolveMemberSecret("u2", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: false });
  if (r3 !== null) throw new Error("write 폴백 금지 위반 — " + JSON.stringify(r3));
  // 단, member 자격이 있으면 write 경로도 그걸로 해소
  const r3b = await v.resolveMemberSecret("u1", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: false });
  ...
  const r3c = await v.resolveMemberSecret("u2", "gitlab_pat", { scopeKey: "git.example.com" }); // allowFallback 생략
  if (r3c !== null) throw new Error("기본값이 폴백 허용됨(위험) — " + JSON.stringify(r3c));
  ok(step);

  step = "⑦ 멤버 격리 — u1 자격이 u3 에게 안 샘(member 없고 fallback 금지)";
  const r4 = await v.resolveMemberSecret("u3", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: false });
  if (r4 !== null) throw new Error("격리 위반 — u3 가 자격 획득 " + JSON.stringify(r4));
  ok(step);

  step = "⑧ secret 생략 갱신 — meta 만 바꾸고 기존 secret 유지(COALESCE)";
  await v.setMemberSecret(v.memberOwner("u1"), "gitlab_pat", "git.example.com", { meta: { username: "u1-renamed" } }, "u1");
  const g2 = await v.getMemberSecret(v.memberOwner("u1"), "gitlab_pat", "git.example.com");
  ...
  await v.setMemberSecret(v.memberOwner("u1"), "aws_role_arn", "", { meta: { role_arn: "arn:aws:iam::123456789012:role/lively-ro" } }, "u1");
  const aws = await v.resolveMemberSecret("u1", "aws_role_arn", { allowFallback: false });
  if (aws?.secret !== null || aws?.meta?.role_arn !== "arn:aws:iam::123456789012:role/lively-ro") throw new Error("meta-only 실패 " + JSON.stringify(aws));
  ok(step);
  ...
  await v.setMemberSecret(v.GATEWAY_OWNER, "aws_role_arn", "", { meta: { role_arn: "arn:aws:iam::123456789012:role/org-ro", region: "ap-northeast-2" } }, "admin");
  ...
  if (orgOnly?.meta?.role_arn !== "arn:aws:iam::123456789012:role/org-ro") throw new Error("null 해소가 gateway role 이 아님 — " + JSON.stringify(orgOnly?.meta));
  ...
  step = "⑩ 삭제 → 해소 null";
  const del = await v.deleteMemberSecret(v.memberOwner("u1"), "gitlab_pat", "git.example.com");
  if (!del) throw new Error("삭제 실패");
  const after = await v.resolveMemberSecret("u1", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: false });
