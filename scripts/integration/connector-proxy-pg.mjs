...
const { proxyAuthFallback } = await import(path.join(DIST, "mcp/dynamic-tools.js"));
...
try {
  ...
  await store.upsertTool({
    ...
    url: "https://git.example.com/api/v4/merge_requests", method: "GET",
    auth_kind: "gitlab_pat", auth_scope_key: "git.example.com", level: "L1", pii_scrub: true,
    description: "GitLab MR 목록(요청자 PAT)",
  }, { actor: "admin" });
  const t = await store.getTool("gitlab_mr_list");
  if (t.auth_kind !== "gitlab_pat" || t.auth_scope_key !== "git.example.com" || t.level !== "L1" || t.pii_scrub !== true || t.auth_env !== null)
  ...
  await v.setMemberSecret(v.GATEWAY_OWNER, "gitlab_pat", "git.example.com", { secret: "glpat-ORG" }, "admin");
  // u1 개인 자격 없음 → L1 은 통합(gateway) 폴백으로 해소
  const l1 = await v.resolveMemberSecret("u1", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: proxyAuthFallback("L1") });
  if (l1?.secret !== "glpat-ORG") throw new Error("L1 통합 폴백 실패 " + l1?.secret);
  // L2 는 개인 자격 없으면 null(통합 폴백 금지)
  const l2 = await v.resolveMemberSecret("u1", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: proxyAuthFallback("L2") });
  if (l2 !== null) throw new Error("L2 가 통합 폴백함(사칭 위험) " + JSON.stringify(l2));
  // u1 개인 자격 등록 후엔 L2 도 그걸로 해소
  await v.setMemberSecret(v.memberOwner("u1"), "gitlab_pat", "git.example.com", { secret: "glpat-U1" }, "u1");
  const l2b = await v.resolveMemberSecret("u1", "gitlab_pat", { scopeKey: "git.example.com", allowFallback: proxyAuthFallback("L2") });
  ...
  await store.upsertTool({ name: "env2kind", kind: "http_proxy", scope: "items",
    url: "https://prom.example.com/api/v1/query", method: "GET", auth_env: "PROM_TOKEN" }, { actor: "admin" });
