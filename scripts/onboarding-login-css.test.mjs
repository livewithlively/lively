...
test("★ 스테퍼가 쓰는 ob-lg-*·목업 클래스가 스타일시트에 전부 있다", () => {
  ...
  for (const line of SRC.split("\n")) {
    ...
    for (const m of line.matchAll(/["'=\s](ob-(?:lg-[a-z-]+|copychip|bmock|tmock|peek|stuck|bchip|bcap|bbtn|bb-[a-z]+|wsrow|consent|npick|linkline|modal-tr|urlmock|pick))["'\s]/g)) used.add(m[1]);
  }
