...
| ⑤ pg-test | `src/**/*.pg-test.mjs` — **CI 전용** 실 Postgres 통합. 러너 수집 밖이라 test.yml 에 **파일별 스텝으로 등록해야 돈다**(빠뜨리면 `scripts/pg-test-registered.test.mjs` 가 막는다) | CI(test.yml)가 pgvector 서비스로 실행 · 로컬은 `ITEMS_DATABASE_URL=… node <파일>` | 실 PG |
